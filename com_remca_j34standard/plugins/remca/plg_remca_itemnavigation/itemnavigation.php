...
class PlgRemcaItemnavigation extends JPlugin
{
	...
	public function onWa_entry_conversationBeforeDisplay ($context, &$row, &$params, $page=0)
	{
		...
		if ($params->get('show_wa_entry_conversation_navigation') AND 
			($context == 'com_remca.wa_entry_conversation') AND 
			($view == 'wa_entry_conversation')) 
		{
			...
			// Location of current entrada de la conversación whatsapp item in array list.
			$location = array_search($uid, array_keys($list));
			...
			{
				$row->prev = $location -1 ; 
				// The previous entrada de la conversación whatsapp item cannot be in the array position -1.
				for ($i = $location-1; $i >= 0; $i--)
				{
			...
			{
				$row->next = $location +1;
				// The next entrada de la conversación whatsapp item cannot be in an array position greater than the number of array postions.
				for ($i = $location+1; $i <= count($rows)-1; $i++)
				{
					$row->next = $rows[$i];
					break;
			...
			$keep_item_id = (int) $params->get('keep_wa_entry_conversation_itemid', 0);		
					
			if ($row->prev) 
			{
				$row->prev_label =JText::_('JPREV');
				$row->prev = JRoute::_(RemcaHelperRoute::getWa_entry_conversationRoute($row->prev->slug, $row->prev->catid, $layout, $keep_item_id));
			} 
			else 
			{
				$row->prev_label = '';
			...
			if ($row->next) 
			{
				...
				$row->next = JRoute::_(RemcaHelperRoute::getWa_entry_conversationRoute($row->next->slug, $row->next->catid, $layout, $keep_item_id));
			...
			if ($row->prev OR $row->next) 
			{
				...
				$row->pagination = $html;
				$row->paginationposition = $this->params->get('wa_entry_conversation_position', 1);
				// This will default to the 1.5 and 1.6-1.7 behavior.
				$row->paginationrelative = $this->params->get('wa_entry_conversation_relative',0);				
			}
		}
