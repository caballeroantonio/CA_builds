...
/**
 * Methods supporting a list of maincategory records.
 *
 */
class RemcaModelMainCategories extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		// Select the required main_categories from the table.
		$query->select(
			$this->getState(
					'list.select',
					'a.*'
		...
		if ($assoc)
		{
			...
			$query->join('LEFT', $db->quoteName('#__associations').' AS as1 ON '.$db->quoteName('as1.id').' = '.$db->quoteName('a.id').' AND '.$db->quoteName('as1.context').' = '.$db->quote('com_remca.maincategory.item'));
