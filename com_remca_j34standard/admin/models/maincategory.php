 ...
 *
 */
class RemcaModelMainCategory extends JModelAdmin
{
	/**
	...
	/**
	 * @var      string	The type alias for this object (for example, 'com_remca.maincategory')
	 */
	 ...
	 */
	protected $event_before_save = 'onMainCategoryBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onMainCategoryAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onMainCategoryBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onMainCategoryAfterDelete';	
	/**
	 * @var    string	The event to trigger after changing the data's state field.
	 */
	protected $event_change_state = 'onMainCategoryChangeState';	
	...
	public function getItem($pk = null, $recursive = false)
	{
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_remca', '#__rem_main_categories', 'com_remca.maincategory.item', $item->id, 'id', 'alias', null);
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_remca.edit.maincategory.data', array());
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				try
				{
					...
					$query->where('context='.$db->quote('com_remca.maincategory.item'));
				...
				if (!$all_language AND count($associations))
				{
					...
					{
						$query->values((int) $id.','.$db->quote('com_remca.maincategory.item') . ',' . $db->quote($key));
					}
	...
	* @param   array   $data	The data where the original name is stored
	 ...
	 */
	protected function preprocessForm(JForm $form, $data, $group = 'maincategory')
	{
