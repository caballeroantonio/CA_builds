...
class RemcaViewCategory extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		JToolbarHelper::help('JHELP_COMPONENTS_COM_REMCA_CATEGORY_EDIT', true, null, 'com_remca');
		$this->addToolbar();
	...
	protected function addToolbar()
	{
		...
		JToolbarHelper::title(
				JText::_('COM_REMCA_CATEGORIES_' . (isset($checkedOut) && $checkedOut ? 'VIEW_HEADER' : ($is_new ? 'NEW_HEADER' : 'EDIT_HEADER'))), 
				'categories.png'
		);
		...
		if ($this->state->params->get('save_history', 1) AND $this->state->params->get('category_save_history', 1)
			AND !$is_new  
			)
		{
			$item_id = $this->item->id;
			$type_alias = 'com_remca.category';
