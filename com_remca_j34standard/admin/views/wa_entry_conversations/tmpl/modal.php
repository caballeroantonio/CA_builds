...
$function	= $app->input->get('function', 'jSelectWa_entry_conversation');
...
?>
<h3><?php echo JText::_('COM_REMCA_WA_ENTRY_CONVERSATIONS_SELECT_ITEM_LABEL'); ?></h3>
<form action="<?php echo JRoute::_('index.php?option=com_remca&view=wa_entry_conversations&layout=modal&tmpl=component&function='.$function.'&'.JSession::getFormToken().'=1');?>" method="post" name="adminForm" id="adminForm">
	<fieldset class="filter clearfix">
		...
		<div class="filters pull-left">
			<select name="filter_action" class="input-medium js-stools-field-order" onchange="this.form.submit()">
			...
			<select name="filter_created_by" class="input-medium" onchange="this.form.submit()">
				<option value=""><?php echo JText::_('COM_REMCA_SELECT_CREATED_BY');?></option>
	...
	<table class="table table-striped table-condensed">
		...
		<?php foreach ($this->items as $i => $item) : ?>
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td class="center">
					<a class="pointer" href="javascript:void(0)" onclick="if (window.parent) window.parent.<?php echo $this->escape($function);?>('<?php echo $item->id; ?>');">
						<?php echo JHtml::_('jgrid.published', $item->state, $i, 'wa_entry_conversations.', false, 'cb'); ?>
					</a>		
				</td>
				<td class="center">
					<a class="pointer" href="javascript:void(0)" onclick="if (window.parent) window.parent.<?php echo $this->escape($function);?>('<?php echo $item->id; ?>');">
