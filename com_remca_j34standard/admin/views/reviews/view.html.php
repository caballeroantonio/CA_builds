 ...
 *
 */
class RemcaViewReviews extends JViewLegacy
{
	protected $items;
	...
	protected function addToolbar()
	{
		...
		JToolbarHelper::title(JText::_('COM_REMCA_REVIEWS_LIST_HEADER'), 'stack reviews');
		...
		JToolbarHelper::editList('review.edit','JTOOLBAR_EDIT');
			...
			if ($this->state->get('filter.state') != -1 ) 
			{
				...
				else 
				{
					if ($this->state->get('filter.state') == 2) 
					{
						JToolbarHelper::unarchiveList('reviews.publish', 'JTOOLBAR_UNARCHIVE');
					}
				}
			}
		...
		else 
		{
			JToolbarHelper::trash('reviews.trash','JTOOLBAR_TRASH');
                ...
                JToolbarHelper::custom('reviews.export', 'download','download', 'JTOOLBAR_EXPORT', FALSE);
	...
	{	
		JHtmlSidebar::setAction('index.php?option=com_remca&view=reviews');
