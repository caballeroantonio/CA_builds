...
/**
 * Wa_entry_conversation table
 *
 */
class RemcaTableWa_entry_conversations extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('#__rem_wa_entry_conversations', 'id', $db);
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('Wa_entry_conversations','RemcaTable');

		// For all keys
		foreach ($pks as $pk)
		{
			// Load the wa_entry_conversation
			if(!$table->load($pk))
			{
	...
	/**
	 * Stores a Entrada De La Conversación Whatsapp
	 *
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Entrada De La Conversación Whatsapp. A created and created_by field can be set by the user,
