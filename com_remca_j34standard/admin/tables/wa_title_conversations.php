...
/**
 * Wa_title_conversation table
 *
 */
class RemcaTableWa_title_conversations extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('#__rem_wa_title_conversations', 'id', $db);
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('Wa_title_conversations','RemcaTable');

		// For all keys
		foreach ($pks as $pk)
		{
			// Load the wa_title_conversation
			if(!$table->load($pk))
			{
	...
	/**
	 * Stores a Título De La Conversación Whatsapp
	 *
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Título De La Conversación Whatsapp. A created and created_by field can be set by the user,
