...
class JHTMLOrdersDetailIcon
{
	/**
	 * Display an create icon for the ordersdetail.
	 *
	 * @param	Registry   $params	  The ordersdetail parameters
	 * @param   array      $attribs   Optional attributes for the link
	 * @param   boolean    $legacy    True to use legacy images, false to use icomoon based graphic
	 *
	 * @return	string	The HTML for the ordersdetail create icon.
	 * 
	 */
	public static function create($params, $attribs = array(), $legacy = false)
	{
		...
		$url = 'index.php?option=com_remca&task=ordersdetail.add&layout=edit&return='.base64_encode($uri);
		$tmpl = JFactory::getApplication()->input->get('tmpl');
		if($tmpl)
			$url = "{$url}&tmpl={$tmpl}";
		if ($params->get('show_ordersdetail_icons'))
		{
			if ($legacy)
			{
	...
	/**
	 * Display an edit icon for the ordersdetail.
	 *
	 * This icon will not display in a popup window, nor if the orders_detail is trashed.
	 * Edit access checks must be performed in the calling code.
	 *
	 * @param	object		$orders_detail	The ordersdetail in question.
	 * @param	Registry	$params		The ordersdetail parameters
	 * @param	array		$attribs	Not used??
	 * @param   boolean		$legacy   True to use legacy images, false to use icomoon based graphic
	 *
	 * @return	string	The HTML for the ordersdetail edit icon.
	 * 
	 */
	public static function edit($orders_detail, $params, $attribs = array(), $legacy = false)
	{
		...
		$url	= 'index.php?option=com_remca&task=ordersdetail.edit&layout=edit&id='.$orders_detail->id.'&return='.base64_encode($uri);
	...
	/**
	 * Display an delete icon for the ordersdetail.
	 *
	 * This icon will not display in a popup window, nor if the orders_detail is trashed.
	 * Edit access checks must be performed in the calling code.
	 *
	...
	public static function delete($orders_detail, $params, $attribs = array(), $legacy = false)
	{
		...
		$url = 'index.php?option=com_remca&task=ordersdetail.delete&id='.$orders_detail->id.'&'.JSession::getFormToken().'=1'.'&return='.base64_encode($uri);
	...
	/**
	 * Method to generate a link to the email item page for the given ordersdetail
	 *
	 * @param   object     $orders_detail  The ordersdetail information
	...
	public static function email($orders_detail, $params, $attribs = array(), $legacy = false)
	{
		...
		$link	= $base.JRoute::_(RemcaHelperRoute::getOrdersDetailRoute($orders_detail->slug,
									$layout,
									$params->get('keep_ordersdetail_itemid')) , false);
	...
	/**
	 * Method to generate a popup link to print an ordersdetail
	 *
	 * @param   object		$orders_detail  The ordersdetail information
	 * @param   Registry	$params   The item parameters
	 * @param   array		$attribs  Optional attributes for the link
	...
	public static function print_popup($orders_detail, $params, $attribs = array(), $legacy = false)
	{
		...
		$link	= JRoute::_(RemcaHelperRoute::getOrdersDetailRoute($orders_detail->slug,
									$layout,
									$params->get('keep_ordersdetail_itemid')) , false);
	...
	/**
	 * Method to generate a link to print an ordersdetail
	 *
	 * @param   object		$orders_detail  The ordersdetail information
	 * @param   Registry	$params   The item parameters
	 * @param   array		$attribs  Optional attributes for the link
	...
	/**
	 * Method to generate a link to versions an ordersdetail
	 *
	 * @param   object		$ordersdetail  The ordersdetail information
	 * @param   Registry	$params   The item parameters
	 * @param   array		$attribs  Optional attributes for the link
	 * @param   boolean		$legacy   True to use legacy images, false to use icomoon based graphic
	 *
	 * @return  string  The HTML markup for the popup link
	 */
	public static function versions($ordersdetail, $params, $attribs = array(), $legacy = false)
	{
...
//            return "<a href=\"index.php?option=com_remca&task=ordersdetail.showHistory&item_id={$ordersdetail->id}\"  target=\"_blank\" onclick=\"window.open(this.href, this.target, 'width=800,height=600'); return false;\">{$text}</a>";
			return "<a href=\"#\" onclick=\"show_collapsibleModal({$ordersdetail->id});return false;\">{$text}</a>";
	}
}
