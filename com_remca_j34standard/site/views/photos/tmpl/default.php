...
$layout		= $this->params->get('photo_layout', 'default');
...
<div class="remca photos-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_photo_filter_field') != '' AND $this->params->get('show_photo_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_photo_filter_field') != '' AND $this->params->get('show_photo_filter_field') != 'hide') :?>
                <div class="input-append">
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_REMCA_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_REMCA_'.$this->params->get('show_photo_filter_field').'_FILTER_LABEL'); ?>" />
		...
		<?php if ($this->params->get('show_photo_pagination_limit')) : ?>
		...
		<?php if (empty($this->items)) : ?>
			...
			<?php if ($this->params->get('show_no_photos',1)) : ?>
			<p><?php echo JText::_('COM_REMCA_PHOTOS_NO_ITEMS'); ?></p>
			<?php endif; ?>
		...
		<div style="overflow-x:auto;">
			...
			<thead>
				<tr>
					...
					<?php if ($date = $this->params->get('list_show_photo_date')) : ?>
			...
			<tbody>
				...
				?>			
					...
					<?php if ($this->params->get('list_show_photo_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('photo_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_photo_thumbnail_img',1)) : ?>
			...
			<div class="pagination">
				...
				<?php if ($this->params->def('show_photo_pagination_results', 0)) : ?>
				<p class="counter">
		...
		<?php endif; ?>
		<?php // Code to add a link to submit an photo. ?>
		<?php if ($this->params->get('show_photo_add_link', 1)) : ?>
			<?php echo JHtml::_('photoicon.create', $this->params); ?>
		<?php endif; ?>		
		<?php 
			if($user->id == 1){
				//JHtml::_('photoicon.create', $this->params); 
	            echo '<span class="hasTooltip tip" title="Export"><a href="index.php?task=photos.export" class="btn btn-primary"><span class="icon-download"></span>Export</a></span>';
			}
        ?>
	</form>
</div>
