...
$layout		= $this->params->get('main_category_layout', 'default');
...
<div class="remca main_categories-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<?php
		...
		$show_actions = false;
		if ($this->params->get('show_main_category_icons',1) >= 0) :
	...
	<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_main_category_filter_field') != '' AND $this->params->get('show_main_category_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_main_category_filter_field') != '' AND $this->params->get('show_main_category_filter_field') != 'hide') :?>
                <div class="input-append">
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_REMCA_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_REMCA_'.$this->params->get('show_main_category_filter_field').'_FILTER_LABEL'); ?>" />
		...
		<?php if ($this->params->get('show_main_category_pagination_limit')) : ?>
		...
		<div style="overflow-x:auto;">
			<table class="table table-striped" id="main_categories" style="margin-bottom: 200px;">
			<?php if ($this->params->get('show_main_category_headings',1)) :?>
			<thead>
				<tr>
			...
			<tbody>
				...
				?>			
					...
					<?php if (in_array($item->access, $user->getAuthorisedViewLevels()) OR $params->get('show_main_category_noauth')) : ?>
					<td class="list-name">
						<a href="<?php echo JRoute::_(RemcaHelperRoute::getMain_categoryRoute($item->slug, 
																									$item->language,									
																									$layout,									
																									$params->get('keep_main_category_itemid'))); ?>" 
																									>
								<?php echo $this->escape($item->name); ?>
						</a>
					</td>

					<?php if ($this->params->get('list_show_main_category_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('main_category_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_main_category_parent_id',1)) : ?>
					...
					<?php if ($show_actions) : ?>
						<td class="list-actions">
							...
							<?php if ($params->get('show_main_category_print_icon')) : ?>
								<li class="print-icon">
										<?php echo JHtml::_('main_categoryicon.print_popup',  $item, $params); ?>
								</li>
							<?php endif; ?>

							<?php if ($params->get('show_main_category_email_icon')) : ?>
								<li class="email-icon">
										<?php echo JHtml::_('main_categoryicon.email',  $item, $params); ?>
								</li>
							<?php endif; ?>
							<?php if ($can_edit AND $params->get('save_history') AND $params->get('main_category_save_history')) : ?>
								<li class="version-icon">
									<?php echo JHtml::_('main_categoryicon.versions',$item, $params); ?>
								</li>	
							<?php endif; ?>	
                                </ul>
                            </div>
			...
			<?php if (($this->params->def('show_main_category_pagination', 2) == 1  OR ($this->params->get('show_main_category_pagination') == 2)) AND ($this->pagination->get('pages.total') > 1)) : ?>
			<div class="pagination">

				<?php if ($this->params->def('show_main_category_pagination_results', 0)) : ?>
				<p class="counter">
		...
		<?php endif; ?>
		<?php // Code to add a link to submit an main_category. ?>
		<?php if ($this->params->get('show_main_category_add_link', 1)) : ?>
			<?php echo JHtml::_('main_categoryicon.create', $this->params); ?>
		<?php endif; ?>		
		<?php 
			if($user->id == 1){
				//JHtml::_('main_categoryicon.create', $this->params); 
	            echo '<span class="hasTooltip tip" title="Export"><a href="index.php?task=main_categories.export" class="btn btn-primary"><span class="icon-download"></span>Export</a></span>';
			}
        ?>
	</form>
</div>
...
function show_collapsibleModal(item_id){
	...
	modalBody.prepend('<iframe class="iframe" src="index.php?option=com_remca&task=main_category.showHistory&item_id='+item_id+'" name="titulo" height="450"></iframe>');
	return;
}
</script>
