 ...
 *
 */
class RemcaViewWisheslist extends JViewLegacy
{
	protected $items;
	...
	public function display($tpl = null)
	{
		...
		// Compute the wishlist slugs and set the trigger events.
		foreach ($items as $i => &$item)
		{
			...
			$dispatcher->trigger('onWishlistPrepare', array ('com_remca.wishlist', &$item, &$item->params,$i));

			$item->event = new stdClass;


			$results = $dispatcher->trigger('onWishlistBeforeDisplay', array('com_remca.wishlist', &$item, &$item->params, $i));
			$item->event->beforeDisplayWishlist = JString::trim(implode("\n", $results));

			$results = $dispatcher->trigger('onWishlistAfterDisplay', array('com_remca.wishlist', &$item, &$item->params,$i));
		...
		{			
			...
			$num_leading = (int) $params->def('wishlist_num_leading', 1);
			...
			$this->columns = max(1, $params->def('wishlist_num_columns', 1));
			$order = $params->def('wishlist_multi_column_order', 1);
	...
	protected function prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('COM_REMCA_WISHESLIST'));
		}
