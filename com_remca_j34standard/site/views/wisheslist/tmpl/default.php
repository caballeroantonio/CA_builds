...
<div class="remca wisheslist-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<?php
		...
		$show_actions = false;
		if ($this->params->get('show_wishlist_icons',1) >= 0) :
	...
	<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_wishlist_filter_field') != '' AND $this->params->get('show_wishlist_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_wishlist_filter_field') != '' AND $this->params->get('show_wishlist_filter_field') != 'hide') :?>
                <div class="input-append">
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_REMCA_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_REMCA_'.$this->params->get('show_wishlist_filter_field').'_FILTER_LABEL'); ?>" />
		...
		<?php if ($this->params->get('show_wishlist_pagination_limit')) : ?>
		...
		<?php if (empty($this->items)) : ?>
			...
			<?php if ($this->params->get('show_no_wisheslist',1)) : ?>
			<p><?php echo JText::_('COM_REMCA_WISHESLIST_NO_ITEMS'); ?></p>
			<?php endif; ?>

		<?php else : ?>
		<div style="overflow-x:auto;">
			<table class="table table-striped" id="wisheslist" style="margin-bottom: 200px;">
			<?php if ($this->params->get('show_wishlist_headings',1)) :?>
			<thead>
				<tr>
					<th width="1%" style="display:none;">
					</th>				
					<?php if ($date = $this->params->get('list_show_wishlist_date')) : ?>
						<th class="list-date" id="tableOrderingdate">
							<?php echo JHtml::_('grid.sort', 'COM_REMCA_FIELD_'.JString::strtoupper($date).'_LABEL', 'a.'.$date, $list_dirn, $list_order); ?>
						</th>
					<?php endif; ?>

					<?php if ($this->params->get('list_show_wishlist_created_by',0)) : ?>
			...
			<tbody>
				...
				?>			
					...
					<?php if ($this->params->get('list_show_wishlist_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('wishlist_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_wishlist_created_by',0)) : ?>
					...
					<?php if ($this->params->get('list_show_wishlist_id_house',1)) : ?>
						<td class="list-id_house">
							<?php 
								if ($params->get('list_link_wishlist_id_house')) :
					...
					<?php if ($show_actions) : ?>
						<td class="list-actions">
							...
							<?php if ($params->get('show_wishlist_print_icon')) : ?>
								<li class="print-icon">
										<?php echo JHtml::_('wishlisticon.print_popup',  $item, $params); ?>
								</li>
							<?php endif; ?>
			...
			<?php if (($this->params->def('show_wishlist_pagination', 2) == 1  OR ($this->params->get('show_wishlist_pagination') == 2)) AND ($this->pagination->get('pages.total') > 1)) : ?>
			<div class="pagination">

				<?php if ($this->params->def('show_wishlist_pagination_results', 0)) : ?>
				<p class="counter">
...
function show_collapsibleModal(item_id){
	...
	modalBody.prepend('<iframe class="iframe" src="index.php?option=com_remca&task=wishlist.showHistory&item_id='+item_id+'" name="titulo" height="450"></iframe>');
	return;
}
</script>
