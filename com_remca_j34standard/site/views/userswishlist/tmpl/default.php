...
<div class="remca users_wishlist-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		...
		<div style="overflow-x:auto;">
			<table class="table table-striped" id="users_wishlist" style="margin-bottom: 200px;">
			<?php if ($this->params->get('show_userswishlist_headings',1)) :?>
			<thead>
				<tr>
				...
				?>			
					...
					<?php if ($show_actions) : ?>
						<td class="list-actions">
							...
							<?php if ($params->get('show_userswishlist_print_icon')) : ?>
								<li class="print-icon">
										<?php echo JHtml::_('userswishlisticon.print_popup',  $item, $params); ?>
								</li>
							<?php endif; ?>

							<?php if ($params->get('show_userswishlist_email_icon')) : ?>
								<li class="email-icon">
										<?php echo JHtml::_('userswishlisticon.email',  $item, $params); ?>
								</li>
							<?php endif; ?>
							<?php if ($can_edit AND $params->get('save_history') AND $params->get('userswishlist_save_history')) : ?>
								<li class="version-icon">
									<?php echo JHtml::_('userswishlisticon.versions',$item, $params); ?>
								</li>	
							<?php endif; ?>	
                                </ul>
                            </div>
...
function show_collapsibleModal(item_id){
	...
	modalBody.prepend('<iframe class="iframe" src="index.php?option=com_remca&task=userswishlist.showHistory&item_id='+item_id+'" name="titulo" height="450"></iframe>');
	return;
}
</script>
