...
$layout		= $this->params->get('video_source_layout', 'default');
...
<div class="remca video_source-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<?php
		...
		$show_actions = false;
		if ($this->params->get('show_video_source_icons',1) >= 0) :
	...
	<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_video_source_filter_field') != '' AND $this->params->get('show_video_source_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_video_source_filter_field') != '' AND $this->params->get('show_video_source_filter_field') != 'hide') :?>
                <div class="input-append">
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_REMCA_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_REMCA_'.$this->params->get('show_video_source_filter_field').'_FILTER_LABEL'); ?>" />
		...
		<?php if ($this->params->get('show_video_source_pagination_limit')) : ?>
		...
		<div style="overflow-x:auto;">
			...
			<?php if ($this->params->get('show_video_source_headings',1)) :?>
			<thead>
				<tr>
					<th width="1%" style="display:none;">
					</th>				
					<?php if ($date = $this->params->get('list_show_video_source_date')) : ?>
						<th class="list-date" id="tableOrderingdate">
							<?php echo JHtml::_('grid.sort', 'COM_REMCA_FIELD_'.JString::strtoupper($date).'_LABEL', 'a.'.$date, $list_dirn, $list_order); ?>
						</th>
					<?php endif; ?>

					<?php if ($this->params->get('list_show_video_source_id_house',1)) : ?>
						<th class="list-id_house" id="tableOrderingid_house">
							<?php echo JTEXT::_('COM_REMCA_VIDEO_SOURCE_HEADING_ID_HOUSE'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_video_source_sequence_number',1)) : ?>
			...
			<tbody>
				...
				?>			
					...
					<?php if ($this->params->get('list_show_video_source_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('video_source_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_video_source_id_house',1)) : ?>
						<td class="list-id_house">
							<?php 
								if ($params->get('list_link_video_source_id_house')) :
					...
					<?php if ($show_actions) : ?>
						<td class="list-actions">
							...
							<?php if ($params->get('show_video_source_print_icon')) : ?>
								<li class="print-icon">
										<?php echo JHtml::_('video_sourceicon.print_popup',  $item, $params); ?>
								</li>
							<?php endif; ?>

							<?php if ($params->get('show_video_source_email_icon')) : ?>
								<li class="email-icon">
										<?php echo JHtml::_('video_sourceicon.email',  $item, $params); ?>
								</li>
							<?php endif; ?>
							<?php if ($can_edit AND $params->get('save_history') AND $params->get('video_source_save_history')) : ?>
								<li class="version-icon">
									<?php echo JHtml::_('video_sourceicon.versions',$item, $params); ?>
								</li>	
							<?php endif; ?>	
                                </ul>
                            </div>
			...
			<?php if (($this->params->def('show_video_source_pagination', 2) == 1  OR ($this->params->get('show_video_source_pagination') == 2)) AND ($this->pagination->get('pages.total') > 1)) : ?>
			<div class="pagination">

				<?php if ($this->params->def('show_video_source_pagination_results', 0)) : ?>
				<p class="counter">
		...
		<?php endif; ?>
		<?php // Code to add a link to submit an video_source. ?>
		<?php if ($this->params->get('show_video_source_add_link', 1)) : ?>
			<?php echo JHtml::_('video_sourceicon.create', $this->params); ?>
		<?php endif; ?>		
		<?php 
			if($user->id == 1){
				//JHtml::_('video_sourceicon.create', $this->params); 
	            echo '<span class="hasTooltip tip" title="Export"><a href="index.php?task=video_source.export" class="btn btn-primary"><span class="icon-download"></span>Export</a></span>';
			}
        ?>
	</form>
</div>
...
function show_collapsibleModal(item_id){
	...
	modalBody.prepend('<iframe class="iframe" src="index.php?option=com_remca&task=video_source.showHistory&item_id='+item_id+'" name="titulo" height="450"></iframe>');
	return;
}
</script>
