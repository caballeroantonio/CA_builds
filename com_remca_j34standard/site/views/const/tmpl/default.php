...
<div class="remca const-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		...
		<div style="overflow-x:auto;">
			<table class="table table-striped" id="const" style="margin-bottom: 200px;">
			<?php if ($this->params->get('show_const_headings',1)) :?>
			<thead>
				<tr>
					...
					<?php if ($this->params->get('list_show_const_const',1)) : ?>
						<th class="list-const" id="tableOrderingconst">
							<?php echo JTEXT::_('COM_REMCA_CONST_HEADING_CONST'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_const_sys_type',1)) : ?>
						<th class="list-sys_type" id="tableOrderingsys_type">
							<?php echo JTEXT::_('COM_REMCA_CONST_HEADING_SYS_TYPE'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($show_actions) : ?>
				...
				?>			
					...
					<?php if ($show_actions) : ?>
						<td class="list-actions">
							...
							<?php if ($params->get('show_const_print_icon')) : ?>
								<li class="print-icon">
										<?php echo JHtml::_('consticon.print_popup',  $item, $params); ?>
								</li>
							<?php endif; ?>

							<?php if ($params->get('show_const_email_icon')) : ?>
								<li class="email-icon">
										<?php echo JHtml::_('consticon.email',  $item, $params); ?>
								</li>
							<?php endif; ?>
							<?php if ($can_edit AND $params->get('save_history') AND $params->get('const_save_history')) : ?>
								<li class="version-icon">
									<?php echo JHtml::_('consticon.versions',$item, $params); ?>
								</li>	
							<?php endif; ?>	
                                </ul>
                            </div>
		...
		<?php 
			if($user->id == 1){
				//JHtml::_('consticon.create', $this->params); 
	            echo '<span class="hasTooltip tip" title="Export"><a href="index.php?task=const.export" class="btn btn-primary"><span class="icon-download"></span>Export</a></span>';
			}
        ?>
	</form>
</div>
...
function show_collapsibleModal(item_id){
	...
	modalBody.prepend('<iframe class="iframe" src="index.php?option=com_remca&task=const.showHistory&item_id='+item_id+'" name="titulo" height="450"></iframe>');
	return;
}
</script>
