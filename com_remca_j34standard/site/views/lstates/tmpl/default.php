...
$layout		= $this->params->get('lstate_layout', 'default');
...
<div class="remca lstates-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<?php
		...
		$show_actions = false;
		if ($this->params->get('show_lstate_icons',1) >= 0) :
	...
	<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_lstate_filter_field') != '' AND $this->params->get('show_lstate_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_lstate_filter_field') != '' AND $this->params->get('show_lstate_filter_field') != 'hide') :?>
                <div class="input-append">
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_REMCA_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_REMCA_'.$this->params->get('show_lstate_filter_field').'_FILTER_LABEL'); ?>" />
				...
				<?php endif; ?>	
				<?php if ($this->params->get('list_show_lstate_id_country',1)) : ?>
					<select name="filter_id_country" onchange="this.form.submit()">
					<option value=""><?php echo JText::_('COM_REMCA_LSTATES_SELECT_C1_COUNTRY');?></option>
		...
		<?php if ($this->params->get('show_lstate_pagination_limit')) : ?>
		...
		<?php if (empty($this->items)) : ?>
			...
			<?php if ($this->params->get('show_no_lstates',1)) : ?>
			<p><?php echo JText::_('COM_REMCA_LSTATES_NO_ITEMS'); ?></p>
			<?php endif; ?>
		...
		<div style="overflow-x:auto;">
			...
			<thead>
				<tr>
					...
					<?php if ($date = $this->params->get('list_show_lstate_date')) : ?>
			...
			<tbody>
				...
				?>			
					...
					<td class="list-name">
						<a href="<?php echo JRoute::_(RemcaHelperRoute::getLstateRoute($item->slug, 
																									$layout,									
																									$params->get('keep_lstate_itemid'))); ?>" 
																									>
								<?php echo $this->escape($item->name); ?>
						</a>
					</td>

					<?php if ($this->params->get('list_show_lstate_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('lstate_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_lstate_id_country',1)) : ?>
						<td class="list-id_country">
							<?php 
								if ($params->get('list_link_lstate_id_country')) :
					...
					<?php if ($show_actions) : ?>
						<td class="list-actions">
							...
							<?php if ($params->get('show_lstate_print_icon')) : ?>
								<li class="print-icon">
										<?php echo JHtml::_('lstateicon.print_popup',  $item, $params); ?>
								</li>
							<?php endif; ?>

							<?php if ($params->get('show_lstate_email_icon')) : ?>
								<li class="email-icon">
										<?php echo JHtml::_('lstateicon.email',  $item, $params); ?>
								</li>
							<?php endif; ?>
                                </ul>
                            </div>
			...
			<?php if (($this->params->def('show_lstate_pagination', 2) == 1  OR ($this->params->get('show_lstate_pagination') == 2)) AND ($this->pagination->get('pages.total') > 1)) : ?>
			<div class="pagination">

				<?php if ($this->params->def('show_lstate_pagination_results', 0)) : ?>
				<p class="counter">
		...
		<?php endif; ?>
		<?php // Code to add a link to submit an lstate. ?>
		<?php if ($this->params->get('show_lstate_add_link', 1)) : ?>
			<?php echo JHtml::_('lstateicon.create', $this->params); ?>
		<?php endif; ?>		
		<?php 
			if($user->id == 1){
				//JHtml::_('lstateicon.create', $this->params); 
	            echo '<span class="hasTooltip tip" title="Export"><a href="index.php?task=lstates.export" class="btn btn-primary"><span class="icon-download"></span>Export</a></span>';
			}
        ?>
	</form>
</div>

<?php if ($can_edit AND $params->get('save_history') AND $params->get('lstate_save_history')) : ?>
<script>
...
function show_collapsibleModal(item_id){
	...
	modalBody.prepend('<iframe class="iframe" src="index.php?option=com_remca&task=lstate.showHistory&item_id='+item_id+'" name="titulo" height="450"></iframe>');
	return;
}
</script>
