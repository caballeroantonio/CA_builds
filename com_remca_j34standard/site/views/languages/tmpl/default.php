...
$this->document->addStyleSheet(JUri::root().'media/com_remca/css/site_languages.css');

if ($lang->isRTL())
{
	$this->document->addStyleSheet(JUri::root().'media/com_remca/css/site_remca-rtl.css');
	$this->document->addStyleSheet(JUri::root().'media/com_remca/css/site_languages-rtl.css');
}
...
$layout		= $this->params->get('language_layout', 'default');
...
<div class="remca languages-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<?php
		...
		$show_actions = false;
		if ($this->params->get('show_language_icons',1) >= 0) :
	...
	<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_language_filter_field') != '' AND $this->params->get('show_language_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_language_filter_field') != '' AND $this->params->get('show_language_filter_field') != 'hide') :?>
                <div class="input-append">
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_REMCA_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_REMCA_'.$this->params->get('show_language_filter_field').'_FILTER_LABEL'); ?>" />
		...
		<?php if ($this->params->get('show_language_pagination_limit')) : ?>
		...
		<?php if (empty($this->items)) : ?>
			...
			<?php if ($this->params->get('show_no_languages',1)) : ?>
			<p><?php echo JText::_('COM_REMCA_LANGUAGES_NO_ITEMS'); ?></p>
			<?php endif; ?>

		<?php else : ?>
		<div style="overflow-x:auto;">
			<table class="table table-striped" id="languages" style="margin-bottom: 200px;">
					...
					<?php if ($this->params->get('list_show_language_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('language_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_language_lang_code',1)) : ?>
			...
			<?php if (($this->params->def('show_language_pagination', 2) == 1  OR ($this->params->get('show_language_pagination') == 2)) AND ($this->pagination->get('pages.total') > 1)) : ?>
			<div class="pagination">

				<?php if ($this->params->def('show_language_pagination_results', 0)) : ?>
				<p class="counter">
		...
		<?php endif; ?>
		<?php // Code to add a link to submit an language. ?>
		<?php if ($this->params->get('show_language_add_link', 1)) : ?>
			<?php echo JHtml::_('languageicon.create', $this->params); ?>
		<?php endif; ?>		
		<?php 
			if($user->id == 1){
				//JHtml::_('languageicon.create', $this->params); 
	            echo '<span class="hasTooltip tip" title="Export"><a href="index.php?task=languages.export" class="btn btn-primary"><span class="icon-download"></span>Export</a></span>';
			}
        ?>
	</form>
</div>
