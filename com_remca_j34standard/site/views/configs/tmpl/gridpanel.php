...
function getFields(){
    ...
    $field = array(
        ...
        'FIELD_CODE_NAME' => 'gmaps_api_key',
        'FIELD_DESCRIPTION' => '',//
        'FIELDTYPE_ID' => 1,
    );
    /*
                FIELD_OPTIONS_LANGUAGE_VARS=
                FIELD_CODE_NAME_UPPER=GMAPS_API_KEY
                FIELD_INTRO=
                FIELD_DESCRIPTION_INI=

                FIELD_DB=`` VARCHAR(255) NOT NULL DEFAULT '' Google maps api key

                FIELD_NAME_LATEX=Google maps api key
                FIELD_CODE_NAME_LATEX=gmaps\_api\_key
                FIELD_DBCOMMENT_LATEX=


    */
    $fields['gmaps_api_key'] = $field;
    return $fields;
}
...
<?php /*?>
<?php
if ($this->params->get('save_history') AND $this->params->get('config_save_history'))
{
//	JHtml::_('behavior.modal', 'a.modal_jform_contenthistory');//creo que se carga con el botón versiones
	...
	$model	= JModelLegacy::getInstance('ConfigForm','RemcaModel', array('ignore_request' => FALSE));
	$data = array();
	$data['id'] = 1;
	...
	echo $this->form->getInput('contenthistory');
				...
				echo JHtml::_(
					...
					array(
						'title'  => 'titulo',
						'height' => 450,
						'url' => new JUri('index.php?option=com_remca&view=configform&layout=edit&layout=edit&tmpl=component&id=1&function=on_collapseModal')
					)
				);
}
