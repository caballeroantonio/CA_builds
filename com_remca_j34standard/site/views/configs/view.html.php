 ...
 *
 */
class RemcaViewConfigs extends JViewLegacy
{
	protected $items;
	...
	public function display($tpl = null)
	{
		...
		foreach ($items as $i => &$item)
		{
			...
			$dispatcher->trigger('onConfigPrepare', array ('com_remca.config', &$item, &$item->params,$i));

			$item->event = new stdClass;


			$results = $dispatcher->trigger('onConfigBeforeDisplay', array('com_remca.config', &$item, &$item->params, $i));
			$item->event->beforeDisplayConfig = JString::trim(implode("\n", $results));

			$results = $dispatcher->trigger('onConfigAfterDisplay', array('com_remca.config', &$item, &$item->params,$i));
	...
	protected function prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('COM_REMCA_CONFIGS'));
		}
