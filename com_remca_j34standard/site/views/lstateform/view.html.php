...
class RemcaViewLstateForm extends JViewLegacy
{
	...
	protected function prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('COM_REMCA_LSTATES_EDIT_ITEM'));
		}

		if (!is_null($this->item->id))
		{
			$title = $this->params->def('page_title', JText::_('COM_REMCA_LSTATES_EDIT_ITEM'));
			
			$pathway->addItem(JText::sprintf('COM_REMCA_EDIT_ITEM', $this->escape($this->item->name)),''); 
		}
		else
		{
			$title = $this->params->def('page_title', JText::_('COM_REMCA_LSTATES_CREATE_ITEM'));
			
			$pathway->addItem(JText::_('COM_REMCA_LSTATES_CREATE_ITEM'),'');
		}
		...
		{
			$lstate->name = $lstate->name .' - '. $lstate->page_title;
			$this->document->setTitle($lstate->page_title.' - '.JText::sprintf('COM_REMCA_PAGEBREAK_PAGE_NUM', $this->state->get('page.offset') + 1));
		}
