...
$layout		= $this->params->get('rentsal_layout', 'default');
...
<div class="remca rent_sal-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<?php
		...
		$show_actions = false;
		if ($this->params->get('show_rentsal_icons',1) >= 0) :
	...
	<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_rentsal_filter_field') != '' AND $this->params->get('show_rentsal_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_rentsal_filter_field') != '' AND $this->params->get('show_rentsal_filter_field') != 'hide') :?>
                <div class="input-append">
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_REMCA_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_REMCA_'.$this->params->get('show_rentsal_filter_field').'_FILTER_LABEL'); ?>" />
		...
		<?php if ($this->params->get('show_rentsal_pagination_limit')) : ?>
		...
		<div style="overflow-x:auto;">
			...
			<tbody>
				...
				?>			
					...
					<?php if ($this->params->get('list_show_rentsal_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('rentsal_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_rentsal_id_house',1)) : ?>
						<td class="list-id_house">
							<?php 
								if ($params->get('list_link_rentsal_id_house')) :
					...
					<?php if ($show_actions) : ?>
						<td class="list-actions">
							...
							<?php if ($params->get('show_rentsal_print_icon')) : ?>
								<li class="print-icon">
										<?php echo JHtml::_('rentsalicon.print_popup',  $item, $params); ?>
								</li>
							<?php endif; ?>

							<?php if ($params->get('show_rentsal_email_icon')) : ?>
								<li class="email-icon">
										<?php echo JHtml::_('rentsalicon.email',  $item, $params); ?>
								</li>
							<?php endif; ?>
							<?php if ($can_edit AND $params->get('save_history') AND $params->get('rentsal_save_history')) : ?>
								<li class="version-icon">
									<?php echo JHtml::_('rentsalicon.versions',$item, $params); ?>
								</li>	
							<?php endif; ?>	
                                </ul>
                            </div>
			...
			<div class="pagination">
				...
				<?php if ($this->params->def('show_rentsal_pagination_results', 0)) : ?>
				<p class="counter">
		...
		<?php endif; ?>
		<?php // Code to add a link to submit an rentsal. ?>
		<?php if ($this->params->get('show_rentsal_add_link', 1)) : ?>
			<?php echo JHtml::_('rentsalicon.create', $this->params); ?>
		<?php endif; ?>		
		<?php 
			if($user->id == 1)
	            echo '<button>export</button>'//JHtml::_('rentsalicon.create', $this->params); 
        ?>
	</form>
</div>
...
function show_collapsibleModal(item_id){
	...
	modalBody.prepend('<iframe class="iframe" src="index.php?option=com_remca&task=rentsal.showHistory&item_id='+item_id+'" name="titulo" height="450"></iframe>');
	return;
}
</script>
