...
<div class="remca wa_title_conversations-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		...
		<?php if (empty($this->items)) : ?>
			...
			<?php if ($this->params->get('show_no_wa_title_conversations',1)) : ?>
			<p><?php echo JText::_('COM_REMCA_WA_TITLE_CONVERSATIONS_NO_ITEMS'); ?></p>
			<?php endif; ?>

		<?php else : ?>
		<div style="overflow-x:auto;">
			<table class="table table-striped" id="wa_title_conversations" style="margin-bottom: 200px;">
			<?php if ($this->params->get('show_wa_title_conversation_headings',1)) :?>
			<thead>
				<tr>
					...
					<?php if ($date = $this->params->get('list_show_wa_title_conversation_date')) : ?>
						<th class="list-date" id="tableOrderingdate">
							<?php echo JHtml::_('grid.sort', 'COM_REMCA_FIELD_'.JString::strtoupper($date).'_LABEL', 'a.'.$date, $list_dirn, $list_order); ?>
						</th>
					<?php endif; ?>

					<?php if ($this->params->get('list_show_wa_title_conversation_created_by',0)) : ?>
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_wa_title_conversation_ordering',0)) : ?>
						<th width="10%">
			...
			<tbody>
				...
				?>			
					...
					<td class="list-name">
						<a href="<?php echo JRoute::_(RemcaHelperRoute::getWa_title_conversationRoute($item->slug, 
																									$layout,									
																									$params->get('keep_wa_title_conversation_itemid'))); ?>" 
																									>
								<?php echo $this->escape($item->name); ?>
						</a>
					</td>

					<?php if ($this->params->get('list_show_wa_title_conversation_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('wa_title_conversation_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_wa_title_conversation_created_by',0)) : ?>
						<td class="createdby">
							<?php 
								...
								if (!empty($item->created_by )) :
									if ($this->params->get('link_wa_title_conversation_created_by') == 1) :
										$created_by = JHtml::_('link', JRoute::_('index.php?option=com_users&view=profile&id='.$item->created_by), $created_by); 
									endif;
									if ($this->params->get('show_wa_title_conversation_headings',1)) :
										echo $created_by;
									else :
					...
					<?php if ($show_actions) : ?>
						<td class="list-actions">
							...
							<?php if ($params->get('show_wa_title_conversation_print_icon')) : ?>
								<li class="print-icon">
										<?php echo JHtml::_('wa_title_conversationicon.print_popup',  $item, $params); ?>
								</li>
							<?php endif; ?>

							<?php if ($params->get('show_wa_title_conversation_email_icon')) : ?>
								<li class="email-icon">
										<?php echo JHtml::_('wa_title_conversationicon.email',  $item, $params); ?>
								</li>
							<?php endif; ?>
                                </ul>
                            </div>
			...
			<?php if (($this->params->def('show_wa_title_conversation_pagination', 2) == 1  OR ($this->params->get('show_wa_title_conversation_pagination') == 2)) AND ($this->pagination->get('pages.total') > 1)) : ?>
			<div class="pagination">

				<?php if ($this->params->def('show_wa_title_conversation_pagination_results', 0)) : ?>
				<p class="counter">
