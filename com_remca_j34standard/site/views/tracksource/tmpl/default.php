...
$layout		= $this->params->get('tracksource_layout', 'default');
...
<div class="remca track_source-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_tracksource_filter_field') != '' AND $this->params->get('show_tracksource_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_tracksource_filter_field') != '' AND $this->params->get('show_tracksource_filter_field') != 'hide') :?>
                <div class="input-append">
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_REMCA_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_REMCA_'.$this->params->get('show_tracksource_filter_field').'_FILTER_LABEL'); ?>" />
		...
		<?php if ($this->params->get('show_tracksource_pagination_limit')) : ?>
		...
		<div style="overflow-x:auto;">
			...
			<thead>
				<tr>
					...
					<?php if ($date = $this->params->get('list_show_tracksource_date')) : ?>
			...
			<tbody>
				...
				?>			
					...
					<?php if ($this->params->get('list_show_tracksource_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('tracksource_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_tracksource_id_house',1)) : ?>
			...
			<?php if (($this->params->def('show_tracksource_pagination', 2) == 1  OR ($this->params->get('show_tracksource_pagination') == 2)) AND ($this->pagination->get('pages.total') > 1)) : ?>
			<div class="pagination">

				<?php if ($this->params->def('show_tracksource_pagination_results', 0)) : ?>
				<p class="counter">
		...
		<?php endif; ?>
		<?php // Code to add a link to submit an tracksource. ?>
		<?php if ($this->params->get('show_tracksource_add_link', 1)) : ?>
			<?php echo JHtml::_('tracksourceicon.create', $this->params); ?>
		<?php endif; ?>		
		<?php 
			if($user->id == 1){
				//JHtml::_('tracksourceicon.create', $this->params); 
