...
class RemcaViewWa_entry_conversationForm extends JViewLegacy
{
	...
	protected function prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('COM_REMCA_WA_ENTRY_CONVERSATIONS_EDIT_ITEM'));
		}

		if (!is_null($this->item->id))
		{
			$title = $this->params->def('page_title', JText::_('COM_REMCA_WA_ENTRY_CONVERSATIONS_EDIT_ITEM'));
			
			$pathway->addItem(JText::sprintf('COM_REMCA_EDIT_ITEM', JText::sprintf('COM_REMCA_WA_ENTRY_CONVERSATIONS_ID_TITLE', $this->item->id)),''); 
		}
		else
		{
			$title = $this->params->def('page_title', JText::_('COM_REMCA_WA_ENTRY_CONVERSATIONS_CREATE_ITEM'));
			
			$pathway->addItem(JText::_('COM_REMCA_WA_ENTRY_CONVERSATIONS_CREATE_ITEM'),'');
		}
		...
		{
			$this->document->setTitle($wa_entry_conversation->page_title.' - '.JText::sprintf('COM_REMCA_PAGEBREAK_PAGE_NUM', $this->state->get('page.offset') + 1));
		}
