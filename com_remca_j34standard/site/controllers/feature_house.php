...
/**
 * Feature_house controller class.
 * 
 */
class RemcaControllerFeature_house extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'feature_houseform';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'feature_houses';
	/**
	 * 
	 ...
	 * 
	 */
	public function getModel($name = 'feature_houseform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Feature_house can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('feature_houseform');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_REMCA_FEATURE_HOUSES_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_REMCA_FEATURE_HOUSES_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('feature_house.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		// Get the menu item model.
		$model = $this->getModel('feature_house');
		...
		if ($id > 0) 
		{
			if($model->delete($id))
			{
				$this->setMessage(JText::_('COM_REMCA_FEATURE_HOUSES_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_REMCA_FEATURE_HOUSES_DELETE_FAILED'));
			}
		}
