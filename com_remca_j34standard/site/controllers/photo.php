...
class RemcaControllerPhoto extends JControllerForm
{
	 ...
	 */
	protected $view_item = 'photoform';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'photos';
	/**
	 * 
	 ...
	 * 
	 */
	public function getModel($name = 'photoform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Photo can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_REMCA_PHOTOS_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_REMCA_PHOTOS_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('photo.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			if($model->delete($id))
			{
				$this->setMessage(JText::_('COM_REMCA_PHOTOS_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_REMCA_PHOTOS_DELETE_FAILED'));
			}
		}
