...
class RemcaControllerOrder extends JControllerForm
{
	 ...
	 * 
	 */
	public function getModel($name = 'orderform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Order can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_REMCA_ORDERS_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_REMCA_ORDERS_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('order.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			if($model->delete($id))
			{
				$this->setMessage(JText::_('COM_REMCA_ORDERS_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_REMCA_ORDERS_DELETE_FAILED'));
			}
		}
