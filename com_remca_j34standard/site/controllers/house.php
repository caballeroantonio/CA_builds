 ...
 * 
 */
class RemcaControllerHouse extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'houseform';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'houses';
	/**
	 * 
	 ...
	 * 
	 */
	public function getModel($name = 'houseform', $prefix = '',$config = array('ignore_request' => true))
	{
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_REMCA_HOUSES_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_REMCA_HOUSES_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('house.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_REMCA_HOUSES_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_REMCA_HOUSES_DELETE_FAILED'));
			}
		}
	...
	public function vote()
	{
		...
		if ( $user_rating > -1 )
		{
			...
			{
				$this->setRedirect($url, JText::_('COM_REMCA_HOUSES_VOTE_SUCCESS'));
			}
			else
			{
				$this->setRedirect($url, JText::_('COM_REMCA_HOUSES_VOTE_FAILURE'));
			}
		}
	}
	/**
