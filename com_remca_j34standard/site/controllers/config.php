 ...
 * 
 */
class RemcaControllerConfig extends JControllerForm
{
	/**
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_REMCA_CONFIGS_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_REMCA_CONFIGS_SAVE_SUCCESS'));
		}
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			if($model->delete($id))
			{
				$this->setMessage(JText::_('COM_REMCA_CONFIGS_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_REMCA_CONFIGS_DELETE_FAILED'));
			}
		}
