...
class RemcaControllerWa_entry_conversation extends JControllerForm
{
	 ...
	 */
	protected $view_item = 'wa_entry_conversationform';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'wa_entry_conversations';
	/**
	 * 
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('wa_entry_conversationform')->getItem($record_id);
	...
	protected function allowDelete($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.delete.own', $asset))
		{
			...
			{
				// Need to do a lookup from the model.
				$table = $this->getModel('wa_entry_conversation')->getTable();
	 ...
	 * 
	 */
	public function getModel($name = 'wa_entry_conversationform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Entrada De La Conversación Whatsapp can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('wa_entry_conversationform');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_REMCA_WA_ENTRY_CONVERSATIONS_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_REMCA_WA_ENTRY_CONVERSATIONS_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('wa_entry_conversation.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		// Get the menu item model.
		$model = $this->getModel('wa_entry_conversation');
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_REMCA_WA_ENTRY_CONVERSATIONS_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_REMCA_WA_ENTRY_CONVERSATIONS_DELETE_FAILED'));
			}
		}
