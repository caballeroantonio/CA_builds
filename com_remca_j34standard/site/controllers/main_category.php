...
/**
 * Main_category controller class.
 * 
 */
class RemcaControllerMain_category extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'main_categoryform';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'main_categories';
	/**
	 * 
	 ...
	 * 
	 */
	public function getModel($name = 'main_categoryform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Main_category can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('main_categoryform');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_REMCA_MAIN_CATEGORIES_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_REMCA_MAIN_CATEGORIES_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('main_category.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		// Get the menu item model.
		$model = $this->getModel('main_category');
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_REMCA_MAIN_CATEGORIES_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_REMCA_MAIN_CATEGORIES_DELETE_FAILED'));
			}
		}
