...
class RemcaControllerWa_title_conversation extends JControllerForm
{
	 ...
	 */
	protected $view_item = 'wa_title_conversationform';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'wa_title_conversations';
	/**
	 * 
	 ...
	 * 
	 */
	public function getModel($name = 'wa_title_conversationform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Título De La Conversación Whatsapp can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('wa_title_conversationform');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_REMCA_WA_TITLE_CONVERSATIONS_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_REMCA_WA_TITLE_CONVERSATIONS_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('wa_title_conversation.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		// Get the menu item model.
		$model = $this->getModel('wa_title_conversation');
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_REMCA_WA_TITLE_CONVERSATIONS_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_REMCA_WA_TITLE_CONVERSATIONS_DELETE_FAILED'));
			}
		}
