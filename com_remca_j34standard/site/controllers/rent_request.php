...
/**
 * Rent_request controller class.
 * 
 */
class RemcaControllerRent_request extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'rent_requestform';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'rent_requests';
	/**
	 * 
	 ...
	 * 
	 */
	public function getModel($name = 'rent_requestform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Rent Request can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('rent_requestform');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_REMCA_RENT_REQUESTS_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_REMCA_RENT_REQUESTS_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('rent_request.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		// Get the menu item model.
		$model = $this->getModel('rent_request');
		...
		if ($id > 0) 
		{
			if($model->delete($id))
			{
				$this->setMessage(JText::_('COM_REMCA_RENT_REQUESTS_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_REMCA_RENT_REQUESTS_DELETE_FAILED'));
			}
		}
