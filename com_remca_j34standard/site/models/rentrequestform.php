...
require_once JPATH_ADMINISTRATOR.'/components/com_remca/models/rentrequest.php';

class RemcaModelRentRequestForm extends RemcaModelRentRequest
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.edit.rentrequest';
	/**
	...
	*/
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('rentrequest.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_remca.edit.rentrequest.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('rentrequest.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onRentRequestBeforeSave', array('com_remca.rentrequest', &$table, $is_new));
		...
		$dispatcher->trigger('onRentRequestAfterSave', array('com_remca.rentrequest', $table, $is_new));

		$this->setState('rentrequest.id', $table->id);
