...
class RemcaModelLanguage extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.language';
	...
	{
		if (empty($config['language_filter_fields']))
		{
	...
	*/
	 ...
	 *
	 * @param	integer	$pk	The id of the language.
	 *
	...
	public function getItem($pk = null)
	{
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_REMCA_LANGUAGES_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function delete(&$pks)
	{
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
					...
					$result = $dispatcher->trigger('onLanguageBeforeDelete', array('com_remca.language', &$table));
					...
					$dispatcher->trigger('onLanguageAfterDelete', array('com_remca.language', &$table));
			}
			else
			{
				$this->setError($table->getError());
