 ...
 *
 */
class RemcaModelFeatureHouses extends JModelList
{
	/**
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('featurehouse_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('featurehouse_initial_sort','a.id'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('featurehouse_initial_sort','a.id');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('featurehouse_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('featurehouse_initial_direction','ASC');
		}
	...
	protected function getListQuery()
	{
		...
		if ((is_object($params)) AND ($params->get('show_featurehouse_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_featurehouse_filter_field'))
			{
				default:
					break;
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				else
				{
					...
					// FeatureHouse params override menu item params only if menu param = 'use_featurehouse'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_featurehouse' and there is no featurehouse param, use global

					// create an array of just the params set to 'use_featurehouse'
					$menu_params_array = $this->getState('params')->toArray();
					$feature_house_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_featurehouse')
						{
							// if the featurehouse has a value, use it
							if ($feature_house_params->get($key) != '')
							{
								// get the value from the featurehouse
					...
					// merge the selected featurehouse params
					...
					// get display date
					switch ($item->params->get('list_show_featurehouse_date'))
					{
						default:
							$item->display_date = 0;
							break;
