...
require_once JPATH_ADMINISTRATOR.'/components/com_remca/models/rent.php';

class RemcaModelRentForm extends RemcaModelRent
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.edit.rent';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_remca.rent';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_remca.edit.rent', 'rent', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the rent.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Rent item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('rent.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_remca.edit.rent.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('rent.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onRentBeforeSave', array('com_remca.rent', &$table, $is_new));
		...
		$dispatcher->trigger('onRentAfterSave', array('com_remca.rent', $table, $is_new));

		$this->setState('rent.id', $table->id);
