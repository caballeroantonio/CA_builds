...
/**
 * RealEstateManagerCA Component Municipality Model
 *
 */
class RemcaModelLmunicipality extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.lmunicipality';
	...
	{
		if (empty($config['lmunicipality_filter_fields']))
		{
			$config['lmunicipality_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
	...
	protected function populateState()
	{
		...
		$this->setState('lmunicipality.id', $pk);
	...
	*/
	 ...
	 *
	 * @param	integer	$pk	The id of the municipality.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lmunicipality.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_REMCA_LMUNICIPALITIES_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLmunicipalityChangeState', array('com_remca.lmunicipality', $pks, $value));
	...
	public function delete(&$pks)
	{
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
					...
					$result = $dispatcher->trigger('onLmunicipalityBeforeDelete', array('com_remca.lmunicipality', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLmunicipalityAfterDelete', array('com_remca.lmunicipality', &$table));
			}
			else
			{
				$this->setError($table->getError());
