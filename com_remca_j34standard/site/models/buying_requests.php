 ...
 *
 */
class RemcaModelBuying_requests extends JModelList
{
	/**
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('buying_request_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('buying_request_initial_sort','a.id'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('buying_request_initial_sort','a.id');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('buying_request_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('buying_request_initial_direction','ASC');
		}
	...
	protected function getListQuery()
	{
		...
		if ((is_object($params)) AND ($params->get('show_buying_request_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_buying_request_filter_field'))
			{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				else
				{
					...
					// Buying_request params override menu item params only if menu param = 'use_buying_request'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_buying_request' and there is no buying_request param, use global

					// create an array of just the params set to 'use_buying_request'
					$menu_params_array = $this->getState('params')->toArray();
					$buying_request_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_buying_request')
						{
							// if the buying_request has a value, use it
							if ($buying_request_params->get($key) != '')
							{
								// get the value from the buying_request
					...
					// merge the selected buying_request params
					...
					// get display date
					switch ($item->params->get('list_show_buying_request_date'))
					{
						default:
							$item->display_date = 0;
							break;
