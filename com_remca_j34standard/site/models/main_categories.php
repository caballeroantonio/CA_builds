 ...
 *
 */
class RemcaModelMain_categories extends JModelList
{
	/**
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('main_category_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('main_category_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('main_category_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('main_category_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('main_category_initial_direction','ASC');
		}
		...
		// process show_main_category_noauth parameter
		if (!$params->get('show_main_category_noauth'))
		{
		...
		if ($params->get('filter_main_category_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_main_category_archived'));
	...
	protected function getListQuery()
	{
		...
		if ((is_object($params)) AND ($params->get('show_main_category_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$item->main_category_alternative_readmore = $main_category_params->get('main_category_alternative_readmore');
				...
				else
				{
					...
					// Main_category params override menu item params only if menu param = 'use_main_category'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_main_category' and there is no main_category param, use global

					// create an array of just the params set to 'use_main_category'
					$menu_params_array = $this->getState('params')->toArray();
					$main_category_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_main_category')
						{
							// if the main_category has a value, use it
							if ($main_category_params->get($key) != '')
							{
								// get the value from the main_category
					...
					// merge the selected main_category params
					...
					// get display date
					switch ($item->params->get('list_show_main_category_date'))
					{
						default:
							$item->display_date = 0;
							break;
