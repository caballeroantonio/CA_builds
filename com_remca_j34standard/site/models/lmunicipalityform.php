...
require_once JPATH_ADMINISTRATOR.'/components/com_remca/models/lmunicipality.php';

class RemcaModelLmunicipalityForm extends RemcaModelLmunicipality
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.edit.lmunicipality';
	/**
	...
	*/
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lmunicipality.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_remca.edit.lmunicipality.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lmunicipality.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLmunicipalityBeforeSave', array('com_remca.lmunicipality', &$table, $is_new));
		...
		$dispatcher->trigger('onLmunicipalityAfterSave', array('com_remca.lmunicipality', $table, $is_new));

		$this->setState('lmunicipality.id', $table->id);
