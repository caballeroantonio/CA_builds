...
/**
 * This models supports retrieving lists of rent requests.
 *
 */
class RemcaModelRentRequests extends JModelList
{
	/**
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single rent request has been specified either as a parameter or in the url Request
		$pk = $params->get('rent_request_id', '') == '' ? $app->input->getInt('id', '') : $params->get('rent_request_id');
		$this->setState('filter.rent_request_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('rentrequest_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('rentrequest_initial_sort','a.id'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('rentrequest_initial_sort','a.id');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('rentrequest_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('rentrequest_initial_direction','ASC');
		}
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.rent_request_id'));
		$id .= ':'.$this->getState('filter.rent_request_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of rent_requests subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		// Filter by a single or group of rent_requests.
		$rent_request_id = $this->getState('filter.rent_request_id');
		if ($rent_request_id != '')
		{
			if (is_numeric($rent_request_id))
			{
				$type = $this->getState('filter.rent_request_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $rent_request_id);
			}
			else
			{
				if (is_array($rent_request_id))
				{
					JArrayHelper::toInteger($rent_request_id);
					$rent_request_id = implode(',', $rent_request_id);
					$type = $this->getState('filter.rent_request_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$rent_request_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_rentrequest_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_rentrequest_filter_field'))
			{
				default:
					break;
	...
	/**
	 * Method to get a list of rent requests.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $rent_request_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// RentRequest params override menu item params only if menu param = 'use_rentrequest'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_rentrequest' and there is no rentrequest param, use global

					// create an array of just the params set to 'use_rentrequest'
					$menu_params_array = $this->getState('params')->toArray();
					$rent_request_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_rentrequest')
						{
							// if the rentrequest has a value, use it
							if ($rent_request_params->get($key) != '')
							{
								// get the value from the rentrequest
					...
					// merge the selected rentrequest params
					...
					// get display date
					switch ($item->params->get('list_show_rentrequest_date'))
					{
						default:
							$item->display_date = 0;
							break;
