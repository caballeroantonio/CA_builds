 ...
 *
 */
class RemcaModelOrders_detail extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.orders_detail';
	...
	{
		if (empty($config['orders_detail_filter_fields']))
		{
			$config['orders_detail_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
				'id_order','a.id_order',
				'id_user','a.id_user',
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('orders_detail.id');
	...
	public function delete(&$pks)
	{
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
					...
					$result = $dispatcher->trigger('onOrders_detailBeforeDelete', array('com_remca.orders_detail', &$table));
					...
					$dispatcher->trigger('onOrders_detailAfterDelete', array('com_remca.orders_detail', &$table));
			}
			else
			{
				$this->setError($table->getError());
