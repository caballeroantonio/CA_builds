...
require_once JPATH_ADMINISTRATOR.'/components/com_remca/models/photo.php';

class RemcaModelPhotoForm extends RemcaModelPhoto
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.edit.photo';
	/**
	...
	*/
	 ...
	 *
	 * @param	integer	$item_id	The id of the photo.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Photo item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('photo.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_remca.edit.photo.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('photo.id');
		$is_new		= true;
		...
		$dispatcher->trigger('onPhotoAfterSave', array('com_remca.photo', $table, $is_new));

		$this->setState('photo.id', $table->id);
