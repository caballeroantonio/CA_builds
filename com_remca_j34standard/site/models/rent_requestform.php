...
require_once JPATH_ADMINISTRATOR.'/components/com_remca/models/rent_request.php';

class RemcaModelRent_requestForm extends RemcaModelRent_request
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.edit.rent_request';
	/**
	...
	*/
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('rent_request.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_remca.edit.rent_request.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('rent_request.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onRent_requestBeforeSave', array('com_remca.rent_request', &$table, $is_new));
		...
		$dispatcher->trigger('onRent_requestAfterSave', array('com_remca.rent_request', $table, $is_new));

		$this->setState('rent_request.id', $table->id);
