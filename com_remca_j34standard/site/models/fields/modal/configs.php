...
class JFormFieldModal_Configs extends JFormField
{
	...
	protected function getInput()
	{
		...
		if (empty($title))
		{
			$title = JText::_('COM_REMCA_CONFIGS_SELECT_ITEM_LABEL');
		}
		$link = 'index.php?option=com_remca&amp;view=configs&amp;layout=modal&amp;tmpl=component&amp;function=jSelectConfig_'.$this->id;

		// The current user display field.
		$html[] = '<span class="input-append fltlft">';
		$html[] = '<input type="text" class="input-medium" id="'.$this->id.'_name" value="'.$title.'" disabled="disabled" size="35" />'
				 .'<a class="modal btn" title="'.JText::_('COM_REMCA_CONFIGS_SELECT_BUTTON_LABEL').'"  href="'.$link.'&amp;'.JSession::getFormToken().'=1" rel="{handler: \'iframe\', size: {x: 800, y: 450}}"><i class="icon-file"></i> '.JText::_('COM_REMCA_CONFIGS_SELECT_ITEM_LABEL').'</a>';
		$html[] = '</span>';
