 ...
 *
 */
class RemcaModelBuying_request extends JModelItem
{
	/**
	...
	{
		if (empty($config['buying_request_filter_fields']))
		{
			$config['buying_request_filter_fields'] = array(
				'id', 'a.id',
				'id_house','a.id_house',
				'id_user','a.id_user',
	...
	public function delete(&$pks)
	{
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
					...
					$result = $dispatcher->trigger('onBuying_requestBeforeDelete', array('com_remca.buying_request', &$table));
					...
					$dispatcher->trigger('onBuying_requestAfterDelete', array('com_remca.buying_request', &$table));
			}
			else
			{
				$this->setError($table->getError());
