 ...
 *
 */
class RemcaModelMainCategory extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.maincategory';
	...
	{
		if (empty($config['maincategory_filter_fields']))
		{
			$config['maincategory_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
				'alias', 'a.alias',
	...
	*/
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onMainCategoryChangeState', array('com_remca.maincategory', $pks, $value));
	...
	public function delete(&$pks)
	{
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
					...
					$result = $dispatcher->trigger('onMainCategoryBeforeDelete', array('com_remca.maincategory', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onMainCategoryAfterDelete', array('com_remca.maincategory', &$table));
			}
			else
			{
				$this->setError($table->getError());
