...
/**
 * This models supports retrieving lists of orders_details.
 *
 */
class RemcaModelOrders_details extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_remca.orders_details';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single orders_detail has been specified either as a parameter or in the url Request
		$pk = $params->get('orders_detail_id', '') == '' ? $app->input->getInt('id', '') : $params->get('orders_detail_id');
		$this->setState('filter.orders_detail_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('orders_detail_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('orders_detail_initial_sort','a.name'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('orders_detail_initial_sort','a.name');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('orders_detail_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('orders_detail_initial_direction','ASC');
		}
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.orders_detail_id'));
		$id .= ':'.$this->getState('filter.orders_detail_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of orders_details subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$orders_detail_id = $this->getState('filter.orders_detail_id');
		if ($orders_detail_id != '')
		{
			if (is_numeric($orders_detail_id))
			{
				$type = $this->getState('filter.orders_detail_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $orders_detail_id);
			}
			else
			{
				if (is_array($orders_detail_id))
				{
					JArrayHelper::toInteger($orders_detail_id);
					$orders_detail_id = implode(',', $orders_detail_id);
					$type = $this->getState('filter.orders_detail_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$orders_detail_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_orders_detail_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_orders_detail_filter_field'))
			{
	...
	/**
	 * Method to get a list of orders_details.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $orders_detail_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Orders_detail params override menu item params only if menu param = 'use_orders_detail'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_orders_detail' and there is no orders_detail param, use global

					// create an array of just the params set to 'use_orders_detail'
					$menu_params_array = $this->getState('params')->toArray();
					$orders_detail_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_orders_detail')
						{
							// if the orders_detail has a value, use it
							if ($orders_detail_params->get($key) != '')
							{
								// get the value from the orders_detail
					...
					// merge the selected orders_detail params
					...
					// get display date
					switch ($item->params->get('list_show_orders_detail_date'))
					{
						default:
							$item->display_date = 0;
							break;
