...
/**
 * RealEstateManagerCA Component Entrada De La Conversación Whatsapp Model
 *
 */
class RemcaModelWa_entry_conversation extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.wa_entry_conversation';
	...
	{
		if (empty($config['wa_entry_conversation_filter_fields']))
		{
...
//                    AND $user->authorise('core.edit.state', 'com_remca.category.'.(int) $record->catid);
	...
	*/
	 ...
	 *
	 * @param	integer	$pk	The id of the entrada de la conversación whatsapp.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('wa_entry_conversation.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				// If all categories are published, badcats.id will be null, and we just use the wa_entry_conversation state
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_REMCA_WA_ENTRY_CONVERSATIONS_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onWa_entry_conversationChangeState', array('com_remca.wa_entry_conversation', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onWa_entry_conversationBeforeDelete', array('com_remca.wa_entry_conversation', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onWa_entry_conversationAfterDelete', array('com_remca.wa_entry_conversation', &$table));
				}
				else
				{
					// Prune items that you can't change.
