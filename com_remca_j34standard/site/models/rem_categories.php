...
class RemcaModelRem_categories extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('rem_category_id', '') == '' ? $app->input->getInt('id', '') : $params->get('rem_category_id');
		$this->setState('filter.rem_category_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('rem_category_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('rem_category_initial_sort','a.id'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('rem_category_initial_sort','a.id');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('rem_category_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('rem_category_initial_direction','ASC');
		}
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.rem_category_id'));
		$id .= ':'.$this->getState('filter.rem_category_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of rem_categories subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		// Filter by a single or group of rem_categories.
		$rem_category_id = $this->getState('filter.rem_category_id');
		if ($rem_category_id != '')
		{
			if (is_numeric($rem_category_id))
			{
				$type = $this->getState('filter.rem_category_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $rem_category_id);
			}
			else
			{
				if (is_array($rem_category_id))
				{
					JArrayHelper::toInteger($rem_category_id);
					$rem_category_id = implode(',', $rem_category_id);
					$type = $this->getState('filter.rem_category_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$rem_category_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_rem_category_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_rem_category_filter_field'))
			{
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $rem_category_params;
				}
				else
				{
					...
					// get display date
					switch ($item->params->get('list_show_rem_category_date'))
					{
						default:
							$item->display_date = 0;
							break;
