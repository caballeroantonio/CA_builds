 ...
 *
 */
class RemcaModelRentRequest extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.rentrequest';
	...
	{
		if (empty($config['rentrequest_filter_fields']))
		{
			$config['rentrequest_filter_fields'] = array(
				'id', 'a.id',
				'id_house','a.id_house',
				'id_user','a.id_user',
	...
	protected function populateState()
	{
		...
		$this->setState('rentrequest.id', $pk);
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('rentrequest.id');
	...
	public function delete(&$pks)
	{
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
					...
					$result = $dispatcher->trigger('onRentRequestBeforeDelete', array('com_remca.rentrequest', &$table));
					...
					$dispatcher->trigger('onRentRequestAfterDelete', array('com_remca.rentrequest', &$table));
			}
			else
			{
				$this->setError($table->getError());
