...
/**
 * RealEstateManagerCA Component Título De La Conversación Whatsapp Model
 *
 */
class RemcaModelWa_title_conversation extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.wa_title_conversation';
	...
	{
		if (empty($config['wa_title_conversation_filter_fields']))
		{
			$config['wa_title_conversation_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
				'state', 'a.state',
				'created', 'a.created',
	...
	protected function populateState()
	{
		...
		$this->setState('wa_title_conversation.id', $pk);
	...
	*/
	 ...
	 *
	 * @param	integer	$pk	The id of the título de la conversación whatsapp.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('wa_title_conversation.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_REMCA_WA_TITLE_CONVERSATIONS_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onWa_title_conversationChangeState', array('com_remca.wa_title_conversation', $pks, $value));
	...
	public function delete(&$pks)
	{
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
					...
					$result = $dispatcher->trigger('onWa_title_conversationBeforeDelete', array('com_remca.wa_title_conversation', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onWa_title_conversationAfterDelete', array('com_remca.wa_title_conversation', &$table));
			}
			else
			{
				$this->setError($table->getError());
