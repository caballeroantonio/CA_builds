...
/**
 * RealEstateManagerCA Component State Model
 *
 */
class RemcaModelLstate extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.lstate';
	...
	{
		if (empty($config['lstate_filter_fields']))
		{
			$config['lstate_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
	...
	*/
	 ...
	 *
	 * @param	integer	$pk	The id of the state.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lstate.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_REMCA_LSTATES_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLstateChangeState', array('com_remca.lstate', $pks, $value));
	...
	public function delete(&$pks)
	{
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
					...
					$result = $dispatcher->trigger('onLstateBeforeDelete', array('com_remca.lstate', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLstateAfterDelete', array('com_remca.lstate', &$table));
			}
			else
			{
				$this->setError($table->getError());
