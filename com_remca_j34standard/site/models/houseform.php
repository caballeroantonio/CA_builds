...
require_once JPATH_ADMINISTRATOR.'/components/com_remca/models/house.php';

class RemcaModelHouseForm extends RemcaModelHouse
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.edit.house';
	/**
	...
	protected function populateState()
	{
		...
		$this->setState('house.catid', $app->input->getInt('catid'));
	...
	*/
	...
	public function getForm($data = array(), $load_data = true)
	{
		...
		if ($id = (int) $this->getState('house.id'))
		{
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('house.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_remca.edit.house.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('house.id');
		$is_new		= true;
		...
		$dispatcher->trigger('onHouseAfterSave', array('com_remca.house', $table, $is_new));

		$this->setState('house.id', $table->id);
