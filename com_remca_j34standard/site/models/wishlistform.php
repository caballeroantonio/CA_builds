...
require_once JPATH_ADMINISTRATOR.'/components/com_remca/models/wishlist.php';

class RemcaModelWishlistForm extends RemcaModelWishlist
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.edit.wishlist';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_remca.wishlist';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_remca.edit.wishlist', 'wishlist', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('wishlist.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_remca.edit.wishlist.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('wishlist.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onWishlistBeforeSave', array('com_remca.wishlist', &$table, $is_new));
		...
		$dispatcher->trigger('onWishlistAfterSave', array('com_remca.wishlist', $table, $is_new));

		$this->setState('wishlist.id', $table->id);
