...
require_once JPATH_ADMINISTRATOR.'/components/com_remca/models/buyingrequest.php';

class RemcaModelBuyingRequestForm extends RemcaModelBuyingRequest
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.edit.buyingrequest';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_remca.buyingrequest';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_remca.edit.buyingrequest', 'buyingrequest', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('buyingrequest.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_remca.edit.buyingrequest.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('buyingrequest.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onBuyingRequestBeforeSave', array('com_remca.buyingrequest', &$table, $is_new));
		...
		$dispatcher->trigger('onBuyingRequestAfterSave', array('com_remca.buyingrequest', $table, $is_new));

		$this->setState('buyingrequest.id', $table->id);
