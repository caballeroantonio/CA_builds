...
/**
 * This models supports retrieving lists of states.
 *
 */
class RemcaModelLstates extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_remca.lstates';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single state has been specified either as a parameter or in the url Request
		$pk = $params->get('lstate_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lstate_id');
		$this->setState('filter.lstate_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lstate_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lstate_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lstate_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lstate_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lstate_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lstate_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lstate_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lstate_id'));
		$id .= ':'.$this->getState('filter.lstate_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lstates subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__rem_lstates').' AS a');
		...
		// Filter by a single or group of lstates.
		$lstate_id = $this->getState('filter.lstate_id');
		if ($lstate_id != '')
		{
			if (is_numeric($lstate_id))
			{
				$type = $this->getState('filter.lstate_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lstate_id);
			}
			else
			{
				if (is_array($lstate_id))
				{
					JArrayHelper::toInteger($lstate_id);
					$lstate_id = implode(',', $lstate_id);
					$type = $this->getState('filter.lstate_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lstate_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lstate_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_lstate_filter_field'))
			{
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lstate_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lstate params override menu item params only if menu param = 'use_lstate'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lstate' and there is no lstate param, use global

					// create an array of just the params set to 'use_lstate'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lstate')
						{
							// if the lstate has a value, use it
							if ($lstate_params->get($key) != '')
							{
								// get the value from the lstate
								$lstate_array[$key] = $lstate_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lstate_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lstate params
					if (count($lstate_array) > 0)
					{
						$lstate_params = new Registry;
						$lstate_params->loadArray($lstate_array);
						$item->params->merge($lstate_params);
					}


					// get display date
					switch ($item->params->get('list_show_lstate_date'))
					{
						default:
							$item->display_date = 0;
							break;
