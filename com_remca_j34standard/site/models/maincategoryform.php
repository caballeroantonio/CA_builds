...
require_once JPATH_ADMINISTRATOR.'/components/com_remca/models/maincategory.php';

class RemcaModelMainCategoryForm extends RemcaModelMainCategory
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.edit.maincategory';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_remca.maincategory';
	/**
	...
	protected function populateState()
	{
		...
		$this->setState('maincategory.id', $pk);
	...
	*/
	public function getTable($type = 'MainCategories', $prefix = 'RemcaTable', $config = array())
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_remca.edit.maincategory', 'maincategory', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the main_category.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Main_category item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('maincategory.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_remca.edit.maincategory.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('maincategory.id');
		$is_new		= true;
		...
		// Reorder the main_categories so the new main_category is first
		if (empty($table->id))
		{
		...
		$dispatcher->trigger('onMainCategoryAfterSave', array('com_remca.maincategory', $table, $is_new));

		$this->setState('maincategory.id', $table->id);
	...
	{
		$pk	= (!empty($pk)) ? $pk : (int) $this->getState('maincategory.id');
