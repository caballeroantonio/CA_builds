...
/**
 * RealEstateManagerCA Component Main_category Model
 *
 */
class RemcaModelMain_category extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.main_category';
	...
	{
		if (empty($config['main_category_filter_fields']))
		{
			$config['main_category_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
				'alias', 'a.alias',
				'parent_id','a.parent_id',
				'associate_category','a.associate_category',
				'title','a.title',
				'image','a.image',
				'section','a.section',
	...
	protected function populateState()
	{
		...
		$this->setState('main_category.id', $pk);
	...
	*/
	...
	/**
	 * Method to get Main_category data.
	 *
	 * @param	integer	$pk	The id of the main_category.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('main_category.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_REMCA_MAIN_CATEGORIES_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onMain_categoryChangeState', array('com_remca.main_category', $pks, $value));
	...
	public function delete(&$pks)
	{
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
					...
					$result = $dispatcher->trigger('onMain_categoryBeforeDelete', array('com_remca.main_category', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onMain_categoryAfterDelete', array('com_remca.main_category', &$table));
			}
			else
			{
				$this->setError($table->getError());
