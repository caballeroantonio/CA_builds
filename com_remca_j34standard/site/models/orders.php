...
class RemcaModelOrders extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single order has been specified either as a parameter or in the url Request
		$pk = $params->get('order_id', '') == '' ? $app->input->getInt('id', '') : $params->get('order_id');
		$this->setState('filter.order_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('order_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('order_initial_sort','a.name'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('order_initial_sort','a.name');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('order_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('order_initial_direction','ASC');
		}
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.order_id'));
		$id .= ':'.$this->getState('filter.order_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of orders subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		// Filter by a single or group of orders.
		$order_id = $this->getState('filter.order_id');
		if ($order_id != '')
		{
			if (is_numeric($order_id))
			{
				$type = $this->getState('filter.order_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $order_id);
			}
			else
			{
				if (is_array($order_id))
				{
					...
					$type = $this->getState('filter.order_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$order_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_order_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_order_filter_field'))
			{
				case 'name':
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				else
				{
					...
					// get display date
					switch ($item->params->get('list_show_order_date'))
					{
						default:
							$item->display_date = 0;
							break;
