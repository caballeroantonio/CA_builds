...
require_once JPATH_ADMINISTRATOR.'/components/com_remca/models/ordersdetail.php';

class RemcaModelOrdersDetailForm extends RemcaModelOrdersDetail
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.edit.ordersdetail';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_remca.ordersdetail';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_remca.edit.ordersdetail', 'ordersdetail', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ordersdetail.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_remca.edit.ordersdetail.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ordersdetail.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onOrdersDetailBeforeSave', array('com_remca.ordersdetail', &$table, $is_new));
		...
		$dispatcher->trigger('onOrdersDetailAfterSave', array('com_remca.ordersdetail', $table, $is_new));

		$this->setState('ordersdetail.id', $table->id);
