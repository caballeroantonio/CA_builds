...
require_once JPATH_ADMINISTRATOR.'/components/com_remca/models/country.php';

class RemcaModelCountryForm extends RemcaModelCountry
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.edit.country';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_remca.country';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_remca.edit.country', 'country', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('country.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_remca.edit.country.data', array());
	...
	public function save($data)
	{
		...
		// Reorder the countries so the new country is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onCountryBeforeSave', array('com_remca.country', &$table, $is_new));
		...
		$dispatcher->trigger('onCountryAfterSave', array('com_remca.country', $table, $is_new));
