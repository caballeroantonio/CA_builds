...
/**
 * This models supports retrieving lists of countries.
 *
 */
class RemcaModelCountries extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_remca.countries';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single country has been specified either as a parameter or in the url Request
		$pk = $params->get('country_id', '') == '' ? $app->input->getInt('id', '') : $params->get('country_id');
		$this->setState('filter.country_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('country_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('country_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('country_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('country_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('country_initial_direction','ASC');
		}
	...
	/**
	 * Get the main query for retrieving a list of countries subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		// Filter by a single or group of countries.
		$country_id = $this->getState('filter.country_id');
		if ($country_id != '')
		{
			if (is_numeric($country_id))
			{
				$type = $this->getState('filter.country_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $country_id);
			}
			else
			{
				if (is_array($country_id))
				{
					JArrayHelper::toInteger($country_id);
					$country_id = implode(',', $country_id);
					$type = $this->getState('filter.country_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$country_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_country_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_country_filter_field'))
			{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				else
				{
					...
					// get display date
					switch ($item->params->get('list_show_country_date'))
					{
						default:
							$item->display_date = 0;
							break;
