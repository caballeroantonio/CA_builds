...
require_once JPATH_ADMINISTRATOR.'/components/com_remca/models/language.php';

class RemcaModelLanguageForm extends RemcaModelLanguage
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.edit.language';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_remca.language';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_remca.edit.language', 'language', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('language.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_remca.edit.language.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('language.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLanguageBeforeSave', array('com_remca.language', &$table, $is_new));
		...
		$dispatcher->trigger('onLanguageAfterSave', array('com_remca.language', $table, $is_new));

		$this->setState('language.id', $table->id);
