...
require_once JPATH_ADMINISTRATOR.'/components/com_remca/models/feature.php';

class RemcaModelFeatureForm extends RemcaModelFeature
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.edit.feature';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_remca.feature';
	/**
	...
	*/
	public function getTable($type = 'Feature', $prefix = 'RemcaTable', $config = array())
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_remca.edit.feature', 'feature', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('feature.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_remca.edit.feature.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('feature.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onFeatureBeforeSave', array('com_remca.feature', &$table, $is_new));
		...
		$dispatcher->trigger('onFeatureAfterSave', array('com_remca.feature', $table, $is_new));

		$this->setState('feature.id', $table->id);
