...
require_once JPATH_ADMINISTRATOR.'/components/com_remca/models/buying_request.php';

class RemcaModelBuying_requestForm extends RemcaModelBuying_request
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_remca.edit.buying_request';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_remca.buying_request';
	/**
	...
	protected function populateState()
	{
		...
		$this->setState('buying_request.id', $pk);
	...
	*/
	public function getTable($type = 'Buying_requests', $prefix = 'RemcaTable', $config = array())
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_remca.edit.buying_request', 'buying_request', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('buying_request.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_remca.edit.buying_request.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('buying_request.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onBuying_requestBeforeSave', array('com_remca.buying_request', &$table, $is_new));
		...
		$dispatcher->trigger('onBuying_requestAfterSave', array('com_remca.buying_request', $table, $is_new));

		$this->setState('buying_request.id', $table->id);
	...
	{
		$pk	= (!empty($pk)) ? $pk : (int) $this->getState('buying_request.id');
