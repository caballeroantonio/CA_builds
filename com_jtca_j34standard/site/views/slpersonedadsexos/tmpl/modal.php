...
$function	= $app->input->get('function', 'jSelectSlpersonedadsexo');
 ...
 */
?>
<div class="slpersonedadsexos-modal<?php echo $this->params->get('pageclass_sfx');?>">
	<h3><?php echo JText::_('COM_JTCA_SLPERSONEDADSEXOS_SELECT_ITEM_LABEL'); ?></h3>
	<form action="<?php echo JRoute::_('index.php?option=com_jtca&view=slpersonedadsexos&layout=modal&tmpl=component');?>" method="post" name="adminForm" id="adminForm">
		...
		<table class="slpersonedadsexos">
			<thead>
				<tr>
					<th width="10%">
						<?php echo JTEXT::_('COM_JTCA_SLPERSONEDADSEXOS_HEADING_ID_RECORD'); ?>
					</th>	
					<th width="10%">
						<?php echo JTEXT::_('COM_JTCA_SLPERSONEDADSEXOS_HEADING_ID_FIELD'); ?>
					</th>	
				...
				<tr class="row<?php echo $i % 2; ?>">
					<td class="center">
						<a class="pointer" onclick="if (window.parent) window.parent.<?php echo $this->escape($function);?>('<?php echo $item->id; ?>');">
							<?php 
								echo $item->id_record != '' ? $item->id_record : $empty; 
							?>
						</a>	
					</td>	
					<td class="center">
						<a class="pointer" onclick="if (window.parent) window.parent.<?php echo $this->escape($function);?>('<?php echo $item->id; ?>');">
							<?php 
								echo $item->id_field != '' ? $item->id_field : $empty; 
							?>
						</a>	
					</td>	
...
if ($this->params->get('show_slpersonedadsexo_add_link', 1) AND $can_create)
	echo JHtml::_('slpersonedadsexoicon.create', $this->params);
?>
