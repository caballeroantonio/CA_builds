...
$layout		= $this->params->get('ljc10_layout', 'default');
...
<div class="jtca ljc10s-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<?php
		...
		$show_actions = false;
		if ($this->params->get('show_ljc10_icons',1) >= 0) :
	...
	<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_ljc10_filter_field') != '' AND $this->params->get('show_ljc10_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_ljc10_filter_field') != '' AND $this->params->get('show_ljc10_filter_field') != 'hide') :?>
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_JTCA_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_JTCA_'.$this->params->get('show_ljc10_filter_field').'_FILTER_LABEL'); ?>" />
				<?php endif; ?>	
			</div>
		<?php endif; ?>

		<?php if ($this->params->get('show_ljc10_pagination_limit')) : ?>
		...
		<?php if (empty($this->items)) : ?>
			...
			<?php if ($this->params->get('show_no_ljc10s',1)) : ?>
			<p><?php echo JText::_('COM_JTCA_LJC10S_NO_ITEMS'); ?></p>
			<?php endif; ?>

		<?php else : ?>
		<div style="overflow-x:auto;">
			<table class="table table-striped" id="ljc10s" style="margin-bottom: 200px;">
			<?php if ($this->params->get('show_ljc10_headings',1)) :?>
			<thead>
				<tr>
					<th width="1%" style="display:none;">
					</th>				
					<?php if ($date = $this->params->get('list_show_ljc10_date')) : ?>
						<th class="list-date" id="tableOrderingdate">
							<?php echo JHtml::_('grid.sort', 'COM_JTCA_FIELD_'.JString::strtoupper($date).'_LABEL', 'a.'.$date, $list_dirn, $list_order); ?>
						</th>
					<?php endif; ?>

					<?php if ($this->params->get('list_show_ljc10_created_by',0)) : ?>
					...
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_ljc10_id_secretaria',1)) : ?>
						<th class="list-id_secretaria" id="tableOrderingid_secretaria">
							<?php echo JTEXT::_('COM_JTCA_LJC10S_HEADING_ID_SECRETARIA'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_ljc10_anoj',1)) : ?>
					...
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_ljc10_field6',1)) : ?>
						<th class="list-field6" id="tableOrderingfield6">
							<?php echo JTEXT::_('COM_JTCA_LJC10S_HEADING_FIELD6'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_ljc10_field7',1)) : ?>
						<th class="list-field7" id="tableOrderingfield7">
							<?php echo JTEXT::_('COM_JTCA_LJC10S_HEADING_FIELD7'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_ljc10_field8',1)) : ?>
						<th class="list-field8" id="tableOrderingfield8">
							<?php echo JTEXT::_('COM_JTCA_LJC10S_HEADING_FIELD8'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_ljc10_field9',1)) : ?>
						<th class="list-field9" id="tableOrderingfield9">
							<?php echo JTEXT::_('COM_JTCA_LJC10S_HEADING_FIELD9'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_ljc10_field10',1)) : ?>
						<th class="list-field10" id="tableOrderingfield10">
							<?php echo JTEXT::_('COM_JTCA_LJC10S_HEADING_FIELD10'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_ljc10_field11',1)) : ?>
						<th class="list-field11" id="tableOrderingfield11">
							<?php echo JTEXT::_('COM_JTCA_LJC10S_HEADING_FIELD11'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_ljc10_ordering',0)) : ?>
						<th width="10%">
			...
			<tbody>
				...
				?>			
					...
					<?php if ($this->params->get('list_show_ljc10_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('ljc10_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_ljc10_created_by',0)) : ?>
					...
					<?php if ($show_actions) : ?>
						<td class="list-actions">
							...
							<?php if ($params->get('show_ljc10_print_icon')) : ?>
								<li class="print-icon">
										<?php echo JHtml::_('ljc10icon.print_popup',  $item, $params); ?>
								</li>
							<?php endif; ?>

							<?php if ($params->get('show_ljc10_email_icon')) : ?>
								<li class="email-icon">
										<?php echo JHtml::_('ljc10icon.email',  $item, $params); ?>
								</li>
							<?php endif; ?>
								<?php if ($can_edit ) : ?>
                                    <li class="edit-icon">
                                        <?php echo JHtml::_('ljc10icon.edit',$item, $params); ?>
                                    </li>
                                <?php endif; ?>					
                                <?php if ($can_delete) : ?>
                                    <li class="delete-icon">
                                        <?php echo JHtml::_('ljc10icon.delete',$item, $params); ?>
                                    </li>
                                <?php endif; ?>
							<?php if ($can_edit AND $params->get('save_history') AND $params->get('ljc10_save_history')) : ?>
								<li class="version-icon">
									<?php echo JHtml::_('ljc10icon.versions',$item, $params); ?>
								</li>	
							<?php endif; ?>	
                                </ul>
                            </div>
			...
			<?php if (($this->params->def('show_ljc10_pagination', 2) == 1  OR ($this->params->get('show_ljc10_pagination') == 2)) AND ($this->pagination->get('pages.total') > 1)) : ?>
			<div class="pagination">

				<?php if ($this->params->def('show_ljc10_pagination_results', 0)) : ?>
				<p class="counter">
		...
		<?php endif; ?>
		<?php // Code to add a link to submit an ljc10. ?>
		<?php if ($this->params->get('show_ljc10_add_link', 1)) : ?>
			<?php if ($can_create) : ?>
				<?php echo JHtml::_('ljc10icon.create', $this->params); ?>
			<?php  endif; ?>
		<?php endif; ?>		
                <?php echo '<button>export</button>'//JHtml::_('ljc10icon.create', $this->params); ?>
	</form>
</div>
...
function show_collapsibleModal(item_id){
	...
	modalBody.prepend('<iframe class="iframe" src="index.php?option=com_jtca&task=ljc10.showHistory&item_id='+item_id+'" name="titulo" height="450"></iframe>');
	return;
}
</script>
