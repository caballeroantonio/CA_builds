...
$layout		= $this->params->get('ljc14_layout', 'default');
...
<div class="jtca ljc14s-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<?php
		...
		$show_actions = false;
		if ($this->params->get('show_ljc14_icons',1) >= 0) :
	...
	<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_ljc14_filter_field') != '' AND $this->params->get('show_ljc14_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_ljc14_filter_field') != '' AND $this->params->get('show_ljc14_filter_field') != 'hide') :?>
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_JTCA_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_JTCA_'.$this->params->get('show_ljc14_filter_field').'_FILTER_LABEL'); ?>" />
				<?php endif; ?>	
			</div>
		<?php endif; ?>

		<?php if ($this->params->get('show_ljc14_pagination_limit')) : ?>
		...
		<?php if (empty($this->items)) : ?>
			...
			<?php if ($this->params->get('show_no_ljc14s',1)) : ?>
			<p><?php echo JText::_('COM_JTCA_LJC14S_NO_ITEMS'); ?></p>
			<?php endif; ?>

		<?php else : ?>
		<div style="overflow-x:auto;">
			<table class="table table-striped" id="ljc14s" style="margin-bottom: 200px;">
			<?php if ($this->params->get('show_ljc14_headings',1)) :?>
			<thead>
				<tr>
					<th width="1%" style="display:none;">
					</th>				
					<?php if ($date = $this->params->get('list_show_ljc14_date')) : ?>
						<th class="list-date" id="tableOrderingdate">
							<?php echo JHtml::_('grid.sort', 'COM_JTCA_FIELD_'.JString::strtoupper($date).'_LABEL', 'a.'.$date, $list_dirn, $list_order); ?>
						</th>
					<?php endif; ?>

					<?php if ($this->params->get('list_show_ljc14_created_by',0)) : ?>
			...
			<tbody>
				...
				?>			
					...
					<?php if ($this->params->get('list_show_ljc14_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('ljc14_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_ljc14_created_by',0)) : ?>
					...
					<?php if ($this->params->get('list_show_ljc14_id_expediente',1)) : ?>
						<td class="list-id_expediente">
							<?php 
								if ($params->get('list_link_ljc14_id_expediente')) :
			...
			<div class="pagination">
				...
				<?php if ($this->params->def('show_ljc14_pagination_results', 0)) : ?>
				<p class="counter">
		...
		<?php endif; ?>
		<?php // Code to add a link to submit an ljc14. ?>
		<?php if ($this->params->get('show_ljc14_add_link', 1)) : ?>
			<?php if ($can_create) : ?>
				<?php echo JHtml::_('ljc14icon.create', $this->params); ?>
			<?php  endif; ?>
		<?php endif; ?>		
...
function show_collapsibleModal(item_id){
	...
	modalBody.prepend('<iframe class="iframe" src="index.php?option=com_jtca&task=ljc14.showHistory&item_id='+item_id+'" name="titulo" height="450"></iframe>');
	return;
}
</script>
