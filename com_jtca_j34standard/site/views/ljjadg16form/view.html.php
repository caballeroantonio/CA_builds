...
class JtCaViewLjjadg16Form extends JViewLegacy
{
	...
	protected function prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('COM_JTCA_LJJADG16S_EDIT_ITEM'));
		}

		if (!is_null($this->item->id))
		{
			$title = $this->params->def('page_title', JText::_('COM_JTCA_LJJADG16S_EDIT_ITEM'));
			
			$pathway->addItem(JText::sprintf('COM_JTCA_EDIT_ITEM', JText::sprintf('COM_JTCA_LJJADG16S_ID_TITLE', $this->item->id)),''); 
		}
		else
		{
			$title = $this->params->def('page_title', JText::_('COM_JTCA_LJJADG16S_CREATE_ITEM'));
			
			$pathway->addItem(JText::_('COM_JTCA_LJJADG16S_CREATE_ITEM'),'');
		}
		...
		{
			$this->document->setTitle($ljjadg16->page_title.' - '.JText::sprintf('COM_JTCA_PAGEBREAK_PAGE_NUM', $this->state->get('page.offset') + 1));
		}
