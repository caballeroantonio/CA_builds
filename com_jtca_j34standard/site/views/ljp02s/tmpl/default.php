...
$this->document->addStyleSheet(JUri::root().'media/com_jtca/css/site_ljp02s.css');

if ($lang->isRTL())
{
	$this->document->addStyleSheet(JUri::root().'media/com_jtca/css/site_jtca-rtl.css');
	$this->document->addStyleSheet(JUri::root().'media/com_jtca/css/site_ljp02s-rtl.css');
}
...
$layout		= $this->params->get('ljp02_layout', 'default');
...
<div class="jtca ljp02s-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<?php
		...
		$show_actions = false;
		if ($this->params->get('show_ljp02_icons',1) >= 0) :
	...
	<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_ljp02_filter_field') != '' AND $this->params->get('show_ljp02_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_ljp02_filter_field') != '' AND $this->params->get('show_ljp02_filter_field') != 'hide') :?>
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_JTCA_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_JTCA_'.$this->params->get('show_ljp02_filter_field').'_FILTER_LABEL'); ?>" />
				<?php endif; ?>	
				<?php if ($this->params->get('list_show_ljp02_billete',1)) : ?>
					<select name="filter_billete" onchange="this.form.submit()">
					<option value=""><?php echo JText::_('COM_JTCA_LJP02S_SELECT_BILLETE');?></option>
		...
		<?php if ($this->params->get('show_ljp02_pagination_limit')) : ?>
		...
		<?php if (empty($this->items)) : ?>
			...
			<?php if ($this->params->get('show_no_ljp02s',1)) : ?>
			<p><?php echo JText::_('COM_JTCA_LJP02S_NO_ITEMS'); ?></p>
			<?php endif; ?>

		<?php else : ?>
		<div style="overflow-x:auto;">
			<table class="table table-striped" id="ljp02s" style="margin-bottom: 200px;">
			<?php if ($this->params->get('show_ljp02_headings',1)) :?>
			<thead>
				<tr>
					<th width="1%" style="display:none;">
					</th>				
					<?php if ($date = $this->params->get('list_show_ljp02_date')) : ?>
						<th class="list-date" id="tableOrderingdate">
							<?php echo JHtml::_('grid.sort', 'COM_JTCA_FIELD_'.JString::strtoupper($date).'_LABEL', 'a.'.$date, $list_dirn, $list_order); ?>
						</th>
					<?php endif; ?>

					<?php if ($this->params->get('list_show_ljp02_created_by',0)) : ?>
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_ljp02_id_organo',1)) : ?>
					...
					<?php if ($this->params->get('list_show_ljp02_billete',1)) : ?>
						<th class="list-billete" id="tableOrderingbillete">
							<?php echo JTEXT::_('COM_JTCA_LJP02S_HEADING_BILLETE'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_ljp02_field3',1)) : ?>
						<th class="list-field3" id="tableOrderingfield3">
							<?php echo JTEXT::_('COM_JTCA_LJP02S_HEADING_FIELD3'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_ljp02_field4',1)) : ?>
						<th class="list-field4" id="tableOrderingfield4">
							<?php echo JTEXT::_('COM_JTCA_LJP02S_HEADING_FIELD4'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_ljp02_field5',1)) : ?>
						<th class="list-field5" id="tableOrderingfield5">
							<?php echo JTEXT::_('COM_JTCA_LJP02S_HEADING_FIELD5'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_ljp02_field9',1)) : ?>
						<th class="list-field9" id="tableOrderingfield9">
							<?php echo JTEXT::_('COM_JTCA_LJP02S_HEADING_FIELD9'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_ljp02_field7',1)) : ?>
						<th class="list-field7" id="tableOrderingfield7">
							<?php echo JTEXT::_('COM_JTCA_LJP02S_HEADING_FIELD7'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_ljp02_field8',1)) : ?>
						<th class="list-field8" id="tableOrderingfield8">
							<?php echo JTEXT::_('COM_JTCA_LJP02S_HEADING_FIELD8'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_ljp02_ordering',0)) : ?>
						<th width="10%">
			...
			<tbody>
				...
				?>			
					...
					<?php if ($this->params->get('list_show_ljp02_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('ljp02_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_ljp02_created_by',0)) : ?>
					...
					<?php if ($show_actions) : ?>
						<td class="list-actions">
							...
							<?php if ($params->get('show_ljp02_print_icon')) : ?>
								<li class="print-icon">
										<?php echo JHtml::_('ljp02icon.print_popup',  $item, $params); ?>
								</li>
							<?php endif; ?>

							<?php if ($params->get('show_ljp02_email_icon')) : ?>
								<li class="email-icon">
										<?php echo JHtml::_('ljp02icon.email',  $item, $params); ?>
								</li>
							<?php endif; ?>
								<?php if ($can_edit ) : ?>
                                    <li class="edit-icon">
                                        <?php echo JHtml::_('ljp02icon.edit',$item, $params); ?>
                                    </li>
                                <?php endif; ?>					
                                <?php if ($can_delete) : ?>
                                    <li class="delete-icon">
                                        <?php echo JHtml::_('ljp02icon.delete',$item, $params); ?>
                                    </li>
                                <?php endif; ?>
							<?php if ($can_edit AND $params->get('save_history') AND $params->get('ljp02_save_history')) : ?>
								<li class="version-icon">
									<?php echo JHtml::_('ljp02icon.versions',$item, $params); ?>
								</li>	
							<?php endif; ?>	
                                </ul>
                            </div>
			...
			<?php if (($this->params->def('show_ljp02_pagination', 2) == 1  OR ($this->params->get('show_ljp02_pagination') == 2)) AND ($this->pagination->get('pages.total') > 1)) : ?>
			<div class="pagination">

				<?php if ($this->params->def('show_ljp02_pagination_results', 0)) : ?>
				<p class="counter">
		...
		<?php endif; ?>
		<?php // Code to add a link to submit an ljp02. ?>
		<?php if ($this->params->get('show_ljp02_add_link', 1)) : ?>
			<?php if ($can_create) : ?>
				<?php echo JHtml::_('ljp02icon.create', $this->params); ?>
			<?php  endif; ?>
		<?php endif; ?>		
                <?php echo '<button>export</button>'//JHtml::_('ljp02icon.create', $this->params); ?>
	</form>
</div>
...
function show_collapsibleModal(item_id){
	...
	modalBody.prepend('<iframe class="iframe" src="index.php?option=com_jtca&task=ljp02.showHistory&item_id='+item_id+'" name="titulo" height="450"></iframe>');
	return;
}
</script>
