...
class JtCaViewLjp21s extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		// PREPARE THE DATA
		// Compute the ljp21 slugs and set the trigger events.
		foreach ($items as $i => &$item)
		{
			...
			$dispatcher->trigger('onLjp21Prepare', array ('com_jtca.ljp21', &$item, &$item->params,$i));

			$item->event = new stdClass;


			$results = $dispatcher->trigger('onLjp21BeforeDisplay', array('com_jtca.ljp21', &$item, &$item->params, $i));
			$item->event->beforeDisplayLjp21 = JString::trim(implode("\n", $results));

			$results = $dispatcher->trigger('onLjp21AfterDisplay', array('com_jtca.ljp21', &$item, &$item->params,$i));
	...
	protected function prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('COM_JTCA_LJP21S'));
		}
