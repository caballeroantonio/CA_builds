...
function getFields(){
    ...
    */
    ...
    //{OBJECT_FIELD}
    $field = array(
        'FIELD_NAME' => 'NÚMERO DE CERTIFICADO',
        'FIELD_CODE_NAME' => 'field178',
        'FIELD_DESCRIPTION' => '',//
        'FIELDTYPE_ID' => 1,
    );
    /*
                FIELD_OPTIONS_LANGUAGE_VARS=
                FIELD_CODE_NAME_UPPER=FIELD178
                FIELD_INTRO=
                FIELD_DESCRIPTION_INI=

                FIELD_DB=`` VARCHAR(45) DEFAULT NULL NÚMERO DE CERTIFICADO

                FIELD_NAME_LATEX=N\'UMERO DE CERTIFICADO
                FIELD_CODE_NAME_LATEX=field178
                FIELD_DBCOMMENT_LATEX=


    */
    $fields['field178'] = $field;
    return $fields;
}
...
//	JHtml::_('behavior.modal', 'a.modal_jform_contenthistory');//creo que se carga con el botón versiones
	...
	$model	= JModelLegacy::getInstance('Lsc08Form','JtCaModel', array('ignore_request' => FALSE));
	$data = array();
	$data['id'] = 1;
	...
	echo $this->form->getInput('contenthistory');
				...
				echo JHtml::_(
					...
					array(
						'title'  => 'titulo',
						'height' => 450,
						'url' => new JUri('index.php?option=com_jtca&view=lsc08form&layout=edit&layout=edit&tmpl=component&id=1&function=on_collapseModal')
					)
				);
}
...
document.id = Ext.documentId;
	...
	Ext.create('Ext.grid.Panel', {
		title: 'LIBRO DE PÓLIZAS Y FIANZAS (OFICIAL) - Ingresos',
