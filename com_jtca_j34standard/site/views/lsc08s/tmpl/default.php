...
$this->document->addStyleSheet(JUri::root().'media/com_jtca/css/site_lsc08s.css');

if ($lang->isRTL())
{
	$this->document->addStyleSheet(JUri::root().'media/com_jtca/css/site_jtca-rtl.css');
	$this->document->addStyleSheet(JUri::root().'media/com_jtca/css/site_lsc08s-rtl.css');
}
...
$layout		= $this->params->get('lsc08_layout', 'default');
...
<div class="jtca lsc08s-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<?php
		...
		$show_actions = false;
		if ($this->params->get('show_lsc08_icons',1) >= 0) :
	...
	<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_lsc08_filter_field') != '' AND $this->params->get('show_lsc08_filter_field') != 'hide')) :?>
			<div class="filter-search">
				<?php if ($this->params->get('show_lsc08_filter_field') != '' AND $this->params->get('show_lsc08_filter_field') != 'hide') :?>
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_JTCA_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_JTCA_'.$this->params->get('show_lsc08_filter_field').'_FILTER_LABEL'); ?>" />
				<?php endif; ?>	
				<?php if ($this->params->get('list_show_lsc08_billete',1)) : ?>
					<select name="filter_billete" onchange="this.form.submit()">
					<option value=""><?php echo JText::_('COM_JTCA_LSC08S_SELECT_BILLETE');?></option>
		...
		<?php if ($this->params->get('show_lsc08_pagination_limit')) : ?>
		...
		<?php if (empty($this->items)) : ?>
			...
			<?php if ($this->params->get('show_no_lsc08s',1)) : ?>
			<p><?php echo JText::_('COM_JTCA_LSC08S_NO_ITEMS'); ?></p>
			<?php endif; ?>

		<?php else : ?>
		<div style="overflow-x:auto;">
			<table class="table table-striped" id="lsc08s" style="margin-bottom: 200px;">
			<?php if ($this->params->get('show_lsc08_headings',1)) :?>
			<thead>
				<tr>
					<th width="1%" style="display:none;">
					</th>				
					<?php if ($date = $this->params->get('list_show_lsc08_date')) : ?>
						<th class="list-date" id="tableOrderingdate">
							<?php echo JHtml::_('grid.sort', 'COM_JTCA_FIELD_'.JString::strtoupper($date).'_LABEL', 'a.'.$date, $list_dirn, $list_order); ?>
						</th>
					<?php endif; ?>

					<?php if ($this->params->get('list_show_lsc08_created_by',0)) : ?>
					...
					<?php endif; ?>
					<?php if ($this->params->get('list_show_lsc08_id_organo',1)) : ?>
						<th class="list-id_organo" id="tableOrderingid_organo">
							<?php echo JTEXT::_('COM_JTCA_LSC08S_HEADING_ID_ORGANO'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_lsc08_id_secretaria',1)) : ?>
						<th class="list-id_secretaria" id="tableOrderingid_secretaria">
							<?php echo JTEXT::_('COM_JTCA_LSC08S_HEADING_ID_SECRETARIA'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_lsc08_anoj',1)) : ?>
					...
					<?php if ($this->params->get('list_show_lsc08_billete',1)) : ?>
						<th class="list-billete" id="tableOrderingbillete">
							<?php echo JTEXT::_('COM_JTCA_LSC08S_HEADING_BILLETE'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_lsc08_ordering',0)) : ?>
						<th width="10%">
			...
			<tbody>
				...
				?>			
					...
					<?php if ($this->params->get('list_show_lsc08_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('lsc08_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_lsc08_created_by',0)) : ?>
					...
					<?php if ($show_actions) : ?>
						<td class="list-actions">
							...
							<?php if ($params->get('show_lsc08_print_icon')) : ?>
								<li class="print-icon">
										<?php echo JHtml::_('lsc08icon.print_popup',  $item, $params); ?>
								</li>
							<?php endif; ?>

							<?php if ($params->get('show_lsc08_email_icon')) : ?>
								<li class="email-icon">
										<?php echo JHtml::_('lsc08icon.email',  $item, $params); ?>
								</li>
							<?php endif; ?>
								<?php if ($can_edit ) : ?>
                                    <li class="edit-icon">
                                        <?php echo JHtml::_('lsc08icon.edit',$item, $params); ?>
                                    </li>
                                <?php endif; ?>					
                                <?php if ($can_delete) : ?>
                                    <li class="delete-icon">
                                        <?php echo JHtml::_('lsc08icon.delete',$item, $params); ?>
                                    </li>
                                <?php endif; ?>
							<?php if ($can_edit AND $params->get('save_history') AND $params->get('lsc08_save_history')) : ?>
								<li class="version-icon">
									<?php echo JHtml::_('lsc08icon.versions',$item, $params); ?>
								</li>	
							<?php endif; ?>	
                                </ul>
                            </div>
			...
			<?php if (($this->params->def('show_lsc08_pagination', 2) == 1  OR ($this->params->get('show_lsc08_pagination') == 2)) AND ($this->pagination->get('pages.total') > 1)) : ?>
			<div class="pagination">

				<?php if ($this->params->def('show_lsc08_pagination_results', 0)) : ?>
				<p class="counter">
		...
		<?php endif; ?>
		<?php // Code to add a link to submit an lsc08. ?>
		<?php if ($this->params->get('show_lsc08_add_link', 1)) : ?>
			<?php if ($can_create) : ?>
				<?php echo JHtml::_('lsc08icon.create', $this->params); ?>
			<?php  endif; ?>
		<?php endif; ?>		
                <?php echo '<button>export</button>'//JHtml::_('lsc08icon.create', $this->params); ?>
	</form>
</div>
...
function show_collapsibleModal(item_id){
	...
	modalBody.prepend('<iframe class="iframe" src="index.php?option=com_jtca&task=lsc08.showHistory&item_id='+item_id+'" name="titulo" height="450"></iframe>');
	return;
}
</script>
