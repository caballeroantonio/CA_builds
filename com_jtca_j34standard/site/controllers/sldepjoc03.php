...
/**
 * SldepJoc03 controller class.
 * 
 */
class JtcaControllerSldepJoc03 extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'sldepjoc03form';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'sldep_joc03s';
	/**
	 * 
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('sldepjoc03form')->getItem($record_id);
	 ...
	 * 
	 */
	public function getModel($name = 'sldepjoc03form', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Sub Libro CERTIFICADO DE EGRESOS Del LIBRO DE CERTIFICADO DE DEPÓSITOS DE INGRESOS Y EGRESOS can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('sldepjoc03form');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_JTCA_SLDEP_JOC03S_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_JTCA_SLDEP_JOC03S_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('sldepjoc03.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		// Get the menu item model.
		$model = $this->getModel('sldepjoc03');
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_JTCA_SLDEP_JOC03S_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_JTCA_SLDEP_JOC03S_DELETE_FAILED'));
			}
		}
