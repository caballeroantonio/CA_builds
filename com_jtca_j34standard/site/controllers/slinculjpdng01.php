...
/**
 * SlinculJpdng01 controller class.
 * 
 */
class JtcaControllerSlinculJpdng01 extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'slinculjpdng01form';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'slincul_jpdng01s';
	/**
	 * 
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('slinculjpdng01form')->getItem($record_id);
	 ...
	 * 
	 */
	public function getModel($name = 'slinculjpdng01form', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Sub Libro DATOS DEL(OS) INCULPADO(S) Del LIBRO DE GOBIERNO can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('slinculjpdng01form');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_JTCA_SLINCUL_JPDNG01S_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_JTCA_SLINCUL_JPDNG01S_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('slinculjpdng01.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		// Get the menu item model.
		$model = $this->getModel('slinculjpdng01');
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_JTCA_SLINCUL_JPDNG01S_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_JTCA_SLINCUL_JPDNG01S_DELETE_FAILED'));
			}
		}
