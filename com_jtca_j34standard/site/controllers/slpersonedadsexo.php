...
/**
 * Slpersonedadsexo controller class.
 * 
 */
class JtcaControllerSlpersonedadsexo extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'slpersonedadsexoform';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'slpersonedadsexos';
	/**
	 * 
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('slpersonedadsexoform')->getItem($record_id);
	 ...
	 * 
	 */
	public function getModel($name = 'slpersonedadsexoform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Adolescente can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('slpersonedadsexoform');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_JTCA_SLPERSONEDADSEXOS_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_JTCA_SLPERSONEDADSEXOS_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('slpersonedadsexo.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		// Get the menu item model.
		$model = $this->getModel('slpersonedadsexo');
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_JTCA_SLPERSONEDADSEXOS_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_JTCA_SLPERSONEDADSEXOS_DELETE_FAILED'));
			}
		}
