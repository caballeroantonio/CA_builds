...
/**
 * Ljf15 controller class.
 * 
 */
class JtcaControllerLjf15 extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'ljf15form';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'ljf15s';
	/**
	 * 
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('ljf15form')->getItem($record_id);
	 ...
	 * 
	 */
	public function getModel($name = 'ljf15form', $prefix = '',$config = array('ignore_request' => true))
	{
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_JTCA_LJF15S_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_JTCA_LJF15S_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('ljf15.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_JTCA_LJF15S_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_JTCA_LJF15S_DELETE_FAILED'));
			}
		}
