...
/**
 * Lsps02 controller class.
 * 
 */
class JtCaControllerLsps02 extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'lsps02form';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'lsps02s';
	/**
	 * 
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('lsps02form')->getItem($record_id);
	 ...
	 * 
	 */
	public function getModel($name = 'lsps02form', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the LIBRO DE GOBIERNO (NUEVO SISTEMA) (UNITARIO) can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('lsps02form');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_JTCA_LSPS02S_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_JTCA_LSPS02S_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('lsps02.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_JTCA_LSPS02S_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_JTCA_LSPS02S_DELETE_FAILED'));
			}
		}
