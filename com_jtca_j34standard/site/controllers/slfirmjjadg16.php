...
/**
 * SlfirmJjadg16 controller class.
 * 
 */
class JtcaControllerSlfirmJjadg16 extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'slfirmjjadg16form';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'slfirm_jjadg16s';
	/**
	 * 
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('slfirmjjadg16form')->getItem($record_id);
	 ...
	 * 
	 */
	public function getModel($name = 'slfirmjjadg16form', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Sub Libro CONTROL DE FIRMAS Del LIBRO DE CONTROL DE PRESENTACION DE ADOLESCENTES EN LIBERTAD ANTE EL JUEZ can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('slfirmjjadg16form');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_JTCA_SLFIRM_JJADG16S_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_JTCA_SLFIRM_JJADG16S_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('slfirmjjadg16.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		// Get the menu item model.
		$model = $this->getModel('slfirmjjadg16');
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_JTCA_SLFIRM_JJADG16S_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_JTCA_SLFIRM_JJADG16S_DELETE_FAILED'));
			}
		}
