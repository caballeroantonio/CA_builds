...
/**
 * Ljemst05 controller class.
 * 
 */
class JtCaControllerLjemst05 extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'ljemst05form';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'ljemst05s';
	/**
	 * 
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('ljemst05form')->getItem($record_id);
	 ...
	 * 
	 */
	public function getModel($name = 'ljemst05form', $prefix = '',$config = array('ignore_request' => true))
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('ljemst05form');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_JTCA_LJEMST05S_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_JTCA_LJEMST05S_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('ljemst05.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_JTCA_LJEMST05S_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_JTCA_LJEMST05S_DELETE_FAILED'));
			}
		}
