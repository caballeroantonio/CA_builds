...
/**
 * Lcp20 controller class.
 * 
 */
class JtCaControllerLcp20 extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'lcp20form';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'lcp20s';
	/**
	 * 
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('lcp20form')->getItem($record_id);
	 ...
	 * 
	 */
	public function getModel($name = 'lcp20form', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the LIBRO DE TURNO PARA INCOMPETENCIAS PRIMERA INSTANCIA. can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_JTCA_LCP20S_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_JTCA_LCP20S_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('lcp20.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		if ($id > 0) 
		{
			...
			{
				$this->setMessage(JText::_('COM_JTCA_LCP20S_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_JTCA_LCP20S_DELETE_FAILED'));
			}
		}
