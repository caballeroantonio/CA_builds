...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpes15.php';

class JtCaModelLjpes15Form extends JtCaModelLjpes15
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpes15';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpes15';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpes15', 'ljpes15', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de beneficios revocados y ordenes de re-aprehensión.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE BENEFICIOS REVOCADOS Y ORDENES DE RE-APREHENSIÓN item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpes15.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpes15.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpes15.id');
		$is_new		= true;
		...
		// Reorder the libro de beneficios revocados y ordenes de re-apre so the new libro de beneficios revocados y ordenes de re-aprehensión is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpes15BeforeSave', array('com_jtca.ljpes15', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpes15AfterSave', array('com_jtca.ljpes15', $table, $is_new));

		$this->setState('ljpes15.id', $table->id);
