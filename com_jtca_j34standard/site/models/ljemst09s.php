...
class JtCaModelLjemst09s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljemst09_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljemst09_id');
		$this->setState('filter.ljemst09_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljemst09_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljemst09_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljemst09_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljemst09_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljemst09_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljemst09_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljemst09_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljemst09_id'));
		$id .= ':'.$this->getState('filter.ljemst09_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljemst09s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljemst09s').' AS a');
		...
		// Filter by a single or group of ljemst09s.
		$ljemst09_id = $this->getState('filter.ljemst09_id');
		if ($ljemst09_id != '')
		{
			if (is_numeric($ljemst09_id))
			{
				$type = $this->getState('filter.ljemst09_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljemst09_id);
			}
			else
			{
				if (is_array($ljemst09_id))
				{
					JArrayHelper::toInteger($ljemst09_id);
					$ljemst09_id = implode(',', $ljemst09_id);
					$type = $this->getState('filter.ljemst09_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljemst09_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljemst09_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljemst09_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljemst09_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljemst09 params override menu item params only if menu param = 'use_ljemst09'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljemst09' and there is no ljemst09 param, use global

					// create an array of just the params set to 'use_ljemst09'
					$menu_params_array = $this->getState('params')->toArray();
					$ljemst09_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljemst09')
						{
							// if the ljemst09 has a value, use it
							if ($ljemst09_params->get($key) != '')
							{
								// get the value from the ljemst09
								$ljemst09_array[$key] = $ljemst09_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljemst09_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljemst09 params
					if (count($ljemst09_array) > 0)
					{
						$ljemst09_params = new Registry;
						$ljemst09_params->loadArray($ljemst09_array);
						$item->params->merge($ljemst09_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljemst09_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljemst09, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
