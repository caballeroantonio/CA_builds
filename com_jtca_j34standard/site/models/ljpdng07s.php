...
class JtcaModelLjpdng07s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpdng07_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng07_id');
		$this->setState('filter.ljpdng07_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng07_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng07_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng07_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng07_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng07_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng07_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng07_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng07_id'));
		$id .= ':'.$this->getState('filter.ljpdng07_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng07s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng07s').' AS a');
		...
		// Filter by a single or group of ljpdng07s.
		$ljpdng07_id = $this->getState('filter.ljpdng07_id');
		if ($ljpdng07_id != '')
		{
			if (is_numeric($ljpdng07_id))
			{
				$type = $this->getState('filter.ljpdng07_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng07_id);
			}
			else
			{
				if (is_array($ljpdng07_id))
				{
					JArrayHelper::toInteger($ljpdng07_id);
					$ljpdng07_id = implode(',', $ljpdng07_id);
					$type = $this->getState('filter.ljpdng07_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng07_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng07_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng07_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng07_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng07 params override menu item params only if menu param = 'use_ljpdng07'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng07' and there is no ljpdng07 param, use global

					// create an array of just the params set to 'use_ljpdng07'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng07_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng07')
						{
							// if the ljpdng07 has a value, use it
							if ($ljpdng07_params->get($key) != '')
							{
								// get the value from the ljpdng07
								$ljpdng07_array[$key] = $ljpdng07_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng07_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng07 params
					if (count($ljpdng07_array) > 0)
					{
						$ljpdng07_params = new Registry;
						$ljpdng07_params->loadArray($ljpdng07_array);
						$item->params->merge($ljpdng07_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng07_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng07, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
