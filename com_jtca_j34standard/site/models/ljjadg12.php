 ...
 *
 */
class JtCaModelLjjadg12 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljjadg12';
	...
	{
		if (empty($config['ljjadg12_filter_fields']))
		{
			$config['ljjadg12_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljjadg12s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljjadg12.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJJADG12S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjjadg12ChangeState', array('com_jtca.ljjadg12', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjjadg12BeforeDelete', array('com_jtca.ljjadg12', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjjadg12AfterDelete', array('com_jtca.ljjadg12', &$table));
				}
				else
				{
					// Prune items that you can't change.
