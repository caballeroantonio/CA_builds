...
/**
 * This models supports retrieving lists of libro de turno para incompetencias adolescentes.
 *
 */
class JtCaModelLcp06s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp06s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno para incompetencias adolescentes has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp06_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp06_id');
		$this->setState('filter.lcp06_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp06_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp06_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp06_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp06_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp06_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp06_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp06_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp06_id'));
		$id .= ':'.$this->getState('filter.lcp06_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp06s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp06s').' AS a');
		...
		// Filter by a single or group of lcp06s.
		$lcp06_id = $this->getState('filter.lcp06_id');
		if ($lcp06_id != '')
		{
			if (is_numeric($lcp06_id))
			{
				$type = $this->getState('filter.lcp06_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp06_id);
			}
			else
			{
				if (is_array($lcp06_id))
				{
					JArrayHelper::toInteger($lcp06_id);
					$lcp06_id = implode(',', $lcp06_id);
					$type = $this->getState('filter.lcp06_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp06_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp06_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp06_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno para incompetencias adolescentes.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp06_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp06 params override menu item params only if menu param = 'use_lcp06'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp06' and there is no lcp06 param, use global

					// create an array of just the params set to 'use_lcp06'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp06')
						{
							// if the lcp06 has a value, use it
							if ($lcp06_params->get($key) != '')
							{
								// get the value from the lcp06
								$lcp06_array[$key] = $lcp06_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp06_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp06 params
					if (count($lcp06_array) > 0)
					{
						$lcp06_params = new Registry;
						$lcp06_params->loadArray($lcp06_array);
						$item->params->merge($lcp06_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp06_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp06, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
