...
class JtcaModelLsps14s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('lsps14_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsps14_id');
		$this->setState('filter.lsps14_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsps14_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsps14_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsps14_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsps14_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsps14_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsps14_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsps14_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsps14_id'));
		$id .= ':'.$this->getState('filter.lsps14_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsps14s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsps14s').' AS a');
		...
		// Filter by a single or group of lsps14s.
		$lsps14_id = $this->getState('filter.lsps14_id');
		if ($lsps14_id != '')
		{
			if (is_numeric($lsps14_id))
			{
				$type = $this->getState('filter.lsps14_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsps14_id);
			}
			else
			{
				if (is_array($lsps14_id))
				{
					JArrayHelper::toInteger($lsps14_id);
					$lsps14_id = implode(',', $lsps14_id);
					$type = $this->getState('filter.lsps14_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsps14_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsps14_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsps14_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsps14_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsps14 params override menu item params only if menu param = 'use_lsps14'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsps14' and there is no lsps14 param, use global

					// create an array of just the params set to 'use_lsps14'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsps14')
						{
							// if the lsps14 has a value, use it
							if ($lsps14_params->get($key) != '')
							{
								// get the value from the lsps14
								$lsps14_array[$key] = $lsps14_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsps14_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsps14 params
					if (count($lsps14_array) > 0)
					{
						$lsps14_params = new Registry;
						$lsps14_params->loadArray($lsps14_array);
						$item->params->merge($lsps14_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsps14_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsps14, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
