...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpes04.php';

class JtCaModelLjpes04Form extends JtCaModelLjpes04
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpes04';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpes04';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpes04', 'ljpes04', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpes04.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpes04.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpes04.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjpes04BeforeSave', array('com_jtca.ljpes04', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpes04AfterSave', array('com_jtca.ljpes04', $table, $is_new));

		$this->setState('ljpes04.id', $table->id);
