...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp13.php';

class JtCaModelLcp13Form extends JtCaModelLcp13
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp13';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp13';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp13', 'lcp13', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de expedientillos de juzgados de ejecución para juzgados penal o no graves..
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE EXPEDIENTILLOS DE JUZGADOS DE EJECUCIÓN PARA JUZGADOS PENAL O NO GRAVES. item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp13.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp13.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp13.id');
		$is_new		= true;
		...
		// Reorder the libreta de expedientillos de juzgados de ejecución so the new libreta de expedientillos de juzgados de ejecución para juzgados penal o no graves. is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp13BeforeSave', array('com_jtca.lcp13', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp13AfterSave', array('com_jtca.lcp13', $table, $is_new));

		$this->setState('lcp13.id', $table->id);
