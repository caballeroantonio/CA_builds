...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadng07.php';

class JtcaModelLjjadng07Form extends JtcaModelLjjadng07
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadng07';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadng07';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadng07', 'ljjadng07', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de oficios.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE OFICIOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadng07.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadng07.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadng07.id');
		$is_new		= true;
		...
		// Reorder the libro de oficios so the new libro de oficios is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadng07BeforeSave', array('com_jtca.ljjadng07', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadng07AfterSave', array('com_jtca.ljjadng07', $table, $is_new));

		$this->setState('ljjadng07.id', $table->id);
