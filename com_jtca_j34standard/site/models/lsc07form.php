...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsc07.php';

class JtCaModelLsc07Form extends JtCaModelLsc07
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsc07';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsc07';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsc07', 'lsc07', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de egresos (oficial).
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE EGRESOS (OFICIAL) item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsc07.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsc07.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsc07.id');
		$is_new		= true;
		...
		// Reorder the libro de egresos (oficial) so the new libro de egresos (oficial) is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsc07BeforeSave', array('com_jtca.lsc07', &$table, $is_new));
		...
		$dispatcher->trigger('onLsc07AfterSave', array('com_jtca.lsc07', $table, $is_new));

		$this->setState('lsc07.id', $table->id);
