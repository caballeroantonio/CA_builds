...
class JtCaModelLcp06 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp06';
	...
	{
		if (empty($config['lcp06_filter_fields']))
		{
	...
	*/
	...
	/**
	 * Method to get LIBRO DE TURNO PARA INCOMPETENCIAS ADOLESCENTES data.
	 *
	 * @param	integer	$pk	The id of the libro de turno para incompetencias adolescentes.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp06.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP06S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp06ChangeState', array('com_jtca.lcp06', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp06BeforeDelete', array('com_jtca.lcp06', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp06AfterDelete', array('com_jtca.lcp06', &$table));
				}
				else
				{
					// Prune items that you can't change.
