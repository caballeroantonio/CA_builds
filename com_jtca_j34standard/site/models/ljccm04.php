...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE REGISTRO DE PROMOCIONES Model
 *
 */
class JtCaModelLjccm04 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljccm04';
	...
	{
		if (empty($config['ljccm04_filter_fields']))
		{
			$config['ljccm04_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljccm04s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE REGISTRO DE PROMOCIONES data.
	 *
	 * @param	integer	$pk	The id of the libro de registro de promociones.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljccm04.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJCCM04S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjccm04ChangeState', array('com_jtca.ljccm04', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjccm04BeforeDelete', array('com_jtca.ljccm04', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjccm04AfterDelete', array('com_jtca.ljccm04', &$table));
				}
				else
				{
					// Prune items that you can't change.
