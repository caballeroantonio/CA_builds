...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsps12.php';

class JtcaModelLsps12Form extends JtcaModelLsps12
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsps12';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsps12';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsps12', 'lsps12', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the registro de promociones.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	REGISTRO DE PROMOCIONES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsps12.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsps12.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsps12.id');
		$is_new		= true;
		...
		// Reorder the registro de promociones so the new registro de promociones is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsps12BeforeSave', array('com_jtca.lsps12', &$table, $is_new));
		...
		$dispatcher->trigger('onLsps12AfterSave', array('com_jtca.lsps12', $table, $is_new));

		$this->setState('lsps12.id', $table->id);
