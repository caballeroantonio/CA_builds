...
/**
 * This models supports retrieving lists of libreta de  oficios de amparos de conocimiento..
 *
 */
class JtCaModelLcp16s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp16s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de  oficios de amparos de conocimiento. has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp16_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp16_id');
		$this->setState('filter.lcp16_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp16_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp16_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp16_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp16_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp16_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp16_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp16_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp16_id'));
		$id .= ':'.$this->getState('filter.lcp16_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp16s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp16s').' AS a');
		...
		// Filter by a single or group of lcp16s.
		$lcp16_id = $this->getState('filter.lcp16_id');
		if ($lcp16_id != '')
		{
			if (is_numeric($lcp16_id))
			{
				$type = $this->getState('filter.lcp16_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp16_id);
			}
			else
			{
				if (is_array($lcp16_id))
				{
					JArrayHelper::toInteger($lcp16_id);
					$lcp16_id = implode(',', $lcp16_id);
					$type = $this->getState('filter.lcp16_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp16_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp16_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp16_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de  oficios de amparos de conocimiento..
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp16_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp16 params override menu item params only if menu param = 'use_lcp16'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp16' and there is no lcp16 param, use global

					// create an array of just the params set to 'use_lcp16'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp16')
						{
							// if the lcp16 has a value, use it
							if ($lcp16_params->get($key) != '')
							{
								// get the value from the lcp16
								$lcp16_array[$key] = $lcp16_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp16_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp16 params
					if (count($lcp16_array) > 0)
					{
						$lcp16_params = new Registry;
						$lcp16_params->loadArray($lcp16_array);
						$item->params->merge($lcp16_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp16_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp16, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
