...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljoc14.php';

class JtCaModelLjoc14Form extends JtCaModelLjoc14
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljoc14';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljoc14';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljoc14', 'ljoc14', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljoc14.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljoc14.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljoc14.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjoc14BeforeSave', array('com_jtca.ljoc14', &$table, $is_new));
		...
		$dispatcher->trigger('onLjoc14AfterSave', array('com_jtca.ljoc14', $table, $is_new));

		$this->setState('ljoc14.id', $table->id);
