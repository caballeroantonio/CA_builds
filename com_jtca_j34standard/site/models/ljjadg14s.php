...
class JtCaModelLjjadg14s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadg14_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg14_id');
		$this->setState('filter.ljjadg14_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg14_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg14_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg14_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg14_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg14_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg14_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg14_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg14_id'));
		$id .= ':'.$this->getState('filter.ljjadg14_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg14s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg14s').' AS a');
		...
		// Filter by a single or group of ljjadg14s.
		$ljjadg14_id = $this->getState('filter.ljjadg14_id');
		if ($ljjadg14_id != '')
		{
			if (is_numeric($ljjadg14_id))
			{
				$type = $this->getState('filter.ljjadg14_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg14_id);
			}
			else
			{
				if (is_array($ljjadg14_id))
				{
					JArrayHelper::toInteger($ljjadg14_id);
					$ljjadg14_id = implode(',', $ljjadg14_id);
					$type = $this->getState('filter.ljjadg14_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg14_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg14_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg14_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg14_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg14 params override menu item params only if menu param = 'use_ljjadg14'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg14' and there is no ljjadg14 param, use global

					// create an array of just the params set to 'use_ljjadg14'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg14_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg14')
						{
							// if the ljjadg14 has a value, use it
							if ($ljjadg14_params->get($key) != '')
							{
								// get the value from the ljjadg14
								$ljjadg14_array[$key] = $ljjadg14_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg14_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg14 params
					if (count($ljjadg14_array) > 0)
					{
						$ljjadg14_params = new Registry;
						$ljjadg14_params->loadArray($ljjadg14_array);
						$item->params->merge($ljjadg14_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg14_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg14, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
