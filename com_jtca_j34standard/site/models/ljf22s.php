...
class JtCaModelLjf22s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf22_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf22_id');
		$this->setState('filter.ljf22_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf22_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf22_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf22_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf22_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf22_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf22_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf22_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf22_id'));
		$id .= ':'.$this->getState('filter.ljf22_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf22s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf22s').' AS a');
		...
		// Filter by a single or group of ljf22s.
		$ljf22_id = $this->getState('filter.ljf22_id');
		if ($ljf22_id != '')
		{
			if (is_numeric($ljf22_id))
			{
				$type = $this->getState('filter.ljf22_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf22_id);
			}
			else
			{
				if (is_array($ljf22_id))
				{
					JArrayHelper::toInteger($ljf22_id);
					$ljf22_id = implode(',', $ljf22_id);
					$type = $this->getState('filter.ljf22_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf22_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf22_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf22_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf22_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf22 params override menu item params only if menu param = 'use_ljf22'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf22' and there is no ljf22 param, use global

					// create an array of just the params set to 'use_ljf22'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf22')
						{
							// if the ljf22 has a value, use it
							if ($ljf22_params->get($key) != '')
							{
								// get the value from the ljf22
								$ljf22_array[$key] = $ljf22_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf22_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf22 params
					if (count($ljf22_array) > 0)
					{
						$ljf22_params = new Registry;
						$ljf22_params->loadArray($ljf22_array);
						$item->params->merge($ljf22_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf22_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf22, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
