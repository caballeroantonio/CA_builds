...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE CONTROL DE RESTITUCIONES INTERNACIONALES Model
 *
 */
class JtCaModelLjomf22 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljomf22';
	...
	{
		if (empty($config['ljomf22_filter_fields']))
		{
			$config['ljomf22_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljomf22s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE CONTROL DE RESTITUCIONES INTERNACIONALES data.
	 *
	 * @param	integer	$pk	The id of the libro de control de restituciones internacionales.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljomf22.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJOMF22S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjomf22ChangeState', array('com_jtca.ljomf22', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjomf22BeforeDelete', array('com_jtca.ljomf22', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjomf22AfterDelete', array('com_jtca.ljomf22', &$table));
				}
				else
				{
					// Prune items that you can't change.
