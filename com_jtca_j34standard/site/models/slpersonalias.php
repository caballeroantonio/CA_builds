...
/**
 * TSJ CDMX Libros TxCA Component Inculpado Model
 *
 */
class JtcaModelSlpersonalias extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.slpersonalias';
	...
	{
		if (empty($config['slpersonalias_filter_fields']))
		{
			$config['slpersonalias_filter_fields'] = array(
				'id', 'a.id',
	...
	protected function populateState()
	{
		...
		$this->setState('slpersonalias.id', $pk);
	...
	*/
	public function getTable($type = 'Slpersonaliases', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get Inculpado data.
	 *
	 * @param	integer	$pk	The id of the inculpado.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('slpersonalias.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_SLPERSONALIASES_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onSlpersonaliasChangeState', array('com_jtca.slpersonalias', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onSlpersonaliasBeforeDelete', array('com_jtca.slpersonalias', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onSlpersonaliasAfterDelete', array('com_jtca.slpersonalias', &$table));
				}
				else
				{
					// Prune items that you can't change.
