...
class JtCaModelLjc11s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljc11_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc11_id');
		$this->setState('filter.ljc11_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc11_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc11_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc11_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc11_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc11_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc11_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc11_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc11_id'));
		$id .= ':'.$this->getState('filter.ljc11_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc11s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc11s').' AS a');
		...
		// Filter by a single or group of ljc11s.
		$ljc11_id = $this->getState('filter.ljc11_id');
		if ($ljc11_id != '')
		{
			if (is_numeric($ljc11_id))
			{
				$type = $this->getState('filter.ljc11_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc11_id);
			}
			else
			{
				if (is_array($ljc11_id))
				{
					JArrayHelper::toInteger($ljc11_id);
					$ljc11_id = implode(',', $ljc11_id);
					$type = $this->getState('filter.ljc11_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc11_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc11_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc11_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc11_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc11 params override menu item params only if menu param = 'use_ljc11'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc11' and there is no ljc11 param, use global

					// create an array of just the params set to 'use_ljc11'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc11')
						{
							// if the ljc11 has a value, use it
							if ($ljc11_params->get($key) != '')
							{
								// get the value from the ljc11
								$ljc11_array[$key] = $ljc11_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc11_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc11 params
					if (count($ljc11_array) > 0)
					{
						$ljc11_params = new Registry;
						$ljc11_params->loadArray($ljc11_array);
						$item->params->merge($ljc11_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc11_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc11, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
