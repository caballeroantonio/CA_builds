...
class JtCaModelLjjadg18 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljjadg18';
	...
	{
		if (empty($config['ljjadg18_filter_fields']))
		{
			$config['ljjadg18_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljjadg18s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljjadg18.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJJADG18S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjjadg18ChangeState', array('com_jtca.ljjadg18', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjjadg18BeforeDelete', array('com_jtca.ljjadg18', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjjadg18AfterDelete', array('com_jtca.ljjadg18', &$table));
				}
				else
				{
					// Prune items that you can't change.
