...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE BENEFICIOS REVOCADOS Y ORDENES DE RE-APREHENSIÓN Model
 *
 */
class JtCaModelLjpes15 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpes15';
	...
	{
		if (empty($config['ljpes15_filter_fields']))
		{
			$config['ljpes15_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljpes15s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE BENEFICIOS REVOCADOS Y ORDENES DE RE-APREHENSIÓN data.
	 *
	 * @param	integer	$pk	The id of the libro de beneficios revocados y ordenes de re-aprehensión.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpes15.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPES15S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpes15ChangeState', array('com_jtca.ljpes15', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpes15BeforeDelete', array('com_jtca.ljpes15', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpes15AfterDelete', array('com_jtca.ljpes15', &$table));
				}
				else
				{
					// Prune items that you can't change.
