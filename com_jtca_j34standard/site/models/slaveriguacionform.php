...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/slaveriguacion.php';

class JtcaModelSlaveriguacionForm extends JtcaModelSlaveriguacion
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.slaveriguacion';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.slaveriguacion';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.slaveriguacion', 'slaveriguacion', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the averiguaciones previas.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Averiguaciones Previas item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('slaveriguacion.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.slaveriguacion.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('slaveriguacion.id');
		$is_new		= true;
		...
		// Reorder the averiguaciones previas so the new averiguaciones previas is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSlaveriguacionBeforeSave', array('com_jtca.slaveriguacion', &$table, $is_new));
		...
		$dispatcher->trigger('onSlaveriguacionAfterSave', array('com_jtca.slaveriguacion', $table, $is_new));

		$this->setState('slaveriguacion.id', $table->id);
