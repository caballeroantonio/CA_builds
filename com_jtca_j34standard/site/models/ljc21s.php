...
/**
 * This models supports retrieving lists of jc21.
 *
 */
class JtCaModelLjc21s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljc21s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de ministerio p̼blico has been specified either as a parameter or in the url Request
		$pk = $params->get('ljc21_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc21_id');
		$this->setState('filter.ljc21_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc21_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc21_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc21_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc21_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc21_initial_direction','ASC');
		}
		...
		{
			$this->setState('filter.archived', $params->get('filter_ljc21_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc21_id'));
		$id .= ':'.$this->getState('filter.ljc21_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc21s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc21s').' AS a');
		...
		// Filter by a single or group of ljc21s.
		$ljc21_id = $this->getState('filter.ljc21_id');
		if ($ljc21_id != '')
		{
			if (is_numeric($ljc21_id))
			{
				$type = $this->getState('filter.ljc21_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc21_id);
			}
			else
			{
				if (is_array($ljc21_id))
				{
					JArrayHelper::toInteger($ljc21_id);
					$ljc21_id = implode(',', $ljc21_id);
					$type = $this->getState('filter.ljc21_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc21_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc21_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc21_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc21_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc21 params override menu item params only if menu param = 'use_ljc21'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc21' and there is no ljc21 param, use global

					// create an array of just the params set to 'use_ljc21'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc21')
						{
							// if the ljc21 has a value, use it
							if ($ljc21_params->get($key) != '')
							{
								// get the value from the ljc21
								$ljc21_array[$key] = $ljc21_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc21_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc21 params
					if (count($ljc21_array) > 0)
					{
						$ljc21_params = new Registry;
						$ljc21_params->loadArray($ljc21_array);
						$item->params->merge($ljc21_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc21_date'))
					{
						case 'modified':
