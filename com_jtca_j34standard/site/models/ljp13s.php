...
class JtcaModelLjp13s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljp13_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp13_id');
		$this->setState('filter.ljp13_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp13_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp13_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp13_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp13_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp13_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp13_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp13_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp13_id'));
		$id .= ':'.$this->getState('filter.ljp13_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp13s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp13s').' AS a');
		...
		// Filter by a single or group of ljp13s.
		$ljp13_id = $this->getState('filter.ljp13_id');
		if ($ljp13_id != '')
		{
			if (is_numeric($ljp13_id))
			{
				$type = $this->getState('filter.ljp13_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp13_id);
			}
			else
			{
				if (is_array($ljp13_id))
				{
					JArrayHelper::toInteger($ljp13_id);
					$ljp13_id = implode(',', $ljp13_id);
					$type = $this->getState('filter.ljp13_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp13_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp13_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp13_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp13_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp13 params override menu item params only if menu param = 'use_ljp13'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp13' and there is no ljp13 param, use global

					// create an array of just the params set to 'use_ljp13'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp13')
						{
							// if the ljp13 has a value, use it
							if ($ljp13_params->get($key) != '')
							{
								// get the value from the ljp13
								$ljp13_array[$key] = $ljp13_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp13_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp13 params
					if (count($ljp13_array) > 0)
					{
						$ljp13_params = new Registry;
						$ljp13_params->loadArray($ljp13_array);
						$item->params->merge($ljp13_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp13_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp13, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
