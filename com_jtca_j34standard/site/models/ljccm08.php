 ...
 *
 */
class JtCaModelLjccm08 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljccm08';
	...
	{
		if (empty($config['ljccm08_filter_fields']))
		{
			$config['ljccm08_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljccm08s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljccm08.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJCCM08S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjccm08ChangeState', array('com_jtca.ljccm08', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjccm08BeforeDelete', array('com_jtca.ljccm08', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjccm08AfterDelete', array('com_jtca.ljccm08', &$table));
				}
				else
				{
					// Prune items that you can't change.
