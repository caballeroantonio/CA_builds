...
class JtCaModelLjc13 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljc13';
	...
	{
		if (empty($config['ljc13_filter_fields']))
		{
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljc13.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJC13S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjc13ChangeState', array('com_jtca.ljc13', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjc13BeforeDelete', array('com_jtca.ljc13', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjc13AfterDelete', array('com_jtca.ljc13', &$table));
				}
				else
				{
					// Prune items that you can't change.
