...
/**
 * This models supports retrieving lists of control de firmas del procesado.
 *
 */
class JtcaModelLsps13s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsps13s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single control de firmas del procesado has been specified either as a parameter or in the url Request
		$pk = $params->get('lsps13_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsps13_id');
		$this->setState('filter.lsps13_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsps13_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsps13_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsps13_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsps13_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsps13_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsps13_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsps13_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsps13_id'));
		$id .= ':'.$this->getState('filter.lsps13_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsps13s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsps13s').' AS a');
		...
		// Filter by a single or group of lsps13s.
		$lsps13_id = $this->getState('filter.lsps13_id');
		if ($lsps13_id != '')
		{
			if (is_numeric($lsps13_id))
			{
				$type = $this->getState('filter.lsps13_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsps13_id);
			}
			else
			{
				if (is_array($lsps13_id))
				{
					JArrayHelper::toInteger($lsps13_id);
					$lsps13_id = implode(',', $lsps13_id);
					$type = $this->getState('filter.lsps13_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsps13_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsps13_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsps13_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of control de firmas del procesado.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsps13_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsps13 params override menu item params only if menu param = 'use_lsps13'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsps13' and there is no lsps13 param, use global

					// create an array of just the params set to 'use_lsps13'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsps13')
						{
							// if the lsps13 has a value, use it
							if ($lsps13_params->get($key) != '')
							{
								// get the value from the lsps13
								$lsps13_array[$key] = $lsps13_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsps13_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsps13 params
					if (count($lsps13_array) > 0)
					{
						$lsps13_params = new Registry;
						$lsps13_params->loadArray($lsps13_array);
						$item->params->merge($lsps13_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsps13_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsps13, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
