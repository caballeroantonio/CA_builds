...
class JtCaModelLsps06s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('lsps06_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsps06_id');
		$this->setState('filter.lsps06_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsps06_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsps06_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsps06_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsps06_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsps06_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsps06_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsps06_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsps06_id'));
		$id .= ':'.$this->getState('filter.lsps06_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsps06s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsps06s').' AS a');
		...
		// Filter by a single or group of lsps06s.
		$lsps06_id = $this->getState('filter.lsps06_id');
		if ($lsps06_id != '')
		{
			if (is_numeric($lsps06_id))
			{
				$type = $this->getState('filter.lsps06_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsps06_id);
			}
			else
			{
				if (is_array($lsps06_id))
				{
					JArrayHelper::toInteger($lsps06_id);
					$lsps06_id = implode(',', $lsps06_id);
					$type = $this->getState('filter.lsps06_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsps06_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsps06_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsps06_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsps06_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsps06 params override menu item params only if menu param = 'use_lsps06'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsps06' and there is no lsps06 param, use global

					// create an array of just the params set to 'use_lsps06'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsps06')
						{
							// if the lsps06 has a value, use it
							if ($lsps06_params->get($key) != '')
							{
								// get the value from the lsps06
								$lsps06_array[$key] = $lsps06_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsps06_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsps06 params
					if (count($lsps06_array) > 0)
					{
						$lsps06_params = new Registry;
						$lsps06_params->loadArray($lsps06_array);
						$item->params->merge($lsps06_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsps06_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsps06, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
