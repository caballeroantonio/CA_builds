...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsps03.php';

class JtcaModelLsps03Form extends JtcaModelLsps03
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsps03';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsps03';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsps03', 'lsps03', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de gobierno (nuevo sistema) (colegiado).
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE GOBIERNO (NUEVO SISTEMA) (COLEGIADO) item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsps03.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsps03.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsps03.id');
		$is_new		= true;
		...
		// Reorder the libro de gobierno (nuevo sistema) (colegiado) so the new libro de gobierno (nuevo sistema) (colegiado) is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsps03BeforeSave', array('com_jtca.lsps03', &$table, $is_new));
		...
		$dispatcher->trigger('onLsps03AfterSave', array('com_jtca.lsps03', $table, $is_new));

		$this->setState('lsps03.id', $table->id);
