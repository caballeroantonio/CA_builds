...
/**
 * TSJ CDMX Libros TxCA Component Sub Libro DILIGENCIA Del LIBRO DE ACTUARIOS Model
 *
 */
class JtcaModelSldiliJpdng09 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.sldilijpdng09';
	...
	{
		if (empty($config['sldilijpdng09_filter_fields']))
		{
			$config['sldilijpdng09_filter_fields'] = array(
				'id', 'a.id',
				'id_record', 'a.id_record',
				'id_field', 'a.id_field',
				'id_record','a.id_record',
				'id_field','a.id_field',
				'sfield2_isMoral','a.sfield2_isMoral',
				'sfield2_paterno','a.sfield2_paterno',
				'sfield2_materno','a.sfield2_materno',
				'sfield2_nombre','a.sfield2_nombre',
				'sfield3','a.sfield3',
				'sfield4','a.sfield4',
				'sfield7','a.sfield7',
				'sfield8','a.sfield8',
	...
	protected function populateState()
	{
		...
		$this->setState('sldilijpdng09.id', $pk);
	...
	*/
	public function getTable($type = 'SldiliJpdng09s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get Sub Libro DILIGENCIA Del LIBRO DE ACTUARIOS data.
	 *
	 * @param	integer	$pk	The id of the sub libro diligencia del libro de actuarios.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('sldilijpdng09.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_SLDILI_JPDNG09S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onSldiliJpdng09ChangeState', array('com_jtca.sldilijpdng09', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onSldiliJpdng09BeforeDelete', array('com_jtca.sldilijpdng09', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onSldiliJpdng09AfterDelete', array('com_jtca.sldilijpdng09', &$table));
				}
				else
				{
					// Prune items that you can't change.
