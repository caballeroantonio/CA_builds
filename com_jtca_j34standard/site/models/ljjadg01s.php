...
class JtCaModelLjjadg01s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadg01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg01_id');
		$this->setState('filter.ljjadg01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg01_id'));
		$id .= ':'.$this->getState('filter.ljjadg01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg01s').' AS a');
		...
		// Filter by a single or group of ljjadg01s.
		$ljjadg01_id = $this->getState('filter.ljjadg01_id');
		if ($ljjadg01_id != '')
		{
			if (is_numeric($ljjadg01_id))
			{
				$type = $this->getState('filter.ljjadg01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg01_id);
			}
			else
			{
				if (is_array($ljjadg01_id))
				{
					JArrayHelper::toInteger($ljjadg01_id);
					$ljjadg01_id = implode(',', $ljjadg01_id);
					$type = $this->getState('filter.ljjadg01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg01_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg01 params override menu item params only if menu param = 'use_ljjadg01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg01' and there is no ljjadg01 param, use global

					// create an array of just the params set to 'use_ljjadg01'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg01_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg01')
						{
							// if the ljjadg01 has a value, use it
							if ($ljjadg01_params->get($key) != '')
							{
								// get the value from the ljjadg01
								$ljjadg01_array[$key] = $ljjadg01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg01 params
					if (count($ljjadg01_array) > 0)
					{
						$ljjadg01_params = new Registry;
						$ljjadg01_params->loadArray($ljjadg01_array);
						$item->params->merge($ljjadg01_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
