...
/**
 * This models supports retrieving lists of libro de actuarios.
 *
 */
class JtcaModelLjpdng09s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpdng09s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de actuarios has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpdng09_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng09_id');
		$this->setState('filter.ljpdng09_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng09_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng09_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng09_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng09_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng09_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng09_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng09_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng09_id'));
		$id .= ':'.$this->getState('filter.ljpdng09_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng09s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng09s').' AS a');
		...
		// Filter by a single or group of ljpdng09s.
		$ljpdng09_id = $this->getState('filter.ljpdng09_id');
		if ($ljpdng09_id != '')
		{
			if (is_numeric($ljpdng09_id))
			{
				$type = $this->getState('filter.ljpdng09_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng09_id);
			}
			else
			{
				if (is_array($ljpdng09_id))
				{
					JArrayHelper::toInteger($ljpdng09_id);
					$ljpdng09_id = implode(',', $ljpdng09_id);
					$type = $this->getState('filter.ljpdng09_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng09_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng09_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng09_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de actuarios.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng09_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng09 params override menu item params only if menu param = 'use_ljpdng09'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng09' and there is no ljpdng09 param, use global

					// create an array of just the params set to 'use_ljpdng09'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng09_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng09')
						{
							// if the ljpdng09 has a value, use it
							if ($ljpdng09_params->get($key) != '')
							{
								// get the value from the ljpdng09
								$ljpdng09_array[$key] = $ljpdng09_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng09_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng09 params
					if (count($ljpdng09_array) > 0)
					{
						$ljpdng09_params = new Registry;
						$ljpdng09_params->loadArray($ljpdng09_array);
						$item->params->merge($ljpdng09_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng09_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng09, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
