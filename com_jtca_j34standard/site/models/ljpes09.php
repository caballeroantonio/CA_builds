...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE MULTAS JUDICIALES Model
 *
 */
class JtCaModelLjpes09 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpes09';
	...
	{
		if (empty($config['ljpes09_filter_fields']))
		{
			$config['ljpes09_filter_fields'] = array(
				'id', 'a.id',
				...
				'field3','a.field3',
				'field4','a.field4',
				'txt_field1248','a.txt_field1248',
				'id_field1248','a.id_field1248',
				'field7','a.field7',
				'field8','a.field8',
				'field6','a.field6',
				'state', 'a.state',
	...
	*/
	public function getTable($type = 'Ljpes09s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE MULTAS JUDICIALES data.
	 *
	 * @param	integer	$pk	The id of the libro de multas judiciales.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpes09.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPES09S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpes09ChangeState', array('com_jtca.ljpes09', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpes09BeforeDelete', array('com_jtca.ljpes09', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpes09AfterDelete', array('com_jtca.ljpes09', &$table));
				}
				else
				{
					// Prune items that you can't change.
