...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadng11.php';

class JtCaModelLjjadng11Form extends JtCaModelLjjadng11
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadng11';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadng11';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadng11', 'ljjadng11', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de remisión al archivo.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE REMISIÓN AL ARCHIVO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadng11.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadng11.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadng11.id');
		$is_new		= true;
		...
		// Reorder the libro de remisión al archivo so the new libro de remisión al archivo is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadng11BeforeSave', array('com_jtca.ljjadng11', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadng11AfterSave', array('com_jtca.ljjadng11', $table, $is_new));

		$this->setState('ljjadng11.id', $table->id);
