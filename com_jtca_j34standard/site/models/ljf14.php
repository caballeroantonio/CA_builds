 ...
 *
 */
class JtcaModelLjf14 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljf14';
	...
	{
		if (empty($config['ljf14_filter_fields']))
		{
			$config['ljf14_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljf14s', $prefix = 'JtcaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljf14.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJF14S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjf14ChangeState', array('com_jtca.ljf14', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjf14BeforeDelete', array('com_jtca.ljf14', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjf14AfterDelete', array('com_jtca.ljf14', &$table));
				}
				else
				{
					// Prune items that you can't change.
