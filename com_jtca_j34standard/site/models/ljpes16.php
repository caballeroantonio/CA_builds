...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE DESISTIMIENTOS Model
 *
 */
class JtCaModelLjpes16 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpes16';
	...
	{
		if (empty($config['ljpes16_filter_fields']))
		{
			$config['ljpes16_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	 ...
	 *
	 * @param	integer	$pk	The id of the libro de desistimientos.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpes16.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPES16S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpes16ChangeState', array('com_jtca.ljpes16', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpes16BeforeDelete', array('com_jtca.ljpes16', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpes16AfterDelete', array('com_jtca.ljpes16', &$table));
				}
				else
				{
					// Prune items that you can't change.
