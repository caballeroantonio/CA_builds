...
/**
 * This models supports retrieving lists of libro de turno para sentencia.
 *
 */
class JtCaModelLjccm05s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljccm05s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljccm05_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljccm05_id');
		$this->setState('filter.ljccm05_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljccm05_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljccm05_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljccm05_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljccm05_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljccm05_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljccm05_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljccm05_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljccm05_id'));
		$id .= ':'.$this->getState('filter.ljccm05_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljccm05s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljccm05s').' AS a');
		...
		// Filter by a single or group of ljccm05s.
		$ljccm05_id = $this->getState('filter.ljccm05_id');
		if ($ljccm05_id != '')
		{
			if (is_numeric($ljccm05_id))
			{
				$type = $this->getState('filter.ljccm05_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljccm05_id);
			}
			else
			{
				if (is_array($ljccm05_id))
				{
					JArrayHelper::toInteger($ljccm05_id);
					$ljccm05_id = implode(',', $ljccm05_id);
					$type = $this->getState('filter.ljccm05_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljccm05_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljccm05_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljccm05_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno para sentencia.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljccm05_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljccm05 params override menu item params only if menu param = 'use_ljccm05'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljccm05' and there is no ljccm05 param, use global

					// create an array of just the params set to 'use_ljccm05'
					$menu_params_array = $this->getState('params')->toArray();
					$ljccm05_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljccm05')
						{
							// if the ljccm05 has a value, use it
							if ($ljccm05_params->get($key) != '')
							{
								// get the value from the ljccm05
								$ljccm05_array[$key] = $ljccm05_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljccm05_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljccm05 params
					if (count($ljccm05_array) > 0)
					{
						$ljccm05_params = new Registry;
						$ljccm05_params->loadArray($ljccm05_array);
						$item->params->merge($ljccm05_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljccm05_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljccm05, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
