...
/**
 * This models supports retrieving lists of libro de amparos.
 *
 */
class JtCaModelLjjadng08s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadng08s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de amparos has been specified either as a parameter or in the url Request
		$pk = $params->get('ljjadng08_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadng08_id');
		$this->setState('filter.ljjadng08_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadng08_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadng08_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadng08_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadng08_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadng08_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadng08_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadng08_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadng08_id'));
		$id .= ':'.$this->getState('filter.ljjadng08_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadng08s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadng08s').' AS a');
		...
		// Filter by a single or group of ljjadng08s.
		$ljjadng08_id = $this->getState('filter.ljjadng08_id');
		if ($ljjadng08_id != '')
		{
			if (is_numeric($ljjadng08_id))
			{
				$type = $this->getState('filter.ljjadng08_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadng08_id);
			}
			else
			{
				if (is_array($ljjadng08_id))
				{
					JArrayHelper::toInteger($ljjadng08_id);
					$ljjadng08_id = implode(',', $ljjadng08_id);
					$type = $this->getState('filter.ljjadng08_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadng08_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadng08_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadng08_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de amparos.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadng08_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadng08 params override menu item params only if menu param = 'use_ljjadng08'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadng08' and there is no ljjadng08 param, use global

					// create an array of just the params set to 'use_ljjadng08'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadng08_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadng08')
						{
							// if the ljjadng08 has a value, use it
							if ($ljjadng08_params->get($key) != '')
							{
								// get the value from the ljjadng08
								$ljjadng08_array[$key] = $ljjadng08_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadng08_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadng08 params
					if (count($ljjadng08_array) > 0)
					{
						$ljjadng08_params = new Registry;
						$ljjadng08_params->loadArray($ljjadng08_array);
						$item->params->merge($ljjadng08_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadng08_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadng08, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
