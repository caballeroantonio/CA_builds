...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljc17.php';

class JtCaModelLjc17Form extends JtCaModelLjc17
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljc17';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljc17';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljc17', 'ljc17', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljc17.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljc17.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljc17.id');
		$is_new		= true;
		...
		// Reorder the jc17 so the new libro de remisión al archivo is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjc17BeforeSave', array('com_jtca.ljc17', &$table, $is_new));
		...
		$dispatcher->trigger('onLjc17AfterSave', array('com_jtca.ljc17', $table, $is_new));

		$this->setState('ljc17.id', $table->id);
