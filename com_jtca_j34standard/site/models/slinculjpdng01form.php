...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/slinculjpdng01.php';

class JtcaModelSlinculJpdng01Form extends JtcaModelSlinculJpdng01
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.slinculjpdng01';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.slinculjpdng01';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.slinculjpdng01', 'slinculjpdng01', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the sub libro datos del(os) inculpado(s) del libro de gobierno.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Sub Libro DATOS DEL(OS) INCULPADO(S) Del LIBRO DE GOBIERNO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('slinculjpdng01.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.slinculjpdng01.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('slinculjpdng01.id');
		$is_new		= true;
		...
		// Reorder the datos del(os) inculpado(s) del libro de gobierno so the new sub libro datos del(os) inculpado(s) del libro de gobierno is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSlinculJpdng01BeforeSave', array('com_jtca.slinculjpdng01', &$table, $is_new));
		...
		$dispatcher->trigger('onSlinculJpdng01AfterSave', array('com_jtca.slinculjpdng01', $table, $is_new));

		$this->setState('slinculjpdng01.id', $table->id);
