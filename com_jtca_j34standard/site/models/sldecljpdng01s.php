...
/**
 * This models supports retrieving lists of declaracion preparatoria del libro de gobierno.
 *
 */
class JtcaModelSldeclJpdng01s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.sldecl_jpdng01s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single sub libro declaracion preparatoria del libro de gobierno has been specified either as a parameter or in the url Request
		$pk = $params->get('sldecl_jpdng01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('sldecl_jpdng01_id');
		$this->setState('filter.sldecl_jpdng01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('sldecljpdng01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('sldecljpdng01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('sldecljpdng01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('sldecljpdng01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('sldecljpdng01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_sldecljpdng01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_sldecljpdng01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.sldecl_jpdng01_id'));
		$id .= ':'.$this->getState('filter.sldecl_jpdng01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of sldecl_jpdng01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_sldecl_jpdng01s').' AS a');
		...
		// Filter by a single or group of sldecl_jpdng01s.
		$sldecl_jpdng01_id = $this->getState('filter.sldecl_jpdng01_id');
		if ($sldecl_jpdng01_id != '')
		{
			if (is_numeric($sldecl_jpdng01_id))
			{
				$type = $this->getState('filter.sldecl_jpdng01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $sldecl_jpdng01_id);
			}
			else
			{
				if (is_array($sldecl_jpdng01_id))
				{
					JArrayHelper::toInteger($sldecl_jpdng01_id);
					$sldecl_jpdng01_id = implode(',', $sldecl_jpdng01_id);
					$type = $this->getState('filter.sldecl_jpdng01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$sldecl_jpdng01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_sldecljpdng01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_sldecljpdng01_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of declaracion preparatoria del libro de gobierno.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$sldecl_jpdng01_params = new Registry;
				...
				{
					$item->params = $sldecl_jpdng01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// SldeclJpdng01 params override menu item params only if menu param = 'use_sldecljpdng01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_sldecljpdng01' and there is no sldecljpdng01 param, use global

					// create an array of just the params set to 'use_sldecljpdng01'
					$menu_params_array = $this->getState('params')->toArray();
					$sldecl_jpdng01_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_sldecljpdng01')
						{
							// if the sldecljpdng01 has a value, use it
							if ($sldecl_jpdng01_params->get($key) != '')
							{
								// get the value from the sldecljpdng01
								$sldecl_jpdng01_array[$key] = $sldecl_jpdng01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$sldecl_jpdng01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected sldecljpdng01 params
					if (count($sldecl_jpdng01_array) > 0)
					{
						$sldecl_jpdng01_params = new Registry;
						$sldecl_jpdng01_params->loadArray($sldecl_jpdng01_array);
						$item->params->merge($sldecl_jpdng01_params);
					}


					// get display date
					switch ($item->params->get('list_show_sldecljpdng01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an sldecljpdng01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
