...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE TURNO PARA SENTENCIA Model
 *
 */
class JtCaModelLjccm05 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljccm05';
	...
	{
		if (empty($config['ljccm05_filter_fields']))
		{
			$config['ljccm05_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljccm05s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE TURNO PARA SENTENCIA data.
	 *
	 * @param	integer	$pk	The id of the libro de turno para sentencia.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljccm05.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJCCM05S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjccm05ChangeState', array('com_jtca.ljccm05', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjccm05BeforeDelete', array('com_jtca.ljccm05', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjccm05AfterDelete', array('com_jtca.ljccm05', &$table));
				}
				else
				{
					// Prune items that you can't change.
