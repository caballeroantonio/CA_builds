...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljoc09.php';

class JtCaModelLjoc09Form extends JtCaModelLjoc09
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljoc09';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljoc09';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljoc09', 'ljoc09', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljoc09.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljoc09.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljoc09.id');
		$is_new		= true;
		...
		// Reorder the libro de auxiliares de la administración de justic so the new libro de auxiliares de la administración de justicia is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjoc09BeforeSave', array('com_jtca.ljoc09', &$table, $is_new));
		...
		$dispatcher->trigger('onLjoc09AfterSave', array('com_jtca.ljoc09', $table, $is_new));

		$this->setState('ljoc09.id', $table->id);
