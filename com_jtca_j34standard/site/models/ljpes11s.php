...
/**
 * This models supports retrieving lists of libro de ingresos y egresos de billetes de depósit.
 *
 */
class JtcaModelLjpes11s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpes11s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de ingresos y egresos de billetes de depósito has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpes11_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes11_id');
		$this->setState('filter.ljpes11_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes11_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes11_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes11_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes11_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes11_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes11_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes11_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes11_id'));
		$id .= ':'.$this->getState('filter.ljpes11_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes11s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes11s').' AS a');
		...
		// Filter by a single or group of ljpes11s.
		$ljpes11_id = $this->getState('filter.ljpes11_id');
		if ($ljpes11_id != '')
		{
			if (is_numeric($ljpes11_id))
			{
				$type = $this->getState('filter.ljpes11_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes11_id);
			}
			else
			{
				if (is_array($ljpes11_id))
				{
					JArrayHelper::toInteger($ljpes11_id);
					$ljpes11_id = implode(',', $ljpes11_id);
					$type = $this->getState('filter.ljpes11_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes11_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes11_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes11_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de ingresos y egresos de billetes de depósit.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes11_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes11 params override menu item params only if menu param = 'use_ljpes11'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes11' and there is no ljpes11 param, use global

					// create an array of just the params set to 'use_ljpes11'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes11_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes11')
						{
							// if the ljpes11 has a value, use it
							if ($ljpes11_params->get($key) != '')
							{
								// get the value from the ljpes11
								$ljpes11_array[$key] = $ljpes11_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes11_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes11 params
					if (count($ljpes11_array) > 0)
					{
						$ljpes11_params = new Registry;
						$ljpes11_params->loadArray($ljpes11_array);
						$item->params->merge($ljpes11_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes11_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes11, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
