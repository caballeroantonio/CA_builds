...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljccm11.php';

class JtcaModelLjccm11Form extends JtcaModelLjccm11
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljccm11';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljccm11';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljccm11', 'ljccm11', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljccm11.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljccm11.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljccm11.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjccm11BeforeSave', array('com_jtca.ljccm11', &$table, $is_new));
		...
		$dispatcher->trigger('onLjccm11AfterSave', array('com_jtca.ljccm11', $table, $is_new));

		$this->setState('ljccm11.id', $table->id);
