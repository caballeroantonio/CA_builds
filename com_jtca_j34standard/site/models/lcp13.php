...
class JtcaModelLcp13 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp13';
	...
	{
		if (empty($config['lcp13_filter_fields']))
		{
	...
	*/
	...
	/**
	 * Method to get LIBRETA DE EXPEDIENTILLOS DE JUZGADOS DE EJECUCIÓN PARA JUZGADOS PENAL O NO GRAVES. data.
	 *
	 * @param	integer	$pk	The id of the libreta de expedientillos de juzgados de ejecución para juzgados penal o no graves..
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp13.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP13S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp13ChangeState', array('com_jtca.lcp13', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp13BeforeDelete', array('com_jtca.lcp13', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp13AfterDelete', array('com_jtca.lcp13', &$table));
				}
				else
				{
					// Prune items that you can't change.
