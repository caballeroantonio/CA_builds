...
/**
 * This models supports retrieving lists of registro de billetes de deposito.
 *
 */
class JtcaModelLsps07s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsps07s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single registro de billetes de deposito has been specified either as a parameter or in the url Request
		$pk = $params->get('lsps07_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsps07_id');
		$this->setState('filter.lsps07_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsps07_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsps07_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsps07_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsps07_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsps07_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsps07_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsps07_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsps07_id'));
		$id .= ':'.$this->getState('filter.lsps07_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsps07s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsps07s').' AS a');
		...
		// Filter by a single or group of lsps07s.
		$lsps07_id = $this->getState('filter.lsps07_id');
		if ($lsps07_id != '')
		{
			if (is_numeric($lsps07_id))
			{
				$type = $this->getState('filter.lsps07_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsps07_id);
			}
			else
			{
				if (is_array($lsps07_id))
				{
					JArrayHelper::toInteger($lsps07_id);
					$lsps07_id = implode(',', $lsps07_id);
					$type = $this->getState('filter.lsps07_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsps07_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsps07_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsps07_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of registro de billetes de deposito.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsps07_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsps07 params override menu item params only if menu param = 'use_lsps07'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsps07' and there is no lsps07 param, use global

					// create an array of just the params set to 'use_lsps07'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsps07')
						{
							// if the lsps07 has a value, use it
							if ($lsps07_params->get($key) != '')
							{
								// get the value from the lsps07
								$lsps07_array[$key] = $lsps07_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsps07_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsps07 params
					if (count($lsps07_array) > 0)
					{
						$lsps07_params = new Registry;
						$lsps07_params->loadArray($lsps07_array);
						$item->params->merge($lsps07_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsps07_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsps07, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
