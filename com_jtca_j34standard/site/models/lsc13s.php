...
/**
 * This models supports retrieving lists of papeletas para el pr̩stamo de expedientes.
 *
 */
class JtCaModelLsc13s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsc13s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single papeletas para el pr̩stamo de expedientes has been specified either as a parameter or in the url Request
		$pk = $params->get('lsc13_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsc13_id');
		$this->setState('filter.lsc13_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsc13_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsc13_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsc13_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsc13_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsc13_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsc13_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsc13_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsc13_id'));
		$id .= ':'.$this->getState('filter.lsc13_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsc13s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsc13s').' AS a');
		...
		// Filter by a single or group of lsc13s.
		$lsc13_id = $this->getState('filter.lsc13_id');
		if ($lsc13_id != '')
		{
			if (is_numeric($lsc13_id))
			{
				$type = $this->getState('filter.lsc13_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsc13_id);
			}
			else
			{
				if (is_array($lsc13_id))
				{
					JArrayHelper::toInteger($lsc13_id);
					$lsc13_id = implode(',', $lsc13_id);
					$type = $this->getState('filter.lsc13_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsc13_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsc13_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsc13_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of papeletas para el pr̩stamo de expedientes.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsc13_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsc13 params override menu item params only if menu param = 'use_lsc13'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsc13' and there is no lsc13 param, use global

					// create an array of just the params set to 'use_lsc13'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsc13')
						{
							// if the lsc13 has a value, use it
							if ($lsc13_params->get($key) != '')
							{
								// get the value from the lsc13
								$lsc13_array[$key] = $lsc13_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsc13_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsc13 params
					if (count($lsc13_array) > 0)
					{
						$lsc13_params = new Registry;
						$lsc13_params->loadArray($lsc13_array);
						$item->params->merge($lsc13_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsc13_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsc13, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
