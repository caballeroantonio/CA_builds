...
/**
 * This models supports retrieving lists of libro de egresos (oficial).
 *
 */
class JtcaModelLsc07s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsc07s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de egresos (oficial) has been specified either as a parameter or in the url Request
		$pk = $params->get('lsc07_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsc07_id');
		$this->setState('filter.lsc07_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsc07_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsc07_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsc07_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsc07_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsc07_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsc07_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsc07_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsc07_id'));
		$id .= ':'.$this->getState('filter.lsc07_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsc07s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsc07s').' AS a');
		...
		// Filter by a single or group of lsc07s.
		$lsc07_id = $this->getState('filter.lsc07_id');
		if ($lsc07_id != '')
		{
			if (is_numeric($lsc07_id))
			{
				$type = $this->getState('filter.lsc07_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsc07_id);
			}
			else
			{
				if (is_array($lsc07_id))
				{
					JArrayHelper::toInteger($lsc07_id);
					$lsc07_id = implode(',', $lsc07_id);
					$type = $this->getState('filter.lsc07_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsc07_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsc07_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsc07_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de egresos (oficial).
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsc07_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsc07 params override menu item params only if menu param = 'use_lsc07'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsc07' and there is no lsc07 param, use global

					// create an array of just the params set to 'use_lsc07'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsc07')
						{
							// if the lsc07 has a value, use it
							if ($lsc07_params->get($key) != '')
							{
								// get the value from the lsc07
								$lsc07_array[$key] = $lsc07_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsc07_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsc07 params
					if (count($lsc07_array) > 0)
					{
						$lsc07_params = new Registry;
						$lsc07_params->loadArray($lsc07_array);
						$item->params->merge($lsc07_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsc07_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsc07, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
