...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljf20.php';

class JtCaModelLjf20Form extends JtCaModelLjf20
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljf20';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljf20';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljf20', 'ljf20', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljf20.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljf20.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljf20.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjf20BeforeSave', array('com_jtca.ljf20', &$table, $is_new));
		...
		$dispatcher->trigger('onLjf20AfterSave', array('com_jtca.ljf20', $table, $is_new));

		$this->setState('ljf20.id', $table->id);
