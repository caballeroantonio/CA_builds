...
class JtCaModelLjoc08s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljoc08_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljoc08_id');
		$this->setState('filter.ljoc08_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljoc08_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljoc08_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljoc08_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljoc08_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljoc08_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljoc08_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljoc08_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljoc08_id'));
		$id .= ':'.$this->getState('filter.ljoc08_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljoc08s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljoc08s').' AS a');
		...
		// Filter by a single or group of ljoc08s.
		$ljoc08_id = $this->getState('filter.ljoc08_id');
		if ($ljoc08_id != '')
		{
			if (is_numeric($ljoc08_id))
			{
				$type = $this->getState('filter.ljoc08_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljoc08_id);
			}
			else
			{
				if (is_array($ljoc08_id))
				{
					JArrayHelper::toInteger($ljoc08_id);
					$ljoc08_id = implode(',', $ljoc08_id);
					$type = $this->getState('filter.ljoc08_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljoc08_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljoc08_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljoc08_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljoc08_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljoc08 params override menu item params only if menu param = 'use_ljoc08'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljoc08' and there is no ljoc08 param, use global

					// create an array of just the params set to 'use_ljoc08'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljoc08')
						{
							// if the ljoc08 has a value, use it
							if ($ljoc08_params->get($key) != '')
							{
								// get the value from the ljoc08
								$ljoc08_array[$key] = $ljoc08_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljoc08_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljoc08 params
					if (count($ljoc08_array) > 0)
					{
						$ljoc08_params = new Registry;
						$ljoc08_params->loadArray($ljoc08_array);
						$item->params->merge($ljoc08_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljoc08_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljoc08, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
