...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadg01.php';

class JtCaModelLjjadg01Form extends JtCaModelLjjadg01
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadg01';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadg01';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadg01', 'ljjadg01', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadg01.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadg01.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadg01.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjjadg01BeforeSave', array('com_jtca.ljjadg01', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadg01AfterSave', array('com_jtca.ljjadg01', $table, $is_new));

		$this->setState('ljjadg01.id', $table->id);
