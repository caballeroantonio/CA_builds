 ...
 *
 */
class JtCaModelLjf04 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljf04';
	...
	{
		if (empty($config['ljf04_filter_fields']))
		{
			$config['ljf04_filter_fields'] = array(
				'id', 'a.id',
				...
				'field5_materno','a.field5_materno',
				'field5_nombre','a.field5_nombre',
				'field6','a.field6',
				'field8','a.field8',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Ljf04s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljf04.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJF04S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjf04ChangeState', array('com_jtca.ljf04', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjf04BeforeDelete', array('com_jtca.ljf04', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjf04AfterDelete', array('com_jtca.ljf04', &$table));
				}
				else
				{
					// Prune items that you can't change.
