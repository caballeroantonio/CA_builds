...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/slofenjpdng01.php';

class JtcaModelSlofenJpdng01Form extends JtcaModelSlofenJpdng01
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.slofenjpdng01';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.slofenjpdng01';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.slofenjpdng01', 'slofenjpdng01', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the sub libro datos del(os) ofendido(s) del libro de gobierno.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Sub Libro DATOS DEL(OS) OFENDIDO(S) Del LIBRO DE GOBIERNO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('slofenjpdng01.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.slofenjpdng01.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('slofenjpdng01.id');
		$is_new		= true;
		...
		// Reorder the datos del(os) ofendido(s) del libro de gobierno so the new sub libro datos del(os) ofendido(s) del libro de gobierno is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSlofenJpdng01BeforeSave', array('com_jtca.slofenjpdng01', &$table, $is_new));
		...
		$dispatcher->trigger('onSlofenJpdng01AfterSave', array('com_jtca.slofenjpdng01', $table, $is_new));

		$this->setState('slofenjpdng01.id', $table->id);
