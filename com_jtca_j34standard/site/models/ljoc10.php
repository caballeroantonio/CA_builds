 ...
 *
 */
class JtCaModelLjoc10 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljoc10';
	...
	{
		if (empty($config['ljoc10_filter_fields']))
		{
			$config['ljoc10_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljoc10s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljoc10.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJOC10S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjoc10ChangeState', array('com_jtca.ljoc10', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjoc10BeforeDelete', array('com_jtca.ljoc10', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjoc10AfterDelete', array('com_jtca.ljoc10', &$table));
				}
				else
				{
					// Prune items that you can't change.
