...
/**
 * This models supports retrieving lists of jc04.
 *
 */
class JtCaModelLjc04s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljc04s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de registro de promociones has been specified either as a parameter or in the url Request
		$pk = $params->get('ljc04_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc04_id');
		$this->setState('filter.ljc04_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc04_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc04_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc04_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc04_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc04_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc04_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc04_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc04_id'));
		$id .= ':'.$this->getState('filter.ljc04_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc04s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc04s').' AS a');
		...
		// Filter by a single or group of ljc04s.
		$ljc04_id = $this->getState('filter.ljc04_id');
		if ($ljc04_id != '')
		{
			if (is_numeric($ljc04_id))
			{
				$type = $this->getState('filter.ljc04_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc04_id);
			}
			else
			{
				if (is_array($ljc04_id))
				{
					JArrayHelper::toInteger($ljc04_id);
					$ljc04_id = implode(',', $ljc04_id);
					$type = $this->getState('filter.ljc04_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc04_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc04_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc04_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc04_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc04 params override menu item params only if menu param = 'use_ljc04'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc04' and there is no ljc04 param, use global

					// create an array of just the params set to 'use_ljc04'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc04')
						{
							// if the ljc04 has a value, use it
							if ($ljc04_params->get($key) != '')
							{
								// get the value from the ljc04
								$ljc04_array[$key] = $ljc04_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc04_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc04 params
					if (count($ljc04_array) > 0)
					{
						$ljc04_params = new Registry;
						$ljc04_params->loadArray($ljc04_array);
						$item->params->merge($ljc04_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc04_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc04, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
