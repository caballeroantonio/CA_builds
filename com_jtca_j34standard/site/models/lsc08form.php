...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsc08.php';

class JtCaModelLsc08Form extends JtCaModelLsc08
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsc08';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsc08';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsc08', 'lsc08', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de pólizas y fianzas (oficial) - ingresos.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE PÓLIZAS Y FIANZAS (OFICIAL) - Ingresos item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsc08.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsc08.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsc08.id');
		$is_new		= true;
		...
		// Reorder the libro de pólizas y fianzas (oficial) - ingresos so the new libro de pólizas y fianzas (oficial) - ingresos is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsc08BeforeSave', array('com_jtca.lsc08', &$table, $is_new));
		...
		$dispatcher->trigger('onLsc08AfterSave', array('com_jtca.lsc08', $table, $is_new));

		$this->setState('lsc08.id', $table->id);
