...
class JtCaModelLjemst01s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljemst01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljemst01_id');
		$this->setState('filter.ljemst01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljemst01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljemst01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljemst01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljemst01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljemst01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljemst01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljemst01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljemst01_id'));
		$id .= ':'.$this->getState('filter.ljemst01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljemst01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljemst01s').' AS a');
		...
		// Filter by a single or group of ljemst01s.
		$ljemst01_id = $this->getState('filter.ljemst01_id');
		if ($ljemst01_id != '')
		{
			if (is_numeric($ljemst01_id))
			{
				$type = $this->getState('filter.ljemst01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljemst01_id);
			}
			else
			{
				if (is_array($ljemst01_id))
				{
					JArrayHelper::toInteger($ljemst01_id);
					$ljemst01_id = implode(',', $ljemst01_id);
					$type = $this->getState('filter.ljemst01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljemst01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljemst01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljemst01_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljemst01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljemst01 params override menu item params only if menu param = 'use_ljemst01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljemst01' and there is no ljemst01 param, use global

					// create an array of just the params set to 'use_ljemst01'
					$menu_params_array = $this->getState('params')->toArray();
					$ljemst01_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljemst01')
						{
							// if the ljemst01 has a value, use it
							if ($ljemst01_params->get($key) != '')
							{
								// get the value from the ljemst01
								$ljemst01_array[$key] = $ljemst01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljemst01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljemst01 params
					if (count($ljemst01_array) > 0)
					{
						$ljemst01_params = new Registry;
						$ljemst01_params->loadArray($ljemst01_array);
						$item->params->merge($ljemst01_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljemst01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljemst01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
