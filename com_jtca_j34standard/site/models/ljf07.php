 ...
 *
 */
class JtCaModelLjf07 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljf07';
	...
	{
		if (empty($config['ljf07_filter_fields']))
		{
			$config['ljf07_filter_fields'] = array(
				'id', 'a.id',
				...
				'field7','a.field7',
				'field8','a.field8',
				'txt_field1967','a.txt_field1967',
				'id_field1967','a.id_field1967',
				'field10','a.field10',
				'field11','a.field11',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Ljf07s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljf07.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJF07S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjf07ChangeState', array('com_jtca.ljf07', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjf07BeforeDelete', array('com_jtca.ljf07', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjf07AfterDelete', array('com_jtca.ljf07', &$table));
				}
				else
				{
					// Prune items that you can't change.
