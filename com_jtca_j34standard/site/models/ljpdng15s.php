...
class JtCaModelLjpdng15s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpdng15_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng15_id');
		$this->setState('filter.ljpdng15_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng15_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng15_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng15_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng15_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng15_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng15_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng15_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng15_id'));
		$id .= ':'.$this->getState('filter.ljpdng15_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng15s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng15s').' AS a');
		...
		// Filter by a single or group of ljpdng15s.
		$ljpdng15_id = $this->getState('filter.ljpdng15_id');
		if ($ljpdng15_id != '')
		{
			if (is_numeric($ljpdng15_id))
			{
				$type = $this->getState('filter.ljpdng15_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng15_id);
			}
			else
			{
				if (is_array($ljpdng15_id))
				{
					JArrayHelper::toInteger($ljpdng15_id);
					$ljpdng15_id = implode(',', $ljpdng15_id);
					$type = $this->getState('filter.ljpdng15_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng15_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng15_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng15_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng15_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng15 params override menu item params only if menu param = 'use_ljpdng15'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng15' and there is no ljpdng15 param, use global

					// create an array of just the params set to 'use_ljpdng15'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng15_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng15')
						{
							// if the ljpdng15 has a value, use it
							if ($ljpdng15_params->get($key) != '')
							{
								// get the value from the ljpdng15
								$ljpdng15_array[$key] = $ljpdng15_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng15_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng15 params
					if (count($ljpdng15_array) > 0)
					{
						$ljpdng15_params = new Registry;
						$ljpdng15_params->loadArray($ljpdng15_array);
						$item->params->merge($ljpdng15_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng15_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng15, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
