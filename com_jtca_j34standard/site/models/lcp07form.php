...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp07.php';

class JtCaModelLcp07Form extends JtCaModelLcp07
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp07';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp07';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp07', 'lcp07', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno para excusas de adolescentes.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO PARA EXCUSAS DE ADOLESCENTES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp07.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp07.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp07.id');
		$is_new		= true;
		...
		// Reorder the libro de turno para excusas de adolescentes so the new libro de turno para excusas de adolescentes is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp07BeforeSave', array('com_jtca.lcp07', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp07AfterSave', array('com_jtca.lcp07', $table, $is_new));

		$this->setState('lcp07.id', $table->id);
