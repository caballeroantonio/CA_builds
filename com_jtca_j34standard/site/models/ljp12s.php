...
/**
 * This models supports retrieving lists of libro de registro de amparos circulantes o negados.
 *
 */
class JtCaModelLjp12s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljp12s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de registro de amparos circulantes o negados has been specified either as a parameter or in the url Request
		$pk = $params->get('ljp12_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp12_id');
		$this->setState('filter.ljp12_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp12_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp12_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp12_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp12_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp12_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp12_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp12_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp12_id'));
		$id .= ':'.$this->getState('filter.ljp12_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp12s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp12s').' AS a');
		...
		// Filter by a single or group of ljp12s.
		$ljp12_id = $this->getState('filter.ljp12_id');
		if ($ljp12_id != '')
		{
			if (is_numeric($ljp12_id))
			{
				$type = $this->getState('filter.ljp12_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp12_id);
			}
			else
			{
				if (is_array($ljp12_id))
				{
					JArrayHelper::toInteger($ljp12_id);
					$ljp12_id = implode(',', $ljp12_id);
					$type = $this->getState('filter.ljp12_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp12_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp12_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp12_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de registro de amparos circulantes o negados.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp12_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp12 params override menu item params only if menu param = 'use_ljp12'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp12' and there is no ljp12 param, use global

					// create an array of just the params set to 'use_ljp12'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp12')
						{
							// if the ljp12 has a value, use it
							if ($ljp12_params->get($key) != '')
							{
								// get the value from the ljp12
								$ljp12_array[$key] = $ljp12_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp12_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp12 params
					if (count($ljp12_array) > 0)
					{
						$ljp12_params = new Registry;
						$ljp12_params->loadArray($ljp12_array);
						$item->params->merge($ljp12_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp12_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp12, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
