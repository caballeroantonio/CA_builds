...
class JtCaModelLjpes04s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpes04_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes04_id');
		$this->setState('filter.ljpes04_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes04_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes04_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes04_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes04_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes04_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes04_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes04_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes04_id'));
		$id .= ':'.$this->getState('filter.ljpes04_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes04s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes04s').' AS a');
		...
		// Filter by a single or group of ljpes04s.
		$ljpes04_id = $this->getState('filter.ljpes04_id');
		if ($ljpes04_id != '')
		{
			if (is_numeric($ljpes04_id))
			{
				$type = $this->getState('filter.ljpes04_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes04_id);
			}
			else
			{
				if (is_array($ljpes04_id))
				{
					JArrayHelper::toInteger($ljpes04_id);
					$ljpes04_id = implode(',', $ljpes04_id);
					$type = $this->getState('filter.ljpes04_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes04_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes04_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes04_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes04_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes04 params override menu item params only if menu param = 'use_ljpes04'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes04' and there is no ljpes04 param, use global

					// create an array of just the params set to 'use_ljpes04'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes04_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes04')
						{
							// if the ljpes04 has a value, use it
							if ($ljpes04_params->get($key) != '')
							{
								// get the value from the ljpes04
								$ljpes04_array[$key] = $ljpes04_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes04_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes04 params
					if (count($ljpes04_array) > 0)
					{
						$ljpes04_params = new Registry;
						$ljpes04_params->loadArray($ljpes04_array);
						$item->params->merge($ljpes04_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes04_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes04, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
