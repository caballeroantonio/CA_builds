...
/**
 * This models supports retrieving lists of libro de desistimiento.
 *
 */
class JtcaModelLspe05s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lspe05s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de desistimiento has been specified either as a parameter or in the url Request
		$pk = $params->get('lspe05_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lspe05_id');
		$this->setState('filter.lspe05_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lspe05_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lspe05_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lspe05_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lspe05_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lspe05_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lspe05_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lspe05_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lspe05_id'));
		$id .= ':'.$this->getState('filter.lspe05_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lspe05s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lspe05s').' AS a');
		...
		// Filter by a single or group of lspe05s.
		$lspe05_id = $this->getState('filter.lspe05_id');
		if ($lspe05_id != '')
		{
			if (is_numeric($lspe05_id))
			{
				$type = $this->getState('filter.lspe05_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lspe05_id);
			}
			else
			{
				if (is_array($lspe05_id))
				{
					JArrayHelper::toInteger($lspe05_id);
					$lspe05_id = implode(',', $lspe05_id);
					$type = $this->getState('filter.lspe05_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lspe05_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lspe05_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lspe05_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de desistimiento.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lspe05_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lspe05 params override menu item params only if menu param = 'use_lspe05'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lspe05' and there is no lspe05 param, use global

					// create an array of just the params set to 'use_lspe05'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lspe05')
						{
							// if the lspe05 has a value, use it
							if ($lspe05_params->get($key) != '')
							{
								// get the value from the lspe05
								$lspe05_array[$key] = $lspe05_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lspe05_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lspe05 params
					if (count($lspe05_array) > 0)
					{
						$lspe05_params = new Registry;
						$lspe05_params->loadArray($lspe05_array);
						$item->params->merge($lspe05_params);
					}


					// get display date
					switch ($item->params->get('list_show_lspe05_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lspe05, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
