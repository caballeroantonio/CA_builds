...
/**
 * This models supports retrieving lists of jc07.
 *
 */
class JtCaModelLjc07s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljc07_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc07_id');
		$this->setState('filter.ljc07_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc07_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc07_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc07_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc07_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc07_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc07_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc07_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc07_id'));
		$id .= ':'.$this->getState('filter.ljc07_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc07s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		// Filter by a single or group of ljc07s.
		$ljc07_id = $this->getState('filter.ljc07_id');
		if ($ljc07_id != '')
		{
			if (is_numeric($ljc07_id))
			{
				$type = $this->getState('filter.ljc07_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc07_id);
			}
			else
			{
				if (is_array($ljc07_id))
				{
					JArrayHelper::toInteger($ljc07_id);
					$ljc07_id = implode(',', $ljc07_id);
					$type = $this->getState('filter.ljc07_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc07_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc07_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc07_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc07_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc07 params override menu item params only if menu param = 'use_ljc07'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc07' and there is no ljc07 param, use global

					// create an array of just the params set to 'use_ljc07'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc07')
						{
							// if the ljc07 has a value, use it
							if ($ljc07_params->get($key) != '')
							{
								// get the value from the ljc07
								$ljc07_array[$key] = $ljc07_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc07_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc07 params
					if (count($ljc07_array) > 0)
					{
						$ljc07_params = new Registry;
						$ljc07_params->loadArray($ljc07_array);
						$item->params->merge($ljc07_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc07_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc07, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
