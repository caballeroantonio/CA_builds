...
class JtCaModelLjccm07s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljccm07_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljccm07_id');
		$this->setState('filter.ljccm07_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljccm07_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljccm07_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljccm07_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljccm07_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljccm07_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljccm07_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljccm07_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljccm07_id'));
		$id .= ':'.$this->getState('filter.ljccm07_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljccm07s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljccm07s').' AS a');
		...
		// Filter by a single or group of ljccm07s.
		$ljccm07_id = $this->getState('filter.ljccm07_id');
		if ($ljccm07_id != '')
		{
			if (is_numeric($ljccm07_id))
			{
				$type = $this->getState('filter.ljccm07_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljccm07_id);
			}
			else
			{
				if (is_array($ljccm07_id))
				{
					JArrayHelper::toInteger($ljccm07_id);
					$ljccm07_id = implode(',', $ljccm07_id);
					$type = $this->getState('filter.ljccm07_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljccm07_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljccm07_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljccm07_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljccm07_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljccm07 params override menu item params only if menu param = 'use_ljccm07'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljccm07' and there is no ljccm07 param, use global

					// create an array of just the params set to 'use_ljccm07'
					$menu_params_array = $this->getState('params')->toArray();
					$ljccm07_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljccm07')
						{
							// if the ljccm07 has a value, use it
							if ($ljccm07_params->get($key) != '')
							{
								// get the value from the ljccm07
								$ljccm07_array[$key] = $ljccm07_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljccm07_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljccm07 params
					if (count($ljccm07_array) > 0)
					{
						$ljccm07_params = new Registry;
						$ljccm07_params->loadArray($ljccm07_array);
						$item->params->merge($ljccm07_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljccm07_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljccm07, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
