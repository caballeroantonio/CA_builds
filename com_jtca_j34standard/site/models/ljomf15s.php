...
/**
 * This models supports retrieving lists of libro de turno a cargo de los secretarios de acuer.
 *
 */
class JtCaModelLjomf15s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljomf15s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno a cargo de los secretarios de acuerdos has been specified either as a parameter or in the url Request
		$pk = $params->get('ljomf15_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf15_id');
		$this->setState('filter.ljomf15_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf15_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf15_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf15_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf15_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf15_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf15_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf15_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf15_id'));
		$id .= ':'.$this->getState('filter.ljomf15_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf15s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf15s').' AS a');
		...
		// Filter by a single or group of ljomf15s.
		$ljomf15_id = $this->getState('filter.ljomf15_id');
		if ($ljomf15_id != '')
		{
			if (is_numeric($ljomf15_id))
			{
				$type = $this->getState('filter.ljomf15_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf15_id);
			}
			else
			{
				if (is_array($ljomf15_id))
				{
					JArrayHelper::toInteger($ljomf15_id);
					$ljomf15_id = implode(',', $ljomf15_id);
					$type = $this->getState('filter.ljomf15_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf15_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf15_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf15_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno a cargo de los secretarios de acuer.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf15_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf15 params override menu item params only if menu param = 'use_ljomf15'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf15' and there is no ljomf15 param, use global

					// create an array of just the params set to 'use_ljomf15'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf15_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf15')
						{
							// if the ljomf15 has a value, use it
							if ($ljomf15_params->get($key) != '')
							{
								// get the value from the ljomf15
								$ljomf15_array[$key] = $ljomf15_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf15_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf15 params
					if (count($ljomf15_array) > 0)
					{
						$ljomf15_params = new Registry;
						$ljomf15_params->loadArray($ljomf15_array);
						$item->params->merge($ljomf15_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf15_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf15, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
