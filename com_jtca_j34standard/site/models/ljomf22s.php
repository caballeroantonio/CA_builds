...
/**
 * This models supports retrieving lists of libro de control de restituciones internacionales.
 *
 */
class JtCaModelLjomf22s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljomf22s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de control de restituciones internacionales has been specified either as a parameter or in the url Request
		$pk = $params->get('ljomf22_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf22_id');
		$this->setState('filter.ljomf22_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf22_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf22_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf22_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf22_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf22_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf22_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf22_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf22_id'));
		$id .= ':'.$this->getState('filter.ljomf22_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf22s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf22s').' AS a');
		...
		// Filter by a single or group of ljomf22s.
		$ljomf22_id = $this->getState('filter.ljomf22_id');
		if ($ljomf22_id != '')
		{
			if (is_numeric($ljomf22_id))
			{
				$type = $this->getState('filter.ljomf22_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf22_id);
			}
			else
			{
				if (is_array($ljomf22_id))
				{
					JArrayHelper::toInteger($ljomf22_id);
					$ljomf22_id = implode(',', $ljomf22_id);
					$type = $this->getState('filter.ljomf22_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf22_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf22_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf22_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de control de restituciones internacionales.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf22_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf22 params override menu item params only if menu param = 'use_ljomf22'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf22' and there is no ljomf22 param, use global

					// create an array of just the params set to 'use_ljomf22'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf22_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf22')
						{
							// if the ljomf22 has a value, use it
							if ($ljomf22_params->get($key) != '')
							{
								// get the value from the ljomf22
								$ljomf22_array[$key] = $ljomf22_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf22_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf22 params
					if (count($ljomf22_array) > 0)
					{
						$ljomf22_params = new Registry;
						$ljomf22_params->loadArray($ljomf22_array);
						$item->params->merge($ljomf22_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf22_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf22, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
