...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE TURNO DE BENEFICIOS PENITENCIARIOS. Model
 *
 */
class JtCaModelLcp10 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp10';
	...
	{
		if (empty($config['lcp10_filter_fields']))
		{
			$config['lcp10_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp10s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE TURNO DE BENEFICIOS PENITENCIARIOS. data.
	 *
	 * @param	integer	$pk	The id of the libro de turno de beneficios penitenciarios..
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp10.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP10S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp10ChangeState', array('com_jtca.lcp10', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp10BeforeDelete', array('com_jtca.lcp10', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp10AfterDelete', array('com_jtca.lcp10', &$table));
				}
				else
				{
					// Prune items that you can't change.
