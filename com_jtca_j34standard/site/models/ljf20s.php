...
class JtCaModelLjf20s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf20_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf20_id');
		$this->setState('filter.ljf20_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf20_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf20_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf20_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf20_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf20_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf20_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf20_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf20_id'));
		$id .= ':'.$this->getState('filter.ljf20_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf20s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf20s').' AS a');
		...
		// Filter by a single or group of ljf20s.
		$ljf20_id = $this->getState('filter.ljf20_id');
		if ($ljf20_id != '')
		{
			if (is_numeric($ljf20_id))
			{
				$type = $this->getState('filter.ljf20_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf20_id);
			}
			else
			{
				if (is_array($ljf20_id))
				{
					JArrayHelper::toInteger($ljf20_id);
					$ljf20_id = implode(',', $ljf20_id);
					$type = $this->getState('filter.ljf20_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf20_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf20_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf20_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf20_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf20 params override menu item params only if menu param = 'use_ljf20'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf20' and there is no ljf20 param, use global

					// create an array of just the params set to 'use_ljf20'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf20')
						{
							// if the ljf20 has a value, use it
							if ($ljf20_params->get($key) != '')
							{
								// get the value from the ljf20
								$ljf20_array[$key] = $ljf20_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf20_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf20 params
					if (count($ljf20_array) > 0)
					{
						$ljf20_params = new Registry;
						$ljf20_params->loadArray($ljf20_array);
						$item->params->merge($ljf20_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf20_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf20, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
