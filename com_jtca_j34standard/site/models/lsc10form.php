...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsc10.php';

class JtcaModelLsc10Form extends JtcaModelLsc10
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsc10';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsc10';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsc10', 'lsc10', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de actuario (oficial).
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE ACTUARIO (OFICIAL) item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsc10.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsc10.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsc10.id');
		$is_new		= true;
		...
		// Reorder the libro de actuario (oficial) so the new libro de actuario (oficial) is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsc10BeforeSave', array('com_jtca.lsc10', &$table, $is_new));
		...
		$dispatcher->trigger('onLsc10AfterSave', array('com_jtca.lsc10', $table, $is_new));

		$this->setState('lsc10.id', $table->id);
