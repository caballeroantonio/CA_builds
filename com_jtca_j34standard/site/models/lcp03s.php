...
/**
 * This models supports retrieving lists of libro de turno de remisiones con detenido de juzga.
 *
 */
class JtCaModelLcp03s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp03s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno de remisiones con detenido de juzgados adolescentes has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp03_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp03_id');
		$this->setState('filter.lcp03_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp03_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp03_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp03_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp03_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp03_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp03_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp03_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp03_id'));
		$id .= ':'.$this->getState('filter.lcp03_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp03s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp03s').' AS a');
		...
		// Filter by a single or group of lcp03s.
		$lcp03_id = $this->getState('filter.lcp03_id');
		if ($lcp03_id != '')
		{
			if (is_numeric($lcp03_id))
			{
				$type = $this->getState('filter.lcp03_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp03_id);
			}
			else
			{
				if (is_array($lcp03_id))
				{
					JArrayHelper::toInteger($lcp03_id);
					$lcp03_id = implode(',', $lcp03_id);
					$type = $this->getState('filter.lcp03_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp03_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp03_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp03_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno de remisiones con detenido de juzga.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp03_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp03 params override menu item params only if menu param = 'use_lcp03'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp03' and there is no lcp03 param, use global

					// create an array of just the params set to 'use_lcp03'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp03')
						{
							// if the lcp03 has a value, use it
							if ($lcp03_params->get($key) != '')
							{
								// get the value from the lcp03
								$lcp03_array[$key] = $lcp03_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp03_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp03 params
					if (count($lcp03_array) > 0)
					{
						$lcp03_params = new Registry;
						$lcp03_params->loadArray($lcp03_array);
						$item->params->merge($lcp03_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp03_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp03, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
