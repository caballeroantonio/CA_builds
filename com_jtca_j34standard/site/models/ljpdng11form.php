...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpdng11.php';

class JtCaModelLjpdng11Form extends JtCaModelLjpdng11
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpdng11';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpdng11';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpdng11', 'ljpdng11', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de amparos circulantes o negados.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE AMPAROS CIRCULANTES O NEGADOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpdng11.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpdng11.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpdng11.id');
		$is_new		= true;
		...
		// Reorder the libro de amparos circulantes o negados so the new libro de amparos circulantes o negados is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpdng11BeforeSave', array('com_jtca.ljpdng11', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpdng11AfterSave', array('com_jtca.ljpdng11', $table, $is_new));

		$this->setState('ljpdng11.id', $table->id);
