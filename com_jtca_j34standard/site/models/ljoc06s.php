...
class JtCaModelLjoc06s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljoc06_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljoc06_id');
		$this->setState('filter.ljoc06_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljoc06_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljoc06_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljoc06_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljoc06_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljoc06_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljoc06_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljoc06_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljoc06_id'));
		$id .= ':'.$this->getState('filter.ljoc06_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljoc06s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljoc06s').' AS a');
		...
		// Filter by a single or group of ljoc06s.
		$ljoc06_id = $this->getState('filter.ljoc06_id');
		if ($ljoc06_id != '')
		{
			if (is_numeric($ljoc06_id))
			{
				$type = $this->getState('filter.ljoc06_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljoc06_id);
			}
			else
			{
				if (is_array($ljoc06_id))
				{
					JArrayHelper::toInteger($ljoc06_id);
					$ljoc06_id = implode(',', $ljoc06_id);
					$type = $this->getState('filter.ljoc06_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljoc06_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljoc06_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljoc06_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljoc06_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljoc06 params override menu item params only if menu param = 'use_ljoc06'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljoc06' and there is no ljoc06 param, use global

					// create an array of just the params set to 'use_ljoc06'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljoc06')
						{
							// if the ljoc06 has a value, use it
							if ($ljoc06_params->get($key) != '')
							{
								// get the value from the ljoc06
								$ljoc06_array[$key] = $ljoc06_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljoc06_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljoc06 params
					if (count($ljoc06_array) > 0)
					{
						$ljoc06_params = new Registry;
						$ljoc06_params->loadArray($ljoc06_array);
						$item->params->merge($ljoc06_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljoc06_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljoc06, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
