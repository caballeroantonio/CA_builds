...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE CONTROL DE FIRMAS DE PROCESADOS Model
 *
 */
class JtCaModelLjp19 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljp19';
	...
	{
		if (empty($config['ljp19_filter_fields']))
		{
			$config['ljp19_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljp19s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE CONTROL DE FIRMAS DE PROCESADOS data.
	 *
	 * @param	integer	$pk	The id of the libro de control de firmas de procesados.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljp19.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJP19S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjp19ChangeState', array('com_jtca.ljp19', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjp19BeforeDelete', array('com_jtca.ljp19', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjp19AfterDelete', array('com_jtca.ljp19', &$table));
				}
				else
				{
					// Prune items that you can't change.
