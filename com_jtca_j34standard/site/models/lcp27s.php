...
/**
 * This models supports retrieving lists of libro de turno para incompetencias delitos no grav.
 *
 */
class JtCaModelLcp27s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp27s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno para incompetencias delitos no graves has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp27_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp27_id');
		$this->setState('filter.lcp27_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp27_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp27_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp27_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp27_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp27_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp27_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp27_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp27_id'));
		$id .= ':'.$this->getState('filter.lcp27_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp27s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp27s').' AS a');
		...
		// Filter by a single or group of lcp27s.
		$lcp27_id = $this->getState('filter.lcp27_id');
		if ($lcp27_id != '')
		{
			if (is_numeric($lcp27_id))
			{
				$type = $this->getState('filter.lcp27_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp27_id);
			}
			else
			{
				if (is_array($lcp27_id))
				{
					JArrayHelper::toInteger($lcp27_id);
					$lcp27_id = implode(',', $lcp27_id);
					$type = $this->getState('filter.lcp27_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp27_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp27_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp27_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno para incompetencias delitos no grav.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp27_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp27 params override menu item params only if menu param = 'use_lcp27'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp27' and there is no lcp27 param, use global

					// create an array of just the params set to 'use_lcp27'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp27')
						{
							// if the lcp27 has a value, use it
							if ($lcp27_params->get($key) != '')
							{
								// get the value from the lcp27
								$lcp27_array[$key] = $lcp27_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp27_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp27 params
					if (count($lcp27_array) > 0)
					{
						$lcp27_params = new Registry;
						$lcp27_params->loadArray($lcp27_array);
						$item->params->merge($lcp27_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp27_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp27, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
