...
class JtCaModelLjc19 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljc19';
	...
	{
		if (empty($config['ljc19_filter_fields']))
		{
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljc19.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJC19S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjc19ChangeState', array('com_jtca.ljc19', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjc19BeforeDelete', array('com_jtca.ljc19', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjc19AfterDelete', array('com_jtca.ljc19', &$table));
				}
				else
				{
					// Prune items that you can't change.
