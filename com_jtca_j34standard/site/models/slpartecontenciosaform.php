...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/slpartecontenciosa.php';

class JtcaModelSlpartecontenciosaForm extends JtcaModelSlpartecontenciosa
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.slpartecontenciosa';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.slpartecontenciosa';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.slpartecontenciosa', 'slpartecontenciosa', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the partes contenciosas en juicio.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Partes Contenciosas En Juicio item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('slpartecontenciosa.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.slpartecontenciosa.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('slpartecontenciosa.id');
		$is_new		= true;
		...
		// Reorder the partes contenciosas en juicio so the new partes contenciosas en juicio is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSlpartecontenciosaBeforeSave', array('com_jtca.slpartecontenciosa', &$table, $is_new));
		...
		$dispatcher->trigger('onSlpartecontenciosaAfterSave', array('com_jtca.slpartecontenciosa', $table, $is_new));

		$this->setState('slpartecontenciosa.id', $table->id);
