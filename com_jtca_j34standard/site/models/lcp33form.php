...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp33.php';

class JtCaModelLcp33Form extends JtCaModelLcp33
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp33';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp33';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp33', 'lcp33', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de libertades de salas penales.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE LIBERTADES DE SALAS PENALES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp33.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp33.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp33.id');
		$is_new		= true;
		...
		// Reorder the libreta de libertades de salas penales so the new libreta de libertades de salas penales is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp33BeforeSave', array('com_jtca.lcp33', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp33AfterSave', array('com_jtca.lcp33', $table, $is_new));

		$this->setState('lcp33.id', $table->id);
