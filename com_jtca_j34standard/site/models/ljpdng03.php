...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE CERTIFICADO DE DEPÓSITOS DE EGRESO Model
 *
 */
class JtCaModelLjpdng03 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpdng03';
	...
	{
		if (empty($config['ljpdng03_filter_fields']))
		{
			$config['ljpdng03_filter_fields'] = array(
				'id', 'a.id',
				'billete', 'a.billete',
				...
				'field6_paterno','a.field6_paterno',
				'field6_materno','a.field6_materno',
				'field6_nombre','a.field6_nombre',
				'field7','a.field7',
				'field7h','a.field7h',
				'field8','a.field8',
				'field9','a.field9',
	...
	*/
	public function getTable($type = 'Ljpdng03s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE CERTIFICADO DE DEPÓSITOS DE EGRESO data.
	 *
	 * @param	integer	$pk	The id of the libro de certificado de depósitos de egreso.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpdng03.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPDNG03S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpdng03ChangeState', array('com_jtca.ljpdng03', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpdng03BeforeDelete', array('com_jtca.ljpdng03', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpdng03AfterDelete', array('com_jtca.ljpdng03', &$table));
				}
				else
				{
					// Prune items that you can't change.
