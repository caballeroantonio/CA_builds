...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE AMPAROS CIRCULANTES O NEGADOS Model
 *
 */
class JtcaModelLjpdng11 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpdng11';
	...
	{
		if (empty($config['ljpdng11_filter_fields']))
		{
			$config['ljpdng11_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljpdng11s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE AMPAROS CIRCULANTES O NEGADOS data.
	 *
	 * @param	integer	$pk	The id of the libro de amparos circulantes o negados.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpdng11.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPDNG11S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpdng11ChangeState', array('com_jtca.ljpdng11', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpdng11BeforeDelete', array('com_jtca.ljpdng11', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpdng11AfterDelete', array('com_jtca.ljpdng11', &$table));
				}
				else
				{
					// Prune items that you can't change.
