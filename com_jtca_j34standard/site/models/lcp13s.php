...
/**
 * This models supports retrieving lists of libreta de expedientillos de juzgados de ejecuciÃ³n.
 *
 */
class JtCaModelLcp13s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp13s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de expedientillos de juzgados de ejecuciÃ³n para juzgados penal o no graves. has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp13_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp13_id');
		$this->setState('filter.lcp13_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp13_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp13_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp13_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp13_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp13_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp13_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp13_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp13_id'));
		$id .= ':'.$this->getState('filter.lcp13_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp13s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp13s').' AS a');
		...
		// Filter by a single or group of lcp13s.
		$lcp13_id = $this->getState('filter.lcp13_id');
		if ($lcp13_id != '')
		{
			if (is_numeric($lcp13_id))
			{
				$type = $this->getState('filter.lcp13_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp13_id);
			}
			else
			{
				if (is_array($lcp13_id))
				{
					JArrayHelper::toInteger($lcp13_id);
					$lcp13_id = implode(',', $lcp13_id);
					$type = $this->getState('filter.lcp13_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp13_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp13_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp13_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de expedientillos de juzgados de ejecuciÃ³n.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp13_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp13 params override menu item params only if menu param = 'use_lcp13'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp13' and there is no lcp13 param, use global

					// create an array of just the params set to 'use_lcp13'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp13')
						{
							// if the lcp13 has a value, use it
							if ($lcp13_params->get($key) != '')
							{
								// get the value from the lcp13
								$lcp13_array[$key] = $lcp13_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp13_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp13 params
					if (count($lcp13_array) > 0)
					{
						$lcp13_params = new Registry;
						$lcp13_params->loadArray($lcp13_array);
						$item->params->merge($lcp13_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp13_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp13, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
