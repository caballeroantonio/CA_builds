...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/sldilijpdng09.php';

class JtcaModelSldiliJpdng09Form extends JtcaModelSldiliJpdng09
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.sldilijpdng09';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.sldilijpdng09';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.sldilijpdng09', 'sldilijpdng09', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the sub libro diligencia del libro de actuarios.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Sub Libro DILIGENCIA Del LIBRO DE ACTUARIOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('sldilijpdng09.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.sldilijpdng09.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('sldilijpdng09.id');
		$is_new		= true;
		...
		// Reorder the diligencia del libro de actuarios so the new sub libro diligencia del libro de actuarios is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSldiliJpdng09BeforeSave', array('com_jtca.sldilijpdng09', &$table, $is_new));
		...
		$dispatcher->trigger('onSldiliJpdng09AfterSave', array('com_jtca.sldilijpdng09', $table, $is_new));

		$this->setState('sldilijpdng09.id', $table->id);
