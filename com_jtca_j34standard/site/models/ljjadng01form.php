...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadng01.php';

class JtCaModelLjjadng01Form extends JtCaModelLjjadng01
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadng01';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadng01';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadng01', 'ljjadng01', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de gobierno.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE GOBIERNO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadng01.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadng01.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadng01.id');
		$is_new		= true;
		...
		// Reorder the libro de gobierno so the new libro de gobierno is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadng01BeforeSave', array('com_jtca.ljjadng01', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadng01AfterSave', array('com_jtca.ljjadng01', $table, $is_new));

		$this->setState('ljjadng01.id', $table->id);
