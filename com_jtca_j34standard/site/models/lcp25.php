...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE TURNO PARA CONSIGNACIONES CON DETENIDO DELITOS NO GRAVES Model
 *
 */
class JtCaModelLcp25 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp25';
	...
	{
		if (empty($config['lcp25_filter_fields']))
		{
			$config['lcp25_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp25s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE TURNO PARA CONSIGNACIONES CON DETENIDO DELITOS NO GRAVES data.
	 *
	 * @param	integer	$pk	The id of the libro de turno para consignaciones con detenido delitos no graves.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp25.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP25S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp25ChangeState', array('com_jtca.lcp25', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp25BeforeDelete', array('com_jtca.lcp25', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp25AfterDelete', array('com_jtca.lcp25', &$table));
				}
				else
				{
					// Prune items that you can't change.
