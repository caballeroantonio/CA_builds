...
/**
 * This models supports retrieving lists of libro de etapa de ejecuciÃ³n para adolescentes.
 *
 */
class JtCaModelLcp09s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp09s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de etapa de ejecuciÃ³n para adolescentes has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp09_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp09_id');
		$this->setState('filter.lcp09_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp09_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp09_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp09_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp09_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp09_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp09_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp09_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp09_id'));
		$id .= ':'.$this->getState('filter.lcp09_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp09s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp09s').' AS a');
		...
		// Filter by a single or group of lcp09s.
		$lcp09_id = $this->getState('filter.lcp09_id');
		if ($lcp09_id != '')
		{
			if (is_numeric($lcp09_id))
			{
				$type = $this->getState('filter.lcp09_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp09_id);
			}
			else
			{
				if (is_array($lcp09_id))
				{
					JArrayHelper::toInteger($lcp09_id);
					$lcp09_id = implode(',', $lcp09_id);
					$type = $this->getState('filter.lcp09_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp09_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp09_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp09_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de etapa de ejecuciÃ³n para adolescentes.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp09_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp09 params override menu item params only if menu param = 'use_lcp09'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp09' and there is no lcp09 param, use global

					// create an array of just the params set to 'use_lcp09'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp09')
						{
							// if the lcp09 has a value, use it
							if ($lcp09_params->get($key) != '')
							{
								// get the value from the lcp09
								$lcp09_array[$key] = $lcp09_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp09_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp09 params
					if (count($lcp09_array) > 0)
					{
						$lcp09_params = new Registry;
						$lcp09_params->loadArray($lcp09_array);
						$item->params->merge($lcp09_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp09_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp09, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
