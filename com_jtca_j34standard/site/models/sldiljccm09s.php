...
class JtcaModelSldilJccm09s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('sldil_jccm09_id', '') == '' ? $app->input->getInt('id', '') : $params->get('sldil_jccm09_id');
		$this->setState('filter.sldil_jccm09_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('sldiljccm09_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('sldiljccm09_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('sldiljccm09_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('sldiljccm09_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('sldiljccm09_initial_direction','ASC');
		}
		...
		if ($params->get('filter_sldiljccm09_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_sldiljccm09_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.sldil_jccm09_id'));
		$id .= ':'.$this->getState('filter.sldil_jccm09_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of sldil_jccm09s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_sldil_jccm09s').' AS a');
		...
		// Filter by a single or group of sldil_jccm09s.
		$sldil_jccm09_id = $this->getState('filter.sldil_jccm09_id');
		if ($sldil_jccm09_id != '')
		{
			if (is_numeric($sldil_jccm09_id))
			{
				$type = $this->getState('filter.sldil_jccm09_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $sldil_jccm09_id);
			}
			else
			{
				if (is_array($sldil_jccm09_id))
				{
					JArrayHelper::toInteger($sldil_jccm09_id);
					$sldil_jccm09_id = implode(',', $sldil_jccm09_id);
					$type = $this->getState('filter.sldil_jccm09_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$sldil_jccm09_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_sldiljccm09_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_sldiljccm09_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$sldil_jccm09_params = new Registry;
				...
				{
					$item->params = $sldil_jccm09_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// SldilJccm09 params override menu item params only if menu param = 'use_sldiljccm09'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_sldiljccm09' and there is no sldiljccm09 param, use global

					// create an array of just the params set to 'use_sldiljccm09'
					$menu_params_array = $this->getState('params')->toArray();
					$sldil_jccm09_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_sldiljccm09')
						{
							// if the sldiljccm09 has a value, use it
							if ($sldil_jccm09_params->get($key) != '')
							{
								// get the value from the sldiljccm09
								$sldil_jccm09_array[$key] = $sldil_jccm09_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$sldil_jccm09_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected sldiljccm09 params
					if (count($sldil_jccm09_array) > 0)
					{
						$sldil_jccm09_params = new Registry;
						$sldil_jccm09_params->loadArray($sldil_jccm09_array);
						$item->params->merge($sldil_jccm09_params);
					}


					// get display date
					switch ($item->params->get('list_show_sldiljccm09_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an sldiljccm09, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
