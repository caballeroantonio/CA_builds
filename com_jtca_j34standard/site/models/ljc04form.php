...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljc04.php';

class JtCaModelLjc04Form extends JtCaModelLjc04
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljc04';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljc04';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljc04', 'ljc04', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de registro de promociones.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE REGISTRO DE PROMOCIONES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljc04.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljc04.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljc04.id');
		$is_new		= true;
		...
		// Reorder the jc04 so the new libro de registro de promociones is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjc04BeforeSave', array('com_jtca.ljc04', &$table, $is_new));
		...
		$dispatcher->trigger('onLjc04AfterSave', array('com_jtca.ljc04', $table, $is_new));

		$this->setState('ljc04.id', $table->id);
	...
	*/
	protected function getReorderConditions($table)
	{
		...
		$condition = array();
		$condition[] = $db->quoteName('id_expediente').' = '. $db->quote($table->id_expediente);	
