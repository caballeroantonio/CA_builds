...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadng08.php';

class JtCaModelLjjadng08Form extends JtCaModelLjjadng08
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadng08';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadng08';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadng08', 'ljjadng08', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de amparos.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE AMPAROS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadng08.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadng08.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadng08.id');
		$is_new		= true;
		...
		// Reorder the libro de amparos so the new libro de amparos is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadng08BeforeSave', array('com_jtca.ljjadng08', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadng08AfterSave', array('com_jtca.ljjadng08', $table, $is_new));

		$this->setState('ljjadng08.id', $table->id);
