...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE REMISIÓN AL ARCHIVO Model
 *
 */
class JtCaModelLjjadg17 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljjadg17';
	...
	{
		if (empty($config['ljjadg17_filter_fields']))
		{
			$config['ljjadg17_filter_fields'] = array(
				'id', 'a.id',
				...
				'field7_paterno','a.field7_paterno',
				'field7_materno','a.field7_materno',
				'field7_nombre','a.field7_nombre',
				'field8','a.field8',
				'field9','a.field9',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Ljjadg17s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE REMISIÓN AL ARCHIVO data.
	 *
	 * @param	integer	$pk	The id of the libro de remisión al archivo.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljjadg17.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJJADG17S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjjadg17ChangeState', array('com_jtca.ljjadg17', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjjadg17BeforeDelete', array('com_jtca.ljjadg17', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjjadg17AfterDelete', array('com_jtca.ljjadg17', &$table));
				}
				else
				{
					// Prune items that you can't change.
