...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp29.php';

class JtCaModelLcp29Form extends JtCaModelLcp29
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp29';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp29';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp29', 'lcp29', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de ordenes para arraigos y cateos delitos no graves.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE ORDENES PARA ARRAIGOS Y CATEOS DELITOS NO GRAVES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp29.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp29.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp29.id');
		$is_new		= true;
		...
		// Reorder the libro de ordenes para arraigos y cateos delitos no so the new libro de ordenes para arraigos y cateos delitos no graves is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp29BeforeSave', array('com_jtca.lcp29', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp29AfterSave', array('com_jtca.lcp29', $table, $is_new));

		$this->setState('lcp29.id', $table->id);
