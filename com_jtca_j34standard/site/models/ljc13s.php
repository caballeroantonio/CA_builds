...
/**
 * This models supports retrieving lists of jc13.
 *
 */
class JtCaModelLjc13s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljc13s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de control de fianzas has been specified either as a parameter or in the url Request
		$pk = $params->get('ljc13_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc13_id');
		$this->setState('filter.ljc13_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc13_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc13_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc13_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc13_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc13_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc13_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc13_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc13_id'));
		$id .= ':'.$this->getState('filter.ljc13_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc13s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc13s').' AS a');
		...
		// Filter by a single or group of ljc13s.
		$ljc13_id = $this->getState('filter.ljc13_id');
		if ($ljc13_id != '')
		{
			if (is_numeric($ljc13_id))
			{
				$type = $this->getState('filter.ljc13_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc13_id);
			}
			else
			{
				if (is_array($ljc13_id))
				{
					JArrayHelper::toInteger($ljc13_id);
					$ljc13_id = implode(',', $ljc13_id);
					$type = $this->getState('filter.ljc13_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc13_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc13_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc13_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc13_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc13 params override menu item params only if menu param = 'use_ljc13'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc13' and there is no ljc13 param, use global

					// create an array of just the params set to 'use_ljc13'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc13')
						{
							// if the ljc13 has a value, use it
							if ($ljc13_params->get($key) != '')
							{
								// get the value from the ljc13
								$ljc13_array[$key] = $ljc13_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc13_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc13 params
					if (count($ljc13_array) > 0)
					{
						$ljc13_params = new Registry;
						$ljc13_params->loadArray($ljc13_array);
						$item->params->merge($ljc13_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc13_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc13, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
