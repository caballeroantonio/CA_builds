...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE RESOLUCIONES DE PLANO Model
 *
 */
class JtCaModelLspe02 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lspe02';
	...
	{
		if (empty($config['lspe02_filter_fields']))
		{
			$config['lspe02_filter_fields'] = array(
				'id', 'a.id',
				'id_organo','a.id_organo',
				'anoj','a.anoj',
				'id_expediente','a.id_expediente',
				'id_secretaria','a.id_secretaria',
				'field2299','a.field2299',
				'field2300','a.field2300',
				'field2301','a.field2301',
				'field2302','a.field2302',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lspe02s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE RESOLUCIONES DE PLANO data.
	 *
	 * @param	integer	$pk	The id of the libro de resoluciones de plano.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lspe02.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPE02S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLspe02ChangeState', array('com_jtca.lspe02', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLspe02BeforeDelete', array('com_jtca.lspe02', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLspe02AfterDelete', array('com_jtca.lspe02', &$table));
				}
				else
				{
					// Prune items that you can't change.
