...
/**
 * This models supports retrieving lists of libro de ingresos.
 *
 */
class JtCaModelLjjadg03s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadg03s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de ingresos has been specified either as a parameter or in the url Request
		$pk = $params->get('ljjadg03_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg03_id');
		$this->setState('filter.ljjadg03_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg03_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg03_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg03_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg03_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg03_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg03_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg03_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg03_id'));
		$id .= ':'.$this->getState('filter.ljjadg03_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg03s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg03s').' AS a');
		...
		// Filter by a single or group of ljjadg03s.
		$ljjadg03_id = $this->getState('filter.ljjadg03_id');
		if ($ljjadg03_id != '')
		{
			if (is_numeric($ljjadg03_id))
			{
				$type = $this->getState('filter.ljjadg03_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg03_id);
			}
			else
			{
				if (is_array($ljjadg03_id))
				{
					JArrayHelper::toInteger($ljjadg03_id);
					$ljjadg03_id = implode(',', $ljjadg03_id);
					$type = $this->getState('filter.ljjadg03_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg03_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg03_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg03_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de ingresos.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg03_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg03 params override menu item params only if menu param = 'use_ljjadg03'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg03' and there is no ljjadg03 param, use global

					// create an array of just the params set to 'use_ljjadg03'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg03_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg03')
						{
							// if the ljjadg03 has a value, use it
							if ($ljjadg03_params->get($key) != '')
							{
								// get the value from the ljjadg03
								$ljjadg03_array[$key] = $ljjadg03_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg03_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg03 params
					if (count($ljjadg03_array) > 0)
					{
						$ljjadg03_params = new Registry;
						$ljjadg03_params->loadArray($ljjadg03_array);
						$item->params->merge($ljjadg03_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg03_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg03, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
