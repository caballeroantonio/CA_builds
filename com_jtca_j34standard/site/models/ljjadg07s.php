...
class JtCaModelLjjadg07s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadg07_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg07_id');
		$this->setState('filter.ljjadg07_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg07_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg07_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg07_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg07_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg07_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg07_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg07_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg07_id'));
		$id .= ':'.$this->getState('filter.ljjadg07_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg07s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg07s').' AS a');
		...
		// Filter by a single or group of ljjadg07s.
		$ljjadg07_id = $this->getState('filter.ljjadg07_id');
		if ($ljjadg07_id != '')
		{
			if (is_numeric($ljjadg07_id))
			{
				$type = $this->getState('filter.ljjadg07_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg07_id);
			}
			else
			{
				if (is_array($ljjadg07_id))
				{
					JArrayHelper::toInteger($ljjadg07_id);
					$ljjadg07_id = implode(',', $ljjadg07_id);
					$type = $this->getState('filter.ljjadg07_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg07_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg07_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg07_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg07_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg07 params override menu item params only if menu param = 'use_ljjadg07'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg07' and there is no ljjadg07 param, use global

					// create an array of just the params set to 'use_ljjadg07'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg07_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg07')
						{
							// if the ljjadg07 has a value, use it
							if ($ljjadg07_params->get($key) != '')
							{
								// get the value from the ljjadg07
								$ljjadg07_array[$key] = $ljjadg07_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg07_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg07 params
					if (count($ljjadg07_array) > 0)
					{
						$ljjadg07_params = new Registry;
						$ljjadg07_params->loadArray($ljjadg07_array);
						$item->params->merge($ljjadg07_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg07_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg07, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
