...
class JtCaModelLcp01s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno de justicia para adolescentes con detenido has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp01_id');
		$this->setState('filter.lcp01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp01_id'));
		$id .= ':'.$this->getState('filter.lcp01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp01s').' AS a');
		...
		// Filter by a single or group of lcp01s.
		$lcp01_id = $this->getState('filter.lcp01_id');
		if ($lcp01_id != '')
		{
			if (is_numeric($lcp01_id))
			{
				$type = $this->getState('filter.lcp01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp01_id);
			}
			else
			{
				if (is_array($lcp01_id))
				{
					JArrayHelper::toInteger($lcp01_id);
					$lcp01_id = implode(',', $lcp01_id);
					$type = $this->getState('filter.lcp01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp01_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp01 params override menu item params only if menu param = 'use_lcp01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp01' and there is no lcp01 param, use global

					// create an array of just the params set to 'use_lcp01'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp01')
						{
							// if the lcp01 has a value, use it
							if ($lcp01_params->get($key) != '')
							{
								// get the value from the lcp01
								$lcp01_array[$key] = $lcp01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp01 params
					if (count($lcp01_array) > 0)
					{
						$lcp01_params = new Registry;
						$lcp01_params->loadArray($lcp01_array);
						$item->params->merge($lcp01_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
