...
/**
 * This models supports retrieving lists of libro de turno para sentencias.
 *
 */
class JtcaModelLjpdng05s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpdng05s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno para sentencias has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpdng05_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng05_id');
		$this->setState('filter.ljpdng05_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng05_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng05_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng05_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng05_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng05_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng05_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng05_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng05_id'));
		$id .= ':'.$this->getState('filter.ljpdng05_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng05s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng05s').' AS a');
		...
		// Filter by a single or group of ljpdng05s.
		$ljpdng05_id = $this->getState('filter.ljpdng05_id');
		if ($ljpdng05_id != '')
		{
			if (is_numeric($ljpdng05_id))
			{
				$type = $this->getState('filter.ljpdng05_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng05_id);
			}
			else
			{
				if (is_array($ljpdng05_id))
				{
					JArrayHelper::toInteger($ljpdng05_id);
					$ljpdng05_id = implode(',', $ljpdng05_id);
					$type = $this->getState('filter.ljpdng05_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng05_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng05_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng05_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno para sentencias.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng05_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng05 params override menu item params only if menu param = 'use_ljpdng05'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng05' and there is no ljpdng05 param, use global

					// create an array of just the params set to 'use_ljpdng05'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng05_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng05')
						{
							// if the ljpdng05 has a value, use it
							if ($ljpdng05_params->get($key) != '')
							{
								// get the value from the ljpdng05
								$ljpdng05_array[$key] = $ljpdng05_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng05_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng05 params
					if (count($ljpdng05_array) > 0)
					{
						$ljpdng05_params = new Registry;
						$ljpdng05_params->loadArray($ljpdng05_array);
						$item->params->merge($ljpdng05_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng05_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng05, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
