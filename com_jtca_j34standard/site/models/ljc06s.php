...
/**
 * This models supports retrieving lists of jc06.
 *
 */
class JtCaModelLjc06s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljc06s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc06_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc06_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc06_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc06_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc06_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc06_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc06_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc06_id'));
		$id .= ':'.$this->getState('filter.ljc06_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc06s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc06s').' AS a');
		...
		// Filter by a single or group of ljc06s.
		$ljc06_id = $this->getState('filter.ljc06_id');
		if ($ljc06_id != '')
		{
			if (is_numeric($ljc06_id))
			{
				$type = $this->getState('filter.ljc06_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc06_id);
			}
			else
			{
				if (is_array($ljc06_id))
				{
					JArrayHelper::toInteger($ljc06_id);
					$ljc06_id = implode(',', $ljc06_id);
					$type = $this->getState('filter.ljc06_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc06_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc06_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc06_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc06_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc06 params override menu item params only if menu param = 'use_ljc06'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc06' and there is no ljc06 param, use global

					// create an array of just the params set to 'use_ljc06'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc06')
						{
							// if the ljc06 has a value, use it
							if ($ljc06_params->get($key) != '')
							{
								// get the value from the ljc06
								$ljc06_array[$key] = $ljc06_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc06_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc06 params
					if (count($ljc06_array) > 0)
					{
						$ljc06_params = new Registry;
						$ljc06_params->loadArray($ljc06_array);
						$item->params->merge($ljc06_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc06_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc06, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
