...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljc21.php';

class JtCaModelLjc21Form extends JtCaModelLjc21
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljc21';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljc21';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljc21', 'ljc21', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljc21.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljc21.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljc21.id');
		$is_new		= true;
		...
		// Reorder the libro de ministerio público so the new libro de ministerio público is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjc21BeforeSave', array('com_jtca.ljc21', &$table, $is_new));
		...
		$dispatcher->trigger('onLjc21AfterSave', array('com_jtca.ljc21', $table, $is_new));

		$this->setState('ljc21.id', $table->id);
