...
/**
 * This models supports retrieving lists of libro de control de pedimentos.
 *
 */
class JtCaModelLjjadng10s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadng10s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de control de pedimentos has been specified either as a parameter or in the url Request
		$pk = $params->get('ljjadng10_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadng10_id');
		$this->setState('filter.ljjadng10_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadng10_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadng10_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadng10_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadng10_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadng10_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadng10_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadng10_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadng10_id'));
		$id .= ':'.$this->getState('filter.ljjadng10_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadng10s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadng10s').' AS a');
		...
		// Filter by a single or group of ljjadng10s.
		$ljjadng10_id = $this->getState('filter.ljjadng10_id');
		if ($ljjadng10_id != '')
		{
			if (is_numeric($ljjadng10_id))
			{
				$type = $this->getState('filter.ljjadng10_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadng10_id);
			}
			else
			{
				if (is_array($ljjadng10_id))
				{
					JArrayHelper::toInteger($ljjadng10_id);
					$ljjadng10_id = implode(',', $ljjadng10_id);
					$type = $this->getState('filter.ljjadng10_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadng10_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadng10_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadng10_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de control de pedimentos.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadng10_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadng10 params override menu item params only if menu param = 'use_ljjadng10'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadng10' and there is no ljjadng10 param, use global

					// create an array of just the params set to 'use_ljjadng10'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadng10_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadng10')
						{
							// if the ljjadng10 has a value, use it
							if ($ljjadng10_params->get($key) != '')
							{
								// get the value from the ljjadng10
								$ljjadng10_array[$key] = $ljjadng10_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadng10_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadng10 params
					if (count($ljjadng10_array) > 0)
					{
						$ljjadng10_params = new Registry;
						$ljjadng10_params->loadArray($ljjadng10_array);
						$item->params->merge($ljjadng10_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadng10_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadng10, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
