...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE CONTROL DE PEDIMENTOS Model
 *
 */
class JtCaModelLjjadng10 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljjadng10';
	...
	{
		if (empty($config['ljjadng10_filter_fields']))
		{
			$config['ljjadng10_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljjadng10s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE CONTROL DE PEDIMENTOS data.
	 *
	 * @param	integer	$pk	The id of the libro de control de pedimentos.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljjadng10.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJJADNG10S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjjadng10ChangeState', array('com_jtca.ljjadng10', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjjadng10BeforeDelete', array('com_jtca.ljjadng10', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjjadng10AfterDelete', array('com_jtca.ljjadng10', &$table));
				}
				else
				{
					// Prune items that you can't change.
