 ...
 *
 */
class JtCaModelLjpes04 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpes04';
	...
	{
		if (empty($config['ljpes04_filter_fields']))
		{
			$config['ljpes04_filter_fields'] = array(
				'id', 'a.id',
				...
				'field1','a.field1',
				'field2','a.field2',
				'txt_field1219','a.txt_field1219',
				'id_field1219','a.id_field1219',
				'field4','a.field4',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Ljpes04s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpes04.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPES04S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpes04ChangeState', array('com_jtca.ljpes04', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpes04BeforeDelete', array('com_jtca.ljpes04', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpes04AfterDelete', array('com_jtca.ljpes04', &$table));
				}
				else
				{
					// Prune items that you can't change.
