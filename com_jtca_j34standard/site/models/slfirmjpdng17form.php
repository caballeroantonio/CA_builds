...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/slfirmjpdng17.php';

class JtcaModelSlfirmJpdng17Form extends JtcaModelSlfirmJpdng17
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.slfirmjpdng17';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.slfirmjpdng17';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.slfirmjpdng17', 'slfirmjpdng17', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the sub libro control de firmas del libro de control de firmas procesados.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Sub Libro CONTROL DE FIRMAS Del LIBRO DE CONTROL DE FIRMAS PROCESADOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('slfirmjpdng17.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.slfirmjpdng17.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('slfirmjpdng17.id');
		$is_new		= true;
		...
		// Reorder the control de firmas del libro de control de firmas p so the new sub libro control de firmas del libro de control de firmas procesados is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSlfirmJpdng17BeforeSave', array('com_jtca.slfirmjpdng17', &$table, $is_new));
		...
		$dispatcher->trigger('onSlfirmJpdng17AfterSave', array('com_jtca.slfirmjpdng17', $table, $is_new));

		$this->setState('slfirmjpdng17.id', $table->id);
