...
/**
 * This models supports retrieving lists of libro de pólizas y fianzas (oficial) - egresos.
 *
 */
class JtCaModelLsc14s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsc14s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de pólizas y fianzas (oficial) - egresos has been specified either as a parameter or in the url Request
		$pk = $params->get('lsc14_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsc14_id');
		$this->setState('filter.lsc14_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsc14_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsc14_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsc14_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsc14_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsc14_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsc14_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsc14_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsc14_id'));
		$id .= ':'.$this->getState('filter.lsc14_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsc14s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsc14s').' AS a');
		...
		// Filter by a single or group of lsc14s.
		$lsc14_id = $this->getState('filter.lsc14_id');
		if ($lsc14_id != '')
		{
			if (is_numeric($lsc14_id))
			{
				$type = $this->getState('filter.lsc14_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsc14_id);
			}
			else
			{
				if (is_array($lsc14_id))
				{
					JArrayHelper::toInteger($lsc14_id);
					$lsc14_id = implode(',', $lsc14_id);
					$type = $this->getState('filter.lsc14_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsc14_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsc14_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsc14_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de pólizas y fianzas (oficial) - egresos.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsc14_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsc14 params override menu item params only if menu param = 'use_lsc14'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsc14' and there is no lsc14 param, use global

					// create an array of just the params set to 'use_lsc14'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsc14')
						{
							// if the lsc14 has a value, use it
							if ($lsc14_params->get($key) != '')
							{
								// get the value from the lsc14
								$lsc14_array[$key] = $lsc14_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsc14_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsc14 params
					if (count($lsc14_array) > 0)
					{
						$lsc14_params = new Registry;
						$lsc14_params->loadArray($lsc14_array);
						$item->params->merge($lsc14_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsc14_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsc14, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
	...
	/**
	 * Build a list of distinct values in the NÚMERO DE PÓLIZA field
	 *
	 * @return	JDatabaseQuery
	 */
