...
/**
 * This models supports retrieving lists of agenda de audiencias para juicios orales.
 *
 */
class JtCaModelLjpes10s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpes10s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single agenda de audiencias para juicios orales has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpes10_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes10_id');
		$this->setState('filter.ljpes10_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes10_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes10_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes10_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes10_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes10_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes10_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes10_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes10_id'));
		$id .= ':'.$this->getState('filter.ljpes10_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes10s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes10s').' AS a');
		...
		// Filter by a single or group of ljpes10s.
		$ljpes10_id = $this->getState('filter.ljpes10_id');
		if ($ljpes10_id != '')
		{
			if (is_numeric($ljpes10_id))
			{
				$type = $this->getState('filter.ljpes10_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes10_id);
			}
			else
			{
				if (is_array($ljpes10_id))
				{
					JArrayHelper::toInteger($ljpes10_id);
					$ljpes10_id = implode(',', $ljpes10_id);
					$type = $this->getState('filter.ljpes10_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes10_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes10_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes10_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of agenda de audiencias para juicios orales.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes10_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes10 params override menu item params only if menu param = 'use_ljpes10'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes10' and there is no ljpes10 param, use global

					// create an array of just the params set to 'use_ljpes10'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes10_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes10')
						{
							// if the ljpes10 has a value, use it
							if ($ljpes10_params->get($key) != '')
							{
								// get the value from the ljpes10
								$ljpes10_array[$key] = $ljpes10_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes10_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes10 params
					if (count($ljpes10_array) > 0)
					{
						$ljpes10_params = new Registry;
						$ljpes10_params->loadArray($ljpes10_array);
						$item->params->merge($ljpes10_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes10_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes10, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
