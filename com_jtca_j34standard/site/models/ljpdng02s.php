...
/**
 * This models supports retrieving lists of libro de certificado de depósitos de ingreso.
 *
 */
class JtCaModelLjpdng02s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpdng02s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de certificado de depósitos de ingreso has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpdng02_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng02_id');
		$this->setState('filter.ljpdng02_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng02_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng02_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng02_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng02_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng02_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng02_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng02_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng02_id'));
		$id .= ':'.$this->getState('filter.ljpdng02_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng02s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng02s').' AS a');
		...
		if ($billete = $this->getState('filter.billete'))
		{
			...
			$query->where($db->quoteName('a.billete').' = ' . $db->quote($billete));
		}	

		// Filter by a single or group of ljpdng02s.
		$ljpdng02_id = $this->getState('filter.ljpdng02_id');
		if ($ljpdng02_id != '')
		{
			if (is_numeric($ljpdng02_id))
			{
				$type = $this->getState('filter.ljpdng02_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng02_id);
			}
			else
			{
				if (is_array($ljpdng02_id))
				{
					JArrayHelper::toInteger($ljpdng02_id);
					$ljpdng02_id = implode(',', $ljpdng02_id);
					$type = $this->getState('filter.ljpdng02_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng02_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng02_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng02_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de certificado de depósitos de ingreso.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng02_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng02 params override menu item params only if menu param = 'use_ljpdng02'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng02' and there is no ljpdng02 param, use global

					// create an array of just the params set to 'use_ljpdng02'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng02_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng02')
						{
							// if the ljpdng02 has a value, use it
							if ($ljpdng02_params->get($key) != '')
							{
								// get the value from the ljpdng02
								$ljpdng02_array[$key] = $ljpdng02_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng02_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng02 params
					if (count($ljpdng02_array) > 0)
					{
						$ljpdng02_params = new Registry;
						$ljpdng02_params->loadArray($ljpdng02_array);
						$item->params->merge($ljpdng02_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng02_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng02, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
	...
	/**
	 * Build a list of distinct values in the No. DE BILLETE field
	 *
	 * @return	JDatabaseQuery
	 */
