...
/**
 * This models supports retrieving lists of libro de actuario (oficial).
 *
 */
class JtCaModelLsc10s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsc10s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de actuario (oficial) has been specified either as a parameter or in the url Request
		$pk = $params->get('lsc10_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsc10_id');
		$this->setState('filter.lsc10_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsc10_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsc10_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsc10_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsc10_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsc10_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsc10_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsc10_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsc10_id'));
		$id .= ':'.$this->getState('filter.lsc10_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsc10s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsc10s').' AS a');
		...
		// Filter by a single or group of lsc10s.
		$lsc10_id = $this->getState('filter.lsc10_id');
		if ($lsc10_id != '')
		{
			if (is_numeric($lsc10_id))
			{
				$type = $this->getState('filter.lsc10_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsc10_id);
			}
			else
			{
				if (is_array($lsc10_id))
				{
					JArrayHelper::toInteger($lsc10_id);
					$lsc10_id = implode(',', $lsc10_id);
					$type = $this->getState('filter.lsc10_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsc10_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsc10_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsc10_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de actuario (oficial).
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsc10_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsc10 params override menu item params only if menu param = 'use_lsc10'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsc10' and there is no lsc10 param, use global

					// create an array of just the params set to 'use_lsc10'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsc10')
						{
							// if the lsc10 has a value, use it
							if ($lsc10_params->get($key) != '')
							{
								// get the value from the lsc10
								$lsc10_array[$key] = $lsc10_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsc10_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsc10 params
					if (count($lsc10_array) > 0)
					{
						$lsc10_params = new Registry;
						$lsc10_params->loadArray($lsc10_array);
						$item->params->merge($lsc10_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsc10_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsc10, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
