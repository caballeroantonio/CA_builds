...
/**
 * This models supports retrieving lists of jc12.
 *
 */
class JtCaModelLjc12s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljc12s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljc12_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc12_id');
		$this->setState('filter.ljc12_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc12_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc12_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc12_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc12_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc12_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc12_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc12_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc12_id'));
		$id .= ':'.$this->getState('filter.ljc12_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc12s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc12s').' AS a');
		...
		// Filter by a single or group of ljc12s.
		$ljc12_id = $this->getState('filter.ljc12_id');
		if ($ljc12_id != '')
		{
			if (is_numeric($ljc12_id))
			{
				$type = $this->getState('filter.ljc12_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc12_id);
			}
			else
			{
				if (is_array($ljc12_id))
				{
					JArrayHelper::toInteger($ljc12_id);
					$ljc12_id = implode(',', $ljc12_id);
					$type = $this->getState('filter.ljc12_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc12_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc12_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc12_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc12_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc12 params override menu item params only if menu param = 'use_ljc12'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc12' and there is no ljc12 param, use global

					// create an array of just the params set to 'use_ljc12'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc12')
						{
							// if the ljc12 has a value, use it
							if ($ljc12_params->get($key) != '')
							{
								// get the value from the ljc12
								$ljc12_array[$key] = $ljc12_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc12_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc12 params
					if (count($ljc12_array) > 0)
					{
						$ljc12_params = new Registry;
						$ljc12_params->loadArray($ljc12_array);
						$item->params->merge($ljc12_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc12_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc12, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
