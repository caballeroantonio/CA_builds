...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE CONTROL DE ADOPCIONES INTERNACIONALES Model
 *
 */
class JtCaModelLjomf21 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljomf21';
	...
	{
		if (empty($config['ljomf21_filter_fields']))
		{
			$config['ljomf21_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljomf21s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE CONTROL DE ADOPCIONES INTERNACIONALES data.
	 *
	 * @param	integer	$pk	The id of the libro de control de adopciones internacionales.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljomf21.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJOMF21S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjomf21ChangeState', array('com_jtca.ljomf21', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjomf21BeforeDelete', array('com_jtca.ljomf21', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjomf21AfterDelete', array('com_jtca.ljomf21', &$table));
				}
				else
				{
					// Prune items that you can't change.
