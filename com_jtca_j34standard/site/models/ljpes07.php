...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE AMPAROS CIRCULANTES Model
 *
 */
class JtcaModelLjpes07 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpes07';
	...
	{
		if (empty($config['ljpes07_filter_fields']))
		{
			$config['ljpes07_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljpes07s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE AMPAROS CIRCULANTES data.
	 *
	 * @param	integer	$pk	The id of the libro de amparos circulantes.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpes07.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPES07S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpes07ChangeState', array('com_jtca.ljpes07', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpes07BeforeDelete', array('com_jtca.ljpes07', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpes07AfterDelete', array('com_jtca.ljpes07', &$table));
				}
				else
				{
					// Prune items that you can't change.
