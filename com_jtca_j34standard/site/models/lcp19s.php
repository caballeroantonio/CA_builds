...
/**
 * This models supports retrieving lists of libro de turno para consignaciones sin detenido pr.
 *
 */
class JtcaModelLcp19s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp19s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno para consignaciones sin detenido primera instancia. has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp19_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp19_id');
		$this->setState('filter.lcp19_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp19_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp19_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp19_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp19_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp19_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp19_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp19_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp19_id'));
		$id .= ':'.$this->getState('filter.lcp19_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp19s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp19s').' AS a');
		...
		// Filter by a single or group of lcp19s.
		$lcp19_id = $this->getState('filter.lcp19_id');
		if ($lcp19_id != '')
		{
			if (is_numeric($lcp19_id))
			{
				$type = $this->getState('filter.lcp19_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp19_id);
			}
			else
			{
				if (is_array($lcp19_id))
				{
					JArrayHelper::toInteger($lcp19_id);
					$lcp19_id = implode(',', $lcp19_id);
					$type = $this->getState('filter.lcp19_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp19_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp19_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp19_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno para consignaciones sin detenido pr.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp19_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp19 params override menu item params only if menu param = 'use_lcp19'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp19' and there is no lcp19 param, use global

					// create an array of just the params set to 'use_lcp19'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp19')
						{
							// if the lcp19 has a value, use it
							if ($lcp19_params->get($key) != '')
							{
								// get the value from the lcp19
								$lcp19_array[$key] = $lcp19_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp19_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp19 params
					if (count($lcp19_array) > 0)
					{
						$lcp19_params = new Registry;
						$lcp19_params->loadArray($lcp19_array);
						$item->params->merge($lcp19_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp19_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp19, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
