...
/**
 * TSJ CDMX Libros TxCA Component LIBRETA DE LIBERTADES JUZGADOS DE ADOLESCENTES Model
 *
 */
class JtcaModelLcp34 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp34';
	...
	{
		if (empty($config['lcp34_filter_fields']))
		{
			$config['lcp34_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp34s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRETA DE LIBERTADES JUZGADOS DE ADOLESCENTES data.
	 *
	 * @param	integer	$pk	The id of the libreta de libertades juzgados de adolescentes.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp34.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP34S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp34ChangeState', array('com_jtca.lcp34', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp34BeforeDelete', array('com_jtca.lcp34', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp34AfterDelete', array('com_jtca.lcp34', &$table));
				}
				else
				{
					// Prune items that you can't change.
