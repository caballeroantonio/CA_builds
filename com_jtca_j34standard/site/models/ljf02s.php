...
class JtCaModelLjf02s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf02_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf02_id');
		$this->setState('filter.ljf02_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf02_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf02_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf02_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf02_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf02_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf02_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf02_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf02_id'));
		$id .= ':'.$this->getState('filter.ljf02_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf02s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf02s').' AS a');
		...
		// Filter by a single or group of ljf02s.
		$ljf02_id = $this->getState('filter.ljf02_id');
		if ($ljf02_id != '')
		{
			if (is_numeric($ljf02_id))
			{
				$type = $this->getState('filter.ljf02_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf02_id);
			}
			else
			{
				if (is_array($ljf02_id))
				{
					JArrayHelper::toInteger($ljf02_id);
					$ljf02_id = implode(',', $ljf02_id);
					$type = $this->getState('filter.ljf02_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf02_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf02_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf02_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf02_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf02 params override menu item params only if menu param = 'use_ljf02'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf02' and there is no ljf02 param, use global

					// create an array of just the params set to 'use_ljf02'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf02')
						{
							// if the ljf02 has a value, use it
							if ($ljf02_params->get($key) != '')
							{
								// get the value from the ljf02
								$ljf02_array[$key] = $ljf02_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf02_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf02 params
					if (count($ljf02_array) > 0)
					{
						$ljf02_params = new Registry;
						$ljf02_params->loadArray($ljf02_array);
						$item->params->merge($ljf02_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf02_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf02, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
