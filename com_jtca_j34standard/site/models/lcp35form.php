...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp35.php';

class JtCaModelLcp35Form extends JtCaModelLcp35
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp35';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp35';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp35', 'lcp35', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de libertades de salas de adolescentes.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE LIBERTADES DE SALAS DE ADOLESCENTES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp35.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp35.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp35.id');
		$is_new		= true;
		...
		// Reorder the libreta de libertades de salas de adolescentes so the new libreta de libertades de salas de adolescentes is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp35BeforeSave', array('com_jtca.lcp35', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp35AfterSave', array('com_jtca.lcp35', $table, $is_new));

		$this->setState('lcp35.id', $table->id);
