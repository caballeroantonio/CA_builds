...
class JtCaModelLjemst12s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljemst12_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljemst12_id');
		$this->setState('filter.ljemst12_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljemst12_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljemst12_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljemst12_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljemst12_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljemst12_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljemst12_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljemst12_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljemst12_id'));
		$id .= ':'.$this->getState('filter.ljemst12_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljemst12s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljemst12s').' AS a');
		...
		// Filter by a single or group of ljemst12s.
		$ljemst12_id = $this->getState('filter.ljemst12_id');
		if ($ljemst12_id != '')
		{
			if (is_numeric($ljemst12_id))
			{
				$type = $this->getState('filter.ljemst12_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljemst12_id);
			}
			else
			{
				if (is_array($ljemst12_id))
				{
					JArrayHelper::toInteger($ljemst12_id);
					$ljemst12_id = implode(',', $ljemst12_id);
					$type = $this->getState('filter.ljemst12_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljemst12_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljemst12_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljemst12_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljemst12_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljemst12 params override menu item params only if menu param = 'use_ljemst12'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljemst12' and there is no ljemst12 param, use global

					// create an array of just the params set to 'use_ljemst12'
					$menu_params_array = $this->getState('params')->toArray();
					$ljemst12_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljemst12')
						{
							// if the ljemst12 has a value, use it
							if ($ljemst12_params->get($key) != '')
							{
								// get the value from the ljemst12
								$ljemst12_array[$key] = $ljemst12_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljemst12_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljemst12 params
					if (count($ljemst12_array) > 0)
					{
						$ljemst12_params = new Registry;
						$ljemst12_params->loadArray($ljemst12_array);
						$item->params->merge($ljemst12_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljemst12_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljemst12, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
