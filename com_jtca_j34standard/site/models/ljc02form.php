...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljc02.php';

class JtCaModelLjc02Form extends JtCaModelLjc02
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljc02';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljc02';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljc02', 'ljc02', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljc02.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljc02.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljc02.id');
		$is_new		= true;
		...
		// Reorder the jc02 so the new libro de ingresos de valores is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjc02BeforeSave', array('com_jtca.ljc02', &$table, $is_new));
		...
		$dispatcher->trigger('onLjc02AfterSave', array('com_jtca.ljc02', $table, $is_new));

		$this->setState('ljc02.id', $table->id);
