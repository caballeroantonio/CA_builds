...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpes11.php';

class JtcaModelLjpes11Form extends JtcaModelLjpes11
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpes11';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpes11';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpes11', 'ljpes11', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de ingresos y egresos de billetes de depósito.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE INGRESOS Y EGRESOS DE BILLETES DE DEPÓSITO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpes11.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpes11.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpes11.id');
		$is_new		= true;
		...
		// Reorder the libro de ingresos y egresos de billetes de depósit so the new libro de ingresos y egresos de billetes de depósito is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpes11BeforeSave', array('com_jtca.ljpes11', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpes11AfterSave', array('com_jtca.ljpes11', $table, $is_new));

		$this->setState('ljpes11.id', $table->id);
