...
/**
 * This models supports retrieving lists of libros de valores.
 *
 */
class JtcaModelLjjadng12s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadng12s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libros de valores has been specified either as a parameter or in the url Request
		$pk = $params->get('ljjadng12_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadng12_id');
		$this->setState('filter.ljjadng12_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadng12_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadng12_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadng12_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadng12_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadng12_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadng12_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadng12_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadng12_id'));
		$id .= ':'.$this->getState('filter.ljjadng12_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadng12s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadng12s').' AS a');
		...
		// Filter by a single or group of ljjadng12s.
		$ljjadng12_id = $this->getState('filter.ljjadng12_id');
		if ($ljjadng12_id != '')
		{
			if (is_numeric($ljjadng12_id))
			{
				$type = $this->getState('filter.ljjadng12_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadng12_id);
			}
			else
			{
				if (is_array($ljjadng12_id))
				{
					JArrayHelper::toInteger($ljjadng12_id);
					$ljjadng12_id = implode(',', $ljjadng12_id);
					$type = $this->getState('filter.ljjadng12_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadng12_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadng12_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadng12_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libros de valores.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadng12_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadng12 params override menu item params only if menu param = 'use_ljjadng12'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadng12' and there is no ljjadng12 param, use global

					// create an array of just the params set to 'use_ljjadng12'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadng12_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadng12')
						{
							// if the ljjadng12 has a value, use it
							if ($ljjadng12_params->get($key) != '')
							{
								// get the value from the ljjadng12
								$ljjadng12_array[$key] = $ljjadng12_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadng12_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadng12 params
					if (count($ljjadng12_array) > 0)
					{
						$ljjadng12_params = new Registry;
						$ljjadng12_params->loadArray($ljjadng12_array);
						$item->params->merge($ljjadng12_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadng12_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadng12, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
	...
	/**
	 * Build a list of distinct values in the NÚMERO DE BILLETE DE DEPÓSITO field
	 *
	 * @return	JDatabaseQuery
	 */
