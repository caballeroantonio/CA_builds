...
class JtCaModelLjemst05s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljemst05_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljemst05_id');
		$this->setState('filter.ljemst05_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljemst05_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljemst05_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljemst05_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljemst05_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljemst05_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljemst05_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljemst05_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljemst05_id'));
		$id .= ':'.$this->getState('filter.ljemst05_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljemst05s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljemst05s').' AS a');
		...
		// Filter by a single or group of ljemst05s.
		$ljemst05_id = $this->getState('filter.ljemst05_id');
		if ($ljemst05_id != '')
		{
			if (is_numeric($ljemst05_id))
			{
				$type = $this->getState('filter.ljemst05_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljemst05_id);
			}
			else
			{
				if (is_array($ljemst05_id))
				{
					JArrayHelper::toInteger($ljemst05_id);
					$ljemst05_id = implode(',', $ljemst05_id);
					$type = $this->getState('filter.ljemst05_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljemst05_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljemst05_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljemst05_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljemst05_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljemst05 params override menu item params only if menu param = 'use_ljemst05'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljemst05' and there is no ljemst05 param, use global

					// create an array of just the params set to 'use_ljemst05'
					$menu_params_array = $this->getState('params')->toArray();
					$ljemst05_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljemst05')
						{
							// if the ljemst05 has a value, use it
							if ($ljemst05_params->get($key) != '')
							{
								// get the value from the ljemst05
								$ljemst05_array[$key] = $ljemst05_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljemst05_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljemst05 params
					if (count($ljemst05_array) > 0)
					{
						$ljemst05_params = new Registry;
						$ljemst05_params->loadArray($ljemst05_array);
						$item->params->merge($ljemst05_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljemst05_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljemst05, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
