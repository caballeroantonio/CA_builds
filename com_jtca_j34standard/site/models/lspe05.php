...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE DESISTIMIENTO Model
 *
 */
class JtCaModelLspe05 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lspe05';
	...
	{
		if (empty($config['lspe05_filter_fields']))
		{
			$config['lspe05_filter_fields'] = array(
				'id', 'a.id',
				'id_organo','a.id_organo',
				'anoj','a.anoj',
				'id_expediente','a.id_expediente',
				'id_secretaria','a.id_secretaria',
				'field2316','a.field2316',
				'field2317','a.field2317',
				'field2318','a.field2318',
				'field2319','a.field2319',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lspe05s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE DESISTIMIENTO data.
	 *
	 * @param	integer	$pk	The id of the libro de desistimiento.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lspe05.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPE05S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLspe05ChangeState', array('com_jtca.lspe05', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLspe05BeforeDelete', array('com_jtca.lspe05', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLspe05AfterDelete', array('com_jtca.lspe05', &$table));
				}
				else
				{
					// Prune items that you can't change.
