...
/**
 * This models supports retrieving lists of libreta de libertades juzgados de ejecuciÃ³n de sen.
 *
 */
class JtCaModelLcp32s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp32s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de libertades juzgados de ejecuciÃ³n de sentencia has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp32_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp32_id');
		$this->setState('filter.lcp32_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp32_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp32_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp32_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp32_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp32_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp32_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp32_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp32_id'));
		$id .= ':'.$this->getState('filter.lcp32_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp32s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp32s').' AS a');
		...
		// Filter by a single or group of lcp32s.
		$lcp32_id = $this->getState('filter.lcp32_id');
		if ($lcp32_id != '')
		{
			if (is_numeric($lcp32_id))
			{
				$type = $this->getState('filter.lcp32_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp32_id);
			}
			else
			{
				if (is_array($lcp32_id))
				{
					JArrayHelper::toInteger($lcp32_id);
					$lcp32_id = implode(',', $lcp32_id);
					$type = $this->getState('filter.lcp32_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp32_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp32_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp32_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de libertades juzgados de ejecuciÃ³n de sen.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp32_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp32 params override menu item params only if menu param = 'use_lcp32'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp32' and there is no lcp32 param, use global

					// create an array of just the params set to 'use_lcp32'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp32')
						{
							// if the lcp32 has a value, use it
							if ($lcp32_params->get($key) != '')
							{
								// get the value from the lcp32
								$lcp32_array[$key] = $lcp32_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp32_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp32 params
					if (count($lcp32_array) > 0)
					{
						$lcp32_params = new Registry;
						$lcp32_params->loadArray($lcp32_array);
						$item->params->merge($lcp32_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp32_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp32, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
