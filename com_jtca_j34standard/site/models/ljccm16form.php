...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljccm16.php';

class JtCaModelLjccm16Form extends JtCaModelLjccm16
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljccm16';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljccm16';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljccm16', 'ljccm16', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de remisión de expedientes al archivo.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE REMISIÓN DE EXPEDIENTES AL ARCHIVO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljccm16.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljccm16.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljccm16.id');
		$is_new		= true;
		...
		// Reorder the libro de remisión de expedientes al archivo so the new libro de remisión de expedientes al archivo is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjccm16BeforeSave', array('com_jtca.ljccm16', &$table, $is_new));
		...
		$dispatcher->trigger('onLjccm16AfterSave', array('com_jtca.ljccm16', $table, $is_new));

		$this->setState('ljccm16.id', $table->id);
