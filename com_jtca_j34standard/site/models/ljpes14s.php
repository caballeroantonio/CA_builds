...
/**
 * This models supports retrieving lists of libro de resoluciones de plano.
 *
 */
class JtCaModelLjpes14s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpes14s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de resoluciones de plano has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpes14_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes14_id');
		$this->setState('filter.ljpes14_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes14_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes14_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes14_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes14_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes14_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes14_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes14_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes14_id'));
		$id .= ':'.$this->getState('filter.ljpes14_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes14s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes14s').' AS a');
		...
		// Filter by a single or group of ljpes14s.
		$ljpes14_id = $this->getState('filter.ljpes14_id');
		if ($ljpes14_id != '')
		{
			if (is_numeric($ljpes14_id))
			{
				$type = $this->getState('filter.ljpes14_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes14_id);
			}
			else
			{
				if (is_array($ljpes14_id))
				{
					JArrayHelper::toInteger($ljpes14_id);
					$ljpes14_id = implode(',', $ljpes14_id);
					$type = $this->getState('filter.ljpes14_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes14_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes14_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes14_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de resoluciones de plano.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes14_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes14 params override menu item params only if menu param = 'use_ljpes14'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes14' and there is no ljpes14 param, use global

					// create an array of just the params set to 'use_ljpes14'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes14_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes14')
						{
							// if the ljpes14 has a value, use it
							if ($ljpes14_params->get($key) != '')
							{
								// get the value from the ljpes14
								$ljpes14_array[$key] = $ljpes14_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes14_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes14 params
					if (count($ljpes14_array) > 0)
					{
						$ljpes14_params = new Registry;
						$ljpes14_params->loadArray($ljpes14_array);
						$item->params->merge($ljpes14_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes14_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes14, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
