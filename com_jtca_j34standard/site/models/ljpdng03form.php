...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpdng03.php';

class JtCaModelLjpdng03Form extends JtCaModelLjpdng03
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpdng03';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpdng03';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpdng03', 'ljpdng03', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de certificado de depósitos de egreso.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE CERTIFICADO DE DEPÓSITOS DE EGRESO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpdng03.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpdng03.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpdng03.id');
		$is_new		= true;
		...
		// Reorder the libro de certificado de depósitos de egreso so the new libro de certificado de depósitos de egreso is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpdng03BeforeSave', array('com_jtca.ljpdng03', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpdng03AfterSave', array('com_jtca.ljpdng03', $table, $is_new));

		$this->setState('ljpdng03.id', $table->id);
