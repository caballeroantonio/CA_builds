...
class JtCaModelLcp19 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp19';
	...
	{
		if (empty($config['lcp19_filter_fields']))
		{
	...
	*/
	...
	/**
	 * Method to get LIBRO DE TURNO PARA CONSIGNACIONES SIN DETENIDO PRIMERA INSTANCIA. data.
	 *
	 * @param	integer	$pk	The id of the libro de turno para consignaciones sin detenido primera instancia..
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp19.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP19S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp19ChangeState', array('com_jtca.lcp19', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp19BeforeDelete', array('com_jtca.lcp19', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp19AfterDelete', array('com_jtca.lcp19', &$table));
				}
				else
				{
					// Prune items that you can't change.
