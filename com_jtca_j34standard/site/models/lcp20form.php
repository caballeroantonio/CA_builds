...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp20.php';

class JtCaModelLcp20Form extends JtCaModelLcp20
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp20';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp20';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp20', 'lcp20', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno para incompetencias primera instancia..
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO PARA INCOMPETENCIAS PRIMERA INSTANCIA. item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp20.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp20.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp20.id');
		$is_new		= true;
		...
		// Reorder the libro de turno para incompetencias primera instanc so the new libro de turno para incompetencias primera instancia. is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp20BeforeSave', array('com_jtca.lcp20', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp20AfterSave', array('com_jtca.lcp20', $table, $is_new));

		$this->setState('lcp20.id', $table->id);
