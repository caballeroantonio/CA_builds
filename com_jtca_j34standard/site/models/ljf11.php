...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE REGISTRO PARA NOTARIOS Model
 *
 */
class JtCaModelLjf11 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljf11';
	...
	{
		if (empty($config['ljf11_filter_fields']))
		{
			$config['ljf11_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljf11s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE REGISTRO PARA NOTARIOS data.
	 *
	 * @param	integer	$pk	The id of the libro de registro para notarios.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljf11.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJF11S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjf11ChangeState', array('com_jtca.ljf11', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjf11BeforeDelete', array('com_jtca.ljf11', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjf11AfterDelete', array('com_jtca.ljf11', &$table));
				}
				else
				{
					// Prune items that you can't change.
