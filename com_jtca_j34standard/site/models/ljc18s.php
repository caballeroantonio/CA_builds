...
/**
 * This models supports retrieving lists of jc18.
 *
 */
class JtCaModelLjc18s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljc18s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de remisiÃ³n de documentos al archivo has been specified either as a parameter or in the url Request
		$pk = $params->get('ljc18_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc18_id');
		$this->setState('filter.ljc18_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc18_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc18_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc18_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc18_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc18_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc18_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc18_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc18_id'));
		$id .= ':'.$this->getState('filter.ljc18_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc18s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc18s').' AS a');
		...
		// Filter by a single or group of ljc18s.
		$ljc18_id = $this->getState('filter.ljc18_id');
		if ($ljc18_id != '')
		{
			if (is_numeric($ljc18_id))
			{
				$type = $this->getState('filter.ljc18_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc18_id);
			}
			else
			{
				if (is_array($ljc18_id))
				{
					JArrayHelper::toInteger($ljc18_id);
					$ljc18_id = implode(',', $ljc18_id);
					$type = $this->getState('filter.ljc18_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc18_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc18_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc18_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc18_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc18 params override menu item params only if menu param = 'use_ljc18'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc18' and there is no ljc18 param, use global

					// create an array of just the params set to 'use_ljc18'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc18')
						{
							// if the ljc18 has a value, use it
							if ($ljc18_params->get($key) != '')
							{
								// get the value from the ljc18
								$ljc18_array[$key] = $ljc18_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc18_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc18 params
					if (count($ljc18_array) > 0)
					{
						$ljc18_params = new Registry;
						$ljc18_params->loadArray($ljc18_array);
						$item->params->merge($ljc18_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc18_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc18, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
