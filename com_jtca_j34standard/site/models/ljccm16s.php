...
/**
 * This models supports retrieving lists of libro de remisiÃ³n de expedientes al archivo.
 *
 */
class JtCaModelLjccm16s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljccm16s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de remisiÃ³n de expedientes al archivo has been specified either as a parameter or in the url Request
		$pk = $params->get('ljccm16_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljccm16_id');
		$this->setState('filter.ljccm16_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljccm16_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljccm16_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljccm16_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljccm16_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljccm16_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljccm16_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljccm16_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljccm16_id'));
		$id .= ':'.$this->getState('filter.ljccm16_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljccm16s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljccm16s').' AS a');
		...
		// Filter by a single or group of ljccm16s.
		$ljccm16_id = $this->getState('filter.ljccm16_id');
		if ($ljccm16_id != '')
		{
			if (is_numeric($ljccm16_id))
			{
				$type = $this->getState('filter.ljccm16_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljccm16_id);
			}
			else
			{
				if (is_array($ljccm16_id))
				{
					JArrayHelper::toInteger($ljccm16_id);
					$ljccm16_id = implode(',', $ljccm16_id);
					$type = $this->getState('filter.ljccm16_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljccm16_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljccm16_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljccm16_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de remisiÃ³n de expedientes al archivo.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljccm16_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljccm16 params override menu item params only if menu param = 'use_ljccm16'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljccm16' and there is no ljccm16 param, use global

					// create an array of just the params set to 'use_ljccm16'
					$menu_params_array = $this->getState('params')->toArray();
					$ljccm16_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljccm16')
						{
							// if the ljccm16 has a value, use it
							if ($ljccm16_params->get($key) != '')
							{
								// get the value from the ljccm16
								$ljccm16_array[$key] = $ljccm16_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljccm16_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljccm16 params
					if (count($ljccm16_array) > 0)
					{
						$ljccm16_params = new Registry;
						$ljccm16_params->loadArray($ljccm16_array);
						$item->params->merge($ljccm16_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljccm16_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljccm16, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
