...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpes18.php';

class JtcaModelLjpes18Form extends JtcaModelLjpes18
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpes18';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpes18';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpes18', 'ljpes18', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpes18.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpes18.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpes18.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjpes18BeforeSave', array('com_jtca.ljpes18', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpes18AfterSave', array('com_jtca.ljpes18', $table, $is_new));

		$this->setState('ljpes18.id', $table->id);
