...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljp02.php';

class JtCaModelLjp02Form extends JtCaModelLjp02
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljp02';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljp02';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljp02', 'ljp02', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de registro de billetes de depósito (ingresos).
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE REGISTRO DE BILLETES DE DEPÓSITO (INGRESOS) item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljp02.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljp02.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljp02.id');
		$is_new		= true;
		...
		// Reorder the libro de registro de billetes de depósito (ingreso so the new libro de registro de billetes de depósito (ingresos) is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjp02BeforeSave', array('com_jtca.ljp02', &$table, $is_new));
		...
		$dispatcher->trigger('onLjp02AfterSave', array('com_jtca.ljp02', $table, $is_new));

		$this->setState('ljp02.id', $table->id);
