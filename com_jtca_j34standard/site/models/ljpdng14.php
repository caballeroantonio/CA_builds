...
/**
 * TSJ CDMX Libros TxCA Component AGENDA DE AUDIENCIAS Model
 *
 */
class JtCaModelLjpdng14 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpdng14';
	...
	{
		if (empty($config['ljpdng14_filter_fields']))
		{
			$config['ljpdng14_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljpdng14s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get AGENDA DE AUDIENCIAS data.
	 *
	 * @param	integer	$pk	The id of the agenda de audiencias.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpdng14.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPDNG14S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpdng14ChangeState', array('com_jtca.ljpdng14', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpdng14BeforeDelete', array('com_jtca.ljpdng14', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpdng14AfterDelete', array('com_jtca.ljpdng14', &$table));
				}
				else
				{
					// Prune items that you can't change.
