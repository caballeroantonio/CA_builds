...
/**
 * This models supports retrieving lists of libro de gobierno (sistema tradicional).
 *
 */
class JtCaModelLsps01s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsps01s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de gobierno (sistema tradicional) has been specified either as a parameter or in the url Request
		$pk = $params->get('lsps01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsps01_id');
		$this->setState('filter.lsps01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsps01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsps01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsps01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsps01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsps01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsps01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsps01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsps01_id'));
		$id .= ':'.$this->getState('filter.lsps01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsps01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsps01s').' AS a');
		...
		// Filter by a single or group of lsps01s.
		$lsps01_id = $this->getState('filter.lsps01_id');
		if ($lsps01_id != '')
		{
			if (is_numeric($lsps01_id))
			{
				$type = $this->getState('filter.lsps01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsps01_id);
			}
			else
			{
				if (is_array($lsps01_id))
				{
					JArrayHelper::toInteger($lsps01_id);
					$lsps01_id = implode(',', $lsps01_id);
					$type = $this->getState('filter.lsps01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsps01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsps01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsps01_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de gobierno (sistema tradicional).
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsps01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsps01 params override menu item params only if menu param = 'use_lsps01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsps01' and there is no lsps01 param, use global

					// create an array of just the params set to 'use_lsps01'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsps01')
						{
							// if the lsps01 has a value, use it
							if ($lsps01_params->get($key) != '')
							{
								// get the value from the lsps01
								$lsps01_array[$key] = $lsps01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsps01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsps01 params
					if (count($lsps01_array) > 0)
					{
						$lsps01_params = new Registry;
						$lsps01_params->loadArray($lsps01_array);
						$item->params->merge($lsps01_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsps01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsps01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
