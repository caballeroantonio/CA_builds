...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/sldiljccm07.php';

class JtcaModelSldilJccm07Form extends JtcaModelSldilJccm07
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.sldiljccm07';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.sldiljccm07';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.sldiljccm07', 'sldiljccm07', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the sub libro diligencia del libro de exhortos.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Sub Libro DILIGENCIA Del LIBRO DE EXHORTOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('sldiljccm07.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.sldiljccm07.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('sldiljccm07.id');
		$is_new		= true;
		...
		// Reorder the diligencia del libro de exhortos so the new sub libro diligencia del libro de exhortos is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSldilJccm07BeforeSave', array('com_jtca.sldiljccm07', &$table, $is_new));
		...
		$dispatcher->trigger('onSldilJccm07AfterSave', array('com_jtca.sldiljccm07', $table, $is_new));

		$this->setState('sldiljccm07.id', $table->id);
