...
/**
 * This models supports retrieving lists of libro de gobierno.
 *
 */
class JtcaModelLjjadng01s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadng01s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadng01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadng01_id');
		$this->setState('filter.ljjadng01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadng01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadng01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadng01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadng01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadng01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadng01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadng01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadng01_id'));
		$id .= ':'.$this->getState('filter.ljjadng01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadng01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadng01s').' AS a');
		...
		// Filter by a single or group of ljjadng01s.
		$ljjadng01_id = $this->getState('filter.ljjadng01_id');
		if ($ljjadng01_id != '')
		{
			if (is_numeric($ljjadng01_id))
			{
				$type = $this->getState('filter.ljjadng01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadng01_id);
			}
			else
			{
				if (is_array($ljjadng01_id))
				{
					JArrayHelper::toInteger($ljjadng01_id);
					$ljjadng01_id = implode(',', $ljjadng01_id);
					$type = $this->getState('filter.ljjadng01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadng01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadng01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadng01_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de gobierno.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadng01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadng01 params override menu item params only if menu param = 'use_ljjadng01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadng01' and there is no ljjadng01 param, use global

					// create an array of just the params set to 'use_ljjadng01'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadng01_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadng01')
						{
							// if the ljjadng01 has a value, use it
							if ($ljjadng01_params->get($key) != '')
							{
								// get the value from the ljjadng01
								$ljjadng01_array[$key] = $ljjadng01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadng01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadng01 params
					if (count($ljjadng01_array) > 0)
					{
						$ljjadng01_params = new Registry;
						$ljjadng01_params->loadArray($ljjadng01_array);
						$item->params->merge($ljjadng01_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadng01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadng01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
