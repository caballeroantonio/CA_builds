...
/**
 * This models supports retrieving lists of libro de exhortos.
 *
 */
class JtCaModelLjjadng06s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadng06s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de exhortos has been specified either as a parameter or in the url Request
		$pk = $params->get('ljjadng06_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadng06_id');
		$this->setState('filter.ljjadng06_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadng06_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadng06_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadng06_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadng06_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadng06_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadng06_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadng06_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadng06_id'));
		$id .= ':'.$this->getState('filter.ljjadng06_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadng06s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadng06s').' AS a');
		...
		// Filter by a single or group of ljjadng06s.
		$ljjadng06_id = $this->getState('filter.ljjadng06_id');
		if ($ljjadng06_id != '')
		{
			if (is_numeric($ljjadng06_id))
			{
				$type = $this->getState('filter.ljjadng06_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadng06_id);
			}
			else
			{
				if (is_array($ljjadng06_id))
				{
					JArrayHelper::toInteger($ljjadng06_id);
					$ljjadng06_id = implode(',', $ljjadng06_id);
					$type = $this->getState('filter.ljjadng06_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadng06_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadng06_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadng06_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de exhortos.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadng06_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadng06 params override menu item params only if menu param = 'use_ljjadng06'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadng06' and there is no ljjadng06 param, use global

					// create an array of just the params set to 'use_ljjadng06'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadng06_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadng06')
						{
							// if the ljjadng06 has a value, use it
							if ($ljjadng06_params->get($key) != '')
							{
								// get the value from the ljjadng06
								$ljjadng06_array[$key] = $ljjadng06_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadng06_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadng06 params
					if (count($ljjadng06_array) > 0)
					{
						$ljjadng06_params = new Registry;
						$ljjadng06_params->loadArray($ljjadng06_array);
						$item->params->merge($ljjadng06_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadng06_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadng06, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
