...
class JtcaModelLspe01s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('lspe01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lspe01_id');
		$this->setState('filter.lspe01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lspe01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lspe01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lspe01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lspe01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lspe01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lspe01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lspe01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lspe01_id'));
		$id .= ':'.$this->getState('filter.lspe01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lspe01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lspe01s').' AS a');
		...
		// Filter by a single or group of lspe01s.
		$lspe01_id = $this->getState('filter.lspe01_id');
		if ($lspe01_id != '')
		{
			if (is_numeric($lspe01_id))
			{
				$type = $this->getState('filter.lspe01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lspe01_id);
			}
			else
			{
				if (is_array($lspe01_id))
				{
					JArrayHelper::toInteger($lspe01_id);
					$lspe01_id = implode(',', $lspe01_id);
					$type = $this->getState('filter.lspe01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lspe01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lspe01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lspe01_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lspe01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lspe01 params override menu item params only if menu param = 'use_lspe01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lspe01' and there is no lspe01 param, use global

					// create an array of just the params set to 'use_lspe01'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lspe01')
						{
							// if the lspe01 has a value, use it
							if ($lspe01_params->get($key) != '')
							{
								// get the value from the lspe01
								$lspe01_array[$key] = $lspe01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lspe01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lspe01 params
					if (count($lspe01_array) > 0)
					{
						$lspe01_params = new Registry;
						$lspe01_params->loadArray($lspe01_array);
						$item->params->merge($lspe01_params);
					}


					// get display date
					switch ($item->params->get('list_show_lspe01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lspe01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
