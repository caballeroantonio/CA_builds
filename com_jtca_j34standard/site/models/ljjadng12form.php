...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadng12.php';

class JtCaModelLjjadng12Form extends JtCaModelLjjadng12
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadng12';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadng12';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadng12', 'ljjadng12', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libros de valores.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBROS DE VALORES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadng12.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadng12.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadng12.id');
		$is_new		= true;
		...
		// Reorder the libros de valores so the new libros de valores is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadng12BeforeSave', array('com_jtca.ljjadng12', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadng12AfterSave', array('com_jtca.ljjadng12', $table, $is_new));

		$this->setState('ljjadng12.id', $table->id);
