...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/slpersondelito.php';

class JtcaModelSlpersondelitoForm extends JtcaModelSlpersondelito
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.slpersondelito';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.slpersondelito';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.slpersondelito', 'slpersondelito', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the inculpados.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Inculpados item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('slpersondelito.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.slpersondelito.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('slpersondelito.id');
		$is_new		= true;
		...
		// Reorder the inculpados so the new inculpados is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSlpersondelitoBeforeSave', array('com_jtca.slpersondelito', &$table, $is_new));
		...
		$dispatcher->trigger('onSlpersondelitoAfterSave', array('com_jtca.slpersondelito', $table, $is_new));

		$this->setState('slpersondelito.id', $table->id);
