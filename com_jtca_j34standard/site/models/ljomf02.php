 ...
 *
 */
class JtCaModelLjomf02 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljomf02';
	...
	{
		if (empty($config['ljomf02_filter_fields']))
		{
			$config['ljomf02_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljomf02s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE INGRESOS DE VALORES data.
	 *
	 * @param	integer	$pk	The id of the libro de ingresos de valores.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljomf02.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJOMF02S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjomf02ChangeState', array('com_jtca.ljomf02', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjomf02BeforeDelete', array('com_jtca.ljomf02', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjomf02AfterDelete', array('com_jtca.ljomf02', &$table));
				}
				else
				{
					// Prune items that you can't change.
