...
class JtCaModelLjoc01s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljoc01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljoc01_id');
		$this->setState('filter.ljoc01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljoc01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljoc01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljoc01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljoc01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljoc01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljoc01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljoc01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljoc01_id'));
		$id .= ':'.$this->getState('filter.ljoc01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljoc01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljoc01s').' AS a');
		...
		// Filter by a single or group of ljoc01s.
		$ljoc01_id = $this->getState('filter.ljoc01_id');
		if ($ljoc01_id != '')
		{
			if (is_numeric($ljoc01_id))
			{
				$type = $this->getState('filter.ljoc01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljoc01_id);
			}
			else
			{
				if (is_array($ljoc01_id))
				{
					JArrayHelper::toInteger($ljoc01_id);
					$ljoc01_id = implode(',', $ljoc01_id);
					$type = $this->getState('filter.ljoc01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljoc01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljoc01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljoc01_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljoc01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljoc01 params override menu item params only if menu param = 'use_ljoc01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljoc01' and there is no ljoc01 param, use global

					// create an array of just the params set to 'use_ljoc01'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljoc01')
						{
							// if the ljoc01 has a value, use it
							if ($ljoc01_params->get($key) != '')
							{
								// get the value from the ljoc01
								$ljoc01_array[$key] = $ljoc01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljoc01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljoc01 params
					if (count($ljoc01_array) > 0)
					{
						$ljoc01_params = new Registry;
						$ljoc01_params->loadArray($ljoc01_array);
						$item->params->merge($ljoc01_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljoc01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljoc01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
