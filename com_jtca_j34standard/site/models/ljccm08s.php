...
class JtCaModelLjccm08s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljccm08_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljccm08_id');
		$this->setState('filter.ljccm08_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljccm08_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljccm08_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljccm08_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljccm08_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljccm08_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljccm08_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljccm08_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljccm08_id'));
		$id .= ':'.$this->getState('filter.ljccm08_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljccm08s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljccm08s').' AS a');
		...
		// Filter by a single or group of ljccm08s.
		$ljccm08_id = $this->getState('filter.ljccm08_id');
		if ($ljccm08_id != '')
		{
			if (is_numeric($ljccm08_id))
			{
				$type = $this->getState('filter.ljccm08_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljccm08_id);
			}
			else
			{
				if (is_array($ljccm08_id))
				{
					JArrayHelper::toInteger($ljccm08_id);
					$ljccm08_id = implode(',', $ljccm08_id);
					$type = $this->getState('filter.ljccm08_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljccm08_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljccm08_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljccm08_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljccm08_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljccm08 params override menu item params only if menu param = 'use_ljccm08'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljccm08' and there is no ljccm08 param, use global

					// create an array of just the params set to 'use_ljccm08'
					$menu_params_array = $this->getState('params')->toArray();
					$ljccm08_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljccm08')
						{
							// if the ljccm08 has a value, use it
							if ($ljccm08_params->get($key) != '')
							{
								// get the value from the ljccm08
								$ljccm08_array[$key] = $ljccm08_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljccm08_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljccm08 params
					if (count($ljccm08_array) > 0)
					{
						$ljccm08_params = new Registry;
						$ljccm08_params->loadArray($ljccm08_array);
						$item->params->merge($ljccm08_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljccm08_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljccm08, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
