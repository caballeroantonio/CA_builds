...
class JtCaModelLjpes01s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpes01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes01_id');
		$this->setState('filter.ljpes01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes01_id'));
		$id .= ':'.$this->getState('filter.ljpes01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes01s').' AS a');
		...
		// Filter by a single or group of ljpes01s.
		$ljpes01_id = $this->getState('filter.ljpes01_id');
		if ($ljpes01_id != '')
		{
			if (is_numeric($ljpes01_id))
			{
				$type = $this->getState('filter.ljpes01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes01_id);
			}
			else
			{
				if (is_array($ljpes01_id))
				{
					JArrayHelper::toInteger($ljpes01_id);
					$ljpes01_id = implode(',', $ljpes01_id);
					$type = $this->getState('filter.ljpes01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes01_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes01 params override menu item params only if menu param = 'use_ljpes01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes01' and there is no ljpes01 param, use global

					// create an array of just the params set to 'use_ljpes01'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes01_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes01')
						{
							// if the ljpes01 has a value, use it
							if ($ljpes01_params->get($key) != '')
							{
								// get the value from the ljpes01
								$ljpes01_array[$key] = $ljpes01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes01 params
					if (count($ljpes01_array) > 0)
					{
						$ljpes01_params = new Registry;
						$ljpes01_params->loadArray($ljpes01_array);
						$item->params->merge($ljpes01_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
