...
/**
 * This models supports retrieving lists of libro de remisiÃ³n de documentos al archivo.
 *
 */
class JtCaModelLjomf18s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljomf18s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf18_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf18_id');
		$this->setState('filter.ljomf18_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf18_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf18_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf18_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf18_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf18_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf18_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf18_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf18_id'));
		$id .= ':'.$this->getState('filter.ljomf18_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf18s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf18s').' AS a');
		...
		// Filter by a single or group of ljomf18s.
		$ljomf18_id = $this->getState('filter.ljomf18_id');
		if ($ljomf18_id != '')
		{
			if (is_numeric($ljomf18_id))
			{
				$type = $this->getState('filter.ljomf18_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf18_id);
			}
			else
			{
				if (is_array($ljomf18_id))
				{
					JArrayHelper::toInteger($ljomf18_id);
					$ljomf18_id = implode(',', $ljomf18_id);
					$type = $this->getState('filter.ljomf18_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf18_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf18_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf18_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de remisiÃ³n de documentos al archivo.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf18_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf18 params override menu item params only if menu param = 'use_ljomf18'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf18' and there is no ljomf18 param, use global

					// create an array of just the params set to 'use_ljomf18'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf18_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf18')
						{
							// if the ljomf18 has a value, use it
							if ($ljomf18_params->get($key) != '')
							{
								// get the value from the ljomf18
								$ljomf18_array[$key] = $ljomf18_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf18_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf18 params
					if (count($ljomf18_array) > 0)
					{
						$ljomf18_params = new Registry;
						$ljomf18_params->loadArray($ljomf18_array);
						$item->params->merge($ljomf18_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf18_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf18, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
