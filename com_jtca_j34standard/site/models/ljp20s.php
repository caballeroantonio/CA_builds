...
/**
 * This models supports retrieving lists of libro de turno para conclusiones.
 *
 */
class JtCaModelLjp20s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljp20s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno para conclusiones has been specified either as a parameter or in the url Request
		$pk = $params->get('ljp20_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp20_id');
		$this->setState('filter.ljp20_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp20_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp20_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp20_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp20_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp20_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp20_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp20_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp20_id'));
		$id .= ':'.$this->getState('filter.ljp20_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp20s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp20s').' AS a');
		...
		// Filter by a single or group of ljp20s.
		$ljp20_id = $this->getState('filter.ljp20_id');
		if ($ljp20_id != '')
		{
			if (is_numeric($ljp20_id))
			{
				$type = $this->getState('filter.ljp20_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp20_id);
			}
			else
			{
				if (is_array($ljp20_id))
				{
					JArrayHelper::toInteger($ljp20_id);
					$ljp20_id = implode(',', $ljp20_id);
					$type = $this->getState('filter.ljp20_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp20_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp20_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp20_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno para conclusiones.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp20_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp20 params override menu item params only if menu param = 'use_ljp20'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp20' and there is no ljp20 param, use global

					// create an array of just the params set to 'use_ljp20'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp20')
						{
							// if the ljp20 has a value, use it
							if ($ljp20_params->get($key) != '')
							{
								// get the value from the ljp20
								$ljp20_array[$key] = $ljp20_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp20_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp20 params
					if (count($ljp20_array) > 0)
					{
						$ljp20_params = new Registry;
						$ljp20_params->loadArray($ljp20_array);
						$item->params->merge($ljp20_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp20_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp20, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
