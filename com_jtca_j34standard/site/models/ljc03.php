...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE EGRESOS DE VALORES Model
 *
 */
class JtCaModelLjc03 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljc03';
	...
	{
		if (empty($config['ljc03_filter_fields']))
		{
			$config['ljc03_filter_fields'] = array(
				'id', 'a.id',
				'billete', 'a.billete',
				...
				'anoj','a.anoj',
				'field1_paterno','a.field1_paterno',
				'field1_materno','a.field1_materno',
				'field1_nombre','a.field1_nombre',
				'field1_isMoral','a.field1_isMoral',
				'field1_isMoral','a.field1_isMoral',
				'field1_paterno','a.field1_paterno',
				'field1_materno','a.field1_materno',
				'field1_nombre','a.field1_nombre',
	...
	*/
	public function getTable($type = 'Ljc03s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE EGRESOS DE VALORES data.
	 *
	 * @param	integer	$pk	The id of the libro de egresos de valores.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljc03.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJC03S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjc03ChangeState', array('com_jtca.ljc03', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjc03BeforeDelete', array('com_jtca.ljc03', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjc03AfterDelete', array('com_jtca.ljc03', &$table));
				}
				else
				{
					// Prune items that you can't change.
