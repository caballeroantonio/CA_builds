...
class JtCaModelLjpdng13s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de control de multas has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpdng13_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng13_id');
		$this->setState('filter.ljpdng13_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng13_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng13_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng13_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng13_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng13_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng13_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng13_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng13_id'));
		$id .= ':'.$this->getState('filter.ljpdng13_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng13s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng13s').' AS a');
		...
		// Filter by a single or group of ljpdng13s.
		$ljpdng13_id = $this->getState('filter.ljpdng13_id');
		if ($ljpdng13_id != '')
		{
			if (is_numeric($ljpdng13_id))
			{
				$type = $this->getState('filter.ljpdng13_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng13_id);
			}
			else
			{
				if (is_array($ljpdng13_id))
				{
					JArrayHelper::toInteger($ljpdng13_id);
					$ljpdng13_id = implode(',', $ljpdng13_id);
					$type = $this->getState('filter.ljpdng13_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng13_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng13_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng13_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de control de multas.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng13_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng13 params override menu item params only if menu param = 'use_ljpdng13'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng13' and there is no ljpdng13 param, use global

					// create an array of just the params set to 'use_ljpdng13'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng13_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng13')
						{
							// if the ljpdng13 has a value, use it
							if ($ljpdng13_params->get($key) != '')
							{
								// get the value from the ljpdng13
								$ljpdng13_array[$key] = $ljpdng13_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng13_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng13 params
					if (count($ljpdng13_array) > 0)
					{
						$ljpdng13_params = new Registry;
						$ljpdng13_params->loadArray($ljpdng13_array);
						$item->params->merge($ljpdng13_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng13_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng13, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
