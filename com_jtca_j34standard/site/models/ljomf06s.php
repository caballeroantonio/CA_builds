...
class JtCaModelLjomf06s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf06_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf06_id');
		$this->setState('filter.ljomf06_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf06_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf06_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf06_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf06_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf06_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf06_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf06_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf06_id'));
		$id .= ':'.$this->getState('filter.ljomf06_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf06s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf06s').' AS a');
		...
		// Filter by a single or group of ljomf06s.
		$ljomf06_id = $this->getState('filter.ljomf06_id');
		if ($ljomf06_id != '')
		{
			if (is_numeric($ljomf06_id))
			{
				$type = $this->getState('filter.ljomf06_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf06_id);
			}
			else
			{
				if (is_array($ljomf06_id))
				{
					JArrayHelper::toInteger($ljomf06_id);
					$ljomf06_id = implode(',', $ljomf06_id);
					$type = $this->getState('filter.ljomf06_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf06_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf06_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf06_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf06_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf06 params override menu item params only if menu param = 'use_ljomf06'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf06' and there is no ljomf06 param, use global

					// create an array of just the params set to 'use_ljomf06'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf06_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf06')
						{
							// if the ljomf06 has a value, use it
							if ($ljomf06_params->get($key) != '')
							{
								// get the value from the ljomf06
								$ljomf06_array[$key] = $ljomf06_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf06_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf06 params
					if (count($ljomf06_array) > 0)
					{
						$ljomf06_params = new Registry;
						$ljomf06_params->loadArray($ljomf06_array);
						$item->params->merge($ljomf06_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf06_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf06, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
