...
class JtCaModelLjc20 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljc20';
	...
	{
		if (empty($config['ljc20_filter_fields']))
		{
			$config['ljc20_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljc20s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljc20.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJC20S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjc20ChangeState', array('com_jtca.ljc20', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjc20BeforeDelete', array('com_jtca.ljc20', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjc20AfterDelete', array('com_jtca.ljc20', &$table));
				}
				else
				{
					// Prune items that you can't change.
