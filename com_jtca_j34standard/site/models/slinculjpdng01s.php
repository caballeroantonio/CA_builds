...
/**
 * This models supports retrieving lists of datos del(os) inculpado(s) del libro de gobierno.
 *
 */
class JtcaModelSlinculJpdng01s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.slincul_jpdng01s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single sub libro datos del(os) inculpado(s) del libro de gobierno has been specified either as a parameter or in the url Request
		$pk = $params->get('slincul_jpdng01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('slincul_jpdng01_id');
		$this->setState('filter.slincul_jpdng01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('slinculjpdng01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('slinculjpdng01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('slinculjpdng01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('slinculjpdng01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('slinculjpdng01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_slinculjpdng01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_slinculjpdng01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.slincul_jpdng01_id'));
		$id .= ':'.$this->getState('filter.slincul_jpdng01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of slincul_jpdng01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_slincul_jpdng01s').' AS a');
		...
		// Filter by a single or group of slincul_jpdng01s.
		$slincul_jpdng01_id = $this->getState('filter.slincul_jpdng01_id');
		if ($slincul_jpdng01_id != '')
		{
			if (is_numeric($slincul_jpdng01_id))
			{
				$type = $this->getState('filter.slincul_jpdng01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $slincul_jpdng01_id);
			}
			else
			{
				if (is_array($slincul_jpdng01_id))
				{
					JArrayHelper::toInteger($slincul_jpdng01_id);
					$slincul_jpdng01_id = implode(',', $slincul_jpdng01_id);
					$type = $this->getState('filter.slincul_jpdng01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$slincul_jpdng01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_slinculjpdng01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_slinculjpdng01_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of datos del(os) inculpado(s) del libro de gobierno.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$slincul_jpdng01_params = new Registry;
				...
				{
					$item->params = $slincul_jpdng01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// SlinculJpdng01 params override menu item params only if menu param = 'use_slinculjpdng01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_slinculjpdng01' and there is no slinculjpdng01 param, use global

					// create an array of just the params set to 'use_slinculjpdng01'
					$menu_params_array = $this->getState('params')->toArray();
					$slincul_jpdng01_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_slinculjpdng01')
						{
							// if the slinculjpdng01 has a value, use it
							if ($slincul_jpdng01_params->get($key) != '')
							{
								// get the value from the slinculjpdng01
								$slincul_jpdng01_array[$key] = $slincul_jpdng01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$slincul_jpdng01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected slinculjpdng01 params
					if (count($slincul_jpdng01_array) > 0)
					{
						$slincul_jpdng01_params = new Registry;
						$slincul_jpdng01_params->loadArray($slincul_jpdng01_array);
						$item->params->merge($slincul_jpdng01_params);
					}


					// get display date
					switch ($item->params->get('list_show_slinculjpdng01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an slinculjpdng01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
