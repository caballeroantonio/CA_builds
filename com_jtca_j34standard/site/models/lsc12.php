...
class JtCaModelLsc12 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsc12';
	...
	{
		if (empty($config['lsc12_filter_fields']))
		{
			$config['lsc12_filter_fields'] = array(
				'id', 'a.id',
				...
				'field216','a.field216',
				'field217','a.field217',
				'field219_isMoral','a.field219_isMoral',
				'field219_paterno','a.field219_paterno',
				'field219_materno','a.field219_materno',
				'field219_nombre','a.field219_nombre',
				'field220','a.field220',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	...
	/**
	 * Method to get LIBRO DE REGISTRO DE AMPAROS (OFICIAL) data.
	 *
	 * @param	integer	$pk	The id of the libro de registro de amparos (oficial).
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsc12.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSC12S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsc12ChangeState', array('com_jtca.lsc12', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsc12BeforeDelete', array('com_jtca.lsc12', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsc12AfterDelete', array('com_jtca.lsc12', &$table));
				}
				else
				{
					// Prune items that you can't change.
