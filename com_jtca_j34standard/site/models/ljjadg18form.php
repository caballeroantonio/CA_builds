...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadg18.php';

class JtcaModelLjjadg18Form extends JtcaModelLjjadg18
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadg18';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadg18';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadg18', 'ljjadg18', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadg18.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadg18.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadg18.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjjadg18BeforeSave', array('com_jtca.ljjadg18', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadg18AfterSave', array('com_jtca.ljjadg18', $table, $is_new));

		$this->setState('ljjadg18.id', $table->id);
