...
class JtCaModelLjf19s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf19_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf19_id');
		$this->setState('filter.ljf19_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf19_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf19_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf19_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf19_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf19_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf19_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf19_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf19_id'));
		$id .= ':'.$this->getState('filter.ljf19_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf19s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf19s').' AS a');
		...
		// Filter by a single or group of ljf19s.
		$ljf19_id = $this->getState('filter.ljf19_id');
		if ($ljf19_id != '')
		{
			if (is_numeric($ljf19_id))
			{
				$type = $this->getState('filter.ljf19_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf19_id);
			}
			else
			{
				if (is_array($ljf19_id))
				{
					JArrayHelper::toInteger($ljf19_id);
					$ljf19_id = implode(',', $ljf19_id);
					$type = $this->getState('filter.ljf19_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf19_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf19_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf19_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf19_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf19 params override menu item params only if menu param = 'use_ljf19'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf19' and there is no ljf19 param, use global

					// create an array of just the params set to 'use_ljf19'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf19')
						{
							// if the ljf19 has a value, use it
							if ($ljf19_params->get($key) != '')
							{
								// get the value from the ljf19
								$ljf19_array[$key] = $ljf19_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf19_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf19 params
					if (count($ljf19_array) > 0)
					{
						$ljf19_params = new Registry;
						$ljf19_params->loadArray($ljf19_array);
						$item->params->merge($ljf19_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf19_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf19, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
