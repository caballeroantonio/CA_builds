...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp36.php';

class JtcaModelLcp36Form extends JtcaModelLcp36
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp36';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp36';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp36', 'lcp36', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de returno de adolescentes.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE RETURNO DE ADOLESCENTES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp36.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp36.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp36.id');
		$is_new		= true;
		...
		// Reorder the libreta de returno de adolescentes so the new libreta de returno de adolescentes is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp36BeforeSave', array('com_jtca.lcp36', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp36AfterSave', array('com_jtca.lcp36', $table, $is_new));

		$this->setState('lcp36.id', $table->id);
