...
/**
 * This models supports retrieving lists of libro de archivo.
 *
 */
class JtcaModelLspe06s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lspe06s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de archivo has been specified either as a parameter or in the url Request
		$pk = $params->get('lspe06_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lspe06_id');
		$this->setState('filter.lspe06_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lspe06_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lspe06_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lspe06_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lspe06_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lspe06_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lspe06_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lspe06_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lspe06_id'));
		$id .= ':'.$this->getState('filter.lspe06_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lspe06s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lspe06s').' AS a');
		...
		// Filter by a single or group of lspe06s.
		$lspe06_id = $this->getState('filter.lspe06_id');
		if ($lspe06_id != '')
		{
			if (is_numeric($lspe06_id))
			{
				$type = $this->getState('filter.lspe06_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lspe06_id);
			}
			else
			{
				if (is_array($lspe06_id))
				{
					JArrayHelper::toInteger($lspe06_id);
					$lspe06_id = implode(',', $lspe06_id);
					$type = $this->getState('filter.lspe06_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lspe06_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lspe06_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lspe06_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de archivo.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lspe06_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lspe06 params override menu item params only if menu param = 'use_lspe06'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lspe06' and there is no lspe06 param, use global

					// create an array of just the params set to 'use_lspe06'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lspe06')
						{
							// if the lspe06 has a value, use it
							if ($lspe06_params->get($key) != '')
							{
								// get the value from the lspe06
								$lspe06_array[$key] = $lspe06_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lspe06_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lspe06 params
					if (count($lspe06_array) > 0)
					{
						$lspe06_params = new Registry;
						$lspe06_params->loadArray($lspe06_array);
						$item->params->merge($lspe06_params);
					}


					// get display date
					switch ($item->params->get('list_show_lspe06_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lspe06, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
