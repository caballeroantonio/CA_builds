...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsps08.php';

class JtcaModelLsps08Form extends JtcaModelLsps08
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsps08';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsps08';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsps08', 'lsps08', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno de entregas de tocas.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO DE ENTREGAS DE TOCAS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsps08.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsps08.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsps08.id');
		$is_new		= true;
		...
		// Reorder the libro de turno de entregas de tocas so the new libro de turno de entregas de tocas is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsps08BeforeSave', array('com_jtca.lsps08', &$table, $is_new));
		...
		$dispatcher->trigger('onLsps08AfterSave', array('com_jtca.lsps08', $table, $is_new));

		$this->setState('lsps08.id', $table->id);
