...
class JtCaModelLsc09 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsc09';
	...
	{
		if (empty($config['lsc09_filter_fields']))
		{
			$config['lsc09_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	...
	/**
	 * Method to get LIBRO DE GOBIERNO (OFICIAL) data.
	 *
	 * @param	integer	$pk	The id of the libro de gobierno (oficial).
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsc09.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSC09S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsc09ChangeState', array('com_jtca.lsc09', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsc09BeforeDelete', array('com_jtca.lsc09', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsc09AfterDelete', array('com_jtca.lsc09', &$table));
				}
				else
				{
					// Prune items that you can't change.
