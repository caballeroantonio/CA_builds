...
class JtCaModelLjccm13s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljccm13_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljccm13_id');
		$this->setState('filter.ljccm13_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljccm13_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljccm13_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljccm13_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljccm13_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljccm13_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljccm13_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljccm13_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljccm13_id'));
		$id .= ':'.$this->getState('filter.ljccm13_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljccm13s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljccm13s').' AS a');
		...
		// Filter by a single or group of ljccm13s.
		$ljccm13_id = $this->getState('filter.ljccm13_id');
		if ($ljccm13_id != '')
		{
			if (is_numeric($ljccm13_id))
			{
				$type = $this->getState('filter.ljccm13_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljccm13_id);
			}
			else
			{
				if (is_array($ljccm13_id))
				{
					JArrayHelper::toInteger($ljccm13_id);
					$ljccm13_id = implode(',', $ljccm13_id);
					$type = $this->getState('filter.ljccm13_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljccm13_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljccm13_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljccm13_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljccm13_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljccm13 params override menu item params only if menu param = 'use_ljccm13'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljccm13' and there is no ljccm13 param, use global

					// create an array of just the params set to 'use_ljccm13'
					$menu_params_array = $this->getState('params')->toArray();
					$ljccm13_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljccm13')
						{
							// if the ljccm13 has a value, use it
							if ($ljccm13_params->get($key) != '')
							{
								// get the value from the ljccm13
								$ljccm13_array[$key] = $ljccm13_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljccm13_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljccm13 params
					if (count($ljccm13_array) > 0)
					{
						$ljccm13_params = new Registry;
						$ljccm13_params->loadArray($ljccm13_array);
						$item->params->merge($ljccm13_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljccm13_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljccm13, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
