...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp40.php';

class JtCaModelLcp40Form extends JtCaModelLcp40
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp40';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp40';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp40', 'lcp40', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de ordenes de detención para juzgados de justicia para adolescentes.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE ORDENES DE DETENCIÓN PARA JUZGADOS DE JUSTICIA PARA ADOLESCENTES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp40.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp40.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp40.id');
		$is_new		= true;
		...
		// Reorder the libro de ordenes de detención para juzgados de jus so the new libro de ordenes de detención para juzgados de justicia para adolescentes is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp40BeforeSave', array('com_jtca.lcp40', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp40AfterSave', array('com_jtca.lcp40', $table, $is_new));

		$this->setState('lcp40.id', $table->id);
