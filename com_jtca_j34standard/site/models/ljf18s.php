...
class JtCaModelLjf18s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf18_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf18_id');
		$this->setState('filter.ljf18_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf18_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf18_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf18_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf18_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf18_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf18_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf18_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf18_id'));
		$id .= ':'.$this->getState('filter.ljf18_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf18s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf18s').' AS a');
		...
		// Filter by a single or group of ljf18s.
		$ljf18_id = $this->getState('filter.ljf18_id');
		if ($ljf18_id != '')
		{
			if (is_numeric($ljf18_id))
			{
				$type = $this->getState('filter.ljf18_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf18_id);
			}
			else
			{
				if (is_array($ljf18_id))
				{
					JArrayHelper::toInteger($ljf18_id);
					$ljf18_id = implode(',', $ljf18_id);
					$type = $this->getState('filter.ljf18_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf18_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf18_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf18_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf18_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf18 params override menu item params only if menu param = 'use_ljf18'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf18' and there is no ljf18 param, use global

					// create an array of just the params set to 'use_ljf18'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf18')
						{
							// if the ljf18 has a value, use it
							if ($ljf18_params->get($key) != '')
							{
								// get the value from the ljf18
								$ljf18_array[$key] = $ljf18_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf18_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf18 params
					if (count($ljf18_array) > 0)
					{
						$ljf18_params = new Registry;
						$ljf18_params->loadArray($ljf18_array);
						$item->params->merge($ljf18_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf18_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf18, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
