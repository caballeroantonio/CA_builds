...
class JtcaModelLjf11s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf11_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf11_id');
		$this->setState('filter.ljf11_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf11_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf11_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf11_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf11_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf11_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf11_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf11_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf11_id'));
		$id .= ':'.$this->getState('filter.ljf11_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf11s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf11s').' AS a');
		...
		// Filter by a single or group of ljf11s.
		$ljf11_id = $this->getState('filter.ljf11_id');
		if ($ljf11_id != '')
		{
			if (is_numeric($ljf11_id))
			{
				$type = $this->getState('filter.ljf11_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf11_id);
			}
			else
			{
				if (is_array($ljf11_id))
				{
					JArrayHelper::toInteger($ljf11_id);
					$ljf11_id = implode(',', $ljf11_id);
					$type = $this->getState('filter.ljf11_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf11_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf11_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf11_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf11_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf11 params override menu item params only if menu param = 'use_ljf11'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf11' and there is no ljf11 param, use global

					// create an array of just the params set to 'use_ljf11'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf11')
						{
							// if the ljf11 has a value, use it
							if ($ljf11_params->get($key) != '')
							{
								// get the value from the ljf11
								$ljf11_array[$key] = $ljf11_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf11_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf11 params
					if (count($ljf11_array) > 0)
					{
						$ljf11_params = new Registry;
						$ljf11_params->loadArray($ljf11_array);
						$item->params->merge($ljf11_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf11_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf11, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
