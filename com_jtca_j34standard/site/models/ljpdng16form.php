...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpdng16.php';

class JtcaModelLjpdng16Form extends JtcaModelLjpdng16
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpdng16';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpdng16';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpdng16', 'ljpdng16', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de registro en que se haya obsequiado o negado la orden de aprehensión, comparecencia, presentación, artículo 36 del código de procedimientos penales y otros.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE REGISTRO EN QUE SE HAYA OBSEQUIADO O NEGADO LA ORDEN DE APREHENSIÓN, COMPARECENCIA, PRESENTACIÓN, ARTÍCULO 36 DEL CÓDIGO DE PROCEDIMIENTOS PENALES Y OTROS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpdng16.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpdng16.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpdng16.id');
		$is_new		= true;
		...
		// Reorder the libro de registro en que se haya obsequiado o nega so the new libro de registro en que se haya obsequiado o negado la orden de aprehensión, comparecencia, presentación, artículo 36 del código de procedimientos penales y otros is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpdng16BeforeSave', array('com_jtca.ljpdng16', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpdng16AfterSave', array('com_jtca.ljpdng16', $table, $is_new));

		$this->setState('ljpdng16.id', $table->id);
