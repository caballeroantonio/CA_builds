 ...
 *
 */
class JtcaModelLjpdng08 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpdng08';
	...
	{
		if (empty($config['ljpdng08_filter_fields']))
		{
			$config['ljpdng08_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljpdng08s', $prefix = 'JtcaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpdng08.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPDNG08S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpdng08ChangeState', array('com_jtca.ljpdng08', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpdng08BeforeDelete', array('com_jtca.ljpdng08', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpdng08AfterDelete', array('com_jtca.ljpdng08', &$table));
				}
				else
				{
					// Prune items that you can't change.
