...
class JtCaModelLjccm15s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljccm15_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljccm15_id');
		$this->setState('filter.ljccm15_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljccm15_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljccm15_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljccm15_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljccm15_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljccm15_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljccm15_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljccm15_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljccm15_id'));
		$id .= ':'.$this->getState('filter.ljccm15_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljccm15s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljccm15s').' AS a');
		...
		// Filter by a single or group of ljccm15s.
		$ljccm15_id = $this->getState('filter.ljccm15_id');
		if ($ljccm15_id != '')
		{
			if (is_numeric($ljccm15_id))
			{
				$type = $this->getState('filter.ljccm15_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljccm15_id);
			}
			else
			{
				if (is_array($ljccm15_id))
				{
					JArrayHelper::toInteger($ljccm15_id);
					$ljccm15_id = implode(',', $ljccm15_id);
					$type = $this->getState('filter.ljccm15_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljccm15_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljccm15_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljccm15_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljccm15_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljccm15 params override menu item params only if menu param = 'use_ljccm15'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljccm15' and there is no ljccm15 param, use global

					// create an array of just the params set to 'use_ljccm15'
					$menu_params_array = $this->getState('params')->toArray();
					$ljccm15_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljccm15')
						{
							// if the ljccm15 has a value, use it
							if ($ljccm15_params->get($key) != '')
							{
								// get the value from the ljccm15
								$ljccm15_array[$key] = $ljccm15_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljccm15_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljccm15 params
					if (count($ljccm15_array) > 0)
					{
						$ljccm15_params = new Registry;
						$ljccm15_params->loadArray($ljccm15_array);
						$item->params->merge($ljccm15_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljccm15_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljccm15, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
