...
class JtcaModelLjf09s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf09_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf09_id');
		$this->setState('filter.ljf09_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf09_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf09_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf09_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf09_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf09_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf09_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf09_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf09_id'));
		$id .= ':'.$this->getState('filter.ljf09_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf09s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf09s').' AS a');
		...
		// Filter by a single or group of ljf09s.
		$ljf09_id = $this->getState('filter.ljf09_id');
		if ($ljf09_id != '')
		{
			if (is_numeric($ljf09_id))
			{
				$type = $this->getState('filter.ljf09_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf09_id);
			}
			else
			{
				if (is_array($ljf09_id))
				{
					JArrayHelper::toInteger($ljf09_id);
					$ljf09_id = implode(',', $ljf09_id);
					$type = $this->getState('filter.ljf09_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf09_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf09_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf09_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf09_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf09 params override menu item params only if menu param = 'use_ljf09'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf09' and there is no ljf09 param, use global

					// create an array of just the params set to 'use_ljf09'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf09')
						{
							// if the ljf09 has a value, use it
							if ($ljf09_params->get($key) != '')
							{
								// get the value from the ljf09
								$ljf09_array[$key] = $ljf09_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf09_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf09 params
					if (count($ljf09_array) > 0)
					{
						$ljf09_params = new Registry;
						$ljf09_params->loadArray($ljf09_array);
						$item->params->merge($ljf09_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf09_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf09, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
