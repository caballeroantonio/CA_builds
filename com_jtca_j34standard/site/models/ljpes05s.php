...
class JtCaModelLjpes05s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpes05_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes05_id');
		$this->setState('filter.ljpes05_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes05_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes05_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes05_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes05_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes05_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes05_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes05_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes05_id'));
		$id .= ':'.$this->getState('filter.ljpes05_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes05s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes05s').' AS a');
		...
		// Filter by a single or group of ljpes05s.
		$ljpes05_id = $this->getState('filter.ljpes05_id');
		if ($ljpes05_id != '')
		{
			if (is_numeric($ljpes05_id))
			{
				$type = $this->getState('filter.ljpes05_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes05_id);
			}
			else
			{
				if (is_array($ljpes05_id))
				{
					JArrayHelper::toInteger($ljpes05_id);
					$ljpes05_id = implode(',', $ljpes05_id);
					$type = $this->getState('filter.ljpes05_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes05_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes05_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes05_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes05_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes05 params override menu item params only if menu param = 'use_ljpes05'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes05' and there is no ljpes05 param, use global

					// create an array of just the params set to 'use_ljpes05'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes05_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes05')
						{
							// if the ljpes05 has a value, use it
							if ($ljpes05_params->get($key) != '')
							{
								// get the value from the ljpes05
								$ljpes05_array[$key] = $ljpes05_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes05_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes05 params
					if (count($ljpes05_array) > 0)
					{
						$ljpes05_params = new Registry;
						$ljpes05_params->loadArray($ljpes05_array);
						$item->params->merge($ljpes05_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes05_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes05, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
