...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsc06.php';

class JtCaModelLsc06Form extends JtCaModelLsc06
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsc06';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsc06';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsc06', 'lsc06', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de control de billetes y pólizas (oficial).
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE CONTROL DE BILLETES Y PÓLIZAS (OFICIAL) item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsc06.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsc06.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsc06.id');
		$is_new		= true;
		...
		// Reorder the libro de control de billetes y pólizas (oficial) so the new libro de control de billetes y pólizas (oficial) is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsc06BeforeSave', array('com_jtca.lsc06', &$table, $is_new));
		...
		$dispatcher->trigger('onLsc06AfterSave', array('com_jtca.lsc06', $table, $is_new));

		$this->setState('lsc06.id', $table->id);
