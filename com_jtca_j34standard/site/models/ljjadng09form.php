...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadng09.php';

class JtCaModelLjjadng09Form extends JtCaModelLjjadng09
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadng09';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadng09';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadng09', 'ljjadng09', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de actuario.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE ACTUARIO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadng09.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadng09.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadng09.id');
		$is_new		= true;
		...
		// Reorder the libro de actuario so the new libro de actuario is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadng09BeforeSave', array('com_jtca.ljjadng09', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadng09AfterSave', array('com_jtca.ljjadng09', $table, $is_new));

		$this->setState('ljjadng09.id', $table->id);
