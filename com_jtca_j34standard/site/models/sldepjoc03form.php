...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/sldepjoc03.php';

class JtcaModelSldepJoc03Form extends JtcaModelSldepJoc03
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.sldepjoc03';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.sldepjoc03';
	/**
	...
	protected function populateState()
	{
		...
		$this->setState('sldepjoc03.id', $pk);
	...
	*/
	public function getTable($type = 'SldepJoc03s', $prefix = 'JtcaTable', $config = array())
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.sldepjoc03', 'sldepjoc03', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	/**
	 * Method to get sub libro certificado de egresos del libro de certificado de depósitos de ingresos y egresos data.
	 *
	 * @param	integer	$item_id	The id of the sub libro certificado de egresos del libro de certificado de depósitos de ingresos y egresos.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Sub Libro CERTIFICADO DE EGRESOS Del LIBRO DE CERTIFICADO DE DEPÓSITOS DE INGRESOS Y EGRESOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('sldepjoc03.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.sldepjoc03.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('sldepjoc03.id');
		$is_new		= true;
		...
		// Reorder the certificado de egresos del libro de certificado de so the new sub libro certificado de egresos del libro de certificado de depósitos de ingresos y egresos is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSldepJoc03BeforeSave', array('com_jtca.sldepjoc03', &$table, $is_new));
		...
		$dispatcher->trigger('onSldepJoc03AfterSave', array('com_jtca.sldepjoc03', $table, $is_new));

		$this->setState('sldepjoc03.id', $table->id);
