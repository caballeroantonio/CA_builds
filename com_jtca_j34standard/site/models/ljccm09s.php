...
class JtCaModelLjccm09s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljccm09_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljccm09_id');
		$this->setState('filter.ljccm09_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljccm09_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljccm09_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljccm09_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljccm09_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljccm09_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljccm09_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljccm09_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljccm09_id'));
		$id .= ':'.$this->getState('filter.ljccm09_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljccm09s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljccm09s').' AS a');
		...
		// Filter by a single or group of ljccm09s.
		$ljccm09_id = $this->getState('filter.ljccm09_id');
		if ($ljccm09_id != '')
		{
			if (is_numeric($ljccm09_id))
			{
				$type = $this->getState('filter.ljccm09_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljccm09_id);
			}
			else
			{
				if (is_array($ljccm09_id))
				{
					JArrayHelper::toInteger($ljccm09_id);
					$ljccm09_id = implode(',', $ljccm09_id);
					$type = $this->getState('filter.ljccm09_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljccm09_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljccm09_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljccm09_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljccm09_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljccm09 params override menu item params only if menu param = 'use_ljccm09'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljccm09' and there is no ljccm09 param, use global

					// create an array of just the params set to 'use_ljccm09'
					$menu_params_array = $this->getState('params')->toArray();
					$ljccm09_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljccm09')
						{
							// if the ljccm09 has a value, use it
							if ($ljccm09_params->get($key) != '')
							{
								// get the value from the ljccm09
								$ljccm09_array[$key] = $ljccm09_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljccm09_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljccm09 params
					if (count($ljccm09_array) > 0)
					{
						$ljccm09_params = new Registry;
						$ljccm09_params->loadArray($ljccm09_array);
						$item->params->merge($ljccm09_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljccm09_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljccm09, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
