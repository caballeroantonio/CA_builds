...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp12.php';

class JtCaModelLcp12Form extends JtCaModelLcp12
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp12';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp12';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp12', 'lcp12', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de turno de salas para juzgados orales..
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE TURNO DE SALAS PARA JUZGADOS ORALES. item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp12.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp12.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp12.id');
		$is_new		= true;
		...
		// Reorder the libreta de turno de salas para juzgados orales. so the new libreta de turno de salas para juzgados orales. is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp12BeforeSave', array('com_jtca.lcp12', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp12AfterSave', array('com_jtca.lcp12', $table, $is_new));

		$this->setState('lcp12.id', $table->id);
