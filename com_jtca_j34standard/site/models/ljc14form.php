...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljc14.php';

class JtCaModelLjc14Form extends JtCaModelLjc14
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljc14';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljc14';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljc14', 'ljc14', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljc14.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljc14.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljc14.id');
		$is_new		= true;
		...
		// Reorder the jc14 so the new libro de control de multas is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjc14BeforeSave', array('com_jtca.ljc14', &$table, $is_new));
		...
		$dispatcher->trigger('onLjc14AfterSave', array('com_jtca.ljc14', $table, $is_new));

		$this->setState('ljc14.id', $table->id);
