 ...
 *
 */
class JtCaModelLjoc08 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljoc08';
	...
	{
		if (empty($config['ljoc08_filter_fields']))
		{
			$config['ljoc08_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljoc08.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJOC08S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjoc08ChangeState', array('com_jtca.ljoc08', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjoc08BeforeDelete', array('com_jtca.ljoc08', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjoc08AfterDelete', array('com_jtca.ljoc08', &$table));
				}
				else
				{
					// Prune items that you can't change.
