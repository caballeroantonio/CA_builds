...
class JtCaModelLjpdng08s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpdng08_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng08_id');
		$this->setState('filter.ljpdng08_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng08_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng08_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng08_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng08_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng08_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng08_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng08_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng08_id'));
		$id .= ':'.$this->getState('filter.ljpdng08_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng08s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng08s').' AS a');
		...
		// Filter by a single or group of ljpdng08s.
		$ljpdng08_id = $this->getState('filter.ljpdng08_id');
		if ($ljpdng08_id != '')
		{
			if (is_numeric($ljpdng08_id))
			{
				$type = $this->getState('filter.ljpdng08_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng08_id);
			}
			else
			{
				if (is_array($ljpdng08_id))
				{
					JArrayHelper::toInteger($ljpdng08_id);
					$ljpdng08_id = implode(',', $ljpdng08_id);
					$type = $this->getState('filter.ljpdng08_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng08_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng08_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng08_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng08_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng08 params override menu item params only if menu param = 'use_ljpdng08'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng08' and there is no ljpdng08 param, use global

					// create an array of just the params set to 'use_ljpdng08'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng08_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng08')
						{
							// if the ljpdng08 has a value, use it
							if ($ljpdng08_params->get($key) != '')
							{
								// get the value from the ljpdng08
								$ljpdng08_array[$key] = $ljpdng08_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng08_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng08 params
					if (count($ljpdng08_array) > 0)
					{
						$ljpdng08_params = new Registry;
						$ljpdng08_params->loadArray($ljpdng08_array);
						$item->params->merge($ljpdng08_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng08_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng08, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
