...
/**
 * This models supports retrieving lists of libreta de libertades juzgados de delitos no grave.
 *
 */
class JtCaModelLcp31s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp31s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de libertades juzgados de delitos no graves has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp31_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp31_id');
		$this->setState('filter.lcp31_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp31_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp31_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp31_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp31_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp31_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp31_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp31_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp31_id'));
		$id .= ':'.$this->getState('filter.lcp31_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp31s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp31s').' AS a');
		...
		// Filter by a single or group of lcp31s.
		$lcp31_id = $this->getState('filter.lcp31_id');
		if ($lcp31_id != '')
		{
			if (is_numeric($lcp31_id))
			{
				$type = $this->getState('filter.lcp31_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp31_id);
			}
			else
			{
				if (is_array($lcp31_id))
				{
					JArrayHelper::toInteger($lcp31_id);
					$lcp31_id = implode(',', $lcp31_id);
					$type = $this->getState('filter.lcp31_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp31_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp31_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp31_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de libertades juzgados de delitos no grave.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp31_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp31 params override menu item params only if menu param = 'use_lcp31'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp31' and there is no lcp31 param, use global

					// create an array of just the params set to 'use_lcp31'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp31')
						{
							// if the lcp31 has a value, use it
							if ($lcp31_params->get($key) != '')
							{
								// get the value from the lcp31
								$lcp31_array[$key] = $lcp31_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp31_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp31 params
					if (count($lcp31_array) > 0)
					{
						$lcp31_params = new Registry;
						$lcp31_params->loadArray($lcp31_array);
						$item->params->merge($lcp31_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp31_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp31, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
