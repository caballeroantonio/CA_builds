...
class JtCaModelLjjadg17s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadg17_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg17_id');
		$this->setState('filter.ljjadg17_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg17_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg17_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg17_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg17_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg17_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg17_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg17_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg17_id'));
		$id .= ':'.$this->getState('filter.ljjadg17_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg17s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg17s').' AS a');
		...
		// Filter by a single or group of ljjadg17s.
		$ljjadg17_id = $this->getState('filter.ljjadg17_id');
		if ($ljjadg17_id != '')
		{
			if (is_numeric($ljjadg17_id))
			{
				$type = $this->getState('filter.ljjadg17_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg17_id);
			}
			else
			{
				if (is_array($ljjadg17_id))
				{
					JArrayHelper::toInteger($ljjadg17_id);
					$ljjadg17_id = implode(',', $ljjadg17_id);
					$type = $this->getState('filter.ljjadg17_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg17_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg17_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg17_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg17_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg17 params override menu item params only if menu param = 'use_ljjadg17'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg17' and there is no ljjadg17 param, use global

					// create an array of just the params set to 'use_ljjadg17'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg17_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg17')
						{
							// if the ljjadg17 has a value, use it
							if ($ljjadg17_params->get($key) != '')
							{
								// get the value from the ljjadg17
								$ljjadg17_array[$key] = $ljjadg17_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg17_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg17 params
					if (count($ljjadg17_array) > 0)
					{
						$ljjadg17_params = new Registry;
						$ljjadg17_params->loadArray($ljjadg17_array);
						$item->params->merge($ljjadg17_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg17_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg17, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
