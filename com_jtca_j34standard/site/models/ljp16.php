 ...
 *
 */
class JtCaModelLjp16 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljp16';
	...
	{
		if (empty($config['ljp16_filter_fields']))
		{
			$config['ljp16_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljp16s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljp16.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJP16S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjp16ChangeState', array('com_jtca.ljp16', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjp16BeforeDelete', array('com_jtca.ljp16', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjp16AfterDelete', array('com_jtca.ljp16', &$table));
				}
				else
				{
					// Prune items that you can't change.
