...
/**
 * This models supports retrieving lists of libreta de libertades juzgados de primera instanci.
 *
 */
class JtCaModelLcp30s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp30s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de libertades juzgados de primera instancia has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp30_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp30_id');
		$this->setState('filter.lcp30_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp30_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp30_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp30_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp30_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp30_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp30_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp30_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp30_id'));
		$id .= ':'.$this->getState('filter.lcp30_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp30s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp30s').' AS a');
		...
		// Filter by a single or group of lcp30s.
		$lcp30_id = $this->getState('filter.lcp30_id');
		if ($lcp30_id != '')
		{
			if (is_numeric($lcp30_id))
			{
				$type = $this->getState('filter.lcp30_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp30_id);
			}
			else
			{
				if (is_array($lcp30_id))
				{
					JArrayHelper::toInteger($lcp30_id);
					$lcp30_id = implode(',', $lcp30_id);
					$type = $this->getState('filter.lcp30_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp30_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp30_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp30_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de libertades juzgados de primera instanci.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp30_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp30 params override menu item params only if menu param = 'use_lcp30'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp30' and there is no lcp30 param, use global

					// create an array of just the params set to 'use_lcp30'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp30')
						{
							// if the lcp30 has a value, use it
							if ($lcp30_params->get($key) != '')
							{
								// get the value from the lcp30
								$lcp30_array[$key] = $lcp30_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp30_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp30 params
					if (count($lcp30_array) > 0)
					{
						$lcp30_params = new Registry;
						$lcp30_params->loadArray($lcp30_array);
						$item->params->merge($lcp30_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp30_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp30, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
