...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljccm05.php';

class JtCaModelLjccm05Form extends JtCaModelLjccm05
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljccm05';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljccm05';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljccm05', 'ljccm05', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljccm05.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljccm05.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljccm05.id');
		$is_new		= true;
		...
		// Reorder the libro de turno para sentencia so the new libro de turno para sentencia is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjccm05BeforeSave', array('com_jtca.ljccm05', &$table, $is_new));
		...
		$dispatcher->trigger('onLjccm05AfterSave', array('com_jtca.ljccm05', $table, $is_new));

		$this->setState('ljccm05.id', $table->id);
