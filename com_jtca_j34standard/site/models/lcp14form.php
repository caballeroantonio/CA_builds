...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp14.php';

class JtCaModelLcp14Form extends JtCaModelLcp14
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp14';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp14';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp14', 'lcp14', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de cumplimiento de ejecutorias para juzgados de ejecución..
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE CUMPLIMIENTO DE EJECUTORIAS PARA JUZGADOS DE EJECUCIÓN. item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp14.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp14.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp14.id');
		$is_new		= true;
		...
		// Reorder the libreta de cumplimiento de ejecutorias para juzgad so the new libreta de cumplimiento de ejecutorias para juzgados de ejecución. is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp14BeforeSave', array('com_jtca.lcp14', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp14AfterSave', array('com_jtca.lcp14', $table, $is_new));

		$this->setState('lcp14.id', $table->id);
