...
/**
 * This models supports retrieving lists of expedientes.
 *
 */
class JtcaModelExpedientes extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.expedientes';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single expedientes has been specified either as a parameter or in the url Request
		$pk = $params->get('expediente_id', '') == '' ? $app->input->getInt('id', '') : $params->get('expediente_id');
		$this->setState('filter.expediente_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('expediente_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('expediente_initial_sort','a.name'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('expediente_initial_sort','a.name');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('expediente_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('expediente_initial_direction','ASC');
		}
		...
		if ($params->get('filter_expediente_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_expediente_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.expediente_id'));
		$id .= ':'.$this->getState('filter.expediente_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of expedientes subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_expedientes').' AS a');
		...
		// Filter by and return name for id_exp_orig level.
		$query->select($db->quoteName('e.name').' AS e_expediente_name');
		$query->select($db->quoteName('e.id').' AS e_expediente_id');

		$query->join('LEFT', $db->quoteName('jt_expedientes').' AS e ON '.$db->quoteName('e.id').' = '.$db->quoteName('a.id_exp_orig'));	
					

		// Filter by a single or group of expedientes.
		$expediente_id = $this->getState('filter.expediente_id');
		if ($expediente_id != '')
		{
			if (is_numeric($expediente_id))
			{
				$type = $this->getState('filter.expediente_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $expediente_id);
			}
			else
			{
				if (is_array($expediente_id))
				{
					JArrayHelper::toInteger($expediente_id);
					$expediente_id = implode(',', $expediente_id);
					$type = $this->getState('filter.expediente_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$expediente_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_expediente_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_expediente_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of expedientes.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$expediente_params = new Registry;
				...
				{
					$item->params = $expediente_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Expediente params override menu item params only if menu param = 'use_expediente'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_expediente' and there is no expediente param, use global

					// create an array of just the params set to 'use_expediente'
					$menu_params_array = $this->getState('params')->toArray();
					$expediente_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_expediente')
						{
							// if the expediente has a value, use it
							if ($expediente_params->get($key) != '')
							{
								// get the value from the expediente
								$expediente_array[$key] = $expediente_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$expediente_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected expediente params
					if (count($expediente_array) > 0)
					{
						$expediente_params = new Registry;
						$expediente_params->loadArray($expediente_array);
						$item->params->merge($expediente_params);
					}


					// get display date
					switch ($item->params->get('list_show_expediente_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an expediente, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
