...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lejemplo.php';

class JtCaModelLejemploForm extends JtCaModelLejemplo
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lejemplo';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lejemplo';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lejemplo', 'lejemplo', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de ejemplo.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Libro De Ejemplo item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lejemplo.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lejemplo.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lejemplo.id');
		$is_new		= true;
		...
		$dispatcher->trigger('onLejemploAfterSave', array('com_jtca.lejemplo', $table, $is_new));

		$this->setState('lejemplo.id', $table->id);
