...
/**
 * This models supports retrieving lists of libro de beneficios revocados y ordenes de re-apre.
 *
 */
class JtCaModelLjpes15s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpes15s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de beneficios revocados y ordenes de re-aprehensiÃ³n has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpes15_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes15_id');
		$this->setState('filter.ljpes15_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes15_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes15_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes15_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes15_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes15_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes15_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes15_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes15_id'));
		$id .= ':'.$this->getState('filter.ljpes15_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes15s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes15s').' AS a');
		...
		// Filter by a single or group of ljpes15s.
		$ljpes15_id = $this->getState('filter.ljpes15_id');
		if ($ljpes15_id != '')
		{
			if (is_numeric($ljpes15_id))
			{
				$type = $this->getState('filter.ljpes15_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes15_id);
			}
			else
			{
				if (is_array($ljpes15_id))
				{
					JArrayHelper::toInteger($ljpes15_id);
					$ljpes15_id = implode(',', $ljpes15_id);
					$type = $this->getState('filter.ljpes15_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes15_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes15_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes15_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de beneficios revocados y ordenes de re-apre.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes15_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes15 params override menu item params only if menu param = 'use_ljpes15'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes15' and there is no ljpes15 param, use global

					// create an array of just the params set to 'use_ljpes15'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes15_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes15')
						{
							// if the ljpes15 has a value, use it
							if ($ljpes15_params->get($key) != '')
							{
								// get the value from the ljpes15
								$ljpes15_array[$key] = $ljpes15_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes15_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes15 params
					if (count($ljpes15_array) > 0)
					{
						$ljpes15_params = new Registry;
						$ljpes15_params->loadArray($ljpes15_array);
						$item->params->merge($ljpes15_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes15_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes15, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
