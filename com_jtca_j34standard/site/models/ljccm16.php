...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE REMISIÓN DE EXPEDIENTES AL ARCHIVO Model
 *
 */
class JtCaModelLjccm16 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljccm16';
	...
	{
		if (empty($config['ljccm16_filter_fields']))
		{
			$config['ljccm16_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljccm16s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE REMISIÓN DE EXPEDIENTES AL ARCHIVO data.
	 *
	 * @param	integer	$pk	The id of the libro de remisión de expedientes al archivo.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljccm16.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJCCM16S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjccm16ChangeState', array('com_jtca.ljccm16', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjccm16BeforeDelete', array('com_jtca.ljccm16', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjccm16AfterDelete', array('com_jtca.ljccm16', &$table));
				}
				else
				{
					// Prune items that you can't change.
