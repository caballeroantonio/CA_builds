...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljomf02.php';

class JtCaModelLjomf02Form extends JtCaModelLjomf02
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljomf02';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljomf02';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljomf02', 'ljomf02', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de ingresos de valores.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE INGRESOS DE VALORES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljomf02.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljomf02.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljomf02.id');
		$is_new		= true;
		...
		// Reorder the libro de ingresos de valores so the new libro de ingresos de valores is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjomf02BeforeSave', array('com_jtca.ljomf02', &$table, $is_new));
		...
		$dispatcher->trigger('onLjomf02AfterSave', array('com_jtca.ljomf02', $table, $is_new));

		$this->setState('ljomf02.id', $table->id);
