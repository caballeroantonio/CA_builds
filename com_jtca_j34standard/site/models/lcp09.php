...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE ETAPA DE EJECUCIÓN PARA ADOLESCENTES Model
 *
 */
class JtCaModelLcp09 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp09';
	...
	{
		if (empty($config['lcp09_filter_fields']))
		{
			$config['lcp09_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp09s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE ETAPA DE EJECUCIÓN PARA ADOLESCENTES data.
	 *
	 * @param	integer	$pk	The id of the libro de etapa de ejecución para adolescentes.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp09.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP09S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp09ChangeState', array('com_jtca.lcp09', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp09BeforeDelete', array('com_jtca.lcp09', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp09AfterDelete', array('com_jtca.lcp09', &$table));
				}
				else
				{
					// Prune items that you can't change.
