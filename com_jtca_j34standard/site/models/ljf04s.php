...
class JtCaModelLjf04s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf04_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf04_id');
		$this->setState('filter.ljf04_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf04_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf04_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf04_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf04_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf04_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf04_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf04_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf04_id'));
		$id .= ':'.$this->getState('filter.ljf04_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf04s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf04s').' AS a');
		...
		// Filter by a single or group of ljf04s.
		$ljf04_id = $this->getState('filter.ljf04_id');
		if ($ljf04_id != '')
		{
			if (is_numeric($ljf04_id))
			{
				$type = $this->getState('filter.ljf04_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf04_id);
			}
			else
			{
				if (is_array($ljf04_id))
				{
					JArrayHelper::toInteger($ljf04_id);
					$ljf04_id = implode(',', $ljf04_id);
					$type = $this->getState('filter.ljf04_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf04_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf04_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf04_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf04_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf04 params override menu item params only if menu param = 'use_ljf04'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf04' and there is no ljf04 param, use global

					// create an array of just the params set to 'use_ljf04'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf04')
						{
							// if the ljf04 has a value, use it
							if ($ljf04_params->get($key) != '')
							{
								// get the value from the ljf04
								$ljf04_array[$key] = $ljf04_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf04_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf04 params
					if (count($ljf04_array) > 0)
					{
						$ljf04_params = new Registry;
						$ljf04_params->loadArray($ljf04_array);
						$item->params->merge($ljf04_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf04_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf04, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
