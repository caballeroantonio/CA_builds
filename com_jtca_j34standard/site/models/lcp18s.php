...
/**
 * This models supports retrieving lists of libro de turno para consignaciones con detenido pr.
 *
 */
class JtCaModelLcp18s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp18s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno para consignaciones con detenido primera instancia. has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp18_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp18_id');
		$this->setState('filter.lcp18_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp18_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp18_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp18_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp18_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp18_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp18_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp18_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp18_id'));
		$id .= ':'.$this->getState('filter.lcp18_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp18s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp18s').' AS a');
		...
		// Filter by a single or group of lcp18s.
		$lcp18_id = $this->getState('filter.lcp18_id');
		if ($lcp18_id != '')
		{
			if (is_numeric($lcp18_id))
			{
				$type = $this->getState('filter.lcp18_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp18_id);
			}
			else
			{
				if (is_array($lcp18_id))
				{
					JArrayHelper::toInteger($lcp18_id);
					$lcp18_id = implode(',', $lcp18_id);
					$type = $this->getState('filter.lcp18_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp18_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp18_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp18_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno para consignaciones con detenido pr.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp18_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp18 params override menu item params only if menu param = 'use_lcp18'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp18' and there is no lcp18 param, use global

					// create an array of just the params set to 'use_lcp18'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp18')
						{
							// if the lcp18 has a value, use it
							if ($lcp18_params->get($key) != '')
							{
								// get the value from the lcp18
								$lcp18_array[$key] = $lcp18_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp18_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp18 params
					if (count($lcp18_array) > 0)
					{
						$lcp18_params = new Registry;
						$lcp18_params->loadArray($lcp18_array);
						$item->params->merge($lcp18_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp18_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp18, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
