...
class JtcaModelLjpdng01s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpdng01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng01_id');
		$this->setState('filter.ljpdng01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng01_id'));
		$id .= ':'.$this->getState('filter.ljpdng01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng01s').' AS a');
		...
		// Filter by a single or group of ljpdng01s.
		$ljpdng01_id = $this->getState('filter.ljpdng01_id');
		if ($ljpdng01_id != '')
		{
			if (is_numeric($ljpdng01_id))
			{
				$type = $this->getState('filter.ljpdng01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng01_id);
			}
			else
			{
				if (is_array($ljpdng01_id))
				{
					JArrayHelper::toInteger($ljpdng01_id);
					$ljpdng01_id = implode(',', $ljpdng01_id);
					$type = $this->getState('filter.ljpdng01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng01_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng01 params override menu item params only if menu param = 'use_ljpdng01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng01' and there is no ljpdng01 param, use global

					// create an array of just the params set to 'use_ljpdng01'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng01_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng01')
						{
							// if the ljpdng01 has a value, use it
							if ($ljpdng01_params->get($key) != '')
							{
								// get the value from the ljpdng01
								$ljpdng01_array[$key] = $ljpdng01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng01 params
					if (count($ljpdng01_array) > 0)
					{
						$ljpdng01_params = new Registry;
						$ljpdng01_params->loadArray($ljpdng01_array);
						$item->params->merge($ljpdng01_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
