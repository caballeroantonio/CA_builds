...
class JtCaModelLjomf03s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf03_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf03_id');
		$this->setState('filter.ljomf03_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf03_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf03_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf03_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf03_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf03_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf03_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf03_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf03_id'));
		$id .= ':'.$this->getState('filter.ljomf03_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf03s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf03s').' AS a');
		...
		// Filter by a single or group of ljomf03s.
		$ljomf03_id = $this->getState('filter.ljomf03_id');
		if ($ljomf03_id != '')
		{
			if (is_numeric($ljomf03_id))
			{
				$type = $this->getState('filter.ljomf03_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf03_id);
			}
			else
			{
				if (is_array($ljomf03_id))
				{
					JArrayHelper::toInteger($ljomf03_id);
					$ljomf03_id = implode(',', $ljomf03_id);
					$type = $this->getState('filter.ljomf03_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf03_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf03_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf03_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf03_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf03 params override menu item params only if menu param = 'use_ljomf03'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf03' and there is no ljomf03 param, use global

					// create an array of just the params set to 'use_ljomf03'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf03_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf03')
						{
							// if the ljomf03 has a value, use it
							if ($ljomf03_params->get($key) != '')
							{
								// get the value from the ljomf03
								$ljomf03_array[$key] = $ljomf03_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf03_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf03 params
					if (count($ljomf03_array) > 0)
					{
						$ljomf03_params = new Registry;
						$ljomf03_params->loadArray($ljomf03_array);
						$item->params->merge($ljomf03_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf03_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf03, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
