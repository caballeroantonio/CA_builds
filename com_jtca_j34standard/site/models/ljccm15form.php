...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljccm15.php';

class JtCaModelLjccm15Form extends JtCaModelLjccm15
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljccm15';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljccm15';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljccm15', 'ljccm15', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljccm15.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljccm15.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljccm15.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjccm15BeforeSave', array('com_jtca.ljccm15', &$table, $is_new));
		...
		$dispatcher->trigger('onLjccm15AfterSave', array('com_jtca.ljccm15', $table, $is_new));

		$this->setState('ljccm15.id', $table->id);
