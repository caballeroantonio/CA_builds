...
class JtCaModelLcp02s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno de justicia para adolescentes sin detenido has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp02_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp02_id');
		$this->setState('filter.lcp02_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp02_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp02_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp02_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp02_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp02_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp02_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp02_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp02_id'));
		$id .= ':'.$this->getState('filter.lcp02_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp02s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp02s').' AS a');
		...
		// Filter by a single or group of lcp02s.
		$lcp02_id = $this->getState('filter.lcp02_id');
		if ($lcp02_id != '')
		{
			if (is_numeric($lcp02_id))
			{
				$type = $this->getState('filter.lcp02_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp02_id);
			}
			else
			{
				if (is_array($lcp02_id))
				{
					JArrayHelper::toInteger($lcp02_id);
					$lcp02_id = implode(',', $lcp02_id);
					$type = $this->getState('filter.lcp02_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp02_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp02_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp02_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp02_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp02 params override menu item params only if menu param = 'use_lcp02'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp02' and there is no lcp02 param, use global

					// create an array of just the params set to 'use_lcp02'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp02')
						{
							// if the lcp02 has a value, use it
							if ($lcp02_params->get($key) != '')
							{
								// get the value from the lcp02
								$lcp02_array[$key] = $lcp02_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp02_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp02 params
					if (count($lcp02_array) > 0)
					{
						$lcp02_params = new Registry;
						$lcp02_params->loadArray($lcp02_array);
						$item->params->merge($lcp02_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp02_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp02, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
