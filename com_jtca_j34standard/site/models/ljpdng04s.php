...
class JtcaModelLjpdng04s extends JModelList
{
	 ...
	 */
	protected $context = 'com_jtca.ljpdng04s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpdng04_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng04_id');
		$this->setState('filter.ljpdng04_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng04_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng04_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng04_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng04_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng04_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng04_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng04_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng04_id'));
		$id .= ':'.$this->getState('filter.ljpdng04_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng04s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng04s').' AS a');
		...
		// Filter by a single or group of ljpdng04s.
		$ljpdng04_id = $this->getState('filter.ljpdng04_id');
		if ($ljpdng04_id != '')
		{
			if (is_numeric($ljpdng04_id))
			{
				$type = $this->getState('filter.ljpdng04_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng04_id);
			}
			else
			{
				if (is_array($ljpdng04_id))
				{
					JArrayHelper::toInteger($ljpdng04_id);
					$ljpdng04_id = implode(',', $ljpdng04_id);
					$type = $this->getState('filter.ljpdng04_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng04_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng04_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng04_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng04_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng04 params override menu item params only if menu param = 'use_ljpdng04'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng04' and there is no ljpdng04 param, use global

					// create an array of just the params set to 'use_ljpdng04'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng04_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng04')
						{
							// if the ljpdng04 has a value, use it
							if ($ljpdng04_params->get($key) != '')
							{
								// get the value from the ljpdng04
								$ljpdng04_array[$key] = $ljpdng04_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng04_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng04 params
					if (count($ljpdng04_array) > 0)
					{
						$ljpdng04_params = new Registry;
						$ljpdng04_params->loadArray($ljpdng04_array);
						$item->params->merge($ljpdng04_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng04_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng04, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
