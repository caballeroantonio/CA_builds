...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/slfirmjjadg16.php';

class JtcaModelSlfirmJjadg16Form extends JtcaModelSlfirmJjadg16
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.slfirmjjadg16';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.slfirmjjadg16';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.slfirmjjadg16', 'slfirmjjadg16', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the sub libro control de firmas del libro de control de presentacion de adolescentes en libertad ante el juez.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Sub Libro CONTROL DE FIRMAS Del LIBRO DE CONTROL DE PRESENTACION DE ADOLESCENTES EN LIBERTAD ANTE EL JUEZ item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('slfirmjjadg16.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.slfirmjjadg16.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('slfirmjjadg16.id');
		$is_new		= true;
		...
		// Reorder the control de firmas del libro de control de presenta so the new sub libro control de firmas del libro de control de presentacion de adolescentes en libertad ante el juez is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSlfirmJjadg16BeforeSave', array('com_jtca.slfirmjjadg16', &$table, $is_new));
		...
		$dispatcher->trigger('onSlfirmJjadg16AfterSave', array('com_jtca.slfirmjjadg16', $table, $is_new));

		$this->setState('slfirmjjadg16.id', $table->id);
