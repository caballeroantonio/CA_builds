...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpdng01.php';

class JtCaModelLjpdng01Form extends JtCaModelLjpdng01
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpdng01';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpdng01';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpdng01', 'ljpdng01', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpdng01.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpdng01.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpdng01.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjpdng01BeforeSave', array('com_jtca.ljpdng01', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpdng01AfterSave', array('com_jtca.ljpdng01', $table, $is_new));

		$this->setState('ljpdng01.id', $table->id);
