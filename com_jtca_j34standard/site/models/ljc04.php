 ...
 *
 */
class JtCaModelLjc04 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljc04';
	...
	{
		if (empty($config['ljc04_filter_fields']))
		{
			$config['ljc04_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljc04s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljc04.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJC04S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjc04ChangeState', array('com_jtca.ljc04', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjc04BeforeDelete', array('com_jtca.ljc04', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjc04AfterDelete', array('com_jtca.ljc04', &$table));
				}
				else
				{
					// Prune items that you can't change.
