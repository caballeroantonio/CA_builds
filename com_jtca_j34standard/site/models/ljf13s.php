...
class JtCaModelLjf13s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf13_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf13_id');
		$this->setState('filter.ljf13_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf13_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf13_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf13_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf13_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf13_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf13_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf13_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf13_id'));
		$id .= ':'.$this->getState('filter.ljf13_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf13s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf13s').' AS a');
		...
		// Filter by a single or group of ljf13s.
		$ljf13_id = $this->getState('filter.ljf13_id');
		if ($ljf13_id != '')
		{
			if (is_numeric($ljf13_id))
			{
				$type = $this->getState('filter.ljf13_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf13_id);
			}
			else
			{
				if (is_array($ljf13_id))
				{
					JArrayHelper::toInteger($ljf13_id);
					$ljf13_id = implode(',', $ljf13_id);
					$type = $this->getState('filter.ljf13_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf13_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf13_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf13_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf13_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf13 params override menu item params only if menu param = 'use_ljf13'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf13' and there is no ljf13 param, use global

					// create an array of just the params set to 'use_ljf13'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf13')
						{
							// if the ljf13 has a value, use it
							if ($ljf13_params->get($key) != '')
							{
								// get the value from the ljf13
								$ljf13_array[$key] = $ljf13_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf13_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf13 params
					if (count($ljf13_array) > 0)
					{
						$ljf13_params = new Registry;
						$ljf13_params->loadArray($ljf13_array);
						$item->params->merge($ljf13_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf13_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf13, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
