...
/**
 * This models supports retrieving lists of libro de oficios.
 *
 */
class JtCaModelLjjadng07s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadng07s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadng07_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadng07_id');
		$this->setState('filter.ljjadng07_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadng07_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadng07_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadng07_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadng07_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadng07_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadng07_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadng07_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadng07_id'));
		$id .= ':'.$this->getState('filter.ljjadng07_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadng07s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadng07s').' AS a');
		...
		// Filter by a single or group of ljjadng07s.
		$ljjadng07_id = $this->getState('filter.ljjadng07_id');
		if ($ljjadng07_id != '')
		{
			if (is_numeric($ljjadng07_id))
			{
				$type = $this->getState('filter.ljjadng07_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadng07_id);
			}
			else
			{
				if (is_array($ljjadng07_id))
				{
					JArrayHelper::toInteger($ljjadng07_id);
					$ljjadng07_id = implode(',', $ljjadng07_id);
					$type = $this->getState('filter.ljjadng07_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadng07_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadng07_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadng07_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de oficios.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadng07_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadng07 params override menu item params only if menu param = 'use_ljjadng07'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadng07' and there is no ljjadng07 param, use global

					// create an array of just the params set to 'use_ljjadng07'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadng07_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadng07')
						{
							// if the ljjadng07 has a value, use it
							if ($ljjadng07_params->get($key) != '')
							{
								// get the value from the ljjadng07
								$ljjadng07_array[$key] = $ljjadng07_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadng07_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadng07 params
					if (count($ljjadng07_array) > 0)
					{
						$ljjadng07_params = new Registry;
						$ljjadng07_params->loadArray($ljjadng07_array);
						$item->params->merge($ljjadng07_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadng07_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadng07, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
