...
class JtCaModelLjomf09s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf09_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf09_id');
		$this->setState('filter.ljomf09_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf09_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf09_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf09_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf09_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf09_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf09_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf09_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf09_id'));
		$id .= ':'.$this->getState('filter.ljomf09_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf09s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf09s').' AS a');
		...
		// Filter by a single or group of ljomf09s.
		$ljomf09_id = $this->getState('filter.ljomf09_id');
		if ($ljomf09_id != '')
		{
			if (is_numeric($ljomf09_id))
			{
				$type = $this->getState('filter.ljomf09_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf09_id);
			}
			else
			{
				if (is_array($ljomf09_id))
				{
					JArrayHelper::toInteger($ljomf09_id);
					$ljomf09_id = implode(',', $ljomf09_id);
					$type = $this->getState('filter.ljomf09_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf09_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf09_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf09_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf09_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf09 params override menu item params only if menu param = 'use_ljomf09'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf09' and there is no ljomf09 param, use global

					// create an array of just the params set to 'use_ljomf09'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf09_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf09')
						{
							// if the ljomf09 has a value, use it
							if ($ljomf09_params->get($key) != '')
							{
								// get the value from the ljomf09
								$ljomf09_array[$key] = $ljomf09_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf09_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf09 params
					if (count($ljomf09_array) > 0)
					{
						$ljomf09_params = new Registry;
						$ljomf09_params->loadArray($ljomf09_array);
						$item->params->merge($ljomf09_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf09_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf09, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
