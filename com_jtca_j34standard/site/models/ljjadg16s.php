...
/**
 * This models supports retrieving lists of libro de control de presentaciÃ³n de adolescentes e.
 *
 */
class JtCaModelLjjadg16s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadg16s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de control de presentaciÃ³n de adolescentes en libertad ante el juez has been specified either as a parameter or in the url Request
		$pk = $params->get('ljjadg16_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg16_id');
		$this->setState('filter.ljjadg16_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg16_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg16_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg16_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg16_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg16_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg16_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg16_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg16_id'));
		$id .= ':'.$this->getState('filter.ljjadg16_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg16s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg16s').' AS a');
		...
		// Filter by a single or group of ljjadg16s.
		$ljjadg16_id = $this->getState('filter.ljjadg16_id');
		if ($ljjadg16_id != '')
		{
			if (is_numeric($ljjadg16_id))
			{
				$type = $this->getState('filter.ljjadg16_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg16_id);
			}
			else
			{
				if (is_array($ljjadg16_id))
				{
					JArrayHelper::toInteger($ljjadg16_id);
					$ljjadg16_id = implode(',', $ljjadg16_id);
					$type = $this->getState('filter.ljjadg16_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg16_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg16_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg16_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de control de presentaciÃ³n de adolescentes e.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg16_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg16 params override menu item params only if menu param = 'use_ljjadg16'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg16' and there is no ljjadg16 param, use global

					// create an array of just the params set to 'use_ljjadg16'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg16_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg16')
						{
							// if the ljjadg16 has a value, use it
							if ($ljjadg16_params->get($key) != '')
							{
								// get the value from the ljjadg16
								$ljjadg16_array[$key] = $ljjadg16_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg16_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg16 params
					if (count($ljjadg16_array) > 0)
					{
						$ljjadg16_params = new Registry;
						$ljjadg16_params->loadArray($ljjadg16_array);
						$item->params->merge($ljjadg16_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg16_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg16, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
