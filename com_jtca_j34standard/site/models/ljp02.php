...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE REGISTRO DE BILLETES DE DEPÓSITO (INGRESOS) Model
 *
 */
class JtCaModelLjp02 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljp02';
	...
	{
		if (empty($config['ljp02_filter_fields']))
		{
			$config['ljp02_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljp02s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE REGISTRO DE BILLETES DE DEPÓSITO (INGRESOS) data.
	 *
	 * @param	integer	$pk	The id of the libro de registro de billetes de depósito (ingresos).
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljp02.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJP02S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjp02ChangeState', array('com_jtca.ljp02', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjp02BeforeDelete', array('com_jtca.ljp02', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjp02AfterDelete', array('com_jtca.ljp02', &$table));
				}
				else
				{
					// Prune items that you can't change.
