...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpdng10.php';

class JtcaModelLjpdng10Form extends JtcaModelLjpdng10
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpdng10';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpdng10';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpdng10', 'ljpdng10', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de registro de amparos.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE REGISTRO DE AMPAROS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpdng10.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpdng10.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpdng10.id');
		$is_new		= true;
		...
		// Reorder the libro de registro de amparos so the new libro de registro de amparos is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpdng10BeforeSave', array('com_jtca.ljpdng10', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpdng10AfterSave', array('com_jtca.ljpdng10', $table, $is_new));

		$this->setState('ljpdng10.id', $table->id);
