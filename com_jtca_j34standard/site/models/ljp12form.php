...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljp12.php';

class JtcaModelLjp12Form extends JtcaModelLjp12
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljp12';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljp12';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljp12', 'ljp12', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de registro de amparos circulantes o negados.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE REGISTRO DE AMPAROS CIRCULANTES O NEGADOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljp12.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljp12.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljp12.id');
		$is_new		= true;
		...
		// Reorder the libro de registro de amparos circulantes o negados so the new libro de registro de amparos circulantes o negados is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjp12BeforeSave', array('com_jtca.ljp12', &$table, $is_new));
		...
		$dispatcher->trigger('onLjp12AfterSave', array('com_jtca.ljp12', $table, $is_new));

		$this->setState('ljp12.id', $table->id);
