...
class JtCaModelLjp18s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljp18_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp18_id');
		$this->setState('filter.ljp18_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp18_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp18_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp18_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp18_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp18_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp18_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp18_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp18_id'));
		$id .= ':'.$this->getState('filter.ljp18_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp18s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp18s').' AS a');
		...
		// Filter by a single or group of ljp18s.
		$ljp18_id = $this->getState('filter.ljp18_id');
		if ($ljp18_id != '')
		{
			if (is_numeric($ljp18_id))
			{
				$type = $this->getState('filter.ljp18_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp18_id);
			}
			else
			{
				if (is_array($ljp18_id))
				{
					JArrayHelper::toInteger($ljp18_id);
					$ljp18_id = implode(',', $ljp18_id);
					$type = $this->getState('filter.ljp18_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp18_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp18_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp18_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp18_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp18 params override menu item params only if menu param = 'use_ljp18'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp18' and there is no ljp18 param, use global

					// create an array of just the params set to 'use_ljp18'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp18')
						{
							// if the ljp18 has a value, use it
							if ($ljp18_params->get($key) != '')
							{
								// get the value from the ljp18
								$ljp18_array[$key] = $ljp18_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp18_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp18 params
					if (count($ljp18_array) > 0)
					{
						$ljp18_params = new Registry;
						$ljp18_params->loadArray($ljp18_array);
						$item->params->merge($ljp18_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp18_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp18, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
