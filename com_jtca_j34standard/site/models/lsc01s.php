...
/**
 * This models supports retrieving lists of libreta de registro de sentencias -turno- (oficial.
 *
 */
class JtCaModelLsc01s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsc01s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de registro de sentencias -turno- (oficial) has been specified either as a parameter or in the url Request
		$pk = $params->get('lsc01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsc01_id');
		$this->setState('filter.lsc01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsc01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsc01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsc01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsc01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsc01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsc01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsc01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsc01_id'));
		$id .= ':'.$this->getState('filter.lsc01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsc01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsc01s').' AS a');
		...
		// Filter by a single or group of lsc01s.
		$lsc01_id = $this->getState('filter.lsc01_id');
		if ($lsc01_id != '')
		{
			if (is_numeric($lsc01_id))
			{
				$type = $this->getState('filter.lsc01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsc01_id);
			}
			else
			{
				if (is_array($lsc01_id))
				{
					JArrayHelper::toInteger($lsc01_id);
					$lsc01_id = implode(',', $lsc01_id);
					$type = $this->getState('filter.lsc01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsc01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsc01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsc01_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de registro de sentencias -turno- (oficial.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsc01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsc01 params override menu item params only if menu param = 'use_lsc01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsc01' and there is no lsc01 param, use global

					// create an array of just the params set to 'use_lsc01'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsc01')
						{
							// if the lsc01 has a value, use it
							if ($lsc01_params->get($key) != '')
							{
								// get the value from the lsc01
								$lsc01_array[$key] = $lsc01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsc01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsc01 params
					if (count($lsc01_array) > 0)
					{
						$lsc01_params = new Registry;
						$lsc01_params->loadArray($lsc01_array);
						$item->params->merge($lsc01_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsc01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsc01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
