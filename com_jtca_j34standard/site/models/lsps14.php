 ...
 *
 */
class JtcaModelLsps14 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsps14';
	...
	{
		if (empty($config['lsps14_filter_fields']))
		{
			$config['lsps14_filter_fields'] = array(
				'id', 'a.id',
				'id_organo','a.id_organo',
				'id_secretaria','a.id_secretaria',
				'anoj','a.anoj',
				'id_expediente','a.id_expediente',
				'field2447','a.field2447',
				'field2448','a.field2448',
				'field2449','a.field2449',
				'field2450','a.field2450',
				'field2451','a.field2451',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lsps14s', $prefix = 'JtcaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsps14.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPS14S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsps14ChangeState', array('com_jtca.lsps14', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsps14BeforeDelete', array('com_jtca.lsps14', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsps14AfterDelete', array('com_jtca.lsps14', &$table));
				}
				else
				{
					// Prune items that you can't change.
