...
/**
 * This models supports retrieving lists of jc01.
 *
 */
class JtCaModelLjc01s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljc01s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de gobierno has been specified either as a parameter or in the url Request
		$pk = $params->get('ljc01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc01_id');
		$this->setState('filter.ljc01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc01_id'));
		$id .= ':'.$this->getState('filter.ljc01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc01s').' AS a');
		...
		// Filter by a single or group of ljc01s.
		$ljc01_id = $this->getState('filter.ljc01_id');
		if ($ljc01_id != '')
		{
			if (is_numeric($ljc01_id))
			{
				$type = $this->getState('filter.ljc01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc01_id);
			}
			else
			{
				if (is_array($ljc01_id))
				{
					JArrayHelper::toInteger($ljc01_id);
					$ljc01_id = implode(',', $ljc01_id);
					$type = $this->getState('filter.ljc01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc01_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc01 params override menu item params only if menu param = 'use_ljc01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc01' and there is no ljc01 param, use global

					// create an array of just the params set to 'use_ljc01'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc01')
						{
							// if the ljc01 has a value, use it
							if ($ljc01_params->get($key) != '')
							{
								// get the value from the ljc01
								$ljc01_array[$key] = $ljc01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc01 params
					if (count($ljc01_array) > 0)
					{
						$ljc01_params = new Registry;
						$ljc01_params->loadArray($ljc01_array);
						$item->params->merge($ljc01_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
