...
/**
 * This models supports retrieving lists of libro de resoluciones en audiencia.
 *
 */
class JtCaModelLjpes13s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpes13s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de resoluciones en audiencia has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpes13_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes13_id');
		$this->setState('filter.ljpes13_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes13_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes13_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes13_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes13_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes13_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes13_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes13_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes13_id'));
		$id .= ':'.$this->getState('filter.ljpes13_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes13s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes13s').' AS a');
		...
		// Filter by a single or group of ljpes13s.
		$ljpes13_id = $this->getState('filter.ljpes13_id');
		if ($ljpes13_id != '')
		{
			if (is_numeric($ljpes13_id))
			{
				$type = $this->getState('filter.ljpes13_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes13_id);
			}
			else
			{
				if (is_array($ljpes13_id))
				{
					JArrayHelper::toInteger($ljpes13_id);
					$ljpes13_id = implode(',', $ljpes13_id);
					$type = $this->getState('filter.ljpes13_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes13_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes13_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes13_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de resoluciones en audiencia.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes13_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes13 params override menu item params only if menu param = 'use_ljpes13'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes13' and there is no ljpes13 param, use global

					// create an array of just the params set to 'use_ljpes13'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes13_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes13')
						{
							// if the ljpes13 has a value, use it
							if ($ljpes13_params->get($key) != '')
							{
								// get the value from the ljpes13
								$ljpes13_array[$key] = $ljpes13_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes13_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes13 params
					if (count($ljpes13_array) > 0)
					{
						$ljpes13_params = new Registry;
						$ljpes13_params->loadArray($ljpes13_array);
						$item->params->merge($ljpes13_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes13_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes13, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
