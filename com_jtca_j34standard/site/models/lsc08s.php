...
/**
 * This models supports retrieving lists of libro de pólizas y fianzas (oficial) - ingresos.
 *
 */
class JtcaModelLsc08s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsc08s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de pólizas y fianzas (oficial) - ingresos has been specified either as a parameter or in the url Request
		$pk = $params->get('lsc08_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsc08_id');
		$this->setState('filter.lsc08_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsc08_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsc08_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsc08_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsc08_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsc08_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsc08_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsc08_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsc08_id'));
		$id .= ':'.$this->getState('filter.lsc08_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsc08s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsc08s').' AS a');
		...
		// Filter by a single or group of lsc08s.
		$lsc08_id = $this->getState('filter.lsc08_id');
		if ($lsc08_id != '')
		{
			if (is_numeric($lsc08_id))
			{
				$type = $this->getState('filter.lsc08_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsc08_id);
			}
			else
			{
				if (is_array($lsc08_id))
				{
					JArrayHelper::toInteger($lsc08_id);
					$lsc08_id = implode(',', $lsc08_id);
					$type = $this->getState('filter.lsc08_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsc08_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsc08_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsc08_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de pólizas y fianzas (oficial) - ingresos.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsc08_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsc08 params override menu item params only if menu param = 'use_lsc08'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsc08' and there is no lsc08 param, use global

					// create an array of just the params set to 'use_lsc08'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsc08')
						{
							// if the lsc08 has a value, use it
							if ($lsc08_params->get($key) != '')
							{
								// get the value from the lsc08
								$lsc08_array[$key] = $lsc08_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsc08_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsc08 params
					if (count($lsc08_array) > 0)
					{
						$lsc08_params = new Registry;
						$lsc08_params->loadArray($lsc08_array);
						$item->params->merge($lsc08_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsc08_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsc08, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
	...
	/**
	 * Build a list of distinct values in the NÚMERO DE CERTIFICADO field
	 *
	 * @return	JDatabaseQuery
	 */
