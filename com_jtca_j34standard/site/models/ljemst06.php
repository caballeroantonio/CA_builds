 ...
 *
 */
class JtCaModelLjemst06 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljemst06';
	...
	{
		if (empty($config['ljemst06_filter_fields']))
		{
			$config['ljemst06_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljemst06s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljemst06.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJEMST06S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjemst06ChangeState', array('com_jtca.ljemst06', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjemst06BeforeDelete', array('com_jtca.ljemst06', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjemst06AfterDelete', array('com_jtca.ljemst06', &$table));
				}
				else
				{
					// Prune items that you can't change.
