...
class JtCaModelLsps10s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('lsps10_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsps10_id');
		$this->setState('filter.lsps10_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsps10_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsps10_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsps10_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsps10_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsps10_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsps10_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsps10_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsps10_id'));
		$id .= ':'.$this->getState('filter.lsps10_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsps10s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsps10s').' AS a');
		...
		// Filter by a single or group of lsps10s.
		$lsps10_id = $this->getState('filter.lsps10_id');
		if ($lsps10_id != '')
		{
			if (is_numeric($lsps10_id))
			{
				$type = $this->getState('filter.lsps10_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsps10_id);
			}
			else
			{
				if (is_array($lsps10_id))
				{
					JArrayHelper::toInteger($lsps10_id);
					$lsps10_id = implode(',', $lsps10_id);
					$type = $this->getState('filter.lsps10_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsps10_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsps10_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsps10_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsps10_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsps10 params override menu item params only if menu param = 'use_lsps10'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsps10' and there is no lsps10 param, use global

					// create an array of just the params set to 'use_lsps10'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsps10')
						{
							// if the lsps10 has a value, use it
							if ($lsps10_params->get($key) != '')
							{
								// get the value from the lsps10
								$lsps10_array[$key] = $lsps10_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsps10_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsps10 params
					if (count($lsps10_array) > 0)
					{
						$lsps10_params = new Registry;
						$lsps10_params->loadArray($lsps10_array);
						$item->params->merge($lsps10_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsps10_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsps10, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
