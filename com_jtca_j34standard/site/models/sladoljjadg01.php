...
/**
 * TSJ CDMX Libros TxCA Component Sub Libro ADOLESCENTE Del LIBRO DE GOBIERNO Model
 *
 */
class JtcaModelSladolJjadg01 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.sladoljjadg01';
	...
	{
		if (empty($config['sladoljjadg01_filter_fields']))
		{
			$config['sladoljjadg01_filter_fields'] = array(
				'id', 'a.id',
				'id_record', 'a.id_record',
				'id_field', 'a.id_field',
				'id_record','a.id_record',
				'id_field','a.id_field',
				'txt_field2031','a.txt_field2031',
				'id_field2031','a.id_field2031',
				'sfield29','a.sfield29',
				'sfield9','a.sfield9',
				'sfield10','a.sfield10',
				'sfield11','a.sfield11',
	...
	protected function populateState()
	{
		...
		$this->setState('sladoljjadg01.id', $pk);
	...
	*/
	public function getTable($type = 'SladolJjadg01s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get Sub Libro ADOLESCENTE Del LIBRO DE GOBIERNO data.
	 *
	 * @param	integer	$pk	The id of the sub libro adolescente del libro de gobierno.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('sladoljjadg01.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_SLADOL_JJADG01S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onSladolJjadg01ChangeState', array('com_jtca.sladoljjadg01', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onSladolJjadg01BeforeDelete', array('com_jtca.sladoljjadg01', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onSladolJjadg01AfterDelete', array('com_jtca.sladoljjadg01', &$table));
				}
				else
				{
					// Prune items that you can't change.
