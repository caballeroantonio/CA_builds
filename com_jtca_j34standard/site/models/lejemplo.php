...
class JtCaModelLejemplo extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lejemplo';
	...
	{
		if (empty($config['lejemplo_filter_fields']))
		{
	...
	*/
	public function getTable($type = 'Lejemplos', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get Libro De Ejemplo data.
	 *
	 * @param	integer	$pk	The id of the libro de ejemplo.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lejemplo.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LEJEMPLOS_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLejemploChangeState', array('com_jtca.lejemplo', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLejemploBeforeDelete', array('com_jtca.lejemplo', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLejemploAfterDelete', array('com_jtca.lejemplo', &$table));
				}
				else
				{
					// Prune items that you can't change.
