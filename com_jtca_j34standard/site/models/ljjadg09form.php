...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadg09.php';

class JtcaModelLjjadg09Form extends JtcaModelLjjadg09
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadg09';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadg09';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadg09', 'ljjadg09', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de actuarios.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE ACTUARIOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadg09.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadg09.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadg09.id');
		$is_new		= true;
		...
		// Reorder the libro de actuarios so the new libro de actuarios is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadg09BeforeSave', array('com_jtca.ljjadg09', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadg09AfterSave', array('com_jtca.ljjadg09', $table, $is_new));

		$this->setState('ljjadg09.id', $table->id);
