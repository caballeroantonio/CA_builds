...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE CONTROL DE FIRMAS PROCESADOS Model
 *
 */
class JtCaModelLjpdng17 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpdng17';
	...
	{
		if (empty($config['ljpdng17_filter_fields']))
		{
			$config['ljpdng17_filter_fields'] = array(
				'id', 'a.id',
				...
				'field2','a.field2',
				'field3_isMoral','a.field3_isMoral',
				'field3_paterno','a.field3_paterno',
				'field3_materno','a.field3_materno',
				'field3_nombre','a.field3_nombre',
	...
	*/
	public function getTable($type = 'Ljpdng17s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE CONTROL DE FIRMAS PROCESADOS data.
	 *
	 * @param	integer	$pk	The id of the libro de control de firmas procesados.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpdng17.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPDNG17S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpdng17ChangeState', array('com_jtca.ljpdng17', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpdng17BeforeDelete', array('com_jtca.ljpdng17', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpdng17AfterDelete', array('com_jtca.ljpdng17', &$table));
				}
				else
				{
					// Prune items that you can't change.
