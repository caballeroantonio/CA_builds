...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpdng02.php';

class JtCaModelLjpdng02Form extends JtCaModelLjpdng02
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpdng02';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpdng02';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpdng02', 'ljpdng02', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de certificado de depósitos de ingreso.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE CERTIFICADO DE DEPÓSITOS DE INGRESO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpdng02.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpdng02.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpdng02.id');
		$is_new		= true;
		...
		// Reorder the libro de certificado de depósitos de ingreso so the new libro de certificado de depósitos de ingreso is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpdng02BeforeSave', array('com_jtca.ljpdng02', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpdng02AfterSave', array('com_jtca.ljpdng02', $table, $is_new));

		$this->setState('ljpdng02.id', $table->id);
