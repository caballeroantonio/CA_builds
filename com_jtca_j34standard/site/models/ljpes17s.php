...
class JtCaModelLjpes17s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpes17_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes17_id');
		$this->setState('filter.ljpes17_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes17_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes17_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes17_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes17_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes17_initial_direction','ASC');
		}
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes17_id'));
		$id .= ':'.$this->getState('filter.ljpes17_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes17s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		// Filter by a single or group of ljpes17s.
		$ljpes17_id = $this->getState('filter.ljpes17_id');
		if ($ljpes17_id != '')
		{
			if (is_numeric($ljpes17_id))
			{
				$type = $this->getState('filter.ljpes17_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes17_id);
			}
			else
			{
				if (is_array($ljpes17_id))
				{
					JArrayHelper::toInteger($ljpes17_id);
					$ljpes17_id = implode(',', $ljpes17_id);
					$type = $this->getState('filter.ljpes17_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes17_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes17_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes17_filter_field'))
			{
				case 'created_by':
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				else
				{
					...
					// get display date
					switch ($item->params->get('list_show_ljpes17_date'))
					{
						case 'modified':
