...
class JtcaModelLsc03 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsc03';
	...
	{
		if (empty($config['lsc03_filter_fields']))
		{
			$config['lsc03_filter_fields'] = array(
				'id', 'a.id',
				...
				'field131','a.field131',
				'field134','a.field134',
				'field135_isMoral','a.field135_isMoral',
				'field135_paterno','a.field135_paterno',
				'field135_materno','a.field135_materno',
				'field135_nombre','a.field135_nombre',
				'field254','a.field254',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	...
	/**
	 * Method to get LIBRETA DE VISTAS AL M.P (OFICIAL) data.
	 *
	 * @param	integer	$pk	The id of the libreta de vistas al m.p (oficial).
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsc03.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSC03S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsc03ChangeState', array('com_jtca.lsc03', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsc03BeforeDelete', array('com_jtca.lsc03', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsc03AfterDelete', array('com_jtca.lsc03', &$table));
				}
				else
				{
					// Prune items that you can't change.
