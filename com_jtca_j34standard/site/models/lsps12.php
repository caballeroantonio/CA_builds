...
/**
 * TSJ CDMX Libros TxCA Component REGISTRO DE PROMOCIONES Model
 *
 */
class JtcaModelLsps12 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsps12';
	...
	{
		if (empty($config['lsps12_filter_fields']))
		{
			$config['lsps12_filter_fields'] = array(
				'id', 'a.id',
				'id_organo','a.id_organo',
				'id_secretaria','a.id_secretaria',
				'anoj','a.anoj',
				'id_expediente','a.id_expediente',
				'field2441','a.field2441',
				'field2442_isMoral','a.field2442_isMoral',
				'field2442_paterno','a.field2442_paterno',
				'field2442_materno','a.field2442_materno',
				'field2442_nombre','a.field2442_nombre',
				'field2443','a.field2443',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lsps12s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get REGISTRO DE PROMOCIONES data.
	 *
	 * @param	integer	$pk	The id of the registro de promociones.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsps12.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPS12S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsps12ChangeState', array('com_jtca.lsps12', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsps12BeforeDelete', array('com_jtca.lsps12', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsps12AfterDelete', array('com_jtca.lsps12', &$table));
				}
				else
				{
					// Prune items that you can't change.
