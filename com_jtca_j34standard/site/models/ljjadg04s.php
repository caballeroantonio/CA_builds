...
/**
 * This models supports retrieving lists of libro de egresos.
 *
 */
class JtCaModelLjjadg04s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadg04s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de egresos has been specified either as a parameter or in the url Request
		$pk = $params->get('ljjadg04_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg04_id');
		$this->setState('filter.ljjadg04_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg04_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg04_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg04_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg04_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg04_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg04_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg04_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg04_id'));
		$id .= ':'.$this->getState('filter.ljjadg04_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg04s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg04s').' AS a');
		...
		// Filter by a single or group of ljjadg04s.
		$ljjadg04_id = $this->getState('filter.ljjadg04_id');
		if ($ljjadg04_id != '')
		{
			if (is_numeric($ljjadg04_id))
			{
				$type = $this->getState('filter.ljjadg04_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg04_id);
			}
			else
			{
				if (is_array($ljjadg04_id))
				{
					JArrayHelper::toInteger($ljjadg04_id);
					$ljjadg04_id = implode(',', $ljjadg04_id);
					$type = $this->getState('filter.ljjadg04_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg04_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg04_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg04_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de egresos.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg04_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg04 params override menu item params only if menu param = 'use_ljjadg04'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg04' and there is no ljjadg04 param, use global

					// create an array of just the params set to 'use_ljjadg04'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg04_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg04')
						{
							// if the ljjadg04 has a value, use it
							if ($ljjadg04_params->get($key) != '')
							{
								// get the value from the ljjadg04
								$ljjadg04_array[$key] = $ljjadg04_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg04_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg04 params
					if (count($ljjadg04_array) > 0)
					{
						$ljjadg04_params = new Registry;
						$ljjadg04_params->loadArray($ljjadg04_array);
						$item->params->merge($ljjadg04_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg04_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg04, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
