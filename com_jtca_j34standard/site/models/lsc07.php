...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE EGRESOS (OFICIAL) Model
 *
 */
class JtCaModelLsc07 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsc07';
	...
	{
		if (empty($config['lsc07_filter_fields']))
		{
			$config['lsc07_filter_fields'] = array(
				'id', 'a.id',
				...
				'field162','a.field162',
				'field163','a.field163',
				'field164_isMoral','a.field164_isMoral',
				'field164_paterno','a.field164_paterno',
				'field164_materno','a.field164_materno',
				'field164_nombre','a.field164_nombre',
				'field165','a.field165',
				'field166','a.field166',
				'field167_isMoral','a.field167_isMoral',
				'field167_paterno','a.field167_paterno',
				'field167_materno','a.field167_materno',
				'field167_nombre','a.field167_nombre',
				'field168','a.field168',
				'field169','a.field169',
				'field169b','a.field169b',
				'field170','a.field170',
				'field171','a.field171',
				'field172','a.field172',
				'field173','a.field173',
	...
	*/
	public function getTable($type = 'Lsc07s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE EGRESOS (OFICIAL) data.
	 *
	 * @param	integer	$pk	The id of the libro de egresos (oficial).
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsc07.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSC07S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsc07ChangeState', array('com_jtca.lsc07', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsc07BeforeDelete', array('com_jtca.lsc07', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsc07AfterDelete', array('com_jtca.lsc07', &$table));
				}
				else
				{
					// Prune items that you can't change.
