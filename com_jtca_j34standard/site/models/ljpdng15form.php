...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpdng15.php';

class JtCaModelLjpdng15Form extends JtCaModelLjpdng15
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpdng15';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpdng15';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpdng15', 'ljpdng15', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpdng15.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpdng15.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpdng15.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjpdng15BeforeSave', array('com_jtca.ljpdng15', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpdng15AfterSave', array('com_jtca.ljpdng15', $table, $is_new));

		$this->setState('ljpdng15.id', $table->id);
