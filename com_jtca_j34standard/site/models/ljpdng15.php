 ...
 *
 */
class JtCaModelLjpdng15 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpdng15';
	...
	{
		if (empty($config['ljpdng15_filter_fields']))
		{
			$config['ljpdng15_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljpdng15s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpdng15.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPDNG15S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpdng15ChangeState', array('com_jtca.ljpdng15', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpdng15BeforeDelete', array('com_jtca.ljpdng15', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpdng15AfterDelete', array('com_jtca.ljpdng15', &$table));
				}
				else
				{
					// Prune items that you can't change.
