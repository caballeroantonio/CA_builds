...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE SENTENCIAS Model
 *
 */
class JtCaModelLjjadng04 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljjadng04';
	...
	{
		if (empty($config['ljjadng04_filter_fields']))
		{
			$config['ljjadng04_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljjadng04s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE SENTENCIAS data.
	 *
	 * @param	integer	$pk	The id of the libro de sentencias.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljjadng04.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJJADNG04S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjjadng04ChangeState', array('com_jtca.ljjadng04', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjjadng04BeforeDelete', array('com_jtca.ljjadng04', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjjadng04AfterDelete', array('com_jtca.ljjadng04', &$table));
				}
				else
				{
					// Prune items that you can't change.
