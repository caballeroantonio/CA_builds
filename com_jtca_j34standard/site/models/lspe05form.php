...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lspe05.php';

class JtCaModelLspe05Form extends JtCaModelLspe05
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lspe05';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lspe05';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lspe05', 'lspe05', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de desistimiento.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE DESISTIMIENTO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lspe05.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lspe05.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lspe05.id');
		$is_new		= true;
		...
		// Reorder the libro de desistimiento so the new libro de desistimiento is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLspe05BeforeSave', array('com_jtca.lspe05', &$table, $is_new));
		...
		$dispatcher->trigger('onLspe05AfterSave', array('com_jtca.lspe05', $table, $is_new));

		$this->setState('lspe05.id', $table->id);
