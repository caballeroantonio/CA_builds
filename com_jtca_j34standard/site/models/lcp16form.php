...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp16.php';

class JtCaModelLcp16Form extends JtCaModelLcp16
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp16';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp16';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp16', 'lcp16', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de  oficios de amparos de conocimiento..
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE  OFICIOS DE AMPAROS DE CONOCIMIENTO. item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp16.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp16.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp16.id');
		$is_new		= true;
		...
		// Reorder the libreta de  oficios de amparos de conocimiento. so the new libreta de  oficios de amparos de conocimiento. is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp16BeforeSave', array('com_jtca.lcp16', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp16AfterSave', array('com_jtca.lcp16', $table, $is_new));

		$this->setState('lcp16.id', $table->id);
