...
/**
 * This models supports retrieving lists of libro de turno para excusas de adolescentes.
 *
 */
class JtCaModelLcp07s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp07s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno para excusas de adolescentes has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp07_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp07_id');
		$this->setState('filter.lcp07_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp07_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp07_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp07_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp07_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp07_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp07_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp07_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp07_id'));
		$id .= ':'.$this->getState('filter.lcp07_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp07s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp07s').' AS a');
		...
		// Filter by a single or group of lcp07s.
		$lcp07_id = $this->getState('filter.lcp07_id');
		if ($lcp07_id != '')
		{
			if (is_numeric($lcp07_id))
			{
				$type = $this->getState('filter.lcp07_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp07_id);
			}
			else
			{
				if (is_array($lcp07_id))
				{
					JArrayHelper::toInteger($lcp07_id);
					$lcp07_id = implode(',', $lcp07_id);
					$type = $this->getState('filter.lcp07_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp07_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp07_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp07_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno para excusas de adolescentes.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp07_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp07 params override menu item params only if menu param = 'use_lcp07'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp07' and there is no lcp07 param, use global

					// create an array of just the params set to 'use_lcp07'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp07')
						{
							// if the lcp07 has a value, use it
							if ($lcp07_params->get($key) != '')
							{
								// get the value from the lcp07
								$lcp07_array[$key] = $lcp07_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp07_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp07 params
					if (count($lcp07_array) > 0)
					{
						$lcp07_params = new Registry;
						$lcp07_params->loadArray($lcp07_array);
						$item->params->merge($lcp07_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp07_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp07, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
