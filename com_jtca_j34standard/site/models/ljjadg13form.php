...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadg13.php';

class JtCaModelLjjadg13Form extends JtCaModelLjjadg13
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadg13';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadg13';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadg13', 'ljjadg13', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadg13.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadg13.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadg13.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjjadg13BeforeSave', array('com_jtca.ljjadg13', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadg13AfterSave', array('com_jtca.ljjadg13', $table, $is_new));

		$this->setState('ljjadg13.id', $table->id);
