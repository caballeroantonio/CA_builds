...
/**
 * This models supports retrieving lists of jc10.
 *
 */
class JtCaModelLjc10s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljc10s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de auxiliares de la administraciÃ³n de justicia has been specified either as a parameter or in the url Request
		$pk = $params->get('ljc10_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc10_id');
		$this->setState('filter.ljc10_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc10_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc10_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc10_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc10_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc10_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc10_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc10_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id	.= ':'.$this->getState('filter.id_expediente');	
		$id .= ':'.serialize($this->getState('filter.ljc10_id'));
		$id .= ':'.$this->getState('filter.ljc10_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc10s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc10s').' AS a');
		...
		if ($id_expediente = $this->getState('filter.id_expediente'))
		{
			$id_expediente = $db->escape(JString::strtolower($id_expediente), true);			
			$query->where('LOWER('.$db->quoteName('a.id_expediente').') = ' . $db->quote($id_expediente));
		}	

		// Filter by a single or group of ljc10s.
		$ljc10_id = $this->getState('filter.ljc10_id');
		if ($ljc10_id != '')
		{
			if (is_numeric($ljc10_id))
			{
				$type = $this->getState('filter.ljc10_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc10_id);
			}
			else
			{
				if (is_array($ljc10_id))
				{
					JArrayHelper::toInteger($ljc10_id);
					$ljc10_id = implode(',', $ljc10_id);
					$type = $this->getState('filter.ljc10_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc10_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc10_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc10_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc10_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc10 params override menu item params only if menu param = 'use_ljc10'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc10' and there is no ljc10 param, use global

					// create an array of just the params set to 'use_ljc10'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc10')
						{
							// if the ljc10 has a value, use it
							if ($ljc10_params->get($key) != '')
							{
								// get the value from the ljc10
								$ljc10_array[$key] = $ljc10_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc10_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc10 params
					if (count($ljc10_array) > 0)
					{
						$ljc10_params = new Registry;
						$ljc10_params->loadArray($ljc10_array);
						$item->params->merge($ljc10_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc10_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc10, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
	...
	/**
	 * Build a list of distinct values in the Expediente field
	 *
	 * @return	JDatabaseQuery
	 */
	public function getIdexpedientevalues()
	{
	}				
