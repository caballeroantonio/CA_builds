...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE TURNO PARA EXCUSAS DE ADOLESCENTES Model
 *
 */
class JtCaModelLcp07 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp07';
	...
	{
		if (empty($config['lcp07_filter_fields']))
		{
			$config['lcp07_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp07s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE TURNO PARA EXCUSAS DE ADOLESCENTES data.
	 *
	 * @param	integer	$pk	The id of the libro de turno para excusas de adolescentes.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp07.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP07S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp07ChangeState', array('com_jtca.lcp07', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp07BeforeDelete', array('com_jtca.lcp07', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp07AfterDelete', array('com_jtca.lcp07', &$table));
				}
				else
				{
					// Prune items that you can't change.
