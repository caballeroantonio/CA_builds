...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE TURNO A CARGO DE LOS SECRETARIOS DE ACUERDOS Model
 *
 */
class JtCaModelLjomf15 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljomf15';
	...
	{
		if (empty($config['ljomf15_filter_fields']))
		{
			$config['ljomf15_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljomf15s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE TURNO A CARGO DE LOS SECRETARIOS DE ACUERDOS data.
	 *
	 * @param	integer	$pk	The id of the libro de turno a cargo de los secretarios de acuerdos.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljomf15.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJOMF15S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjomf15ChangeState', array('com_jtca.ljomf15', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjomf15BeforeDelete', array('com_jtca.ljomf15', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjomf15AfterDelete', array('com_jtca.ljomf15', &$table));
				}
				else
				{
					// Prune items that you can't change.
