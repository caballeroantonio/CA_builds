...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp28.php';

class JtCaModelLcp28Form extends JtCaModelLcp28
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp28';
	/**
	...
	*/
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno de excusas para juzgados  de delitos no graves.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO DE EXCUSAS PARA JUZGADOS  DE DELITOS NO GRAVES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp28.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp28.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp28.id');
		$is_new		= true;
		...
		// Reorder the libro de turno de excusas para juzgados  de delito so the new libro de turno de excusas para juzgados  de delitos no graves is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp28BeforeSave', array('com_jtca.lcp28', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp28AfterSave', array('com_jtca.lcp28', $table, $is_new));

		$this->setState('lcp28.id', $table->id);
