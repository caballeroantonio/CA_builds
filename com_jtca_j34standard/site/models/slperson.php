...
/**
 * TSJ CDMX Libros TxCA Component Sub Libro Personas Model
 *
 */
class JtcaModelSlperson extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.slperson';
	...
	{
		if (empty($config['slperson_filter_fields']))
		{
			$config['slperson_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Slpersons', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get Sub Libro Personas data.
	 *
	 * @param	integer	$pk	The id of the sub libro personas.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('slperson.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_SLPERSONS_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onSlpersonChangeState', array('com_jtca.slperson', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onSlpersonBeforeDelete', array('com_jtca.slperson', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onSlpersonAfterDelete', array('com_jtca.slperson', &$table));
				}
				else
				{
					// Prune items that you can't change.
