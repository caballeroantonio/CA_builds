...
/**
 * This models supports retrieving lists of jc17.
 *
 */
class JtCaModelLjc17s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljc17s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de remisiÃ³n al archivo has been specified either as a parameter or in the url Request
		$pk = $params->get('ljc17_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc17_id');
		$this->setState('filter.ljc17_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc17_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc17_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc17_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc17_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc17_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc17_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc17_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc17_id'));
		$id .= ':'.$this->getState('filter.ljc17_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc17s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc17s').' AS a');
		...
		// Filter by a single or group of ljc17s.
		$ljc17_id = $this->getState('filter.ljc17_id');
		if ($ljc17_id != '')
		{
			if (is_numeric($ljc17_id))
			{
				$type = $this->getState('filter.ljc17_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc17_id);
			}
			else
			{
				if (is_array($ljc17_id))
				{
					JArrayHelper::toInteger($ljc17_id);
					$ljc17_id = implode(',', $ljc17_id);
					$type = $this->getState('filter.ljc17_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc17_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc17_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc17_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc17_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc17 params override menu item params only if menu param = 'use_ljc17'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc17' and there is no ljc17 param, use global

					// create an array of just the params set to 'use_ljc17'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc17')
						{
							// if the ljc17 has a value, use it
							if ($ljc17_params->get($key) != '')
							{
								// get the value from the ljc17
								$ljc17_array[$key] = $ljc17_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc17_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc17 params
					if (count($ljc17_array) > 0)
					{
						$ljc17_params = new Registry;
						$ljc17_params->loadArray($ljc17_array);
						$item->params->merge($ljc17_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc17_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc17, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
