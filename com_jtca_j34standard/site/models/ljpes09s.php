...
/**
 * This models supports retrieving lists of libro de multas judiciales.
 *
 */
class JtcaModelLjpes09s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpes09s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de multas judiciales has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpes09_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes09_id');
		$this->setState('filter.ljpes09_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes09_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes09_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes09_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes09_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes09_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes09_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes09_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes09_id'));
		$id .= ':'.$this->getState('filter.ljpes09_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes09s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes09s').' AS a');
		...
		// Filter by a single or group of ljpes09s.
		$ljpes09_id = $this->getState('filter.ljpes09_id');
		if ($ljpes09_id != '')
		{
			if (is_numeric($ljpes09_id))
			{
				$type = $this->getState('filter.ljpes09_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes09_id);
			}
			else
			{
				if (is_array($ljpes09_id))
				{
					JArrayHelper::toInteger($ljpes09_id);
					$ljpes09_id = implode(',', $ljpes09_id);
					$type = $this->getState('filter.ljpes09_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes09_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes09_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes09_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de multas judiciales.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes09_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes09 params override menu item params only if menu param = 'use_ljpes09'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes09' and there is no ljpes09 param, use global

					// create an array of just the params set to 'use_ljpes09'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes09_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes09')
						{
							// if the ljpes09 has a value, use it
							if ($ljpes09_params->get($key) != '')
							{
								// get the value from the ljpes09
								$ljpes09_array[$key] = $ljpes09_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes09_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes09 params
					if (count($ljpes09_array) > 0)
					{
						$ljpes09_params = new Registry;
						$ljpes09_params->loadArray($ljpes09_array);
						$item->params->merge($ljpes09_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes09_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes09, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
