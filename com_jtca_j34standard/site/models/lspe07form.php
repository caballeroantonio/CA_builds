...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lspe07.php';

class JtCaModelLspe07Form extends JtCaModelLspe07
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lspe07';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lspe07';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lspe07', 'lspe07', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de finanzas.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE FINANZAS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lspe07.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lspe07.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lspe07.id');
		$is_new		= true;
		...
		// Reorder the libro de finanzas so the new libro de finanzas is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLspe07BeforeSave', array('com_jtca.lspe07', &$table, $is_new));
		...
		$dispatcher->trigger('onLspe07AfterSave', array('com_jtca.lspe07', $table, $is_new));

		$this->setState('lspe07.id', $table->id);
