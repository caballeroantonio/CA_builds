...
class JtCaModelLjomf11s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf11_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf11_id');
		$this->setState('filter.ljomf11_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf11_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf11_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf11_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf11_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf11_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf11_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf11_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf11_id'));
		$id .= ':'.$this->getState('filter.ljomf11_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf11s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf11s').' AS a');
		...
		// Filter by a single or group of ljomf11s.
		$ljomf11_id = $this->getState('filter.ljomf11_id');
		if ($ljomf11_id != '')
		{
			if (is_numeric($ljomf11_id))
			{
				$type = $this->getState('filter.ljomf11_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf11_id);
			}
			else
			{
				if (is_array($ljomf11_id))
				{
					JArrayHelper::toInteger($ljomf11_id);
					$ljomf11_id = implode(',', $ljomf11_id);
					$type = $this->getState('filter.ljomf11_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf11_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf11_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf11_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf11_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf11 params override menu item params only if menu param = 'use_ljomf11'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf11' and there is no ljomf11 param, use global

					// create an array of just the params set to 'use_ljomf11'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf11_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf11')
						{
							// if the ljomf11 has a value, use it
							if ($ljomf11_params->get($key) != '')
							{
								// get the value from the ljomf11
								$ljomf11_array[$key] = $ljomf11_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf11_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf11 params
					if (count($ljomf11_array) > 0)
					{
						$ljomf11_params = new Registry;
						$ljomf11_params->loadArray($ljomf11_array);
						$item->params->merge($ljomf11_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf11_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf11, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
