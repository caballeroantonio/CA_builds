...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp39.php';

class JtCaModelLcp39Form extends JtCaModelLcp39
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp39';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp39';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp39', 'lcp39', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno de excusas para juzgados de justicia para adolescentes.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO DE EXCUSAS PARA JUZGADOS DE JUSTICIA PARA ADOLESCENTES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp39.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp39.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp39.id');
		$is_new		= true;
		...
		// Reorder the libro de turno de excusas para juzgados de justici so the new libro de turno de excusas para juzgados de justicia para adolescentes is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp39BeforeSave', array('com_jtca.lcp39', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp39AfterSave', array('com_jtca.lcp39', $table, $is_new));

		$this->setState('lcp39.id', $table->id);
