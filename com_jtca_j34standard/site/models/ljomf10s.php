...
/**
 * This models supports retrieving lists of libro de auxiliares de la administraciÃ³n de justic.
 *
 */
class JtCaModelLjomf10s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljomf10s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de auxiliares de la administraciÃ³n de justicia, tutores y curadores has been specified either as a parameter or in the url Request
		$pk = $params->get('ljomf10_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf10_id');
		$this->setState('filter.ljomf10_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf10_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf10_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf10_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf10_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf10_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf10_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf10_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf10_id'));
		$id .= ':'.$this->getState('filter.ljomf10_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf10s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf10s').' AS a');
		...
		// Filter by a single or group of ljomf10s.
		$ljomf10_id = $this->getState('filter.ljomf10_id');
		if ($ljomf10_id != '')
		{
			if (is_numeric($ljomf10_id))
			{
				$type = $this->getState('filter.ljomf10_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf10_id);
			}
			else
			{
				if (is_array($ljomf10_id))
				{
					JArrayHelper::toInteger($ljomf10_id);
					$ljomf10_id = implode(',', $ljomf10_id);
					$type = $this->getState('filter.ljomf10_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf10_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf10_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf10_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de auxiliares de la administraciÃ³n de justic.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf10_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf10 params override menu item params only if menu param = 'use_ljomf10'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf10' and there is no ljomf10 param, use global

					// create an array of just the params set to 'use_ljomf10'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf10_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf10')
						{
							// if the ljomf10 has a value, use it
							if ($ljomf10_params->get($key) != '')
							{
								// get the value from the ljomf10
								$ljomf10_array[$key] = $ljomf10_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf10_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf10 params
					if (count($ljomf10_array) > 0)
					{
						$ljomf10_params = new Registry;
						$ljomf10_params->loadArray($ljomf10_array);
						$item->params->merge($ljomf10_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf10_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf10, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
