...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadg15.php';

class JtCaModelLjjadg15Form extends JtCaModelLjjadg15
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadg15';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadg15';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadg15', 'ljjadg15', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the articulo 36 o articulo 28 ljadf.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	ARTICULO 36 O ARTICULO 28 LJADF item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadg15.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadg15.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadg15.id');
		$is_new		= true;
		...
		// Reorder the articulo 36 o articulo 28 ljadf so the new articulo 36 o articulo 28 ljadf is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadg15BeforeSave', array('com_jtca.ljjadg15', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadg15AfterSave', array('com_jtca.ljjadg15', $table, $is_new));

		$this->setState('ljjadg15.id', $table->id);
