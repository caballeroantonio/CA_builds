...
/**
 * This models supports retrieving lists of jc20.
 *
 */
class JtCaModelLjc20s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljc20s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc20_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc20_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc20_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc20_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc20_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc20_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc20_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc20_id'));
		$id .= ':'.$this->getState('filter.ljc20_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc20s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc20s').' AS a');
		...
		// Filter by a single or group of ljc20s.
		$ljc20_id = $this->getState('filter.ljc20_id');
		if ($ljc20_id != '')
		{
			if (is_numeric($ljc20_id))
			{
				$type = $this->getState('filter.ljc20_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc20_id);
			}
			else
			{
				if (is_array($ljc20_id))
				{
					JArrayHelper::toInteger($ljc20_id);
					$ljc20_id = implode(',', $ljc20_id);
					$type = $this->getState('filter.ljc20_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc20_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc20_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc20_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc20_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc20 params override menu item params only if menu param = 'use_ljc20'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc20' and there is no ljc20 param, use global

					// create an array of just the params set to 'use_ljc20'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc20')
						{
							// if the ljc20 has a value, use it
							if ($ljc20_params->get($key) != '')
							{
								// get the value from the ljc20
								$ljc20_array[$key] = $ljc20_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc20_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc20 params
					if (count($ljc20_array) > 0)
					{
						$ljc20_params = new Registry;
						$ljc20_params->loadArray($ljc20_array);
						$item->params->merge($ljc20_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc20_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc20, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
