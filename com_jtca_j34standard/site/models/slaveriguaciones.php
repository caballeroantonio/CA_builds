...
/**
 * This models supports retrieving lists of averiguaciones previas.
 *
 */
class JtcaModelSlaveriguaciones extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.slaveriguaciones';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single averiguaciones previas has been specified either as a parameter or in the url Request
		$pk = $params->get('slaveriguacion_id', '') == '' ? $app->input->getInt('id', '') : $params->get('slaveriguacion_id');
		$this->setState('filter.slaveriguacion_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('slaveriguacion_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('slaveriguacion_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('slaveriguacion_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('slaveriguacion_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('slaveriguacion_initial_direction','ASC');
		}
		...
		if ($params->get('filter_slaveriguacion_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_slaveriguacion_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.slaveriguacion_id'));
		$id .= ':'.$this->getState('filter.slaveriguacion_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of slaveriguaciones subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_slaveriguaciones').' AS a');
		...
		// Filter by a single or group of slaveriguaciones.
		$slaveriguacion_id = $this->getState('filter.slaveriguacion_id');
		if ($slaveriguacion_id != '')
		{
			if (is_numeric($slaveriguacion_id))
			{
				$type = $this->getState('filter.slaveriguacion_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $slaveriguacion_id);
			}
			else
			{
				if (is_array($slaveriguacion_id))
				{
					JArrayHelper::toInteger($slaveriguacion_id);
					$slaveriguacion_id = implode(',', $slaveriguacion_id);
					$type = $this->getState('filter.slaveriguacion_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$slaveriguacion_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_slaveriguacion_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_slaveriguacion_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of averiguaciones previas.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$slaveriguacion_params = new Registry;
				...
				{
					$item->params = $slaveriguacion_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Slaveriguacion params override menu item params only if menu param = 'use_slaveriguacion'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_slaveriguacion' and there is no slaveriguacion param, use global

					// create an array of just the params set to 'use_slaveriguacion'
					$menu_params_array = $this->getState('params')->toArray();
					$slaveriguacion_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_slaveriguacion')
						{
							// if the slaveriguacion has a value, use it
							if ($slaveriguacion_params->get($key) != '')
							{
								// get the value from the slaveriguacion
								$slaveriguacion_array[$key] = $slaveriguacion_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$slaveriguacion_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected slaveriguacion params
					if (count($slaveriguacion_array) > 0)
					{
						$slaveriguacion_params = new Registry;
						$slaveriguacion_params->loadArray($slaveriguacion_array);
						$item->params->merge($slaveriguacion_params);
					}


					// get display date
					switch ($item->params->get('list_show_slaveriguacion_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an slaveriguacion, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
