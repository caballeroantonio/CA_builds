...
class JtCaModelLjomf07s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf07_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf07_id');
		$this->setState('filter.ljomf07_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf07_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf07_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf07_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf07_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf07_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf07_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf07_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf07_id'));
		$id .= ':'.$this->getState('filter.ljomf07_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf07s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf07s').' AS a');
		...
		// Filter by a single or group of ljomf07s.
		$ljomf07_id = $this->getState('filter.ljomf07_id');
		if ($ljomf07_id != '')
		{
			if (is_numeric($ljomf07_id))
			{
				$type = $this->getState('filter.ljomf07_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf07_id);
			}
			else
			{
				if (is_array($ljomf07_id))
				{
					JArrayHelper::toInteger($ljomf07_id);
					$ljomf07_id = implode(',', $ljomf07_id);
					$type = $this->getState('filter.ljomf07_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf07_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf07_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf07_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf07_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf07 params override menu item params only if menu param = 'use_ljomf07'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf07' and there is no ljomf07 param, use global

					// create an array of just the params set to 'use_ljomf07'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf07_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf07')
						{
							// if the ljomf07 has a value, use it
							if ($ljomf07_params->get($key) != '')
							{
								// get the value from the ljomf07
								$ljomf07_array[$key] = $ljomf07_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf07_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf07 params
					if (count($ljomf07_array) > 0)
					{
						$ljomf07_params = new Registry;
						$ljomf07_params->loadArray($ljomf07_array);
						$item->params->merge($ljomf07_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf07_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf07, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
