...
class JtcaModelLjoc07s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljoc07_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljoc07_id');
		$this->setState('filter.ljoc07_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljoc07_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljoc07_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljoc07_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljoc07_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljoc07_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljoc07_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljoc07_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljoc07_id'));
		$id .= ':'.$this->getState('filter.ljoc07_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljoc07s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljoc07s').' AS a');
		...
		// Filter by a single or group of ljoc07s.
		$ljoc07_id = $this->getState('filter.ljoc07_id');
		if ($ljoc07_id != '')
		{
			if (is_numeric($ljoc07_id))
			{
				$type = $this->getState('filter.ljoc07_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljoc07_id);
			}
			else
			{
				if (is_array($ljoc07_id))
				{
					JArrayHelper::toInteger($ljoc07_id);
					$ljoc07_id = implode(',', $ljoc07_id);
					$type = $this->getState('filter.ljoc07_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljoc07_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljoc07_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljoc07_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljoc07_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljoc07 params override menu item params only if menu param = 'use_ljoc07'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljoc07' and there is no ljoc07 param, use global

					// create an array of just the params set to 'use_ljoc07'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljoc07')
						{
							// if the ljoc07 has a value, use it
							if ($ljoc07_params->get($key) != '')
							{
								// get the value from the ljoc07
								$ljoc07_array[$key] = $ljoc07_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljoc07_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljoc07 params
					if (count($ljoc07_array) > 0)
					{
						$ljoc07_params = new Registry;
						$ljoc07_params->loadArray($ljoc07_array);
						$item->params->merge($ljoc07_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljoc07_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljoc07, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
