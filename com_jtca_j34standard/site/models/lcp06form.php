...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp06.php';

class JtCaModelLcp06Form extends JtCaModelLcp06
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp06';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp06';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp06', 'lcp06', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno para incompetencias adolescentes.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO PARA INCOMPETENCIAS ADOLESCENTES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp06.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp06.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp06.id');
		$is_new		= true;
		...
		// Reorder the libro de turno para incompetencias adolescentes so the new libro de turno para incompetencias adolescentes is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp06BeforeSave', array('com_jtca.lcp06', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp06AfterSave', array('com_jtca.lcp06', $table, $is_new));

		$this->setState('lcp06.id', $table->id);
