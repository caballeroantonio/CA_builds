...
/**
 * This models supports retrieving lists of libreta de libertades juzgados de adolescentes.
 *
 */
class JtCaModelLcp34s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp34s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de libertades juzgados de adolescentes has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp34_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp34_id');
		$this->setState('filter.lcp34_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp34_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp34_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp34_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp34_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp34_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp34_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp34_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp34_id'));
		$id .= ':'.$this->getState('filter.lcp34_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp34s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp34s').' AS a');
		...
		// Filter by a single or group of lcp34s.
		$lcp34_id = $this->getState('filter.lcp34_id');
		if ($lcp34_id != '')
		{
			if (is_numeric($lcp34_id))
			{
				$type = $this->getState('filter.lcp34_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp34_id);
			}
			else
			{
				if (is_array($lcp34_id))
				{
					JArrayHelper::toInteger($lcp34_id);
					$lcp34_id = implode(',', $lcp34_id);
					$type = $this->getState('filter.lcp34_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp34_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp34_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp34_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de libertades juzgados de adolescentes.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp34_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp34 params override menu item params only if menu param = 'use_lcp34'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp34' and there is no lcp34 param, use global

					// create an array of just the params set to 'use_lcp34'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp34')
						{
							// if the lcp34 has a value, use it
							if ($lcp34_params->get($key) != '')
							{
								// get the value from the lcp34
								$lcp34_array[$key] = $lcp34_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp34_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp34 params
					if (count($lcp34_array) > 0)
					{
						$lcp34_params = new Registry;
						$lcp34_params->loadArray($lcp34_array);
						$item->params->merge($lcp34_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp34_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp34, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
