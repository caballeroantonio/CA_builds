...
/**
 * This models supports retrieving lists of jc02.
 *
 */
class JtCaModelLjc02s extends JModelList
{
	 ...
	 */
	protected $context = 'com_jtca.ljc02s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de ingresos de valores has been specified either as a parameter or in the url Request
		$pk = $params->get('ljc02_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc02_id');
		$this->setState('filter.ljc02_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc02_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc02_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc02_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc02_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc02_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc02_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc02_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc02_id'));
		$id .= ':'.$this->getState('filter.ljc02_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc02s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc02s').' AS a');
		...
		// Filter by a single or group of ljc02s.
		$ljc02_id = $this->getState('filter.ljc02_id');
		if ($ljc02_id != '')
		{
			if (is_numeric($ljc02_id))
			{
				$type = $this->getState('filter.ljc02_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc02_id);
			}
			else
			{
				if (is_array($ljc02_id))
				{
					JArrayHelper::toInteger($ljc02_id);
					$ljc02_id = implode(',', $ljc02_id);
					$type = $this->getState('filter.ljc02_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc02_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc02_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc02_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc02_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc02 params override menu item params only if menu param = 'use_ljc02'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc02' and there is no ljc02 param, use global

					// create an array of just the params set to 'use_ljc02'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc02')
						{
							// if the ljc02 has a value, use it
							if ($ljc02_params->get($key) != '')
							{
								// get the value from the ljc02
								$ljc02_array[$key] = $ljc02_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc02_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc02 params
					if (count($ljc02_array) > 0)
					{
						$ljc02_params = new Registry;
						$ljc02_params->loadArray($ljc02_array);
						$item->params->merge($ljc02_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc02_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc02, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
