...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadg17.php';

class JtCaModelLjjadg17Form extends JtCaModelLjjadg17
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadg17';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadg17';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadg17', 'ljjadg17', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadg17.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadg17.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadg17.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjjadg17BeforeSave', array('com_jtca.ljjadg17', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadg17AfterSave', array('com_jtca.ljjadg17', $table, $is_new));

		$this->setState('ljjadg17.id', $table->id);
