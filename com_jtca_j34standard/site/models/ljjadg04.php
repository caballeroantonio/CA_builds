...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE EGRESOS Model
 *
 */
class JtCaModelLjjadg04 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljjadg04';
	...
	{
		if (empty($config['ljjadg04_filter_fields']))
		{
			$config['ljjadg04_filter_fields'] = array(
				'id', 'a.id',
				'billete', 'a.billete',
	...
	*/
	public function getTable($type = 'Ljjadg04s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE EGRESOS data.
	 *
	 * @param	integer	$pk	The id of the libro de egresos.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljjadg04.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJJADG04S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjjadg04ChangeState', array('com_jtca.ljjadg04', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjjadg04BeforeDelete', array('com_jtca.ljjadg04', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjjadg04AfterDelete', array('com_jtca.ljjadg04', &$table));
				}
				else
				{
					// Prune items that you can't change.
