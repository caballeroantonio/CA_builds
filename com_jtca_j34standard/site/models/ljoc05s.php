...
class JtcaModelLjoc05s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljoc05_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljoc05_id');
		$this->setState('filter.ljoc05_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljoc05_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljoc05_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljoc05_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljoc05_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljoc05_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljoc05_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljoc05_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljoc05_id'));
		$id .= ':'.$this->getState('filter.ljoc05_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljoc05s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljoc05s').' AS a');
		...
		// Filter by a single or group of ljoc05s.
		$ljoc05_id = $this->getState('filter.ljoc05_id');
		if ($ljoc05_id != '')
		{
			if (is_numeric($ljoc05_id))
			{
				$type = $this->getState('filter.ljoc05_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljoc05_id);
			}
			else
			{
				if (is_array($ljoc05_id))
				{
					JArrayHelper::toInteger($ljoc05_id);
					$ljoc05_id = implode(',', $ljoc05_id);
					$type = $this->getState('filter.ljoc05_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljoc05_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljoc05_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljoc05_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljoc05_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljoc05 params override menu item params only if menu param = 'use_ljoc05'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljoc05' and there is no ljoc05 param, use global

					// create an array of just the params set to 'use_ljoc05'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljoc05')
						{
							// if the ljoc05 has a value, use it
							if ($ljoc05_params->get($key) != '')
							{
								// get the value from the ljoc05
								$ljoc05_array[$key] = $ljoc05_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljoc05_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljoc05 params
					if (count($ljoc05_array) > 0)
					{
						$ljoc05_params = new Registry;
						$ljoc05_params->loadArray($ljoc05_array);
						$item->params->merge($ljoc05_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljoc05_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljoc05, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
