...
class JtCaModelLcp41 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp41';
	...
	{
		if (empty($config['lcp41_filter_fields']))
		{
	...
	*/
	...
	/**
	 * Method to get LIBRO DE TURNO DE REMISIONES SIN DETENIDO DE JUZGADOS ADOLESCENTES data.
	 *
	 * @param	integer	$pk	The id of the libro de turno de remisiones sin detenido de juzgados adolescentes.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp41.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP41S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp41ChangeState', array('com_jtca.lcp41', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp41BeforeDelete', array('com_jtca.lcp41', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp41AfterDelete', array('com_jtca.lcp41', &$table));
				}
				else
				{
					// Prune items that you can't change.
