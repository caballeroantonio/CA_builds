 ...
 *
 */
class JtcaModelSldilJccm09 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.sldiljccm09';
	...
	{
		if (empty($config['sldiljccm09_filter_fields']))
		{
			$config['sldiljccm09_filter_fields'] = array(
				'id', 'a.id',
				...
				'id_field','a.id_field',
				'sfield5','a.sfield5',
				'sfield8_isMoral','a.sfield8_isMoral',
				'sfield8_paterno','a.sfield8_paterno',
				'sfield8_materno','a.sfield8_materno',
				'sfield8_nombre','a.sfield8_nombre',
				'sfield9','a.sfield9',
				'sfield10','a.sfield10',
				'sfield11','a.sfield11',
				'state', 'a.state',
	...
	protected function populateState()
	{
		...
		$this->setState('sldiljccm09.id', $pk);
	...
	*/
	public function getTable($type = 'SldilJccm09s', $prefix = 'JtcaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('sldiljccm09.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_SLDIL_JCCM09S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onSldilJccm09ChangeState', array('com_jtca.sldiljccm09', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onSldilJccm09BeforeDelete', array('com_jtca.sldiljccm09', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onSldilJccm09AfterDelete', array('com_jtca.sldiljccm09', &$table));
				}
				else
				{
					// Prune items that you can't change.
