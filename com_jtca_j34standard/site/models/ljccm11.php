 ...
 *
 */
class JtcaModelLjccm11 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljccm11';
	...
	{
		if (empty($config['ljccm11_filter_fields']))
		{
			$config['ljccm11_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljccm11s', $prefix = 'JtcaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljccm11.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJCCM11S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjccm11ChangeState', array('com_jtca.ljccm11', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjccm11BeforeDelete', array('com_jtca.ljccm11', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjccm11AfterDelete', array('com_jtca.ljccm11', &$table));
				}
				else
				{
					// Prune items that you can't change.
