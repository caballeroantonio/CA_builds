...
/**
 * This models supports retrieving lists of libro de control de adopciones internacionales.
 *
 */
class JtCaModelLjomf21s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljomf21s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de control de adopciones internacionales has been specified either as a parameter or in the url Request
		$pk = $params->get('ljomf21_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf21_id');
		$this->setState('filter.ljomf21_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf21_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf21_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf21_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf21_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf21_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf21_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf21_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf21_id'));
		$id .= ':'.$this->getState('filter.ljomf21_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf21s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf21s').' AS a');
		...
		// Filter by a single or group of ljomf21s.
		$ljomf21_id = $this->getState('filter.ljomf21_id');
		if ($ljomf21_id != '')
		{
			if (is_numeric($ljomf21_id))
			{
				$type = $this->getState('filter.ljomf21_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf21_id);
			}
			else
			{
				if (is_array($ljomf21_id))
				{
					JArrayHelper::toInteger($ljomf21_id);
					$ljomf21_id = implode(',', $ljomf21_id);
					$type = $this->getState('filter.ljomf21_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf21_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf21_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf21_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de control de adopciones internacionales.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf21_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf21 params override menu item params only if menu param = 'use_ljomf21'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf21' and there is no ljomf21 param, use global

					// create an array of just the params set to 'use_ljomf21'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf21_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf21')
						{
							// if the ljomf21 has a value, use it
							if ($ljomf21_params->get($key) != '')
							{
								// get the value from the ljomf21
								$ljomf21_array[$key] = $ljomf21_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf21_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf21 params
					if (count($ljomf21_array) > 0)
					{
						$ljomf21_params = new Registry;
						$ljomf21_params->loadArray($ljomf21_array);
						$item->params->merge($ljomf21_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf21_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf21, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
