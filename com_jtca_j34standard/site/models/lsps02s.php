...
/**
 * This models supports retrieving lists of libro de gobierno (nuevo sistema) (unitario).
 *
 */
class JtCaModelLsps02s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsps02s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de gobierno (nuevo sistema) (unitario) has been specified either as a parameter or in the url Request
		$pk = $params->get('lsps02_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsps02_id');
		$this->setState('filter.lsps02_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsps02_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsps02_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsps02_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsps02_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsps02_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsps02_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsps02_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsps02_id'));
		$id .= ':'.$this->getState('filter.lsps02_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsps02s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsps02s').' AS a');
		...
		// Filter by a single or group of lsps02s.
		$lsps02_id = $this->getState('filter.lsps02_id');
		if ($lsps02_id != '')
		{
			if (is_numeric($lsps02_id))
			{
				$type = $this->getState('filter.lsps02_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsps02_id);
			}
			else
			{
				if (is_array($lsps02_id))
				{
					JArrayHelper::toInteger($lsps02_id);
					$lsps02_id = implode(',', $lsps02_id);
					$type = $this->getState('filter.lsps02_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsps02_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsps02_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsps02_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de gobierno (nuevo sistema) (unitario).
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsps02_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsps02 params override menu item params only if menu param = 'use_lsps02'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsps02' and there is no lsps02 param, use global

					// create an array of just the params set to 'use_lsps02'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsps02')
						{
							// if the lsps02 has a value, use it
							if ($lsps02_params->get($key) != '')
							{
								// get the value from the lsps02
								$lsps02_array[$key] = $lsps02_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsps02_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsps02 params
					if (count($lsps02_array) > 0)
					{
						$lsps02_params = new Registry;
						$lsps02_params->loadArray($lsps02_array);
						$item->params->merge($lsps02_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsps02_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsps02, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
