...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadg03.php';

class JtcaModelLjjadg03Form extends JtcaModelLjjadg03
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadg03';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadg03';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadg03', 'ljjadg03', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de ingresos.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE INGRESOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadg03.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadg03.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadg03.id');
		$is_new		= true;
		...
		// Reorder the libro de ingresos so the new libro de ingresos is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadg03BeforeSave', array('com_jtca.ljjadg03', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadg03AfterSave', array('com_jtca.ljjadg03', $table, $is_new));

		$this->setState('ljjadg03.id', $table->id);
