...
/**
 * This models supports retrieving lists of libro de turno para consignaciones con detenido de.
 *
 */
class JtCaModelLcp25s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp25s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno para consignaciones con detenido delitos no graves has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp25_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp25_id');
		$this->setState('filter.lcp25_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp25_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp25_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp25_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp25_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp25_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp25_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp25_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp25_id'));
		$id .= ':'.$this->getState('filter.lcp25_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp25s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp25s').' AS a');
		...
		// Filter by a single or group of lcp25s.
		$lcp25_id = $this->getState('filter.lcp25_id');
		if ($lcp25_id != '')
		{
			if (is_numeric($lcp25_id))
			{
				$type = $this->getState('filter.lcp25_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp25_id);
			}
			else
			{
				if (is_array($lcp25_id))
				{
					JArrayHelper::toInteger($lcp25_id);
					$lcp25_id = implode(',', $lcp25_id);
					$type = $this->getState('filter.lcp25_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp25_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp25_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp25_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno para consignaciones con detenido de.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp25_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp25 params override menu item params only if menu param = 'use_lcp25'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp25' and there is no lcp25 param, use global

					// create an array of just the params set to 'use_lcp25'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp25')
						{
							// if the lcp25 has a value, use it
							if ($lcp25_params->get($key) != '')
							{
								// get the value from the lcp25
								$lcp25_array[$key] = $lcp25_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp25_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp25 params
					if (count($lcp25_array) > 0)
					{
						$lcp25_params = new Registry;
						$lcp25_params->loadArray($lcp25_array);
						$item->params->merge($lcp25_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp25_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp25, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
