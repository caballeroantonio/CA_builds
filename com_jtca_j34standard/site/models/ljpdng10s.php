...
class JtcaModelLjpdng10s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpdng10_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng10_id');
		$this->setState('filter.ljpdng10_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng10_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng10_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng10_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng10_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng10_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng10_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng10_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng10_id'));
		$id .= ':'.$this->getState('filter.ljpdng10_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng10s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng10s').' AS a');
		...
		// Filter by a single or group of ljpdng10s.
		$ljpdng10_id = $this->getState('filter.ljpdng10_id');
		if ($ljpdng10_id != '')
		{
			if (is_numeric($ljpdng10_id))
			{
				$type = $this->getState('filter.ljpdng10_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng10_id);
			}
			else
			{
				if (is_array($ljpdng10_id))
				{
					JArrayHelper::toInteger($ljpdng10_id);
					$ljpdng10_id = implode(',', $ljpdng10_id);
					$type = $this->getState('filter.ljpdng10_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng10_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng10_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng10_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng10_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng10 params override menu item params only if menu param = 'use_ljpdng10'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng10' and there is no ljpdng10 param, use global

					// create an array of just the params set to 'use_ljpdng10'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng10_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng10')
						{
							// if the ljpdng10 has a value, use it
							if ($ljpdng10_params->get($key) != '')
							{
								// get the value from the ljpdng10
								$ljpdng10_array[$key] = $ljpdng10_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng10_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng10 params
					if (count($ljpdng10_array) > 0)
					{
						$ljpdng10_params = new Registry;
						$ljpdng10_params->loadArray($ljpdng10_array);
						$item->params->merge($ljpdng10_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng10_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng10, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
