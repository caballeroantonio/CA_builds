...
class JtCaModelLjpes08s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpes08_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes08_id');
		$this->setState('filter.ljpes08_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes08_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes08_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes08_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes08_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes08_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes08_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes08_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes08_id'));
		$id .= ':'.$this->getState('filter.ljpes08_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes08s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes08s').' AS a');
		...
		// Filter by a single or group of ljpes08s.
		$ljpes08_id = $this->getState('filter.ljpes08_id');
		if ($ljpes08_id != '')
		{
			if (is_numeric($ljpes08_id))
			{
				$type = $this->getState('filter.ljpes08_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes08_id);
			}
			else
			{
				if (is_array($ljpes08_id))
				{
					JArrayHelper::toInteger($ljpes08_id);
					$ljpes08_id = implode(',', $ljpes08_id);
					$type = $this->getState('filter.ljpes08_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes08_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes08_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes08_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes08_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes08 params override menu item params only if menu param = 'use_ljpes08'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes08' and there is no ljpes08 param, use global

					// create an array of just the params set to 'use_ljpes08'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes08_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes08')
						{
							// if the ljpes08 has a value, use it
							if ($ljpes08_params->get($key) != '')
							{
								// get the value from the ljpes08
								$ljpes08_array[$key] = $ljpes08_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes08_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes08 params
					if (count($ljpes08_array) > 0)
					{
						$ljpes08_params = new Registry;
						$ljpes08_params->loadArray($ljpes08_array);
						$item->params->merge($ljpes08_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes08_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes08, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
