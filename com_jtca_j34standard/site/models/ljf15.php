 ...
 *
 */
class JtCaModelLjf15 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljf15';
	...
	{
		if (empty($config['ljf15_filter_fields']))
		{
			$config['ljf15_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljf15s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljf15.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJF15S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjf15ChangeState', array('com_jtca.ljf15', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjf15BeforeDelete', array('com_jtca.ljf15', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjf15AfterDelete', array('com_jtca.ljf15', &$table));
				}
				else
				{
					// Prune items that you can't change.
