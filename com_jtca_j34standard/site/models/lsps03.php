...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE GOBIERNO (NUEVO SISTEMA) (COLEGIADO) Model
 *
 */
class JtCaModelLsps03 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsps03';
	...
	{
		if (empty($config['lsps03_filter_fields']))
		{
			$config['lsps03_filter_fields'] = array(
				'id', 'a.id',
				'id_expediente','a.id_expediente',
				'id_organo','a.id_organo',
				'id_secretaria','a.id_secretaria',
				'anoj','a.anoj',
				'field2370','a.field2370',
				'field2371','a.field2371',
				'field2372','a.field2372',
				'field2373','a.field2373',
				'field2374','a.field2374',
				'field2375','a.field2375',
				'field2376','a.field2376',
				'field2377','a.field2377',
				'field2378','a.field2378',
				'field2379','a.field2379',
				'field2380','a.field2380',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lsps03s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE GOBIERNO (NUEVO SISTEMA) (COLEGIADO) data.
	 *
	 * @param	integer	$pk	The id of the libro de gobierno (nuevo sistema) (colegiado).
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsps03.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPS03S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsps03ChangeState', array('com_jtca.lsps03', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsps03BeforeDelete', array('com_jtca.lsps03', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsps03AfterDelete', array('com_jtca.lsps03', &$table));
				}
				else
				{
					// Prune items that you can't change.
