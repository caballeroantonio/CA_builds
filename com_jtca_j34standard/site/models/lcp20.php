...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE TURNO PARA INCOMPETENCIAS PRIMERA INSTANCIA. Model
 *
 */
class JtCaModelLcp20 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp20';
	...
	{
		if (empty($config['lcp20_filter_fields']))
		{
			$config['lcp20_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp20s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE TURNO PARA INCOMPETENCIAS PRIMERA INSTANCIA. data.
	 *
	 * @param	integer	$pk	The id of the libro de turno para incompetencias primera instancia..
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp20.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP20S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp20ChangeState', array('com_jtca.lcp20', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp20BeforeDelete', array('com_jtca.lcp20', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp20AfterDelete', array('com_jtca.lcp20', &$table));
				}
				else
				{
					// Prune items that you can't change.
