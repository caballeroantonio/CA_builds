...
class JtCaModelLjf17s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf17_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf17_id');
		$this->setState('filter.ljf17_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf17_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf17_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf17_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf17_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf17_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf17_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf17_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf17_id'));
		$id .= ':'.$this->getState('filter.ljf17_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf17s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf17s').' AS a');
		...
		// Filter by a single or group of ljf17s.
		$ljf17_id = $this->getState('filter.ljf17_id');
		if ($ljf17_id != '')
		{
			if (is_numeric($ljf17_id))
			{
				$type = $this->getState('filter.ljf17_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf17_id);
			}
			else
			{
				if (is_array($ljf17_id))
				{
					JArrayHelper::toInteger($ljf17_id);
					$ljf17_id = implode(',', $ljf17_id);
					$type = $this->getState('filter.ljf17_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf17_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf17_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf17_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf17_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf17 params override menu item params only if menu param = 'use_ljf17'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf17' and there is no ljf17 param, use global

					// create an array of just the params set to 'use_ljf17'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf17')
						{
							// if the ljf17 has a value, use it
							if ($ljf17_params->get($key) != '')
							{
								// get the value from the ljf17
								$ljf17_array[$key] = $ljf17_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf17_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf17 params
					if (count($ljf17_array) > 0)
					{
						$ljf17_params = new Registry;
						$ljf17_params->loadArray($ljf17_array);
						$item->params->merge($ljf17_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf17_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf17, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
