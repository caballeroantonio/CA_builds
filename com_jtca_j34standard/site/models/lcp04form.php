...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp04.php';

class JtcaModelLcp04Form extends JtcaModelLcp04
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp04';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp04';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp04', 'lcp04', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno de justicia para adolescentes con detenido de pgr.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO DE JUSTICIA PARA ADOLESCENTES CON DETENIDO DE PGR item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp04.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp04.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp04.id');
		$is_new		= true;
		...
		// Reorder the libro de turno de justicia para adolescentes con d so the new libro de turno de justicia para adolescentes con detenido de pgr is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp04BeforeSave', array('com_jtca.lcp04', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp04AfterSave', array('com_jtca.lcp04', $table, $is_new));

		$this->setState('lcp04.id', $table->id);
