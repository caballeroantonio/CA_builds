...
/**
 * This models supports retrieving lists of libro de notarios.
 *
 */
class JtCaModelLjoc13s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljoc13s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de notarios has been specified either as a parameter or in the url Request
		$pk = $params->get('ljoc13_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljoc13_id');
		$this->setState('filter.ljoc13_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljoc13_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljoc13_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljoc13_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljoc13_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljoc13_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljoc13_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljoc13_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljoc13_id'));
		$id .= ':'.$this->getState('filter.ljoc13_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljoc13s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljoc13s').' AS a');
		...
		// Filter by a single or group of ljoc13s.
		$ljoc13_id = $this->getState('filter.ljoc13_id');
		if ($ljoc13_id != '')
		{
			if (is_numeric($ljoc13_id))
			{
				$type = $this->getState('filter.ljoc13_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljoc13_id);
			}
			else
			{
				if (is_array($ljoc13_id))
				{
					JArrayHelper::toInteger($ljoc13_id);
					$ljoc13_id = implode(',', $ljoc13_id);
					$type = $this->getState('filter.ljoc13_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljoc13_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljoc13_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljoc13_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de notarios.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljoc13_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljoc13 params override menu item params only if menu param = 'use_ljoc13'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljoc13' and there is no ljoc13 param, use global

					// create an array of just the params set to 'use_ljoc13'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljoc13')
						{
							// if the ljoc13 has a value, use it
							if ($ljoc13_params->get($key) != '')
							{
								// get the value from the ljoc13
								$ljoc13_array[$key] = $ljoc13_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljoc13_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljoc13 params
					if (count($ljoc13_array) > 0)
					{
						$ljoc13_params = new Registry;
						$ljoc13_params->loadArray($ljoc13_array);
						$item->params->merge($ljoc13_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljoc13_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljoc13, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
