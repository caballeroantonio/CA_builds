...
class JtCaModelLjomf05s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf05_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf05_id');
		$this->setState('filter.ljomf05_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf05_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf05_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf05_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf05_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf05_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf05_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf05_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf05_id'));
		$id .= ':'.$this->getState('filter.ljomf05_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf05s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf05s').' AS a');
		...
		// Filter by a single or group of ljomf05s.
		$ljomf05_id = $this->getState('filter.ljomf05_id');
		if ($ljomf05_id != '')
		{
			if (is_numeric($ljomf05_id))
			{
				$type = $this->getState('filter.ljomf05_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf05_id);
			}
			else
			{
				if (is_array($ljomf05_id))
				{
					JArrayHelper::toInteger($ljomf05_id);
					$ljomf05_id = implode(',', $ljomf05_id);
					$type = $this->getState('filter.ljomf05_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf05_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf05_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf05_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf05_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf05 params override menu item params only if menu param = 'use_ljomf05'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf05' and there is no ljomf05 param, use global

					// create an array of just the params set to 'use_ljomf05'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf05_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf05')
						{
							// if the ljomf05 has a value, use it
							if ($ljomf05_params->get($key) != '')
							{
								// get the value from the ljomf05
								$ljomf05_array[$key] = $ljomf05_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf05_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf05 params
					if (count($ljomf05_array) > 0)
					{
						$ljomf05_params = new Registry;
						$ljomf05_params->loadArray($ljomf05_array);
						$item->params->merge($ljomf05_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf05_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf05, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
