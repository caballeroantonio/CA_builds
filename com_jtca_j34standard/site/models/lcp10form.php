...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp10.php';

class JtcaModelLcp10Form extends JtcaModelLcp10
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp10';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp10';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp10', 'lcp10', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno de beneficios penitenciarios..
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO DE BENEFICIOS PENITENCIARIOS. item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp10.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp10.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp10.id');
		$is_new		= true;
		...
		// Reorder the libro de turno de beneficios penitenciarios. so the new libro de turno de beneficios penitenciarios. is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp10BeforeSave', array('com_jtca.lcp10', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp10AfterSave', array('com_jtca.lcp10', $table, $is_new));

		$this->setState('lcp10.id', $table->id);
