...
/**
 * This models supports retrieving lists of partes contenciosas en juicio.
 *
 */
class JtcaModelSlpartescontenciosas extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.slpartescontenciosas';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single partes contenciosas en juicio has been specified either as a parameter or in the url Request
		$pk = $params->get('slpartecontenciosa_id', '') == '' ? $app->input->getInt('id', '') : $params->get('slpartecontenciosa_id');
		$this->setState('filter.slpartecontenciosa_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('slpartecontenciosa_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('slpartecontenciosa_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('slpartecontenciosa_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('slpartecontenciosa_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('slpartecontenciosa_initial_direction','ASC');
		}
		...
		if ($params->get('filter_slpartecontenciosa_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_slpartecontenciosa_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.slpartecontenciosa_id'));
		$id .= ':'.$this->getState('filter.slpartecontenciosa_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of slpartescontenciosas subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_slpartescontenciosas').' AS a');
		...
		// Filter by a single or group of slpartescontenciosas.
		$slpartecontenciosa_id = $this->getState('filter.slpartecontenciosa_id');
		if ($slpartecontenciosa_id != '')
		{
			if (is_numeric($slpartecontenciosa_id))
			{
				$type = $this->getState('filter.slpartecontenciosa_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $slpartecontenciosa_id);
			}
			else
			{
				if (is_array($slpartecontenciosa_id))
				{
					JArrayHelper::toInteger($slpartecontenciosa_id);
					$slpartecontenciosa_id = implode(',', $slpartecontenciosa_id);
					$type = $this->getState('filter.slpartecontenciosa_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$slpartecontenciosa_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_slpartecontenciosa_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_slpartecontenciosa_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of partes contenciosas en juicio.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$slpartecontenciosa_params = new Registry;
				...
				{
					$item->params = $slpartecontenciosa_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Slpartecontenciosa params override menu item params only if menu param = 'use_slpartecontenciosa'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_slpartecontenciosa' and there is no slpartecontenciosa param, use global

					// create an array of just the params set to 'use_slpartecontenciosa'
					$menu_params_array = $this->getState('params')->toArray();
					$slpartecontenciosa_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_slpartecontenciosa')
						{
							// if the slpartecontenciosa has a value, use it
							if ($slpartecontenciosa_params->get($key) != '')
							{
								// get the value from the slpartecontenciosa
								$slpartecontenciosa_array[$key] = $slpartecontenciosa_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$slpartecontenciosa_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected slpartecontenciosa params
					if (count($slpartecontenciosa_array) > 0)
					{
						$slpartecontenciosa_params = new Registry;
						$slpartecontenciosa_params->loadArray($slpartecontenciosa_array);
						$item->params->merge($slpartecontenciosa_params);
					}


					// get display date
					switch ($item->params->get('list_show_slpartecontenciosa_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an slpartecontenciosa, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
