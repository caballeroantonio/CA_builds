...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE PÓLIZAS Y FIANZAS (OFICIAL) - Egresos Model
 *
 */
class JtCaModelLsc14 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsc14';
	...
	{
		if (empty($config['lsc14_filter_fields']))
		{
			$config['lsc14_filter_fields'] = array(
				'id', 'a.id',
				'billete', 'a.billete',
	...
	*/
	public function getTable($type = 'Lsc14s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE PÓLIZAS Y FIANZAS (OFICIAL) - Egresos data.
	 *
	 * @param	integer	$pk	The id of the libro de pólizas y fianzas (oficial) - egresos.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsc14.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSC14S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsc14ChangeState', array('com_jtca.lsc14', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsc14BeforeDelete', array('com_jtca.lsc14', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsc14AfterDelete', array('com_jtca.lsc14', &$table));
				}
				else
				{
					// Prune items that you can't change.
