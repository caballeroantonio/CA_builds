...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/slpersonaliasedad.php';

class JtcaModelSlpersonaliasedadForm extends JtcaModelSlpersonaliasedad
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.slpersonaliasedad';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.slpersonaliasedad';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.slpersonaliasedad', 'slpersonaliasedad', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the adolescentes.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Adolescentes item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('slpersonaliasedad.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.slpersonaliasedad.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('slpersonaliasedad.id');
		$is_new		= true;
		...
		// Reorder the adolescentes so the new adolescentes is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSlpersonaliasedadBeforeSave', array('com_jtca.slpersonaliasedad', &$table, $is_new));
		...
		$dispatcher->trigger('onSlpersonaliasedadAfterSave', array('com_jtca.slpersonaliasedad', $table, $is_new));

		$this->setState('slpersonaliasedad.id', $table->id);
