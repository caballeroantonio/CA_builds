 ...
 *
 */
class JtCaModelLjemst03 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljemst03';
	...
	{
		if (empty($config['ljemst03_filter_fields']))
		{
			$config['ljemst03_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljemst03s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljemst03.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJEMST03S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjemst03ChangeState', array('com_jtca.ljemst03', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjemst03BeforeDelete', array('com_jtca.ljemst03', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjemst03AfterDelete', array('com_jtca.ljemst03', &$table));
				}
				else
				{
					// Prune items that you can't change.
