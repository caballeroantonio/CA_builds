...
/**
 * This models supports retrieving lists of libro de turno de justicia para adolescentes sin d.
 *
 */
class JtCaModelLcp05s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp05s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno de justicia para adolescentes sin detenido de pgr has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp05_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp05_id');
		$this->setState('filter.lcp05_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp05_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp05_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp05_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp05_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp05_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp05_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp05_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp05_id'));
		$id .= ':'.$this->getState('filter.lcp05_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp05s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp05s').' AS a');
		...
		// Filter by a single or group of lcp05s.
		$lcp05_id = $this->getState('filter.lcp05_id');
		if ($lcp05_id != '')
		{
			if (is_numeric($lcp05_id))
			{
				$type = $this->getState('filter.lcp05_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp05_id);
			}
			else
			{
				if (is_array($lcp05_id))
				{
					JArrayHelper::toInteger($lcp05_id);
					$lcp05_id = implode(',', $lcp05_id);
					$type = $this->getState('filter.lcp05_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp05_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp05_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp05_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno de justicia para adolescentes sin d.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp05_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp05 params override menu item params only if menu param = 'use_lcp05'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp05' and there is no lcp05 param, use global

					// create an array of just the params set to 'use_lcp05'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp05')
						{
							// if the lcp05 has a value, use it
							if ($lcp05_params->get($key) != '')
							{
								// get the value from the lcp05
								$lcp05_array[$key] = $lcp05_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp05_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp05 params
					if (count($lcp05_array) > 0)
					{
						$lcp05_params = new Registry;
						$lcp05_params->loadArray($lcp05_array);
						$item->params->merge($lcp05_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp05_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp05, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
