...
class JtCaModelLjp08s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljp08_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp08_id');
		$this->setState('filter.ljp08_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp08_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp08_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp08_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp08_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp08_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp08_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp08_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp08_id'));
		$id .= ':'.$this->getState('filter.ljp08_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp08s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp08s').' AS a');
		...
		// Filter by a single or group of ljp08s.
		$ljp08_id = $this->getState('filter.ljp08_id');
		if ($ljp08_id != '')
		{
			if (is_numeric($ljp08_id))
			{
				$type = $this->getState('filter.ljp08_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp08_id);
			}
			else
			{
				if (is_array($ljp08_id))
				{
					JArrayHelper::toInteger($ljp08_id);
					$ljp08_id = implode(',', $ljp08_id);
					$type = $this->getState('filter.ljp08_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp08_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp08_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp08_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp08_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp08 params override menu item params only if menu param = 'use_ljp08'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp08' and there is no ljp08 param, use global

					// create an array of just the params set to 'use_ljp08'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp08')
						{
							// if the ljp08 has a value, use it
							if ($ljp08_params->get($key) != '')
							{
								// get the value from the ljp08
								$ljp08_array[$key] = $ljp08_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp08_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp08 params
					if (count($ljp08_array) > 0)
					{
						$ljp08_params = new Registry;
						$ljp08_params->loadArray($ljp08_array);
						$item->params->merge($ljp08_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp08_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp08, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
