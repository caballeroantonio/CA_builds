...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpes06.php';

class JtCaModelLjpes06Form extends JtCaModelLjpes06
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpes06';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpes06';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpes06', 'ljpes06', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpes06.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpes06.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpes06.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjpes06BeforeSave', array('com_jtca.ljpes06', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpes06AfterSave', array('com_jtca.ljpes06', $table, $is_new));

		$this->setState('ljpes06.id', $table->id);
