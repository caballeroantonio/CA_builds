...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsps02.php';

class JtcaModelLsps02Form extends JtcaModelLsps02
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsps02';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsps02';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsps02', 'lsps02', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de gobierno (nuevo sistema) (unitario).
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE GOBIERNO (NUEVO SISTEMA) (UNITARIO) item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsps02.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsps02.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsps02.id');
		$is_new		= true;
		...
		// Reorder the libro de gobierno (nuevo sistema) (unitario) so the new libro de gobierno (nuevo sistema) (unitario) is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsps02BeforeSave', array('com_jtca.lsps02', &$table, $is_new));
		...
		$dispatcher->trigger('onLsps02AfterSave', array('com_jtca.lsps02', $table, $is_new));

		$this->setState('lsps02.id', $table->id);
