...
class JtCaModelLejemplos extends JModelList
{
	 ...
	 */
	protected $context = 'com_jtca.lejemplos';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de ejemplo has been specified either as a parameter or in the url Request
		$pk = $params->get('lejemplo_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lejemplo_id');
		$this->setState('filter.lejemplo_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lejemplo_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lejemplo_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lejemplo_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lejemplo_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lejemplo_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lejemplo_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lejemplo_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lejemplo_id'));
		$id .= ':'.$this->getState('filter.lejemplo_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lejemplos subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$lejemplo_id = $this->getState('filter.lejemplo_id');
		if ($lejemplo_id != '')
		{
			if (is_numeric($lejemplo_id))
			{
				$type = $this->getState('filter.lejemplo_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lejemplo_id);
			}
			else
			{
				if (is_array($lejemplo_id))
				{
					JArrayHelper::toInteger($lejemplo_id);
					$lejemplo_id = implode(',', $lejemplo_id);
					$type = $this->getState('filter.lejemplo_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lejemplo_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lejemplo_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lejemplo_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lejemplo_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lejemplo params override menu item params only if menu param = 'use_lejemplo'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lejemplo' and there is no lejemplo param, use global

					// create an array of just the params set to 'use_lejemplo'
					$menu_params_array = $this->getState('params')->toArray();
					$lejemplo_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lejemplo')
						{
							// if the lejemplo has a value, use it
							if ($lejemplo_params->get($key) != '')
							{
								// get the value from the lejemplo
								$lejemplo_array[$key] = $lejemplo_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lejemplo_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lejemplo params
					if (count($lejemplo_array) > 0)
					{
						$lejemplo_params = new Registry;
						$lejemplo_params->loadArray($lejemplo_array);
						$item->params->merge($lejemplo_params);
					}


					// get display date
					switch ($item->params->get('list_show_lejemplo_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lejemplo, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
