...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadng05.php';

class JtCaModelLjjadng05Form extends JtCaModelLjjadng05
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadng05';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadng05';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadng05', 'ljjadng05', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadng05.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadng05.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadng05.id');
		$is_new		= true;
		...
		// Reorder the libro de recursos de apelación so the new libro de recursos de apelación is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadng05BeforeSave', array('com_jtca.ljjadng05', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadng05AfterSave', array('com_jtca.ljjadng05', $table, $is_new));

		$this->setState('ljjadng05.id', $table->id);
