...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE ORDENES PARA ARRAIGOS Y CATEOS DELITOS NO GRAVES Model
 *
 */
class JtCaModelLcp29 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp29';
	...
	{
		if (empty($config['lcp29_filter_fields']))
		{
			$config['lcp29_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp29s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE ORDENES PARA ARRAIGOS Y CATEOS DELITOS NO GRAVES data.
	 *
	 * @param	integer	$pk	The id of the libro de ordenes para arraigos y cateos delitos no graves.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp29.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP29S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp29ChangeState', array('com_jtca.lcp29', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp29BeforeDelete', array('com_jtca.lcp29', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp29AfterDelete', array('com_jtca.lcp29', &$table));
				}
				else
				{
					// Prune items that you can't change.
