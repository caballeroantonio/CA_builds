...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE CONTROL DE FIANZA Model
 *
 */
class JtcaModelLsps11 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsps11';
	...
	{
		if (empty($config['lsps11_filter_fields']))
		{
			$config['lsps11_filter_fields'] = array(
				'id', 'a.id',
				'id_organo','a.id_organo',
				'id_secretaria','a.id_secretaria',
				'anoj','a.anoj',
				'id_expediente','a.id_expediente',
				'field2433_isMoral','a.field2433_isMoral',
				'field2433_paterno','a.field2433_paterno',
				'field2433_materno','a.field2433_materno',
				'field2433_nombre','a.field2433_nombre',
				'field2434','a.field2434',
				'field2435','a.field2435',
				'field2436','a.field2436',
				'field2437','a.field2437',
				'field2438','a.field2438',
				'field2439','a.field2439',
				'field2440_isMoral','a.field2440_isMoral',
				'field2440_paterno','a.field2440_paterno',
				'field2440_materno','a.field2440_materno',
				'field2440_nombre','a.field2440_nombre',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lsps11s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE CONTROL DE FIANZA data.
	 *
	 * @param	integer	$pk	The id of the libro de control de fianza.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsps11.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPS11S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsps11ChangeState', array('com_jtca.lsps11', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsps11BeforeDelete', array('com_jtca.lsps11', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsps11AfterDelete', array('com_jtca.lsps11', &$table));
				}
				else
				{
					// Prune items that you can't change.
