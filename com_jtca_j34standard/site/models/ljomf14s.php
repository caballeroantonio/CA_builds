...
class JtCaModelLjomf14s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf14_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf14_id');
		$this->setState('filter.ljomf14_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf14_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf14_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf14_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf14_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf14_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf14_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf14_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf14_id'));
		$id .= ':'.$this->getState('filter.ljomf14_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf14s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf14s').' AS a');
		...
		// Filter by a single or group of ljomf14s.
		$ljomf14_id = $this->getState('filter.ljomf14_id');
		if ($ljomf14_id != '')
		{
			if (is_numeric($ljomf14_id))
			{
				$type = $this->getState('filter.ljomf14_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf14_id);
			}
			else
			{
				if (is_array($ljomf14_id))
				{
					JArrayHelper::toInteger($ljomf14_id);
					$ljomf14_id = implode(',', $ljomf14_id);
					$type = $this->getState('filter.ljomf14_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf14_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf14_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf14_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf14_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf14 params override menu item params only if menu param = 'use_ljomf14'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf14' and there is no ljomf14 param, use global

					// create an array of just the params set to 'use_ljomf14'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf14_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf14')
						{
							// if the ljomf14 has a value, use it
							if ($ljomf14_params->get($key) != '')
							{
								// get the value from the ljomf14
								$ljomf14_array[$key] = $ljomf14_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf14_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf14 params
					if (count($ljomf14_array) > 0)
					{
						$ljomf14_params = new Registry;
						$ljomf14_params->loadArray($ljomf14_array);
						$item->params->merge($ljomf14_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf14_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf14, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
