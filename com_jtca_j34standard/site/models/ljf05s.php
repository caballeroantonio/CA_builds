...
class JtCaModelLjf05s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf05_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf05_id');
		$this->setState('filter.ljf05_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf05_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf05_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf05_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf05_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf05_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf05_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf05_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf05_id'));
		$id .= ':'.$this->getState('filter.ljf05_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf05s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf05s').' AS a');
		...
		// Filter by a single or group of ljf05s.
		$ljf05_id = $this->getState('filter.ljf05_id');
		if ($ljf05_id != '')
		{
			if (is_numeric($ljf05_id))
			{
				$type = $this->getState('filter.ljf05_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf05_id);
			}
			else
			{
				if (is_array($ljf05_id))
				{
					JArrayHelper::toInteger($ljf05_id);
					$ljf05_id = implode(',', $ljf05_id);
					$type = $this->getState('filter.ljf05_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf05_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf05_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf05_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf05_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf05 params override menu item params only if menu param = 'use_ljf05'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf05' and there is no ljf05 param, use global

					// create an array of just the params set to 'use_ljf05'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf05')
						{
							// if the ljf05 has a value, use it
							if ($ljf05_params->get($key) != '')
							{
								// get the value from the ljf05
								$ljf05_array[$key] = $ljf05_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf05_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf05 params
					if (count($ljf05_array) > 0)
					{
						$ljf05_params = new Registry;
						$ljf05_params->loadArray($ljf05_array);
						$item->params->merge($ljf05_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf05_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf05, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
