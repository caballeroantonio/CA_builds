...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp19.php';

class JtCaModelLcp19Form extends JtCaModelLcp19
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp19';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp19';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp19', 'lcp19', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno para consignaciones sin detenido primera instancia..
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO PARA CONSIGNACIONES SIN DETENIDO PRIMERA INSTANCIA. item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp19.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp19.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp19.id');
		$is_new		= true;
		...
		// Reorder the libro de turno para consignaciones sin detenido pr so the new libro de turno para consignaciones sin detenido primera instancia. is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp19BeforeSave', array('com_jtca.lcp19', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp19AfterSave', array('com_jtca.lcp19', $table, $is_new));

		$this->setState('lcp19.id', $table->id);
