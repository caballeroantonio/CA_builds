...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpdng18.php';

class JtCaModelLjpdng18Form extends JtCaModelLjpdng18
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpdng18';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpdng18';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpdng18', 'ljpdng18', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpdng18.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpdng18.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpdng18.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjpdng18BeforeSave', array('com_jtca.ljpdng18', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpdng18AfterSave', array('com_jtca.ljpdng18', $table, $is_new));

		$this->setState('ljpdng18.id', $table->id);
