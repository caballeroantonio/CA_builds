...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljc08.php';

class JtCaModelLjc08Form extends JtCaModelLjc08
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljc08';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljc08';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljc08', 'ljc08', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljc08.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljc08.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljc08.id');
		$is_new		= true;
		...
		// Reorder the jc08 so the new libro de oficios is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjc08BeforeSave', array('com_jtca.ljc08', &$table, $is_new));
		...
		$dispatcher->trigger('onLjc08AfterSave', array('com_jtca.ljc08', $table, $is_new));

		$this->setState('ljc08.id', $table->id);
