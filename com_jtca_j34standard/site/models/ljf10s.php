...
class JtCaModelLjf10s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf10_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf10_id');
		$this->setState('filter.ljf10_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf10_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf10_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf10_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf10_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf10_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf10_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf10_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf10_id'));
		$id .= ':'.$this->getState('filter.ljf10_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf10s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf10s').' AS a');
		...
		// Filter by a single or group of ljf10s.
		$ljf10_id = $this->getState('filter.ljf10_id');
		if ($ljf10_id != '')
		{
			if (is_numeric($ljf10_id))
			{
				$type = $this->getState('filter.ljf10_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf10_id);
			}
			else
			{
				if (is_array($ljf10_id))
				{
					JArrayHelper::toInteger($ljf10_id);
					$ljf10_id = implode(',', $ljf10_id);
					$type = $this->getState('filter.ljf10_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf10_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf10_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf10_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf10_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf10 params override menu item params only if menu param = 'use_ljf10'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf10' and there is no ljf10 param, use global

					// create an array of just the params set to 'use_ljf10'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf10')
						{
							// if the ljf10 has a value, use it
							if ($ljf10_params->get($key) != '')
							{
								// get the value from the ljf10
								$ljf10_array[$key] = $ljf10_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf10_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf10 params
					if (count($ljf10_array) > 0)
					{
						$ljf10_params = new Registry;
						$ljf10_params->loadArray($ljf10_array);
						$item->params->merge($ljf10_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf10_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf10, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
