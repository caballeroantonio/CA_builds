...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadg02.php';

class JtCaModelLjjadg02Form extends JtCaModelLjjadg02
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadg02';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadg02';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadg02', 'ljjadg02', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadg02.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadg02.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadg02.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjjadg02BeforeSave', array('com_jtca.ljjadg02', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadg02AfterSave', array('com_jtca.ljjadg02', $table, $is_new));

		$this->setState('ljjadg02.id', $table->id);
