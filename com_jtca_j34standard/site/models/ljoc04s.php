...
class JtcaModelLjoc04s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljoc04_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljoc04_id');
		$this->setState('filter.ljoc04_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljoc04_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljoc04_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljoc04_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljoc04_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljoc04_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljoc04_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljoc04_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljoc04_id'));
		$id .= ':'.$this->getState('filter.ljoc04_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljoc04s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljoc04s').' AS a');
		...
		// Filter by a single or group of ljoc04s.
		$ljoc04_id = $this->getState('filter.ljoc04_id');
		if ($ljoc04_id != '')
		{
			if (is_numeric($ljoc04_id))
			{
				$type = $this->getState('filter.ljoc04_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljoc04_id);
			}
			else
			{
				if (is_array($ljoc04_id))
				{
					JArrayHelper::toInteger($ljoc04_id);
					$ljoc04_id = implode(',', $ljoc04_id);
					$type = $this->getState('filter.ljoc04_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljoc04_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljoc04_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljoc04_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljoc04_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljoc04 params override menu item params only if menu param = 'use_ljoc04'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljoc04' and there is no ljoc04 param, use global

					// create an array of just the params set to 'use_ljoc04'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljoc04')
						{
							// if the ljoc04 has a value, use it
							if ($ljoc04_params->get($key) != '')
							{
								// get the value from the ljoc04
								$ljoc04_array[$key] = $ljoc04_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljoc04_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljoc04 params
					if (count($ljoc04_array) > 0)
					{
						$ljoc04_params = new Registry;
						$ljoc04_params->loadArray($ljoc04_array);
						$item->params->merge($ljoc04_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljoc04_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljoc04, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
