...
class JtCaModelLjomf08s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf08_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf08_id');
		$this->setState('filter.ljomf08_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf08_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf08_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf08_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf08_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf08_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf08_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf08_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf08_id'));
		$id .= ':'.$this->getState('filter.ljomf08_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf08s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf08s').' AS a');
		...
		// Filter by a single or group of ljomf08s.
		$ljomf08_id = $this->getState('filter.ljomf08_id');
		if ($ljomf08_id != '')
		{
			if (is_numeric($ljomf08_id))
			{
				$type = $this->getState('filter.ljomf08_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf08_id);
			}
			else
			{
				if (is_array($ljomf08_id))
				{
					JArrayHelper::toInteger($ljomf08_id);
					$ljomf08_id = implode(',', $ljomf08_id);
					$type = $this->getState('filter.ljomf08_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf08_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf08_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf08_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf08_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf08 params override menu item params only if menu param = 'use_ljomf08'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf08' and there is no ljomf08 param, use global

					// create an array of just the params set to 'use_ljomf08'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf08_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf08')
						{
							// if the ljomf08 has a value, use it
							if ($ljomf08_params->get($key) != '')
							{
								// get the value from the ljomf08
								$ljomf08_array[$key] = $ljomf08_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf08_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf08 params
					if (count($ljomf08_array) > 0)
					{
						$ljomf08_params = new Registry;
						$ljomf08_params->loadArray($ljomf08_array);
						$item->params->merge($ljomf08_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf08_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf08, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
