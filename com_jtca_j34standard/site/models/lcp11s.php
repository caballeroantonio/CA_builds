...
/**
 * This models supports retrieving lists of libreta de oficios..
 *
 */
class JtCaModelLcp11s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp11s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de oficios. has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp11_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp11_id');
		$this->setState('filter.lcp11_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp11_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp11_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp11_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp11_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp11_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp11_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp11_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp11_id'));
		$id .= ':'.$this->getState('filter.lcp11_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp11s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp11s').' AS a');
		...
		// Filter by a single or group of lcp11s.
		$lcp11_id = $this->getState('filter.lcp11_id');
		if ($lcp11_id != '')
		{
			if (is_numeric($lcp11_id))
			{
				$type = $this->getState('filter.lcp11_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp11_id);
			}
			else
			{
				if (is_array($lcp11_id))
				{
					JArrayHelper::toInteger($lcp11_id);
					$lcp11_id = implode(',', $lcp11_id);
					$type = $this->getState('filter.lcp11_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp11_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp11_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp11_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de oficios..
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp11_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp11 params override menu item params only if menu param = 'use_lcp11'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp11' and there is no lcp11 param, use global

					// create an array of just the params set to 'use_lcp11'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp11')
						{
							// if the lcp11 has a value, use it
							if ($lcp11_params->get($key) != '')
							{
								// get the value from the lcp11
								$lcp11_array[$key] = $lcp11_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp11_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp11 params
					if (count($lcp11_array) > 0)
					{
						$lcp11_params = new Registry;
						$lcp11_params->loadArray($lcp11_array);
						$item->params->merge($lcp11_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp11_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp11, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
