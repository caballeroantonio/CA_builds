...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpdng17.php';

class JtcaModelLjpdng17Form extends JtcaModelLjpdng17
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpdng17';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpdng17';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpdng17', 'ljpdng17', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de control de firmas procesados.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE CONTROL DE FIRMAS PROCESADOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpdng17.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpdng17.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpdng17.id');
		$is_new		= true;
		...
		// Reorder the libro de control de firmas procesados so the new libro de control de firmas procesados is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpdng17BeforeSave', array('com_jtca.ljpdng17', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpdng17AfterSave', array('com_jtca.ljpdng17', $table, $is_new));

		$this->setState('ljpdng17.id', $table->id);
