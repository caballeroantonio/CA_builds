...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpes12.php';

class JtCaModelLjpes12Form extends JtCaModelLjpes12
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpes12';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpes12';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpes12', 'ljpes12', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de fianzas.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE FIANZAS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpes12.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpes12.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpes12.id');
		$is_new		= true;
		...
		// Reorder the libro de fianzas so the new libro de fianzas is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpes12BeforeSave', array('com_jtca.ljpes12', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpes12AfterSave', array('com_jtca.ljpes12', $table, $is_new));

		$this->setState('ljpes12.id', $table->id);
