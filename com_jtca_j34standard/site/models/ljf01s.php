...
class JtcaModelLjf01s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf01_id');
		$this->setState('filter.ljf01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf01_id'));
		$id .= ':'.$this->getState('filter.ljf01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf01s').' AS a');
		...
		// Filter by a single or group of ljf01s.
		$ljf01_id = $this->getState('filter.ljf01_id');
		if ($ljf01_id != '')
		{
			if (is_numeric($ljf01_id))
			{
				$type = $this->getState('filter.ljf01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf01_id);
			}
			else
			{
				if (is_array($ljf01_id))
				{
					JArrayHelper::toInteger($ljf01_id);
					$ljf01_id = implode(',', $ljf01_id);
					$type = $this->getState('filter.ljf01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf01_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf01 params override menu item params only if menu param = 'use_ljf01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf01' and there is no ljf01 param, use global

					// create an array of just the params set to 'use_ljf01'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf01')
						{
							// if the ljf01 has a value, use it
							if ($ljf01_params->get($key) != '')
							{
								// get the value from the ljf01
								$ljf01_array[$key] = $ljf01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf01 params
					if (count($ljf01_array) > 0)
					{
						$ljf01_params = new Registry;
						$ljf01_params->loadArray($ljf01_array);
						$item->params->merge($ljf01_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
