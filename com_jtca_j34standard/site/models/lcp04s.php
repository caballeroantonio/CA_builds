...
/**
 * This models supports retrieving lists of libro de turno de justicia para adolescentes con d.
 *
 */
class JtCaModelLcp04s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp04s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno de justicia para adolescentes con detenido de pgr has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp04_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp04_id');
		$this->setState('filter.lcp04_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp04_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp04_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp04_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp04_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp04_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp04_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp04_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp04_id'));
		$id .= ':'.$this->getState('filter.lcp04_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp04s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp04s').' AS a');
		...
		// Filter by a single or group of lcp04s.
		$lcp04_id = $this->getState('filter.lcp04_id');
		if ($lcp04_id != '')
		{
			if (is_numeric($lcp04_id))
			{
				$type = $this->getState('filter.lcp04_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp04_id);
			}
			else
			{
				if (is_array($lcp04_id))
				{
					JArrayHelper::toInteger($lcp04_id);
					$lcp04_id = implode(',', $lcp04_id);
					$type = $this->getState('filter.lcp04_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp04_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp04_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp04_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno de justicia para adolescentes con d.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp04_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp04 params override menu item params only if menu param = 'use_lcp04'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp04' and there is no lcp04 param, use global

					// create an array of just the params set to 'use_lcp04'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp04')
						{
							// if the lcp04 has a value, use it
							if ($lcp04_params->get($key) != '')
							{
								// get the value from the lcp04
								$lcp04_array[$key] = $lcp04_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp04_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp04 params
					if (count($lcp04_array) > 0)
					{
						$lcp04_params = new Registry;
						$lcp04_params->loadArray($lcp04_array);
						$item->params->merge($lcp04_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp04_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp04, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
