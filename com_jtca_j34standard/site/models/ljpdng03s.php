...
/**
 * This models supports retrieving lists of libro de certificado de depósitos de egreso.
 *
 */
class JtCaModelLjpdng03s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpdng03s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de certificado de depósitos de egreso has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpdng03_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng03_id');
		$this->setState('filter.ljpdng03_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng03_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng03_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng03_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng03_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng03_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng03_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng03_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng03_id'));
		$id .= ':'.$this->getState('filter.ljpdng03_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng03s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng03s').' AS a');
		...
		// Filter by a single or group of ljpdng03s.
		$ljpdng03_id = $this->getState('filter.ljpdng03_id');
		if ($ljpdng03_id != '')
		{
			if (is_numeric($ljpdng03_id))
			{
				$type = $this->getState('filter.ljpdng03_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng03_id);
			}
			else
			{
				if (is_array($ljpdng03_id))
				{
					JArrayHelper::toInteger($ljpdng03_id);
					$ljpdng03_id = implode(',', $ljpdng03_id);
					$type = $this->getState('filter.ljpdng03_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng03_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng03_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng03_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de certificado de depósitos de egreso.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng03_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng03 params override menu item params only if menu param = 'use_ljpdng03'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng03' and there is no ljpdng03 param, use global

					// create an array of just the params set to 'use_ljpdng03'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng03_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng03')
						{
							// if the ljpdng03 has a value, use it
							if ($ljpdng03_params->get($key) != '')
							{
								// get the value from the ljpdng03
								$ljpdng03_array[$key] = $ljpdng03_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng03_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng03 params
					if (count($ljpdng03_array) > 0)
					{
						$ljpdng03_params = new Registry;
						$ljpdng03_params->loadArray($ljpdng03_array);
						$item->params->merge($ljpdng03_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng03_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng03, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
