...
/**
 * This models supports retrieving lists of libro de turno de excusas para juzgados de primera.
 *
 */
class JtCaModelLcp21s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp21s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno de excusas para juzgados de primera instancia. has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp21_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp21_id');
		$this->setState('filter.lcp21_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp21_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp21_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp21_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp21_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp21_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp21_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp21_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp21_id'));
		$id .= ':'.$this->getState('filter.lcp21_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp21s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp21s').' AS a');
		...
		// Filter by a single or group of lcp21s.
		$lcp21_id = $this->getState('filter.lcp21_id');
		if ($lcp21_id != '')
		{
			if (is_numeric($lcp21_id))
			{
				$type = $this->getState('filter.lcp21_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp21_id);
			}
			else
			{
				if (is_array($lcp21_id))
				{
					JArrayHelper::toInteger($lcp21_id);
					$lcp21_id = implode(',', $lcp21_id);
					$type = $this->getState('filter.lcp21_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp21_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp21_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp21_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno de excusas para juzgados de primera.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp21_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp21 params override menu item params only if menu param = 'use_lcp21'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp21' and there is no lcp21 param, use global

					// create an array of just the params set to 'use_lcp21'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp21')
						{
							// if the lcp21 has a value, use it
							if ($lcp21_params->get($key) != '')
							{
								// get the value from the lcp21
								$lcp21_array[$key] = $lcp21_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp21_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp21 params
					if (count($lcp21_array) > 0)
					{
						$lcp21_params = new Registry;
						$lcp21_params->loadArray($lcp21_array);
						$item->params->merge($lcp21_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp21_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp21, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
