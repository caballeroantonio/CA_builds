...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE TURNO DE JUSTICIA PARA ADOLESCENTES CON DETENIDO Model
 *
 */
class JtCaModelLcp01 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp01';
	...
	{
		if (empty($config['lcp01_filter_fields']))
		{
			$config['lcp01_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp01s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE TURNO DE JUSTICIA PARA ADOLESCENTES CON DETENIDO data.
	 *
	 * @param	integer	$pk	The id of the libro de turno de justicia para adolescentes con detenido.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp01.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP01S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp01ChangeState', array('com_jtca.lcp01', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp01BeforeDelete', array('com_jtca.lcp01', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp01AfterDelete', array('com_jtca.lcp01', &$table));
				}
				else
				{
					// Prune items that you can't change.
