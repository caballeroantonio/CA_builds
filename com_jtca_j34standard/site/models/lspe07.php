...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE FINANZAS Model
 *
 */
class JtCaModelLspe07 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lspe07';
	...
	{
		if (empty($config['lspe07_filter_fields']))
		{
			$config['lspe07_filter_fields'] = array(
				'id', 'a.id',
				'id_organo','a.id_organo',
				'anoj','a.anoj',
				'id_expediente','a.id_expediente',
				'id_secretaria','a.id_secretaria',
				'field2325','a.field2325',
				'field2326','a.field2326',
				'field2327','a.field2327',
				'field2328','a.field2328',
				'field2329','a.field2329',
				'field2330','a.field2330',
				'field2331','a.field2331',
				'field2332','a.field2332',
				'field2333','a.field2333',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lspe07s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE FINANZAS data.
	 *
	 * @param	integer	$pk	The id of the libro de finanzas.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lspe07.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPE07S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLspe07ChangeState', array('com_jtca.lspe07', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLspe07BeforeDelete', array('com_jtca.lspe07', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLspe07AfterDelete', array('com_jtca.lspe07', &$table));
				}
				else
				{
					// Prune items that you can't change.
