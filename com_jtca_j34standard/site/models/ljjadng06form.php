...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadng06.php';

class JtCaModelLjjadng06Form extends JtCaModelLjjadng06
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadng06';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadng06';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadng06', 'ljjadng06', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de exhortos.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE EXHORTOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadng06.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadng06.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadng06.id');
		$is_new		= true;
		...
		// Reorder the libro de exhortos so the new libro de exhortos is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadng06BeforeSave', array('com_jtca.ljjadng06', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadng06AfterSave', array('com_jtca.ljjadng06', $table, $is_new));

		$this->setState('ljjadng06.id', $table->id);
