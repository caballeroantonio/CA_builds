...
/**
 * This models supports retrieving lists of jc03.
 *
 */
class JtCaModelLjc03s extends JModelList
{
	 ...
	 */
	protected $context = 'com_jtca.ljc03s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljc03_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc03_id');
		$this->setState('filter.ljc03_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc03_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc03_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc03_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc03_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc03_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc03_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc03_archived'));
	...
	/**
	 * Get the main query for retrieving a list of ljc03s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc03s').' AS a');
		...
		if ($ljc03_id != '')
		{
			if (is_numeric($ljc03_id))
			{
				$type = $this->getState('filter.ljc03_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc03_id);
			}
			else
			{
				if (is_array($ljc03_id))
				{
					JArrayHelper::toInteger($ljc03_id);
					$ljc03_id = implode(',', $ljc03_id);
					$type = $this->getState('filter.ljc03_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc03_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc03_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc03_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc03_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc03 params override menu item params only if menu param = 'use_ljc03'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc03' and there is no ljc03 param, use global

					// create an array of just the params set to 'use_ljc03'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc03')
						{
							// if the ljc03 has a value, use it
							if ($ljc03_params->get($key) != '')
							{
								// get the value from the ljc03
								$ljc03_array[$key] = $ljc03_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc03_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc03 params
					if (count($ljc03_array) > 0)
					{
						$ljc03_params = new Registry;
						$ljc03_params->loadArray($ljc03_array);
						$item->params->merge($ljc03_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc03_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc03, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
