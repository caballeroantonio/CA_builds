...
/**
 * This models supports retrieving lists of libro de ordenes de arraigos y de cateos primera i.
 *
 */
class JtcaModelLcp22s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp22s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de ordenes de arraigos y de cateos primera instancia. has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp22_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp22_id');
		$this->setState('filter.lcp22_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp22_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp22_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp22_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp22_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp22_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp22_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp22_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp22_id'));
		$id .= ':'.$this->getState('filter.lcp22_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp22s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp22s').' AS a');
		...
		// Filter by a single or group of lcp22s.
		$lcp22_id = $this->getState('filter.lcp22_id');
		if ($lcp22_id != '')
		{
			if (is_numeric($lcp22_id))
			{
				$type = $this->getState('filter.lcp22_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp22_id);
			}
			else
			{
				if (is_array($lcp22_id))
				{
					JArrayHelper::toInteger($lcp22_id);
					$lcp22_id = implode(',', $lcp22_id);
					$type = $this->getState('filter.lcp22_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp22_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp22_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp22_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de ordenes de arraigos y de cateos primera i.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp22_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp22 params override menu item params only if menu param = 'use_lcp22'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp22' and there is no lcp22 param, use global

					// create an array of just the params set to 'use_lcp22'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp22')
						{
							// if the lcp22 has a value, use it
							if ($lcp22_params->get($key) != '')
							{
								// get the value from the lcp22
								$lcp22_array[$key] = $lcp22_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp22_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp22 params
					if (count($lcp22_array) > 0)
					{
						$lcp22_params = new Registry;
						$lcp22_params->loadArray($lcp22_array);
						$item->params->merge($lcp22_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp22_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp22, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
