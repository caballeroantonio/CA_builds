...
class JtCaModelLjf21s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf21_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf21_id');
		$this->setState('filter.ljf21_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf21_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf21_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf21_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf21_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf21_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf21_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf21_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf21_id'));
		$id .= ':'.$this->getState('filter.ljf21_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf21s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf21s').' AS a');
		...
		// Filter by a single or group of ljf21s.
		$ljf21_id = $this->getState('filter.ljf21_id');
		if ($ljf21_id != '')
		{
			if (is_numeric($ljf21_id))
			{
				$type = $this->getState('filter.ljf21_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf21_id);
			}
			else
			{
				if (is_array($ljf21_id))
				{
					JArrayHelper::toInteger($ljf21_id);
					$ljf21_id = implode(',', $ljf21_id);
					$type = $this->getState('filter.ljf21_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf21_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf21_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf21_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf21_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf21 params override menu item params only if menu param = 'use_ljf21'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf21' and there is no ljf21 param, use global

					// create an array of just the params set to 'use_ljf21'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf21')
						{
							// if the ljf21 has a value, use it
							if ($ljf21_params->get($key) != '')
							{
								// get the value from the ljf21
								$ljf21_array[$key] = $ljf21_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf21_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf21 params
					if (count($ljf21_array) > 0)
					{
						$ljf21_params = new Registry;
						$ljf21_params->loadArray($ljf21_array);
						$item->params->merge($ljf21_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf21_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf21, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
