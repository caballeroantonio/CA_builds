 ...
 *
 */
class JtCaModelLjjadng01 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljjadng01';
	...
	{
		if (empty($config['ljjadng01_filter_fields']))
		{
			$config['ljjadng01_filter_fields'] = array(
				'id', 'a.id',
				...
				'field2','a.field2',
				'txt_field2032','a.txt_field2032',
				'id_field2032','a.id_field2032',
				'field6','a.field6',
				'field7','a.field7',
				'field8','a.field8',
				'field9','a.field9',
	...
	*/
	public function getTable($type = 'Ljjadng01s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljjadng01.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJJADNG01S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjjadng01ChangeState', array('com_jtca.ljjadng01', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjjadng01BeforeDelete', array('com_jtca.ljjadng01', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjjadng01AfterDelete', array('com_jtca.ljjadng01', &$table));
				}
				else
				{
					// Prune items that you can't change.
