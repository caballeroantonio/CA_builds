...
/**
 * This models supports retrieving lists of libro de registro de amparos (oficial).
 *
 */
class JtcaModelLsc12s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsc12s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de registro de amparos (oficial) has been specified either as a parameter or in the url Request
		$pk = $params->get('lsc12_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsc12_id');
		$this->setState('filter.lsc12_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsc12_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsc12_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsc12_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsc12_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsc12_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsc12_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsc12_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsc12_id'));
		$id .= ':'.$this->getState('filter.lsc12_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsc12s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsc12s').' AS a');
		...
		// Filter by a single or group of lsc12s.
		$lsc12_id = $this->getState('filter.lsc12_id');
		if ($lsc12_id != '')
		{
			if (is_numeric($lsc12_id))
			{
				$type = $this->getState('filter.lsc12_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsc12_id);
			}
			else
			{
				if (is_array($lsc12_id))
				{
					JArrayHelper::toInteger($lsc12_id);
					$lsc12_id = implode(',', $lsc12_id);
					$type = $this->getState('filter.lsc12_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsc12_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsc12_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsc12_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de registro de amparos (oficial).
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsc12_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsc12 params override menu item params only if menu param = 'use_lsc12'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsc12' and there is no lsc12 param, use global

					// create an array of just the params set to 'use_lsc12'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsc12')
						{
							// if the lsc12 has a value, use it
							if ($lsc12_params->get($key) != '')
							{
								// get the value from the lsc12
								$lsc12_array[$key] = $lsc12_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsc12_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsc12 params
					if (count($lsc12_array) > 0)
					{
						$lsc12_params = new Registry;
						$lsc12_params->loadArray($lsc12_array);
						$item->params->merge($lsc12_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsc12_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsc12, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
