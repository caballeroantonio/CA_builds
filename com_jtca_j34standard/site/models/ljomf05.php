 ...
 *
 */
class JtCaModelLjomf05 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljomf05';
	...
	{
		if (empty($config['ljomf05_filter_fields']))
		{
			$config['ljomf05_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljomf05s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljomf05.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJOMF05S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjomf05ChangeState', array('com_jtca.ljomf05', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjomf05BeforeDelete', array('com_jtca.ljomf05', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjomf05AfterDelete', array('com_jtca.ljomf05', &$table));
				}
				else
				{
					// Prune items that you can't change.
