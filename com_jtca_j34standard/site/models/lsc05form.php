...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsc05.php';

class JtCaModelLsc05Form extends JtCaModelLsc05
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsc05';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsc05';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsc05', 'lsc05', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the agenda (oficial).
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	AGENDA (OFICIAL) item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsc05.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsc05.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsc05.id');
		$is_new		= true;
		...
		// Reorder the agenda (oficial) so the new agenda (oficial) is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsc05BeforeSave', array('com_jtca.lsc05', &$table, $is_new));
		...
		$dispatcher->trigger('onLsc05AfterSave', array('com_jtca.lsc05', $table, $is_new));

		$this->setState('lsc05.id', $table->id);
