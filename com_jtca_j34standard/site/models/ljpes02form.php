...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpes02.php';

class JtcaModelLjpes02Form extends JtcaModelLjpes02
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpes02';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpes02';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpes02', 'ljpes02', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpes02.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpes02.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpes02.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjpes02BeforeSave', array('com_jtca.ljpes02', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpes02AfterSave', array('com_jtca.ljpes02', $table, $is_new));

		$this->setState('ljpes02.id', $table->id);
