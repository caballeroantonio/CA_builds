...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsc01.php';

class JtcaModelLsc01Form extends JtcaModelLsc01
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsc01';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsc01';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsc01', 'lsc01', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de registro de sentencias -turno- (oficial).
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE REGISTRO DE SENTENCIAS -TURNO- (OFICIAL) item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsc01.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsc01.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsc01.id');
		$is_new		= true;
		...
		// Reorder the libreta de registro de sentencias -turno- (oficial so the new libreta de registro de sentencias -turno- (oficial) is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsc01BeforeSave', array('com_jtca.lsc01', &$table, $is_new));
		...
		$dispatcher->trigger('onLsc01AfterSave', array('com_jtca.lsc01', $table, $is_new));

		$this->setState('lsc01.id', $table->id);
