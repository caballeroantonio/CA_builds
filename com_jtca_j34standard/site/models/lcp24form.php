...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp24.php';

class JtCaModelLcp24Form extends JtCaModelLcp24
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp24';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp24';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp24', 'lcp24', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de registro de la ley de acceso a las mujeres a una vida libre de violencia..
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE REGISTRO DE LA LEY DE ACCESO A LAS MUJERES A UNA VIDA LIBRE DE VIOLENCIA. item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp24.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp24.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp24.id');
		$is_new		= true;
		...
		// Reorder the libro de registro de la ley de acceso a las mujere so the new libro de registro de la ley de acceso a las mujeres a una vida libre de violencia. is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp24BeforeSave', array('com_jtca.lcp24', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp24AfterSave', array('com_jtca.lcp24', $table, $is_new));

		$this->setState('lcp24.id', $table->id);
