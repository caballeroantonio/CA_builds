...
class JtCaModelLjomf13s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf13_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf13_id');
		$this->setState('filter.ljomf13_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf13_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf13_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf13_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf13_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf13_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf13_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf13_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf13_id'));
		$id .= ':'.$this->getState('filter.ljomf13_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf13s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf13s').' AS a');
		...
		// Filter by a single or group of ljomf13s.
		$ljomf13_id = $this->getState('filter.ljomf13_id');
		if ($ljomf13_id != '')
		{
			if (is_numeric($ljomf13_id))
			{
				$type = $this->getState('filter.ljomf13_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf13_id);
			}
			else
			{
				if (is_array($ljomf13_id))
				{
					JArrayHelper::toInteger($ljomf13_id);
					$ljomf13_id = implode(',', $ljomf13_id);
					$type = $this->getState('filter.ljomf13_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf13_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf13_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf13_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf13_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf13 params override menu item params only if menu param = 'use_ljomf13'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf13' and there is no ljomf13 param, use global

					// create an array of just the params set to 'use_ljomf13'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf13_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf13')
						{
							// if the ljomf13 has a value, use it
							if ($ljomf13_params->get($key) != '')
							{
								// get the value from the ljomf13
								$ljomf13_array[$key] = $ljomf13_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf13_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf13 params
					if (count($ljomf13_array) > 0)
					{
						$ljomf13_params = new Registry;
						$ljomf13_params->loadArray($ljomf13_array);
						$item->params->merge($ljomf13_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf13_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf13, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
