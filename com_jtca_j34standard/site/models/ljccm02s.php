...
/**
 * This models supports retrieving lists of libro de ingreso de valores.
 *
 */
class JtcaModelLjccm02s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljccm02s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de ingreso de valores has been specified either as a parameter or in the url Request
		$pk = $params->get('ljccm02_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljccm02_id');
		$this->setState('filter.ljccm02_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljccm02_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljccm02_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljccm02_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljccm02_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljccm02_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljccm02_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljccm02_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljccm02_id'));
		$id .= ':'.$this->getState('filter.ljccm02_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljccm02s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljccm02s').' AS a');
		...
		// Filter by a single or group of ljccm02s.
		$ljccm02_id = $this->getState('filter.ljccm02_id');
		if ($ljccm02_id != '')
		{
			if (is_numeric($ljccm02_id))
			{
				$type = $this->getState('filter.ljccm02_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljccm02_id);
			}
			else
			{
				if (is_array($ljccm02_id))
				{
					JArrayHelper::toInteger($ljccm02_id);
					$ljccm02_id = implode(',', $ljccm02_id);
					$type = $this->getState('filter.ljccm02_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljccm02_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljccm02_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljccm02_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de ingreso de valores.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljccm02_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljccm02 params override menu item params only if menu param = 'use_ljccm02'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljccm02' and there is no ljccm02 param, use global

					// create an array of just the params set to 'use_ljccm02'
					$menu_params_array = $this->getState('params')->toArray();
					$ljccm02_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljccm02')
						{
							// if the ljccm02 has a value, use it
							if ($ljccm02_params->get($key) != '')
							{
								// get the value from the ljccm02
								$ljccm02_array[$key] = $ljccm02_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljccm02_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljccm02 params
					if (count($ljccm02_array) > 0)
					{
						$ljccm02_params = new Registry;
						$ljccm02_params->loadArray($ljccm02_array);
						$item->params->merge($ljccm02_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljccm02_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljccm02, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
	...
	/**
	 * Build a list of distinct values in the No. DEL DOCUMENTO field
	 *
	 * @return	JDatabaseQuery
	 */
