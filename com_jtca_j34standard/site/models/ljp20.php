...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE TURNO PARA CONCLUSIONES Model
 *
 */
class JtCaModelLjp20 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljp20';
	...
	{
		if (empty($config['ljp20_filter_fields']))
		{
			$config['ljp20_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljp20s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE TURNO PARA CONCLUSIONES data.
	 *
	 * @param	integer	$pk	The id of the libro de turno para conclusiones.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljp20.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJP20S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjp20ChangeState', array('com_jtca.ljp20', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjp20BeforeDelete', array('com_jtca.ljp20', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjp20AfterDelete', array('com_jtca.ljp20', &$table));
				}
				else
				{
					// Prune items that you can't change.
