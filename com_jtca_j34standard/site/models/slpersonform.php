...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/slperson.php';

class JtcaModelSlpersonForm extends JtcaModelSlperson
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.slperson';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.slperson';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.slperson', 'slperson', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the sub libro personas.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Sub Libro Personas item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('slperson.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.slperson.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('slperson.id');
		$is_new		= true;
		...
		// Reorder the persons so the new sub libro personas is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSlpersonBeforeSave', array('com_jtca.slperson', &$table, $is_new));
		...
		$dispatcher->trigger('onSlpersonAfterSave', array('com_jtca.slperson', $table, $is_new));

		$this->setState('slperson.id', $table->id);
