...
/**
 * TSJ CDMX Libros TxCA Component LIBRETA DE OFICIOS (OFICIAL) Model
 *
 */
class JtcaModelLsc11 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsc11';
	...
	{
		if (empty($config['lsc11_filter_fields']))
		{
			$config['lsc11_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lsc11s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRETA DE OFICIOS (OFICIAL) data.
	 *
	 * @param	integer	$pk	The id of the libreta de oficios (oficial).
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsc11.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSC11S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsc11ChangeState', array('com_jtca.lsc11', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsc11BeforeDelete', array('com_jtca.lsc11', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsc11AfterDelete', array('com_jtca.lsc11', &$table));
				}
				else
				{
					// Prune items that you can't change.
