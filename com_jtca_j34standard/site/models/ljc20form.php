...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljc20.php';

class JtcaModelLjc20Form extends JtcaModelLjc20
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljc20';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljc20';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljc20', 'ljc20', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljc20.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljc20.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljc20.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjc20BeforeSave', array('com_jtca.ljc20', &$table, $is_new));
		...
		$dispatcher->trigger('onLjc20AfterSave', array('com_jtca.ljc20', $table, $is_new));

		$this->setState('ljc20.id', $table->id);
