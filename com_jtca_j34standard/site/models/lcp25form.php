...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp25.php';

class JtcaModelLcp25Form extends JtcaModelLcp25
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp25';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp25';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp25', 'lcp25', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno para consignaciones con detenido delitos no graves.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO PARA CONSIGNACIONES CON DETENIDO DELITOS NO GRAVES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp25.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp25.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp25.id');
		$is_new		= true;
		...
		// Reorder the libro de turno para consignaciones con detenido de so the new libro de turno para consignaciones con detenido delitos no graves is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp25BeforeSave', array('com_jtca.lcp25', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp25AfterSave', array('com_jtca.lcp25', $table, $is_new));

		$this->setState('lcp25.id', $table->id);
