 ...
 *
 */
class JtCaModelLjpes01 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpes01';
	...
	{
		if (empty($config['ljpes01_filter_fields']))
		{
			$config['ljpes01_filter_fields'] = array(
				'id', 'a.id',
				...
				'field5','a.field5',
				'field6','a.field6',
				'txt_field1198','a.txt_field1198',
				'id_field1198','a.id_field1198',
				'txt_field1199','a.txt_field1199',
				'id_field1199','a.id_field1199',
				'field10','a.field10',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Ljpes01s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpes01.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPES01S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpes01ChangeState', array('com_jtca.ljpes01', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpes01BeforeDelete', array('com_jtca.ljpes01', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpes01AfterDelete', array('com_jtca.ljpes01', &$table));
				}
				else
				{
					// Prune items that you can't change.
