...
class JtCaModelLjf03s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf03_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf03_id');
		$this->setState('filter.ljf03_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf03_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf03_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf03_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf03_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf03_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf03_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf03_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf03_id'));
		$id .= ':'.$this->getState('filter.ljf03_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf03s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf03s').' AS a');
		...
		// Filter by a single or group of ljf03s.
		$ljf03_id = $this->getState('filter.ljf03_id');
		if ($ljf03_id != '')
		{
			if (is_numeric($ljf03_id))
			{
				$type = $this->getState('filter.ljf03_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf03_id);
			}
			else
			{
				if (is_array($ljf03_id))
				{
					JArrayHelper::toInteger($ljf03_id);
					$ljf03_id = implode(',', $ljf03_id);
					$type = $this->getState('filter.ljf03_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf03_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf03_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf03_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf03_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf03 params override menu item params only if menu param = 'use_ljf03'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf03' and there is no ljf03 param, use global

					// create an array of just the params set to 'use_ljf03'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf03')
						{
							// if the ljf03 has a value, use it
							if ($ljf03_params->get($key) != '')
							{
								// get the value from the ljf03
								$ljf03_array[$key] = $ljf03_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf03_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf03 params
					if (count($ljf03_array) > 0)
					{
						$ljf03_params = new Registry;
						$ljf03_params->loadArray($ljf03_array);
						$item->params->merge($ljf03_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf03_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf03, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
