...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadg11.php';

class JtCaModelLjjadg11Form extends JtCaModelLjjadg11
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadg11';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadg11';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadg11', 'ljjadg11', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadg11.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadg11.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadg11.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjjadg11BeforeSave', array('com_jtca.ljjadg11', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadg11AfterSave', array('com_jtca.ljjadg11', $table, $is_new));

		$this->setState('ljjadg11.id', $table->id);
