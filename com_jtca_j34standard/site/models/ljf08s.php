...
class JtCaModelLjf08s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf08_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf08_id');
		$this->setState('filter.ljf08_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf08_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf08_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf08_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf08_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf08_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf08_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf08_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf08_id'));
		$id .= ':'.$this->getState('filter.ljf08_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf08s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf08s').' AS a');
		...
		// Filter by a single or group of ljf08s.
		$ljf08_id = $this->getState('filter.ljf08_id');
		if ($ljf08_id != '')
		{
			if (is_numeric($ljf08_id))
			{
				$type = $this->getState('filter.ljf08_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf08_id);
			}
			else
			{
				if (is_array($ljf08_id))
				{
					JArrayHelper::toInteger($ljf08_id);
					$ljf08_id = implode(',', $ljf08_id);
					$type = $this->getState('filter.ljf08_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf08_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf08_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf08_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf08_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf08 params override menu item params only if menu param = 'use_ljf08'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf08' and there is no ljf08 param, use global

					// create an array of just the params set to 'use_ljf08'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf08')
						{
							// if the ljf08 has a value, use it
							if ($ljf08_params->get($key) != '')
							{
								// get the value from the ljf08
								$ljf08_array[$key] = $ljf08_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf08_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf08 params
					if (count($ljf08_array) > 0)
					{
						$ljf08_params = new Registry;
						$ljf08_params->loadArray($ljf08_array);
						$item->params->merge($ljf08_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf08_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf08, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
