...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/sldecljpdng01.php';

class JtcaModelSldeclJpdng01Form extends JtcaModelSldeclJpdng01
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.sldecljpdng01';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.sldecljpdng01';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.sldecljpdng01', 'sldecljpdng01', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the sub libro declaracion preparatoria del libro de gobierno.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Sub Libro DECLARACION PREPARATORIA Del LIBRO DE GOBIERNO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('sldecljpdng01.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.sldecljpdng01.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('sldecljpdng01.id');
		$is_new		= true;
		...
		// Reorder the declaracion preparatoria del libro de gobierno so the new sub libro declaracion preparatoria del libro de gobierno is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSldeclJpdng01BeforeSave', array('com_jtca.sldecljpdng01', &$table, $is_new));
		...
		$dispatcher->trigger('onSldeclJpdng01AfterSave', array('com_jtca.sldecljpdng01', $table, $is_new));

		$this->setState('sldecljpdng01.id', $table->id);
