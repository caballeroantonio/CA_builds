...
class JtCaModelLsc01 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsc01';
	...
	{
		if (empty($config['lsc01_filter_fields']))
		{
			$config['lsc01_filter_fields'] = array(
				'id', 'a.id',
				...
				'field122','a.field122',
				'field236','a.field236',
				'field2347','a.field2347',
				'field123','a.field123',
				'field124','a.field124',
				'field2348','a.field2348',
				'field253','a.field253',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	...
	/**
	 * Method to get LIBRETA DE REGISTRO DE SENTENCIAS -TURNO- (OFICIAL) data.
	 *
	 * @param	integer	$pk	The id of the libreta de registro de sentencias -turno- (oficial).
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsc01.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSC01S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsc01ChangeState', array('com_jtca.lsc01', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsc01BeforeDelete', array('com_jtca.lsc01', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsc01AfterDelete', array('com_jtca.lsc01', &$table));
				}
				else
				{
					// Prune items that you can't change.
