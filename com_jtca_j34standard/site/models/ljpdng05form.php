...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpdng05.php';

class JtCaModelLjpdng05Form extends JtCaModelLjpdng05
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpdng05';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpdng05';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpdng05', 'ljpdng05', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno para sentencias.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO PARA SENTENCIAS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpdng05.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpdng05.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpdng05.id');
		$is_new		= true;
		...
		// Reorder the libro de turno para sentencias so the new libro de turno para sentencias is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpdng05BeforeSave', array('com_jtca.ljpdng05', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpdng05AfterSave', array('com_jtca.ljpdng05', $table, $is_new));

		$this->setState('ljpdng05.id', $table->id);
