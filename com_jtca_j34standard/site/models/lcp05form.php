...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp05.php';

class JtcaModelLcp05Form extends JtcaModelLcp05
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp05';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp05';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp05', 'lcp05', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno de justicia para adolescentes sin detenido de pgr.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO DE JUSTICIA PARA ADOLESCENTES SIN DETENIDO DE PGR item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp05.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp05.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp05.id');
		$is_new		= true;
		...
		// Reorder the libro de turno de justicia para adolescentes sin d so the new libro de turno de justicia para adolescentes sin detenido de pgr is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp05BeforeSave', array('com_jtca.lcp05', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp05AfterSave', array('com_jtca.lcp05', $table, $is_new));

		$this->setState('lcp05.id', $table->id);
