...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsps07.php';

class JtcaModelLsps07Form extends JtcaModelLsps07
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsps07';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsps07';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsps07', 'lsps07', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the registro de billetes de deposito.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	REGISTRO DE BILLETES DE DEPOSITO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsps07.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsps07.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsps07.id');
		$is_new		= true;
		...
		// Reorder the registro de billetes de deposito so the new registro de billetes de deposito is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsps07BeforeSave', array('com_jtca.lsps07', &$table, $is_new));
		...
		$dispatcher->trigger('onLsps07AfterSave', array('com_jtca.lsps07', $table, $is_new));

		$this->setState('lsps07.id', $table->id);
