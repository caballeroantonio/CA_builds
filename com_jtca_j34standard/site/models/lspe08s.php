...
/**
 * This models supports retrieving lists of libro de ingreso y egreso de billetes.
 *
 */
class JtcaModelLspe08s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lspe08s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de ingreso y egreso de billetes has been specified either as a parameter or in the url Request
		$pk = $params->get('lspe08_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lspe08_id');
		$this->setState('filter.lspe08_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lspe08_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lspe08_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lspe08_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lspe08_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lspe08_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lspe08_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lspe08_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lspe08_id'));
		$id .= ':'.$this->getState('filter.lspe08_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lspe08s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lspe08s').' AS a');
		...
		// Filter by a single or group of lspe08s.
		$lspe08_id = $this->getState('filter.lspe08_id');
		if ($lspe08_id != '')
		{
			if (is_numeric($lspe08_id))
			{
				$type = $this->getState('filter.lspe08_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lspe08_id);
			}
			else
			{
				if (is_array($lspe08_id))
				{
					JArrayHelper::toInteger($lspe08_id);
					$lspe08_id = implode(',', $lspe08_id);
					$type = $this->getState('filter.lspe08_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lspe08_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lspe08_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lspe08_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de ingreso y egreso de billetes.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lspe08_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lspe08 params override menu item params only if menu param = 'use_lspe08'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lspe08' and there is no lspe08 param, use global

					// create an array of just the params set to 'use_lspe08'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lspe08')
						{
							// if the lspe08 has a value, use it
							if ($lspe08_params->get($key) != '')
							{
								// get the value from the lspe08
								$lspe08_array[$key] = $lspe08_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lspe08_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lspe08 params
					if (count($lspe08_array) > 0)
					{
						$lspe08_params = new Registry;
						$lspe08_params->loadArray($lspe08_array);
						$item->params->merge($lspe08_params);
					}


					// get display date
					switch ($item->params->get('list_show_lspe08_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lspe08, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
