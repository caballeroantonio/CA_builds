...
/**
 * This models supports retrieving lists of agenda (oficial).
 *
 */
class JtCaModelLsc05s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsc05s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single agenda (oficial) has been specified either as a parameter or in the url Request
		$pk = $params->get('lsc05_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsc05_id');
		$this->setState('filter.lsc05_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsc05_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsc05_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsc05_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsc05_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsc05_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsc05_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsc05_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsc05_id'));
		$id .= ':'.$this->getState('filter.lsc05_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsc05s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsc05s').' AS a');
		...
		// Filter by a single or group of lsc05s.
		$lsc05_id = $this->getState('filter.lsc05_id');
		if ($lsc05_id != '')
		{
			if (is_numeric($lsc05_id))
			{
				$type = $this->getState('filter.lsc05_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsc05_id);
			}
			else
			{
				if (is_array($lsc05_id))
				{
					JArrayHelper::toInteger($lsc05_id);
					$lsc05_id = implode(',', $lsc05_id);
					$type = $this->getState('filter.lsc05_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsc05_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsc05_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsc05_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of agenda (oficial).
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsc05_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsc05 params override menu item params only if menu param = 'use_lsc05'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsc05' and there is no lsc05 param, use global

					// create an array of just the params set to 'use_lsc05'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsc05')
						{
							// if the lsc05 has a value, use it
							if ($lsc05_params->get($key) != '')
							{
								// get the value from the lsc05
								$lsc05_array[$key] = $lsc05_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsc05_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsc05 params
					if (count($lsc05_array) > 0)
					{
						$lsc05_params = new Registry;
						$lsc05_params->loadArray($lsc05_array);
						$item->params->merge($lsc05_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsc05_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsc05, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
