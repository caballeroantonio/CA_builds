...
/**
 * TSJ CDMX Libros TxCA Component LIBRETA DE LIBERTADES DE SALAS PENALES Model
 *
 */
class JtCaModelLcp33 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp33';
	...
	{
		if (empty($config['lcp33_filter_fields']))
		{
			$config['lcp33_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp33s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRETA DE LIBERTADES DE SALAS PENALES data.
	 *
	 * @param	integer	$pk	The id of the libreta de libertades de salas penales.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp33.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP33S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp33ChangeState', array('com_jtca.lcp33', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp33BeforeDelete', array('com_jtca.lcp33', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp33AfterDelete', array('com_jtca.lcp33', &$table));
				}
				else
				{
					// Prune items that you can't change.
