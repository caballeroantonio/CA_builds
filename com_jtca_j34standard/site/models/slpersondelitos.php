...
/**
 * This models supports retrieving lists of inculpados.
 *
 */
class JtcaModelSlpersondelitos extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.slpersondelitos';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single inculpados has been specified either as a parameter or in the url Request
		$pk = $params->get('slpersondelito_id', '') == '' ? $app->input->getInt('id', '') : $params->get('slpersondelito_id');
		$this->setState('filter.slpersondelito_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('slpersondelito_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('slpersondelito_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('slpersondelito_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('slpersondelito_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('slpersondelito_initial_direction','ASC');
		}
		...
		if ($params->get('filter_slpersondelito_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_slpersondelito_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.slpersondelito_id'));
		$id .= ':'.$this->getState('filter.slpersondelito_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of slpersondelitos subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_slpersondelitos').' AS a');
		...
		// Filter by a single or group of slpersondelitos.
		$slpersondelito_id = $this->getState('filter.slpersondelito_id');
		if ($slpersondelito_id != '')
		{
			if (is_numeric($slpersondelito_id))
			{
				$type = $this->getState('filter.slpersondelito_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $slpersondelito_id);
			}
			else
			{
				if (is_array($slpersondelito_id))
				{
					JArrayHelper::toInteger($slpersondelito_id);
					$slpersondelito_id = implode(',', $slpersondelito_id);
					$type = $this->getState('filter.slpersondelito_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$slpersondelito_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_slpersondelito_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_slpersondelito_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of inculpados.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$slpersondelito_params = new Registry;
				...
				{
					$item->params = $slpersondelito_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Slpersondelito params override menu item params only if menu param = 'use_slpersondelito'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_slpersondelito' and there is no slpersondelito param, use global

					// create an array of just the params set to 'use_slpersondelito'
					$menu_params_array = $this->getState('params')->toArray();
					$slpersondelito_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_slpersondelito')
						{
							// if the slpersondelito has a value, use it
							if ($slpersondelito_params->get($key) != '')
							{
								// get the value from the slpersondelito
								$slpersondelito_array[$key] = $slpersondelito_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$slpersondelito_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected slpersondelito params
					if (count($slpersondelito_array) > 0)
					{
						$slpersondelito_params = new Registry;
						$slpersondelito_params->loadArray($slpersondelito_array);
						$item->params->merge($slpersondelito_params);
					}


					// get display date
					switch ($item->params->get('list_show_slpersondelito_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an slpersondelito, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
