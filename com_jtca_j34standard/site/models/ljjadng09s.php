...
/**
 * This models supports retrieving lists of libro de actuario.
 *
 */
class JtCaModelLjjadng09s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadng09s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de actuario has been specified either as a parameter or in the url Request
		$pk = $params->get('ljjadng09_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadng09_id');
		$this->setState('filter.ljjadng09_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadng09_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadng09_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadng09_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadng09_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadng09_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadng09_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadng09_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadng09_id'));
		$id .= ':'.$this->getState('filter.ljjadng09_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadng09s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadng09s').' AS a');
		...
		// Filter by a single or group of ljjadng09s.
		$ljjadng09_id = $this->getState('filter.ljjadng09_id');
		if ($ljjadng09_id != '')
		{
			if (is_numeric($ljjadng09_id))
			{
				$type = $this->getState('filter.ljjadng09_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadng09_id);
			}
			else
			{
				if (is_array($ljjadng09_id))
				{
					JArrayHelper::toInteger($ljjadng09_id);
					$ljjadng09_id = implode(',', $ljjadng09_id);
					$type = $this->getState('filter.ljjadng09_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadng09_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadng09_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadng09_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de actuario.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadng09_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadng09 params override menu item params only if menu param = 'use_ljjadng09'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadng09' and there is no ljjadng09 param, use global

					// create an array of just the params set to 'use_ljjadng09'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadng09_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadng09')
						{
							// if the ljjadng09 has a value, use it
							if ($ljjadng09_params->get($key) != '')
							{
								// get the value from the ljjadng09
								$ljjadng09_array[$key] = $ljjadng09_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadng09_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadng09 params
					if (count($ljjadng09_array) > 0)
					{
						$ljjadng09_params = new Registry;
						$ljjadng09_params->loadArray($ljjadng09_array);
						$item->params->merge($ljjadng09_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadng09_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadng09, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
