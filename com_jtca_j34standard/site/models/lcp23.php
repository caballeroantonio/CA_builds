...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE ORDENES DE APREHENSIÓN, REAPREHENSION Y TRASLADO. Model
 *
 */
class JtcaModelLcp23 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp23';
	...
	{
		if (empty($config['lcp23_filter_fields']))
		{
			$config['lcp23_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp23s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE ORDENES DE APREHENSIÓN, REAPREHENSION Y TRASLADO. data.
	 *
	 * @param	integer	$pk	The id of the libro de ordenes de aprehensión, reaprehension y traslado..
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp23.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP23S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp23ChangeState', array('com_jtca.lcp23', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp23BeforeDelete', array('com_jtca.lcp23', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp23AfterDelete', array('com_jtca.lcp23', &$table));
				}
				else
				{
					// Prune items that you can't change.
