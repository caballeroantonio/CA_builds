...
/**
 * This models supports retrieving lists of libro de control de fianza.
 *
 */
class JtCaModelLsps11s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsps11s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de control de fianza has been specified either as a parameter or in the url Request
		$pk = $params->get('lsps11_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsps11_id');
		$this->setState('filter.lsps11_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsps11_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsps11_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsps11_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsps11_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsps11_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsps11_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsps11_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsps11_id'));
		$id .= ':'.$this->getState('filter.lsps11_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsps11s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsps11s').' AS a');
		...
		// Filter by a single or group of lsps11s.
		$lsps11_id = $this->getState('filter.lsps11_id');
		if ($lsps11_id != '')
		{
			if (is_numeric($lsps11_id))
			{
				$type = $this->getState('filter.lsps11_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsps11_id);
			}
			else
			{
				if (is_array($lsps11_id))
				{
					JArrayHelper::toInteger($lsps11_id);
					$lsps11_id = implode(',', $lsps11_id);
					$type = $this->getState('filter.lsps11_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsps11_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsps11_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsps11_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de control de fianza.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsps11_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsps11 params override menu item params only if menu param = 'use_lsps11'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsps11' and there is no lsps11 param, use global

					// create an array of just the params set to 'use_lsps11'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsps11')
						{
							// if the lsps11 has a value, use it
							if ($lsps11_params->get($key) != '')
							{
								// get the value from the lsps11
								$lsps11_array[$key] = $lsps11_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsps11_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsps11 params
					if (count($lsps11_array) > 0)
					{
						$lsps11_params = new Registry;
						$lsps11_params->loadArray($lsps11_array);
						$item->params->merge($lsps11_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsps11_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsps11, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
