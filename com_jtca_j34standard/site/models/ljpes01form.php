...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpes01.php';

class JtCaModelLjpes01Form extends JtCaModelLjpes01
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpes01';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpes01';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpes01', 'ljpes01', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpes01.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpes01.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpes01.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjpes01BeforeSave', array('com_jtca.ljpes01', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpes01AfterSave', array('com_jtca.ljpes01', $table, $is_new));

		$this->setState('ljpes01.id', $table->id);
