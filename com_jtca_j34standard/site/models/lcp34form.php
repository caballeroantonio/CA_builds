...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp34.php';

class JtCaModelLcp34Form extends JtCaModelLcp34
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp34';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp34';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp34', 'lcp34', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de libertades juzgados de adolescentes.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE LIBERTADES JUZGADOS DE ADOLESCENTES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp34.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp34.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp34.id');
		$is_new		= true;
		...
		// Reorder the libreta de libertades juzgados de adolescentes so the new libreta de libertades juzgados de adolescentes is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp34BeforeSave', array('com_jtca.lcp34', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp34AfterSave', array('com_jtca.lcp34', $table, $is_new));

		$this->setState('lcp34.id', $table->id);
