 ...
 *
 */
class JtcaModelLjf05 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljf05';
	...
	{
		if (empty($config['ljf05_filter_fields']))
		{
			$config['ljf05_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljf05s', $prefix = 'JtcaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljf05.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJF05S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjf05ChangeState', array('com_jtca.ljf05', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjf05BeforeDelete', array('com_jtca.ljf05', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjf05AfterDelete', array('com_jtca.ljf05', &$table));
				}
				else
				{
					// Prune items that you can't change.
