...
class JtcaModelLjf16s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf16_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf16_id');
		$this->setState('filter.ljf16_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf16_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf16_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf16_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf16_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf16_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf16_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf16_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf16_id'));
		$id .= ':'.$this->getState('filter.ljf16_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf16s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf16s').' AS a');
		...
		// Filter by a single or group of ljf16s.
		$ljf16_id = $this->getState('filter.ljf16_id');
		if ($ljf16_id != '')
		{
			if (is_numeric($ljf16_id))
			{
				$type = $this->getState('filter.ljf16_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf16_id);
			}
			else
			{
				if (is_array($ljf16_id))
				{
					JArrayHelper::toInteger($ljf16_id);
					$ljf16_id = implode(',', $ljf16_id);
					$type = $this->getState('filter.ljf16_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf16_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf16_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf16_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf16_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf16 params override menu item params only if menu param = 'use_ljf16'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf16' and there is no ljf16 param, use global

					// create an array of just the params set to 'use_ljf16'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf16')
						{
							// if the ljf16 has a value, use it
							if ($ljf16_params->get($key) != '')
							{
								// get the value from the ljf16
								$ljf16_array[$key] = $ljf16_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf16_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf16 params
					if (count($ljf16_array) > 0)
					{
						$ljf16_params = new Registry;
						$ljf16_params->loadArray($ljf16_array);
						$item->params->merge($ljf16_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf16_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf16, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
