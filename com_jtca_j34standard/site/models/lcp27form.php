...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp27.php';

class JtCaModelLcp27Form extends JtCaModelLcp27
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp27';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp27';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp27', 'lcp27', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno para incompetencias delitos no graves.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO PARA INCOMPETENCIAS DELITOS NO GRAVES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp27.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp27.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp27.id');
		$is_new		= true;
		...
		// Reorder the libro de turno para incompetencias delitos no grav so the new libro de turno para incompetencias delitos no graves is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp27BeforeSave', array('com_jtca.lcp27', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp27AfterSave', array('com_jtca.lcp27', $table, $is_new));

		$this->setState('lcp27.id', $table->id);
