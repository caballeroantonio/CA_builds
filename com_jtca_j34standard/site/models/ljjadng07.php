...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE OFICIOS Model
 *
 */
class JtcaModelLjjadng07 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljjadng07';
	...
	{
		if (empty($config['ljjadng07_filter_fields']))
		{
			$config['ljjadng07_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljjadng07s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE OFICIOS data.
	 *
	 * @param	integer	$pk	The id of the libro de oficios.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljjadng07.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJJADNG07S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjjadng07ChangeState', array('com_jtca.ljjadng07', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjjadng07BeforeDelete', array('com_jtca.ljjadng07', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjjadng07AfterDelete', array('com_jtca.ljjadng07', &$table));
				}
				else
				{
					// Prune items that you can't change.
