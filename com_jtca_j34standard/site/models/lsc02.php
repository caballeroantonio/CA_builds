...
class JtcaModelLsc02 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsc02';
	...
	{
		if (empty($config['lsc02_filter_fields']))
		{
			$config['lsc02_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	...
	/**
	 * Method to get LIBRETA DE REGISTRO DE ASISTENCIA (OFICIAL) data.
	 *
	 * @param	integer	$pk	The id of the libreta de registro de asistencia (oficial).
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsc02.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSC02S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsc02ChangeState', array('com_jtca.lsc02', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsc02BeforeDelete', array('com_jtca.lsc02', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsc02AfterDelete', array('com_jtca.lsc02', &$table));
				}
				else
				{
					// Prune items that you can't change.
