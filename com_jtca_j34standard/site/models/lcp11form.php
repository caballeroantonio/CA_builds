...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp11.php';

class JtCaModelLcp11Form extends JtCaModelLcp11
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp11';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp11';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp11', 'lcp11', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de oficios..
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE OFICIOS. item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp11.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp11.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp11.id');
		$is_new		= true;
		...
		// Reorder the libreta de oficios. so the new libreta de oficios. is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp11BeforeSave', array('com_jtca.lcp11', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp11AfterSave', array('com_jtca.lcp11', $table, $is_new));

		$this->setState('lcp11.id', $table->id);
