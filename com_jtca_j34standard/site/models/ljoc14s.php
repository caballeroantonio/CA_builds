...
class JtCaModelLjoc14s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljoc14_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljoc14_id');
		$this->setState('filter.ljoc14_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljoc14_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljoc14_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljoc14_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljoc14_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljoc14_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljoc14_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljoc14_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljoc14_id'));
		$id .= ':'.$this->getState('filter.ljoc14_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljoc14s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljoc14s').' AS a');
		...
		// Filter by a single or group of ljoc14s.
		$ljoc14_id = $this->getState('filter.ljoc14_id');
		if ($ljoc14_id != '')
		{
			if (is_numeric($ljoc14_id))
			{
				$type = $this->getState('filter.ljoc14_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljoc14_id);
			}
			else
			{
				if (is_array($ljoc14_id))
				{
					JArrayHelper::toInteger($ljoc14_id);
					$ljoc14_id = implode(',', $ljoc14_id);
					$type = $this->getState('filter.ljoc14_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljoc14_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljoc14_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljoc14_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljoc14_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljoc14 params override menu item params only if menu param = 'use_ljoc14'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljoc14' and there is no ljoc14 param, use global

					// create an array of just the params set to 'use_ljoc14'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljoc14')
						{
							// if the ljoc14 has a value, use it
							if ($ljoc14_params->get($key) != '')
							{
								// get the value from the ljoc14
								$ljoc14_array[$key] = $ljoc14_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljoc14_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljoc14 params
					if (count($ljoc14_array) > 0)
					{
						$ljoc14_params = new Registry;
						$ljoc14_params->loadArray($ljoc14_array);
						$item->params->merge($ljoc14_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljoc14_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljoc14, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
