...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljccm12.php';

class JtCaModelLjccm12Form extends JtCaModelLjccm12
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljccm12';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljccm12';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljccm12', 'ljccm12', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljccm12.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljccm12.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljccm12.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjccm12BeforeSave', array('com_jtca.ljccm12', &$table, $is_new));
		...
		$dispatcher->trigger('onLjccm12AfterSave', array('com_jtca.ljccm12', $table, $is_new));

		$this->setState('ljccm12.id', $table->id);
