...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadg10.php';

class JtCaModelLjjadg10Form extends JtCaModelLjjadg10
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadg10';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadg10';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadg10', 'ljjadg10', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadg10.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadg10.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadg10.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjjadg10BeforeSave', array('com_jtca.ljjadg10', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadg10AfterSave', array('com_jtca.ljjadg10', $table, $is_new));

		$this->setState('ljjadg10.id', $table->id);
