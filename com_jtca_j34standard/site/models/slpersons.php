...
/**
 * This models supports retrieving lists of persons.
 *
 */
class JtcaModelSlpersons extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.slpersons';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single sub libro personas has been specified either as a parameter or in the url Request
		$pk = $params->get('slperson_id', '') == '' ? $app->input->getInt('id', '') : $params->get('slperson_id');
		$this->setState('filter.slperson_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('slperson_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('slperson_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('slperson_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('slperson_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('slperson_initial_direction','ASC');
		}
		...
		if ($params->get('filter_slperson_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_slperson_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.slperson_id'));
		$id .= ':'.$this->getState('filter.slperson_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of slpersons subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_slpersons').' AS a');
		...
		// Filter by a single or group of slpersons.
		$slperson_id = $this->getState('filter.slperson_id');
		if ($slperson_id != '')
		{
			if (is_numeric($slperson_id))
			{
				$type = $this->getState('filter.slperson_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $slperson_id);
			}
			else
			{
				if (is_array($slperson_id))
				{
					JArrayHelper::toInteger($slperson_id);
					$slperson_id = implode(',', $slperson_id);
					$type = $this->getState('filter.slperson_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$slperson_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_slperson_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_slperson_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $slperson_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Slperson params override menu item params only if menu param = 'use_slperson'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_slperson' and there is no slperson param, use global

					// create an array of just the params set to 'use_slperson'
					$menu_params_array = $this->getState('params')->toArray();
					$slperson_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_slperson')
						{
							// if the slperson has a value, use it
							if ($slperson_params->get($key) != '')
							{
								// get the value from the slperson
								$slperson_array[$key] = $slperson_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$slperson_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected slperson params
					if (count($slperson_array) > 0)
					{
						$slperson_params = new Registry;
						$slperson_params->loadArray($slperson_array);
						$item->params->merge($slperson_params);
					}


					// get display date
					switch ($item->params->get('list_show_slperson_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an slperson, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
