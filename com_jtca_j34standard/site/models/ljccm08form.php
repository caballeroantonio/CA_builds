...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljccm08.php';

class JtCaModelLjccm08Form extends JtCaModelLjccm08
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljccm08';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljccm08';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljccm08', 'ljccm08', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljccm08.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljccm08.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljccm08.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjccm08BeforeSave', array('com_jtca.ljccm08', &$table, $is_new));
		...
		$dispatcher->trigger('onLjccm08AfterSave', array('com_jtca.ljccm08', $table, $is_new));

		$this->setState('ljccm08.id', $table->id);
