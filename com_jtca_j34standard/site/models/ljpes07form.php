...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpes07.php';

class JtCaModelLjpes07Form extends JtCaModelLjpes07
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpes07';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpes07';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpes07', 'ljpes07', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de amparos circulantes.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE AMPAROS CIRCULANTES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpes07.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpes07.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpes07.id');
		$is_new		= true;
		...
		// Reorder the libro de amparos circulantes so the new libro de amparos circulantes is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpes07BeforeSave', array('com_jtca.ljpes07', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpes07AfterSave', array('com_jtca.ljpes07', $table, $is_new));

		$this->setState('ljpes07.id', $table->id);
