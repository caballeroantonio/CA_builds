...
class JtCaModelLjf06s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf06_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf06_id');
		$this->setState('filter.ljf06_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf06_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf06_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf06_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf06_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf06_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf06_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf06_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf06_id'));
		$id .= ':'.$this->getState('filter.ljf06_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf06s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf06s').' AS a');
		...
		// Filter by a single or group of ljf06s.
		$ljf06_id = $this->getState('filter.ljf06_id');
		if ($ljf06_id != '')
		{
			if (is_numeric($ljf06_id))
			{
				$type = $this->getState('filter.ljf06_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf06_id);
			}
			else
			{
				if (is_array($ljf06_id))
				{
					JArrayHelper::toInteger($ljf06_id);
					$ljf06_id = implode(',', $ljf06_id);
					$type = $this->getState('filter.ljf06_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf06_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf06_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf06_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf06_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf06 params override menu item params only if menu param = 'use_ljf06'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf06' and there is no ljf06 param, use global

					// create an array of just the params set to 'use_ljf06'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf06')
						{
							// if the ljf06 has a value, use it
							if ($ljf06_params->get($key) != '')
							{
								// get the value from the ljf06
								$ljf06_array[$key] = $ljf06_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf06_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf06 params
					if (count($ljf06_array) > 0)
					{
						$ljf06_params = new Registry;
						$ljf06_params->loadArray($ljf06_array);
						$item->params->merge($ljf06_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf06_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf06, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
