...
class JtCaModelLjjadg08s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadg08_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg08_id');
		$this->setState('filter.ljjadg08_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg08_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg08_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg08_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg08_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg08_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg08_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg08_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg08_id'));
		$id .= ':'.$this->getState('filter.ljjadg08_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg08s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg08s').' AS a');
		...
		// Filter by a single or group of ljjadg08s.
		$ljjadg08_id = $this->getState('filter.ljjadg08_id');
		if ($ljjadg08_id != '')
		{
			if (is_numeric($ljjadg08_id))
			{
				$type = $this->getState('filter.ljjadg08_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg08_id);
			}
			else
			{
				if (is_array($ljjadg08_id))
				{
					JArrayHelper::toInteger($ljjadg08_id);
					$ljjadg08_id = implode(',', $ljjadg08_id);
					$type = $this->getState('filter.ljjadg08_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg08_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg08_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg08_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg08_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg08 params override menu item params only if menu param = 'use_ljjadg08'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg08' and there is no ljjadg08 param, use global

					// create an array of just the params set to 'use_ljjadg08'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg08_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg08')
						{
							// if the ljjadg08 has a value, use it
							if ($ljjadg08_params->get($key) != '')
							{
								// get the value from the ljjadg08
								$ljjadg08_array[$key] = $ljjadg08_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg08_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg08 params
					if (count($ljjadg08_array) > 0)
					{
						$ljjadg08_params = new Registry;
						$ljjadg08_params->loadArray($ljjadg08_array);
						$item->params->merge($ljjadg08_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg08_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg08, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
