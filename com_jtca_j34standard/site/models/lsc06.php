...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE CONTROL DE BILLETES Y PÓLIZAS (OFICIAL) Model
 *
 */
class JtcaModelLsc06 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsc06';
	...
	{
		if (empty($config['lsc06_filter_fields']))
		{
			$config['lsc06_filter_fields'] = array(
				'id', 'a.id',
				...
				'field153','a.field153',
				'field154','a.field154',
				'field155_isMoral','a.field155_isMoral',
				'field155_paterno','a.field155_paterno',
				'field155_materno','a.field155_materno',
				'field155_nombre','a.field155_nombre',
				'field157','a.field157',
				'field158_isMoral','a.field158_isMoral',
				'field158_paterno','a.field158_paterno',
				'field158_materno','a.field158_materno',
				'field158_nombre','a.field158_nombre',
				'field160','a.field160',
				'field159','a.field159',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lsc06s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE CONTROL DE BILLETES Y PÓLIZAS (OFICIAL) data.
	 *
	 * @param	integer	$pk	The id of the libro de control de billetes y pólizas (oficial).
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsc06.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSC06S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsc06ChangeState', array('com_jtca.lsc06', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsc06BeforeDelete', array('com_jtca.lsc06', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsc06AfterDelete', array('com_jtca.lsc06', &$table));
				}
				else
				{
					// Prune items that you can't change.
