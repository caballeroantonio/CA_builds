...
class JtCaModelLjomf04s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf04_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf04_id');
		$this->setState('filter.ljomf04_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf04_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf04_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf04_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf04_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf04_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf04_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf04_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf04_id'));
		$id .= ':'.$this->getState('filter.ljomf04_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf04s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf04s').' AS a');
		...
		// Filter by a single or group of ljomf04s.
		$ljomf04_id = $this->getState('filter.ljomf04_id');
		if ($ljomf04_id != '')
		{
			if (is_numeric($ljomf04_id))
			{
				$type = $this->getState('filter.ljomf04_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf04_id);
			}
			else
			{
				if (is_array($ljomf04_id))
				{
					JArrayHelper::toInteger($ljomf04_id);
					$ljomf04_id = implode(',', $ljomf04_id);
					$type = $this->getState('filter.ljomf04_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf04_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf04_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf04_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf04_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf04 params override menu item params only if menu param = 'use_ljomf04'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf04' and there is no ljomf04 param, use global

					// create an array of just the params set to 'use_ljomf04'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf04_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf04')
						{
							// if the ljomf04 has a value, use it
							if ($ljomf04_params->get($key) != '')
							{
								// get the value from the ljomf04
								$ljomf04_array[$key] = $ljomf04_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf04_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf04 params
					if (count($ljomf04_array) > 0)
					{
						$ljomf04_params = new Registry;
						$ljomf04_params->loadArray($ljomf04_array);
						$item->params->merge($ljomf04_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf04_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf04, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
