...
/**
 * This models supports retrieving lists of libro de ordenes para arraigos y cateos delitos no.
 *
 */
class JtCaModelLcp29s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp29s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de ordenes para arraigos y cateos delitos no graves has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp29_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp29_id');
		$this->setState('filter.lcp29_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp29_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp29_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp29_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp29_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp29_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp29_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp29_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp29_id'));
		$id .= ':'.$this->getState('filter.lcp29_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp29s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp29s').' AS a');
		...
		// Filter by a single or group of lcp29s.
		$lcp29_id = $this->getState('filter.lcp29_id');
		if ($lcp29_id != '')
		{
			if (is_numeric($lcp29_id))
			{
				$type = $this->getState('filter.lcp29_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp29_id);
			}
			else
			{
				if (is_array($lcp29_id))
				{
					JArrayHelper::toInteger($lcp29_id);
					$lcp29_id = implode(',', $lcp29_id);
					$type = $this->getState('filter.lcp29_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp29_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp29_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp29_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de ordenes para arraigos y cateos delitos no.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp29_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp29 params override menu item params only if menu param = 'use_lcp29'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp29' and there is no lcp29 param, use global

					// create an array of just the params set to 'use_lcp29'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp29')
						{
							// if the lcp29 has a value, use it
							if ($lcp29_params->get($key) != '')
							{
								// get the value from the lcp29
								$lcp29_array[$key] = $lcp29_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp29_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp29 params
					if (count($lcp29_array) > 0)
					{
						$lcp29_params = new Registry;
						$lcp29_params->loadArray($lcp29_array);
						$item->params->merge($lcp29_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp29_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp29, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
