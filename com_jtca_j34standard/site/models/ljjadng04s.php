...
/**
 * This models supports retrieving lists of libro de sentencias.
 *
 */
class JtCaModelLjjadng04s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadng04s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de sentencias has been specified either as a parameter or in the url Request
		$pk = $params->get('ljjadng04_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadng04_id');
		$this->setState('filter.ljjadng04_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadng04_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadng04_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadng04_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadng04_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadng04_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadng04_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadng04_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadng04_id'));
		$id .= ':'.$this->getState('filter.ljjadng04_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadng04s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadng04s').' AS a');
		...
		// Filter by a single or group of ljjadng04s.
		$ljjadng04_id = $this->getState('filter.ljjadng04_id');
		if ($ljjadng04_id != '')
		{
			if (is_numeric($ljjadng04_id))
			{
				$type = $this->getState('filter.ljjadng04_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadng04_id);
			}
			else
			{
				if (is_array($ljjadng04_id))
				{
					JArrayHelper::toInteger($ljjadng04_id);
					$ljjadng04_id = implode(',', $ljjadng04_id);
					$type = $this->getState('filter.ljjadng04_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadng04_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadng04_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadng04_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de sentencias.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadng04_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadng04 params override menu item params only if menu param = 'use_ljjadng04'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadng04' and there is no ljjadng04 param, use global

					// create an array of just the params set to 'use_ljjadng04'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadng04_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadng04')
						{
							// if the ljjadng04 has a value, use it
							if ($ljjadng04_params->get($key) != '')
							{
								// get the value from the ljjadng04
								$ljjadng04_array[$key] = $ljjadng04_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadng04_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadng04 params
					if (count($ljjadng04_array) > 0)
					{
						$ljjadng04_params = new Registry;
						$ljjadng04_params->loadArray($ljjadng04_array);
						$item->params->merge($ljjadng04_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadng04_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadng04, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
