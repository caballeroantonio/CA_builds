...
/**
 * This models supports retrieving lists of adolescentes.
 *
 */
class JtcaModelSlpersonaliasedades extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.slpersonaliasedades';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single adolescentes has been specified either as a parameter or in the url Request
		$pk = $params->get('slpersonaliasedad_id', '') == '' ? $app->input->getInt('id', '') : $params->get('slpersonaliasedad_id');
		$this->setState('filter.slpersonaliasedad_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('slpersonaliasedad_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('slpersonaliasedad_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('slpersonaliasedad_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('slpersonaliasedad_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('slpersonaliasedad_initial_direction','ASC');
		}
		...
		if ($params->get('filter_slpersonaliasedad_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_slpersonaliasedad_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.slpersonaliasedad_id'));
		$id .= ':'.$this->getState('filter.slpersonaliasedad_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of slpersonaliasedades subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_slpersonaliasedades').' AS a');
		...
		// Filter by a single or group of slpersonaliasedades.
		$slpersonaliasedad_id = $this->getState('filter.slpersonaliasedad_id');
		if ($slpersonaliasedad_id != '')
		{
			if (is_numeric($slpersonaliasedad_id))
			{
				$type = $this->getState('filter.slpersonaliasedad_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $slpersonaliasedad_id);
			}
			else
			{
				if (is_array($slpersonaliasedad_id))
				{
					JArrayHelper::toInteger($slpersonaliasedad_id);
					$slpersonaliasedad_id = implode(',', $slpersonaliasedad_id);
					$type = $this->getState('filter.slpersonaliasedad_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$slpersonaliasedad_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_slpersonaliasedad_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_slpersonaliasedad_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of adolescentes.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$slpersonaliasedad_params = new Registry;
				...
				{
					$item->params = $slpersonaliasedad_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Slpersonaliasedad params override menu item params only if menu param = 'use_slpersonaliasedad'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_slpersonaliasedad' and there is no slpersonaliasedad param, use global

					// create an array of just the params set to 'use_slpersonaliasedad'
					$menu_params_array = $this->getState('params')->toArray();
					$slpersonaliasedad_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_slpersonaliasedad')
						{
							// if the slpersonaliasedad has a value, use it
							if ($slpersonaliasedad_params->get($key) != '')
							{
								// get the value from the slpersonaliasedad
								$slpersonaliasedad_array[$key] = $slpersonaliasedad_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$slpersonaliasedad_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected slpersonaliasedad params
					if (count($slpersonaliasedad_array) > 0)
					{
						$slpersonaliasedad_params = new Registry;
						$slpersonaliasedad_params->loadArray($slpersonaliasedad_array);
						$item->params->merge($slpersonaliasedad_params);
					}


					// get display date
					switch ($item->params->get('list_show_slpersonaliasedad_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an slpersonaliasedad, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
