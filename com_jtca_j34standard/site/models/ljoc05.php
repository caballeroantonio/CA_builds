 ...
 *
 */
class JtcaModelLjoc05 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljoc05';
	...
	{
		if (empty($config['ljoc05_filter_fields']))
		{
			$config['ljoc05_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljoc05s', $prefix = 'JtcaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljoc05.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJOC05S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjoc05ChangeState', array('com_jtca.ljoc05', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjoc05BeforeDelete', array('com_jtca.ljoc05', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjoc05AfterDelete', array('com_jtca.ljoc05', &$table));
				}
				else
				{
					// Prune items that you can't change.
