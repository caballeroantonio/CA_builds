...
/**
 * This models supports retrieving lists of libro de control de firmas procesados.
 *
 */
class JtCaModelLjpdng17s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpdng17s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de control de firmas procesados has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpdng17_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng17_id');
		$this->setState('filter.ljpdng17_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng17_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng17_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng17_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng17_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng17_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng17_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng17_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng17_id'));
		$id .= ':'.$this->getState('filter.ljpdng17_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng17s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng17s').' AS a');
		...
		// Filter by a single or group of ljpdng17s.
		$ljpdng17_id = $this->getState('filter.ljpdng17_id');
		if ($ljpdng17_id != '')
		{
			if (is_numeric($ljpdng17_id))
			{
				$type = $this->getState('filter.ljpdng17_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng17_id);
			}
			else
			{
				if (is_array($ljpdng17_id))
				{
					JArrayHelper::toInteger($ljpdng17_id);
					$ljpdng17_id = implode(',', $ljpdng17_id);
					$type = $this->getState('filter.ljpdng17_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng17_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng17_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng17_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de control de firmas procesados.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng17_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng17 params override menu item params only if menu param = 'use_ljpdng17'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng17' and there is no ljpdng17 param, use global

					// create an array of just the params set to 'use_ljpdng17'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng17_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng17')
						{
							// if the ljpdng17 has a value, use it
							if ($ljpdng17_params->get($key) != '')
							{
								// get the value from the ljpdng17
								$ljpdng17_array[$key] = $ljpdng17_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng17_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng17 params
					if (count($ljpdng17_array) > 0)
					{
						$ljpdng17_params = new Registry;
						$ljpdng17_params->loadArray($ljpdng17_array);
						$item->params->merge($ljpdng17_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng17_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng17, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
