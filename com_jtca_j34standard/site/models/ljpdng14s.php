...
class JtCaModelLjpdng14s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpdng14_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng14_id');
		$this->setState('filter.ljpdng14_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng14_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng14_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng14_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng14_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng14_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng14_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng14_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng14_id'));
		$id .= ':'.$this->getState('filter.ljpdng14_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng14s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng14s').' AS a');
		...
		// Filter by a single or group of ljpdng14s.
		$ljpdng14_id = $this->getState('filter.ljpdng14_id');
		if ($ljpdng14_id != '')
		{
			if (is_numeric($ljpdng14_id))
			{
				$type = $this->getState('filter.ljpdng14_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng14_id);
			}
			else
			{
				if (is_array($ljpdng14_id))
				{
					JArrayHelper::toInteger($ljpdng14_id);
					$ljpdng14_id = implode(',', $ljpdng14_id);
					$type = $this->getState('filter.ljpdng14_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng14_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng14_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng14_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng14_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng14 params override menu item params only if menu param = 'use_ljpdng14'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng14' and there is no ljpdng14 param, use global

					// create an array of just the params set to 'use_ljpdng14'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng14_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng14')
						{
							// if the ljpdng14 has a value, use it
							if ($ljpdng14_params->get($key) != '')
							{
								// get the value from the ljpdng14
								$ljpdng14_array[$key] = $ljpdng14_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng14_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng14 params
					if (count($ljpdng14_array) > 0)
					{
						$ljpdng14_params = new Registry;
						$ljpdng14_params->loadArray($ljpdng14_array);
						$item->params->merge($ljpdng14_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng14_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng14, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
