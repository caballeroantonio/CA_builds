...
/**
 * This models supports retrieving lists of libro de finanzas.
 *
 */
class JtcaModelLspe07s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lspe07s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de finanzas has been specified either as a parameter or in the url Request
		$pk = $params->get('lspe07_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lspe07_id');
		$this->setState('filter.lspe07_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lspe07_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lspe07_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lspe07_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lspe07_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lspe07_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lspe07_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lspe07_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lspe07_id'));
		$id .= ':'.$this->getState('filter.lspe07_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lspe07s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lspe07s').' AS a');
		...
		// Filter by a single or group of lspe07s.
		$lspe07_id = $this->getState('filter.lspe07_id');
		if ($lspe07_id != '')
		{
			if (is_numeric($lspe07_id))
			{
				$type = $this->getState('filter.lspe07_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lspe07_id);
			}
			else
			{
				if (is_array($lspe07_id))
				{
					JArrayHelper::toInteger($lspe07_id);
					$lspe07_id = implode(',', $lspe07_id);
					$type = $this->getState('filter.lspe07_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lspe07_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lspe07_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lspe07_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de finanzas.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lspe07_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lspe07 params override menu item params only if menu param = 'use_lspe07'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lspe07' and there is no lspe07 param, use global

					// create an array of just the params set to 'use_lspe07'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lspe07')
						{
							// if the lspe07 has a value, use it
							if ($lspe07_params->get($key) != '')
							{
								// get the value from the lspe07
								$lspe07_array[$key] = $lspe07_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lspe07_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lspe07 params
					if (count($lspe07_array) > 0)
					{
						$lspe07_params = new Registry;
						$lspe07_params->loadArray($lspe07_array);
						$item->params->merge($lspe07_params);
					}


					// get display date
					switch ($item->params->get('list_show_lspe07_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lspe07, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
