...
class JtCaModelLjccm01s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljccm01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljccm01_id');
		$this->setState('filter.ljccm01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljccm01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljccm01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljccm01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljccm01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljccm01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljccm01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljccm01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljccm01_id'));
		$id .= ':'.$this->getState('filter.ljccm01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljccm01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljccm01s').' AS a');
		...
		// Filter by a single or group of ljccm01s.
		$ljccm01_id = $this->getState('filter.ljccm01_id');
		if ($ljccm01_id != '')
		{
			if (is_numeric($ljccm01_id))
			{
				$type = $this->getState('filter.ljccm01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljccm01_id);
			}
			else
			{
				if (is_array($ljccm01_id))
				{
					JArrayHelper::toInteger($ljccm01_id);
					$ljccm01_id = implode(',', $ljccm01_id);
					$type = $this->getState('filter.ljccm01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljccm01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljccm01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljccm01_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljccm01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljccm01 params override menu item params only if menu param = 'use_ljccm01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljccm01' and there is no ljccm01 param, use global

					// create an array of just the params set to 'use_ljccm01'
					$menu_params_array = $this->getState('params')->toArray();
					$ljccm01_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljccm01')
						{
							// if the ljccm01 has a value, use it
							if ($ljccm01_params->get($key) != '')
							{
								// get the value from the ljccm01
								$ljccm01_array[$key] = $ljccm01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljccm01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljccm01 params
					if (count($ljccm01_array) > 0)
					{
						$ljccm01_params = new Registry;
						$ljccm01_params->loadArray($ljccm01_array);
						$item->params->merge($ljccm01_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljccm01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljccm01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
