...
/**
 * This models supports retrieving lists of libro de ordenes de detenciÃ³n para juzgados de jus.
 *
 */
class JtCaModelLcp40s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp40s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de ordenes de detenciÃ³n para juzgados de justicia para adolescentes has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp40_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp40_id');
		$this->setState('filter.lcp40_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp40_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp40_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp40_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp40_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp40_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp40_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp40_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp40_id'));
		$id .= ':'.$this->getState('filter.lcp40_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp40s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp40s').' AS a');
		...
		// Filter by a single or group of lcp40s.
		$lcp40_id = $this->getState('filter.lcp40_id');
		if ($lcp40_id != '')
		{
			if (is_numeric($lcp40_id))
			{
				$type = $this->getState('filter.lcp40_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp40_id);
			}
			else
			{
				if (is_array($lcp40_id))
				{
					JArrayHelper::toInteger($lcp40_id);
					$lcp40_id = implode(',', $lcp40_id);
					$type = $this->getState('filter.lcp40_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp40_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp40_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp40_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de ordenes de detenciÃ³n para juzgados de jus.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp40_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp40 params override menu item params only if menu param = 'use_lcp40'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp40' and there is no lcp40 param, use global

					// create an array of just the params set to 'use_lcp40'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp40')
						{
							// if the lcp40 has a value, use it
							if ($lcp40_params->get($key) != '')
							{
								// get the value from the lcp40
								$lcp40_array[$key] = $lcp40_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp40_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp40 params
					if (count($lcp40_array) > 0)
					{
						$lcp40_params = new Registry;
						$lcp40_params->loadArray($lcp40_array);
						$item->params->merge($lcp40_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp40_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp40, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
