...
/**
 * This models supports retrieving lists of libro de registro de la ley de acceso a las mujere.
 *
 */
class JtCaModelLcp24s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp24s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de registro de la ley de acceso a las mujeres a una vida libre de violencia. has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp24_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp24_id');
		$this->setState('filter.lcp24_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp24_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp24_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp24_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp24_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp24_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp24_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp24_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp24_id'));
		$id .= ':'.$this->getState('filter.lcp24_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp24s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp24s').' AS a');
		...
		// Filter by a single or group of lcp24s.
		$lcp24_id = $this->getState('filter.lcp24_id');
		if ($lcp24_id != '')
		{
			if (is_numeric($lcp24_id))
			{
				$type = $this->getState('filter.lcp24_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp24_id);
			}
			else
			{
				if (is_array($lcp24_id))
				{
					JArrayHelper::toInteger($lcp24_id);
					$lcp24_id = implode(',', $lcp24_id);
					$type = $this->getState('filter.lcp24_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp24_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp24_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp24_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de registro de la ley de acceso a las mujere.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp24_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp24 params override menu item params only if menu param = 'use_lcp24'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp24' and there is no lcp24 param, use global

					// create an array of just the params set to 'use_lcp24'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp24')
						{
							// if the lcp24 has a value, use it
							if ($lcp24_params->get($key) != '')
							{
								// get the value from the lcp24
								$lcp24_array[$key] = $lcp24_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp24_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp24 params
					if (count($lcp24_array) > 0)
					{
						$lcp24_params = new Registry;
						$lcp24_params->loadArray($lcp24_array);
						$item->params->merge($lcp24_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp24_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp24, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
