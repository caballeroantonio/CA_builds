...
/**
 * This models supports retrieving lists of libro de turno de remisiones sin detenido de juzga.
 *
 */
class JtCaModelLcp41s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp41s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno de remisiones sin detenido de juzgados adolescentes has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp41_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp41_id');
		$this->setState('filter.lcp41_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp41_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp41_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp41_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp41_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp41_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp41_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp41_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp41_id'));
		$id .= ':'.$this->getState('filter.lcp41_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp41s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp41s').' AS a');
		...
		// Filter by a single or group of lcp41s.
		$lcp41_id = $this->getState('filter.lcp41_id');
		if ($lcp41_id != '')
		{
			if (is_numeric($lcp41_id))
			{
				$type = $this->getState('filter.lcp41_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp41_id);
			}
			else
			{
				if (is_array($lcp41_id))
				{
					JArrayHelper::toInteger($lcp41_id);
					$lcp41_id = implode(',', $lcp41_id);
					$type = $this->getState('filter.lcp41_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp41_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp41_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp41_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno de remisiones sin detenido de juzga.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp41_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp41 params override menu item params only if menu param = 'use_lcp41'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp41' and there is no lcp41 param, use global

					// create an array of just the params set to 'use_lcp41'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp41')
						{
							// if the lcp41 has a value, use it
							if ($lcp41_params->get($key) != '')
							{
								// get the value from the lcp41
								$lcp41_array[$key] = $lcp41_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp41_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp41 params
					if (count($lcp41_array) > 0)
					{
						$lcp41_params = new Registry;
						$lcp41_params->loadArray($lcp41_array);
						$item->params->merge($lcp41_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp41_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp41, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
