...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljp19.php';

class JtcaModelLjp19Form extends JtcaModelLjp19
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljp19';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljp19';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljp19', 'ljp19', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de control de firmas de procesados.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE CONTROL DE FIRMAS DE PROCESADOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljp19.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljp19.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljp19.id');
		$is_new		= true;
		...
		// Reorder the libro de control de firmas de procesados so the new libro de control de firmas de procesados is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjp19BeforeSave', array('com_jtca.ljp19', &$table, $is_new));
		...
		$dispatcher->trigger('onLjp19AfterSave', array('com_jtca.ljp19', $table, $is_new));

		$this->setState('ljp19.id', $table->id);
