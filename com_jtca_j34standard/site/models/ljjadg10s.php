...
class JtCaModelLjjadg10s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadg10_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg10_id');
		$this->setState('filter.ljjadg10_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg10_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg10_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg10_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg10_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg10_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg10_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg10_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg10_id'));
		$id .= ':'.$this->getState('filter.ljjadg10_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg10s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg10s').' AS a');
		...
		// Filter by a single or group of ljjadg10s.
		$ljjadg10_id = $this->getState('filter.ljjadg10_id');
		if ($ljjadg10_id != '')
		{
			if (is_numeric($ljjadg10_id))
			{
				$type = $this->getState('filter.ljjadg10_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg10_id);
			}
			else
			{
				if (is_array($ljjadg10_id))
				{
					JArrayHelper::toInteger($ljjadg10_id);
					$ljjadg10_id = implode(',', $ljjadg10_id);
					$type = $this->getState('filter.ljjadg10_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg10_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg10_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg10_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg10_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg10 params override menu item params only if menu param = 'use_ljjadg10'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg10' and there is no ljjadg10 param, use global

					// create an array of just the params set to 'use_ljjadg10'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg10_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg10')
						{
							// if the ljjadg10 has a value, use it
							if ($ljjadg10_params->get($key) != '')
							{
								// get the value from the ljjadg10
								$ljjadg10_array[$key] = $ljjadg10_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg10_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg10 params
					if (count($ljjadg10_array) > 0)
					{
						$ljjadg10_params = new Registry;
						$ljjadg10_params->loadArray($ljjadg10_array);
						$item->params->merge($ljjadg10_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg10_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg10, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
