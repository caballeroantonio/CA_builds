...
class JtCaModelLjc21 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljc21';
	...
	{
		if (empty($config['ljc21_filter_fields']))
		{
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljc21.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJC21S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjc21ChangeState', array('com_jtca.ljc21', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjc21BeforeDelete', array('com_jtca.ljc21', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjc21AfterDelete', array('com_jtca.ljc21', &$table));
				}
				else
				{
					// Prune items that you can't change.
