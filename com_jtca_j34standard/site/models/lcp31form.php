...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp31.php';

class JtCaModelLcp31Form extends JtCaModelLcp31
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp31';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp31';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp31', 'lcp31', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de libertades juzgados de delitos no graves.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE LIBERTADES JUZGADOS DE DELITOS NO GRAVES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp31.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp31.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp31.id');
		$is_new		= true;
		...
		// Reorder the libreta de libertades juzgados de delitos no grave so the new libreta de libertades juzgados de delitos no graves is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp31BeforeSave', array('com_jtca.lcp31', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp31AfterSave', array('com_jtca.lcp31', $table, $is_new));

		$this->setState('lcp31.id', $table->id);
