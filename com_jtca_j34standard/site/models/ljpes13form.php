...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpes13.php';

class JtCaModelLjpes13Form extends JtCaModelLjpes13
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpes13';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpes13';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpes13', 'ljpes13', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de resoluciones en audiencia.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE RESOLUCIONES EN AUDIENCIA item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpes13.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpes13.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpes13.id');
		$is_new		= true;
		...
		// Reorder the libro de resoluciones en audiencia so the new libro de resoluciones en audiencia is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpes13BeforeSave', array('com_jtca.ljpes13', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpes13AfterSave', array('com_jtca.ljpes13', $table, $is_new));

		$this->setState('ljpes13.id', $table->id);
