...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpes14.php';

class JtCaModelLjpes14Form extends JtCaModelLjpes14
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpes14';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpes14';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpes14', 'ljpes14', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de resoluciones de plano.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE RESOLUCIONES DE PLANO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpes14.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpes14.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpes14.id');
		$is_new		= true;
		...
		// Reorder the libro de resoluciones de plano so the new libro de resoluciones de plano is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpes14BeforeSave', array('com_jtca.ljpes14', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpes14AfterSave', array('com_jtca.ljpes14', $table, $is_new));

		$this->setState('ljpes14.id', $table->id);
