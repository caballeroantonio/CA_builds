...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljc07.php';

class JtCaModelLjc07Form extends JtCaModelLjc07
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljc07';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljc07';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljc07', 'ljc07', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljc07.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljc07.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljc07.id');
		$is_new		= true;
		...
		// Reorder the jc07 so the new libro de exhortos is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjc07BeforeSave', array('com_jtca.ljc07', &$table, $is_new));
		...
		$dispatcher->trigger('onLjc07AfterSave', array('com_jtca.ljc07', $table, $is_new));

		$this->setState('ljc07.id', $table->id);
