...
/**
 * TSJ CDMX Libros TxCA Component Expedientes Model
 *
 */
class JtcaModelExpediente extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.expediente';
	...
	{
		if (empty($config['expediente_filter_fields']))
		{
			$config['expediente_filter_fields'] = array(
				'id', 'a.id',
				'name', 'a.name',
				...
				'bis','a.bis',
				'nrecurso','a.nrecurso',
				'id_tipoarchivo','a.id_tipoarchivo',
				'txt_tipojuicio','a.txt_tipojuicio',
				'id_tipojuicio','a.id_tipojuicio',
				'partescontenciosas','a.partescontenciosas',
				'id_opc','a.id_opc',
				'txt_opc','a.txt_opc',
				'delito','a.delito',
				'pena','a.pena',
				'id_exp_orig','a.id_exp_orig',
				'state', 'a.state',
				'created', 'a.created',
	...
	protected function populateState()
	{
		...
		$this->setState('expediente.id', $pk);
	...
	*/
	public function getTable($type = 'Expedientes', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get Expedientes data.
	 *
	 * @param	integer	$pk	The id of the expedientes.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('expediente.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_EXPEDIENTES_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onExpedienteChangeState', array('com_jtca.expediente', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onExpedienteBeforeDelete', array('com_jtca.expediente', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onExpedienteAfterDelete', array('com_jtca.expediente', &$table));
				}
				else
				{
					// Prune items that you can't change.
