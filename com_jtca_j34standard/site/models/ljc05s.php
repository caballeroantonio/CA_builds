...
/**
 * This models supports retrieving lists of jc05.
 *
 */
class JtCaModelLjc05s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljc05s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno para sentencia has been specified either as a parameter or in the url Request
		$pk = $params->get('ljc05_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc05_id');
		$this->setState('filter.ljc05_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc05_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc05_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc05_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc05_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc05_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc05_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc05_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc05_id'));
		$id .= ':'.$this->getState('filter.ljc05_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc05s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc05s').' AS a');
		...
		// Filter by a single or group of ljc05s.
		$ljc05_id = $this->getState('filter.ljc05_id');
		if ($ljc05_id != '')
		{
			if (is_numeric($ljc05_id))
			{
				$type = $this->getState('filter.ljc05_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc05_id);
			}
			else
			{
				if (is_array($ljc05_id))
				{
					JArrayHelper::toInteger($ljc05_id);
					$ljc05_id = implode(',', $ljc05_id);
					$type = $this->getState('filter.ljc05_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc05_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc05_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc05_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc05_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc05 params override menu item params only if menu param = 'use_ljc05'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc05' and there is no ljc05 param, use global

					// create an array of just the params set to 'use_ljc05'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc05')
						{
							// if the ljc05 has a value, use it
							if ($ljc05_params->get($key) != '')
							{
								// get the value from the ljc05
								$ljc05_array[$key] = $ljc05_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc05_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc05 params
					if (count($ljc05_array) > 0)
					{
						$ljc05_params = new Registry;
						$ljc05_params->loadArray($ljc05_array);
						$item->params->merge($ljc05_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc05_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc05, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
