...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljc09.php';

class JtCaModelLjc09Form extends JtCaModelLjc09
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljc09';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljc09';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljc09', 'ljc09', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljc09.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljc09.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljc09.id');
		$is_new		= true;
		...
		// Reorder the jc09 so the new libro de actuarios is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjc09BeforeSave', array('com_jtca.ljc09', &$table, $is_new));
		...
		$dispatcher->trigger('onLjc09AfterSave', array('com_jtca.ljc09', $table, $is_new));

		$this->setState('ljc09.id', $table->id);
