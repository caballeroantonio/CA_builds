...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljccm06.php';

class JtCaModelLjccm06Form extends JtCaModelLjccm06
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljccm06';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljccm06';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljccm06', 'ljccm06', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljccm06.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljccm06.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljccm06.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjccm06BeforeSave', array('com_jtca.ljccm06', &$table, $is_new));
		...
		$dispatcher->trigger('onLjccm06AfterSave', array('com_jtca.ljccm06', $table, $is_new));

		$this->setState('ljccm06.id', $table->id);
