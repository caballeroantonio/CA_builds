...
class JtCaModelLsps09s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('lsps09_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsps09_id');
		$this->setState('filter.lsps09_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsps09_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsps09_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsps09_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsps09_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsps09_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsps09_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsps09_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsps09_id'));
		$id .= ':'.$this->getState('filter.lsps09_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsps09s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsps09s').' AS a');
		...
		// Filter by a single or group of lsps09s.
		$lsps09_id = $this->getState('filter.lsps09_id');
		if ($lsps09_id != '')
		{
			if (is_numeric($lsps09_id))
			{
				$type = $this->getState('filter.lsps09_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsps09_id);
			}
			else
			{
				if (is_array($lsps09_id))
				{
					JArrayHelper::toInteger($lsps09_id);
					$lsps09_id = implode(',', $lsps09_id);
					$type = $this->getState('filter.lsps09_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsps09_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsps09_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsps09_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsps09_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsps09 params override menu item params only if menu param = 'use_lsps09'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsps09' and there is no lsps09 param, use global

					// create an array of just the params set to 'use_lsps09'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsps09')
						{
							// if the lsps09 has a value, use it
							if ($lsps09_params->get($key) != '')
							{
								// get the value from the lsps09
								$lsps09_array[$key] = $lsps09_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsps09_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsps09 params
					if (count($lsps09_array) > 0)
					{
						$lsps09_params = new Registry;
						$lsps09_params->loadArray($lsps09_array);
						$item->params->merge($lsps09_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsps09_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsps09, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
