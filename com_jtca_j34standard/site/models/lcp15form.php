...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp15.php';

class JtCaModelLcp15Form extends JtCaModelLcp15
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp15';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp15';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp15', 'lcp15', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de oficios de amparos de tramite..
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE OFICIOS DE AMPAROS DE TRAMITE. item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp15.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp15.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp15.id');
		$is_new		= true;
		...
		// Reorder the libreta de oficios de amparos de tramite. so the new libreta de oficios de amparos de tramite. is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp15BeforeSave', array('com_jtca.lcp15', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp15AfterSave', array('com_jtca.lcp15', $table, $is_new));

		$this->setState('lcp15.id', $table->id);
