...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE TURNO PARA CONSIGNACIONES CON DETENIDO PRIMERA INSTANCIA. Model
 *
 */
class JtcaModelLcp18 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp18';
	...
	{
		if (empty($config['lcp18_filter_fields']))
		{
			$config['lcp18_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp18s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE TURNO PARA CONSIGNACIONES CON DETENIDO PRIMERA INSTANCIA. data.
	 *
	 * @param	integer	$pk	The id of the libro de turno para consignaciones con detenido primera instancia..
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp18.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP18S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp18ChangeState', array('com_jtca.lcp18', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp18BeforeDelete', array('com_jtca.lcp18', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp18AfterDelete', array('com_jtca.lcp18', &$table));
				}
				else
				{
					// Prune items that you can't change.
