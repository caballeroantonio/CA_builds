...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsc11.php';

class JtCaModelLsc11Form extends JtCaModelLsc11
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsc11';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsc11';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsc11', 'lsc11', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de oficios (oficial).
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE OFICIOS (OFICIAL) item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsc11.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsc11.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsc11.id');
		$is_new		= true;
		...
		// Reorder the libreta de oficios (oficial) so the new libreta de oficios (oficial) is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsc11BeforeSave', array('com_jtca.lsc11', &$table, $is_new));
		...
		$dispatcher->trigger('onLsc11AfterSave', array('com_jtca.lsc11', $table, $is_new));

		$this->setState('lsc11.id', $table->id);
