...
class JtCaModelLjccm06s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljccm06_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljccm06_id');
		$this->setState('filter.ljccm06_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljccm06_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljccm06_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljccm06_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljccm06_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljccm06_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljccm06_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljccm06_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljccm06_id'));
		$id .= ':'.$this->getState('filter.ljccm06_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljccm06s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljccm06s').' AS a');
		...
		// Filter by a single or group of ljccm06s.
		$ljccm06_id = $this->getState('filter.ljccm06_id');
		if ($ljccm06_id != '')
		{
			if (is_numeric($ljccm06_id))
			{
				$type = $this->getState('filter.ljccm06_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljccm06_id);
			}
			else
			{
				if (is_array($ljccm06_id))
				{
					JArrayHelper::toInteger($ljccm06_id);
					$ljccm06_id = implode(',', $ljccm06_id);
					$type = $this->getState('filter.ljccm06_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljccm06_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljccm06_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljccm06_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljccm06_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljccm06 params override menu item params only if menu param = 'use_ljccm06'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljccm06' and there is no ljccm06 param, use global

					// create an array of just the params set to 'use_ljccm06'
					$menu_params_array = $this->getState('params')->toArray();
					$ljccm06_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljccm06')
						{
							// if the ljccm06 has a value, use it
							if ($ljccm06_params->get($key) != '')
							{
								// get the value from the ljccm06
								$ljccm06_array[$key] = $ljccm06_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljccm06_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljccm06 params
					if (count($ljccm06_array) > 0)
					{
						$ljccm06_params = new Registry;
						$ljccm06_params->loadArray($ljccm06_array);
						$item->params->merge($ljccm06_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljccm06_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljccm06, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
