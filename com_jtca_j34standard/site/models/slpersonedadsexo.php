...
/**
 * TSJ CDMX Libros TxCA Component Adolescente Model
 *
 */
class JtcaModelSlpersonedadsexo extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.slpersonedadsexo';
	...
	{
		if (empty($config['slpersonedadsexo_filter_fields']))
		{
			$config['slpersonedadsexo_filter_fields'] = array(
				'id', 'a.id',
	...
	protected function populateState()
	{
		...
		$this->setState('slpersonedadsexo.id', $pk);
	...
	*/
	public function getTable($type = 'Slpersonedadsexos', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get Adolescente data.
	 *
	 * @param	integer	$pk	The id of the adolescente.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('slpersonedadsexo.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_SLPERSONEDADSEXOS_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onSlpersonedadsexoChangeState', array('com_jtca.slpersonedadsexo', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onSlpersonedadsexoBeforeDelete', array('com_jtca.slpersonedadsexo', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onSlpersonedadsexoAfterDelete', array('com_jtca.slpersonedadsexo', &$table));
				}
				else
				{
					// Prune items that you can't change.
