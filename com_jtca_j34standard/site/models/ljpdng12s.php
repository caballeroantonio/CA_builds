...
/**
 * This models supports retrieving lists of libro de control de fianzas.
 *
 */
class JtcaModelLjpdng12s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpdng12s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpdng12_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng12_id');
		$this->setState('filter.ljpdng12_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng12_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng12_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng12_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng12_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng12_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng12_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng12_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng12_id'));
		$id .= ':'.$this->getState('filter.ljpdng12_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng12s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng12s').' AS a');
		...
		// Filter by a single or group of ljpdng12s.
		$ljpdng12_id = $this->getState('filter.ljpdng12_id');
		if ($ljpdng12_id != '')
		{
			if (is_numeric($ljpdng12_id))
			{
				$type = $this->getState('filter.ljpdng12_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng12_id);
			}
			else
			{
				if (is_array($ljpdng12_id))
				{
					JArrayHelper::toInteger($ljpdng12_id);
					$ljpdng12_id = implode(',', $ljpdng12_id);
					$type = $this->getState('filter.ljpdng12_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng12_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng12_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng12_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de control de fianzas.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng12_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng12 params override menu item params only if menu param = 'use_ljpdng12'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng12' and there is no ljpdng12 param, use global

					// create an array of just the params set to 'use_ljpdng12'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng12_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng12')
						{
							// if the ljpdng12 has a value, use it
							if ($ljpdng12_params->get($key) != '')
							{
								// get the value from the ljpdng12
								$ljpdng12_array[$key] = $ljpdng12_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng12_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng12 params
					if (count($ljpdng12_array) > 0)
					{
						$ljpdng12_params = new Registry;
						$ljpdng12_params->loadArray($ljpdng12_array);
						$item->params->merge($ljpdng12_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng12_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng12, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
