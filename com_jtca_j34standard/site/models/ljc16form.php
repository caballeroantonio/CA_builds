...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljc16.php';

class JtCaModelLjc16Form extends JtCaModelLjc16
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljc16';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljc16';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljc16', 'ljc16', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljc16.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljc16.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljc16.id');
		$is_new		= true;
		...
		// Reorder the jc16 so the new agenda de audiencias is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjc16BeforeSave', array('com_jtca.ljc16', &$table, $is_new));
		...
		$dispatcher->trigger('onLjc16AfterSave', array('com_jtca.ljc16', $table, $is_new));

		$this->setState('ljc16.id', $table->id);
