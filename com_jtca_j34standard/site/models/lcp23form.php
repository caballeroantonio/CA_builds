...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp23.php';

class JtcaModelLcp23Form extends JtcaModelLcp23
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp23';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp23';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp23', 'lcp23', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de ordenes de aprehensión, reaprehension y traslado..
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE ORDENES DE APREHENSIÓN, REAPREHENSION Y TRASLADO. item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp23.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp23.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp23.id');
		$is_new		= true;
		...
		// Reorder the libro de ordenes de aprehensión, reaprehension y t so the new libro de ordenes de aprehensión, reaprehension y traslado. is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp23BeforeSave', array('com_jtca.lcp23', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp23AfterSave', array('com_jtca.lcp23', $table, $is_new));

		$this->setState('lcp23.id', $table->id);
