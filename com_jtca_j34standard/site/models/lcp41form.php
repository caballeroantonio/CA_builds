...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp41.php';

class JtCaModelLcp41Form extends JtCaModelLcp41
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp41';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp41';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp41', 'lcp41', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno de remisiones sin detenido de juzgados adolescentes.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO DE REMISIONES SIN DETENIDO DE JUZGADOS ADOLESCENTES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp41.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp41.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp41.id');
		$is_new		= true;
		...
		// Reorder the libro de turno de remisiones sin detenido de juzga so the new libro de turno de remisiones sin detenido de juzgados adolescentes is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp41BeforeSave', array('com_jtca.lcp41', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp41AfterSave', array('com_jtca.lcp41', $table, $is_new));

		$this->setState('lcp41.id', $table->id);
