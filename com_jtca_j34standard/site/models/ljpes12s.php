...
/**
 * This models supports retrieving lists of libro de fianzas.
 *
 */
class JtCaModelLjpes12s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpes12s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de fianzas has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpes12_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes12_id');
		$this->setState('filter.ljpes12_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes12_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes12_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes12_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes12_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes12_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes12_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes12_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes12_id'));
		$id .= ':'.$this->getState('filter.ljpes12_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes12s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes12s').' AS a');
		...
		// Filter by a single or group of ljpes12s.
		$ljpes12_id = $this->getState('filter.ljpes12_id');
		if ($ljpes12_id != '')
		{
			if (is_numeric($ljpes12_id))
			{
				$type = $this->getState('filter.ljpes12_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes12_id);
			}
			else
			{
				if (is_array($ljpes12_id))
				{
					JArrayHelper::toInteger($ljpes12_id);
					$ljpes12_id = implode(',', $ljpes12_id);
					$type = $this->getState('filter.ljpes12_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes12_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes12_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes12_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de fianzas.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes12_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes12 params override menu item params only if menu param = 'use_ljpes12'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes12' and there is no ljpes12 param, use global

					// create an array of just the params set to 'use_ljpes12'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes12_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes12')
						{
							// if the ljpes12 has a value, use it
							if ($ljpes12_params->get($key) != '')
							{
								// get the value from the ljpes12
								$ljpes12_array[$key] = $ljpes12_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes12_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes12 params
					if (count($ljpes12_array) > 0)
					{
						$ljpes12_params = new Registry;
						$ljpes12_params->loadArray($ljpes12_array);
						$item->params->merge($ljpes12_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes12_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes12, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
