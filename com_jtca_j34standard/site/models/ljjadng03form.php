...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadng03.php';

class JtCaModelLjjadng03Form extends JtCaModelLjjadng03
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadng03';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadng03';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadng03', 'ljjadng03', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the agenda de audiencias.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	AGENDA DE AUDIENCIAS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadng03.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadng03.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadng03.id');
		$is_new		= true;
		...
		// Reorder the agenda de audiencias so the new agenda de audiencias is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadng03BeforeSave', array('com_jtca.ljjadng03', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadng03AfterSave', array('com_jtca.ljjadng03', $table, $is_new));

		$this->setState('ljjadng03.id', $table->id);
