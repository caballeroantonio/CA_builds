...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljc05.php';

class JtCaModelLjc05Form extends JtCaModelLjc05
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljc05';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljc05';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljc05', 'ljc05', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno para sentencia.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO PARA SENTENCIA item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljc05.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljc05.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljc05.id');
		$is_new		= true;
		...
		// Reorder the jc05 so the new libro de turno para sentencia is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjc05BeforeSave', array('com_jtca.ljc05', &$table, $is_new));
		...
		$dispatcher->trigger('onLjc05AfterSave', array('com_jtca.ljc05', $table, $is_new));

		$this->setState('ljc05.id', $table->id);
