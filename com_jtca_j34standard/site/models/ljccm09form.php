...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljccm09.php';

class JtCaModelLjccm09Form extends JtCaModelLjccm09
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljccm09';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljccm09';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljccm09', 'ljccm09', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljccm09.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljccm09.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljccm09.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjccm09BeforeSave', array('com_jtca.ljccm09', &$table, $is_new));
		...
		$dispatcher->trigger('onLjccm09AfterSave', array('com_jtca.ljccm09', $table, $is_new));

		$this->setState('ljccm09.id', $table->id);
