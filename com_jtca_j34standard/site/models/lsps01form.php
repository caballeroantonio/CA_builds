...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsps01.php';

class JtcaModelLsps01Form extends JtcaModelLsps01
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsps01';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsps01';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsps01', 'lsps01', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de gobierno (sistema tradicional).
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE GOBIERNO (SISTEMA TRADICIONAL) item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsps01.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsps01.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsps01.id');
		$is_new		= true;
		...
		// Reorder the libro de gobierno (sistema tradicional) so the new libro de gobierno (sistema tradicional) is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsps01BeforeSave', array('com_jtca.lsps01', &$table, $is_new));
		...
		$dispatcher->trigger('onLsps01AfterSave', array('com_jtca.lsps01', $table, $is_new));

		$this->setState('lsps01.id', $table->id);
