...
/**
 * TSJ CDMX Libros TxCA Component Adolescentes Model
 *
 */
class JtcaModelSlpersonaliasedad extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.slpersonaliasedad';
	...
	{
		if (empty($config['slpersonaliasedad_filter_fields']))
		{
			$config['slpersonaliasedad_filter_fields'] = array(
				'id', 'a.id',
	...
	protected function populateState()
	{
		...
		$this->setState('slpersonaliasedad.id', $pk);
	...
	*/
	public function getTable($type = 'Slpersonaliasedades', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get Adolescentes data.
	 *
	 * @param	integer	$pk	The id of the adolescentes.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('slpersonaliasedad.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_SLPERSONALIASEDADES_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onSlpersonaliasedadChangeState', array('com_jtca.slpersonaliasedad', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onSlpersonaliasedadBeforeDelete', array('com_jtca.slpersonaliasedad', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onSlpersonaliasedadAfterDelete', array('com_jtca.slpersonaliasedad', &$table));
				}
				else
				{
					// Prune items that you can't change.
