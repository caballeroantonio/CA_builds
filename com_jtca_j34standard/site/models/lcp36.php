...
/**
 * TSJ CDMX Libros TxCA Component LIBRETA DE RETURNO DE ADOLESCENTES Model
 *
 */
class JtcaModelLcp36 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp36';
	...
	{
		if (empty($config['lcp36_filter_fields']))
		{
			$config['lcp36_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp36s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRETA DE RETURNO DE ADOLESCENTES data.
	 *
	 * @param	integer	$pk	The id of the libreta de returno de adolescentes.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp36.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP36S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp36ChangeState', array('com_jtca.lcp36', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp36BeforeDelete', array('com_jtca.lcp36', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp36AfterDelete', array('com_jtca.lcp36', &$table));
				}
				else
				{
					// Prune items that you can't change.
