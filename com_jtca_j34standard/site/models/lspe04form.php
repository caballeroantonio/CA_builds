...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lspe04.php';

class JtCaModelLspe04Form extends JtCaModelLspe04
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lspe04';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lspe04';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lspe04', 'lspe04', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de beneficios revocado y o/a.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE BENEFICIOS REVOCADO Y O/A item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lspe04.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lspe04.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lspe04.id');
		$is_new		= true;
		...
		// Reorder the libro de beneficios revocado y o/a so the new libro de beneficios revocado y o/a is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLspe04BeforeSave', array('com_jtca.lspe04', &$table, $is_new));
		...
		$dispatcher->trigger('onLspe04AfterSave', array('com_jtca.lspe04', $table, $is_new));

		$this->setState('lspe04.id', $table->id);
