...
class JtCaModelLjomf01s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf01_id');
		$this->setState('filter.ljomf01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf01_id'));
		$id .= ':'.$this->getState('filter.ljomf01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf01s').' AS a');
		...
		// Filter by a single or group of ljomf01s.
		$ljomf01_id = $this->getState('filter.ljomf01_id');
		if ($ljomf01_id != '')
		{
			if (is_numeric($ljomf01_id))
			{
				$type = $this->getState('filter.ljomf01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf01_id);
			}
			else
			{
				if (is_array($ljomf01_id))
				{
					JArrayHelper::toInteger($ljomf01_id);
					$ljomf01_id = implode(',', $ljomf01_id);
					$type = $this->getState('filter.ljomf01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf01_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf01 params override menu item params only if menu param = 'use_ljomf01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf01' and there is no ljomf01 param, use global

					// create an array of just the params set to 'use_ljomf01'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf01_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf01')
						{
							// if the ljomf01 has a value, use it
							if ($ljomf01_params->get($key) != '')
							{
								// get the value from the ljomf01
								$ljomf01_array[$key] = $ljomf01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf01 params
					if (count($ljomf01_array) > 0)
					{
						$ljomf01_params = new Registry;
						$ljomf01_params->loadArray($ljomf01_array);
						$item->params->merge($ljomf01_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
