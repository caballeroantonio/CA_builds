...
/**
 * This models supports retrieving lists of jc08.
 *
 */
class JtCaModelLjc08s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljc08s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de oficios has been specified either as a parameter or in the url Request
		$pk = $params->get('ljc08_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc08_id');
		$this->setState('filter.ljc08_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc08_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc08_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc08_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc08_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc08_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc08_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc08_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc08_id'));
		$id .= ':'.$this->getState('filter.ljc08_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc08s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc08s').' AS a');
		...
		// Filter by a single or group of ljc08s.
		$ljc08_id = $this->getState('filter.ljc08_id');
		if ($ljc08_id != '')
		{
			if (is_numeric($ljc08_id))
			{
				$type = $this->getState('filter.ljc08_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc08_id);
			}
			else
			{
				if (is_array($ljc08_id))
				{
					JArrayHelper::toInteger($ljc08_id);
					$ljc08_id = implode(',', $ljc08_id);
					$type = $this->getState('filter.ljc08_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc08_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc08_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc08_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc08_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc08 params override menu item params only if menu param = 'use_ljc08'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc08' and there is no ljc08 param, use global

					// create an array of just the params set to 'use_ljc08'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc08')
						{
							// if the ljc08 has a value, use it
							if ($ljc08_params->get($key) != '')
							{
								// get the value from the ljc08
								$ljc08_array[$key] = $ljc08_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc08_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc08 params
					if (count($ljc08_array) > 0)
					{
						$ljc08_params = new Registry;
						$ljc08_params->loadArray($ljc08_array);
						$item->params->merge($ljc08_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc08_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc08, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
