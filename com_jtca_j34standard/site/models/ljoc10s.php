...
class JtCaModelLjoc10s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljoc10_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljoc10_id');
		$this->setState('filter.ljoc10_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljoc10_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljoc10_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljoc10_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljoc10_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljoc10_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljoc10_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljoc10_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljoc10_id'));
		$id .= ':'.$this->getState('filter.ljoc10_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljoc10s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljoc10s').' AS a');
		...
		// Filter by a single or group of ljoc10s.
		$ljoc10_id = $this->getState('filter.ljoc10_id');
		if ($ljoc10_id != '')
		{
			if (is_numeric($ljoc10_id))
			{
				$type = $this->getState('filter.ljoc10_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljoc10_id);
			}
			else
			{
				if (is_array($ljoc10_id))
				{
					JArrayHelper::toInteger($ljoc10_id);
					$ljoc10_id = implode(',', $ljoc10_id);
					$type = $this->getState('filter.ljoc10_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljoc10_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljoc10_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljoc10_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljoc10_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljoc10 params override menu item params only if menu param = 'use_ljoc10'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljoc10' and there is no ljoc10 param, use global

					// create an array of just the params set to 'use_ljoc10'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljoc10')
						{
							// if the ljoc10 has a value, use it
							if ($ljoc10_params->get($key) != '')
							{
								// get the value from the ljoc10
								$ljoc10_array[$key] = $ljoc10_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljoc10_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljoc10 params
					if (count($ljoc10_array) > 0)
					{
						$ljoc10_params = new Registry;
						$ljoc10_params->loadArray($ljoc10_array);
						$item->params->merge($ljoc10_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljoc10_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljoc10, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
