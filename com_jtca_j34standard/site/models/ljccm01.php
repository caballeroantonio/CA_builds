 ...
 *
 */
class JtCaModelLjccm01 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljccm01';
	...
	{
		if (empty($config['ljccm01_filter_fields']))
		{
			$config['ljccm01_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljccm01s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljccm01.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJCCM01S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjccm01ChangeState', array('com_jtca.ljccm01', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjccm01BeforeDelete', array('com_jtca.ljccm01', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjccm01AfterDelete', array('com_jtca.ljccm01', &$table));
				}
				else
				{
					// Prune items that you can't change.
