...
class JtCaModelLjf15s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf15_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf15_id');
		$this->setState('filter.ljf15_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf15_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf15_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf15_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf15_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf15_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf15_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf15_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf15_id'));
		$id .= ':'.$this->getState('filter.ljf15_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf15s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf15s').' AS a');
		...
		// Filter by a single or group of ljf15s.
		$ljf15_id = $this->getState('filter.ljf15_id');
		if ($ljf15_id != '')
		{
			if (is_numeric($ljf15_id))
			{
				$type = $this->getState('filter.ljf15_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf15_id);
			}
			else
			{
				if (is_array($ljf15_id))
				{
					JArrayHelper::toInteger($ljf15_id);
					$ljf15_id = implode(',', $ljf15_id);
					$type = $this->getState('filter.ljf15_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf15_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf15_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf15_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf15_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf15 params override menu item params only if menu param = 'use_ljf15'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf15' and there is no ljf15 param, use global

					// create an array of just the params set to 'use_ljf15'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf15')
						{
							// if the ljf15 has a value, use it
							if ($ljf15_params->get($key) != '')
							{
								// get the value from the ljf15
								$ljf15_array[$key] = $ljf15_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf15_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf15 params
					if (count($ljf15_array) > 0)
					{
						$ljf15_params = new Registry;
						$ljf15_params->loadArray($ljf15_array);
						$item->params->merge($ljf15_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf15_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf15, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
