...
/**
 * TSJ CDMX Libros TxCA Component CONTROL DE FIRMAS DEL PROCESADO Model
 *
 */
class JtcaModelLsps13 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsps13';
	...
	{
		if (empty($config['lsps13_filter_fields']))
		{
			$config['lsps13_filter_fields'] = array(
				'id', 'a.id',
				'id_organo','a.id_organo',
				'id_secretaria','a.id_secretaria',
				'anoj','a.anoj',
				'id_expediente','a.id_expediente',
				'field2444_isMoral','a.field2444_isMoral',
				'field2444_paterno','a.field2444_paterno',
				'field2444_materno','a.field2444_materno',
				'field2444_nombre','a.field2444_nombre',
				'field2445','a.field2445',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lsps13s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get CONTROL DE FIRMAS DEL PROCESADO data.
	 *
	 * @param	integer	$pk	The id of the control de firmas del procesado.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsps13.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPS13S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsps13ChangeState', array('com_jtca.lsps13', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsps13BeforeDelete', array('com_jtca.lsps13', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsps13AfterDelete', array('com_jtca.lsps13', &$table));
				}
				else
				{
					// Prune items that you can't change.
