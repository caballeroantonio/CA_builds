...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/expediente.php';

class JtcaModelExpedienteForm extends JtcaModelExpediente
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.expediente';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.expediente';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.expediente', 'expediente', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the expedientes.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Expedientes item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('expediente.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.expediente.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('expediente.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onExpedienteBeforeSave', array('com_jtca.expediente', &$table, $is_new));
		...
		$dispatcher->trigger('onExpedienteAfterSave', array('com_jtca.expediente', $table, $is_new));

		$this->setState('expediente.id', $table->id);
