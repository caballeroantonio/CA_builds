...
/**
 * This models supports retrieving lists of libreta de oficios de amparos de tramite..
 *
 */
class JtCaModelLcp15s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp15s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de oficios de amparos de tramite. has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp15_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp15_id');
		$this->setState('filter.lcp15_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp15_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp15_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp15_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp15_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp15_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp15_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp15_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp15_id'));
		$id .= ':'.$this->getState('filter.lcp15_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp15s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp15s').' AS a');
		...
		// Filter by a single or group of lcp15s.
		$lcp15_id = $this->getState('filter.lcp15_id');
		if ($lcp15_id != '')
		{
			if (is_numeric($lcp15_id))
			{
				$type = $this->getState('filter.lcp15_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp15_id);
			}
			else
			{
				if (is_array($lcp15_id))
				{
					JArrayHelper::toInteger($lcp15_id);
					$lcp15_id = implode(',', $lcp15_id);
					$type = $this->getState('filter.lcp15_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp15_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp15_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp15_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de oficios de amparos de tramite..
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp15_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp15 params override menu item params only if menu param = 'use_lcp15'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp15' and there is no lcp15 param, use global

					// create an array of just the params set to 'use_lcp15'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp15')
						{
							// if the lcp15 has a value, use it
							if ($lcp15_params->get($key) != '')
							{
								// get the value from the lcp15
								$lcp15_array[$key] = $lcp15_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp15_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp15 params
					if (count($lcp15_array) > 0)
					{
						$lcp15_params = new Registry;
						$lcp15_params->loadArray($lcp15_array);
						$item->params->merge($lcp15_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp15_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp15, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
