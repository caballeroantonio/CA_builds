 ...
 *
 */
class JtcaModelLjpdng01 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpdng01';
	...
	{
		if (empty($config['ljpdng01_filter_fields']))
		{
			$config['ljpdng01_filter_fields'] = array(
				'id', 'a.id',
				...
				'field7','a.field7',
				'txt_field975','a.txt_field975',
				'id_field975','a.id_field975',
				'field11','a.field11',
				'field13','a.field13',
				'field14','a.field14',
				'field15','a.field15',
	...
	*/
	public function getTable($type = 'Ljpdng01s', $prefix = 'JtcaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpdng01.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPDNG01S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpdng01ChangeState', array('com_jtca.ljpdng01', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpdng01BeforeDelete', array('com_jtca.ljpdng01', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpdng01AfterDelete', array('com_jtca.ljpdng01', &$table));
				}
				else
				{
					// Prune items that you can't change.
