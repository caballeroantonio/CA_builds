...
/**
 * This models supports retrieving lists of libro de amparos 'locos'.
 *
 */
class JtcaModelLjjadg18s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadg18s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg18_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg18_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg18_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg18_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg18_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg18_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg18_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg18_id'));
		$id .= ':'.$this->getState('filter.ljjadg18_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg18s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg18s').' AS a');
		...
		// Filter by a single or group of ljjadg18s.
		$ljjadg18_id = $this->getState('filter.ljjadg18_id');
		if ($ljjadg18_id != '')
		{
			if (is_numeric($ljjadg18_id))
			{
				$type = $this->getState('filter.ljjadg18_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg18_id);
			}
			else
			{
				if (is_array($ljjadg18_id))
				{
					JArrayHelper::toInteger($ljjadg18_id);
					$ljjadg18_id = implode(',', $ljjadg18_id);
					$type = $this->getState('filter.ljjadg18_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg18_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg18_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg18_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de amparos 'locos'.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg18_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg18 params override menu item params only if menu param = 'use_ljjadg18'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg18' and there is no ljjadg18 param, use global

					// create an array of just the params set to 'use_ljjadg18'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg18_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg18')
						{
							// if the ljjadg18 has a value, use it
							if ($ljjadg18_params->get($key) != '')
							{
								// get the value from the ljjadg18
								$ljjadg18_array[$key] = $ljjadg18_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg18_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg18 params
					if (count($ljjadg18_array) > 0)
					{
						$ljjadg18_params = new Registry;
						$ljjadg18_params->loadArray($ljjadg18_array);
						$item->params->merge($ljjadg18_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg18_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg18, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
