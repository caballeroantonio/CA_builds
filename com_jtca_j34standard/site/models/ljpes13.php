...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE RESOLUCIONES EN AUDIENCIA Model
 *
 */
class JtCaModelLjpes13 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpes13';
	...
	{
		if (empty($config['ljpes13_filter_fields']))
		{
			$config['ljpes13_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljpes13s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE RESOLUCIONES EN AUDIENCIA data.
	 *
	 * @param	integer	$pk	The id of the libro de resoluciones en audiencia.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpes13.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPES13S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpes13ChangeState', array('com_jtca.ljpes13', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpes13BeforeDelete', array('com_jtca.ljpes13', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpes13AfterDelete', array('com_jtca.ljpes13', &$table));
				}
				else
				{
					// Prune items that you can't change.
