...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpes10.php';

class JtCaModelLjpes10Form extends JtCaModelLjpes10
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpes10';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpes10';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpes10', 'ljpes10', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the agenda de audiencias para juicios orales.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	AGENDA DE AUDIENCIAS PARA JUICIOS ORALES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpes10.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpes10.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpes10.id');
		$is_new		= true;
		...
		// Reorder the agenda de audiencias para juicios orales so the new agenda de audiencias para juicios orales is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpes10BeforeSave', array('com_jtca.ljpes10', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpes10AfterSave', array('com_jtca.ljpes10', $table, $is_new));

		$this->setState('ljpes10.id', $table->id);
