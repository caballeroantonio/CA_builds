...
class JtCaModelLjp06s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljp06_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp06_id');
		$this->setState('filter.ljp06_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp06_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp06_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp06_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp06_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp06_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp06_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp06_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp06_id'));
		$id .= ':'.$this->getState('filter.ljp06_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp06s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp06s').' AS a');
		...
		// Filter by a single or group of ljp06s.
		$ljp06_id = $this->getState('filter.ljp06_id');
		if ($ljp06_id != '')
		{
			if (is_numeric($ljp06_id))
			{
				$type = $this->getState('filter.ljp06_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp06_id);
			}
			else
			{
				if (is_array($ljp06_id))
				{
					JArrayHelper::toInteger($ljp06_id);
					$ljp06_id = implode(',', $ljp06_id);
					$type = $this->getState('filter.ljp06_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp06_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp06_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp06_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp06_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp06 params override menu item params only if menu param = 'use_ljp06'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp06' and there is no ljp06 param, use global

					// create an array of just the params set to 'use_ljp06'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp06')
						{
							// if the ljp06 has a value, use it
							if ($ljp06_params->get($key) != '')
							{
								// get the value from the ljp06
								$ljp06_array[$key] = $ljp06_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp06_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp06 params
					if (count($ljp06_array) > 0)
					{
						$ljp06_params = new Registry;
						$ljp06_params->loadArray($ljp06_array);
						$item->params->merge($ljp06_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp06_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp06, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
