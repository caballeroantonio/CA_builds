...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadng02.php';

class JtCaModelLjjadng02Form extends JtCaModelLjjadng02
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadng02';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadng02';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadng02', 'ljjadng02', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de promociones.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE PROMOCIONES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadng02.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadng02.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadng02.id');
		$is_new		= true;
		...
		// Reorder the libro de promociones so the new libro de promociones is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadng02BeforeSave', array('com_jtca.ljjadng02', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadng02AfterSave', array('com_jtca.ljjadng02', $table, $is_new));

		$this->setState('ljjadng02.id', $table->id);
