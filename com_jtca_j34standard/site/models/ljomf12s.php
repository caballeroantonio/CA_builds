...
class JtCaModelLjomf12s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf12_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf12_id');
		$this->setState('filter.ljomf12_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf12_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf12_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf12_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf12_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf12_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf12_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf12_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf12_id'));
		$id .= ':'.$this->getState('filter.ljomf12_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf12s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf12s').' AS a');
		...
		// Filter by a single or group of ljomf12s.
		$ljomf12_id = $this->getState('filter.ljomf12_id');
		if ($ljomf12_id != '')
		{
			if (is_numeric($ljomf12_id))
			{
				$type = $this->getState('filter.ljomf12_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf12_id);
			}
			else
			{
				if (is_array($ljomf12_id))
				{
					JArrayHelper::toInteger($ljomf12_id);
					$ljomf12_id = implode(',', $ljomf12_id);
					$type = $this->getState('filter.ljomf12_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf12_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf12_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf12_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf12_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf12 params override menu item params only if menu param = 'use_ljomf12'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf12' and there is no ljomf12 param, use global

					// create an array of just the params set to 'use_ljomf12'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf12_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf12')
						{
							// if the ljomf12 has a value, use it
							if ($ljomf12_params->get($key) != '')
							{
								// get the value from the ljomf12
								$ljomf12_array[$key] = $ljomf12_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf12_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf12 params
					if (count($ljomf12_array) > 0)
					{
						$ljomf12_params = new Registry;
						$ljomf12_params->loadArray($ljomf12_array);
						$item->params->merge($ljomf12_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf12_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf12, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
