...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp01.php';

class JtcaModelLcp01Form extends JtcaModelLcp01
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp01';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp01';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp01', 'lcp01', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno de justicia para adolescentes con detenido.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO DE JUSTICIA PARA ADOLESCENTES CON DETENIDO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp01.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp01.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp01.id');
		$is_new		= true;
		...
		// Reorder the libro de turno de justicia para adolescentes con d so the new libro de turno de justicia para adolescentes con detenido is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp01BeforeSave', array('com_jtca.lcp01', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp01AfterSave', array('com_jtca.lcp01', $table, $is_new));

		$this->setState('lcp01.id', $table->id);
