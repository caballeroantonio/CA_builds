...
/**
 * This models supports retrieving lists of libro de gobierno (oficial).
 *
 */
class JtCaModelLsc09s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsc09s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de gobierno (oficial) has been specified either as a parameter or in the url Request
		$pk = $params->get('lsc09_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsc09_id');
		$this->setState('filter.lsc09_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsc09_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsc09_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsc09_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsc09_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsc09_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsc09_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsc09_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsc09_id'));
		$id .= ':'.$this->getState('filter.lsc09_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsc09s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsc09s').' AS a');
		...
		// Filter by a single or group of lsc09s.
		$lsc09_id = $this->getState('filter.lsc09_id');
		if ($lsc09_id != '')
		{
			if (is_numeric($lsc09_id))
			{
				$type = $this->getState('filter.lsc09_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsc09_id);
			}
			else
			{
				if (is_array($lsc09_id))
				{
					JArrayHelper::toInteger($lsc09_id);
					$lsc09_id = implode(',', $lsc09_id);
					$type = $this->getState('filter.lsc09_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsc09_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsc09_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsc09_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de gobierno (oficial).
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsc09_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsc09 params override menu item params only if menu param = 'use_lsc09'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsc09' and there is no lsc09 param, use global

					// create an array of just the params set to 'use_lsc09'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsc09')
						{
							// if the lsc09 has a value, use it
							if ($lsc09_params->get($key) != '')
							{
								// get the value from the lsc09
								$lsc09_array[$key] = $lsc09_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsc09_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsc09 params
					if (count($lsc09_array) > 0)
					{
						$lsc09_params = new Registry;
						$lsc09_params->loadArray($lsc09_array);
						$item->params->merge($lsc09_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsc09_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsc09, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
