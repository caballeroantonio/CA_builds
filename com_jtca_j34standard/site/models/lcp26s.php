...
/**
 * This models supports retrieving lists of libro de turno para consignaciones sin detenido de.
 *
 */
class JtCaModelLcp26s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp26s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno para consignaciones sin detenido delitos no graves has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp26_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp26_id');
		$this->setState('filter.lcp26_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp26_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp26_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp26_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp26_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp26_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp26_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp26_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp26_id'));
		$id .= ':'.$this->getState('filter.lcp26_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp26s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp26s').' AS a');
		...
		// Filter by a single or group of lcp26s.
		$lcp26_id = $this->getState('filter.lcp26_id');
		if ($lcp26_id != '')
		{
			if (is_numeric($lcp26_id))
			{
				$type = $this->getState('filter.lcp26_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp26_id);
			}
			else
			{
				if (is_array($lcp26_id))
				{
					JArrayHelper::toInteger($lcp26_id);
					$lcp26_id = implode(',', $lcp26_id);
					$type = $this->getState('filter.lcp26_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp26_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp26_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp26_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno para consignaciones sin detenido de.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp26_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp26 params override menu item params only if menu param = 'use_lcp26'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp26' and there is no lcp26 param, use global

					// create an array of just the params set to 'use_lcp26'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp26')
						{
							// if the lcp26 has a value, use it
							if ($lcp26_params->get($key) != '')
							{
								// get the value from the lcp26
								$lcp26_array[$key] = $lcp26_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp26_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp26 params
					if (count($lcp26_array) > 0)
					{
						$lcp26_params = new Registry;
						$lcp26_params->loadArray($lcp26_array);
						$item->params->merge($lcp26_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp26_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp26, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
