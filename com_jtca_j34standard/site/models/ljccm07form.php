...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljccm07.php';

class JtcaModelLjccm07Form extends JtcaModelLjccm07
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljccm07';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljccm07';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljccm07', 'ljccm07', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljccm07.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljccm07.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljccm07.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjccm07BeforeSave', array('com_jtca.ljccm07', &$table, $is_new));
		...
		$dispatcher->trigger('onLjccm07AfterSave', array('com_jtca.ljccm07', $table, $is_new));

		$this->setState('ljccm07.id', $table->id);
