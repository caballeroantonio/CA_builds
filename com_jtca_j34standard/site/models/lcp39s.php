...
/**
 * This models supports retrieving lists of libro de turno de excusas para juzgados de justici.
 *
 */
class JtCaModelLcp39s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp39s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno de excusas para juzgados de justicia para adolescentes has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp39_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp39_id');
		$this->setState('filter.lcp39_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp39_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp39_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp39_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp39_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp39_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp39_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp39_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp39_id'));
		$id .= ':'.$this->getState('filter.lcp39_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp39s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp39s').' AS a');
		...
		// Filter by a single or group of lcp39s.
		$lcp39_id = $this->getState('filter.lcp39_id');
		if ($lcp39_id != '')
		{
			if (is_numeric($lcp39_id))
			{
				$type = $this->getState('filter.lcp39_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp39_id);
			}
			else
			{
				if (is_array($lcp39_id))
				{
					JArrayHelper::toInteger($lcp39_id);
					$lcp39_id = implode(',', $lcp39_id);
					$type = $this->getState('filter.lcp39_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp39_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp39_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp39_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno de excusas para juzgados de justici.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp39_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp39 params override menu item params only if menu param = 'use_lcp39'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp39' and there is no lcp39 param, use global

					// create an array of just the params set to 'use_lcp39'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp39')
						{
							// if the lcp39 has a value, use it
							if ($lcp39_params->get($key) != '')
							{
								// get the value from the lcp39
								$lcp39_array[$key] = $lcp39_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp39_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp39 params
					if (count($lcp39_array) > 0)
					{
						$lcp39_params = new Registry;
						$lcp39_params->loadArray($lcp39_array);
						$item->params->merge($lcp39_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp39_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp39, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
