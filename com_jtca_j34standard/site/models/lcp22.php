...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE ORDENES DE ARRAIGOS Y DE CATEOS PRIMERA INSTANCIA. Model
 *
 */
class JtCaModelLcp22 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp22';
	...
	{
		if (empty($config['lcp22_filter_fields']))
		{
			$config['lcp22_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp22s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE ORDENES DE ARRAIGOS Y DE CATEOS PRIMERA INSTANCIA. data.
	 *
	 * @param	integer	$pk	The id of the libro de ordenes de arraigos y de cateos primera instancia..
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp22.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP22S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp22ChangeState', array('com_jtca.lcp22', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp22BeforeDelete', array('com_jtca.lcp22', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp22AfterDelete', array('com_jtca.lcp22', &$table));
				}
				else
				{
					// Prune items that you can't change.
