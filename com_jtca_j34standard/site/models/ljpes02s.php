...
class JtCaModelLjpes02s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpes02_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes02_id');
		$this->setState('filter.ljpes02_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes02_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes02_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes02_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes02_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes02_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes02_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes02_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes02_id'));
		$id .= ':'.$this->getState('filter.ljpes02_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes02s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes02s').' AS a');
		...
		// Filter by a single or group of ljpes02s.
		$ljpes02_id = $this->getState('filter.ljpes02_id');
		if ($ljpes02_id != '')
		{
			if (is_numeric($ljpes02_id))
			{
				$type = $this->getState('filter.ljpes02_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes02_id);
			}
			else
			{
				if (is_array($ljpes02_id))
				{
					JArrayHelper::toInteger($ljpes02_id);
					$ljpes02_id = implode(',', $ljpes02_id);
					$type = $this->getState('filter.ljpes02_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes02_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes02_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes02_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes02_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes02 params override menu item params only if menu param = 'use_ljpes02'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes02' and there is no ljpes02 param, use global

					// create an array of just the params set to 'use_ljpes02'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes02_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes02')
						{
							// if the ljpes02 has a value, use it
							if ($ljpes02_params->get($key) != '')
							{
								// get the value from the ljpes02
								$ljpes02_array[$key] = $ljpes02_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes02_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes02 params
					if (count($ljpes02_array) > 0)
					{
						$ljpes02_params = new Registry;
						$ljpes02_params->loadArray($ljpes02_array);
						$item->params->merge($ljpes02_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes02_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes02, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
