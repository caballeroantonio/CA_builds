...
class JtCaModelLjomf17s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf17_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf17_id');
		$this->setState('filter.ljomf17_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf17_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf17_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf17_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf17_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf17_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf17_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf17_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf17_id'));
		$id .= ':'.$this->getState('filter.ljomf17_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf17s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf17s').' AS a');
		...
		// Filter by a single or group of ljomf17s.
		$ljomf17_id = $this->getState('filter.ljomf17_id');
		if ($ljomf17_id != '')
		{
			if (is_numeric($ljomf17_id))
			{
				$type = $this->getState('filter.ljomf17_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf17_id);
			}
			else
			{
				if (is_array($ljomf17_id))
				{
					JArrayHelper::toInteger($ljomf17_id);
					$ljomf17_id = implode(',', $ljomf17_id);
					$type = $this->getState('filter.ljomf17_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf17_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf17_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf17_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf17_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf17 params override menu item params only if menu param = 'use_ljomf17'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf17' and there is no ljomf17 param, use global

					// create an array of just the params set to 'use_ljomf17'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf17_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf17')
						{
							// if the ljomf17 has a value, use it
							if ($ljomf17_params->get($key) != '')
							{
								// get the value from the ljomf17
								$ljomf17_array[$key] = $ljomf17_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf17_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf17 params
					if (count($ljomf17_array) > 0)
					{
						$ljomf17_params = new Registry;
						$ljomf17_params->loadArray($ljomf17_array);
						$item->params->merge($ljomf17_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf17_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf17, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
