...
class JtCaModelLjemst10s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljemst10_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljemst10_id');
		$this->setState('filter.ljemst10_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljemst10_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljemst10_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljemst10_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljemst10_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljemst10_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljemst10_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljemst10_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljemst10_id'));
		$id .= ':'.$this->getState('filter.ljemst10_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljemst10s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljemst10s').' AS a');
		...
		// Filter by a single or group of ljemst10s.
		$ljemst10_id = $this->getState('filter.ljemst10_id');
		if ($ljemst10_id != '')
		{
			if (is_numeric($ljemst10_id))
			{
				$type = $this->getState('filter.ljemst10_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljemst10_id);
			}
			else
			{
				if (is_array($ljemst10_id))
				{
					JArrayHelper::toInteger($ljemst10_id);
					$ljemst10_id = implode(',', $ljemst10_id);
					$type = $this->getState('filter.ljemst10_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljemst10_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljemst10_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljemst10_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljemst10_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljemst10 params override menu item params only if menu param = 'use_ljemst10'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljemst10' and there is no ljemst10 param, use global

					// create an array of just the params set to 'use_ljemst10'
					$menu_params_array = $this->getState('params')->toArray();
					$ljemst10_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljemst10')
						{
							// if the ljemst10 has a value, use it
							if ($ljemst10_params->get($key) != '')
							{
								// get the value from the ljemst10
								$ljemst10_array[$key] = $ljemst10_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljemst10_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljemst10 params
					if (count($ljemst10_array) > 0)
					{
						$ljemst10_params = new Registry;
						$ljemst10_params->loadArray($ljemst10_array);
						$item->params->merge($ljemst10_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljemst10_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljemst10, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
