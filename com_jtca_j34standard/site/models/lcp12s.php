...
/**
 * This models supports retrieving lists of libreta de turno de salas para juzgados orales..
 *
 */
class JtCaModelLcp12s extends JModelList
{
	 ...
	 */
	protected $context = 'com_jtca.lcp12s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de turno de salas para juzgados orales. has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp12_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp12_id');
		$this->setState('filter.lcp12_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp12_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp12_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp12_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp12_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp12_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp12_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp12_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp12_id'));
		$id .= ':'.$this->getState('filter.lcp12_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp12s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp12s').' AS a');
		...
		// Filter by a single or group of lcp12s.
		$lcp12_id = $this->getState('filter.lcp12_id');
		if ($lcp12_id != '')
		{
			if (is_numeric($lcp12_id))
			{
				$type = $this->getState('filter.lcp12_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp12_id);
			}
			else
			{
				if (is_array($lcp12_id))
				{
					JArrayHelper::toInteger($lcp12_id);
					$lcp12_id = implode(',', $lcp12_id);
					$type = $this->getState('filter.lcp12_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp12_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp12_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp12_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de turno de salas para juzgados orales..
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp12_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp12 params override menu item params only if menu param = 'use_lcp12'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp12' and there is no lcp12 param, use global

					// create an array of just the params set to 'use_lcp12'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp12')
						{
							// if the lcp12 has a value, use it
							if ($lcp12_params->get($key) != '')
							{
								// get the value from the lcp12
								$lcp12_array[$key] = $lcp12_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp12_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp12 params
					if (count($lcp12_array) > 0)
					{
						$lcp12_params = new Registry;
						$lcp12_params->loadArray($lcp12_array);
						$item->params->merge($lcp12_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp12_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp12, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
