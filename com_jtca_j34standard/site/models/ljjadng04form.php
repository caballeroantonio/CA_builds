...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadng04.php';

class JtCaModelLjjadng04Form extends JtCaModelLjjadng04
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadng04';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadng04';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadng04', 'ljjadng04', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de sentencias.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE SENTENCIAS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadng04.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadng04.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadng04.id');
		$is_new		= true;
		...
		// Reorder the libro de sentencias so the new libro de sentencias is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadng04BeforeSave', array('com_jtca.ljjadng04', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadng04AfterSave', array('com_jtca.ljjadng04', $table, $is_new));

		$this->setState('ljjadng04.id', $table->id);
