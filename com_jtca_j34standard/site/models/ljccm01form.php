...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljccm01.php';

class JtcaModelLjccm01Form extends JtcaModelLjccm01
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljccm01';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljccm01';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljccm01', 'ljccm01', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljccm01.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljccm01.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljccm01.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjccm01BeforeSave', array('com_jtca.ljccm01', &$table, $is_new));
		...
		$dispatcher->trigger('onLjccm01AfterSave', array('com_jtca.ljccm01', $table, $is_new));

		$this->setState('ljccm01.id', $table->id);
