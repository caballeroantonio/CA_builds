...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp32.php';

class JtcaModelLcp32Form extends JtcaModelLcp32
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp32';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp32';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp32', 'lcp32', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de libertades juzgados de ejecución de sentencia.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE LIBERTADES JUZGADOS DE EJECUCIÓN DE SENTENCIA item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp32.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp32.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp32.id');
		$is_new		= true;
		...
		// Reorder the libreta de libertades juzgados de ejecución de sen so the new libreta de libertades juzgados de ejecución de sentencia is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp32BeforeSave', array('com_jtca.lcp32', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp32AfterSave', array('com_jtca.lcp32', $table, $is_new));

		$this->setState('lcp32.id', $table->id);
