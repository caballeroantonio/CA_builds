...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljomf19.php';

class JtCaModelLjomf19Form extends JtCaModelLjomf19
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljomf19';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljomf19';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljomf19', 'ljomf19', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljomf19.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljomf19.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljomf19.id');
		$is_new		= true;
		...
		// Reorder the libro de envío de expedientes al archivo judicial  so the new libro de envío de expedientes al archivo judicial para su destrucción is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjomf19BeforeSave', array('com_jtca.ljomf19', &$table, $is_new));
		...
		$dispatcher->trigger('onLjomf19AfterSave', array('com_jtca.ljomf19', $table, $is_new));

		$this->setState('ljomf19.id', $table->id);
