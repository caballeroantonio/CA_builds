...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE FIANZAS Model
 *
 */
class JtCaModelLjoc14 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljoc14';
	...
	{
		if (empty($config['ljoc14_filter_fields']))
		{
			$config['ljoc14_filter_fields'] = array(
				'id', 'a.id',
				...
				'field11','a.field11',
				'field12','a.field12',
				'field13_isMoral','a.field13_isMoral',
				'field13_paterno','a.field13_paterno',
				'field13_materno','a.field13_materno',
				'field13_nombre','a.field13_nombre',
				'field14','a.field14',
				'field15','a.field15',
				'field16','a.field16',
				'field16h','a.field16h',
	...
	*/
	public function getTable($type = 'Ljoc14s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE FIANZAS data.
	 *
	 * @param	integer	$pk	The id of the libro de fianzas.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljoc14.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJOC14S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjoc14ChangeState', array('com_jtca.ljoc14', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjoc14BeforeDelete', array('com_jtca.ljoc14', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjoc14AfterDelete', array('com_jtca.ljoc14', &$table));
				}
				else
				{
					// Prune items that you can't change.
