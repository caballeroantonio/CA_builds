...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpdng07.php';

class JtcaModelLjpdng07Form extends JtcaModelLjpdng07
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpdng07';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpdng07';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpdng07', 'ljpdng07', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpdng07.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpdng07.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpdng07.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjpdng07BeforeSave', array('com_jtca.ljpdng07', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpdng07AfterSave', array('com_jtca.ljpdng07', $table, $is_new));

		$this->setState('ljpdng07.id', $table->id);
