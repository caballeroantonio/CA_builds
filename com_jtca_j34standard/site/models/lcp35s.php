...
class JtcaModelLcp35s extends JModelList
{
	 ...
	 */
	protected $context = 'com_jtca.lcp35s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de libertades de salas de adolescentes has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp35_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp35_id');
		$this->setState('filter.lcp35_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp35_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp35_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp35_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp35_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp35_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp35_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp35_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp35_id'));
		$id .= ':'.$this->getState('filter.lcp35_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp35s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp35s').' AS a');
		...
		// Filter by a single or group of lcp35s.
		$lcp35_id = $this->getState('filter.lcp35_id');
		if ($lcp35_id != '')
		{
			if (is_numeric($lcp35_id))
			{
				$type = $this->getState('filter.lcp35_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp35_id);
			}
			else
			{
				if (is_array($lcp35_id))
				{
					JArrayHelper::toInteger($lcp35_id);
					$lcp35_id = implode(',', $lcp35_id);
					$type = $this->getState('filter.lcp35_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp35_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp35_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp35_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de libertades de salas de adolescentes.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp35_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp35 params override menu item params only if menu param = 'use_lcp35'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp35' and there is no lcp35 param, use global

					// create an array of just the params set to 'use_lcp35'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp35')
						{
							// if the lcp35 has a value, use it
							if ($lcp35_params->get($key) != '')
							{
								// get the value from the lcp35
								$lcp35_array[$key] = $lcp35_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp35_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp35 params
					if (count($lcp35_array) > 0)
					{
						$lcp35_params = new Registry;
						$lcp35_params->loadArray($lcp35_array);
						$item->params->merge($lcp35_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp35_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp35, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
