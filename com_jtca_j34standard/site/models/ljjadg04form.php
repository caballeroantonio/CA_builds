...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadg04.php';

class JtCaModelLjjadg04Form extends JtCaModelLjjadg04
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadg04';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadg04';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadg04', 'ljjadg04', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de egresos.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE EGRESOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadg04.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadg04.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadg04.id');
		$is_new		= true;
		...
		// Reorder the libro de egresos so the new libro de egresos is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadg04BeforeSave', array('com_jtca.ljjadg04', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadg04AfterSave', array('com_jtca.ljjadg04', $table, $is_new));

		$this->setState('ljjadg04.id', $table->id);
