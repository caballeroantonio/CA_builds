...
/**
 * This models supports retrieving lists of libro de certificado de depÃ³sitos de ingresos y eg.
 *
 */
class JtCaModelLjoc03s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljoc03s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de certificado de depÃ³sitos de ingresos y egresos has been specified either as a parameter or in the url Request
		$pk = $params->get('ljoc03_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljoc03_id');
		$this->setState('filter.ljoc03_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljoc03_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljoc03_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljoc03_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljoc03_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljoc03_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljoc03_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljoc03_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljoc03_id'));
		$id .= ':'.$this->getState('filter.ljoc03_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljoc03s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljoc03s').' AS a');
		...
		// Filter by a single or group of ljoc03s.
		$ljoc03_id = $this->getState('filter.ljoc03_id');
		if ($ljoc03_id != '')
		{
			if (is_numeric($ljoc03_id))
			{
				$type = $this->getState('filter.ljoc03_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljoc03_id);
			}
			else
			{
				if (is_array($ljoc03_id))
				{
					JArrayHelper::toInteger($ljoc03_id);
					$ljoc03_id = implode(',', $ljoc03_id);
					$type = $this->getState('filter.ljoc03_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljoc03_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljoc03_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljoc03_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de certificado de depÃ³sitos de ingresos y eg.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljoc03_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljoc03 params override menu item params only if menu param = 'use_ljoc03'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljoc03' and there is no ljoc03 param, use global

					// create an array of just the params set to 'use_ljoc03'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljoc03')
						{
							// if the ljoc03 has a value, use it
							if ($ljoc03_params->get($key) != '')
							{
								// get the value from the ljoc03
								$ljoc03_array[$key] = $ljoc03_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljoc03_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljoc03 params
					if (count($ljoc03_array) > 0)
					{
						$ljoc03_params = new Registry;
						$ljoc03_params->loadArray($ljoc03_array);
						$item->params->merge($ljoc03_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljoc03_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljoc03, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
