...
class JtCaModelLjoc08Form extends JtCaModelLjoc08
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljoc08';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljoc08';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljoc08', 'ljoc08', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljoc08.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljoc08.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljoc08.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjoc08BeforeSave', array('com_jtca.ljoc08', &$table, $is_new));
		...
		$dispatcher->trigger('onLjoc08AfterSave', array('com_jtca.ljoc08', $table, $is_new));

		$this->setState('ljoc08.id', $table->id);
