...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljoc03.php';

class JtcaModelLjoc03Form extends JtcaModelLjoc03
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljoc03';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljoc03';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljoc03', 'ljoc03', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de certificado de depósitos de ingresos y egresos.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE CERTIFICADO DE DEPÓSITOS DE INGRESOS Y EGRESOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljoc03.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljoc03.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljoc03.id');
		$is_new		= true;
		...
		// Reorder the libro de certificado de depósitos de ingresos y eg so the new libro de certificado de depósitos de ingresos y egresos is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjoc03BeforeSave', array('com_jtca.ljoc03', &$table, $is_new));
		...
		$dispatcher->trigger('onLjoc03AfterSave', array('com_jtca.ljoc03', $table, $is_new));

		$this->setState('ljoc03.id', $table->id);
