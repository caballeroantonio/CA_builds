...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE RESOLUCIONES Model
 *
 */
class JtCaModelLspe03 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lspe03';
	...
	{
		if (empty($config['lspe03_filter_fields']))
		{
			$config['lspe03_filter_fields'] = array(
				'id', 'a.id',
				'id_organo','a.id_organo',
				'anoj','a.anoj',
				'id_expediente','a.id_expediente',
				'id_secretaria','a.id_secretaria',
				'field2303','a.field2303',
				'field2304','a.field2304',
				'field2305','a.field2305',
				'field2306','a.field2306',
				'field2307','a.field2307',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lspe03s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE RESOLUCIONES data.
	 *
	 * @param	integer	$pk	The id of the libro de resoluciones.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lspe03.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPE03S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLspe03ChangeState', array('com_jtca.lspe03', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLspe03BeforeDelete', array('com_jtca.lspe03', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLspe03AfterDelete', array('com_jtca.lspe03', &$table));
				}
				else
				{
					// Prune items that you can't change.
