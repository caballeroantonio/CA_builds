...
/**
 * This models supports retrieving lists of libro de control de billetes y pÃ³lizas (oficial).
 *
 */
class JtCaModelLsc06s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsc06s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de control de billetes y pÃ³lizas (oficial) has been specified either as a parameter or in the url Request
		$pk = $params->get('lsc06_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsc06_id');
		$this->setState('filter.lsc06_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsc06_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsc06_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsc06_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsc06_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsc06_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsc06_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsc06_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsc06_id'));
		$id .= ':'.$this->getState('filter.lsc06_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsc06s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsc06s').' AS a');
		...
		// Filter by a single or group of lsc06s.
		$lsc06_id = $this->getState('filter.lsc06_id');
		if ($lsc06_id != '')
		{
			if (is_numeric($lsc06_id))
			{
				$type = $this->getState('filter.lsc06_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsc06_id);
			}
			else
			{
				if (is_array($lsc06_id))
				{
					JArrayHelper::toInteger($lsc06_id);
					$lsc06_id = implode(',', $lsc06_id);
					$type = $this->getState('filter.lsc06_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsc06_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsc06_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsc06_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de control de billetes y pÃ³lizas (oficial).
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsc06_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsc06 params override menu item params only if menu param = 'use_lsc06'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsc06' and there is no lsc06 param, use global

					// create an array of just the params set to 'use_lsc06'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsc06')
						{
							// if the lsc06 has a value, use it
							if ($lsc06_params->get($key) != '')
							{
								// get the value from the lsc06
								$lsc06_array[$key] = $lsc06_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsc06_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsc06 params
					if (count($lsc06_array) > 0)
					{
						$lsc06_params = new Registry;
						$lsc06_params->loadArray($lsc06_array);
						$item->params->merge($lsc06_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsc06_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsc06, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
