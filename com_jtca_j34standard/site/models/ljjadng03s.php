...
/**
 * This models supports retrieving lists of agenda de audiencias.
 *
 */
class JtCaModelLjjadng03s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadng03s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadng03_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadng03_id');
		$this->setState('filter.ljjadng03_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadng03_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadng03_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadng03_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadng03_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadng03_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadng03_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadng03_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadng03_id'));
		$id .= ':'.$this->getState('filter.ljjadng03_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadng03s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadng03s').' AS a');
		...
		// Filter by a single or group of ljjadng03s.
		$ljjadng03_id = $this->getState('filter.ljjadng03_id');
		if ($ljjadng03_id != '')
		{
			if (is_numeric($ljjadng03_id))
			{
				$type = $this->getState('filter.ljjadng03_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadng03_id);
			}
			else
			{
				if (is_array($ljjadng03_id))
				{
					JArrayHelper::toInteger($ljjadng03_id);
					$ljjadng03_id = implode(',', $ljjadng03_id);
					$type = $this->getState('filter.ljjadng03_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadng03_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadng03_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadng03_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of agenda de audiencias.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadng03_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadng03 params override menu item params only if menu param = 'use_ljjadng03'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadng03' and there is no ljjadng03 param, use global

					// create an array of just the params set to 'use_ljjadng03'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadng03_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadng03')
						{
							// if the ljjadng03 has a value, use it
							if ($ljjadng03_params->get($key) != '')
							{
								// get the value from the ljjadng03
								$ljjadng03_array[$key] = $ljjadng03_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadng03_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadng03 params
					if (count($ljjadng03_array) > 0)
					{
						$ljjadng03_params = new Registry;
						$ljjadng03_params->loadArray($ljjadng03_array);
						$item->params->merge($ljjadng03_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadng03_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadng03, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
