...
/**
 * TSJ CDMX Libros TxCA Component LIBRETA DE TURNO DE SALAS PARA JUZGADOS ORALES. Model
 *
 */
class JtCaModelLcp12 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp12';
	...
	{
		if (empty($config['lcp12_filter_fields']))
		{
			$config['lcp12_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	...
	/**
	 * Method to get LIBRETA DE TURNO DE SALAS PARA JUZGADOS ORALES. data.
	 *
	 * @param	integer	$pk	The id of the libreta de turno de salas para juzgados orales..
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp12.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP12S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp12ChangeState', array('com_jtca.lcp12', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp12BeforeDelete', array('com_jtca.lcp12', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp12AfterDelete', array('com_jtca.lcp12', &$table));
				}
				else
				{
					// Prune items that you can't change.
