 ...
 *
 */
class JtCaModelLjp17 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljp17';
	...
	{
		if (empty($config['ljp17_filter_fields']))
		{
			$config['ljp17_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljp17s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljp17.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJP17S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjp17ChangeState', array('com_jtca.ljp17', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjp17BeforeDelete', array('com_jtca.ljp17', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjp17AfterDelete', array('com_jtca.ljp17', &$table));
				}
				else
				{
					// Prune items that you can't change.
