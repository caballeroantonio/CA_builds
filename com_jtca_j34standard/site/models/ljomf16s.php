...
class JtCaModelLjomf16s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf16_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf16_id');
		$this->setState('filter.ljomf16_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf16_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf16_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf16_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf16_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf16_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf16_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf16_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf16_id'));
		$id .= ':'.$this->getState('filter.ljomf16_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf16s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf16s').' AS a');
		...
		// Filter by a single or group of ljomf16s.
		$ljomf16_id = $this->getState('filter.ljomf16_id');
		if ($ljomf16_id != '')
		{
			if (is_numeric($ljomf16_id))
			{
				$type = $this->getState('filter.ljomf16_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf16_id);
			}
			else
			{
				if (is_array($ljomf16_id))
				{
					JArrayHelper::toInteger($ljomf16_id);
					$ljomf16_id = implode(',', $ljomf16_id);
					$type = $this->getState('filter.ljomf16_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf16_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf16_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf16_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf16_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf16 params override menu item params only if menu param = 'use_ljomf16'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf16' and there is no ljomf16 param, use global

					// create an array of just the params set to 'use_ljomf16'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf16_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf16')
						{
							// if the ljomf16 has a value, use it
							if ($ljomf16_params->get($key) != '')
							{
								// get the value from the ljomf16
								$ljomf16_array[$key] = $ljomf16_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf16_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf16 params
					if (count($ljomf16_array) > 0)
					{
						$ljomf16_params = new Registry;
						$ljomf16_params->loadArray($ljomf16_array);
						$item->params->merge($ljomf16_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf16_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf16, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
