...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljccm02.php';

class JtcaModelLjccm02Form extends JtcaModelLjccm02
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljccm02';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljccm02';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljccm02', 'ljccm02', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de ingreso de valores.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE INGRESO DE VALORES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljccm02.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljccm02.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljccm02.id');
		$is_new		= true;
		...
		// Reorder the libro de ingreso de valores so the new libro de ingreso de valores is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjccm02BeforeSave', array('com_jtca.ljccm02', &$table, $is_new));
		...
		$dispatcher->trigger('onLjccm02AfterSave', array('com_jtca.ljccm02', $table, $is_new));

		$this->setState('ljccm02.id', $table->id);
