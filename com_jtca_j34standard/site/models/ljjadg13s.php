...
class JtCaModelLjjadg13s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadg13_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg13_id');
		$this->setState('filter.ljjadg13_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg13_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg13_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg13_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg13_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg13_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg13_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg13_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg13_id'));
		$id .= ':'.$this->getState('filter.ljjadg13_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg13s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg13s').' AS a');
		...
		// Filter by a single or group of ljjadg13s.
		$ljjadg13_id = $this->getState('filter.ljjadg13_id');
		if ($ljjadg13_id != '')
		{
			if (is_numeric($ljjadg13_id))
			{
				$type = $this->getState('filter.ljjadg13_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg13_id);
			}
			else
			{
				if (is_array($ljjadg13_id))
				{
					JArrayHelper::toInteger($ljjadg13_id);
					$ljjadg13_id = implode(',', $ljjadg13_id);
					$type = $this->getState('filter.ljjadg13_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg13_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg13_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg13_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg13_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg13 params override menu item params only if menu param = 'use_ljjadg13'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg13' and there is no ljjadg13 param, use global

					// create an array of just the params set to 'use_ljjadg13'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg13_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg13')
						{
							// if the ljjadg13 has a value, use it
							if ($ljjadg13_params->get($key) != '')
							{
								// get the value from the ljjadg13
								$ljjadg13_array[$key] = $ljjadg13_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg13_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg13 params
					if (count($ljjadg13_array) > 0)
					{
						$ljjadg13_params = new Registry;
						$ljjadg13_params->loadArray($ljjadg13_array);
						$item->params->merge($ljjadg13_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg13_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg13, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
