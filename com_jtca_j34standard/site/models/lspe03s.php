...
/**
 * This models supports retrieving lists of libro de resoluciones.
 *
 */
class JtcaModelLspe03s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lspe03s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de resoluciones has been specified either as a parameter or in the url Request
		$pk = $params->get('lspe03_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lspe03_id');
		$this->setState('filter.lspe03_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lspe03_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lspe03_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lspe03_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lspe03_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lspe03_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lspe03_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lspe03_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lspe03_id'));
		$id .= ':'.$this->getState('filter.lspe03_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lspe03s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lspe03s').' AS a');
		...
		// Filter by a single or group of lspe03s.
		$lspe03_id = $this->getState('filter.lspe03_id');
		if ($lspe03_id != '')
		{
			if (is_numeric($lspe03_id))
			{
				$type = $this->getState('filter.lspe03_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lspe03_id);
			}
			else
			{
				if (is_array($lspe03_id))
				{
					JArrayHelper::toInteger($lspe03_id);
					$lspe03_id = implode(',', $lspe03_id);
					$type = $this->getState('filter.lspe03_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lspe03_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lspe03_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lspe03_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de resoluciones.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lspe03_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lspe03 params override menu item params only if menu param = 'use_lspe03'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lspe03' and there is no lspe03 param, use global

					// create an array of just the params set to 'use_lspe03'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lspe03')
						{
							// if the lspe03 has a value, use it
							if ($lspe03_params->get($key) != '')
							{
								// get the value from the lspe03
								$lspe03_array[$key] = $lspe03_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lspe03_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lspe03 params
					if (count($lspe03_array) > 0)
					{
						$lspe03_params = new Registry;
						$lspe03_params->loadArray($lspe03_array);
						$item->params->merge($lspe03_params);
					}


					// get display date
					switch ($item->params->get('list_show_lspe03_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lspe03, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
