...
class JtCaModelLjpdng18s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpdng18_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng18_id');
		$this->setState('filter.ljpdng18_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng18_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng18_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng18_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng18_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng18_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng18_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng18_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng18_id'));
		$id .= ':'.$this->getState('filter.ljpdng18_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng18s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng18s').' AS a');
		...
		// Filter by a single or group of ljpdng18s.
		$ljpdng18_id = $this->getState('filter.ljpdng18_id');
		if ($ljpdng18_id != '')
		{
			if (is_numeric($ljpdng18_id))
			{
				$type = $this->getState('filter.ljpdng18_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng18_id);
			}
			else
			{
				if (is_array($ljpdng18_id))
				{
					JArrayHelper::toInteger($ljpdng18_id);
					$ljpdng18_id = implode(',', $ljpdng18_id);
					$type = $this->getState('filter.ljpdng18_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng18_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng18_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng18_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng18_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng18 params override menu item params only if menu param = 'use_ljpdng18'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng18' and there is no ljpdng18 param, use global

					// create an array of just the params set to 'use_ljpdng18'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng18_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng18')
						{
							// if the ljpdng18 has a value, use it
							if ($ljpdng18_params->get($key) != '')
							{
								// get the value from the ljpdng18
								$ljpdng18_array[$key] = $ljpdng18_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng18_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng18 params
					if (count($ljpdng18_array) > 0)
					{
						$ljpdng18_params = new Registry;
						$ljpdng18_params->loadArray($ljpdng18_array);
						$item->params->merge($ljpdng18_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng18_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng18, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
