...
class JtCaModelLjjadg09s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadg09_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg09_id');
		$this->setState('filter.ljjadg09_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg09_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg09_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg09_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg09_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg09_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg09_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg09_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg09_id'));
		$id .= ':'.$this->getState('filter.ljjadg09_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg09s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg09s').' AS a');
		...
		// Filter by a single or group of ljjadg09s.
		$ljjadg09_id = $this->getState('filter.ljjadg09_id');
		if ($ljjadg09_id != '')
		{
			if (is_numeric($ljjadg09_id))
			{
				$type = $this->getState('filter.ljjadg09_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg09_id);
			}
			else
			{
				if (is_array($ljjadg09_id))
				{
					JArrayHelper::toInteger($ljjadg09_id);
					$ljjadg09_id = implode(',', $ljjadg09_id);
					$type = $this->getState('filter.ljjadg09_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg09_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg09_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg09_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg09_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg09 params override menu item params only if menu param = 'use_ljjadg09'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg09' and there is no ljjadg09 param, use global

					// create an array of just the params set to 'use_ljjadg09'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg09_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg09')
						{
							// if the ljjadg09 has a value, use it
							if ($ljjadg09_params->get($key) != '')
							{
								// get the value from the ljjadg09
								$ljjadg09_array[$key] = $ljjadg09_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg09_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg09 params
					if (count($ljjadg09_array) > 0)
					{
						$ljjadg09_params = new Registry;
						$ljjadg09_params->loadArray($ljjadg09_array);
						$item->params->merge($ljjadg09_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg09_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg09, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
