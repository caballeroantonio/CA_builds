...
/**
 * This models supports retrieving lists of libreta de libertades de salas penales.
 *
 */
class JtCaModelLcp33s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp33s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de libertades de salas penales has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp33_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp33_id');
		$this->setState('filter.lcp33_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp33_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp33_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp33_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp33_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp33_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp33_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp33_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp33_id'));
		$id .= ':'.$this->getState('filter.lcp33_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp33s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp33s').' AS a');
		...
		// Filter by a single or group of lcp33s.
		$lcp33_id = $this->getState('filter.lcp33_id');
		if ($lcp33_id != '')
		{
			if (is_numeric($lcp33_id))
			{
				$type = $this->getState('filter.lcp33_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp33_id);
			}
			else
			{
				if (is_array($lcp33_id))
				{
					JArrayHelper::toInteger($lcp33_id);
					$lcp33_id = implode(',', $lcp33_id);
					$type = $this->getState('filter.lcp33_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp33_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp33_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp33_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de libertades de salas penales.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp33_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp33 params override menu item params only if menu param = 'use_lcp33'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp33' and there is no lcp33 param, use global

					// create an array of just the params set to 'use_lcp33'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp33')
						{
							// if the lcp33 has a value, use it
							if ($lcp33_params->get($key) != '')
							{
								// get the value from the lcp33
								$lcp33_array[$key] = $lcp33_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp33_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp33 params
					if (count($lcp33_array) > 0)
					{
						$lcp33_params = new Registry;
						$lcp33_params->loadArray($lcp33_array);
						$item->params->merge($lcp33_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp33_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp33, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
