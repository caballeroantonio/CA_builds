...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpdng14.php';

class JtCaModelLjpdng14Form extends JtCaModelLjpdng14
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpdng14';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpdng14';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpdng14', 'ljpdng14', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpdng14.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpdng14.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpdng14.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjpdng14BeforeSave', array('com_jtca.ljpdng14', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpdng14AfterSave', array('com_jtca.ljpdng14', $table, $is_new));

		$this->setState('ljpdng14.id', $table->id);
