...
/**
 * TSJ CDMX Libros TxCA Component LIBRETA DE  OFICIOS DE AMPAROS DE CONOCIMIENTO. Model
 *
 */
class JtCaModelLcp16 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp16';
	...
	{
		if (empty($config['lcp16_filter_fields']))
		{
			$config['lcp16_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp16s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRETA DE  OFICIOS DE AMPAROS DE CONOCIMIENTO. data.
	 *
	 * @param	integer	$pk	The id of the libreta de  oficios de amparos de conocimiento..
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp16.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP16S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp16ChangeState', array('com_jtca.lcp16', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp16BeforeDelete', array('com_jtca.lcp16', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp16AfterDelete', array('com_jtca.lcp16', &$table));
				}
				else
				{
					// Prune items that you can't change.
