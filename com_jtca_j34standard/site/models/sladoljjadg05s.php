...
/**
 * This models supports retrieving lists of adolescentes del libro de sentencias.
 *
 */
class JtcaModelSladolJjadg05s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.sladol_jjadg05s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single sub libro adolescentes del libro de sentencias has been specified either as a parameter or in the url Request
		$pk = $params->get('sladol_jjadg05_id', '') == '' ? $app->input->getInt('id', '') : $params->get('sladol_jjadg05_id');
		$this->setState('filter.sladol_jjadg05_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('sladoljjadg05_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('sladoljjadg05_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('sladoljjadg05_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('sladoljjadg05_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('sladoljjadg05_initial_direction','ASC');
		}
		...
		if ($params->get('filter_sladoljjadg05_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_sladoljjadg05_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.sladol_jjadg05_id'));
		$id .= ':'.$this->getState('filter.sladol_jjadg05_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of sladol_jjadg05s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_sladol_jjadg05s').' AS a');
		...
		// Filter by a single or group of sladol_jjadg05s.
		$sladol_jjadg05_id = $this->getState('filter.sladol_jjadg05_id');
		if ($sladol_jjadg05_id != '')
		{
			if (is_numeric($sladol_jjadg05_id))
			{
				$type = $this->getState('filter.sladol_jjadg05_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $sladol_jjadg05_id);
			}
			else
			{
				if (is_array($sladol_jjadg05_id))
				{
					JArrayHelper::toInteger($sladol_jjadg05_id);
					$sladol_jjadg05_id = implode(',', $sladol_jjadg05_id);
					$type = $this->getState('filter.sladol_jjadg05_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$sladol_jjadg05_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_sladoljjadg05_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_sladoljjadg05_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of adolescentes del libro de sentencias.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$sladol_jjadg05_params = new Registry;
				...
				{
					$item->params = $sladol_jjadg05_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// SladolJjadg05 params override menu item params only if menu param = 'use_sladoljjadg05'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_sladoljjadg05' and there is no sladoljjadg05 param, use global

					// create an array of just the params set to 'use_sladoljjadg05'
					$menu_params_array = $this->getState('params')->toArray();
					$sladol_jjadg05_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_sladoljjadg05')
						{
							// if the sladoljjadg05 has a value, use it
							if ($sladol_jjadg05_params->get($key) != '')
							{
								// get the value from the sladoljjadg05
								$sladol_jjadg05_array[$key] = $sladol_jjadg05_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$sladol_jjadg05_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected sladoljjadg05 params
					if (count($sladol_jjadg05_array) > 0)
					{
						$sladol_jjadg05_params = new Registry;
						$sladol_jjadg05_params->loadArray($sladol_jjadg05_array);
						$item->params->merge($sladol_jjadg05_params);
					}


					// get display date
					switch ($item->params->get('list_show_sladoljjadg05_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an sladoljjadg05, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
