...
/**
 * This models supports retrieving lists of libro de turno de beneficios penitenciarios..
 *
 */
class JtCaModelLcp10s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp10s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno de beneficios penitenciarios. has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp10_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp10_id');
		$this->setState('filter.lcp10_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp10_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp10_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp10_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp10_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp10_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp10_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp10_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp10_id'));
		$id .= ':'.$this->getState('filter.lcp10_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp10s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp10s').' AS a');
		...
		// Filter by a single or group of lcp10s.
		$lcp10_id = $this->getState('filter.lcp10_id');
		if ($lcp10_id != '')
		{
			if (is_numeric($lcp10_id))
			{
				$type = $this->getState('filter.lcp10_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp10_id);
			}
			else
			{
				if (is_array($lcp10_id))
				{
					JArrayHelper::toInteger($lcp10_id);
					$lcp10_id = implode(',', $lcp10_id);
					$type = $this->getState('filter.lcp10_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp10_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp10_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp10_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno de beneficios penitenciarios..
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp10_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp10 params override menu item params only if menu param = 'use_lcp10'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp10' and there is no lcp10 param, use global

					// create an array of just the params set to 'use_lcp10'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp10')
						{
							// if the lcp10 has a value, use it
							if ($lcp10_params->get($key) != '')
							{
								// get the value from the lcp10
								$lcp10_array[$key] = $lcp10_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp10_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp10 params
					if (count($lcp10_array) > 0)
					{
						$lcp10_params = new Registry;
						$lcp10_params->loadArray($lcp10_array);
						$item->params->merge($lcp10_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp10_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp10, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
