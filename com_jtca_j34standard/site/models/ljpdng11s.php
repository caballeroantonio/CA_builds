...
/**
 * This models supports retrieving lists of libro de amparos circulantes o negados.
 *
 */
class JtcaModelLjpdng11s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpdng11s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de amparos circulantes o negados has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpdng11_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng11_id');
		$this->setState('filter.ljpdng11_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng11_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng11_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng11_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng11_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng11_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng11_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng11_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng11_id'));
		$id .= ':'.$this->getState('filter.ljpdng11_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng11s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng11s').' AS a');
		...
		// Filter by a single or group of ljpdng11s.
		$ljpdng11_id = $this->getState('filter.ljpdng11_id');
		if ($ljpdng11_id != '')
		{
			if (is_numeric($ljpdng11_id))
			{
				$type = $this->getState('filter.ljpdng11_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng11_id);
			}
			else
			{
				if (is_array($ljpdng11_id))
				{
					JArrayHelper::toInteger($ljpdng11_id);
					$ljpdng11_id = implode(',', $ljpdng11_id);
					$type = $this->getState('filter.ljpdng11_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng11_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng11_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng11_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de amparos circulantes o negados.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng11_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng11 params override menu item params only if menu param = 'use_ljpdng11'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng11' and there is no ljpdng11 param, use global

					// create an array of just the params set to 'use_ljpdng11'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng11_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng11')
						{
							// if the ljpdng11 has a value, use it
							if ($ljpdng11_params->get($key) != '')
							{
								// get the value from the ljpdng11
								$ljpdng11_array[$key] = $ljpdng11_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng11_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng11 params
					if (count($ljpdng11_array) > 0)
					{
						$ljpdng11_params = new Registry;
						$ljpdng11_params->loadArray($ljpdng11_array);
						$item->params->merge($ljpdng11_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng11_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng11, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
