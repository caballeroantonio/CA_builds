...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsc12.php';

class JtCaModelLsc12Form extends JtCaModelLsc12
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsc12';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsc12';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsc12', 'lsc12', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de registro de amparos (oficial).
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE REGISTRO DE AMPAROS (OFICIAL) item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsc12.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsc12.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsc12.id');
		$is_new		= true;
		...
		// Reorder the libro de registro de amparos (oficial) so the new libro de registro de amparos (oficial) is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsc12BeforeSave', array('com_jtca.lsc12', &$table, $is_new));
		...
		$dispatcher->trigger('onLsc12AfterSave', array('com_jtca.lsc12', $table, $is_new));

		$this->setState('lsc12.id', $table->id);
