...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE INGRESO Y EGRESO DE BILLETES Model
 *
 */
class JtCaModelLspe08 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lspe08';
	...
	{
		if (empty($config['lspe08_filter_fields']))
		{
			$config['lspe08_filter_fields'] = array(
				...
				'field2337','a.field2337',
				'field2338','a.field2338',
				'field2339','a.field2339',
				'field2340_isMoral','a.field2340_isMoral',
				'field2340_paterno','a.field2340_paterno',
				'field2340_materno','a.field2340_materno',
				'field2340_nombre','a.field2340_nombre',
				'field2341','a.field2341',
				'field2342','a.field2342',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lspe08s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE INGRESO Y EGRESO DE BILLETES data.
	 *
	 * @param	integer	$pk	The id of the libro de ingreso y egreso de billetes.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lspe08.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPE08S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLspe08ChangeState', array('com_jtca.lspe08', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLspe08BeforeDelete', array('com_jtca.lspe08', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLspe08AfterDelete', array('com_jtca.lspe08', &$table));
				}
				else
				{
					// Prune items that you can't change.
