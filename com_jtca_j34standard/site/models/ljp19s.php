...
/**
 * This models supports retrieving lists of libro de control de firmas de procesados.
 *
 */
class JtcaModelLjp19s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljp19s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de control de firmas de procesados has been specified either as a parameter or in the url Request
		$pk = $params->get('ljp19_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp19_id');
		$this->setState('filter.ljp19_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp19_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp19_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp19_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp19_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp19_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp19_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp19_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp19_id'));
		$id .= ':'.$this->getState('filter.ljp19_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp19s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp19s').' AS a');
		...
		// Filter by a single or group of ljp19s.
		$ljp19_id = $this->getState('filter.ljp19_id');
		if ($ljp19_id != '')
		{
			if (is_numeric($ljp19_id))
			{
				$type = $this->getState('filter.ljp19_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp19_id);
			}
			else
			{
				if (is_array($ljp19_id))
				{
					JArrayHelper::toInteger($ljp19_id);
					$ljp19_id = implode(',', $ljp19_id);
					$type = $this->getState('filter.ljp19_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp19_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp19_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp19_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de control de firmas de procesados.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp19_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp19 params override menu item params only if menu param = 'use_ljp19'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp19' and there is no ljp19 param, use global

					// create an array of just the params set to 'use_ljp19'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp19')
						{
							// if the ljp19 has a value, use it
							if ($ljp19_params->get($key) != '')
							{
								// get the value from the ljp19
								$ljp19_array[$key] = $ljp19_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp19_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp19 params
					if (count($ljp19_array) > 0)
					{
						$ljp19_params = new Registry;
						$ljp19_params->loadArray($ljp19_array);
						$item->params->merge($ljp19_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp19_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp19, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
