...
/**
 * This models supports retrieving lists of inculpado.
 *
 */
class JtcaModelSlpersonaliases extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.slpersonaliases';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single inculpado has been specified either as a parameter or in the url Request
		$pk = $params->get('slpersonalias_id', '') == '' ? $app->input->getInt('id', '') : $params->get('slpersonalias_id');
		$this->setState('filter.slpersonalias_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('slpersonalias_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('slpersonalias_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('slpersonalias_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('slpersonalias_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('slpersonalias_initial_direction','ASC');
		}
		...
		if ($params->get('filter_slpersonalias_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_slpersonalias_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.slpersonalias_id'));
		$id .= ':'.$this->getState('filter.slpersonalias_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of slpersonaliases subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_slpersonaliases').' AS a');
		...
		// Filter by a single or group of slpersonaliases.
		$slpersonalias_id = $this->getState('filter.slpersonalias_id');
		if ($slpersonalias_id != '')
		{
			if (is_numeric($slpersonalias_id))
			{
				$type = $this->getState('filter.slpersonalias_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $slpersonalias_id);
			}
			else
			{
				if (is_array($slpersonalias_id))
				{
					JArrayHelper::toInteger($slpersonalias_id);
					$slpersonalias_id = implode(',', $slpersonalias_id);
					$type = $this->getState('filter.slpersonalias_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$slpersonalias_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_slpersonalias_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_slpersonalias_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of inculpado.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$slpersonalias_params = new Registry;
				...
				{
					$item->params = $slpersonalias_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Slpersonalias params override menu item params only if menu param = 'use_slpersonalias'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_slpersonalias' and there is no slpersonalias param, use global

					// create an array of just the params set to 'use_slpersonalias'
					$menu_params_array = $this->getState('params')->toArray();
					$slpersonalias_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_slpersonalias')
						{
							// if the slpersonalias has a value, use it
							if ($slpersonalias_params->get($key) != '')
							{
								// get the value from the slpersonalias
								$slpersonalias_array[$key] = $slpersonalias_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$slpersonalias_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected slpersonalias params
					if (count($slpersonalias_array) > 0)
					{
						$slpersonalias_params = new Registry;
						$slpersonalias_params->loadArray($slpersonalias_array);
						$item->params->merge($slpersonalias_params);
					}


					// get display date
					switch ($item->params->get('list_show_slpersonalias_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an slpersonalias, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
