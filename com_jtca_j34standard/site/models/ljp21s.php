...
class JtCaModelLjp21s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljp21_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp21_id');
		$this->setState('filter.ljp21_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp21_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp21_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp21_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp21_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp21_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp21_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp21_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp21_id'));
		$id .= ':'.$this->getState('filter.ljp21_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp21s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp21s').' AS a');
		...
		// Filter by a single or group of ljp21s.
		$ljp21_id = $this->getState('filter.ljp21_id');
		if ($ljp21_id != '')
		{
			if (is_numeric($ljp21_id))
			{
				$type = $this->getState('filter.ljp21_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp21_id);
			}
			else
			{
				if (is_array($ljp21_id))
				{
					JArrayHelper::toInteger($ljp21_id);
					$ljp21_id = implode(',', $ljp21_id);
					$type = $this->getState('filter.ljp21_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp21_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp21_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp21_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp21_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp21 params override menu item params only if menu param = 'use_ljp21'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp21' and there is no ljp21 param, use global

					// create an array of just the params set to 'use_ljp21'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp21')
						{
							// if the ljp21 has a value, use it
							if ($ljp21_params->get($key) != '')
							{
								// get the value from the ljp21
								$ljp21_array[$key] = $ljp21_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp21_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp21 params
					if (count($ljp21_array) > 0)
					{
						$ljp21_params = new Registry;
						$ljp21_params->loadArray($ljp21_array);
						$item->params->merge($ljp21_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp21_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp21, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
