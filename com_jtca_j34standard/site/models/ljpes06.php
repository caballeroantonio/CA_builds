 ...
 *
 */
class JtCaModelLjpes06 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpes06';
	...
	{
		if (empty($config['ljpes06_filter_fields']))
		{
			$config['ljpes06_filter_fields'] = array(
				'id', 'a.id',
				...
				'txt_field1233','a.txt_field1233',
				'id_field1233','a.id_field1233',
				'field6','a.field6',
				'field7','a.field7',
				'field8','a.field8',
				'state', 'a.state',
	...
	*/
	public function getTable($type = 'Ljpes06s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpes06.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPES06S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpes06ChangeState', array('com_jtca.ljpes06', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpes06BeforeDelete', array('com_jtca.ljpes06', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpes06AfterDelete', array('com_jtca.ljpes06', &$table));
				}
				else
				{
					// Prune items that you can't change.
