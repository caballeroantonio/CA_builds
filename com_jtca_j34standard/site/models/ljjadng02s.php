...
/**
 * This models supports retrieving lists of libro de promociones.
 *
 */
class JtcaModelLjjadng02s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadng02s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de promociones has been specified either as a parameter or in the url Request
		$pk = $params->get('ljjadng02_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadng02_id');
		$this->setState('filter.ljjadng02_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadng02_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadng02_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadng02_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadng02_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadng02_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadng02_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadng02_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadng02_id'));
		$id .= ':'.$this->getState('filter.ljjadng02_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadng02s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadng02s').' AS a');
		...
		// Filter by a single or group of ljjadng02s.
		$ljjadng02_id = $this->getState('filter.ljjadng02_id');
		if ($ljjadng02_id != '')
		{
			if (is_numeric($ljjadng02_id))
			{
				$type = $this->getState('filter.ljjadng02_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadng02_id);
			}
			else
			{
				if (is_array($ljjadng02_id))
				{
					JArrayHelper::toInteger($ljjadng02_id);
					$ljjadng02_id = implode(',', $ljjadng02_id);
					$type = $this->getState('filter.ljjadng02_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadng02_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadng02_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadng02_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de promociones.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadng02_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadng02 params override menu item params only if menu param = 'use_ljjadng02'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadng02' and there is no ljjadng02 param, use global

					// create an array of just the params set to 'use_ljjadng02'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadng02_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadng02')
						{
							// if the ljjadng02 has a value, use it
							if ($ljjadng02_params->get($key) != '')
							{
								// get the value from the ljjadng02
								$ljjadng02_array[$key] = $ljjadng02_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadng02_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadng02 params
					if (count($ljjadng02_array) > 0)
					{
						$ljjadng02_params = new Registry;
						$ljjadng02_params->loadArray($ljjadng02_array);
						$item->params->merge($ljjadng02_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadng02_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadng02, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
