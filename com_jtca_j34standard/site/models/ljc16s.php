...
/**
 * This models supports retrieving lists of jc16.
 *
 */
class JtCaModelLjc16s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljc16s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single agenda de audiencias has been specified either as a parameter or in the url Request
		$pk = $params->get('ljc16_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc16_id');
		$this->setState('filter.ljc16_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc16_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc16_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc16_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc16_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc16_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc16_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc16_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc16_id'));
		$id .= ':'.$this->getState('filter.ljc16_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc16s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc16s').' AS a');
		...
		// Filter by a single or group of ljc16s.
		$ljc16_id = $this->getState('filter.ljc16_id');
		if ($ljc16_id != '')
		{
			if (is_numeric($ljc16_id))
			{
				$type = $this->getState('filter.ljc16_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc16_id);
			}
			else
			{
				if (is_array($ljc16_id))
				{
					JArrayHelper::toInteger($ljc16_id);
					$ljc16_id = implode(',', $ljc16_id);
					$type = $this->getState('filter.ljc16_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc16_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc16_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc16_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc16_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc16 params override menu item params only if menu param = 'use_ljc16'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc16' and there is no ljc16 param, use global

					// create an array of just the params set to 'use_ljc16'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc16')
						{
							// if the ljc16 has a value, use it
							if ($ljc16_params->get($key) != '')
							{
								// get the value from the ljc16
								$ljc16_array[$key] = $ljc16_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc16_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc16 params
					if (count($ljc16_array) > 0)
					{
						$ljc16_params = new Registry;
						$ljc16_params->loadArray($ljc16_array);
						$item->params->merge($ljc16_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc16_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc16, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
