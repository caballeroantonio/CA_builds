...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljomf15.php';

class JtCaModelLjomf15Form extends JtCaModelLjomf15
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljomf15';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljomf15';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljomf15', 'ljomf15', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno a cargo de los secretarios de acuerdos.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO A CARGO DE LOS SECRETARIOS DE ACUERDOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljomf15.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljomf15.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljomf15.id');
		$is_new		= true;
		...
		// Reorder the libro de turno a cargo de los secretarios de acuer so the new libro de turno a cargo de los secretarios de acuerdos is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjomf15BeforeSave', array('com_jtca.ljomf15', &$table, $is_new));
		...
		$dispatcher->trigger('onLjomf15AfterSave', array('com_jtca.ljomf15', $table, $is_new));

		$this->setState('ljomf15.id', $table->id);
