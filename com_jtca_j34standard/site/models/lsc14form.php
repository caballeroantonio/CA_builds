...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsc14.php';

class JtCaModelLsc14Form extends JtCaModelLsc14
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsc14';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsc14';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsc14', 'lsc14', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de pólizas y fianzas (oficial) - egresos.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE PÓLIZAS Y FIANZAS (OFICIAL) - Egresos item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsc14.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsc14.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsc14.id');
		$is_new		= true;
		...
		// Reorder the libro de pólizas y fianzas (oficial) - egresos so the new libro de pólizas y fianzas (oficial) - egresos is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsc14BeforeSave', array('com_jtca.lsc14', &$table, $is_new));
		...
		$dispatcher->trigger('onLsc14AfterSave', array('com_jtca.lsc14', $table, $is_new));

		$this->setState('lsc14.id', $table->id);
