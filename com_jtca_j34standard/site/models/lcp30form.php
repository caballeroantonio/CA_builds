...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp30.php';

class JtCaModelLcp30Form extends JtCaModelLcp30
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp30';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp30';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp30', 'lcp30', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de libertades juzgados de primera instancia.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE LIBERTADES JUZGADOS DE PRIMERA INSTANCIA item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp30.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp30.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp30.id');
		$is_new		= true;
		...
		// Reorder the libreta de libertades juzgados de primera instanci so the new libreta de libertades juzgados de primera instancia is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp30BeforeSave', array('com_jtca.lcp30', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp30AfterSave', array('com_jtca.lcp30', $table, $is_new));

		$this->setState('lcp30.id', $table->id);
