...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpdng08.php';

class JtCaModelLjpdng08Form extends JtCaModelLjpdng08
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpdng08';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpdng08';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpdng08', 'ljpdng08', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpdng08.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpdng08.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpdng08.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjpdng08BeforeSave', array('com_jtca.ljpdng08', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpdng08AfterSave', array('com_jtca.ljpdng08', $table, $is_new));

		$this->setState('ljpdng08.id', $table->id);
