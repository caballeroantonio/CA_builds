...
class JtCaModelLjf14s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf14_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf14_id');
		$this->setState('filter.ljf14_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf14_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf14_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf14_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf14_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf14_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf14_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf14_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf14_id'));
		$id .= ':'.$this->getState('filter.ljf14_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf14s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf14s').' AS a');
		...
		// Filter by a single or group of ljf14s.
		$ljf14_id = $this->getState('filter.ljf14_id');
		if ($ljf14_id != '')
		{
			if (is_numeric($ljf14_id))
			{
				$type = $this->getState('filter.ljf14_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf14_id);
			}
			else
			{
				if (is_array($ljf14_id))
				{
					JArrayHelper::toInteger($ljf14_id);
					$ljf14_id = implode(',', $ljf14_id);
					$type = $this->getState('filter.ljf14_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf14_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf14_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf14_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf14_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf14 params override menu item params only if menu param = 'use_ljf14'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf14' and there is no ljf14 param, use global

					// create an array of just the params set to 'use_ljf14'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf14')
						{
							// if the ljf14 has a value, use it
							if ($ljf14_params->get($key) != '')
							{
								// get the value from the ljf14
								$ljf14_array[$key] = $ljf14_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf14_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf14 params
					if (count($ljf14_array) > 0)
					{
						$ljf14_params = new Registry;
						$ljf14_params->loadArray($ljf14_array);
						$item->params->merge($ljf14_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf14_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf14, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
