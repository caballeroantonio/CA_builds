...
/**
 * This models supports retrieving lists of libro de registro de billetes de depósito (ingreso.
 *
 */
class JtcaModelLjp02s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljp02s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de registro de billetes de depósito (ingresos) has been specified either as a parameter or in the url Request
		$pk = $params->get('ljp02_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp02_id');
		$this->setState('filter.ljp02_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp02_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp02_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp02_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp02_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp02_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp02_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp02_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp02_id'));
		$id .= ':'.$this->getState('filter.ljp02_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp02s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp02s').' AS a');
		...
		// Filter by a single or group of ljp02s.
		$ljp02_id = $this->getState('filter.ljp02_id');
		if ($ljp02_id != '')
		{
			if (is_numeric($ljp02_id))
			{
				$type = $this->getState('filter.ljp02_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp02_id);
			}
			else
			{
				if (is_array($ljp02_id))
				{
					JArrayHelper::toInteger($ljp02_id);
					$ljp02_id = implode(',', $ljp02_id);
					$type = $this->getState('filter.ljp02_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp02_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp02_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp02_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de registro de billetes de depósito (ingreso.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp02_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp02 params override menu item params only if menu param = 'use_ljp02'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp02' and there is no ljp02 param, use global

					// create an array of just the params set to 'use_ljp02'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp02')
						{
							// if the ljp02 has a value, use it
							if ($ljp02_params->get($key) != '')
							{
								// get the value from the ljp02
								$ljp02_array[$key] = $ljp02_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp02_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp02 params
					if (count($ljp02_array) > 0)
					{
						$ljp02_params = new Registry;
						$ljp02_params->loadArray($ljp02_array);
						$item->params->merge($ljp02_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp02_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp02, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
