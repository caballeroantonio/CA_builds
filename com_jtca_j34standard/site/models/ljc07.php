 ...
 *
 */
class JtCaModelLjc07 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljc07';
	...
	{
		if (empty($config['ljc07_filter_fields']))
		{
			$config['ljc07_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljc07.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJC07S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjc07ChangeState', array('com_jtca.ljc07', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjc07BeforeDelete', array('com_jtca.ljc07', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjc07AfterDelete', array('com_jtca.ljc07', &$table));
				}
				else
				{
					// Prune items that you can't change.
