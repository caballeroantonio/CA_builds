...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE CONTROL DE MULTAS Model
 *
 */
class JtcaModelLsps10 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsps10';
	...
	{
		if (empty($config['lsps10_filter_fields']))
		{
			$config['lsps10_filter_fields'] = array(
				'id', 'a.id',
				'id_organo','a.id_organo',
				'id_secretaria','a.id_secretaria',
				'anoj','a.anoj',
				'id_expediente','a.id_expediente',
				'field2426','a.field2426',
				'field2427','a.field2427',
				'field2428','a.field2428',
				'field2429_isMoral','a.field2429_isMoral',
				'field2429_paterno','a.field2429_paterno',
				'field2429_materno','a.field2429_materno',
				'field2429_nombre','a.field2429_nombre',
				'field2430','a.field2430',
				'field2446','a.field2446',
				'field2431','a.field2431',
				'field2432','a.field2432',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lsps10s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE CONTROL DE MULTAS data.
	 *
	 * @param	integer	$pk	The id of the libro de control de multas.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsps10.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPS10S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsps10ChangeState', array('com_jtca.lsps10', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsps10BeforeDelete', array('com_jtca.lsps10', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsps10AfterDelete', array('com_jtca.lsps10', &$table));
				}
				else
				{
					// Prune items that you can't change.
