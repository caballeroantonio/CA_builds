...
/**
 * This models supports retrieving lists of adolescente del libro de gobierno.
 *
 */
class JtcaModelSladolJjadg01s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.sladol_jjadg01s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single sub libro adolescente del libro de gobierno has been specified either as a parameter or in the url Request
		$pk = $params->get('sladol_jjadg01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('sladol_jjadg01_id');
		$this->setState('filter.sladol_jjadg01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('sladoljjadg01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('sladoljjadg01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('sladoljjadg01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('sladoljjadg01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('sladoljjadg01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_sladoljjadg01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_sladoljjadg01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.sladol_jjadg01_id'));
		$id .= ':'.$this->getState('filter.sladol_jjadg01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of sladol_jjadg01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_sladol_jjadg01s').' AS a');
		...
		// Filter by a single or group of sladol_jjadg01s.
		$sladol_jjadg01_id = $this->getState('filter.sladol_jjadg01_id');
		if ($sladol_jjadg01_id != '')
		{
			if (is_numeric($sladol_jjadg01_id))
			{
				$type = $this->getState('filter.sladol_jjadg01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $sladol_jjadg01_id);
			}
			else
			{
				if (is_array($sladol_jjadg01_id))
				{
					JArrayHelper::toInteger($sladol_jjadg01_id);
					$sladol_jjadg01_id = implode(',', $sladol_jjadg01_id);
					$type = $this->getState('filter.sladol_jjadg01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$sladol_jjadg01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_sladoljjadg01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_sladoljjadg01_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of adolescente del libro de gobierno.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$sladol_jjadg01_params = new Registry;
				...
				{
					$item->params = $sladol_jjadg01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// SladolJjadg01 params override menu item params only if menu param = 'use_sladoljjadg01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_sladoljjadg01' and there is no sladoljjadg01 param, use global

					// create an array of just the params set to 'use_sladoljjadg01'
					$menu_params_array = $this->getState('params')->toArray();
					$sladol_jjadg01_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_sladoljjadg01')
						{
							// if the sladoljjadg01 has a value, use it
							if ($sladol_jjadg01_params->get($key) != '')
							{
								// get the value from the sladoljjadg01
								$sladol_jjadg01_array[$key] = $sladol_jjadg01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$sladol_jjadg01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected sladoljjadg01 params
					if (count($sladol_jjadg01_array) > 0)
					{
						$sladol_jjadg01_params = new Registry;
						$sladol_jjadg01_params->loadArray($sladol_jjadg01_array);
						$item->params->merge($sladol_jjadg01_params);
					}


					// get display date
					switch ($item->params->get('list_show_sladoljjadg01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an sladoljjadg01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
