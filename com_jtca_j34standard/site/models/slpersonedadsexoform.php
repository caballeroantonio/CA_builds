...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/slpersonedadsexo.php';

class JtcaModelSlpersonedadsexoForm extends JtcaModelSlpersonedadsexo
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.slpersonedadsexo';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.slpersonedadsexo';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.slpersonedadsexo', 'slpersonedadsexo', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the adolescente.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Adolescente item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('slpersonedadsexo.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.slpersonedadsexo.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('slpersonedadsexo.id');
		$is_new		= true;
		...
		// Reorder the adolescente so the new adolescente is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSlpersonedadsexoBeforeSave', array('com_jtca.slpersonedadsexo', &$table, $is_new));
		...
		$dispatcher->trigger('onSlpersonedadsexoAfterSave', array('com_jtca.slpersonedadsexo', $table, $is_new));

		$this->setState('slpersonedadsexo.id', $table->id);
