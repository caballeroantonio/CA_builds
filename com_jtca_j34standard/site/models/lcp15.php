...
/**
 * TSJ CDMX Libros TxCA Component LIBRETA DE OFICIOS DE AMPAROS DE TRAMITE. Model
 *
 */
class JtcaModelLcp15 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp15';
	...
	{
		if (empty($config['lcp15_filter_fields']))
		{
			$config['lcp15_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp15s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRETA DE OFICIOS DE AMPAROS DE TRAMITE. data.
	 *
	 * @param	integer	$pk	The id of the libreta de oficios de amparos de tramite..
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp15.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP15S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp15ChangeState', array('com_jtca.lcp15', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp15BeforeDelete', array('com_jtca.lcp15', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp15AfterDelete', array('com_jtca.lcp15', &$table));
				}
				else
				{
					// Prune items that you can't change.
