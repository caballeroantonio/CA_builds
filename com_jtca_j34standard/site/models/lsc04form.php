...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsc04.php';

class JtCaModelLsc04Form extends JtCaModelLsc04
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsc04';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsc04';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsc04', 'lsc04', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de multas (oficial).
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE MULTAS (OFICIAL) item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsc04.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsc04.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsc04.id');
		$is_new		= true;
		...
		// Reorder the libro de multas (oficial) so the new libro de multas (oficial) is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsc04BeforeSave', array('com_jtca.lsc04', &$table, $is_new));
		...
		$dispatcher->trigger('onLsc04AfterSave', array('com_jtca.lsc04', $table, $is_new));

		$this->setState('lsc04.id', $table->id);
