...
/**
 * This models supports retrieving lists of control de firmas del libro de control de firmas p.
 *
 */
class JtcaModelSlfirmJpdng17s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.slfirm_jpdng17s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single sub libro control de firmas del libro de control de firmas procesados has been specified either as a parameter or in the url Request
		$pk = $params->get('slfirm_jpdng17_id', '') == '' ? $app->input->getInt('id', '') : $params->get('slfirm_jpdng17_id');
		$this->setState('filter.slfirm_jpdng17_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('slfirmjpdng17_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('slfirmjpdng17_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('slfirmjpdng17_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('slfirmjpdng17_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('slfirmjpdng17_initial_direction','ASC');
		}
		...
		if ($params->get('filter_slfirmjpdng17_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_slfirmjpdng17_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.slfirm_jpdng17_id'));
		$id .= ':'.$this->getState('filter.slfirm_jpdng17_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of slfirm_jpdng17s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_slfirm_jpdng17s').' AS a');
		...
		// Filter by a single or group of slfirm_jpdng17s.
		$slfirm_jpdng17_id = $this->getState('filter.slfirm_jpdng17_id');
		if ($slfirm_jpdng17_id != '')
		{
			if (is_numeric($slfirm_jpdng17_id))
			{
				$type = $this->getState('filter.slfirm_jpdng17_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $slfirm_jpdng17_id);
			}
			else
			{
				if (is_array($slfirm_jpdng17_id))
				{
					JArrayHelper::toInteger($slfirm_jpdng17_id);
					$slfirm_jpdng17_id = implode(',', $slfirm_jpdng17_id);
					$type = $this->getState('filter.slfirm_jpdng17_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$slfirm_jpdng17_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_slfirmjpdng17_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_slfirmjpdng17_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of control de firmas del libro de control de firmas p.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$slfirm_jpdng17_params = new Registry;
				...
				{
					$item->params = $slfirm_jpdng17_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// SlfirmJpdng17 params override menu item params only if menu param = 'use_slfirmjpdng17'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_slfirmjpdng17' and there is no slfirmjpdng17 param, use global

					// create an array of just the params set to 'use_slfirmjpdng17'
					$menu_params_array = $this->getState('params')->toArray();
					$slfirm_jpdng17_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_slfirmjpdng17')
						{
							// if the slfirmjpdng17 has a value, use it
							if ($slfirm_jpdng17_params->get($key) != '')
							{
								// get the value from the slfirmjpdng17
								$slfirm_jpdng17_array[$key] = $slfirm_jpdng17_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$slfirm_jpdng17_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected slfirmjpdng17 params
					if (count($slfirm_jpdng17_array) > 0)
					{
						$slfirm_jpdng17_params = new Registry;
						$slfirm_jpdng17_params->loadArray($slfirm_jpdng17_array);
						$item->params->merge($slfirm_jpdng17_params);
					}


					// get display date
					switch ($item->params->get('list_show_slfirmjpdng17_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an slfirmjpdng17, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
