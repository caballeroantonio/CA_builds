...
class JtcaModelLsc08 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsc08';
	...
	{
		if (empty($config['lsc08_filter_fields']))
		{
			$config['lsc08_filter_fields'] = array(
				'id', 'a.id',
				'billete', 'a.billete',
	...
	*/
	...
	/**
	 * Method to get LIBRO DE PÓLIZAS Y FIANZAS (OFICIAL) - Ingresos data.
	 *
	 * @param	integer	$pk	The id of the libro de pólizas y fianzas (oficial) - ingresos.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsc08.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSC08S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsc08ChangeState', array('com_jtca.lsc08', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsc08BeforeDelete', array('com_jtca.lsc08', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsc08AfterDelete', array('com_jtca.lsc08', &$table));
				}
				else
				{
					// Prune items that you can't change.
