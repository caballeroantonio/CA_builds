...
/**
 * This models supports retrieving lists of registro de promociones.
 *
 */
class JtCaModelLsps12s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsps12s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single registro de promociones has been specified either as a parameter or in the url Request
		$pk = $params->get('lsps12_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsps12_id');
		$this->setState('filter.lsps12_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsps12_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsps12_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsps12_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsps12_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsps12_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsps12_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsps12_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsps12_id'));
		$id .= ':'.$this->getState('filter.lsps12_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsps12s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsps12s').' AS a');
		...
		// Filter by a single or group of lsps12s.
		$lsps12_id = $this->getState('filter.lsps12_id');
		if ($lsps12_id != '')
		{
			if (is_numeric($lsps12_id))
			{
				$type = $this->getState('filter.lsps12_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsps12_id);
			}
			else
			{
				if (is_array($lsps12_id))
				{
					JArrayHelper::toInteger($lsps12_id);
					$lsps12_id = implode(',', $lsps12_id);
					$type = $this->getState('filter.lsps12_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsps12_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsps12_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsps12_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of registro de promociones.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsps12_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsps12 params override menu item params only if menu param = 'use_lsps12'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsps12' and there is no lsps12 param, use global

					// create an array of just the params set to 'use_lsps12'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsps12')
						{
							// if the lsps12 has a value, use it
							if ($lsps12_params->get($key) != '')
							{
								// get the value from the lsps12
								$lsps12_array[$key] = $lsps12_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsps12_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsps12 params
					if (count($lsps12_array) > 0)
					{
						$lsps12_params = new Registry;
						$lsps12_params->loadArray($lsps12_array);
						$item->params->merge($lsps12_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsps12_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsps12, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
