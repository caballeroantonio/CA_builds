...
/**
 * This models supports retrieving lists of control de firmas del libro de control de presenta.
 *
 */
class JtcaModelSlfirmJjadg16s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.slfirm_jjadg16s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single sub libro control de firmas del libro de control de presentacion de adolescentes en libertad ante el juez has been specified either as a parameter or in the url Request
		$pk = $params->get('slfirm_jjadg16_id', '') == '' ? $app->input->getInt('id', '') : $params->get('slfirm_jjadg16_id');
		$this->setState('filter.slfirm_jjadg16_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('slfirmjjadg16_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('slfirmjjadg16_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('slfirmjjadg16_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('slfirmjjadg16_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('slfirmjjadg16_initial_direction','ASC');
		}
		...
		if ($params->get('filter_slfirmjjadg16_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_slfirmjjadg16_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.slfirm_jjadg16_id'));
		$id .= ':'.$this->getState('filter.slfirm_jjadg16_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of slfirm_jjadg16s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_slfirm_jjadg16s').' AS a');
		...
		// Filter by a single or group of slfirm_jjadg16s.
		$slfirm_jjadg16_id = $this->getState('filter.slfirm_jjadg16_id');
		if ($slfirm_jjadg16_id != '')
		{
			if (is_numeric($slfirm_jjadg16_id))
			{
				$type = $this->getState('filter.slfirm_jjadg16_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $slfirm_jjadg16_id);
			}
			else
			{
				if (is_array($slfirm_jjadg16_id))
				{
					JArrayHelper::toInteger($slfirm_jjadg16_id);
					$slfirm_jjadg16_id = implode(',', $slfirm_jjadg16_id);
					$type = $this->getState('filter.slfirm_jjadg16_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$slfirm_jjadg16_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_slfirmjjadg16_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_slfirmjjadg16_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of control de firmas del libro de control de presenta.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$slfirm_jjadg16_params = new Registry;
				...
				{
					$item->params = $slfirm_jjadg16_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// SlfirmJjadg16 params override menu item params only if menu param = 'use_slfirmjjadg16'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_slfirmjjadg16' and there is no slfirmjjadg16 param, use global

					// create an array of just the params set to 'use_slfirmjjadg16'
					$menu_params_array = $this->getState('params')->toArray();
					$slfirm_jjadg16_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_slfirmjjadg16')
						{
							// if the slfirmjjadg16 has a value, use it
							if ($slfirm_jjadg16_params->get($key) != '')
							{
								// get the value from the slfirmjjadg16
								$slfirm_jjadg16_array[$key] = $slfirm_jjadg16_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$slfirm_jjadg16_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected slfirmjjadg16 params
					if (count($slfirm_jjadg16_array) > 0)
					{
						$slfirm_jjadg16_params = new Registry;
						$slfirm_jjadg16_params->loadArray($slfirm_jjadg16_array);
						$item->params->merge($slfirm_jjadg16_params);
					}


					// get display date
					switch ($item->params->get('list_show_slfirmjjadg16_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an slfirmjjadg16, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
