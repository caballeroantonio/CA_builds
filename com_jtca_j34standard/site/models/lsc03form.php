...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsc03.php';

class JtCaModelLsc03Form extends JtCaModelLsc03
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsc03';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsc03';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsc03', 'lsc03', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de vistas al m.p (oficial).
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE VISTAS AL M.P (OFICIAL) item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsc03.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsc03.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsc03.id');
		$is_new		= true;
		...
		// Reorder the libreta de vistas al m.p (oficial) so the new libreta de vistas al m.p (oficial) is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsc03BeforeSave', array('com_jtca.lsc03', &$table, $is_new));
		...
		$dispatcher->trigger('onLsc03AfterSave', array('com_jtca.lsc03', $table, $is_new));

		$this->setState('lsc03.id', $table->id);
