...
/**
 * This models supports retrieving lists of datos del(os) ofendido(s) del libro de gobierno.
 *
 */
class JtcaModelSlofenJpdng01s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.slofen_jpdng01s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single sub libro datos del(os) ofendido(s) del libro de gobierno has been specified either as a parameter or in the url Request
		$pk = $params->get('slofen_jpdng01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('slofen_jpdng01_id');
		$this->setState('filter.slofen_jpdng01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('slofenjpdng01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('slofenjpdng01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('slofenjpdng01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('slofenjpdng01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('slofenjpdng01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_slofenjpdng01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_slofenjpdng01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.slofen_jpdng01_id'));
		$id .= ':'.$this->getState('filter.slofen_jpdng01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of slofen_jpdng01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_slofen_jpdng01s').' AS a');
		...
		// Filter by a single or group of slofen_jpdng01s.
		$slofen_jpdng01_id = $this->getState('filter.slofen_jpdng01_id');
		if ($slofen_jpdng01_id != '')
		{
			if (is_numeric($slofen_jpdng01_id))
			{
				$type = $this->getState('filter.slofen_jpdng01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $slofen_jpdng01_id);
			}
			else
			{
				if (is_array($slofen_jpdng01_id))
				{
					JArrayHelper::toInteger($slofen_jpdng01_id);
					$slofen_jpdng01_id = implode(',', $slofen_jpdng01_id);
					$type = $this->getState('filter.slofen_jpdng01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$slofen_jpdng01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_slofenjpdng01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_slofenjpdng01_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of datos del(os) ofendido(s) del libro de gobierno.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$slofen_jpdng01_params = new Registry;
				...
				{
					$item->params = $slofen_jpdng01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// SlofenJpdng01 params override menu item params only if menu param = 'use_slofenjpdng01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_slofenjpdng01' and there is no slofenjpdng01 param, use global

					// create an array of just the params set to 'use_slofenjpdng01'
					$menu_params_array = $this->getState('params')->toArray();
					$slofen_jpdng01_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_slofenjpdng01')
						{
							// if the slofenjpdng01 has a value, use it
							if ($slofen_jpdng01_params->get($key) != '')
							{
								// get the value from the slofenjpdng01
								$slofen_jpdng01_array[$key] = $slofen_jpdng01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$slofen_jpdng01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected slofenjpdng01 params
					if (count($slofen_jpdng01_array) > 0)
					{
						$slofen_jpdng01_params = new Registry;
						$slofen_jpdng01_params->loadArray($slofen_jpdng01_array);
						$item->params->merge($slofen_jpdng01_params);
					}


					// get display date
					switch ($item->params->get('list_show_slofenjpdng01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an slofenjpdng01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
