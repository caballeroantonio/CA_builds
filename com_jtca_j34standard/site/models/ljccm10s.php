...
/**
 * This models supports retrieving lists of libro de registro para notarios.
 *
 */
class JtcaModelLjccm10s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljccm10s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de registro para notarios has been specified either as a parameter or in the url Request
		$pk = $params->get('ljccm10_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljccm10_id');
		$this->setState('filter.ljccm10_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljccm10_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljccm10_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljccm10_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljccm10_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljccm10_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljccm10_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljccm10_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljccm10_id'));
		$id .= ':'.$this->getState('filter.ljccm10_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljccm10s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljccm10s').' AS a');
		...
		// Filter by a single or group of ljccm10s.
		$ljccm10_id = $this->getState('filter.ljccm10_id');
		if ($ljccm10_id != '')
		{
			if (is_numeric($ljccm10_id))
			{
				$type = $this->getState('filter.ljccm10_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljccm10_id);
			}
			else
			{
				if (is_array($ljccm10_id))
				{
					JArrayHelper::toInteger($ljccm10_id);
					$ljccm10_id = implode(',', $ljccm10_id);
					$type = $this->getState('filter.ljccm10_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljccm10_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljccm10_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljccm10_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de registro para notarios.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljccm10_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljccm10 params override menu item params only if menu param = 'use_ljccm10'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljccm10' and there is no ljccm10 param, use global

					// create an array of just the params set to 'use_ljccm10'
					$menu_params_array = $this->getState('params')->toArray();
					$ljccm10_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljccm10')
						{
							// if the ljccm10 has a value, use it
							if ($ljccm10_params->get($key) != '')
							{
								// get the value from the ljccm10
								$ljccm10_array[$key] = $ljccm10_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljccm10_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljccm10 params
					if (count($ljccm10_array) > 0)
					{
						$ljccm10_params = new Registry;
						$ljccm10_params->loadArray($ljccm10_array);
						$item->params->merge($ljccm10_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljccm10_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljccm10, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
