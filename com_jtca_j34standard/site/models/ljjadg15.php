...
/**
 * TSJ CDMX Libros TxCA Component ARTICULO 36 O ARTICULO 28 LJADF Model
 *
 */
class JtCaModelLjjadg15 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljjadg15';
	...
	{
		if (empty($config['ljjadg15_filter_fields']))
		{
			$config['ljjadg15_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljjadg15s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get ARTICULO 36 O ARTICULO 28 LJADF data.
	 *
	 * @param	integer	$pk	The id of the articulo 36 o articulo 28 ljadf.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljjadg15.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJJADG15S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjjadg15ChangeState', array('com_jtca.ljjadg15', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjjadg15BeforeDelete', array('com_jtca.ljjadg15', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjjadg15AfterDelete', array('com_jtca.ljjadg15', &$table));
				}
				else
				{
					// Prune items that you can't change.
