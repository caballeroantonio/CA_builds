...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/slpersonalias.php';

class JtcaModelSlpersonaliasForm extends JtcaModelSlpersonalias
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.slpersonalias';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.slpersonalias';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.slpersonalias', 'slpersonalias', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the inculpado.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Inculpado item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('slpersonalias.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.slpersonalias.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('slpersonalias.id');
		$is_new		= true;
		...
		// Reorder the inculpado so the new inculpado is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSlpersonaliasBeforeSave', array('com_jtca.slpersonalias', &$table, $is_new));
		...
		$dispatcher->trigger('onSlpersonaliasAfterSave', array('com_jtca.slpersonalias', $table, $is_new));

		$this->setState('slpersonalias.id', $table->id);
