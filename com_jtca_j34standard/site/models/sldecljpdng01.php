...
/**
 * TSJ CDMX Libros TxCA Component Sub Libro DECLARACION PREPARATORIA Del LIBRO DE GOBIERNO Model
 *
 */
class JtcaModelSldeclJpdng01 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.sldecljpdng01';
	...
	{
		if (empty($config['sldecljpdng01_filter_fields']))
		{
			$config['sldecljpdng01_filter_fields'] = array(
				'id', 'a.id',
	...
	protected function populateState()
	{
		...
		$this->setState('sldecljpdng01.id', $pk);
	...
	*/
	public function getTable($type = 'SldeclJpdng01s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get Sub Libro DECLARACION PREPARATORIA Del LIBRO DE GOBIERNO data.
	 *
	 * @param	integer	$pk	The id of the sub libro declaracion preparatoria del libro de gobierno.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('sldecljpdng01.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_SLDECL_JPDNG01S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onSldeclJpdng01ChangeState', array('com_jtca.sldecljpdng01', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onSldeclJpdng01BeforeDelete', array('com_jtca.sldecljpdng01', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onSldeclJpdng01AfterDelete', array('com_jtca.sldecljpdng01', &$table));
				}
				else
				{
					// Prune items that you can't change.
