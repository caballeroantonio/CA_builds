...
/**
 * This models supports retrieving lists of libro de envío de expedientes al archivo judicial .
 *
 */
class JtCaModelLjomf19s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljomf19s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf19_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf19_id');
		$this->setState('filter.ljomf19_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf19_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf19_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf19_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf19_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf19_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf19_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf19_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf19_id'));
		$id .= ':'.$this->getState('filter.ljomf19_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf19s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf19s').' AS a');
		...
		// Filter by a single or group of ljomf19s.
		$ljomf19_id = $this->getState('filter.ljomf19_id');
		if ($ljomf19_id != '')
		{
			if (is_numeric($ljomf19_id))
			{
				$type = $this->getState('filter.ljomf19_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf19_id);
			}
			else
			{
				if (is_array($ljomf19_id))
				{
					JArrayHelper::toInteger($ljomf19_id);
					$ljomf19_id = implode(',', $ljomf19_id);
					$type = $this->getState('filter.ljomf19_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf19_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf19_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf19_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de envío de expedientes al archivo judicial .
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf19_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf19 params override menu item params only if menu param = 'use_ljomf19'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf19' and there is no ljomf19 param, use global

					// create an array of just the params set to 'use_ljomf19'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf19_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf19')
						{
							// if the ljomf19 has a value, use it
							if ($ljomf19_params->get($key) != '')
							{
								// get the value from the ljomf19
								$ljomf19_array[$key] = $ljomf19_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf19_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf19 params
					if (count($ljomf19_array) > 0)
					{
						$ljomf19_params = new Registry;
						$ljomf19_params->loadArray($ljomf19_array);
						$item->params->merge($ljomf19_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf19_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf19, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
