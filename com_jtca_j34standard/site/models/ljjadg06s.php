...
class JtCaModelLjjadg06s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadg06_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg06_id');
		$this->setState('filter.ljjadg06_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg06_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg06_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg06_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg06_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg06_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg06_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg06_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg06_id'));
		$id .= ':'.$this->getState('filter.ljjadg06_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg06s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg06s').' AS a');
		...
		// Filter by a single or group of ljjadg06s.
		$ljjadg06_id = $this->getState('filter.ljjadg06_id');
		if ($ljjadg06_id != '')
		{
			if (is_numeric($ljjadg06_id))
			{
				$type = $this->getState('filter.ljjadg06_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg06_id);
			}
			else
			{
				if (is_array($ljjadg06_id))
				{
					JArrayHelper::toInteger($ljjadg06_id);
					$ljjadg06_id = implode(',', $ljjadg06_id);
					$type = $this->getState('filter.ljjadg06_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg06_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg06_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg06_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg06_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg06 params override menu item params only if menu param = 'use_ljjadg06'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg06' and there is no ljjadg06 param, use global

					// create an array of just the params set to 'use_ljjadg06'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg06_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg06')
						{
							// if the ljjadg06 has a value, use it
							if ($ljjadg06_params->get($key) != '')
							{
								// get the value from the ljjadg06
								$ljjadg06_array[$key] = $ljjadg06_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg06_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg06 params
					if (count($ljjadg06_array) > 0)
					{
						$ljjadg06_params = new Registry;
						$ljjadg06_params->loadArray($ljjadg06_array);
						$item->params->merge($ljjadg06_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg06_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg06, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
