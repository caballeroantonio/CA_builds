...
/**
 * TSJ CDMX Libros TxCA Component Sub Libro CERTIFICADO DE EGRESOS Del LIBRO DE CERTIFICADO DE DEPÓSITOS DE INGRESOS Y EGRESOS Model
 *
 */
class JtcaModelSldepJoc03 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.sldepjoc03';
	...
	{
		if (empty($config['sldepjoc03_filter_fields']))
		{
			$config['sldepjoc03_filter_fields'] = array(
				'id', 'a.id',
				...
				'sfield8','a.sfield8',
				'sfield9','a.sfield9',
				'sfield10_isMoral','a.sfield10_isMoral',
				'sfield10_paterno','a.sfield10_paterno',
				'sfield10_materno','a.sfield10_materno',
				'sfield10_nombre','a.sfield10_nombre',
				'sfield11','a.sfield11',
				'sfield12','a.sfield12',
				'sfield13','a.sfield13',
				'sfield14','a.sfield14',
	...
	*/
	 ...
	 *
	 * @param	integer	$pk	The id of the sub libro certificado de egresos del libro de certificado de depósitos de ingresos y egresos.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('sldepjoc03.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_SLDEP_JOC03S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onSldepJoc03ChangeState', array('com_jtca.sldepjoc03', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onSldepJoc03BeforeDelete', array('com_jtca.sldepjoc03', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onSldepJoc03AfterDelete', array('com_jtca.sldepjoc03', &$table));
				}
				else
				{
					// Prune items that you can't change.
