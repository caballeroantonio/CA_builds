...
class JtCaModelLjpes03s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpes03_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes03_id');
		$this->setState('filter.ljpes03_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes03_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes03_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes03_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes03_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes03_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes03_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes03_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes03_id'));
		$id .= ':'.$this->getState('filter.ljpes03_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes03s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes03s').' AS a');
		...
		// Filter by a single or group of ljpes03s.
		$ljpes03_id = $this->getState('filter.ljpes03_id');
		if ($ljpes03_id != '')
		{
			if (is_numeric($ljpes03_id))
			{
				$type = $this->getState('filter.ljpes03_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes03_id);
			}
			else
			{
				if (is_array($ljpes03_id))
				{
					JArrayHelper::toInteger($ljpes03_id);
					$ljpes03_id = implode(',', $ljpes03_id);
					$type = $this->getState('filter.ljpes03_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes03_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes03_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes03_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes03_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes03 params override menu item params only if menu param = 'use_ljpes03'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes03' and there is no ljpes03 param, use global

					// create an array of just the params set to 'use_ljpes03'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes03_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes03')
						{
							// if the ljpes03 has a value, use it
							if ($ljpes03_params->get($key) != '')
							{
								// get the value from the ljpes03
								$ljpes03_array[$key] = $ljpes03_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes03_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes03 params
					if (count($ljpes03_array) > 0)
					{
						$ljpes03_params = new Registry;
						$ljpes03_params->loadArray($ljpes03_array);
						$item->params->merge($ljpes03_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes03_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes03, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
