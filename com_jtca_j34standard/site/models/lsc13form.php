...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsc13.php';

class JtcaModelLsc13Form extends JtcaModelLsc13
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsc13';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsc13';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsc13', 'lsc13', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the papeletas para el préstamo de expedientes.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	PAPELETAS PARA EL PRÉSTAMO DE EXPEDIENTES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsc13.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsc13.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsc13.id');
		$is_new		= true;
		...
		// Reorder the papeletas para el préstamo de expedientes so the new papeletas para el préstamo de expedientes is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsc13BeforeSave', array('com_jtca.lsc13', &$table, $is_new));
		...
		$dispatcher->trigger('onLsc13AfterSave', array('com_jtca.lsc13', $table, $is_new));

		$this->setState('lsc13.id', $table->id);
