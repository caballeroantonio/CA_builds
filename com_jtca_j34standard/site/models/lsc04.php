...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE MULTAS (OFICIAL) Model
 *
 */
class JtCaModelLsc04 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsc04';
	...
	{
		if (empty($config['lsc04_filter_fields']))
		{
			$config['lsc04_filter_fields'] = array(
				'id', 'a.id',
				...
				'field139','a.field139',
				'field140','a.field140',
				'field141_isMoral','a.field141_isMoral',
				'field141_paterno','a.field141_paterno',
				'field141_materno','a.field141_materno',
				'field141_nombre','a.field141_nombre',
				'field142','a.field142',
				'field250','a.field250',
				'field143','a.field143',
				'state', 'a.state',
	...
	*/
	public function getTable($type = 'Lsc04s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE MULTAS (OFICIAL) data.
	 *
	 * @param	integer	$pk	The id of the libro de multas (oficial).
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsc04.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSC04S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsc04ChangeState', array('com_jtca.lsc04', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsc04BeforeDelete', array('com_jtca.lsc04', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsc04AfterDelete', array('com_jtca.lsc04', &$table));
				}
				else
				{
					// Prune items that you can't change.
