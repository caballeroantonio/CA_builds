...
/**
 * TSJ CDMX Libros TxCA Component Sub Libro CONTROL DE FIRMAS Del LIBRO DE CONTROL DE FIRMAS PROCESADOS Model
 *
 */
class JtcaModelSlfirmJpdng17 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.slfirmjpdng17';
	...
	{
		if (empty($config['slfirmjpdng17_filter_fields']))
		{
			$config['slfirmjpdng17_filter_fields'] = array(
				'id', 'a.id',
	...
	protected function populateState()
	{
		...
		$this->setState('slfirmjpdng17.id', $pk);
	...
	*/
	public function getTable($type = 'SlfirmJpdng17s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get Sub Libro CONTROL DE FIRMAS Del LIBRO DE CONTROL DE FIRMAS PROCESADOS data.
	 *
	 * @param	integer	$pk	The id of the sub libro control de firmas del libro de control de firmas procesados.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('slfirmjpdng17.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_SLFIRM_JPDNG17S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onSlfirmJpdng17ChangeState', array('com_jtca.slfirmjpdng17', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onSlfirmJpdng17BeforeDelete', array('com_jtca.slfirmjpdng17', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onSlfirmJpdng17AfterDelete', array('com_jtca.slfirmjpdng17', &$table));
				}
				else
				{
					// Prune items that you can't change.
