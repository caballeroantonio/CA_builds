...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE ACTUARIO Model
 *
 */
class JtcaModelLsps04 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsps04';
	...
	{
		if (empty($config['lsps04_filter_fields']))
		{
			$config['lsps04_filter_fields'] = array(
				'id', 'a.id',
				'id_organo','a.id_organo',
				'id_secretaria','a.id_secretaria',
				'anoj','a.anoj',
				'id_expediente','a.id_expediente',
				'field2381','a.field2381',
				'field2382','a.field2382',
				'field2383_isMoral','a.field2383_isMoral',
				'field2383_paterno','a.field2383_paterno',
				'field2383_materno','a.field2383_materno',
				'field2383_nombre','a.field2383_nombre',
				'field2384','a.field2384',
				'field2385','a.field2385',
				'field2386','a.field2386',
				'field2387_isMoral','a.field2387_isMoral',
				'field2387_paterno','a.field2387_paterno',
				'field2387_materno','a.field2387_materno',
				'field2387_nombre','a.field2387_nombre',
				'field2388','a.field2388',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lsps04s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE ACTUARIO data.
	 *
	 * @param	integer	$pk	The id of the libro de actuario.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsps04.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPS04S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsps04ChangeState', array('com_jtca.lsps04', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsps04BeforeDelete', array('com_jtca.lsps04', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsps04AfterDelete', array('com_jtca.lsps04', &$table));
				}
				else
				{
					// Prune items that you can't change.
