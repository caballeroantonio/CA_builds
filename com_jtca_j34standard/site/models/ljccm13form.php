...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljccm13.php';

class JtCaModelLjccm13Form extends JtCaModelLjccm13
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljccm13';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljccm13';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljccm13', 'ljccm13', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljccm13.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljccm13.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljccm13.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjccm13BeforeSave', array('com_jtca.ljccm13', &$table, $is_new));
		...
		$dispatcher->trigger('onLjccm13AfterSave', array('com_jtca.ljccm13', $table, $is_new));

		$this->setState('ljccm13.id', $table->id);
