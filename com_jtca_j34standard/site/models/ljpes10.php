...
/**
 * TSJ CDMX Libros TxCA Component AGENDA DE AUDIENCIAS PARA JUICIOS ORALES Model
 *
 */
class JtCaModelLjpes10 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpes10';
	...
	{
		if (empty($config['ljpes10_filter_fields']))
		{
			$config['ljpes10_filter_fields'] = array(
				'id', 'a.id',
				'id_organo','a.id_organo',
				'anoj','a.anoj',
				'id_expediente','a.id_expediente',
				'id_secretaria','a.id_secretaria',
				'faudiencia','a.faudiencia',
				'txt_field1252','a.txt_field1252',
				'id_field1252','a.id_field1252',
				'field4','a.field4',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Ljpes10s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get AGENDA DE AUDIENCIAS PARA JUICIOS ORALES data.
	 *
	 * @param	integer	$pk	The id of the agenda de audiencias para juicios orales.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpes10.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPES10S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpes10ChangeState', array('com_jtca.ljpes10', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpes10BeforeDelete', array('com_jtca.ljpes10', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpes10AfterDelete', array('com_jtca.ljpes10', &$table));
				}
				else
				{
					// Prune items that you can't change.
