...
class JtCaModelLjemst11s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljemst11_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljemst11_id');
		$this->setState('filter.ljemst11_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljemst11_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljemst11_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljemst11_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljemst11_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljemst11_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljemst11_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljemst11_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljemst11_id'));
		$id .= ':'.$this->getState('filter.ljemst11_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljemst11s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljemst11s').' AS a');
		...
		// Filter by a single or group of ljemst11s.
		$ljemst11_id = $this->getState('filter.ljemst11_id');
		if ($ljemst11_id != '')
		{
			if (is_numeric($ljemst11_id))
			{
				$type = $this->getState('filter.ljemst11_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljemst11_id);
			}
			else
			{
				if (is_array($ljemst11_id))
				{
					JArrayHelper::toInteger($ljemst11_id);
					$ljemst11_id = implode(',', $ljemst11_id);
					$type = $this->getState('filter.ljemst11_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljemst11_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljemst11_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljemst11_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljemst11_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljemst11 params override menu item params only if menu param = 'use_ljemst11'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljemst11' and there is no ljemst11 param, use global

					// create an array of just the params set to 'use_ljemst11'
					$menu_params_array = $this->getState('params')->toArray();
					$ljemst11_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljemst11')
						{
							// if the ljemst11 has a value, use it
							if ($ljemst11_params->get($key) != '')
							{
								// get the value from the ljemst11
								$ljemst11_array[$key] = $ljemst11_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljemst11_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljemst11 params
					if (count($ljemst11_array) > 0)
					{
						$ljemst11_params = new Registry;
						$ljemst11_params->loadArray($ljemst11_array);
						$item->params->merge($ljemst11_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljemst11_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljemst11, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
