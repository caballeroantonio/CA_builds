...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE INGRESOS Y EGRESOS DE BILLETES DE DEPÓSITO Model
 *
 */
class JtCaModelLjpes11 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpes11';
	...
	{
		if (empty($config['ljpes11_filter_fields']))
		{
	...
	*/
	public function getTable($type = 'Ljpes11s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE INGRESOS Y EGRESOS DE BILLETES DE DEPÓSITO data.
	 *
	 * @param	integer	$pk	The id of the libro de ingresos y egresos de billetes de depósito.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpes11.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPES11S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpes11ChangeState', array('com_jtca.ljpes11', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpes11BeforeDelete', array('com_jtca.ljpes11', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpes11AfterDelete', array('com_jtca.ljpes11', &$table));
				}
				else
				{
					// Prune items that you can't change.
