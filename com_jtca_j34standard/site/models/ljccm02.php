...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE INGRESO DE VALORES Model
 *
 */
class JtCaModelLjccm02 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljccm02';
	...
	{
		if (empty($config['ljccm02_filter_fields']))
		{
			$config['ljccm02_filter_fields'] = array(
				'id', 'a.id',
				'billete', 'a.billete',
	...
	*/
	public function getTable($type = 'Ljccm02s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE INGRESO DE VALORES data.
	 *
	 * @param	integer	$pk	The id of the libro de ingreso de valores.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljccm02.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJCCM02S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjccm02ChangeState', array('com_jtca.ljccm02', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjccm02BeforeDelete', array('com_jtca.ljccm02', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjccm02AfterDelete', array('com_jtca.ljccm02', &$table));
				}
				else
				{
					// Prune items that you can't change.
