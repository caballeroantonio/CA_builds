...
/**
 * This models supports retrieving lists of libro de control de asuntos conforme a los artícul.
 *
 */
class JtCaModelLjomf20s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljomf20s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf20_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf20_id');
		$this->setState('filter.ljomf20_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf20_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf20_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf20_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf20_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf20_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf20_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf20_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf20_id'));
		$id .= ':'.$this->getState('filter.ljomf20_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf20s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf20s').' AS a');
		...
		// Filter by a single or group of ljomf20s.
		$ljomf20_id = $this->getState('filter.ljomf20_id');
		if ($ljomf20_id != '')
		{
			if (is_numeric($ljomf20_id))
			{
				$type = $this->getState('filter.ljomf20_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf20_id);
			}
			else
			{
				if (is_array($ljomf20_id))
				{
					JArrayHelper::toInteger($ljomf20_id);
					$ljomf20_id = implode(',', $ljomf20_id);
					$type = $this->getState('filter.ljomf20_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf20_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf20_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf20_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de control de asuntos conforme a los artícul.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf20_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf20 params override menu item params only if menu param = 'use_ljomf20'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf20' and there is no ljomf20 param, use global

					// create an array of just the params set to 'use_ljomf20'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf20_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf20')
						{
							// if the ljomf20 has a value, use it
							if ($ljomf20_params->get($key) != '')
							{
								// get the value from the ljomf20
								$ljomf20_array[$key] = $ljomf20_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf20_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf20 params
					if (count($ljomf20_array) > 0)
					{
						$ljomf20_params = new Registry;
						$ljomf20_params->loadArray($ljomf20_array);
						$item->params->merge($ljomf20_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf20_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf20, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
