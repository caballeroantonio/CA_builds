...
class JtcaModelLjjadg05s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadg05_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg05_id');
		$this->setState('filter.ljjadg05_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg05_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg05_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg05_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg05_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg05_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg05_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg05_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg05_id'));
		$id .= ':'.$this->getState('filter.ljjadg05_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg05s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg05s').' AS a');
		...
		// Filter by a single or group of ljjadg05s.
		$ljjadg05_id = $this->getState('filter.ljjadg05_id');
		if ($ljjadg05_id != '')
		{
			if (is_numeric($ljjadg05_id))
			{
				$type = $this->getState('filter.ljjadg05_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg05_id);
			}
			else
			{
				if (is_array($ljjadg05_id))
				{
					JArrayHelper::toInteger($ljjadg05_id);
					$ljjadg05_id = implode(',', $ljjadg05_id);
					$type = $this->getState('filter.ljjadg05_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg05_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg05_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg05_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg05_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg05 params override menu item params only if menu param = 'use_ljjadg05'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg05' and there is no ljjadg05 param, use global

					// create an array of just the params set to 'use_ljjadg05'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg05_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg05')
						{
							// if the ljjadg05 has a value, use it
							if ($ljjadg05_params->get($key) != '')
							{
								// get the value from the ljjadg05
								$ljjadg05_array[$key] = $ljjadg05_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg05_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg05 params
					if (count($ljjadg05_array) > 0)
					{
						$ljjadg05_params = new Registry;
						$ljjadg05_params->loadArray($ljjadg05_array);
						$item->params->merge($ljjadg05_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg05_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg05, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
