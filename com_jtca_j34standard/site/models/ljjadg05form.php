...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadg05.php';

class JtCaModelLjjadg05Form extends JtCaModelLjjadg05
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadg05';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadg05';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadg05', 'ljjadg05', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadg05.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadg05.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadg05.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjjadg05BeforeSave', array('com_jtca.ljjadg05', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadg05AfterSave', array('com_jtca.ljjadg05', $table, $is_new));

		$this->setState('ljjadg05.id', $table->id);
