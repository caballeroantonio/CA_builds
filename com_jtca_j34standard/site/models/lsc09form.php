...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsc09.php';

class JtcaModelLsc09Form extends JtcaModelLsc09
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsc09';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsc09';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsc09', 'lsc09', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de gobierno (oficial).
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE GOBIERNO (OFICIAL) item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsc09.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsc09.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsc09.id');
		$is_new		= true;
		...
		// Reorder the libro de gobierno (oficial) so the new libro de gobierno (oficial) is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsc09BeforeSave', array('com_jtca.lsc09', &$table, $is_new));
		...
		$dispatcher->trigger('onLsc09AfterSave', array('com_jtca.lsc09', $table, $is_new));

		$this->setState('lsc09.id', $table->id);
