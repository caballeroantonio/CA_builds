...
class JtCaModelLjf12s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf12_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf12_id');
		$this->setState('filter.ljf12_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf12_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf12_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf12_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf12_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf12_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf12_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf12_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf12_id'));
		$id .= ':'.$this->getState('filter.ljf12_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf12s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf12s').' AS a');
		...
		// Filter by a single or group of ljf12s.
		$ljf12_id = $this->getState('filter.ljf12_id');
		if ($ljf12_id != '')
		{
			if (is_numeric($ljf12_id))
			{
				$type = $this->getState('filter.ljf12_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf12_id);
			}
			else
			{
				if (is_array($ljf12_id))
				{
					JArrayHelper::toInteger($ljf12_id);
					$ljf12_id = implode(',', $ljf12_id);
					$type = $this->getState('filter.ljf12_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf12_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf12_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf12_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf12_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf12 params override menu item params only if menu param = 'use_ljf12'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf12' and there is no ljf12 param, use global

					// create an array of just the params set to 'use_ljf12'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf12')
						{
							// if the ljf12 has a value, use it
							if ($ljf12_params->get($key) != '')
							{
								// get the value from the ljf12
								$ljf12_array[$key] = $ljf12_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf12_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf12 params
					if (count($ljf12_array) > 0)
					{
						$ljf12_params = new Registry;
						$ljf12_params->loadArray($ljf12_array);
						$item->params->merge($ljf12_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf12_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf12, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
