...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadg16.php';

class JtCaModelLjjadg16Form extends JtCaModelLjjadg16
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadg16';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadg16';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadg16', 'ljjadg16', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de control de presentación de adolescentes en libertad ante el juez.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE CONTROL DE PRESENTACIÓN DE ADOLESCENTES EN LIBERTAD ANTE EL JUEZ item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadg16.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadg16.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadg16.id');
		$is_new		= true;
		...
		// Reorder the libro de control de presentación de adolescentes e so the new libro de control de presentación de adolescentes en libertad ante el juez is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadg16BeforeSave', array('com_jtca.ljjadg16', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadg16AfterSave', array('com_jtca.ljjadg16', $table, $is_new));

		$this->setState('ljjadg16.id', $table->id);
