...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE EXHORTOS Model
 *
 */
class JtCaModelLjjadng06 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljjadng06';
	...
	{
		if (empty($config['ljjadng06_filter_fields']))
		{
			$config['ljjadng06_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljjadng06s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE EXHORTOS data.
	 *
	 * @param	integer	$pk	The id of the libro de exhortos.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljjadng06.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJJADNG06S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjjadng06ChangeState', array('com_jtca.ljjadng06', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjjadng06BeforeDelete', array('com_jtca.ljjadng06', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjjadng06AfterDelete', array('com_jtca.ljjadng06', &$table));
				}
				else
				{
					// Prune items that you can't change.
