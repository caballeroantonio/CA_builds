...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsps11.php';

class JtcaModelLsps11Form extends JtcaModelLsps11
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsps11';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsps11';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsps11', 'lsps11', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de control de fianza.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE CONTROL DE FIANZA item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsps11.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsps11.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsps11.id');
		$is_new		= true;
		...
		// Reorder the libro de control de fianza so the new libro de control de fianza is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsps11BeforeSave', array('com_jtca.lsps11', &$table, $is_new));
		...
		$dispatcher->trigger('onLsps11AfterSave', array('com_jtca.lsps11', $table, $is_new));

		$this->setState('lsps11.id', $table->id);
