...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljccm10.php';

class JtcaModelLjccm10Form extends JtcaModelLjccm10
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljccm10';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljccm10';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljccm10', 'ljccm10', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de registro para notarios.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE REGISTRO PARA NOTARIOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljccm10.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljccm10.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljccm10.id');
		$is_new		= true;
		...
		// Reorder the libro de registro para notarios so the new libro de registro para notarios is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjccm10BeforeSave', array('com_jtca.ljccm10', &$table, $is_new));
		...
		$dispatcher->trigger('onLjccm10AfterSave', array('com_jtca.ljccm10', $table, $is_new));

		$this->setState('ljccm10.id', $table->id);
