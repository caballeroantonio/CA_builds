...
/**
 * This models supports retrieving lists of diligencia del libro de actuarios.
 *
 */
class JtcaModelSldiliJpdng09s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.sldili_jpdng09s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single sub libro diligencia del libro de actuarios has been specified either as a parameter or in the url Request
		$pk = $params->get('sldili_jpdng09_id', '') == '' ? $app->input->getInt('id', '') : $params->get('sldili_jpdng09_id');
		$this->setState('filter.sldili_jpdng09_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('sldilijpdng09_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('sldilijpdng09_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('sldilijpdng09_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('sldilijpdng09_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('sldilijpdng09_initial_direction','ASC');
		}
		...
		if ($params->get('filter_sldilijpdng09_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_sldilijpdng09_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.sldili_jpdng09_id'));
		$id .= ':'.$this->getState('filter.sldili_jpdng09_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of sldili_jpdng09s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_sldili_jpdng09s').' AS a');
		...
		// Filter by a single or group of sldili_jpdng09s.
		$sldili_jpdng09_id = $this->getState('filter.sldili_jpdng09_id');
		if ($sldili_jpdng09_id != '')
		{
			if (is_numeric($sldili_jpdng09_id))
			{
				$type = $this->getState('filter.sldili_jpdng09_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $sldili_jpdng09_id);
			}
			else
			{
				if (is_array($sldili_jpdng09_id))
				{
					JArrayHelper::toInteger($sldili_jpdng09_id);
					$sldili_jpdng09_id = implode(',', $sldili_jpdng09_id);
					$type = $this->getState('filter.sldili_jpdng09_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$sldili_jpdng09_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_sldilijpdng09_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_sldilijpdng09_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of diligencia del libro de actuarios.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$sldili_jpdng09_params = new Registry;
				...
				{
					$item->params = $sldili_jpdng09_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// SldiliJpdng09 params override menu item params only if menu param = 'use_sldilijpdng09'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_sldilijpdng09' and there is no sldilijpdng09 param, use global

					// create an array of just the params set to 'use_sldilijpdng09'
					$menu_params_array = $this->getState('params')->toArray();
					$sldili_jpdng09_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_sldilijpdng09')
						{
							// if the sldilijpdng09 has a value, use it
							if ($sldili_jpdng09_params->get($key) != '')
							{
								// get the value from the sldilijpdng09
								$sldili_jpdng09_array[$key] = $sldili_jpdng09_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$sldili_jpdng09_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected sldilijpdng09 params
					if (count($sldili_jpdng09_array) > 0)
					{
						$sldili_jpdng09_params = new Registry;
						$sldili_jpdng09_params->loadArray($sldili_jpdng09_array);
						$item->params->merge($sldili_jpdng09_params);
					}


					// get display date
					switch ($item->params->get('list_show_sldilijpdng09_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an sldilijpdng09, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
