...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE TURNO DE EXCUSAS PARA JUZGADOS  DE DELITOS NO GRAVES Model
 *
 */
class JtCaModelLcp28 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp28';
	...
	{
		if (empty($config['lcp28_filter_fields']))
		{
			$config['lcp28_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	...
	/**
	 * Method to get LIBRO DE TURNO DE EXCUSAS PARA JUZGADOS  DE DELITOS NO GRAVES data.
	 *
	 * @param	integer	$pk	The id of the libro de turno de excusas para juzgados  de delitos no graves.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp28.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP28S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp28ChangeState', array('com_jtca.lcp28', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp28BeforeDelete', array('com_jtca.lcp28', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp28AfterDelete', array('com_jtca.lcp28', &$table));
				}
				else
				{
					// Prune items that you can't change.
