...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp21.php';

class JtcaModelLcp21Form extends JtcaModelLcp21
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp21';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp21';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp21', 'lcp21', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno de excusas para juzgados de primera instancia..
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO DE EXCUSAS PARA JUZGADOS DE PRIMERA INSTANCIA. item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp21.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp21.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp21.id');
		$is_new		= true;
		...
		// Reorder the libro de turno de excusas para juzgados de primera so the new libro de turno de excusas para juzgados de primera instancia. is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp21BeforeSave', array('com_jtca.lcp21', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp21AfterSave', array('com_jtca.lcp21', $table, $is_new));

		$this->setState('lcp21.id', $table->id);
