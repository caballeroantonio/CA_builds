...
class JtCaModelLjemst04s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljemst04_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljemst04_id');
		$this->setState('filter.ljemst04_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljemst04_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljemst04_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljemst04_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljemst04_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljemst04_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljemst04_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljemst04_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljemst04_id'));
		$id .= ':'.$this->getState('filter.ljemst04_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljemst04s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljemst04s').' AS a');
		...
		// Filter by a single or group of ljemst04s.
		$ljemst04_id = $this->getState('filter.ljemst04_id');
		if ($ljemst04_id != '')
		{
			if (is_numeric($ljemst04_id))
			{
				$type = $this->getState('filter.ljemst04_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljemst04_id);
			}
			else
			{
				if (is_array($ljemst04_id))
				{
					JArrayHelper::toInteger($ljemst04_id);
					$ljemst04_id = implode(',', $ljemst04_id);
					$type = $this->getState('filter.ljemst04_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljemst04_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljemst04_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljemst04_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljemst04_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljemst04 params override menu item params only if menu param = 'use_ljemst04'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljemst04' and there is no ljemst04 param, use global

					// create an array of just the params set to 'use_ljemst04'
					$menu_params_array = $this->getState('params')->toArray();
					$ljemst04_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljemst04')
						{
							// if the ljemst04 has a value, use it
							if ($ljemst04_params->get($key) != '')
							{
								// get the value from the ljemst04
								$ljemst04_array[$key] = $ljemst04_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljemst04_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljemst04 params
					if (count($ljemst04_array) > 0)
					{
						$ljemst04_params = new Registry;
						$ljemst04_params->loadArray($ljemst04_array);
						$item->params->merge($ljemst04_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljemst04_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljemst04, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
