...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljc01.php';

class JtCaModelLjc01Form extends JtCaModelLjc01
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljc01';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljc01';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljc01', 'ljc01', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljc01.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljc01.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljc01.id');
		$is_new		= true;
		...
		// Reorder the jc01 so the new libro de gobierno is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjc01BeforeSave', array('com_jtca.ljc01', &$table, $is_new));
		...
		$dispatcher->trigger('onLjc01AfterSave', array('com_jtca.ljc01', $table, $is_new));

		$this->setState('ljc01.id', $table->id);
