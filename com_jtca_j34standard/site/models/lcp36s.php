...
/**
 * This models supports retrieving lists of libreta de returno de adolescentes.
 *
 */
class JtCaModelLcp36s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp36s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de returno de adolescentes has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp36_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp36_id');
		$this->setState('filter.lcp36_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp36_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp36_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp36_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp36_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp36_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp36_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp36_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp36_id'));
		$id .= ':'.$this->getState('filter.lcp36_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp36s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp36s').' AS a');
		...
		// Filter by a single or group of lcp36s.
		$lcp36_id = $this->getState('filter.lcp36_id');
		if ($lcp36_id != '')
		{
			if (is_numeric($lcp36_id))
			{
				$type = $this->getState('filter.lcp36_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp36_id);
			}
			else
			{
				if (is_array($lcp36_id))
				{
					JArrayHelper::toInteger($lcp36_id);
					$lcp36_id = implode(',', $lcp36_id);
					$type = $this->getState('filter.lcp36_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp36_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp36_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp36_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de returno de adolescentes.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp36_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp36 params override menu item params only if menu param = 'use_lcp36'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp36' and there is no lcp36 param, use global

					// create an array of just the params set to 'use_lcp36'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp36')
						{
							// if the lcp36 has a value, use it
							if ($lcp36_params->get($key) != '')
							{
								// get the value from the lcp36
								$lcp36_array[$key] = $lcp36_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp36_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp36 params
					if (count($lcp36_array) > 0)
					{
						$lcp36_params = new Registry;
						$lcp36_params->loadArray($lcp36_array);
						$item->params->merge($lcp36_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp36_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp36, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
