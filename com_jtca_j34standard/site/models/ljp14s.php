...
class JtCaModelLjp14s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljp14_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp14_id');
		$this->setState('filter.ljp14_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp14_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp14_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp14_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp14_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp14_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp14_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp14_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp14_id'));
		$id .= ':'.$this->getState('filter.ljp14_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp14s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp14s').' AS a');
		...
		// Filter by a single or group of ljp14s.
		$ljp14_id = $this->getState('filter.ljp14_id');
		if ($ljp14_id != '')
		{
			if (is_numeric($ljp14_id))
			{
				$type = $this->getState('filter.ljp14_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp14_id);
			}
			else
			{
				if (is_array($ljp14_id))
				{
					JArrayHelper::toInteger($ljp14_id);
					$ljp14_id = implode(',', $ljp14_id);
					$type = $this->getState('filter.ljp14_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp14_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp14_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp14_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp14_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp14 params override menu item params only if menu param = 'use_ljp14'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp14' and there is no ljp14 param, use global

					// create an array of just the params set to 'use_ljp14'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp14')
						{
							// if the ljp14 has a value, use it
							if ($ljp14_params->get($key) != '')
							{
								// get the value from the ljp14
								$ljp14_array[$key] = $ljp14_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp14_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp14 params
					if (count($ljp14_array) > 0)
					{
						$ljp14_params = new Registry;
						$ljp14_params->loadArray($ljp14_array);
						$item->params->merge($ljp14_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp14_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp14, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
