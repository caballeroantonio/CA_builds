 ...
 *
 */
class JtcaModelLjoc01 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljoc01';
	...
	{
		if (empty($config['ljoc01_filter_fields']))
		{
			$config['ljoc01_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljoc01s', $prefix = 'JtcaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljoc01.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJOC01S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjoc01ChangeState', array('com_jtca.ljoc01', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjoc01BeforeDelete', array('com_jtca.ljoc01', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjoc01AfterDelete', array('com_jtca.ljoc01', &$table));
				}
				else
				{
					// Prune items that you can't change.
