...
/**
 * This models supports retrieving lists of libro de ingresos de valores.
 *
 */
class JtCaModelLjomf02s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljomf02s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljomf02_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljomf02_id');
		$this->setState('filter.ljomf02_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljomf02_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljomf02_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljomf02_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljomf02_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljomf02_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljomf02_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljomf02_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljomf02_id'));
		$id .= ':'.$this->getState('filter.ljomf02_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljomf02s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljomf02s').' AS a');
		...
		// Filter by a single or group of ljomf02s.
		$ljomf02_id = $this->getState('filter.ljomf02_id');
		if ($ljomf02_id != '')
		{
			if (is_numeric($ljomf02_id))
			{
				$type = $this->getState('filter.ljomf02_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljomf02_id);
			}
			else
			{
				if (is_array($ljomf02_id))
				{
					JArrayHelper::toInteger($ljomf02_id);
					$ljomf02_id = implode(',', $ljomf02_id);
					$type = $this->getState('filter.ljomf02_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljomf02_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljomf02_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljomf02_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de ingresos de valores.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljomf02_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljomf02 params override menu item params only if menu param = 'use_ljomf02'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljomf02' and there is no ljomf02 param, use global

					// create an array of just the params set to 'use_ljomf02'
					$menu_params_array = $this->getState('params')->toArray();
					$ljomf02_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljomf02')
						{
							// if the ljomf02 has a value, use it
							if ($ljomf02_params->get($key) != '')
							{
								// get the value from the ljomf02
								$ljomf02_array[$key] = $ljomf02_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljomf02_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljomf02 params
					if (count($ljomf02_array) > 0)
					{
						$ljomf02_params = new Registry;
						$ljomf02_params->loadArray($ljomf02_array);
						$item->params->merge($ljomf02_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljomf02_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljomf02, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
