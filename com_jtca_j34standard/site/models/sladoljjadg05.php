...
/**
 * TSJ CDMX Libros TxCA Component Sub Libro ADOLESCENTES Del LIBRO DE SENTENCIAS Model
 *
 */
class JtcaModelSladolJjadg05 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.sladoljjadg05';
	...
	{
		if (empty($config['sladoljjadg05_filter_fields']))
		{
			$config['sladoljjadg05_filter_fields'] = array(
				'id', 'a.id',
				'id_record', 'a.id_record',
				'id_field', 'a.id_field',
				'id_record','a.id_record',
				'sfield1_isMoral','a.sfield1_isMoral',
				'sfield1_paterno','a.sfield1_paterno',
				'sfield1_materno','a.sfield1_materno',
				'sfield1_nombre','a.sfield1_nombre',
				'sfield8','a.sfield8',
				'id_field','a.id_field',
				'sfield2','a.sfield2',
				'sfield3','a.sfield3',
	...
	protected function populateState()
	{
		...
		$this->setState('sladoljjadg05.id', $pk);
	...
	*/
	public function getTable($type = 'SladolJjadg05s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get Sub Libro ADOLESCENTES Del LIBRO DE SENTENCIAS data.
	 *
	 * @param	integer	$pk	The id of the sub libro adolescentes del libro de sentencias.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('sladoljjadg05.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_SLADOL_JJADG05S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onSladolJjadg05ChangeState', array('com_jtca.sladoljjadg05', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onSladolJjadg05BeforeDelete', array('com_jtca.sladoljjadg05', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onSladolJjadg05AfterDelete', array('com_jtca.sladoljjadg05', &$table));
				}
				else
				{
					// Prune items that you can't change.
