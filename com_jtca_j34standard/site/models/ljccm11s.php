...
class JtCaModelLjccm11s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljccm11_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljccm11_id');
		$this->setState('filter.ljccm11_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljccm11_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljccm11_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljccm11_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljccm11_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljccm11_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljccm11_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljccm11_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljccm11_id'));
		$id .= ':'.$this->getState('filter.ljccm11_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljccm11s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljccm11s').' AS a');
		...
		// Filter by a single or group of ljccm11s.
		$ljccm11_id = $this->getState('filter.ljccm11_id');
		if ($ljccm11_id != '')
		{
			if (is_numeric($ljccm11_id))
			{
				$type = $this->getState('filter.ljccm11_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljccm11_id);
			}
			else
			{
				if (is_array($ljccm11_id))
				{
					JArrayHelper::toInteger($ljccm11_id);
					$ljccm11_id = implode(',', $ljccm11_id);
					$type = $this->getState('filter.ljccm11_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljccm11_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljccm11_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljccm11_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljccm11_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljccm11 params override menu item params only if menu param = 'use_ljccm11'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljccm11' and there is no ljccm11 param, use global

					// create an array of just the params set to 'use_ljccm11'
					$menu_params_array = $this->getState('params')->toArray();
					$ljccm11_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljccm11')
						{
							// if the ljccm11 has a value, use it
							if ($ljccm11_params->get($key) != '')
							{
								// get the value from the ljccm11
								$ljccm11_array[$key] = $ljccm11_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljccm11_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljccm11 params
					if (count($ljccm11_array) > 0)
					{
						$ljccm11_params = new Registry;
						$ljccm11_params->loadArray($ljccm11_array);
						$item->params->merge($ljccm11_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljccm11_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljccm11, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
