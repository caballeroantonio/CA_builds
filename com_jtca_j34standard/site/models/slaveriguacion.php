...
/**
 * TSJ CDMX Libros TxCA Component Averiguaciones Previas Model
 *
 */
class JtcaModelSlaveriguacion extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.slaveriguacion';
	...
	{
		if (empty($config['slaveriguacion_filter_fields']))
		{
			$config['slaveriguacion_filter_fields'] = array(
				'id', 'a.id',
	...
	protected function populateState()
	{
		...
		$this->setState('slaveriguacion.id', $pk);
	...
	*/
	public function getTable($type = 'Slaveriguaciones', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get Averiguaciones Previas data.
	 *
	 * @param	integer	$pk	The id of the averiguaciones previas.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('slaveriguacion.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_SLAVERIGUACIONES_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onSlaveriguacionChangeState', array('com_jtca.slaveriguacion', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onSlaveriguacionBeforeDelete', array('com_jtca.slaveriguacion', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onSlaveriguacionAfterDelete', array('com_jtca.slaveriguacion', &$table));
				}
				else
				{
					// Prune items that you can't change.
