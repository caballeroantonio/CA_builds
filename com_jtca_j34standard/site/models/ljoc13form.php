...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljoc13.php';

class JtcaModelLjoc13Form extends JtcaModelLjoc13
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljoc13';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljoc13';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljoc13', 'ljoc13', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de notarios.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE NOTARIOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljoc13.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljoc13.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljoc13.id');
		$is_new		= true;
		...
		// Reorder the libro de notarios so the new libro de notarios is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjoc13BeforeSave', array('com_jtca.ljoc13', &$table, $is_new));
		...
		$dispatcher->trigger('onLjoc13AfterSave', array('com_jtca.ljoc13', $table, $is_new));

		$this->setState('ljoc13.id', $table->id);
