...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljc03.php';

class JtCaModelLjc03Form extends JtCaModelLjc03
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljc03';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljc03';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljc03', 'ljc03', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de egresos de valores.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE EGRESOS DE VALORES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljc03.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljc03.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljc03.id');
		$is_new		= true;
		...
		// Reorder the jc03 so the new libro de egresos de valores is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjc03BeforeSave', array('com_jtca.ljc03', &$table, $is_new));
		...
		$dispatcher->trigger('onLjc03AfterSave', array('com_jtca.ljc03', $table, $is_new));

		$this->setState('ljc03.id', $table->id);
