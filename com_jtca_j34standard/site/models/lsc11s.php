...
/**
 * This models supports retrieving lists of libreta de oficios (oficial).
 *
 */
class JtCaModelLsc11s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsc11s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de oficios (oficial) has been specified either as a parameter or in the url Request
		$pk = $params->get('lsc11_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsc11_id');
		$this->setState('filter.lsc11_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsc11_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsc11_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsc11_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsc11_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsc11_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsc11_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsc11_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsc11_id'));
		$id .= ':'.$this->getState('filter.lsc11_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsc11s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsc11s').' AS a');
		...
		// Filter by a single or group of lsc11s.
		$lsc11_id = $this->getState('filter.lsc11_id');
		if ($lsc11_id != '')
		{
			if (is_numeric($lsc11_id))
			{
				$type = $this->getState('filter.lsc11_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsc11_id);
			}
			else
			{
				if (is_array($lsc11_id))
				{
					JArrayHelper::toInteger($lsc11_id);
					$lsc11_id = implode(',', $lsc11_id);
					$type = $this->getState('filter.lsc11_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsc11_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsc11_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsc11_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de oficios (oficial).
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsc11_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsc11 params override menu item params only if menu param = 'use_lsc11'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsc11' and there is no lsc11 param, use global

					// create an array of just the params set to 'use_lsc11'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsc11')
						{
							// if the lsc11 has a value, use it
							if ($lsc11_params->get($key) != '')
							{
								// get the value from the lsc11
								$lsc11_array[$key] = $lsc11_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsc11_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsc11 params
					if (count($lsc11_array) > 0)
					{
						$lsc11_params = new Registry;
						$lsc11_params->loadArray($lsc11_array);
						$item->params->merge($lsc11_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsc11_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsc11, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
