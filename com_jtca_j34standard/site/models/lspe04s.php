...
/**
 * This models supports retrieving lists of libro de beneficios revocado y o/a.
 *
 */
class JtCaModelLspe04s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lspe04s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de beneficios revocado y o/a has been specified either as a parameter or in the url Request
		$pk = $params->get('lspe04_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lspe04_id');
		$this->setState('filter.lspe04_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lspe04_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lspe04_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lspe04_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lspe04_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lspe04_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lspe04_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lspe04_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lspe04_id'));
		$id .= ':'.$this->getState('filter.lspe04_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lspe04s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lspe04s').' AS a');
		...
		// Filter by a single or group of lspe04s.
		$lspe04_id = $this->getState('filter.lspe04_id');
		if ($lspe04_id != '')
		{
			if (is_numeric($lspe04_id))
			{
				$type = $this->getState('filter.lspe04_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lspe04_id);
			}
			else
			{
				if (is_array($lspe04_id))
				{
					JArrayHelper::toInteger($lspe04_id);
					$lspe04_id = implode(',', $lspe04_id);
					$type = $this->getState('filter.lspe04_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lspe04_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lspe04_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lspe04_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de beneficios revocado y o/a.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lspe04_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lspe04 params override menu item params only if menu param = 'use_lspe04'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lspe04' and there is no lspe04 param, use global

					// create an array of just the params set to 'use_lspe04'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lspe04')
						{
							// if the lspe04 has a value, use it
							if ($lspe04_params->get($key) != '')
							{
								// get the value from the lspe04
								$lspe04_array[$key] = $lspe04_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lspe04_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lspe04 params
					if (count($lspe04_array) > 0)
					{
						$lspe04_params = new Registry;
						$lspe04_params->loadArray($lspe04_array);
						$item->params->merge($lspe04_params);
					}


					// get display date
					switch ($item->params->get('list_show_lspe04_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lspe04, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
