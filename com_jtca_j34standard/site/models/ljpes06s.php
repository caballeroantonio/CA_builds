...
class JtCaModelLjpes06s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpes06_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes06_id');
		$this->setState('filter.ljpes06_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes06_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes06_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes06_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes06_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes06_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes06_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes06_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes06_id'));
		$id .= ':'.$this->getState('filter.ljpes06_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes06s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes06s').' AS a');
		...
		// Filter by a single or group of ljpes06s.
		$ljpes06_id = $this->getState('filter.ljpes06_id');
		if ($ljpes06_id != '')
		{
			if (is_numeric($ljpes06_id))
			{
				$type = $this->getState('filter.ljpes06_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes06_id);
			}
			else
			{
				if (is_array($ljpes06_id))
				{
					JArrayHelper::toInteger($ljpes06_id);
					$ljpes06_id = implode(',', $ljpes06_id);
					$type = $this->getState('filter.ljpes06_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes06_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes06_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes06_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes06_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes06 params override menu item params only if menu param = 'use_ljpes06'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes06' and there is no ljpes06 param, use global

					// create an array of just the params set to 'use_ljpes06'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes06_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes06')
						{
							// if the ljpes06 has a value, use it
							if ($ljpes06_params->get($key) != '')
							{
								// get the value from the ljpes06
								$ljpes06_array[$key] = $ljpes06_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes06_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes06 params
					if (count($ljpes06_array) > 0)
					{
						$ljpes06_params = new Registry;
						$ljpes06_params->loadArray($ljpes06_array);
						$item->params->merge($ljpes06_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes06_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes06, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
