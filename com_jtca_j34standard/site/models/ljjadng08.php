...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE AMPAROS Model
 *
 */
class JtCaModelLjjadng08 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljjadng08';
	...
	{
		if (empty($config['ljjadng08_filter_fields']))
		{
			$config['ljjadng08_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljjadng08s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE AMPAROS data.
	 *
	 * @param	integer	$pk	The id of the libro de amparos.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljjadng08.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJJADNG08S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjjadng08ChangeState', array('com_jtca.ljjadng08', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjjadng08BeforeDelete', array('com_jtca.ljjadng08', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjjadng08AfterDelete', array('com_jtca.ljjadng08', &$table));
				}
				else
				{
					// Prune items that you can't change.
