 ...
 *
 */
class JtCaModelLsps06 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsps06';
	...
	{
		if (empty($config['lsps06_filter_fields']))
		{
			$config['lsps06_filter_fields'] = array(
				'id', 'a.id',
				'id_expediente','a.id_expediente',
				'id_organo','a.id_organo',
				'id_secretaria','a.id_secretaria',
				'anoj','a.anoj',
				'field2399','a.field2399',
				'field2400','a.field2400',
				'field2401','a.field2401',
				'field2402','a.field2402',
				'field2403','a.field2403',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lsps06s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsps06.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPS06S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsps06ChangeState', array('com_jtca.lsps06', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsps06BeforeDelete', array('com_jtca.lsps06', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsps06AfterDelete', array('com_jtca.lsps06', &$table));
				}
				else
				{
					// Prune items that you can't change.
