...
class JtCaModelLjp01s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljp01_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp01_id');
		$this->setState('filter.ljp01_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp01_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp01_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp01_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp01_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp01_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp01_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp01_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp01_id'));
		$id .= ':'.$this->getState('filter.ljp01_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp01s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp01s').' AS a');
		...
		// Filter by a single or group of ljp01s.
		$ljp01_id = $this->getState('filter.ljp01_id');
		if ($ljp01_id != '')
		{
			if (is_numeric($ljp01_id))
			{
				$type = $this->getState('filter.ljp01_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp01_id);
			}
			else
			{
				if (is_array($ljp01_id))
				{
					JArrayHelper::toInteger($ljp01_id);
					$ljp01_id = implode(',', $ljp01_id);
					$type = $this->getState('filter.ljp01_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp01_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp01_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp01_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp01_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp01 params override menu item params only if menu param = 'use_ljp01'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp01' and there is no ljp01 param, use global

					// create an array of just the params set to 'use_ljp01'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp01')
						{
							// if the ljp01 has a value, use it
							if ($ljp01_params->get($key) != '')
							{
								// get the value from the ljp01
								$ljp01_array[$key] = $ljp01_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp01_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp01 params
					if (count($ljp01_array) > 0)
					{
						$ljp01_params = new Registry;
						$ljp01_params->loadArray($ljp01_array);
						$item->params->merge($ljp01_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp01_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp01, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
