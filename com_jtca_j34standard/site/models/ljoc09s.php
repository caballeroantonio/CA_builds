...
class JtCaModelLjoc09s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljoc09_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljoc09_id');
		$this->setState('filter.ljoc09_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljoc09_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljoc09_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljoc09_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljoc09_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljoc09_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljoc09_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljoc09_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljoc09_id'));
		$id .= ':'.$this->getState('filter.ljoc09_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljoc09s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljoc09s').' AS a');
		...
		// Filter by a single or group of ljoc09s.
		$ljoc09_id = $this->getState('filter.ljoc09_id');
		if ($ljoc09_id != '')
		{
			if (is_numeric($ljoc09_id))
			{
				$type = $this->getState('filter.ljoc09_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljoc09_id);
			}
			else
			{
				if (is_array($ljoc09_id))
				{
					JArrayHelper::toInteger($ljoc09_id);
					$ljoc09_id = implode(',', $ljoc09_id);
					$type = $this->getState('filter.ljoc09_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljoc09_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljoc09_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljoc09_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljoc09_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljoc09 params override menu item params only if menu param = 'use_ljoc09'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljoc09' and there is no ljoc09 param, use global

					// create an array of just the params set to 'use_ljoc09'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljoc09')
						{
							// if the ljoc09 has a value, use it
							if ($ljoc09_params->get($key) != '')
							{
								// get the value from the ljoc09
								$ljoc09_array[$key] = $ljoc09_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljoc09_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljoc09 params
					if (count($ljoc09_array) > 0)
					{
						$ljoc09_params = new Registry;
						$ljoc09_params->loadArray($ljoc09_array);
						$item->params->merge($ljoc09_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljoc09_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljoc09, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
