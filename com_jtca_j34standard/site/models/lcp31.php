...
/**
 * TSJ CDMX Libros TxCA Component LIBRETA DE LIBERTADES JUZGADOS DE DELITOS NO GRAVES Model
 *
 */
class JtCaModelLcp31 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp31';
	...
	{
		if (empty($config['lcp31_filter_fields']))
		{
			$config['lcp31_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp31s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRETA DE LIBERTADES JUZGADOS DE DELITOS NO GRAVES data.
	 *
	 * @param	integer	$pk	The id of the libreta de libertades juzgados de delitos no graves.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp31.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP31S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp31ChangeState', array('com_jtca.lcp31', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp31BeforeDelete', array('com_jtca.lcp31', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp31AfterDelete', array('com_jtca.lcp31', &$table));
				}
				else
				{
					// Prune items that you can't change.
