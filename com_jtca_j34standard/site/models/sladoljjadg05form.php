...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/sladoljjadg05.php';

class JtcaModelSladolJjadg05Form extends JtcaModelSladolJjadg05
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.sladoljjadg05';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.sladoljjadg05';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.sladoljjadg05', 'sladoljjadg05', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the sub libro adolescentes del libro de sentencias.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	Sub Libro ADOLESCENTES Del LIBRO DE SENTENCIAS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('sladoljjadg05.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.sladoljjadg05.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('sladoljjadg05.id');
		$is_new		= true;
		...
		// Reorder the adolescentes del libro de sentencias so the new sub libro adolescentes del libro de sentencias is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onSladolJjadg05BeforeSave', array('com_jtca.sladoljjadg05', &$table, $is_new));
		...
		$dispatcher->trigger('onSladolJjadg05AfterSave', array('com_jtca.sladoljjadg05', $table, $is_new));

		$this->setState('sladoljjadg05.id', $table->id);
