...
class JtCaModelLjccm12s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljccm12_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljccm12_id');
		$this->setState('filter.ljccm12_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljccm12_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljccm12_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljccm12_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljccm12_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljccm12_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljccm12_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljccm12_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljccm12_id'));
		$id .= ':'.$this->getState('filter.ljccm12_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljccm12s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljccm12s').' AS a');
		...
		// Filter by a single or group of ljccm12s.
		$ljccm12_id = $this->getState('filter.ljccm12_id');
		if ($ljccm12_id != '')
		{
			if (is_numeric($ljccm12_id))
			{
				$type = $this->getState('filter.ljccm12_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljccm12_id);
			}
			else
			{
				if (is_array($ljccm12_id))
				{
					JArrayHelper::toInteger($ljccm12_id);
					$ljccm12_id = implode(',', $ljccm12_id);
					$type = $this->getState('filter.ljccm12_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljccm12_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljccm12_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljccm12_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljccm12_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljccm12 params override menu item params only if menu param = 'use_ljccm12'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljccm12' and there is no ljccm12 param, use global

					// create an array of just the params set to 'use_ljccm12'
					$menu_params_array = $this->getState('params')->toArray();
					$ljccm12_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljccm12')
						{
							// if the ljccm12 has a value, use it
							if ($ljccm12_params->get($key) != '')
							{
								// get the value from the ljccm12
								$ljccm12_array[$key] = $ljccm12_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljccm12_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljccm12 params
					if (count($ljccm12_array) > 0)
					{
						$ljccm12_params = new Registry;
						$ljccm12_params->loadArray($ljccm12_array);
						$item->params->merge($ljccm12_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljccm12_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljccm12, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
