...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lspe03.php';

class JtCaModelLspe03Form extends JtCaModelLspe03
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lspe03';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lspe03';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lspe03', 'lspe03', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de resoluciones.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE RESOLUCIONES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lspe03.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lspe03.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lspe03.id');
		$is_new		= true;
		...
		// Reorder the libro de resoluciones so the new libro de resoluciones is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLspe03BeforeSave', array('com_jtca.lspe03', &$table, $is_new));
		...
		$dispatcher->trigger('onLspe03AfterSave', array('com_jtca.lspe03', $table, $is_new));

		$this->setState('lspe03.id', $table->id);
