...
class JtcaModelLjf07s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljf07_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljf07_id');
		$this->setState('filter.ljf07_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljf07_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljf07_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljf07_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljf07_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljf07_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljf07_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljf07_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljf07_id'));
		$id .= ':'.$this->getState('filter.ljf07_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljf07s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljf07s').' AS a');
		...
		// Filter by a single or group of ljf07s.
		$ljf07_id = $this->getState('filter.ljf07_id');
		if ($ljf07_id != '')
		{
			if (is_numeric($ljf07_id))
			{
				$type = $this->getState('filter.ljf07_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljf07_id);
			}
			else
			{
				if (is_array($ljf07_id))
				{
					JArrayHelper::toInteger($ljf07_id);
					$ljf07_id = implode(',', $ljf07_id);
					$type = $this->getState('filter.ljf07_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljf07_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljf07_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljf07_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljf07_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljf07 params override menu item params only if menu param = 'use_ljf07'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljf07' and there is no ljf07 param, use global

					// create an array of just the params set to 'use_ljf07'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljf07')
						{
							// if the ljf07 has a value, use it
							if ($ljf07_params->get($key) != '')
							{
								// get the value from the ljf07
								$ljf07_array[$key] = $ljf07_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljf07_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljf07 params
					if (count($ljf07_array) > 0)
					{
						$ljf07_params = new Registry;
						$ljf07_params->loadArray($ljf07_array);
						$item->params->merge($ljf07_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljf07_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljf07, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
