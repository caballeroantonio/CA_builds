...
class JtCaModelLjjadg02s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadg02_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg02_id');
		$this->setState('filter.ljjadg02_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg02_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg02_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg02_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg02_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg02_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg02_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg02_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg02_id'));
		$id .= ':'.$this->getState('filter.ljjadg02_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg02s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg02s').' AS a');
		...
		// Filter by a single or group of ljjadg02s.
		$ljjadg02_id = $this->getState('filter.ljjadg02_id');
		if ($ljjadg02_id != '')
		{
			if (is_numeric($ljjadg02_id))
			{
				$type = $this->getState('filter.ljjadg02_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg02_id);
			}
			else
			{
				if (is_array($ljjadg02_id))
				{
					JArrayHelper::toInteger($ljjadg02_id);
					$ljjadg02_id = implode(',', $ljjadg02_id);
					$type = $this->getState('filter.ljjadg02_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg02_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg02_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg02_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg02_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg02 params override menu item params only if menu param = 'use_ljjadg02'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg02' and there is no ljjadg02 param, use global

					// create an array of just the params set to 'use_ljjadg02'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg02_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg02')
						{
							// if the ljjadg02 has a value, use it
							if ($ljjadg02_params->get($key) != '')
							{
								// get the value from the ljjadg02
								$ljjadg02_array[$key] = $ljjadg02_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg02_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg02 params
					if (count($ljjadg02_array) > 0)
					{
						$ljjadg02_params = new Registry;
						$ljjadg02_params->loadArray($ljjadg02_array);
						$item->params->merge($ljjadg02_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg02_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg02, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
