...
/**
 * This models supports retrieving lists of adolescente.
 *
 */
class JtcaModelSlpersonedadsexos extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.slpersonedadsexos';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single adolescente has been specified either as a parameter or in the url Request
		$pk = $params->get('slpersonedadsexo_id', '') == '' ? $app->input->getInt('id', '') : $params->get('slpersonedadsexo_id');
		$this->setState('filter.slpersonedadsexo_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('slpersonedadsexo_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('slpersonedadsexo_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('slpersonedadsexo_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('slpersonedadsexo_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('slpersonedadsexo_initial_direction','ASC');
		}
		...
		if ($params->get('filter_slpersonedadsexo_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_slpersonedadsexo_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.slpersonedadsexo_id'));
		$id .= ':'.$this->getState('filter.slpersonedadsexo_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of slpersonedadsexos subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_slpersonedadsexos').' AS a');
		...
		// Filter by a single or group of slpersonedadsexos.
		$slpersonedadsexo_id = $this->getState('filter.slpersonedadsexo_id');
		if ($slpersonedadsexo_id != '')
		{
			if (is_numeric($slpersonedadsexo_id))
			{
				$type = $this->getState('filter.slpersonedadsexo_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $slpersonedadsexo_id);
			}
			else
			{
				if (is_array($slpersonedadsexo_id))
				{
					JArrayHelper::toInteger($slpersonedadsexo_id);
					$slpersonedadsexo_id = implode(',', $slpersonedadsexo_id);
					$type = $this->getState('filter.slpersonedadsexo_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$slpersonedadsexo_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_slpersonedadsexo_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_slpersonedadsexo_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of adolescente.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$slpersonedadsexo_params = new Registry;
				...
				{
					$item->params = $slpersonedadsexo_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Slpersonedadsexo params override menu item params only if menu param = 'use_slpersonedadsexo'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_slpersonedadsexo' and there is no slpersonedadsexo param, use global

					// create an array of just the params set to 'use_slpersonedadsexo'
					$menu_params_array = $this->getState('params')->toArray();
					$slpersonedadsexo_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_slpersonedadsexo')
						{
							// if the slpersonedadsexo has a value, use it
							if ($slpersonedadsexo_params->get($key) != '')
							{
								// get the value from the slpersonedadsexo
								$slpersonedadsexo_array[$key] = $slpersonedadsexo_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$slpersonedadsexo_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected slpersonedadsexo params
					if (count($slpersonedadsexo_array) > 0)
					{
						$slpersonedadsexo_params = new Registry;
						$slpersonedadsexo_params->loadArray($slpersonedadsexo_array);
						$item->params->merge($slpersonedadsexo_params);
					}


					// get display date
					switch ($item->params->get('list_show_slpersonedadsexo_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an slpersonedadsexo, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
