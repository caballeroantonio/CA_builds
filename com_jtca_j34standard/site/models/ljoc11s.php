...
class JtCaModelLjoc11s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljoc11_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljoc11_id');
		$this->setState('filter.ljoc11_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljoc11_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljoc11_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljoc11_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljoc11_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljoc11_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljoc11_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljoc11_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljoc11_id'));
		$id .= ':'.$this->getState('filter.ljoc11_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljoc11s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljoc11s').' AS a');
		...
		// Filter by a single or group of ljoc11s.
		$ljoc11_id = $this->getState('filter.ljoc11_id');
		if ($ljoc11_id != '')
		{
			if (is_numeric($ljoc11_id))
			{
				$type = $this->getState('filter.ljoc11_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljoc11_id);
			}
			else
			{
				if (is_array($ljoc11_id))
				{
					JArrayHelper::toInteger($ljoc11_id);
					$ljoc11_id = implode(',', $ljoc11_id);
					$type = $this->getState('filter.ljoc11_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljoc11_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljoc11_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljoc11_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljoc11_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljoc11 params override menu item params only if menu param = 'use_ljoc11'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljoc11' and there is no ljoc11 param, use global

					// create an array of just the params set to 'use_ljoc11'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljoc11')
						{
							// if the ljoc11 has a value, use it
							if ($ljoc11_params->get($key) != '')
							{
								// get the value from the ljoc11
								$ljoc11_array[$key] = $ljoc11_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljoc11_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljoc11 params
					if (count($ljoc11_array) > 0)
					{
						$ljoc11_params = new Registry;
						$ljoc11_params->loadArray($ljoc11_array);
						$item->params->merge($ljoc11_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljoc11_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljoc11, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
