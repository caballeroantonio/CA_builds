...
/**
 * This models supports retrieving lists of libro de remisiÃ³n al archivo.
 *
 */
class JtCaModelLjjadng11s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadng11s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadng11_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadng11_id');
		$this->setState('filter.ljjadng11_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadng11_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadng11_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadng11_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadng11_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadng11_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadng11_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadng11_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadng11_id'));
		$id .= ':'.$this->getState('filter.ljjadng11_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadng11s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadng11s').' AS a');
		...
		// Filter by a single or group of ljjadng11s.
		$ljjadng11_id = $this->getState('filter.ljjadng11_id');
		if ($ljjadng11_id != '')
		{
			if (is_numeric($ljjadng11_id))
			{
				$type = $this->getState('filter.ljjadng11_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadng11_id);
			}
			else
			{
				if (is_array($ljjadng11_id))
				{
					JArrayHelper::toInteger($ljjadng11_id);
					$ljjadng11_id = implode(',', $ljjadng11_id);
					$type = $this->getState('filter.ljjadng11_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadng11_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadng11_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadng11_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de remisiÃ³n al archivo.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadng11_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadng11 params override menu item params only if menu param = 'use_ljjadng11'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadng11' and there is no ljjadng11 param, use global

					// create an array of just the params set to 'use_ljjadng11'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadng11_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadng11')
						{
							// if the ljjadng11 has a value, use it
							if ($ljjadng11_params->get($key) != '')
							{
								// get the value from the ljjadng11
								$ljjadng11_array[$key] = $ljjadng11_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadng11_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadng11 params
					if (count($ljjadng11_array) > 0)
					{
						$ljjadng11_params = new Registry;
						$ljjadng11_params->loadArray($ljjadng11_array);
						$item->params->merge($ljjadng11_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadng11_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadng11, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
