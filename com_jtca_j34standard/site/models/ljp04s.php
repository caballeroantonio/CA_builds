...
class JtCaModelLjp04s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljp04_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp04_id');
		$this->setState('filter.ljp04_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp04_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp04_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp04_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp04_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp04_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp04_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp04_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp04_id'));
		$id .= ':'.$this->getState('filter.ljp04_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp04s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp04s').' AS a');
		...
		// Filter by a single or group of ljp04s.
		$ljp04_id = $this->getState('filter.ljp04_id');
		if ($ljp04_id != '')
		{
			if (is_numeric($ljp04_id))
			{
				$type = $this->getState('filter.ljp04_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp04_id);
			}
			else
			{
				if (is_array($ljp04_id))
				{
					JArrayHelper::toInteger($ljp04_id);
					$ljp04_id = implode(',', $ljp04_id);
					$type = $this->getState('filter.ljp04_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp04_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp04_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp04_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp04_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp04 params override menu item params only if menu param = 'use_ljp04'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp04' and there is no ljp04 param, use global

					// create an array of just the params set to 'use_ljp04'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp04')
						{
							// if the ljp04 has a value, use it
							if ($ljp04_params->get($key) != '')
							{
								// get the value from the ljp04
								$ljp04_array[$key] = $ljp04_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp04_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp04 params
					if (count($ljp04_array) > 0)
					{
						$ljp04_params = new Registry;
						$ljp04_params->loadArray($ljp04_array);
						$item->params->merge($ljp04_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp04_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp04, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
