...
/**
 * TSJ CDMX Libros TxCA Component LIBRETA DE LIBERTADES JUZGADOS DE EJECUCIÓN DE SENTENCIA Model
 *
 */
class JtCaModelLcp32 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp32';
	...
	{
		if (empty($config['lcp32_filter_fields']))
		{
			$config['lcp32_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp32s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRETA DE LIBERTADES JUZGADOS DE EJECUCIÓN DE SENTENCIA data.
	 *
	 * @param	integer	$pk	The id of the libreta de libertades juzgados de ejecución de sentencia.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp32.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP32S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp32ChangeState', array('com_jtca.lcp32', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp32BeforeDelete', array('com_jtca.lcp32', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp32AfterDelete', array('com_jtca.lcp32', &$table));
				}
				else
				{
					// Prune items that you can't change.
