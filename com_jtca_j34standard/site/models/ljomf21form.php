...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljomf21.php';

class JtCaModelLjomf21Form extends JtCaModelLjomf21
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljomf21';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljomf21';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljomf21', 'ljomf21', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de control de adopciones internacionales.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE CONTROL DE ADOPCIONES INTERNACIONALES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljomf21.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljomf21.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljomf21.id');
		$is_new		= true;
		...
		// Reorder the libro de control de adopciones internacionales so the new libro de control de adopciones internacionales is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjomf21BeforeSave', array('com_jtca.ljomf21', &$table, $is_new));
		...
		$dispatcher->trigger('onLjomf21AfterSave', array('com_jtca.ljomf21', $table, $is_new));

		$this->setState('ljomf21.id', $table->id);
