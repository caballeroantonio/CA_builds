...
class JtCaModelLsc02s extends JModelList
{
	 ...
	 */
	protected $context = 'com_jtca.lsc02s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de registro de asistencia (oficial) has been specified either as a parameter or in the url Request
		$pk = $params->get('lsc02_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsc02_id');
		$this->setState('filter.lsc02_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsc02_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsc02_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsc02_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsc02_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsc02_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsc02_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsc02_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsc02_id'));
		$id .= ':'.$this->getState('filter.lsc02_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsc02s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsc02s').' AS a');
		...
		// Filter by a single or group of lsc02s.
		$lsc02_id = $this->getState('filter.lsc02_id');
		if ($lsc02_id != '')
		{
			if (is_numeric($lsc02_id))
			{
				$type = $this->getState('filter.lsc02_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsc02_id);
			}
			else
			{
				if (is_array($lsc02_id))
				{
					JArrayHelper::toInteger($lsc02_id);
					$lsc02_id = implode(',', $lsc02_id);
					$type = $this->getState('filter.lsc02_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsc02_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsc02_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsc02_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de registro de asistencia (oficial).
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsc02_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsc02 params override menu item params only if menu param = 'use_lsc02'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsc02' and there is no lsc02 param, use global

					// create an array of just the params set to 'use_lsc02'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsc02')
						{
							// if the lsc02 has a value, use it
							if ($lsc02_params->get($key) != '')
							{
								// get the value from the lsc02
								$lsc02_array[$key] = $lsc02_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsc02_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsc02 params
					if (count($lsc02_array) > 0)
					{
						$lsc02_params = new Registry;
						$lsc02_params->loadArray($lsc02_array);
						$item->params->merge($lsc02_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsc02_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsc02, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
