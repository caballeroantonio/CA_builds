...
class JtCaModelLjemst02s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljemst02_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljemst02_id');
		$this->setState('filter.ljemst02_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljemst02_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljemst02_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljemst02_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljemst02_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljemst02_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljemst02_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljemst02_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljemst02_id'));
		$id .= ':'.$this->getState('filter.ljemst02_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljemst02s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljemst02s').' AS a');
		...
		// Filter by a single or group of ljemst02s.
		$ljemst02_id = $this->getState('filter.ljemst02_id');
		if ($ljemst02_id != '')
		{
			if (is_numeric($ljemst02_id))
			{
				$type = $this->getState('filter.ljemst02_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljemst02_id);
			}
			else
			{
				if (is_array($ljemst02_id))
				{
					JArrayHelper::toInteger($ljemst02_id);
					$ljemst02_id = implode(',', $ljemst02_id);
					$type = $this->getState('filter.ljemst02_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljemst02_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljemst02_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljemst02_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljemst02_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljemst02 params override menu item params only if menu param = 'use_ljemst02'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljemst02' and there is no ljemst02 param, use global

					// create an array of just the params set to 'use_ljemst02'
					$menu_params_array = $this->getState('params')->toArray();
					$ljemst02_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljemst02')
						{
							// if the ljemst02 has a value, use it
							if ($ljemst02_params->get($key) != '')
							{
								// get the value from the ljemst02
								$ljemst02_array[$key] = $ljemst02_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljemst02_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljemst02 params
					if (count($ljemst02_array) > 0)
					{
						$ljemst02_params = new Registry;
						$ljemst02_params->loadArray($ljemst02_array);
						$item->params->merge($ljemst02_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljemst02_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljemst02, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
