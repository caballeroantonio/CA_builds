...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljp11.php';

class JtCaModelLjp11Form extends JtCaModelLjp11
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljp11';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljp11';
	/**
	...
	*/
	public function getTable($type = 'Ljp11s', $prefix = 'JtCaTable', $config = array())
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljp11', 'ljp11', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljp11.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljp11.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljp11.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjp11BeforeSave', array('com_jtca.ljp11', &$table, $is_new));
		...
		$dispatcher->trigger('onLjp11AfterSave', array('com_jtca.ljp11', $table, $is_new));

		$this->setState('ljp11.id', $table->id);
