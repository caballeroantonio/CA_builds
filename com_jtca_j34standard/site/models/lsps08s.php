...
/**
 * This models supports retrieving lists of libro de turno de entregas de tocas.
 *
 */
class JtcaModelLsps08s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsps08s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno de entregas de tocas has been specified either as a parameter or in the url Request
		$pk = $params->get('lsps08_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsps08_id');
		$this->setState('filter.lsps08_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsps08_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsps08_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsps08_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsps08_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsps08_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsps08_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsps08_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsps08_id'));
		$id .= ':'.$this->getState('filter.lsps08_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsps08s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsps08s').' AS a');
		...
		// Filter by a single or group of lsps08s.
		$lsps08_id = $this->getState('filter.lsps08_id');
		if ($lsps08_id != '')
		{
			if (is_numeric($lsps08_id))
			{
				$type = $this->getState('filter.lsps08_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsps08_id);
			}
			else
			{
				if (is_array($lsps08_id))
				{
					JArrayHelper::toInteger($lsps08_id);
					$lsps08_id = implode(',', $lsps08_id);
					$type = $this->getState('filter.lsps08_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsps08_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsps08_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsps08_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno de entregas de tocas.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsps08_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsps08 params override menu item params only if menu param = 'use_lsps08'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsps08' and there is no lsps08 param, use global

					// create an array of just the params set to 'use_lsps08'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsps08')
						{
							// if the lsps08 has a value, use it
							if ($lsps08_params->get($key) != '')
							{
								// get the value from the lsps08
								$lsps08_array[$key] = $lsps08_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsps08_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsps08 params
					if (count($lsps08_array) > 0)
					{
						$lsps08_params = new Registry;
						$lsps08_params->loadArray($lsps08_array);
						$item->params->merge($lsps08_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsps08_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsps08, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
