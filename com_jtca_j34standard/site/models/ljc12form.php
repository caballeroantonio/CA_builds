...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljc12.php';

class JtCaModelLjc12Form extends JtCaModelLjc12
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljc12';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljc12';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljc12', 'ljc12', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljc12.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljc12.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljc12.id');
		$is_new		= true;
		...
		// Reorder the jc12 so the new libro de registro de amparos is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjc12BeforeSave', array('com_jtca.ljc12', &$table, $is_new));
		...
		$dispatcher->trigger('onLjc12AfterSave', array('com_jtca.ljc12', $table, $is_new));

		$this->setState('ljc12.id', $table->id);
