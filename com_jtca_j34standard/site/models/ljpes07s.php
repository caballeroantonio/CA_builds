...
/**
 * This models supports retrieving lists of libro de amparos circulantes.
 *
 */
class JtcaModelLjpes07s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpes07s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de amparos circulantes has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpes07_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes07_id');
		$this->setState('filter.ljpes07_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes07_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes07_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes07_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes07_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes07_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes07_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes07_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes07_id'));
		$id .= ':'.$this->getState('filter.ljpes07_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes07s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes07s').' AS a');
		...
		// Filter by a single or group of ljpes07s.
		$ljpes07_id = $this->getState('filter.ljpes07_id');
		if ($ljpes07_id != '')
		{
			if (is_numeric($ljpes07_id))
			{
				$type = $this->getState('filter.ljpes07_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes07_id);
			}
			else
			{
				if (is_array($ljpes07_id))
				{
					JArrayHelper::toInteger($ljpes07_id);
					$ljpes07_id = implode(',', $ljpes07_id);
					$type = $this->getState('filter.ljpes07_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes07_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes07_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes07_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de amparos circulantes.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes07_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes07 params override menu item params only if menu param = 'use_ljpes07'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes07' and there is no ljpes07 param, use global

					// create an array of just the params set to 'use_ljpes07'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes07_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes07')
						{
							// if the ljpes07 has a value, use it
							if ($ljpes07_params->get($key) != '')
							{
								// get the value from the ljpes07
								$ljpes07_array[$key] = $ljpes07_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes07_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes07 params
					if (count($ljpes07_array) > 0)
					{
						$ljpes07_params = new Registry;
						$ljpes07_params->loadArray($ljpes07_array);
						$item->params->merge($ljpes07_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes07_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes07, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
