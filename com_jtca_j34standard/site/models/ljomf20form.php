...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljomf20.php';

class JtCaModelLjomf20Form extends JtCaModelLjomf20
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljomf20';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljomf20';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljomf20', 'ljomf20', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljomf20.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljomf20.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljomf20.id');
		$is_new		= true;
		...
		// Reorder the libro de control de asuntos conforme a los artícul so the new libro de control de asuntos conforme a los artículos 13 fracción xiv y 25 de la ley de transparencia y acceso a la información pública is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjomf20BeforeSave', array('com_jtca.ljomf20', &$table, $is_new));
		...
		$dispatcher->trigger('onLjomf20AfterSave', array('com_jtca.ljomf20', $table, $is_new));

		$this->setState('ljomf20.id', $table->id);
