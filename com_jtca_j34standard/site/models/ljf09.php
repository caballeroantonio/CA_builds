 ...
 *
 */
class JtCaModelLjf09 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljf09';
	...
	{
		if (empty($config['ljf09_filter_fields']))
		{
			$config['ljf09_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljf09s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljf09.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJF09S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjf09ChangeState', array('com_jtca.ljf09', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjf09BeforeDelete', array('com_jtca.ljf09', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjf09AfterDelete', array('com_jtca.ljf09', &$table));
				}
				else
				{
					// Prune items that you can't change.
