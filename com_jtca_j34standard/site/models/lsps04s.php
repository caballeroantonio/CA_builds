...
class JtCaModelLsps04s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('lsps04_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsps04_id');
		$this->setState('filter.lsps04_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsps04_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsps04_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsps04_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsps04_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsps04_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsps04_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsps04_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsps04_id'));
		$id .= ':'.$this->getState('filter.lsps04_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsps04s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsps04s').' AS a');
		...
		// Filter by a single or group of lsps04s.
		$lsps04_id = $this->getState('filter.lsps04_id');
		if ($lsps04_id != '')
		{
			if (is_numeric($lsps04_id))
			{
				$type = $this->getState('filter.lsps04_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsps04_id);
			}
			else
			{
				if (is_array($lsps04_id))
				{
					JArrayHelper::toInteger($lsps04_id);
					$lsps04_id = implode(',', $lsps04_id);
					$type = $this->getState('filter.lsps04_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsps04_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsps04_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsps04_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsps04_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsps04 params override menu item params only if menu param = 'use_lsps04'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsps04' and there is no lsps04 param, use global

					// create an array of just the params set to 'use_lsps04'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsps04')
						{
							// if the lsps04 has a value, use it
							if ($lsps04_params->get($key) != '')
							{
								// get the value from the lsps04
								$lsps04_array[$key] = $lsps04_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsps04_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsps04 params
					if (count($lsps04_array) > 0)
					{
						$lsps04_params = new Registry;
						$lsps04_params->loadArray($lsps04_array);
						$item->params->merge($lsps04_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsps04_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsps04, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
