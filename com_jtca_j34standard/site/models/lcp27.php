...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE TURNO PARA INCOMPETENCIAS DELITOS NO GRAVES Model
 *
 */
class JtCaModelLcp27 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp27';
	...
	{
		if (empty($config['lcp27_filter_fields']))
		{
			$config['lcp27_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp27s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE TURNO PARA INCOMPETENCIAS DELITOS NO GRAVES data.
	 *
	 * @param	integer	$pk	The id of the libro de turno para incompetencias delitos no graves.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp27.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP27S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp27ChangeState', array('com_jtca.lcp27', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp27BeforeDelete', array('com_jtca.lcp27', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp27AfterDelete', array('com_jtca.lcp27', &$table));
				}
				else
				{
					// Prune items that you can't change.
