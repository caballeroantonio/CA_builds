...
/**
 * TSJ CDMX Libros TxCA Component AGENDA (OFICIAL) Model
 *
 */
class JtCaModelLsc05 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsc05';
	...
	{
		if (empty($config['lsc05_filter_fields']))
		{
			$config['lsc05_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lsc05s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get AGENDA (OFICIAL) data.
	 *
	 * @param	integer	$pk	The id of the agenda (oficial).
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsc05.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSC05S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsc05ChangeState', array('com_jtca.lsc05', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsc05BeforeDelete', array('com_jtca.lsc05', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsc05AfterDelete', array('com_jtca.lsc05', &$table));
				}
				else
				{
					// Prune items that you can't change.
