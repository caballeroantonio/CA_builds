...
/**
 * TSJ CDMX Libros TxCA Component Inculpados Model
 *
 */
class JtcaModelSlpersondelito extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.slpersondelito';
	...
	{
		if (empty($config['slpersondelito_filter_fields']))
		{
			$config['slpersondelito_filter_fields'] = array(
				'id', 'a.id',
	...
	protected function populateState()
	{
		...
		$this->setState('slpersondelito.id', $pk);
	...
	*/
	public function getTable($type = 'Slpersondelitos', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get Inculpados data.
	 *
	 * @param	integer	$pk	The id of the inculpados.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('slpersondelito.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_SLPERSONDELITOS_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onSlpersondelitoChangeState', array('com_jtca.slpersondelito', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onSlpersondelitoBeforeDelete', array('com_jtca.slpersondelito', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onSlpersondelitoAfterDelete', array('com_jtca.slpersondelito', &$table));
				}
				else
				{
					// Prune items that you can't change.
