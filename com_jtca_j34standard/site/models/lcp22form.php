...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp22.php';

class JtcaModelLcp22Form extends JtcaModelLcp22
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp22';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp22';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp22', 'lcp22', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de ordenes de arraigos y de cateos primera instancia..
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE ORDENES DE ARRAIGOS Y DE CATEOS PRIMERA INSTANCIA. item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp22.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp22.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp22.id');
		$is_new		= true;
		...
		// Reorder the libro de ordenes de arraigos y de cateos primera i so the new libro de ordenes de arraigos y de cateos primera instancia. is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp22BeforeSave', array('com_jtca.lcp22', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp22AfterSave', array('com_jtca.lcp22', $table, $is_new));

		$this->setState('lcp22.id', $table->id);
