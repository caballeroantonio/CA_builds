 ...
 *
 */
class JtCaModelLjccm03 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljccm03';
	...
	{
		if (empty($config['ljccm03_filter_fields']))
		{
			$config['ljccm03_filter_fields'] = array(
				'id', 'a.id',
				'billete', 'a.billete',
	...
	*/
	public function getTable($type = 'Ljccm03s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljccm03.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJCCM03S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjccm03ChangeState', array('com_jtca.ljccm03', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjccm03BeforeDelete', array('com_jtca.ljccm03', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjccm03AfterDelete', array('com_jtca.ljccm03', &$table));
				}
				else
				{
					// Prune items that you can't change.
