...
/**
 * This models supports retrieving lists of articulo 36 o articulo 28 ljadf.
 *
 */
class JtCaModelLjjadg15s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadg15s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single articulo 36 o articulo 28 ljadf has been specified either as a parameter or in the url Request
		$pk = $params->get('ljjadg15_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg15_id');
		$this->setState('filter.ljjadg15_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg15_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg15_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg15_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg15_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg15_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg15_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg15_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg15_id'));
		$id .= ':'.$this->getState('filter.ljjadg15_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg15s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg15s').' AS a');
		...
		// Filter by a single or group of ljjadg15s.
		$ljjadg15_id = $this->getState('filter.ljjadg15_id');
		if ($ljjadg15_id != '')
		{
			if (is_numeric($ljjadg15_id))
			{
				$type = $this->getState('filter.ljjadg15_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg15_id);
			}
			else
			{
				if (is_array($ljjadg15_id))
				{
					JArrayHelper::toInteger($ljjadg15_id);
					$ljjadg15_id = implode(',', $ljjadg15_id);
					$type = $this->getState('filter.ljjadg15_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg15_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg15_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg15_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of articulo 36 o articulo 28 ljadf.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg15_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg15 params override menu item params only if menu param = 'use_ljjadg15'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg15' and there is no ljjadg15 param, use global

					// create an array of just the params set to 'use_ljjadg15'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg15_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg15')
						{
							// if the ljjadg15 has a value, use it
							if ($ljjadg15_params->get($key) != '')
							{
								// get the value from the ljjadg15
								$ljjadg15_array[$key] = $ljjadg15_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg15_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg15 params
					if (count($ljjadg15_array) > 0)
					{
						$ljjadg15_params = new Registry;
						$ljjadg15_params->loadArray($ljjadg15_array);
						$item->params->merge($ljjadg15_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg15_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg15, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
