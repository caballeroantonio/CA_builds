...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpdng09.php';

class JtCaModelLjpdng09Form extends JtCaModelLjpdng09
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpdng09';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpdng09';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpdng09', 'ljpdng09', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpdng09.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpdng09.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpdng09.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjpdng09BeforeSave', array('com_jtca.ljpdng09', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpdng09AfterSave', array('com_jtca.ljpdng09', $table, $is_new));

		$this->setState('ljpdng09.id', $table->id);
