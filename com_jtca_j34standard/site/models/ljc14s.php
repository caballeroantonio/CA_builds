...
/**
 * This models supports retrieving lists of jc14.
 *
 */
class JtCaModelLjc14s extends JModelList
{
	 ...
	 */
	protected $context = 'com_jtca.ljc14s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljc14_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc14_id');
		$this->setState('filter.ljc14_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc14_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc14_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc14_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc14_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc14_initial_direction','ASC');
		}
		...
		{
			$this->setState('filter.archived', $params->get('filter_ljc14_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc14_id'));
		$id .= ':'.$this->getState('filter.ljc14_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc14s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc14s').' AS a');
		...
		$ljc14_id = $this->getState('filter.ljc14_id');
		if ($ljc14_id != '')
		{
			if (is_numeric($ljc14_id))
			{
				$type = $this->getState('filter.ljc14_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc14_id);
			}
			else
			{
				if (is_array($ljc14_id))
				{
					JArrayHelper::toInteger($ljc14_id);
					$ljc14_id = implode(',', $ljc14_id);
					$type = $this->getState('filter.ljc14_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc14_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc14_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc14_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc14_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc14 params override menu item params only if menu param = 'use_ljc14'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc14' and there is no ljc14 param, use global

					// create an array of just the params set to 'use_ljc14'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc14')
						{
							// if the ljc14 has a value, use it
							if ($ljc14_params->get($key) != '')
							{
								// get the value from the ljc14
								$ljc14_array[$key] = $ljc14_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc14_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc14 params
					if (count($ljc14_array) > 0)
					{
						$ljc14_params = new Registry;
						$ljc14_params->loadArray($ljc14_array);
						$item->params->merge($ljc14_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc14_date'))
					{
						case 'modified':
