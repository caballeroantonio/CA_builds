...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp09.php';

class JtcaModelLcp09Form extends JtcaModelLcp09
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp09';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp09';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp09', 'lcp09', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de etapa de ejecución para adolescentes.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE ETAPA DE EJECUCIÓN PARA ADOLESCENTES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp09.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp09.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp09.id');
		$is_new		= true;
		...
		// Reorder the libro de etapa de ejecución para adolescentes so the new libro de etapa de ejecución para adolescentes is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp09BeforeSave', array('com_jtca.lcp09', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp09AfterSave', array('com_jtca.lcp09', $table, $is_new));

		$this->setState('lcp09.id', $table->id);
