...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE REGISTRO DE AMPAROS CIRCULANTES O NEGADOS Model
 *
 */
class JtcaModelLjp12 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljp12';
	...
	{
		if (empty($config['ljp12_filter_fields']))
		{
			$config['ljp12_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljp12s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE REGISTRO DE AMPAROS CIRCULANTES O NEGADOS data.
	 *
	 * @param	integer	$pk	The id of the libro de registro de amparos circulantes o negados.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljp12.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJP12S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjp12ChangeState', array('com_jtca.ljp12', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjp12BeforeDelete', array('com_jtca.ljp12', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjp12AfterDelete', array('com_jtca.ljp12', &$table));
				}
				else
				{
					// Prune items that you can't change.
