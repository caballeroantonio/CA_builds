...
class JtcaModelLsps05s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('lsps05_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsps05_id');
		$this->setState('filter.lsps05_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsps05_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsps05_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsps05_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsps05_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsps05_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsps05_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsps05_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsps05_id'));
		$id .= ':'.$this->getState('filter.lsps05_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsps05s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsps05s').' AS a');
		...
		// Filter by a single or group of lsps05s.
		$lsps05_id = $this->getState('filter.lsps05_id');
		if ($lsps05_id != '')
		{
			if (is_numeric($lsps05_id))
			{
				$type = $this->getState('filter.lsps05_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsps05_id);
			}
			else
			{
				if (is_array($lsps05_id))
				{
					JArrayHelper::toInteger($lsps05_id);
					$lsps05_id = implode(',', $lsps05_id);
					$type = $this->getState('filter.lsps05_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsps05_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsps05_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsps05_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsps05_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsps05 params override menu item params only if menu param = 'use_lsps05'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsps05' and there is no lsps05 param, use global

					// create an array of just the params set to 'use_lsps05'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsps05')
						{
							// if the lsps05 has a value, use it
							if ($lsps05_params->get($key) != '')
							{
								// get the value from the lsps05
								$lsps05_array[$key] = $lsps05_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsps05_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsps05 params
					if (count($lsps05_array) > 0)
					{
						$lsps05_params = new Registry;
						$lsps05_params->loadArray($lsps05_array);
						$item->params->merge($lsps05_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsps05_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsps05, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
