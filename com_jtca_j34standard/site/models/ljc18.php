...
class JtCaModelLjc18 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljc18';
	...
	{
		if (empty($config['ljc18_filter_fields']))
		{
			$config['ljc18_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljc18s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljc18.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJC18S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjc18ChangeState', array('com_jtca.ljc18', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjc18BeforeDelete', array('com_jtca.ljc18', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjc18AfterDelete', array('com_jtca.ljc18', &$table));
				}
				else
				{
					// Prune items that you can't change.
