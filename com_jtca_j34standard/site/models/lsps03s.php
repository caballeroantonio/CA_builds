...
/**
 * This models supports retrieving lists of libro de gobierno (nuevo sistema) (colegiado).
 *
 */
class JtcaModelLsps03s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsps03s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de gobierno (nuevo sistema) (colegiado) has been specified either as a parameter or in the url Request
		$pk = $params->get('lsps03_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsps03_id');
		$this->setState('filter.lsps03_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsps03_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsps03_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsps03_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsps03_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsps03_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsps03_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsps03_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsps03_id'));
		$id .= ':'.$this->getState('filter.lsps03_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsps03s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsps03s').' AS a');
		...
		// Filter by a single or group of lsps03s.
		$lsps03_id = $this->getState('filter.lsps03_id');
		if ($lsps03_id != '')
		{
			if (is_numeric($lsps03_id))
			{
				$type = $this->getState('filter.lsps03_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsps03_id);
			}
			else
			{
				if (is_array($lsps03_id))
				{
					JArrayHelper::toInteger($lsps03_id);
					$lsps03_id = implode(',', $lsps03_id);
					$type = $this->getState('filter.lsps03_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsps03_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsps03_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsps03_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de gobierno (nuevo sistema) (colegiado).
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsps03_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsps03 params override menu item params only if menu param = 'use_lsps03'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsps03' and there is no lsps03 param, use global

					// create an array of just the params set to 'use_lsps03'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsps03')
						{
							// if the lsps03 has a value, use it
							if ($lsps03_params->get($key) != '')
							{
								// get the value from the lsps03
								$lsps03_array[$key] = $lsps03_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsps03_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsps03 params
					if (count($lsps03_array) > 0)
					{
						$lsps03_params = new Registry;
						$lsps03_params->loadArray($lsps03_array);
						$item->params->merge($lsps03_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsps03_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsps03, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
