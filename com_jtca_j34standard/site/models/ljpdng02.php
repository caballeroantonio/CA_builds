...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE CERTIFICADO DE DEPÓSITOS DE INGRESO Model
 *
 */
class JtCaModelLjpdng02 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljpdng02';
	...
	{
		if (empty($config['ljpdng02_filter_fields']))
		{
			$config['ljpdng02_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljpdng02s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE CERTIFICADO DE DEPÓSITOS DE INGRESO data.
	 *
	 * @param	integer	$pk	The id of the libro de certificado de depósitos de ingreso.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljpdng02.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJPDNG02S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjpdng02ChangeState', array('com_jtca.ljpdng02', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjpdng02BeforeDelete', array('com_jtca.ljpdng02', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjpdng02AfterDelete', array('com_jtca.ljpdng02', &$table));
				}
				else
				{
					// Prune items that you can't change.
