...
class JtCaModelLjemst07s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljemst07_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljemst07_id');
		$this->setState('filter.ljemst07_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljemst07_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljemst07_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljemst07_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljemst07_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljemst07_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljemst07_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljemst07_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljemst07_id'));
		$id .= ':'.$this->getState('filter.ljemst07_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljemst07s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljemst07s').' AS a');
		...
		// Filter by a single or group of ljemst07s.
		$ljemst07_id = $this->getState('filter.ljemst07_id');
		if ($ljemst07_id != '')
		{
			if (is_numeric($ljemst07_id))
			{
				$type = $this->getState('filter.ljemst07_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljemst07_id);
			}
			else
			{
				if (is_array($ljemst07_id))
				{
					JArrayHelper::toInteger($ljemst07_id);
					$ljemst07_id = implode(',', $ljemst07_id);
					$type = $this->getState('filter.ljemst07_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljemst07_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljemst07_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljemst07_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljemst07_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljemst07 params override menu item params only if menu param = 'use_ljemst07'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljemst07' and there is no ljemst07 param, use global

					// create an array of just the params set to 'use_ljemst07'
					$menu_params_array = $this->getState('params')->toArray();
					$ljemst07_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljemst07')
						{
							// if the ljemst07 has a value, use it
							if ($ljemst07_params->get($key) != '')
							{
								// get the value from the ljemst07
								$ljemst07_array[$key] = $ljemst07_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljemst07_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljemst07 params
					if (count($ljemst07_array) > 0)
					{
						$ljemst07_params = new Registry;
						$ljemst07_params->loadArray($ljemst07_array);
						$item->params->merge($ljemst07_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljemst07_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljemst07, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
