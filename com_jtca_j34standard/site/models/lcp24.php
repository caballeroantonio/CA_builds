...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE REGISTRO DE LA LEY DE ACCESO A LAS MUJERES A UNA VIDA LIBRE DE VIOLENCIA. Model
 *
 */
class JtCaModelLcp24 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp24';
	...
	{
		if (empty($config['lcp24_filter_fields']))
		{
			$config['lcp24_filter_fields'] = array(
				'id', 'a.id',
				...
				'organo1','a.organo1',
				'field8','a.field8',
				'field9_isMoral','a.field9_isMoral',
				'field9_paterno','a.field9_paterno',
				'field9_materno','a.field9_materno',
				'field9_nombre','a.field9_nombre',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lcp24s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE REGISTRO DE LA LEY DE ACCESO A LAS MUJERES A UNA VIDA LIBRE DE VIOLENCIA. data.
	 *
	 * @param	integer	$pk	The id of the libro de registro de la ley de acceso a las mujeres a una vida libre de violencia..
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp24.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP24S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp24ChangeState', array('com_jtca.lcp24', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp24BeforeDelete', array('com_jtca.lcp24', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp24AfterDelete', array('com_jtca.lcp24', &$table));
				}
				else
				{
					// Prune items that you can't change.
