...
class JtCaModelLjpdng06s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpdng06_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng06_id');
		$this->setState('filter.ljpdng06_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng06_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng06_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng06_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng06_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng06_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng06_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng06_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng06_id'));
		$id .= ':'.$this->getState('filter.ljpdng06_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng06s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng06s').' AS a');
		...
		// Filter by a single or group of ljpdng06s.
		$ljpdng06_id = $this->getState('filter.ljpdng06_id');
		if ($ljpdng06_id != '')
		{
			if (is_numeric($ljpdng06_id))
			{
				$type = $this->getState('filter.ljpdng06_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng06_id);
			}
			else
			{
				if (is_array($ljpdng06_id))
				{
					JArrayHelper::toInteger($ljpdng06_id);
					$ljpdng06_id = implode(',', $ljpdng06_id);
					$type = $this->getState('filter.ljpdng06_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng06_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng06_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng06_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng06_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng06 params override menu item params only if menu param = 'use_ljpdng06'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng06' and there is no ljpdng06 param, use global

					// create an array of just the params set to 'use_ljpdng06'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng06_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng06')
						{
							// if the ljpdng06 has a value, use it
							if ($ljpdng06_params->get($key) != '')
							{
								// get the value from the ljpdng06
								$ljpdng06_array[$key] = $ljpdng06_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng06_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng06 params
					if (count($ljpdng06_array) > 0)
					{
						$ljpdng06_params = new Registry;
						$ljpdng06_params->loadArray($ljpdng06_array);
						$item->params->merge($ljpdng06_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng06_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng06, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
