...
class JtCaModelLjp05s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljp05_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp05_id');
		$this->setState('filter.ljp05_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp05_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp05_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp05_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp05_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp05_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp05_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp05_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp05_id'));
		$id .= ':'.$this->getState('filter.ljp05_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp05s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp05s').' AS a');
		...
		// Filter by a single or group of ljp05s.
		$ljp05_id = $this->getState('filter.ljp05_id');
		if ($ljp05_id != '')
		{
			if (is_numeric($ljp05_id))
			{
				$type = $this->getState('filter.ljp05_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp05_id);
			}
			else
			{
				if (is_array($ljp05_id))
				{
					JArrayHelper::toInteger($ljp05_id);
					$ljp05_id = implode(',', $ljp05_id);
					$type = $this->getState('filter.ljp05_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp05_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp05_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp05_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp05_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp05 params override menu item params only if menu param = 'use_ljp05'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp05' and there is no ljp05 param, use global

					// create an array of just the params set to 'use_ljp05'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp05')
						{
							// if the ljp05 has a value, use it
							if ($ljp05_params->get($key) != '')
							{
								// get the value from the ljp05
								$ljp05_array[$key] = $ljp05_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp05_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp05 params
					if (count($ljp05_array) > 0)
					{
						$ljp05_params = new Registry;
						$ljp05_params->loadArray($ljp05_array);
						$item->params->merge($ljp05_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp05_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp05, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
