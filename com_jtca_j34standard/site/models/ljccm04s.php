...
/**
 * This models supports retrieving lists of libro de registro de promociones.
 *
 */
class JtCaModelLjccm04s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljccm04s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljccm04_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljccm04_id');
		$this->setState('filter.ljccm04_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljccm04_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljccm04_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljccm04_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljccm04_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljccm04_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljccm04_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljccm04_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljccm04_id'));
		$id .= ':'.$this->getState('filter.ljccm04_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljccm04s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljccm04s').' AS a');
		...
		// Filter by a single or group of ljccm04s.
		$ljccm04_id = $this->getState('filter.ljccm04_id');
		if ($ljccm04_id != '')
		{
			if (is_numeric($ljccm04_id))
			{
				$type = $this->getState('filter.ljccm04_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljccm04_id);
			}
			else
			{
				if (is_array($ljccm04_id))
				{
					JArrayHelper::toInteger($ljccm04_id);
					$ljccm04_id = implode(',', $ljccm04_id);
					$type = $this->getState('filter.ljccm04_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljccm04_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljccm04_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljccm04_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de registro de promociones.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljccm04_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljccm04 params override menu item params only if menu param = 'use_ljccm04'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljccm04' and there is no ljccm04 param, use global

					// create an array of just the params set to 'use_ljccm04'
					$menu_params_array = $this->getState('params')->toArray();
					$ljccm04_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljccm04')
						{
							// if the ljccm04 has a value, use it
							if ($ljccm04_params->get($key) != '')
							{
								// get the value from the ljccm04
								$ljccm04_array[$key] = $ljccm04_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljccm04_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljccm04 params
					if (count($ljccm04_array) > 0)
					{
						$ljccm04_params = new Registry;
						$ljccm04_params->loadArray($ljccm04_array);
						$item->params->merge($ljccm04_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljccm04_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljccm04, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
