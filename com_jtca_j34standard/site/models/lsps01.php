...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE GOBIERNO (SISTEMA TRADICIONAL) Model
 *
 */
class JtCaModelLsps01 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsps01';
	...
	{
		if (empty($config['lsps01_filter_fields']))
		{
			$config['lsps01_filter_fields'] = array(
				'id', 'a.id',
				'id_expediente','a.id_expediente',
				'id_organo','a.id_organo',
				'id_secretaria','a.id_secretaria',
				'anoj','a.anoj',
				'field2350','a.field2350',
				'txt_field2351','a.txt_field2351',
				'id_field2351','a.id_field2351',
				'txt_field2352','a.txt_field2352',
				'id_field2352','a.id_field2352',
				'field2353','a.field2353',
				'field2354','a.field2354',
				'field2355','a.field2355',
				'field2356','a.field2356',
				'field2357','a.field2357',
				'field2358','a.field2358',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lsps01s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE GOBIERNO (SISTEMA TRADICIONAL) data.
	 *
	 * @param	integer	$pk	The id of the libro de gobierno (sistema tradicional).
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsps01.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPS01S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsps01ChangeState', array('com_jtca.lsps01', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsps01BeforeDelete', array('com_jtca.lsps01', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsps01AfterDelete', array('com_jtca.lsps01', &$table));
				}
				else
				{
					// Prune items that you can't change.
