...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpdng12.php';

class JtCaModelLjpdng12Form extends JtCaModelLjpdng12
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpdng12';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpdng12';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpdng12', 'ljpdng12', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de control de fianzas.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE CONTROL DE FIANZAS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpdng12.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpdng12.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpdng12.id');
		$is_new		= true;
		...
		// Reorder the libro de control de fianzas so the new libro de control de fianzas is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpdng12BeforeSave', array('com_jtca.ljpdng12', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpdng12AfterSave', array('com_jtca.ljpdng12', $table, $is_new));

		$this->setState('ljpdng12.id', $table->id);
