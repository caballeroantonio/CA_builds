...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE CERTIFICADO DE DEPÓSITOS DE INGRESOS Y EGRESOS Model
 *
 */
class JtCaModelLjoc03 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljoc03';
	...
	{
		if (empty($config['ljoc03_filter_fields']))
		{
			$config['ljoc03_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljoc03s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE CERTIFICADO DE DEPÓSITOS DE INGRESOS Y EGRESOS data.
	 *
	 * @param	integer	$pk	The id of the libro de certificado de depósitos de ingresos y egresos.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljoc03.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJOC03S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjoc03ChangeState', array('com_jtca.ljoc03', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjoc03BeforeDelete', array('com_jtca.ljoc03', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjoc03AfterDelete', array('com_jtca.ljoc03', &$table));
				}
				else
				{
					// Prune items that you can't change.
