...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljccm04.php';

class JtCaModelLjccm04Form extends JtCaModelLjccm04
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljccm04';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljccm04';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljccm04', 'ljccm04', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljccm04.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljccm04.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljccm04.id');
		$is_new		= true;
		...
		// Reorder the libro de registro de promociones so the new libro de registro de promociones is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjccm04BeforeSave', array('com_jtca.ljccm04', &$table, $is_new));
		...
		$dispatcher->trigger('onLjccm04AfterSave', array('com_jtca.ljccm04', $table, $is_new));

		$this->setState('ljccm04.id', $table->id);
