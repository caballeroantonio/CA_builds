...
/**
 * TSJ CDMX Libros TxCA Component LIBRETA DE LIBERTADES DE SALAS DE ADOLESCENTES Model
 *
 */
class JtCaModelLcp35 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp35';
	...
	{
		if (empty($config['lcp35_filter_fields']))
		{
			$config['lcp35_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp35s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRETA DE LIBERTADES DE SALAS DE ADOLESCENTES data.
	 *
	 * @param	integer	$pk	The id of the libreta de libertades de salas de adolescentes.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp35.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP35S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp35ChangeState', array('com_jtca.lcp35', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp35BeforeDelete', array('com_jtca.lcp35', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp35AfterDelete', array('com_jtca.lcp35', &$table));
				}
				else
				{
					// Prune items that you can't change.
