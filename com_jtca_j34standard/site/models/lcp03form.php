...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp03.php';

class JtCaModelLcp03Form extends JtCaModelLcp03
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp03';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp03';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp03', 'lcp03', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno de remisiones con detenido de juzgados adolescentes.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO DE REMISIONES CON DETENIDO DE JUZGADOS ADOLESCENTES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp03.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp03.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp03.id');
		$is_new		= true;
		...
		// Reorder the libro de turno de remisiones con detenido de juzga so the new libro de turno de remisiones con detenido de juzgados adolescentes is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp03BeforeSave', array('com_jtca.lcp03', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp03AfterSave', array('com_jtca.lcp03', $table, $is_new));

		$this->setState('lcp03.id', $table->id);
