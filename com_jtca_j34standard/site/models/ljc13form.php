...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljc13.php';

class JtCaModelLjc13Form extends JtCaModelLjc13
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljc13';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljc13';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljc13', 'ljc13', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljc13.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljc13.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljc13.id');
		$is_new		= true;
		...
		// Reorder the jc13 so the new libro de control de fianzas is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjc13BeforeSave', array('com_jtca.ljc13', &$table, $is_new));
		...
		$dispatcher->trigger('onLjc13AfterSave', array('com_jtca.ljc13', $table, $is_new));

		$this->setState('ljc13.id', $table->id);
