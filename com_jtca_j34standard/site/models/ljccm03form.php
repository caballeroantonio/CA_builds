...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljccm03.php';

class JtCaModelLjccm03Form extends JtCaModelLjccm03
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljccm03';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljccm03';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljccm03', 'ljccm03', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljccm03.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljccm03.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljccm03.id');
		$is_new		= true;
		...
		// Reorder the libro de egresos de valores so the new libro de egresos de valores is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjccm03BeforeSave', array('com_jtca.ljccm03', &$table, $is_new));
		...
		$dispatcher->trigger('onLjccm03AfterSave', array('com_jtca.ljccm03', $table, $is_new));

		$this->setState('ljccm03.id', $table->id);
