...
/**
 * This models supports retrieving lists of libro de multas (oficial).
 *
 */
class JtCaModelLsc04s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lsc04s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de multas (oficial) has been specified either as a parameter or in the url Request
		$pk = $params->get('lsc04_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsc04_id');
		$this->setState('filter.lsc04_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsc04_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsc04_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsc04_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsc04_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsc04_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsc04_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsc04_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsc04_id'));
		$id .= ':'.$this->getState('filter.lsc04_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsc04s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsc04s').' AS a');
		...
		// Filter by a single or group of lsc04s.
		$lsc04_id = $this->getState('filter.lsc04_id');
		if ($lsc04_id != '')
		{
			if (is_numeric($lsc04_id))
			{
				$type = $this->getState('filter.lsc04_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsc04_id);
			}
			else
			{
				if (is_array($lsc04_id))
				{
					JArrayHelper::toInteger($lsc04_id);
					$lsc04_id = implode(',', $lsc04_id);
					$type = $this->getState('filter.lsc04_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsc04_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsc04_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsc04_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de multas (oficial).
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsc04_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsc04 params override menu item params only if menu param = 'use_lsc04'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsc04' and there is no lsc04 param, use global

					// create an array of just the params set to 'use_lsc04'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsc04')
						{
							// if the lsc04 has a value, use it
							if ($lsc04_params->get($key) != '')
							{
								// get the value from the lsc04
								$lsc04_array[$key] = $lsc04_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsc04_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsc04 params
					if (count($lsc04_array) > 0)
					{
						$lsc04_params = new Registry;
						$lsc04_params->loadArray($lsc04_array);
						$item->params->merge($lsc04_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsc04_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsc04, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
