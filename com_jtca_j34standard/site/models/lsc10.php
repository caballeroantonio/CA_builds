...
class JtCaModelLsc10 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsc10';
	...
	{
		if (empty($config['lsc10_filter_fields']))
		{
			$config['lsc10_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	...
	/**
	 * Method to get LIBRO DE ACTUARIO (OFICIAL) data.
	 *
	 * @param	integer	$pk	The id of the libro de actuario (oficial).
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsc10.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSC10S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsc10ChangeState', array('com_jtca.lsc10', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsc10BeforeDelete', array('com_jtca.lsc10', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsc10AfterDelete', array('com_jtca.lsc10', &$table));
				}
				else
				{
					// Prune items that you can't change.
