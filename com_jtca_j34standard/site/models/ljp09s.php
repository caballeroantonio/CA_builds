...
class JtcaModelLjp09s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljp09_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp09_id');
		$this->setState('filter.ljp09_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp09_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp09_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp09_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp09_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp09_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp09_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp09_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp09_id'));
		$id .= ':'.$this->getState('filter.ljp09_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp09s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp09s').' AS a');
		...
		// Filter by a single or group of ljp09s.
		$ljp09_id = $this->getState('filter.ljp09_id');
		if ($ljp09_id != '')
		{
			if (is_numeric($ljp09_id))
			{
				$type = $this->getState('filter.ljp09_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp09_id);
			}
			else
			{
				if (is_array($ljp09_id))
				{
					JArrayHelper::toInteger($ljp09_id);
					$ljp09_id = implode(',', $ljp09_id);
					$type = $this->getState('filter.ljp09_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp09_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp09_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp09_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp09_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp09 params override menu item params only if menu param = 'use_ljp09'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp09' and there is no ljp09 param, use global

					// create an array of just the params set to 'use_ljp09'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp09')
						{
							// if the ljp09 has a value, use it
							if ($ljp09_params->get($key) != '')
							{
								// get the value from the ljp09
								$ljp09_array[$key] = $ljp09_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp09_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp09 params
					if (count($ljp09_array) > 0)
					{
						$ljp09_params = new Registry;
						$ljp09_params->loadArray($ljp09_array);
						$item->params->merge($ljp09_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp09_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp09, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
