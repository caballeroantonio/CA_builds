...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsc02.php';

class JtCaModelLsc02Form extends JtCaModelLsc02
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsc02';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsc02';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsc02', 'lsc02', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libreta de registro de asistencia (oficial).
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRETA DE REGISTRO DE ASISTENCIA (OFICIAL) item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsc02.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsc02.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsc02.id');
		$is_new		= true;
		...
		// Reorder the libreta de registro de asistencia (oficial) so the new libreta de registro de asistencia (oficial) is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsc02BeforeSave', array('com_jtca.lsc02', &$table, $is_new));
		...
		$dispatcher->trigger('onLsc02AfterSave', array('com_jtca.lsc02', $table, $is_new));

		$this->setState('lsc02.id', $table->id);
