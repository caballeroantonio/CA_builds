...
class JtCaModelLjemst06s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljemst06_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljemst06_id');
		$this->setState('filter.ljemst06_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljemst06_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljemst06_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljemst06_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljemst06_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljemst06_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljemst06_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljemst06_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljemst06_id'));
		$id .= ':'.$this->getState('filter.ljemst06_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljemst06s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljemst06s').' AS a');
		...
		// Filter by a single or group of ljemst06s.
		$ljemst06_id = $this->getState('filter.ljemst06_id');
		if ($ljemst06_id != '')
		{
			if (is_numeric($ljemst06_id))
			{
				$type = $this->getState('filter.ljemst06_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljemst06_id);
			}
			else
			{
				if (is_array($ljemst06_id))
				{
					JArrayHelper::toInteger($ljemst06_id);
					$ljemst06_id = implode(',', $ljemst06_id);
					$type = $this->getState('filter.ljemst06_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljemst06_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljemst06_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljemst06_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljemst06_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljemst06 params override menu item params only if menu param = 'use_ljemst06'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljemst06' and there is no ljemst06 param, use global

					// create an array of just the params set to 'use_ljemst06'
					$menu_params_array = $this->getState('params')->toArray();
					$ljemst06_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljemst06')
						{
							// if the ljemst06 has a value, use it
							if ($ljemst06_params->get($key) != '')
							{
								// get the value from the ljemst06
								$ljemst06_array[$key] = $ljemst06_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljemst06_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljemst06 params
					if (count($ljemst06_array) > 0)
					{
						$ljemst06_params = new Registry;
						$ljemst06_params->loadArray($ljemst06_array);
						$item->params->merge($ljemst06_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljemst06_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljemst06, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
