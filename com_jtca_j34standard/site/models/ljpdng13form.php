...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpdng13.php';

class JtcaModelLjpdng13Form extends JtcaModelLjpdng13
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpdng13';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpdng13';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpdng13', 'ljpdng13', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de control de multas.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE CONTROL DE MULTAS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpdng13.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpdng13.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpdng13.id');
		$is_new		= true;
		...
		// Reorder the libro de control de multas so the new libro de control de multas is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpdng13BeforeSave', array('com_jtca.ljpdng13', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpdng13AfterSave', array('com_jtca.ljpdng13', $table, $is_new));

		$this->setState('ljpdng13.id', $table->id);
