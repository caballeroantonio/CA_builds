...
/**
 * This models supports retrieving lists of libro de ordenes de aprehensiÃ³n, reaprehension y t.
 *
 */
class JtCaModelLcp23s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp23s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de ordenes de aprehensiÃ³n, reaprehension y traslado. has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp23_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp23_id');
		$this->setState('filter.lcp23_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp23_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp23_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp23_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp23_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp23_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp23_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp23_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp23_id'));
		$id .= ':'.$this->getState('filter.lcp23_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp23s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp23s').' AS a');
		...
		// Filter by a single or group of lcp23s.
		$lcp23_id = $this->getState('filter.lcp23_id');
		if ($lcp23_id != '')
		{
			if (is_numeric($lcp23_id))
			{
				$type = $this->getState('filter.lcp23_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp23_id);
			}
			else
			{
				if (is_array($lcp23_id))
				{
					JArrayHelper::toInteger($lcp23_id);
					$lcp23_id = implode(',', $lcp23_id);
					$type = $this->getState('filter.lcp23_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp23_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp23_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp23_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de ordenes de aprehensiÃ³n, reaprehension y t.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp23_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp23 params override menu item params only if menu param = 'use_lcp23'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp23' and there is no lcp23 param, use global

					// create an array of just the params set to 'use_lcp23'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp23')
						{
							// if the lcp23 has a value, use it
							if ($lcp23_params->get($key) != '')
							{
								// get the value from the lcp23
								$lcp23_array[$key] = $lcp23_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp23_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp23 params
					if (count($lcp23_array) > 0)
					{
						$lcp23_params = new Registry;
						$lcp23_params->loadArray($lcp23_array);
						$item->params->merge($lcp23_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp23_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp23, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
