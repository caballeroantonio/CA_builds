...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE BENEFICIOS REVOCADO Y O/A Model
 *
 */
class JtCaModelLspe04 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lspe04';
	...
	{
		if (empty($config['lspe04_filter_fields']))
		{
			$config['lspe04_filter_fields'] = array(
				'id', 'a.id',
				'id_organo','a.id_organo',
				'anoj','a.anoj',
				'id_expediente','a.id_expediente',
				'id_secretaria','a.id_secretaria',
				'field2308','a.field2308',
				'field2309','a.field2309',
				'field2310','a.field2310',
				'field2311','a.field2311',
				'field2312','a.field2312',
				'field2313','a.field2313',
				'field2314','a.field2314',
				'field2315','a.field2315',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lspe04s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE BENEFICIOS REVOCADO Y O/A data.
	 *
	 * @param	integer	$pk	The id of the libro de beneficios revocado y o/a.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lspe04.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPE04S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLspe04ChangeState', array('com_jtca.lspe04', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLspe04BeforeDelete', array('com_jtca.lspe04', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLspe04AfterDelete', array('com_jtca.lspe04', &$table));
				}
				else
				{
					// Prune items that you can't change.
