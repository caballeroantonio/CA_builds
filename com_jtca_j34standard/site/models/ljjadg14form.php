...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadg14.php';

class JtCaModelLjjadg14Form extends JtCaModelLjjadg14
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadg14';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadg14';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadg14', 'ljjadg14', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadg14.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadg14.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadg14.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjjadg14BeforeSave', array('com_jtca.ljjadg14', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadg14AfterSave', array('com_jtca.ljjadg14', $table, $is_new));

		$this->setState('ljjadg14.id', $table->id);
