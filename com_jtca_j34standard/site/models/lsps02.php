...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE GOBIERNO (NUEVO SISTEMA) (UNITARIO) Model
 *
 */
class JtCaModelLsps02 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsps02';
	...
	{
		if (empty($config['lsps02_filter_fields']))
		{
			$config['lsps02_filter_fields'] = array(
				'id', 'a.id',
				'id_expediente','a.id_expediente',
				'id_organo','a.id_organo',
				'id_secretaria','a.id_secretaria',
				'anoj','a.anoj',
				'field2359','a.field2359',
				'field2360','a.field2360',
				'field2361','a.field2361',
				'field2362','a.field2362',
	...
	*/
	public function getTable($type = 'Lsps02s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE GOBIERNO (NUEVO SISTEMA) (UNITARIO) data.
	 *
	 * @param	integer	$pk	The id of the libro de gobierno (nuevo sistema) (unitario).
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsps02.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPS02S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsps02ChangeState', array('com_jtca.lsps02', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsps02BeforeDelete', array('com_jtca.lsps02', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsps02AfterDelete', array('com_jtca.lsps02', &$table));
				}
				else
				{
					// Prune items that you can't change.
