...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpes05.php';

class JtcaModelLjpes05Form extends JtcaModelLjpes05
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpes05';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpes05';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpes05', 'ljpes05', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpes05.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpes05.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpes05.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onLjpes05BeforeSave', array('com_jtca.ljpes05', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpes05AfterSave', array('com_jtca.ljpes05', $table, $is_new));

		$this->setState('ljpes05.id', $table->id);
