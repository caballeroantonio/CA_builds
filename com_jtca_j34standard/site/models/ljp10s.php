...
class JtCaModelLjp10s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljp10_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp10_id');
		$this->setState('filter.ljp10_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp10_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp10_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp10_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp10_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp10_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp10_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp10_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp10_id'));
		$id .= ':'.$this->getState('filter.ljp10_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp10s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp10s').' AS a');
		...
		// Filter by a single or group of ljp10s.
		$ljp10_id = $this->getState('filter.ljp10_id');
		if ($ljp10_id != '')
		{
			if (is_numeric($ljp10_id))
			{
				$type = $this->getState('filter.ljp10_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp10_id);
			}
			else
			{
				if (is_array($ljp10_id))
				{
					JArrayHelper::toInteger($ljp10_id);
					$ljp10_id = implode(',', $ljp10_id);
					$type = $this->getState('filter.ljp10_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp10_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp10_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp10_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp10_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp10 params override menu item params only if menu param = 'use_ljp10'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp10' and there is no ljp10 param, use global

					// create an array of just the params set to 'use_ljp10'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp10')
						{
							// if the ljp10 has a value, use it
							if ($ljp10_params->get($key) != '')
							{
								// get the value from the ljp10
								$ljp10_array[$key] = $ljp10_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp10_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp10 params
					if (count($ljp10_array) > 0)
					{
						$ljp10_params = new Registry;
						$ljp10_params->loadArray($ljp10_array);
						$item->params->merge($ljp10_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp10_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp10, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
