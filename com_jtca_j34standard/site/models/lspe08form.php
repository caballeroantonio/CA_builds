...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lspe08.php';

class JtcaModelLspe08Form extends JtcaModelLspe08
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lspe08';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lspe08';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lspe08', 'lspe08', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de ingreso y egreso de billetes.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE INGRESO Y EGRESO DE BILLETES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lspe08.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lspe08.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lspe08.id');
		$is_new		= true;
		...
		// Reorder the libro de ingreso y egreso de billetes so the new libro de ingreso y egreso de billetes is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLspe08BeforeSave', array('com_jtca.lspe08', &$table, $is_new));
		...
		$dispatcher->trigger('onLspe08AfterSave', array('com_jtca.lspe08', $table, $is_new));

		$this->setState('lspe08.id', $table->id);
