...
class JtCaModelLjp16s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljp16_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp16_id');
		$this->setState('filter.ljp16_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp16_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp16_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp16_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp16_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp16_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp16_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp16_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp16_id'));
		$id .= ':'.$this->getState('filter.ljp16_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp16s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp16s').' AS a');
		...
		// Filter by a single or group of ljp16s.
		$ljp16_id = $this->getState('filter.ljp16_id');
		if ($ljp16_id != '')
		{
			if (is_numeric($ljp16_id))
			{
				$type = $this->getState('filter.ljp16_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp16_id);
			}
			else
			{
				if (is_array($ljp16_id))
				{
					JArrayHelper::toInteger($ljp16_id);
					$ljp16_id = implode(',', $ljp16_id);
					$type = $this->getState('filter.ljp16_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp16_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp16_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp16_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp16_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp16 params override menu item params only if menu param = 'use_ljp16'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp16' and there is no ljp16 param, use global

					// create an array of just the params set to 'use_ljp16'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp16')
						{
							// if the ljp16 has a value, use it
							if ($ljp16_params->get($key) != '')
							{
								// get the value from the ljp16
								$ljp16_array[$key] = $ljp16_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp16_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp16 params
					if (count($ljp16_array) > 0)
					{
						$ljp16_params = new Registry;
						$ljp16_params->loadArray($ljp16_array);
						$item->params->merge($ljp16_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp16_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp16, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
