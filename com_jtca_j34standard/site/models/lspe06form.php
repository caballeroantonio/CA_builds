...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lspe06.php';

class JtcaModelLspe06Form extends JtcaModelLspe06
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lspe06';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lspe06';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lspe06', 'lspe06', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de archivo.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE ARCHIVO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lspe06.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lspe06.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lspe06.id');
		$is_new		= true;
		...
		// Reorder the libro de archivo so the new libro de archivo is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLspe06BeforeSave', array('com_jtca.lspe06', &$table, $is_new));
		...
		$dispatcher->trigger('onLspe06AfterSave', array('com_jtca.lspe06', $table, $is_new));

		$this->setState('lspe06.id', $table->id);
