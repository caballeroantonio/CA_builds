...
class JtCaModelLjjadg11s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadg11_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg11_id');
		$this->setState('filter.ljjadg11_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg11_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg11_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg11_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg11_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg11_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg11_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg11_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg11_id'));
		$id .= ':'.$this->getState('filter.ljjadg11_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg11s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg11s').' AS a');
		...
		// Filter by a single or group of ljjadg11s.
		$ljjadg11_id = $this->getState('filter.ljjadg11_id');
		if ($ljjadg11_id != '')
		{
			if (is_numeric($ljjadg11_id))
			{
				$type = $this->getState('filter.ljjadg11_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg11_id);
			}
			else
			{
				if (is_array($ljjadg11_id))
				{
					JArrayHelper::toInteger($ljjadg11_id);
					$ljjadg11_id = implode(',', $ljjadg11_id);
					$type = $this->getState('filter.ljjadg11_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg11_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg11_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg11_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg11_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg11 params override menu item params only if menu param = 'use_ljjadg11'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg11' and there is no ljjadg11 param, use global

					// create an array of just the params set to 'use_ljjadg11'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg11_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg11')
						{
							// if the ljjadg11 has a value, use it
							if ($ljjadg11_params->get($key) != '')
							{
								// get the value from the ljjadg11
								$ljjadg11_array[$key] = $ljjadg11_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg11_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg11 params
					if (count($ljjadg11_array) > 0)
					{
						$ljjadg11_params = new Registry;
						$ljjadg11_params->loadArray($ljjadg11_array);
						$item->params->merge($ljjadg11_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg11_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg11, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
