...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE PROMOCIONES Model
 *
 */
class JtCaModelLjjadng02 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljjadng02';
	...
	{
		if (empty($config['ljjadng02_filter_fields']))
		{
			$config['ljjadng02_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Ljjadng02s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE PROMOCIONES data.
	 *
	 * @param	integer	$pk	The id of the libro de promociones.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljjadng02.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJJADNG02S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjjadng02ChangeState', array('com_jtca.ljjadng02', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjjadng02BeforeDelete', array('com_jtca.ljjadng02', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjjadng02AfterDelete', array('com_jtca.ljjadng02', &$table));
				}
				else
				{
					// Prune items that you can't change.
