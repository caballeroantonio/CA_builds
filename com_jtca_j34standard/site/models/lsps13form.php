...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lsps13.php';

class JtcaModelLsps13Form extends JtcaModelLsps13
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lsps13';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lsps13';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lsps13', 'lsps13', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the control de firmas del procesado.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	CONTROL DE FIRMAS DEL PROCESADO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lsps13.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lsps13.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lsps13.id');
		$is_new		= true;
		...
		// Reorder the control de firmas del procesado so the new control de firmas del procesado is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLsps13BeforeSave', array('com_jtca.lsps13', &$table, $is_new));
		...
		$dispatcher->trigger('onLsps13AfterSave', array('com_jtca.lsps13', $table, $is_new));

		$this->setState('lsps13.id', $table->id);
