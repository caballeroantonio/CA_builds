...
/**
 * This models supports retrieving lists of libro de desistimientos.
 *
 */
class JtCaModelLjpes16s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpes16s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de desistimientos has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpes16_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes16_id');
		$this->setState('filter.ljpes16_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes16_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes16_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes16_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes16_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes16_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes16_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes16_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes16_id'));
		$id .= ':'.$this->getState('filter.ljpes16_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes16s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes16s').' AS a');
		...
		// Filter by a single or group of ljpes16s.
		$ljpes16_id = $this->getState('filter.ljpes16_id');
		if ($ljpes16_id != '')
		{
			if (is_numeric($ljpes16_id))
			{
				$type = $this->getState('filter.ljpes16_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes16_id);
			}
			else
			{
				if (is_array($ljpes16_id))
				{
					JArrayHelper::toInteger($ljpes16_id);
					$ljpes16_id = implode(',', $ljpes16_id);
					$type = $this->getState('filter.ljpes16_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes16_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes16_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes16_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de desistimientos.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes16_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes16 params override menu item params only if menu param = 'use_ljpes16'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes16' and there is no ljpes16 param, use global

					// create an array of just the params set to 'use_ljpes16'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes16_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes16')
						{
							// if the ljpes16 has a value, use it
							if ($ljpes16_params->get($key) != '')
							{
								// get the value from the ljpes16
								$ljpes16_array[$key] = $ljpes16_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes16_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes16 params
					if (count($ljpes16_array) > 0)
					{
						$ljpes16_params = new Registry;
						$ljpes16_params->loadArray($ljpes16_array);
						$item->params->merge($ljpes16_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes16_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes16, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
