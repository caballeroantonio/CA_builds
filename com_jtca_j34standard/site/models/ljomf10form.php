...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljomf10.php';

class JtCaModelLjomf10Form extends JtCaModelLjomf10
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljomf10';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljomf10';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljomf10', 'ljomf10', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de auxiliares de la administración de justicia, tutores y curadores.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE AUXILIARES DE LA ADMINISTRACIÓN DE JUSTICIA, TUTORES Y CURADORES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljomf10.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljomf10.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljomf10.id');
		$is_new		= true;
		...
		// Reorder the libro de auxiliares de la administración de justic so the new libro de auxiliares de la administración de justicia, tutores y curadores is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjomf10BeforeSave', array('com_jtca.ljomf10', &$table, $is_new));
		...
		$dispatcher->trigger('onLjomf10AfterSave', array('com_jtca.ljomf10', $table, $is_new));

		$this->setState('ljomf10.id', $table->id);
