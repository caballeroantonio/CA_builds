...
class JtCaModelLsc13 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsc13';
	...
	{
		if (empty($config['lsc13_filter_fields']))
		{
			$config['lsc13_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	...
	/**
	 * Method to get PAPELETAS PARA EL PRÉSTAMO DE EXPEDIENTES data.
	 *
	 * @param	integer	$pk	The id of the papeletas para el préstamo de expedientes.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsc13.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSC13S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsc13ChangeState', array('com_jtca.lsc13', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsc13BeforeDelete', array('com_jtca.lsc13', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsc13AfterDelete', array('com_jtca.lsc13', &$table));
				}
				else
				{
					// Prune items that you can't change.
