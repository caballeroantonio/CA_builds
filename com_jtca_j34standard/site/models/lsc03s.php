...
class JtcaModelLsc03s extends JModelList
{
	 ...
	 */
	protected $context = 'com_jtca.lsc03s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de vistas al m.p (oficial) has been specified either as a parameter or in the url Request
		$pk = $params->get('lsc03_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lsc03_id');
		$this->setState('filter.lsc03_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lsc03_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lsc03_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lsc03_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lsc03_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lsc03_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lsc03_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lsc03_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lsc03_id'));
		$id .= ':'.$this->getState('filter.lsc03_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lsc03s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lsc03s').' AS a');
		...
		$lsc03_id = $this->getState('filter.lsc03_id');
		if ($lsc03_id != '')
		{
			if (is_numeric($lsc03_id))
			{
				$type = $this->getState('filter.lsc03_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lsc03_id);
			}
			else
			{
				if (is_array($lsc03_id))
				{
					JArrayHelper::toInteger($lsc03_id);
					$lsc03_id = implode(',', $lsc03_id);
					$type = $this->getState('filter.lsc03_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lsc03_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lsc03_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lsc03_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de vistas al m.p (oficial).
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lsc03_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lsc03 params override menu item params only if menu param = 'use_lsc03'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lsc03' and there is no lsc03 param, use global

					// create an array of just the params set to 'use_lsc03'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lsc03')
						{
							// if the lsc03 has a value, use it
							if ($lsc03_params->get($key) != '')
							{
								// get the value from the lsc03
								$lsc03_array[$key] = $lsc03_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lsc03_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lsc03 params
					if (count($lsc03_array) > 0)
					{
						$lsc03_params = new Registry;
						$lsc03_params->loadArray($lsc03_array);
						$item->params->merge($lsc03_params);
					}


					// get display date
					switch ($item->params->get('list_show_lsc03_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lsc03, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
