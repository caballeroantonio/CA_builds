...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljjadng10.php';

class JtcaModelLjjadng10Form extends JtcaModelLjjadng10
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljjadng10';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljjadng10';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljjadng10', 'ljjadng10', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de control de pedimentos.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE CONTROL DE PEDIMENTOS item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljjadng10.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljjadng10.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljjadng10.id');
		$is_new		= true;
		...
		// Reorder the libro de control de pedimentos so the new libro de control de pedimentos is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjjadng10BeforeSave', array('com_jtca.ljjadng10', &$table, $is_new));
		...
		$dispatcher->trigger('onLjjadng10AfterSave', array('com_jtca.ljjadng10', $table, $is_new));

		$this->setState('ljjadng10.id', $table->id);
