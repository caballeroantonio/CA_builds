...
class JtCaModelLjoc13 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljoc13';
	...
	{
		if (empty($config['ljoc13_filter_fields']))
		{
	...
	*/
	...
	/**
	 * Method to get LIBRO DE NOTARIOS data.
	 *
	 * @param	integer	$pk	The id of the libro de notarios.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljoc13.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJOC13S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjoc13ChangeState', array('com_jtca.ljoc13', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjoc13BeforeDelete', array('com_jtca.ljoc13', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjoc13AfterDelete', array('com_jtca.ljoc13', &$table));
				}
				else
				{
					// Prune items that you can't change.
