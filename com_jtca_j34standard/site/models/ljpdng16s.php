...
/**
 * This models supports retrieving lists of libro de registro en que se haya obsequiado o nega.
 *
 */
class JtcaModelLjpdng16s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljpdng16s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de registro en que se haya obsequiado o negado la orden de aprehensión, comparecencia, presentación, artículo 36 del código de procedimientos penales y otros has been specified either as a parameter or in the url Request
		$pk = $params->get('ljpdng16_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpdng16_id');
		$this->setState('filter.ljpdng16_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpdng16_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpdng16_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpdng16_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpdng16_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpdng16_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpdng16_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpdng16_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpdng16_id'));
		$id .= ':'.$this->getState('filter.ljpdng16_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpdng16s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpdng16s').' AS a');
		...
		// Filter by a single or group of ljpdng16s.
		$ljpdng16_id = $this->getState('filter.ljpdng16_id');
		if ($ljpdng16_id != '')
		{
			if (is_numeric($ljpdng16_id))
			{
				$type = $this->getState('filter.ljpdng16_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpdng16_id);
			}
			else
			{
				if (is_array($ljpdng16_id))
				{
					JArrayHelper::toInteger($ljpdng16_id);
					$ljpdng16_id = implode(',', $ljpdng16_id);
					$type = $this->getState('filter.ljpdng16_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpdng16_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpdng16_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpdng16_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de registro en que se haya obsequiado o nega.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpdng16_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpdng16 params override menu item params only if menu param = 'use_ljpdng16'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpdng16' and there is no ljpdng16 param, use global

					// create an array of just the params set to 'use_ljpdng16'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpdng16_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpdng16')
						{
							// if the ljpdng16 has a value, use it
							if ($ljpdng16_params->get($key) != '')
							{
								// get the value from the ljpdng16
								$ljpdng16_array[$key] = $ljpdng16_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpdng16_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpdng16 params
					if (count($ljpdng16_array) > 0)
					{
						$ljpdng16_params = new Registry;
						$ljpdng16_params->loadArray($ljpdng16_array);
						$item->params->merge($ljpdng16_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpdng16_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpdng16, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
