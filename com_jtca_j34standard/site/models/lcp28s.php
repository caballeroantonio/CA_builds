...
/**
 * This models supports retrieving lists of libro de turno de excusas para juzgados  de delito.
 *
 */
class JtCaModelLcp28s extends JModelList
{
	/**
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno de excusas para juzgados  de delitos no graves has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp28_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp28_id');
		$this->setState('filter.lcp28_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp28_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp28_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp28_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp28_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp28_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp28_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp28_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp28_id'));
		$id .= ':'.$this->getState('filter.lcp28_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp28s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp28s').' AS a');
		...
		// Filter by a single or group of lcp28s.
		$lcp28_id = $this->getState('filter.lcp28_id');
		if ($lcp28_id != '')
		{
			if (is_numeric($lcp28_id))
			{
				$type = $this->getState('filter.lcp28_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp28_id);
			}
			else
			{
				if (is_array($lcp28_id))
				{
					JArrayHelper::toInteger($lcp28_id);
					$lcp28_id = implode(',', $lcp28_id);
					$type = $this->getState('filter.lcp28_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp28_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp28_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp28_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno de excusas para juzgados  de delito.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp28_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp28 params override menu item params only if menu param = 'use_lcp28'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp28' and there is no lcp28 param, use global

					// create an array of just the params set to 'use_lcp28'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp28')
						{
							// if the lcp28 has a value, use it
							if ($lcp28_params->get($key) != '')
							{
								// get the value from the lcp28
								$lcp28_array[$key] = $lcp28_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp28_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp28 params
					if (count($lcp28_array) > 0)
					{
						$lcp28_params = new Registry;
						$lcp28_params->loadArray($lcp28_array);
						$item->params->merge($lcp28_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp28_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp28, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
