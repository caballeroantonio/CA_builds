...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljomf22.php';

class JtCaModelLjomf22Form extends JtCaModelLjomf22
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljomf22';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljomf22';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljomf22', 'ljomf22', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de control de restituciones internacionales.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE CONTROL DE RESTITUCIONES INTERNACIONALES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljomf22.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljomf22.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljomf22.id');
		$is_new		= true;
		...
		// Reorder the libro de control de restituciones internacionales so the new libro de control de restituciones internacionales is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjomf22BeforeSave', array('com_jtca.ljomf22', &$table, $is_new));
		...
		$dispatcher->trigger('onLjomf22AfterSave', array('com_jtca.ljomf22', $table, $is_new));

		$this->setState('ljomf22.id', $table->id);
