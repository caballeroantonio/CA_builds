...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljccm17.php';

class JtCaModelLjccm17Form extends JtCaModelLjccm17
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljccm17';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljccm17';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljccm17', 'ljccm17', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	public function getItem($item_id = null, $recursive = false)
	{
		...
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljccm17.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljccm17.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljccm17.id');
		$is_new		= true;
		...
		// Reorder the libro de remisión de documentos al archivo so the new libro de remisión de documentos al archivo is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjccm17BeforeSave', array('com_jtca.ljccm17', &$table, $is_new));
		...
		$dispatcher->trigger('onLjccm17AfterSave', array('com_jtca.ljccm17', $table, $is_new));

		$this->setState('ljccm17.id', $table->id);
