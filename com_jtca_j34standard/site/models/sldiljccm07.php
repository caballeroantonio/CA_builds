...
/**
 * TSJ CDMX Libros TxCA Component Sub Libro DILIGENCIA Del LIBRO DE EXHORTOS Model
 *
 */
class JtcaModelSldilJccm07 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.sldiljccm07';
	...
	{
		if (empty($config['sldiljccm07_filter_fields']))
		{
			$config['sldiljccm07_filter_fields'] = array(
				'id', 'a.id',
	...
	protected function populateState()
	{
		...
		$this->setState('sldiljccm07.id', $pk);
	...
	*/
	public function getTable($type = 'SldilJccm07s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get Sub Libro DILIGENCIA Del LIBRO DE EXHORTOS data.
	 *
	 * @param	integer	$pk	The id of the sub libro diligencia del libro de exhortos.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('sldiljccm07.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_SLDIL_JCCM07S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onSldilJccm07ChangeState', array('com_jtca.sldiljccm07', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onSldilJccm07BeforeDelete', array('com_jtca.sldiljccm07', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onSldilJccm07AfterDelete', array('com_jtca.sldiljccm07', &$table));
				}
				else
				{
					// Prune items that you can't change.
