...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljp20.php';

class JtCaModelLjp20Form extends JtCaModelLjp20
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljp20';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljp20';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljp20', 'ljp20', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno para conclusiones.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO PARA CONCLUSIONES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljp20.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljp20.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljp20.id');
		$is_new		= true;
		...
		// Reorder the libro de turno para conclusiones so the new libro de turno para conclusiones is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjp20BeforeSave', array('com_jtca.ljp20', &$table, $is_new));
		...
		$dispatcher->trigger('onLjp20AfterSave', array('com_jtca.ljp20', $table, $is_new));

		$this->setState('ljp20.id', $table->id);
