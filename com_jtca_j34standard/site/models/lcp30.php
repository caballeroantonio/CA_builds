...
/**
 * TSJ CDMX Libros TxCA Component LIBRETA DE LIBERTADES JUZGADOS DE PRIMERA INSTANCIA Model
 *
 */
class JtCaModelLcp30 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lcp30';
	...
	{
		if (empty($config['lcp30_filter_fields']))
		{
			$config['lcp30_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lcp30s', $prefix = 'JtCaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRETA DE LIBERTADES JUZGADOS DE PRIMERA INSTANCIA data.
	 *
	 * @param	integer	$pk	The id of the libreta de libertades juzgados de primera instancia.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lcp30.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LCP30S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLcp30ChangeState', array('com_jtca.lcp30', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLcp30BeforeDelete', array('com_jtca.lcp30', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLcp30AfterDelete', array('com_jtca.lcp30', &$table));
				}
				else
				{
					// Prune items that you can't change.
