...
class JtCaModelLjp17s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljp17_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp17_id');
		$this->setState('filter.ljp17_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp17_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp17_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp17_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp17_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp17_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp17_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp17_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp17_id'));
		$id .= ':'.$this->getState('filter.ljp17_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp17s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp17s').' AS a');
		...
		// Filter by a single or group of ljp17s.
		$ljp17_id = $this->getState('filter.ljp17_id');
		if ($ljp17_id != '')
		{
			if (is_numeric($ljp17_id))
			{
				$type = $this->getState('filter.ljp17_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp17_id);
			}
			else
			{
				if (is_array($ljp17_id))
				{
					JArrayHelper::toInteger($ljp17_id);
					$ljp17_id = implode(',', $ljp17_id);
					$type = $this->getState('filter.ljp17_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp17_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp17_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp17_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp17_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp17 params override menu item params only if menu param = 'use_ljp17'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp17' and there is no ljp17 param, use global

					// create an array of just the params set to 'use_ljp17'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp17')
						{
							// if the ljp17 has a value, use it
							if ($ljp17_params->get($key) != '')
							{
								// get the value from the ljp17
								$ljp17_array[$key] = $ljp17_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp17_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp17 params
					if (count($ljp17_array) > 0)
					{
						$ljp17_params = new Registry;
						$ljp17_params->loadArray($ljp17_array);
						$item->params->merge($ljp17_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp17_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp17, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
