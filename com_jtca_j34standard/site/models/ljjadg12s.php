...
class JtCaModelLjjadg12s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljjadg12_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadg12_id');
		$this->setState('filter.ljjadg12_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadg12_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadg12_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadg12_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadg12_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadg12_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadg12_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadg12_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadg12_id'));
		$id .= ':'.$this->getState('filter.ljjadg12_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadg12s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadg12s').' AS a');
		...
		// Filter by a single or group of ljjadg12s.
		$ljjadg12_id = $this->getState('filter.ljjadg12_id');
		if ($ljjadg12_id != '')
		{
			if (is_numeric($ljjadg12_id))
			{
				$type = $this->getState('filter.ljjadg12_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadg12_id);
			}
			else
			{
				if (is_array($ljjadg12_id))
				{
					JArrayHelper::toInteger($ljjadg12_id);
					$ljjadg12_id = implode(',', $ljjadg12_id);
					$type = $this->getState('filter.ljjadg12_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadg12_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadg12_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadg12_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadg12_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadg12 params override menu item params only if menu param = 'use_ljjadg12'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadg12' and there is no ljjadg12 param, use global

					// create an array of just the params set to 'use_ljjadg12'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadg12_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadg12')
						{
							// if the ljjadg12 has a value, use it
							if ($ljjadg12_params->get($key) != '')
							{
								// get the value from the ljjadg12
								$ljjadg12_array[$key] = $ljjadg12_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadg12_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadg12 params
					if (count($ljjadg12_array) > 0)
					{
						$ljjadg12_params = new Registry;
						$ljjadg12_params->loadArray($ljjadg12_array);
						$item->params->merge($ljjadg12_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadg12_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadg12, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
