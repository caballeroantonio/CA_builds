...
class JtCaModelLjccm03s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de egresos de valores has been specified either as a parameter or in the url Request
		$pk = $params->get('ljccm03_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljccm03_id');
		$this->setState('filter.ljccm03_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljccm03_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljccm03_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljccm03_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljccm03_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljccm03_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljccm03_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljccm03_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljccm03_id'));
		$id .= ':'.$this->getState('filter.ljccm03_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljccm03s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljccm03s').' AS a');
		...
		// Filter by a single or group of ljccm03s.
		$ljccm03_id = $this->getState('filter.ljccm03_id');
		if ($ljccm03_id != '')
		{
			if (is_numeric($ljccm03_id))
			{
				$type = $this->getState('filter.ljccm03_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljccm03_id);
			}
			else
			{
				if (is_array($ljccm03_id))
				{
					JArrayHelper::toInteger($ljccm03_id);
					$ljccm03_id = implode(',', $ljccm03_id);
					$type = $this->getState('filter.ljccm03_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljccm03_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljccm03_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljccm03_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de egresos de valores.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljccm03_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljccm03 params override menu item params only if menu param = 'use_ljccm03'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljccm03' and there is no ljccm03 param, use global

					// create an array of just the params set to 'use_ljccm03'
					$menu_params_array = $this->getState('params')->toArray();
					$ljccm03_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljccm03')
						{
							// if the ljccm03 has a value, use it
							if ($ljccm03_params->get($key) != '')
							{
								// get the value from the ljccm03
								$ljccm03_array[$key] = $ljccm03_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljccm03_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljccm03 params
					if (count($ljccm03_array) > 0)
					{
						$ljccm03_params = new Registry;
						$ljccm03_params->loadArray($ljccm03_array);
						$item->params->merge($ljccm03_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljccm03_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljccm03, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
