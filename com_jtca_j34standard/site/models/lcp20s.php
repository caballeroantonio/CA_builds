...
/**
 * This models supports retrieving lists of libro de turno para incompetencias primera instanc.
 *
 */
class JtCaModelLcp20s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp20s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de turno para incompetencias primera instancia. has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp20_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp20_id');
		$this->setState('filter.lcp20_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp20_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp20_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp20_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp20_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp20_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp20_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp20_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp20_id'));
		$id .= ':'.$this->getState('filter.lcp20_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp20s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp20s').' AS a');
		...
		// Filter by a single or group of lcp20s.
		$lcp20_id = $this->getState('filter.lcp20_id');
		if ($lcp20_id != '')
		{
			if (is_numeric($lcp20_id))
			{
				$type = $this->getState('filter.lcp20_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp20_id);
			}
			else
			{
				if (is_array($lcp20_id))
				{
					JArrayHelper::toInteger($lcp20_id);
					$lcp20_id = implode(',', $lcp20_id);
					$type = $this->getState('filter.lcp20_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp20_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp20_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp20_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de turno para incompetencias primera instanc.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp20_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp20 params override menu item params only if menu param = 'use_lcp20'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp20' and there is no lcp20 param, use global

					// create an array of just the params set to 'use_lcp20'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp20')
						{
							// if the lcp20 has a value, use it
							if ($lcp20_params->get($key) != '')
							{
								// get the value from the lcp20
								$lcp20_array[$key] = $lcp20_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp20_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp20 params
					if (count($lcp20_array) > 0)
					{
						$lcp20_params = new Registry;
						$lcp20_params->loadArray($lcp20_array);
						$item->params->merge($lcp20_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp20_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp20, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
