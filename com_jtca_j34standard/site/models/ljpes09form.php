...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/ljpes09.php';

class JtCaModelLjpes09Form extends JtCaModelLjpes09
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.ljpes09';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.ljpes09';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.ljpes09', 'ljpes09', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de multas judiciales.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE MULTAS JUDICIALES item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('ljpes09.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.ljpes09.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ljpes09.id');
		$is_new		= true;
		...
		// Reorder the libro de multas judiciales so the new libro de multas judiciales is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLjpes09BeforeSave', array('com_jtca.ljpes09', &$table, $is_new));
		...
		$dispatcher->trigger('onLjpes09AfterSave', array('com_jtca.ljpes09', $table, $is_new));

		$this->setState('ljpes09.id', $table->id);
