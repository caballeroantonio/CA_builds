...
/**
 * This models supports retrieving lists of jc19.
 *
 */
class JtCaModelLjc19s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljc19s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de envÃ­o de expedientes al archivo judicial para su destrucciÃ³n has been specified either as a parameter or in the url Request
		$pk = $params->get('ljc19_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc19_id');
		$this->setState('filter.ljc19_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc19_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc19_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc19_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc19_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc19_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljc19_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljc19_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc19_id'));
		$id .= ':'.$this->getState('filter.ljc19_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc19s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljc19s').' AS a');
		...
		// Filter by a single or group of ljc19s.
		$ljc19_id = $this->getState('filter.ljc19_id');
		if ($ljc19_id != '')
		{
			if (is_numeric($ljc19_id))
			{
				$type = $this->getState('filter.ljc19_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc19_id);
			}
			else
			{
				if (is_array($ljc19_id))
				{
					JArrayHelper::toInteger($ljc19_id);
					$ljc19_id = implode(',', $ljc19_id);
					$type = $this->getState('filter.ljc19_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc19_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc19_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc19_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljc19_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljc19 params override menu item params only if menu param = 'use_ljc19'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljc19' and there is no ljc19 param, use global

					// create an array of just the params set to 'use_ljc19'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljc19')
						{
							// if the ljc19 has a value, use it
							if ($ljc19_params->get($key) != '')
							{
								// get the value from the ljc19
								$ljc19_array[$key] = $ljc19_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljc19_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljc19 params
					if (count($ljc19_array) > 0)
					{
						$ljc19_params = new Registry;
						$ljc19_params->loadArray($ljc19_array);
						$item->params->merge($ljc19_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljc19_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc19, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
