...
/**
 * This models supports retrieving lists of libro de recursos de apelaciÃ³n.
 *
 */
class JtCaModelLjjadng05s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljjadng05s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libro de recursos de apelaciÃ³n has been specified either as a parameter or in the url Request
		$pk = $params->get('ljjadng05_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljjadng05_id');
		$this->setState('filter.ljjadng05_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljjadng05_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljjadng05_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljjadng05_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljjadng05_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljjadng05_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljjadng05_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljjadng05_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljjadng05_id'));
		$id .= ':'.$this->getState('filter.ljjadng05_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljjadng05s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljjadng05s').' AS a');
		...
		// Filter by a single or group of ljjadng05s.
		$ljjadng05_id = $this->getState('filter.ljjadng05_id');
		if ($ljjadng05_id != '')
		{
			if (is_numeric($ljjadng05_id))
			{
				$type = $this->getState('filter.ljjadng05_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljjadng05_id);
			}
			else
			{
				if (is_array($ljjadng05_id))
				{
					JArrayHelper::toInteger($ljjadng05_id);
					$ljjadng05_id = implode(',', $ljjadng05_id);
					$type = $this->getState('filter.ljjadng05_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljjadng05_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljjadng05_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljjadng05_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libro de recursos de apelaciÃ³n.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljjadng05_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljjadng05 params override menu item params only if menu param = 'use_ljjadng05'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljjadng05' and there is no ljjadng05 param, use global

					// create an array of just the params set to 'use_ljjadng05'
					$menu_params_array = $this->getState('params')->toArray();
					$ljjadng05_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljjadng05')
						{
							// if the ljjadng05 has a value, use it
							if ($ljjadng05_params->get($key) != '')
							{
								// get the value from the ljjadng05
								$ljjadng05_array[$key] = $ljjadng05_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljjadng05_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljjadng05 params
					if (count($ljjadng05_array) > 0)
					{
						$ljjadng05_params = new Registry;
						$ljjadng05_params->loadArray($ljjadng05_array);
						$item->params->merge($ljjadng05_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljjadng05_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljjadng05, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
