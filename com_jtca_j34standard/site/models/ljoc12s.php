...
class JtCaModelLjoc12s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljoc12_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljoc12_id');
		$this->setState('filter.ljoc12_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljoc12_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljoc12_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljoc12_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljoc12_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljoc12_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljoc12_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljoc12_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljoc12_id'));
		$id .= ':'.$this->getState('filter.ljoc12_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljoc12s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljoc12s').' AS a');
		...
		// Filter by a single or group of ljoc12s.
		$ljoc12_id = $this->getState('filter.ljoc12_id');
		if ($ljoc12_id != '')
		{
			if (is_numeric($ljoc12_id))
			{
				$type = $this->getState('filter.ljoc12_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljoc12_id);
			}
			else
			{
				if (is_array($ljoc12_id))
				{
					JArrayHelper::toInteger($ljoc12_id);
					$ljoc12_id = implode(',', $ljoc12_id);
					$type = $this->getState('filter.ljoc12_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljoc12_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljoc12_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljoc12_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljoc12_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljoc12 params override menu item params only if menu param = 'use_ljoc12'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljoc12' and there is no ljoc12 param, use global

					// create an array of just the params set to 'use_ljoc12'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljoc12')
						{
							// if the ljoc12 has a value, use it
							if ($ljoc12_params->get($key) != '')
							{
								// get the value from the ljoc12
								$ljoc12_array[$key] = $ljoc12_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljoc12_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljoc12 params
					if (count($ljoc12_array) > 0)
					{
						$ljoc12_params = new Registry;
						$ljoc12_params->loadArray($ljoc12_array);
						$item->params->merge($ljoc12_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljoc12_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljoc12, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
