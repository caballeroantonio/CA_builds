...
/**
 * This models supports retrieving lists of libreta de cumplimiento de ejecutorias para juzgad.
 *
 */
class JtCaModelLcp14s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.lcp14s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single libreta de cumplimiento de ejecutorias para juzgados de ejecuciÃ³n. has been specified either as a parameter or in the url Request
		$pk = $params->get('lcp14_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lcp14_id');
		$this->setState('filter.lcp14_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lcp14_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lcp14_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lcp14_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lcp14_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lcp14_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lcp14_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lcp14_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lcp14_id'));
		$id .= ':'.$this->getState('filter.lcp14_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lcp14s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lcp14s').' AS a');
		...
		// Filter by a single or group of lcp14s.
		$lcp14_id = $this->getState('filter.lcp14_id');
		if ($lcp14_id != '')
		{
			if (is_numeric($lcp14_id))
			{
				$type = $this->getState('filter.lcp14_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lcp14_id);
			}
			else
			{
				if (is_array($lcp14_id))
				{
					JArrayHelper::toInteger($lcp14_id);
					$lcp14_id = implode(',', $lcp14_id);
					$type = $this->getState('filter.lcp14_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lcp14_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lcp14_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lcp14_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of libreta de cumplimiento de ejecutorias para juzgad.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lcp14_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lcp14 params override menu item params only if menu param = 'use_lcp14'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lcp14' and there is no lcp14 param, use global

					// create an array of just the params set to 'use_lcp14'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lcp14')
						{
							// if the lcp14 has a value, use it
							if ($lcp14_params->get($key) != '')
							{
								// get the value from the lcp14
								$lcp14_array[$key] = $lcp14_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lcp14_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lcp14 params
					if (count($lcp14_array) > 0)
					{
						$lcp14_params = new Registry;
						$lcp14_params->loadArray($lcp14_array);
						$item->params->merge($lcp14_params);
					}


					// get display date
					switch ($item->params->get('list_show_lcp14_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lcp14, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
