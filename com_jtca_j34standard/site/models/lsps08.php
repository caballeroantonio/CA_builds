...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE TURNO DE ENTREGAS DE TOCAS Model
 *
 */
class JtcaModelLsps08 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsps08';
	...
	{
		if (empty($config['lsps08_filter_fields']))
		{
			$config['lsps08_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Lsps08s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE TURNO DE ENTREGAS DE TOCAS data.
	 *
	 * @param	integer	$pk	The id of the libro de turno de entregas de tocas.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsps08.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPS08S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsps08ChangeState', array('com_jtca.lsps08', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsps08BeforeDelete', array('com_jtca.lsps08', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsps08AfterDelete', array('com_jtca.lsps08', &$table));
				}
				else
				{
					// Prune items that you can't change.
