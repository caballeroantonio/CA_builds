...
/**
 * TSJ CDMX Libros TxCA Component LIBRO DE INGRESOS Model
 *
 */
class JtcaModelLjjadg03 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljjadg03';
	...
	{
		if (empty($config['ljjadg03_filter_fields']))
		{
			$config['ljjadg03_filter_fields'] = array(
				'id', 'a.id',
				'billete', 'a.billete',
	...
	*/
	public function getTable($type = 'Ljjadg03s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBRO DE INGRESOS data.
	 *
	 * @param	integer	$pk	The id of the libro de ingresos.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljjadg03.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJJADG03S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjjadg03ChangeState', array('com_jtca.ljjadg03', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjjadg03BeforeDelete', array('com_jtca.ljjadg03', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjjadg03AfterDelete', array('com_jtca.ljjadg03', &$table));
				}
				else
				{
					// Prune items that you can't change.
