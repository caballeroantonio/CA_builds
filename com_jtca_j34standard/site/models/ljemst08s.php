...
class JtCaModelLjemst08s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljemst08_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljemst08_id');
		$this->setState('filter.ljemst08_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljemst08_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljemst08_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljemst08_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljemst08_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljemst08_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljemst08_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljemst08_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljemst08_id'));
		$id .= ':'.$this->getState('filter.ljemst08_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljemst08s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljemst08s').' AS a');
		...
		// Filter by a single or group of ljemst08s.
		$ljemst08_id = $this->getState('filter.ljemst08_id');
		if ($ljemst08_id != '')
		{
			if (is_numeric($ljemst08_id))
			{
				$type = $this->getState('filter.ljemst08_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljemst08_id);
			}
			else
			{
				if (is_array($ljemst08_id))
				{
					JArrayHelper::toInteger($ljemst08_id);
					$ljemst08_id = implode(',', $ljemst08_id);
					$type = $this->getState('filter.ljemst08_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljemst08_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljemst08_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljemst08_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljemst08_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljemst08 params override menu item params only if menu param = 'use_ljemst08'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljemst08' and there is no ljemst08 param, use global

					// create an array of just the params set to 'use_ljemst08'
					$menu_params_array = $this->getState('params')->toArray();
					$ljemst08_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljemst08')
						{
							// if the ljemst08 has a value, use it
							if ($ljemst08_params->get($key) != '')
							{
								// get the value from the ljemst08
								$ljemst08_array[$key] = $ljemst08_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljemst08_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljemst08 params
					if (count($ljemst08_array) > 0)
					{
						$ljemst08_params = new Registry;
						$ljemst08_params->loadArray($ljemst08_array);
						$item->params->merge($ljemst08_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljemst08_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljemst08, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
