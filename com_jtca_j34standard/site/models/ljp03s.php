...
class JtCaModelLjp03s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljp03_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljp03_id');
		$this->setState('filter.ljp03_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljp03_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljp03_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljp03_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljp03_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljp03_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljp03_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljp03_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljp03_id'));
		$id .= ':'.$this->getState('filter.ljp03_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljp03s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljp03s').' AS a');
		...
		// Filter by a single or group of ljp03s.
		$ljp03_id = $this->getState('filter.ljp03_id');
		if ($ljp03_id != '')
		{
			if (is_numeric($ljp03_id))
			{
				$type = $this->getState('filter.ljp03_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljp03_id);
			}
			else
			{
				if (is_array($ljp03_id))
				{
					JArrayHelper::toInteger($ljp03_id);
					$ljp03_id = implode(',', $ljp03_id);
					$type = $this->getState('filter.ljp03_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljp03_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljp03_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljp03_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljp03_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljp03 params override menu item params only if menu param = 'use_ljp03'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljp03' and there is no ljp03 param, use global

					// create an array of just the params set to 'use_ljp03'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljp03')
						{
							// if the ljp03 has a value, use it
							if ($ljp03_params->get($key) != '')
							{
								// get the value from the ljp03
								$ljp03_array[$key] = $ljp03_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljp03_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljp03 params
					if (count($ljp03_array) > 0)
					{
						$ljp03_params = new Registry;
						$ljp03_params->loadArray($ljp03_array);
						$item->params->merge($ljp03_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljp03_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljp03, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
