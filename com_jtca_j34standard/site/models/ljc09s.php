...
/**
 * This models supports retrieving lists of jc09.
 *
 */
class JtCaModelLjc09s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.ljc09s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljc09_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljc09_id');
		$this->setState('filter.ljc09_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljc09_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljc09_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljc09_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljc09_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljc09_initial_direction','ASC');
		}
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljc09_id'));
		$id .= ':'.$this->getState('filter.ljc09_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljc09s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		// Filter by a single or group of ljc09s.
		$ljc09_id = $this->getState('filter.ljc09_id');
		if ($ljc09_id != '')
		{
			if (is_numeric($ljc09_id))
			{
				$type = $this->getState('filter.ljc09_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljc09_id);
			}
			else
			{
				if (is_array($ljc09_id))
				{
					JArrayHelper::toInteger($ljc09_id);
					$ljc09_id = implode(',', $ljc09_id);
					$type = $this->getState('filter.ljc09_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljc09_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljc09_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljc09_filter_field'))
			{
				case 'created_by':
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				else
				{
					...
					// get display date
					switch ($item->params->get('list_show_ljc09_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljc09, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
