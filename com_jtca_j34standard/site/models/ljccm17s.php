...
class JtCaModelLjccm17s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljccm17_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljccm17_id');
		$this->setState('filter.ljccm17_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljccm17_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljccm17_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljccm17_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljccm17_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljccm17_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljccm17_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljccm17_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljccm17_id'));
		$id .= ':'.$this->getState('filter.ljccm17_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljccm17s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljccm17s').' AS a');
		...
		// Filter by a single or group of ljccm17s.
		$ljccm17_id = $this->getState('filter.ljccm17_id');
		if ($ljccm17_id != '')
		{
			if (is_numeric($ljccm17_id))
			{
				$type = $this->getState('filter.ljccm17_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljccm17_id);
			}
			else
			{
				if (is_array($ljccm17_id))
				{
					JArrayHelper::toInteger($ljccm17_id);
					$ljccm17_id = implode(',', $ljccm17_id);
					$type = $this->getState('filter.ljccm17_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljccm17_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljccm17_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljccm17_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljccm17_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljccm17 params override menu item params only if menu param = 'use_ljccm17'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljccm17' and there is no ljccm17 param, use global

					// create an array of just the params set to 'use_ljccm17'
					$menu_params_array = $this->getState('params')->toArray();
					$ljccm17_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljccm17')
						{
							// if the ljccm17 has a value, use it
							if ($ljccm17_params->get($key) != '')
							{
								// get the value from the ljccm17
								$ljccm17_array[$key] = $ljccm17_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljccm17_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljccm17 params
					if (count($ljccm17_array) > 0)
					{
						$ljccm17_params = new Registry;
						$ljccm17_params->loadArray($ljccm17_array);
						$item->params->merge($ljccm17_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljccm17_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljccm17, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
