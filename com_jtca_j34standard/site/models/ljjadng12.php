...
/**
 * TSJ CDMX Libros TxCA Component LIBROS DE VALORES Model
 *
 */
class JtcaModelLjjadng12 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljjadng12';
	...
	{
		if (empty($config['ljjadng12_filter_fields']))
		{
	...
	*/
	public function getTable($type = 'Ljjadng12s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get LIBROS DE VALORES data.
	 *
	 * @param	integer	$pk	The id of the libros de valores.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljjadng12.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJJADNG12S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjjadng12ChangeState', array('com_jtca.ljjadng12', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjjadng12BeforeDelete', array('com_jtca.ljjadng12', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjjadng12AfterDelete', array('com_jtca.ljjadng12', &$table));
				}
				else
				{
					// Prune items that you can't change.
