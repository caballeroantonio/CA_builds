...
/**
 * This models supports retrieving lists of diligencia del libro de exhortos.
 *
 */
class JtcaModelSldilJccm07s extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_jtca.sldil_jccm07s';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single sub libro diligencia del libro de exhortos has been specified either as a parameter or in the url Request
		$pk = $params->get('sldil_jccm07_id', '') == '' ? $app->input->getInt('id', '') : $params->get('sldil_jccm07_id');
		$this->setState('filter.sldil_jccm07_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('sldiljccm07_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('sldiljccm07_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('sldiljccm07_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('sldiljccm07_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('sldiljccm07_initial_direction','ASC');
		}
		...
		if ($params->get('filter_sldiljccm07_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_sldiljccm07_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.sldil_jccm07_id'));
		$id .= ':'.$this->getState('filter.sldil_jccm07_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of sldil_jccm07s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_sldil_jccm07s').' AS a');
		...
		// Filter by a single or group of sldil_jccm07s.
		$sldil_jccm07_id = $this->getState('filter.sldil_jccm07_id');
		if ($sldil_jccm07_id != '')
		{
			if (is_numeric($sldil_jccm07_id))
			{
				$type = $this->getState('filter.sldil_jccm07_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $sldil_jccm07_id);
			}
			else
			{
				if (is_array($sldil_jccm07_id))
				{
					JArrayHelper::toInteger($sldil_jccm07_id);
					$sldil_jccm07_id = implode(',', $sldil_jccm07_id);
					$type = $this->getState('filter.sldil_jccm07_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$sldil_jccm07_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_sldiljccm07_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_sldiljccm07_filter_field'))
			{
				case 'created_by':
	...
	/**
	 * Method to get a list of diligencia del libro de exhortos.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				$sldil_jccm07_params = new Registry;
				...
				{
					$item->params = $sldil_jccm07_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// SldilJccm07 params override menu item params only if menu param = 'use_sldiljccm07'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_sldiljccm07' and there is no sldiljccm07 param, use global

					// create an array of just the params set to 'use_sldiljccm07'
					$menu_params_array = $this->getState('params')->toArray();
					$sldil_jccm07_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_sldiljccm07')
						{
							// if the sldiljccm07 has a value, use it
							if ($sldil_jccm07_params->get($key) != '')
							{
								// get the value from the sldiljccm07
								$sldil_jccm07_array[$key] = $sldil_jccm07_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$sldil_jccm07_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected sldiljccm07 params
					if (count($sldil_jccm07_array) > 0)
					{
						$sldil_jccm07_params = new Registry;
						$sldil_jccm07_params->loadArray($sldil_jccm07_array);
						$item->params->merge($sldil_jccm07_params);
					}


					// get display date
					switch ($item->params->get('list_show_sldiljccm07_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an sldiljccm07, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
