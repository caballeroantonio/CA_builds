...
require_once JPATH_ADMINISTRATOR.'/components/com_jtca/models/lcp02.php';

class JtCaModelLcp02Form extends JtCaModelLcp02
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.edit.lcp02';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_jtca.lcp02';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_jtca.edit.lcp02', 'lcp02', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the libro de turno de justicia para adolescentes sin detenido.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	LIBRO DE TURNO DE JUSTICIA PARA ADOLESCENTES SIN DETENIDO item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('lcp02.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jtca.edit.lcp02.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('lcp02.id');
		$is_new		= true;
		...
		// Reorder the libro de turno de justicia para adolescentes sin d so the new libro de turno de justicia para adolescentes sin detenido is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onLcp02BeforeSave', array('com_jtca.lcp02', &$table, $is_new));
		...
		$dispatcher->trigger('onLcp02AfterSave', array('com_jtca.lcp02', $table, $is_new));

		$this->setState('lcp02.id', $table->id);
