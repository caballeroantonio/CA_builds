...
class JtcaModelLspe02s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('lspe02_id', '') == '' ? $app->input->getInt('id', '') : $params->get('lspe02_id');
		$this->setState('filter.lspe02_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('lspe02_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('lspe02_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('lspe02_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('lspe02_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('lspe02_initial_direction','ASC');
		}
		...
		if ($params->get('filter_lspe02_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_lspe02_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.lspe02_id'));
		$id .= ':'.$this->getState('filter.lspe02_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of lspe02s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_lspe02s').' AS a');
		...
		// Filter by a single or group of lspe02s.
		$lspe02_id = $this->getState('filter.lspe02_id');
		if ($lspe02_id != '')
		{
			if (is_numeric($lspe02_id))
			{
				$type = $this->getState('filter.lspe02_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $lspe02_id);
			}
			else
			{
				if (is_array($lspe02_id))
				{
					JArrayHelper::toInteger($lspe02_id);
					$lspe02_id = implode(',', $lspe02_id);
					$type = $this->getState('filter.lspe02_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$lspe02_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_lspe02_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_lspe02_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $lspe02_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Lspe02 params override menu item params only if menu param = 'use_lspe02'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_lspe02' and there is no lspe02 param, use global

					// create an array of just the params set to 'use_lspe02'
					...
					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_lspe02')
						{
							// if the lspe02 has a value, use it
							if ($lspe02_params->get($key) != '')
							{
								// get the value from the lspe02
								$lspe02_array[$key] = $lspe02_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$lspe02_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected lspe02 params
					if (count($lspe02_array) > 0)
					{
						$lspe02_params = new Registry;
						$lspe02_params->loadArray($lspe02_array);
						$item->params->merge($lspe02_params);
					}


					// get display date
					switch ($item->params->get('list_show_lspe02_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an lspe02, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
