 ...
 *
 */
class JtCaModelLjp08 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljp08';
	...
	{
		if (empty($config['ljp08_filter_fields']))
		{
			$config['ljp08_filter_fields'] = array(
				'id', 'a.id',
				...
				'field5','a.field5',
				'field6','a.field6',
				'txt_field1975','a.txt_field1975',
				'id_field1975','a.id_field1975',
				'field8','a.field8',
				'field9','a.field9',
				'field10','a.field10',
				'state', 'a.state',
	...
	*/
	public function getTable($type = 'Ljp08s', $prefix = 'JtCaTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljp08.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJP08S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjp08ChangeState', array('com_jtca.ljp08', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjp08BeforeDelete', array('com_jtca.ljp08', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjp08AfterDelete', array('com_jtca.ljp08', &$table));
				}
				else
				{
					// Prune items that you can't change.
