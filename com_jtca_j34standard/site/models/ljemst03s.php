...
class JtCaModelLjemst03s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljemst03_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljemst03_id');
		$this->setState('filter.ljemst03_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljemst03_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljemst03_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljemst03_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljemst03_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljemst03_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljemst03_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljemst03_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljemst03_id'));
		$id .= ':'.$this->getState('filter.ljemst03_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljemst03s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljemst03s').' AS a');
		...
		// Filter by a single or group of ljemst03s.
		$ljemst03_id = $this->getState('filter.ljemst03_id');
		if ($ljemst03_id != '')
		{
			if (is_numeric($ljemst03_id))
			{
				$type = $this->getState('filter.ljemst03_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljemst03_id);
			}
			else
			{
				if (is_array($ljemst03_id))
				{
					JArrayHelper::toInteger($ljemst03_id);
					$ljemst03_id = implode(',', $ljemst03_id);
					$type = $this->getState('filter.ljemst03_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljemst03_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljemst03_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljemst03_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljemst03_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljemst03 params override menu item params only if menu param = 'use_ljemst03'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljemst03' and there is no ljemst03 param, use global

					// create an array of just the params set to 'use_ljemst03'
					$menu_params_array = $this->getState('params')->toArray();
					$ljemst03_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljemst03')
						{
							// if the ljemst03 has a value, use it
							if ($ljemst03_params->get($key) != '')
							{
								// get the value from the ljemst03
								$ljemst03_array[$key] = $ljemst03_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljemst03_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljemst03 params
					if (count($ljemst03_array) > 0)
					{
						$ljemst03_params = new Registry;
						$ljemst03_params->loadArray($ljemst03_array);
						$item->params->merge($ljemst03_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljemst03_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljemst03, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
