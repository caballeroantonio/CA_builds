...
/**
 * TSJ CDMX Libros TxCA Component REGISTRO DE BILLETES DE DEPOSITO Model
 *
 */
class JtcaModelLsps07 extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.lsps07';
	...
	{
		if (empty($config['lsps07_filter_fields']))
		{
			$config['lsps07_filter_fields'] = array(
				'id', 'a.id',
				...
				'billete','a.billete',
				'field2405','a.field2405',
				'field2406','a.field2406',
				'field2407','a.field2407',
				'field2408_isMoral','a.field2408_isMoral',
				'field2408_paterno','a.field2408_paterno',
				'field2408_materno','a.field2408_materno',
				'field2408_nombre','a.field2408_nombre',
				'field2409','a.field2409',
				'state', 'a.state',
				'created', 'a.created',
	...
	*/
	public function getTable($type = 'Lsps07s', $prefix = 'JtcaTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get REGISTRO DE BILLETES DE DEPOSITO data.
	 *
	 * @param	integer	$pk	The id of the registro de billetes de deposito.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('lsps07.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LSPS07S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLsps07ChangeState', array('com_jtca.lsps07', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLsps07BeforeDelete', array('com_jtca.lsps07', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLsps07AfterDelete', array('com_jtca.lsps07', &$table));
				}
				else
				{
					// Prune items that you can't change.
