...
class JtcaModelLjpes18s extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$pk = $params->get('ljpes18_id', '') == '' ? $app->input->getInt('id', '') : $params->get('ljpes18_id');
		$this->setState('filter.ljpes18_id', $pk);
		
		// List state information
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('ljpes18_num_per_page'),'integer');
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('ljpes18_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('ljpes18_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('ljpes18_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('ljpes18_initial_direction','ASC');
		}
		...
		if ($params->get('filter_ljpes18_archived'))
		{
			$this->setState('filter.archived', $params->get('filter_ljpes18_archived'));
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.ljpes18_id'));
		$id .= ':'.$this->getState('filter.ljpes18_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of ljpes18s subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('jt_ljpes18s').' AS a');
		...
		// Filter by a single or group of ljpes18s.
		$ljpes18_id = $this->getState('filter.ljpes18_id');
		if ($ljpes18_id != '')
		{
			if (is_numeric($ljpes18_id))
			{
				$type = $this->getState('filter.ljpes18_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $ljpes18_id);
			}
			else
			{
				if (is_array($ljpes18_id))
				{
					JArrayHelper::toInteger($ljpes18_id);
					$ljpes18_id = implode(',', $ljpes18_id);
					$type = $this->getState('filter.ljpes18_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$ljpes18_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_ljpes18_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			// clean filter variable
			$filter = JString::strtolower($filter);
			$filter = $db->quote('%'.$db->escape($filter, true).'%', false);

			switch ($params->get('show_ljpes18_filter_field'))
			{
				case 'created_by':
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $ljpes18_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Ljpes18 params override menu item params only if menu param = 'use_ljpes18'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_ljpes18' and there is no ljpes18 param, use global

					// create an array of just the params set to 'use_ljpes18'
					$menu_params_array = $this->getState('params')->toArray();
					$ljpes18_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_ljpes18')
						{
							// if the ljpes18 has a value, use it
							if ($ljpes18_params->get($key) != '')
							{
								// get the value from the ljpes18
								$ljpes18_array[$key] = $ljpes18_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$ljpes18_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected ljpes18 params
					if (count($ljpes18_array) > 0)
					{
						$ljpes18_params = new Registry;
						$ljpes18_params->loadArray($ljpes18_array);
						$item->params->merge($ljpes18_params);
					}


					// get display date
					switch ($item->params->get('list_show_ljpes18_date'))
					{
						case 'modified':
				...
				// Technically guest could edit an ljpes18, but lets not check that to improve performance a little.
				if (!$guest) 
				{
					$asset	= 'com_jtca';
