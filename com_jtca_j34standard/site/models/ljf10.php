...
class JtcaModelLjf10 extends JModelItem
{
	 ...
	 * @var		string
	 */
	protected $context = 'com_jtca.ljf10';
	...
	{
		if (empty($config['ljf10_filter_fields']))
		{
	...
	*/
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('ljf10.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_JTCA_LJF10S_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function publish(&$pks, $value = 1)
	{
		...
		$result = $dispatcher->trigger('onLjf10ChangeState', array('com_jtca.ljf10', $pks, $value));
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onLjf10BeforeDelete', array('com_jtca.ljf10', &$table));
					if (in_array(false, $result, true))
					{
						$this->setError($table->getError());
						return false;
					...
					$dispatcher->trigger('onLjf10AfterDelete', array('com_jtca.ljf10', &$table));
				}
				else
				{
					// Prune items that you can't change.
