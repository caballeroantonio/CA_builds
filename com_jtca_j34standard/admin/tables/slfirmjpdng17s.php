 ...
 *
 */
class JtcaTableSlfirmJpdng17s extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('jt_slfirm_jpdng17s', 'id', $db);
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_jtca.slfirmjpdng17'));
	}
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('SlfirmJpdng17s','JtcaTable');

		// For all keys
		foreach ($pks as $pk)
		{
			// Load the slfirmjpdng17
			if(!$table->load($pk))
			{
	...
	/**
	 * Stores a Sub Libro CONTROL DE FIRMAS Del LIBRO DE CONTROL DE FIRMAS PROCESADOS
	 *
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Sub Libro CONTROL DE FIRMAS Del LIBRO DE CONTROL DE FIRMAS PROCESADOS. A created and created_by field can be set by the user,
