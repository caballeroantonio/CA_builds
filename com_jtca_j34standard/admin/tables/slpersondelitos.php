...
/**
 * Slpersondelito table
 *
 */
class JtcaTableSlpersondelitos extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('jt_slpersondelitos', 'id', $db);
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_jtca.slpersondelito'));
	}
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('Slpersondelitos','JtcaTable');

		// For all keys
		foreach ($pks as $pk)
		{
			// Load the slpersondelito
			if(!$table->load($pk))
			{
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Inculpados. A created and created_by field can be set by the user,
