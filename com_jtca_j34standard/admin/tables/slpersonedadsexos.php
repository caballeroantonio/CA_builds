...
/**
 * Slpersonedadsexo table
 *
 */
class JtcaTableSlpersonedadsexos extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('jt_slpersonedadsexos', 'id', $db);
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_jtca.slpersonedadsexo'));
	}
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('Slpersonedadsexos','JtcaTable');

		// For all keys
		foreach ($pks as $pk)
		{
			// Load the slpersonedadsexo
			if(!$table->load($pk))
			{
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Adolescente. A created and created_by field can be set by the user,
