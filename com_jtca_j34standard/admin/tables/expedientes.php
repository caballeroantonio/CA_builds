 ...
 *
 */
class JtcaTableExpedientes extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('jt_expedientes', 'id', $db);
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_jtca.expediente'));
	}
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('Expedientes','JtcaTable');
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Expedientes. A created and created_by field can be set by the user,
		...
		if (isset($this->partescontenciosas) AND is_array($this->partescontenciosas))
		{
			$this->partescontenciosas = implode(',',$this->partescontenciosas);
		}	
