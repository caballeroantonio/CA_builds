 ...
 *
 */
class JtcaTableSldiliJpdng09s extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('jt_sldili_jpdng09s', 'id', $db);
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_jtca.sldilijpdng09'));
	}
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('SldiliJpdng09s','JtcaTable');

		// For all keys
		foreach ($pks as $pk)
		{
			// Load the sldilijpdng09
			if(!$table->load($pk))
			{
	...
	/**
	 * Stores a Sub Libro DILIGENCIA Del LIBRO DE ACTUARIOS
	 *
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Sub Libro DILIGENCIA Del LIBRO DE ACTUARIOS. A created and created_by field can be set by the user,
