 ...
 *
 */
class JtcaTableSlofenJpdng01s extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('jt_slofen_jpdng01s', 'id', $db);
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_jtca.slofenjpdng01'));
	}
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('SlofenJpdng01s','JtcaTable');

		// For all keys
		foreach ($pks as $pk)
		{
			// Load the slofenjpdng01
			if(!$table->load($pk))
			{
	...
	/**
	 * Stores a Sub Libro DATOS DEL(OS) OFENDIDO(S) Del LIBRO DE GOBIERNO
	 *
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Sub Libro DATOS DEL(OS) OFENDIDO(S) Del LIBRO DE GOBIERNO. A created and created_by field can be set by the user,
