 ...
 *
 */
class JtcaTableSlfirmJjadg16s extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('jt_slfirm_jjadg16s', 'id', $db);
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_jtca.slfirmjjadg16'));
	}
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('SlfirmJjadg16s','JtcaTable');

		// For all keys
		foreach ($pks as $pk)
		{
			// Load the slfirmjjadg16
			if(!$table->load($pk))
			{
	...
	/**
	 * Stores a Sub Libro CONTROL DE FIRMAS Del LIBRO DE CONTROL DE PRESENTACION DE ADOLESCENTES EN LIBERTAD ANTE EL JUEZ
	 *
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Sub Libro CONTROL DE FIRMAS Del LIBRO DE CONTROL DE PRESENTACION DE ADOLESCENTES EN LIBERTAD ANTE EL JUEZ. A created and created_by field can be set by the user,
