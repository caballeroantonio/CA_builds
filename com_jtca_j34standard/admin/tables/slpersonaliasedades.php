...
/**
 * Slpersonaliasedad table
 *
 */
class JtcaTableSlpersonaliasedades extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('jt_slpersonaliasedades', 'id', $db);
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_jtca.slpersonaliasedad'));
	}
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('Slpersonaliasedades','JtcaTable');

		// For all keys
		foreach ($pks as $pk)
		{
			// Load the slpersonaliasedad
			if(!$table->load($pk))
			{
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Adolescentes. A created and created_by field can be set by the user,
