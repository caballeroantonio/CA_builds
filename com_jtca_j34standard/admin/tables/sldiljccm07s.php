 ...
 *
 */
class JtcaTableSldilJccm07s extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('jt_sldil_jccm07s', 'id', $db);
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_jtca.sldiljccm07'));
	}
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('SldilJccm07s','JtcaTable');

		// For all keys
		foreach ($pks as $pk)
		{
			// Load the sldiljccm07
			if(!$table->load($pk))
			{
	...
	/**
	 * Stores a Sub Libro DILIGENCIA Del LIBRO DE EXHORTOS
	 *
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Sub Libro DILIGENCIA Del LIBRO DE EXHORTOS. A created and created_by field can be set by the user,
