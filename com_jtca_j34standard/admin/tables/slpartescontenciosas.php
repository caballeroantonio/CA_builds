...
/**
 * Slpartecontenciosa table
 *
 */
class JtcaTableSlpartescontenciosas extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('jt_slpartescontenciosas', 'id', $db);
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_jtca.slpartecontenciosa'));
	}
	...
	public function bind($array, $ignore = array())
	{
		...
		if ( !array_key_exists('isMoral',$array)  ) 
		{
			$array['isMoral'] = '0';
		}
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('Slpartescontenciosas','JtcaTable');

		// For all keys
		foreach ($pks as $pk)
		{
			// Load the slpartecontenciosa
			if(!$table->load($pk))
			{
	...
	/**
	 * Stores a Partes Contenciosas En Juicio
	 *
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Partes Contenciosas En Juicio. A created and created_by field can be set by the user,
