...
/**
 * SlinculJpdng01 table
 *
 */
class JtcaTableSlinculJpdng01s extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('jt_slincul_jpdng01s', 'id', $db);
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_jtca.slinculjpdng01'));
	}
	...
	public function bind($array, $ignore = array())
	{
		...
		if ( !array_key_exists('sfield4',$array)  ) 
		{
			$array['sfield4'] = '0';
		}
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('SlinculJpdng01s','JtcaTable');

		// For all keys
		foreach ($pks as $pk)
		{
			// Load the slinculjpdng01
			if(!$table->load($pk))
			{
	...
	/**
	 * Stores a Sub Libro DATOS DEL(OS) INCULPADO(S) Del LIBRO DE GOBIERNO
	 *
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Sub Libro DATOS DEL(OS) INCULPADO(S) Del LIBRO DE GOBIERNO. A created and created_by field can be set by the user,
