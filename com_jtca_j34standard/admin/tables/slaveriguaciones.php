...
/**
 * Slaveriguacion table
 *
 */
class JtcaTableSlaveriguaciones extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('jt_slaveriguaciones', 'id', $db);
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_jtca.slaveriguacion'));
	}
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('Slaveriguaciones','JtcaTable');

		// For all keys
		foreach ($pks as $pk)
		{
			// Load the slaveriguacion
			if(!$table->load($pk))
			{
	...
	/**
	 * Stores a Averiguaciones Previas
	 *
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Averiguaciones Previas. A created and created_by field can be set by the user,
