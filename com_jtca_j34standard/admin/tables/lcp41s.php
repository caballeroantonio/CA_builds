 ...
 *
 */
class JtCaTableLcp41s extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('jt_lcp41s', 'id', $db);
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_jtca.lcp41'));
	}
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('Lcp41s','JtCaTable');
	...
	/**
	 * Stores a LIBRO DE TURNO DE REMISIONES SIN DETENIDO DE JUZGADOS ADOLESCENTES
	 *
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New LIBRO DE TURNO DE REMISIONES SIN DETENIDO DE JUZGADOS ADOLESCENTES. A created and created_by field can be set by the user,
