 ...
 *
 */
class JtcaTableSladolJjadg05s extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('jt_sladol_jjadg05s', 'id', $db);
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_jtca.sladoljjadg05'));
	}
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('SladolJjadg05s','JtcaTable');

		// For all keys
		foreach ($pks as $pk)
		{
			// Load the sladoljjadg05
			if(!$table->load($pk))
			{
	...
	/**
	 * Stores a Sub Libro ADOLESCENTES Del LIBRO DE SENTENCIAS
	 *
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Sub Libro ADOLESCENTES Del LIBRO DE SENTENCIAS. A created and created_by field can be set by the user,
