 ...
 *
 */
class JtcaTableSlpersonaliases extends JTable
{
	...
	public function __construct(&$db)
	{
		...
		parent::__construct('jt_slpersonaliases', 'id', $db);
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_jtca.slpersonalias'));
	}
	...
	public function publish($pks = null, $state = 1, $user_id = 0)
	{
		...
		$table = JTable::getInstance('Slpersonaliases','JtcaTable');

		// For all keys
		foreach ($pks as $pk)
		{
			// Load the slpersonalias
			if(!$table->load($pk))
			{
	...
	public function store($update_nulls = false)
	{
		...
		if (empty($this->id))
		{
			// New Inculpado. A created and created_by field can be set by the user,
