...
/**
 * Methods supporting a list of slpartecontenciosa records.
 *
 */
class JtcaModelSlpartescontenciosas extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		// Select the required partes contenciosas from the table.
		$query->select(
			$this->getState(
					'list.select',
					'a.*'
		...
		if ($id_record = $this->getState('filter.id_record'))
		{
			...
			$query->where('LOWER('.$db->quoteName('a.id_record').') = ' . $db->quote($id_record));
		}
		if ($id_field = $this->getState('filter.id_field'))
		{
			$id_field = $db->escape(JString::strtolower($id_field), true);			
			$query->where('LOWER('.$db->quoteName('a.id_field').') = ' . $db->quote($id_field));
		}
