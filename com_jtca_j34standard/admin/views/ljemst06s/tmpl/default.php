...
if ($save_order)
{
	$save_ordering_url = 'index.php?option=com_jtca&task=ljemst06s.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'ljemst06-list', 'adminForm', strtolower($list_dirn), $save_ordering_url);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_jtca&view=ljemst06s'); ?>" method="post" name="adminForm" id="adminForm">
...
?>	
	...
	<?php else : ?>
		<table class="table table-striped" id="ljemst06-list">
			<thead>
				<tr>
			...
			<?php foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="center">
						<div class="btn-group">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'ljemst06s.', $can_change); ?>
							<?php
								if ($archived) :
									JHtml::_('actionsdropdown.unarchive', 'cb' . $i, 'ljemst06s');
								else :
									JHtml::_('actionsdropdown.archive', 'cb' . $i, 'ljemst06s');
								endif;
								if ($trashed) :
									JHtml::_('actionsdropdown.untrash', 'cb' . $i, 'ljemst06s');
								else :
									JHtml::_('actionsdropdown.trash', 'cb' . $i, 'ljemst06s');
								endif;
