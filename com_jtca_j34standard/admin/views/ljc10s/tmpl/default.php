...
if ($save_order)
{
	$save_ordering_url = 'index.php?option=com_jtca&task=ljc10s.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'ljc10-list', 'adminForm', strtolower($list_dirn), $save_ordering_url);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_jtca&view=ljc10s'); ?>" method="post" name="adminForm" id="adminForm">
...
?>	
	...
	<?php else : ?>
		<table class="table table-striped" id="ljc10-list">
			<thead>
				<tr>
			...
			<?php foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="center">
						<div class="btn-group">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'ljc10s.', $can_change); ?>
							<?php
								if ($archived) :
									JHtml::_('actionsdropdown.unarchive', 'cb' . $i, 'ljc10s');
								else :
									JHtml::_('actionsdropdown.archive', 'cb' . $i, 'ljc10s');
								endif;
								if ($trashed) :
									JHtml::_('actionsdropdown.untrash', 'cb' . $i, 'ljc10s');
								else :
									JHtml::_('actionsdropdown.trash', 'cb' . $i, 'ljc10s');
								endif;
