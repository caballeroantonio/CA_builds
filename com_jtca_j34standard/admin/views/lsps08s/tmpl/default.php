...
if ($save_order)
{
	$save_ordering_url = 'index.php?option=com_jtca&task=lsps08s.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'lsps08-list', 'adminForm', strtolower($list_dirn), $save_ordering_url);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_jtca&view=lsps08s'); ?>" method="post" name="adminForm" id="adminForm">
...
?>	
	...
	<?php else : ?>
		<table class="table table-striped" id="lsps08-list">
			<thead>
				<tr>
			...
			<?php foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="center">
						<div class="btn-group">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'lsps08s.', $can_change); ?>
							<?php
								if ($archived) :
									JHtml::_('actionsdropdown.unarchive', 'cb' . $i, 'lsps08s');
								else :
									JHtml::_('actionsdropdown.archive', 'cb' . $i, 'lsps08s');
								endif;
								if ($trashed) :
									JHtml::_('actionsdropdown.untrash', 'cb' . $i, 'lsps08s');
								else :
									JHtml::_('actionsdropdown.trash', 'cb' . $i, 'lsps08s');
								endif;
