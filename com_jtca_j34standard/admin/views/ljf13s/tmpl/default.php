...
if ($save_order)
{
	$save_ordering_url = 'index.php?option=com_jtca&task=ljf13s.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'ljf13-list', 'adminForm', strtolower($list_dirn), $save_ordering_url);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_jtca&view=ljf13s'); ?>" method="post" name="adminForm" id="adminForm">
...
?>	
	...
	<?php else : ?>
		<table class="table table-striped" id="ljf13-list">
			<thead>
				<tr>
			...
			<?php foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="center">
						<div class="btn-group">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'ljf13s.', $can_change); ?>
							<?php
								if ($archived) :
									JHtml::_('actionsdropdown.unarchive', 'cb' . $i, 'ljf13s');
								else :
									JHtml::_('actionsdropdown.archive', 'cb' . $i, 'ljf13s');
								endif;
								if ($trashed) :
									JHtml::_('actionsdropdown.untrash', 'cb' . $i, 'ljf13s');
								else :
									JHtml::_('actionsdropdown.trash', 'cb' . $i, 'ljf13s');
								endif;
