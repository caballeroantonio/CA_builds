...
if ($save_order)
{
	$save_ordering_url = 'index.php?option=com_jtca&task=lspe06s.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'lspe06-list', 'adminForm', strtolower($list_dirn), $save_ordering_url);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_jtca&view=lspe06s'); ?>" method="post" name="adminForm" id="adminForm">
...
?>	
	...
	<?php else : ?>
		<table class="table table-striped" id="lspe06-list">
			<thead>
				<tr>
			...
			<?php foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="center">
						<div class="btn-group">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'lspe06s.', $can_change); ?>
							<?php
								if ($archived) :
									JHtml::_('actionsdropdown.unarchive', 'cb' . $i, 'lspe06s');
								else :
									JHtml::_('actionsdropdown.archive', 'cb' . $i, 'lspe06s');
								endif;
								if ($trashed) :
									JHtml::_('actionsdropdown.untrash', 'cb' . $i, 'lspe06s');
								else :
									JHtml::_('actionsdropdown.trash', 'cb' . $i, 'lspe06s');
								endif;
