...
if ($save_order)
{
	$save_ordering_url = 'index.php?option=com_jtca&task=ljjadg15s.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'ljjadg15-list', 'adminForm', strtolower($list_dirn), $save_ordering_url);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_jtca&view=ljjadg15s'); ?>" method="post" name="adminForm" id="adminForm">
...
?>	
	...
	<?php else : ?>
		<table class="table table-striped" id="ljjadg15-list">
			<thead>
				<tr>
			...
			<?php foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="center">
						<div class="btn-group">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'ljjadg15s.', $can_change); ?>
							<?php
								if ($archived) :
									JHtml::_('actionsdropdown.unarchive', 'cb' . $i, 'ljjadg15s');
								else :
									JHtml::_('actionsdropdown.archive', 'cb' . $i, 'ljjadg15s');
								endif;
								if ($trashed) :
									JHtml::_('actionsdropdown.untrash', 'cb' . $i, 'ljjadg15s');
								else :
									JHtml::_('actionsdropdown.trash', 'cb' . $i, 'ljjadg15s');
								endif;
