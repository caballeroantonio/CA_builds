...
if ($save_order)
{
	$save_ordering_url = 'index.php?option=com_jtca&task=lcp02s.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'lcp02-list', 'adminForm', strtolower($list_dirn), $save_ordering_url);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_jtca&view=lcp02s'); ?>" method="post" name="adminForm" id="adminForm">
...
?>	
	...
	<?php else : ?>
		<table class="table table-striped" id="lcp02-list">
			<thead>
				<tr>
			...
			<?php foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="center">
						<div class="btn-group">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'lcp02s.', $can_change); ?>
							<?php
								if ($archived) :
									JHtml::_('actionsdropdown.unarchive', 'cb' . $i, 'lcp02s');
								else :
									JHtml::_('actionsdropdown.archive', 'cb' . $i, 'lcp02s');
								endif;
								if ($trashed) :
									JHtml::_('actionsdropdown.untrash', 'cb' . $i, 'lcp02s');
								else :
									JHtml::_('actionsdropdown.trash', 'cb' . $i, 'lcp02s');
								endif;
