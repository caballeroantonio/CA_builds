...
if ($save_order)
{
	$save_ordering_url = 'index.php?option=com_jtca&task=lspe02s.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'lspe02-list', 'adminForm', strtolower($list_dirn), $save_ordering_url);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_jtca&view=lspe02s'); ?>" method="post" name="adminForm" id="adminForm">
...
?>	
	...
	<?php else : ?>
		<table class="table table-striped" id="lspe02-list">
			<thead>
				<tr>
			...
			<?php foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td class="center">
						<div class="btn-group">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'lspe02s.', $can_change); ?>
							<?php
								if ($archived) :
									JHtml::_('actionsdropdown.unarchive', 'cb' . $i, 'lspe02s');
								else :
									JHtml::_('actionsdropdown.archive', 'cb' . $i, 'lspe02s');
								endif;
								if ($trashed) :
									JHtml::_('actionsdropdown.untrash', 'cb' . $i, 'lspe02s');
								else :
									JHtml::_('actionsdropdown.trash', 'cb' . $i, 'lspe02s');
								endif;
