 ...
 *
 */
class BoletinModelFacuerdo extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 */
	protected $text_prefix = 'COM_BOLETIN_FACUERDOS';
	/**
	 * @var      string	The type alias for this object (for example, 'com_boletin.facuerdo')
	 */
	 ...
	 */
	protected $event_before_save = 'onFacuerdoBeforeSave';
	/**
	 * @var		string	The event to trigger after saving the data.
	 */
	protected $event_after_save = 'onFacuerdoAfterSave';

	/**
	 * @var    string	The event to trigger before deleting the data.
	 */
	protected $event_before_delete = 'onFacuerdoBeforeDelete';	
	/**
	 * @var    string	The event to trigger after deleting the data.
	 */
	protected $event_after_delete = 'onFacuerdoAfterDelete';	
	...
	public function getForm($data = array(), $load_data = true)
	{
		...
		if ($this->getState('facuerdo.id'))
		{
			$id = $this->getState('facuerdo.id');		
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_boletin.edit.facuerdo.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// Prime some default values.
			if ($this->getState('facuerdo.id') == 0)
			{
				$filters = (array) $app->getUserState('com_boletin.facuerdos.filter');
