...
/**
 * Rsps_bacuerdo controller class.
 * 
 */
class BoletinControllerRsps_bacuerdo extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'rsps_bacuerdoform';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'rsps_bacuerdos';
	/**
	 * 
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('rsps_bacuerdoform')->getItem($record_id);
	...
	protected function allowDelete($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.delete.own', $asset))
		{
			...
			{
				// Need to do a lookup from the model.
				$table = $this->getModel('rsps_bacuerdo')->getTable();
	 ...
	 * 
	 */
	public function getModel($name = 'rsps_bacuerdoform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Secretaría De La Función Pública - Servidores Publicos Sancionados can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('rsps_bacuerdoform');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_BOLETIN_RSPS_BACUERDOS_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_BOLETIN_RSPS_BACUERDOS_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('rsps_bacuerdo.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		// Get the menu item model.
		$model = $this->getModel('rsps_bacuerdo');
		...
		if ($id > 0) 
		{
			if($model->delete($id))
			{
				$this->setMessage(JText::_('COM_BOLETIN_RSPS_BACUERDOS_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_BOLETIN_RSPS_BACUERDOS_DELETE_FAILED'));
			}
		}
