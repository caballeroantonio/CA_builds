...
/**
 * Profeco_proveedor controller class.
 * 
 */
class BoletinControllerProfeco_proveedor extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'profeco_proveedorform';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'profeco_proveedores';
	/**
	 * 
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('profeco_proveedorform')->getItem($record_id);
	...
	protected function allowDelete($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.delete.own', $asset))
		{
			...
			{
				// Need to do a lookup from the model.
				$table = $this->getModel('profeco_proveedor')->getTable();
	 ...
	 * 
	 */
	public function getModel($name = 'profeco_proveedorform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Procuraduría Federal Del Consumidor - Buró Comercial can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('profeco_proveedorform');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_BOLETIN_PROFECO_PROVEEDORES_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_BOLETIN_PROFECO_PROVEEDORES_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('profeco_proveedor.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		// Get the menu item model.
		$model = $this->getModel('profeco_proveedor');
		...
		if ($id > 0) 
		{
			if($model->delete($id))
			{
				$this->setMessage(JText::_('COM_BOLETIN_PROFECO_PROVEEDORES_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_BOLETIN_PROFECO_PROVEEDORES_DELETE_FAILED'));
			}
		}
