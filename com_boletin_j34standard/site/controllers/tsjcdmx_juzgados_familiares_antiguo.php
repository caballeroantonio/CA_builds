...
/**
 * Tsjcdmx_juzgados_familiares_antiguo controller class.
 * 
 */
class BoletinControllerTsjcdmx_juzgados_familiares_antiguo extends JControllerForm
{
	/**
	 * @var    string	$view_item	The URL view item variable.
	 */
	protected $view_item = 'tsjcdmx_juzgados_familiares_antiguoform';
	/**
	 * @var    string	$view_list	The URL view list variable.
	 */
	protected $view_list = 'tsjcdmx_juzgados_familiares_antiguos';
	/**
	 * 
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit.own', $asset))
		{
			...
			if (empty($owner_id) AND $record_id)
			{
				...
				$record		= $this->getModel('tsjcdmx_juzgados_familiares_antiguoform')->getItem($record_id);
	...
	protected function allowDelete($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.delete.own', $asset))
		{
			...
			{
				// Need to do a lookup from the model.
				$table = $this->getModel('tsjcdmx_juzgados_familiares_antiguo')->getTable();
	 ...
	 * 
	 */
	public function getModel($name = 'tsjcdmx_juzgados_familiares_antiguoform', $prefix = '',$config = array('ignore_request' => true))
	{
	 ...
	 *
	 * @return  mixed  True if the Tsjcdmx_juzgados_familiares_antiguos can be added, a error object if not.
	 *
	 */
	public function add()
	{
	...
	public function edit($key = 'id', $url_var = null)
	{
		...
		$model = $this->getModel('tsjcdmx_juzgados_familiares_antiguoform');
		// Set the return url
		$this->setReturnPage($context);
	...
	public function save($key = 'id', $url_var = null)
	{
		...
		if ($record_id == 0) 
		{
			$this->setMessage(JText::_('COM_BOLETIN_TSJCDMX_JUZGADOS_FAMILIARES_ANTIGUOS_SUBMIT_SAVE_SUCCESS'));
		} 
		else 
		{
			$this->setMessage(JText::_('COM_BOLETIN_TSJCDMX_JUZGADOS_FAMILIARES_ANTIGUOS_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$record_id = $model->getState('tsjcdmx_juzgados_familiares_antiguo.id');
				$this->holdEditId($context, $record_id);
				$app->setUserState($context.'.data',	null);
	...
	public function delete()
	{
		...
		// Get the menu item model.
		$model = $this->getModel('tsjcdmx_juzgados_familiares_antiguo');
		...
		if ($id > 0) 
		{
			if($model->delete($id))
			{
				$this->setMessage(JText::_('COM_BOLETIN_TSJCDMX_JUZGADOS_FAMILIARES_ANTIGUOS_DELETE_SUCCESS'));
				
			}
			else
			{
				$this->setMessage(JText::_('COM_BOLETIN_TSJCDMX_JUZGADOS_FAMILIARES_ANTIGUOS_DELETE_FAILED'));
			}
		}
