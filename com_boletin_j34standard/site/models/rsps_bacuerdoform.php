...
require_once JPATH_ADMINISTRATOR.'/components/com_boletin/models/rsps_bacuerdo.php';

class BoletinModelRsps_bacuerdoForm extends BoletinModelRsps_bacuerdo
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_boletin.edit.rsps_bacuerdo';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_boletin.rsps_bacuerdo';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_boletin.edit.rsps_bacuerdo', 'rsps_bacuerdo', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the rsps_bacuerdo.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	rsps_bacuerdo item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('rsps_bacuerdo.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_boletin.edit.rsps_bacuerdo.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('rsps_bacuerdo.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onRsps_bacuerdoBeforeSave', array('com_boletin.rsps_bacuerdo', &$table, $is_new));
		...
		$dispatcher->trigger('onRsps_bacuerdoAfterSave', array('com_boletin.rsps_bacuerdo', $table, $is_new));

		$this->setState('rsps_bacuerdo.id', $table->id);
