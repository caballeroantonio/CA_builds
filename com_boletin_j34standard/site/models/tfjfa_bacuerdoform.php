...
require_once JPATH_ADMINISTRATOR.'/components/com_boletin/models/tfjfa_bacuerdo.php';

class BoletinModelTfjfa_bacuerdoForm extends BoletinModelTfjfa_bacuerdo
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_boletin.edit.tfjfa_bacuerdo';
	/**
	...
	protected function populateState()
	{
		...
		$this->setState('tfjfa_bacuerdo.catid', $app->input->getInt('catid'));
	...
	*/
	...
	public function getForm($data = array(), $load_data = true)
	{
		...
		if ($id = (int) $this->getState('tfjfa_bacuerdo.id'))
		{
	 ...
	 *
	 * @param	integer	$item_id	The id of the tfjfa_bacuerdo.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	tfjfa_bacuerdo item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('tfjfa_bacuerdo.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_boletin.edit.tfjfa_bacuerdo.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('tfjfa_bacuerdo.id');
		$is_new		= true;
		...
		// Reorder the tfjfa_bacuerdos so the new tribunal federal de justicia administrativa - boletín jurisdiccional is first
		if (empty($table->id))
		{
		...
		$result = $dispatcher->trigger('onTfjfa_bacuerdoBeforeSave', array('com_boletin.tfjfa_bacuerdo', &$table, $is_new));
		...
		$dispatcher->trigger('onTfjfa_bacuerdoAfterSave', array('com_boletin.tfjfa_bacuerdo', $table, $is_new));

		$this->setState('tfjfa_bacuerdo.id', $table->id);
