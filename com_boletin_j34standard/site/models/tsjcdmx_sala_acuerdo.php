...
/**
 * Boletines Component Tribunal Superior De Justicia De La Ciudad De México - Salas Model
 *
 */
class BoletinModelTsjcdmx_sala_acuerdo extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_boletin.tsjcdmx_sala_acuerdo';
	...
	{
		if (empty($config['tsjcdmx_sala_acuerdo_filter_fields']))
		{
			$config['tsjcdmx_sala_acuerdo_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	 ...
	 *
	 * @param	integer	$pk	The id of the tribunal superior de justicia de la ciudad de méxico - salas.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('tsjcdmx_sala_acuerdo.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_BOLETIN_TSJCDMX_SALA_ACUERDOS_ERROR_ITEM_NOT_FOUND'));
				}
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onTsjcdmx_sala_acuerdoBeforeDelete', array('com_boletin.tsjcdmx_sala_acuerdo', &$table));
					...
					$dispatcher->trigger('onTsjcdmx_sala_acuerdoAfterDelete', array('com_boletin.tsjcdmx_sala_acuerdo', &$table));
				}
				else
				{
