...
/**
 * This models supports retrieving lists of acuerdos de juzgados del tsjcdmx.
 *
 */
class BoletinModelTsjcdmx_juzgado_acuerdos extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_boletin.tsjcdmx_juzgado_acuerdos';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single tribunal superior de justicia de la ciudad de méxico - juzgados has been specified either as a parameter or in the url Request
		$pk = $params->get('tsjcdmx_juzgado_acuerdo_id', '') == '' ? $app->input->getInt('id', '') : $params->get('tsjcdmx_juzgado_acuerdo_id');
		$this->setState('filter.tsjcdmx_juzgado_acuerdo_id', $pk);
		
		// List state information
		if ($app->input->getString('layout', 'default') == 'blog')
		{
			$limit = $params->def('tsjcdmx_juzgado_acuerdo_num_leading', 1) + $params->def('tsjcdmx_juzgado_acuerdo_num_intro', 4) + $params->def('tsjcdmx_juzgado_acuerdo_num_links', 4);
		}
		else
		{		
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('tsjcdmx_juzgado_acuerdo_num_per_page'),'integer');
		}
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('tsjcdmx_juzgado_acuerdo_initial_sort','a.id'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('tsjcdmx_juzgado_acuerdo_initial_sort','a.id');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('tsjcdmx_juzgado_acuerdo_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('tsjcdmx_juzgado_acuerdo_initial_direction','ASC');
		}
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.tsjcdmx_juzgado_acuerdo_id'));
		$id .= ':'.$this->getState('filter.tsjcdmx_juzgado_acuerdo_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of tsjcdmx_juzgado_acuerdos subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		// Filter by a single or group of tsjcdmx_juzgado_acuerdos.
		$tsjcdmx_juzgado_acuerdo_id = $this->getState('filter.tsjcdmx_juzgado_acuerdo_id');
		if ($tsjcdmx_juzgado_acuerdo_id != '')
		{
			if (is_numeric($tsjcdmx_juzgado_acuerdo_id))
			{
				$type = $this->getState('filter.tsjcdmx_juzgado_acuerdo_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $tsjcdmx_juzgado_acuerdo_id);
			}
			else
			{
				if (is_array($tsjcdmx_juzgado_acuerdo_id))
				{
					JArrayHelper::toInteger($tsjcdmx_juzgado_acuerdo_id);
					$tsjcdmx_juzgado_acuerdo_id = implode(',', $tsjcdmx_juzgado_acuerdo_id);
					$type = $this->getState('filter.tsjcdmx_juzgado_acuerdo_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$tsjcdmx_juzgado_acuerdo_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_tsjcdmx_juzgado_acuerdo_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_tsjcdmx_juzgado_acuerdo_filter_field'))
			{
	...
	/**
	 * Method to get a list of acuerdos de juzgados del tsjcdmx.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $tsjcdmx_juzgado_acuerdo_params;
				}
				else
				{
					...
					// get display date
					switch ($item->params->get('list_show_tsjcdmx_juzgado_acuerdo_date'))
					{
						default:
							$item->display_date = 0;
							break;
