...
/**
 * Boletines Component Acuerdos Fiscales Model
 *
 */
class BoletinModelFacuerdo extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_boletin.facuerdo';
	...
	{
		if (empty($config['facuerdo_filter_fields']))
		{
			$config['facuerdo_filter_fields'] = array(
				'id', 'a.id',
	...
	*/
	public function getTable($type = 'Facuerdos', $prefix = 'BoletinTable', $config = array())
	{
		return JTable::getInstance($type, $prefix, $config);
	}
	/**
	 * Method to get Acuerdos Fiscales data.
	 *
	 * @param	integer	$pk	The id of the acuerdos fiscales.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('facuerdo.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				// If all categories are published, badcats.id will be null, and we just use the facuerdo state
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_BOLETIN_FACUERDOS_ERROR_ITEM_NOT_FOUND'));
				}
	...
	public function delete(&$pks)
	{
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
					...
					$result = $dispatcher->trigger('onFacuerdoBeforeDelete', array('com_boletin.facuerdo', &$table));
					...
					$dispatcher->trigger('onFacuerdoAfterDelete', array('com_boletin.facuerdo', &$table));
			}
			else
			{
				$this->setError($table->getError());
