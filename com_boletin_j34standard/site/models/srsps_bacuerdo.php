...
/**
 * Boletines Component Secretaría De Hacienda Y Crédito Público - Sistema Del Registro De Servidores Públicos Sancionados Model
 *
 */
class BoletinModelSrsps_bacuerdo extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_boletin.srsps_bacuerdo';
	...
	{
		if (empty($config['srsps_bacuerdo_filter_fields']))
		{
			$config['srsps_bacuerdo_filter_fields'] = array(
				'id', 'a.id',
				'rfc','a.rfc',
				'nombre','a.nombre',
				'oficio','a.oficio',
				'fecha_oficio','a.fecha_oficio',
				'emisor','a.emisor',
				'id_medio_notificacion','a.id_medio_notificacion',
				'fecha_notificacion','a.fecha_notificacion',
				'fecha_surte','a.fecha_surte',
				);
		}
	...
	*/
	 ...
	 *
	 * @param	integer	$pk	The id of the secretaría de hacienda y crédito público - sistema del registro de servidores públicos sancionados.
	 *
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('srsps_bacuerdo.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($item))
				{
					return JError::raiseError(404, JText::_('COM_BOLETIN_SRSPS_BACUERDOS_ERROR_ITEM_NOT_FOUND'));
				}
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				if ($this->canDelete($table))
				{
					...
					$result = $dispatcher->trigger('onSrsps_bacuerdoBeforeDelete', array('com_boletin.srsps_bacuerdo', &$table));
					...
					$dispatcher->trigger('onSrsps_bacuerdoAfterDelete', array('com_boletin.srsps_bacuerdo', &$table));
				}
				else
				{
