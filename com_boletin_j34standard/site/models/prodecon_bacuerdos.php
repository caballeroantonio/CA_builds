...
/**
 * This models supports retrieving lists of quejas en la prodecon.
 *
 */
class BoletinModelProdecon_bacuerdos extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_boletin.prodecon_bacuerdos';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single procuraduría de la defensa del contribuyente  has been specified either as a parameter or in the url Request
		$pk = $params->get('prodecon_bacuerdo_id', '') == '' ? $app->input->getInt('id', '') : $params->get('prodecon_bacuerdo_id');
		$this->setState('filter.prodecon_bacuerdo_id', $pk);
		
		// List state information
		if ($app->input->getString('layout', 'default') == 'blog')
		{
			$limit = $params->def('prodecon_bacuerdo_num_leading', 1) + $params->def('prodecon_bacuerdo_num_intro', 4) + $params->def('prodecon_bacuerdo_num_links', 4);
		}
		else
		{		
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('prodecon_bacuerdo_num_per_page'),'integer');
		}
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('prodecon_bacuerdo_initial_sort','a.id'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('prodecon_bacuerdo_initial_sort','a.id');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('prodecon_bacuerdo_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('prodecon_bacuerdo_initial_direction','ASC');
		}
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.prodecon_bacuerdo_id'));
		$id .= ':'.$this->getState('filter.prodecon_bacuerdo_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of prodecon_bacuerdos subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		// Filter by a single or group of prodecon_bacuerdos.
		$prodecon_bacuerdo_id = $this->getState('filter.prodecon_bacuerdo_id');
		if ($prodecon_bacuerdo_id != '')
		{
			if (is_numeric($prodecon_bacuerdo_id))
			{
				$type = $this->getState('filter.prodecon_bacuerdo_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $prodecon_bacuerdo_id);
			}
			else
			{
				if (is_array($prodecon_bacuerdo_id))
				{
					JArrayHelper::toInteger($prodecon_bacuerdo_id);
					$prodecon_bacuerdo_id = implode(',', $prodecon_bacuerdo_id);
					$type = $this->getState('filter.prodecon_bacuerdo_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$prodecon_bacuerdo_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_prodecon_bacuerdo_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_prodecon_bacuerdo_filter_field'))
			{
	...
	/**
	 * Method to get a list of quejas en la prodecon.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $prodecon_bacuerdo_params;
				}
				else
				{
					...
					// get display date
					switch ($item->params->get('list_show_prodecon_bacuerdo_date'))
					{
						default:
							$item->display_date = 0;
							break;
