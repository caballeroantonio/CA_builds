...
/**
 * This models supports retrieving lists of acuerdos fiscales.
 *
 */
class BoletinModelFacuerdos extends JModelList
{
	/**
	 * @var    string	$context	Context string for the model type.  This is used to handle uniqueness within sessions data.
	 */
	protected $context = 'com_boletin.facuerdos';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single acuerdos fiscales has been specified either as a parameter or in the url Request
		$pk = $params->get('facuerdo_id', '') == '' ? $app->input->getInt('id', '') : $params->get('facuerdo_id');
		$this->setState('filter.facuerdo_id', $pk);
		
		// List state information
		if ($app->input->getString('layout', 'default') == 'blog')
		{
			$limit = $params->def('facuerdo_num_leading', 1) + $params->def('facuerdo_num_intro', 4) + $params->def('facuerdo_num_links', 4);
		}
		else
		{		
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('facuerdo_num_per_page'),'integer');
		}
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('facuerdo_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('facuerdo_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('facuerdo_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('facuerdo_initial_direction','ASC');
		}
		...
		if ($params->get('filter_facuerdo_categories') AND implode(',', $params->get('filter_facuerdo_categories'))  == true)
		{
			$selected_categories = $params->get('filter_facuerdo_categories');
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.facuerdo_id'));
		$id .= ':'.$this->getState('filter.facuerdo_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of facuerdos subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__rem_facuerdos').' AS a');
		...
		// Filter by a single or group of facuerdos.
		$facuerdo_id = $this->getState('filter.facuerdo_id');
		if ($facuerdo_id != '')
		{
			if (is_numeric($facuerdo_id))
			{
				$type = $this->getState('filter.facuerdo_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $facuerdo_id);
			}
			else
			{
				if (is_array($facuerdo_id))
				{
					JArrayHelper::toInteger($facuerdo_id);
					$facuerdo_id = implode(',', $facuerdo_id);
					$type = $this->getState('filter.facuerdo_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$facuerdo_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_facuerdo_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_facuerdo_filter_field'))
			{
	...
	/**
	 * Method to get a list of acuerdos fiscales.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $facuerdo_params;
				}
				else
				{
					$item->params = clone $this->getState('params');

					// Facuerdo params override menu item params only if menu param = 'use_facuerdo'
					// Otherwise, menu item params control the layout
					// If menu item is 'use_facuerdo' and there is no facuerdo param, use global

					// create an array of just the params set to 'use_facuerdo'
					$menu_params_array = $this->getState('params')->toArray();
					$facuerdo_array = array();

					foreach ($menu_params_array as $key => $value)
					{
						if ($value === 'use_facuerdo')
						{
							// if the facuerdo has a value, use it
							if ($facuerdo_params->get($key) != '')
							{
								// get the value from the facuerdo
								$facuerdo_array[$key] = $facuerdo_params->get($key);
							}
							else
							{
								// otherwise, use the global value
								$facuerdo_array[$key] = $global_params->get($key);
							}
						}
					}

					// merge the selected facuerdo params
					if (count($facuerdo_array) > 0)
					{
						$facuerdo_params = new Registry;
						$facuerdo_params->loadArray($facuerdo_array);
						$item->params->merge($facuerdo_params);
					}


					// get display date
					switch ($item->params->get('list_show_facuerdo_date'))
					{
						default:
							$item->display_date = 0;
							break;
