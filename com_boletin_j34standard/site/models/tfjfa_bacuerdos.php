...
/**
 * This models supports retrieving lists of acuerdos fiscales del tfjfa.
 *
 */
class BoletinModelTfjfa_bacuerdos extends JModelList
{
	/**
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Check to see if a single tribunal federal de justicia administrativa - boletín jurisdiccional has been specified either as a parameter or in the url Request
		$pk = $params->get('tfjfa_bacuerdo_id', '') == '' ? $app->input->getInt('id', '') : $params->get('tfjfa_bacuerdo_id');
		$this->setState('filter.tfjfa_bacuerdo_id', $pk);
		
		// List state information
		if ($app->input->getString('layout', 'default') == 'blog')
		{
			$limit = $params->def('tfjfa_bacuerdo_num_leading', 1) + $params->def('tfjfa_bacuerdo_num_intro', 4) + $params->def('tfjfa_bacuerdo_num_links', 4);
		}
		else
		{		
			$limit = $app->getUserStateFromRequest($this->context.'.list.' . $item_id . '.limit', 'limit', $params->get('tfjfa_bacuerdo_num_per_page'),'integer');
		}
		...
		$order_col = $app->getUserStateFromRequest($this->context. '.filter_order', 'filter_order', $params->get('tfjfa_bacuerdo_initial_sort','a.ordering'), 'string');
		if (!in_array($order_col, $this->filter_fields))
		{
			$order_col = $params->get('tfjfa_bacuerdo_initial_sort','a.ordering');
		}

		$this->setState('list.ordering', $order_col);

		$list_order = $app->getUserStateFromRequest($this->context. '.filter_order_Dir', 'filter_order_Dir',  $params->get('tfjfa_bacuerdo_initial_direction','ASC'), 'cmd');
		if (!in_array(JString::strtoupper($list_order), array('ASC', 'DESC', '')))
		{
			$list_order =  $params->get('tfjfa_bacuerdo_initial_direction','ASC');
		}
		...
		if ($params->get('filter_tfjfa_bacuerdo_categories') AND implode(',', $params->get('filter_tfjfa_bacuerdo_categories'))  == true)
		{
			$selected_categories = $params->get('filter_tfjfa_bacuerdo_categories');
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.serialize($this->getState('filter.tfjfa_bacuerdo_id'));
		$id .= ':'.$this->getState('filter.tfjfa_bacuerdo_id.include');				
	...
	/**
	 * Get the main query for retrieving a list of tfjfa_bacuerdos subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		// Filter by a single or group of tfjfa_bacuerdos.
		$tfjfa_bacuerdo_id = $this->getState('filter.tfjfa_bacuerdo_id');
		if ($tfjfa_bacuerdo_id != '')
		{
			if (is_numeric($tfjfa_bacuerdo_id))
			{
				$type = $this->getState('filter.tfjfa_bacuerdo_id.include', true) ? '= ' : '<> ';
				$query->where($db->quoteName('a.id').' '.$type.(int) $tfjfa_bacuerdo_id);
			}
			else
			{
				if (is_array($tfjfa_bacuerdo_id))
				{
					JArrayHelper::toInteger($tfjfa_bacuerdo_id);
					$tfjfa_bacuerdo_id = implode(',', $tfjfa_bacuerdo_id);
					$type = $this->getState('filter.tfjfa_bacuerdo_id.include', true) ? 'IN' : 'NOT IN';
					$query->where($db->quoteName('a.id').' '.$type.' ('.$tfjfa_bacuerdo_id.')');
				}
			}
		}
		...
		if ((is_object($params)) AND ($params->get('show_tfjfa_bacuerdo_filter_field') != 'hide') AND ($filter = $this->getState('filter.search')))
		{
			...
			switch ($params->get('show_tfjfa_bacuerdo_filter_field'))
			{
	...
	/**
	 * Method to get a list of acuerdos fiscales del tfjfa.
	 *
	...
	public function getItems()
	{
		...
		if ($items = parent::getItems())
		{
			...
			foreach ($items as &$item)
			{
				...
				{
					$item->params = $tfjfa_bacuerdo_params;
				}
				else
				{
					...
					// get display date
					switch ($item->params->get('list_show_tfjfa_bacuerdo_date'))
					{
						default:
							$item->display_date = 0;
							break;
