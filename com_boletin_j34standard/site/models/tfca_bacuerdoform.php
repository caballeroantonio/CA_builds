...
require_once JPATH_ADMINISTRATOR.'/components/com_boletin/models/tfca_bacuerdo.php';

class BoletinModelTfca_bacuerdoForm extends BoletinModelTfca_bacuerdo
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_boletin.edit.tfca_bacuerdo';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_boletin.tfca_bacuerdo';
	/**
	...
	protected function populateState()
	{
		...
		$this->setState('tfca_bacuerdo.id', $pk);
	...
	*/
	public function getTable($type = 'Tfca_bacuerdos', $prefix = 'BoletinTable', $config = array())
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_boletin.edit.tfca_bacuerdo', 'tfca_bacuerdo', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	...
	/**
	 * Method to get tribunal federal de conciliación y arbitraje - boletín laboral data.
	 *
	 * @param	integer	$item_id	The id of the tfca_bacuerdo.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	tfca_bacuerdo item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('tfca_bacuerdo.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_boletin.edit.tfca_bacuerdo.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('tfca_bacuerdo.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onTfca_bacuerdoBeforeSave', array('com_boletin.tfca_bacuerdo', &$table, $is_new));
		...
		$dispatcher->trigger('onTfca_bacuerdoAfterSave', array('com_boletin.tfca_bacuerdo', $table, $is_new));

		$this->setState('tfca_bacuerdo.id', $table->id);
