...
require_once JPATH_ADMINISTRATOR.'/components/com_boletin/models/pjf_bacuerdo.php';

class BoletinModelPjf_bacuerdoForm extends BoletinModelPjf_bacuerdo
{
	/**
	 ...
	 * @var		string
	 */
	protected $context = 'com_boletin.edit.pjf_bacuerdo';
	/**
	 * Model typeAlias string. Used for version history.
	 *
	 * @var        string
	 */
	public $typeAlias = 'com_boletin.pjf_bacuerdo';
	/**
	...
	*/
	...
	{
		// Get the form.
		$form = $this->loadForm('com_boletin.edit.pjf_bacuerdo', 'pjf_bacuerdo', array('control' => 'jform', 'load_data' => $load_data));
		if (empty($form))
		{
			return false;
		}
	 ...
	 *
	 * @param	integer	$item_id	The id of the pjf_bacuerdo.
	 * @param	boolean		Get recursively item children - true or false
	 * @return	mixed	pjf_bacuerdo item data object on success, false on failure.
	 */
	public function getItem($item_id = null, $recursive = false)
	{
		
		$item_id = (int) (!empty($item_id)) ? $item_id : $this->getState('pjf_bacuerdo.id');
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_boletin.edit.pjf_bacuerdo.data', array());
	...
	public function save($data)
	{
		...
		$pk			= (!empty($data['id'])) ? $data['id'] : (int) $this->getState('pjf_bacuerdo.id');
		$is_new		= true;
		...
		$result = $dispatcher->trigger('onPjf_bacuerdoBeforeSave', array('com_boletin.pjf_bacuerdo', &$table, $is_new));
		...
		$dispatcher->trigger('onPjf_bacuerdoAfterSave', array('com_boletin.pjf_bacuerdo', $table, $is_new));

		$this->setState('pjf_bacuerdo.id', $table->id);
