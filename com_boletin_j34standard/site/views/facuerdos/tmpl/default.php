...
$layout		= $this->params->get('facuerdo_layout', 'default');
...
<div class="boletin facuerdos-list<?php echo $this->params->get('pageclass_sfx');?>">
	...
	<?php
		...
		$show_actions = false;
		if ($this->params->get('show_facuerdo_icons',1) >= 0) :
	...
	<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		<?php if (($this->params->get('show_facuerdo_filter_field') != '' AND $this->params->get('show_facuerdo_filter_field') != 'hide')) :?>
			<div class="filter-search">
                ...
                <div class="input-append">
					<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_BOLETIN_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_BOLETIN_'.$this->params->get('show_facuerdo_filter_field').'_FILTER_LABEL'); ?>" />
		...
		<?php if (empty($this->items)) : ?>
			...
			<p><?php echo JText::_('COM_BOLETIN_FACUERDOS_NO_ITEMS'); ?></p>
			<?php endif; ?>

		<?php else : ?>
		<div style="overflow-x:auto;">
			<table class="table table-striped" id="facuerdos" style="margin-bottom: 200px;">
			<?php if ($this->params->get('show_facuerdo_headings',1)) :?>
			<thead>
				<tr>
					...
					<?php if ($this->params->get('list_show_facuerdo_fecha_boletin',1)) : ?>
						<th class="list-fecha_boletin" id="tableOrderingfecha_boletin">
							<?php echo JTEXT::_('COM_BOLETIN_FACUERDOS_HEADING_FECHA_BOLETIN'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_facuerdo_region',1)) : ?>
						<th class="list-region" id="tableOrderingregion">
							<?php echo JTEXT::_('COM_BOLETIN_FACUERDOS_HEADING_REGION'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_facuerdo_instancia',1)) : ?>
						<th class="list-instancia" id="tableOrderinginstancia">
							<?php echo JTEXT::_('COM_BOLETIN_FACUERDOS_HEADING_INSTANCIA'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_facuerdo_expediente',1)) : ?>
						<th class="list-expediente" id="tableOrderingexpediente">
							<?php echo JTEXT::_('COM_BOLETIN_FACUERDOS_HEADING_EXPEDIENTE'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_facuerdo_actor',1)) : ?>
						<th class="list-actor" id="tableOrderingactor">
							<?php echo JTEXT::_('COM_BOLETIN_FACUERDOS_HEADING_ACTOR'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_facuerdo_parte_a_notificar',1)) : ?>
						<th class="list-parte_a_notificar" id="tableOrderingparte_a_notificar">
							<?php echo JTEXT::_('COM_BOLETIN_FACUERDOS_HEADING_PARTE_A_NOTIFICAR'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_facuerdo_auto_o_resolucion',1)) : ?>
						<th class="list-auto_o_resolucion" id="tableOrderingauto_o_resolucion">
							<?php echo JTEXT::_('COM_BOLETIN_FACUERDOS_HEADING_AUTO_O_RESOLUCION'); ?>
						</th>
					<?php endif; ?>	
					<?php if ($this->params->get('list_show_facuerdo_ordering',0)) : ?>
						<th width="10%">
					...
					<?php if ($this->params->get('list_show_facuerdo_date')) : ?>
						<td class="list-date">
							<time datetime="<?php echo JHtml::_('date', $item->display_date, 'c'); ?>">
								<?php echo JHtml::_('date',$item->display_date, $this->escape($this->params->get('facuerdo_date_format', JText::_('DATE_FORMAT_LC3')))); ?>
							</time>
						</td>
					<?php endif; ?>
					<?php if ($this->params->get('list_show_facuerdo_category',0)) : ?>
					...
					<?php if ($show_actions) : ?>
						<td class="list-actions">
							...
							<?php if ($params->get('show_facuerdo_print_icon')) : ?>
								<li class="print-icon">
										<?php echo JHtml::_('facuerdoicon.print_popup',  $item, $params); ?>
								</li>
							<?php endif; ?>

							<?php if ($params->get('show_facuerdo_email_icon')) : ?>
								<li class="email-icon">
										<?php echo JHtml::_('facuerdoicon.email',  $item, $params); ?>
								</li>
							<?php endif; ?>
                                </ul>
                            </div>
			...
			<?php if (($this->params->def('show_facuerdo_pagination', 2) == 1  OR ($this->params->get('show_facuerdo_pagination') == 2)) AND ($this->pagination->get('pages.total') > 1)) : ?>
		...
		<?php if ($this->params->get('show_facuerdo_add_link', 1)) : ?>
			<?php echo JHtml::_('facuerdoicon.create', $this->params); ?>
		<?php endif; ?>		
		<?php 
			if($user->id == 1){
				//JHtml::_('facuerdoicon.create', $this->params); 
	            echo '<span class="hasTooltip tip" title="Export"><a href="index.php?task=facuerdos.export" class="btn btn-primary"><span class="icon-download"></span>Export</a></span>';
			}
        ?>
	</form>
</div>

<?php if ($can_edit AND $params->get('save_history') AND $params->get('facuerdo_save_history')) : ?>
<script>
...
function show_collapsibleModal(item_id){
	...
	modalBody.prepend('<iframe class="iframe" src="index.php?option=com_boletin&task=facuerdo.showHistory&item_id='+item_id+'" name="titulo" height="450"></iframe>');
	return;
}
</script>
