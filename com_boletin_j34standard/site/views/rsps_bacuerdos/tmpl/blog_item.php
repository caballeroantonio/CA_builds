...
$layout		= $params->get('rsps_bacuerdo_layout', 'default');
...
<?php if (!$params->get('show_rsps_bacuerdo_readmore') 
		):?>
	<?php
...
<?php else : ?>
	...
	?>			
	<?php 
        //Optional link to let them register to see the whole rsps_bacuerdo.
		if ($params->get('show_rsps_bacuerdo_readmore')) :
				$link = JRoute::_(BoletinHelperRoute::getRsps_bacuerdoRoute($this->item->slug, 
																							$layout,									
																							$params->get('keep_rsps_bacuerdo_itemid')));
			?>
			<p class="readmore">
			...
			<?php
				...
				if ($item_params->get('rsps_bacuerdo_alternative_readmore') == null) :
						echo JText::sprintf('COM_BOLETIN_READ_MORE');
				else :
					echo $this->item->rsps_bacuerdo_alternative_readmore;
				endif;?>
			</a>
		</p>
	<?php endif; ?>
