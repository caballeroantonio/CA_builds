...
$layout		= $params->get('tfca_bacuerdo_layout', 'default');
...
<?php if (!$params->get('show_tfca_bacuerdo_readmore') 
		):?>
	<?php
...
<?php else : ?>
	...
	?>			
	<?php 
        //Optional link to let them register to see the whole tfca_bacuerdo.
		if ($params->get('show_tfca_bacuerdo_readmore')) :
				$link = JRoute::_(BoletinHelperRoute::getTfca_bacuerdoRoute($this->item->slug, 
																							$layout,									
																							$params->get('keep_tfca_bacuerdo_itemid')));
			?>
			<p class="readmore">
			...
			<?php
				...
				if ($item_params->get('tfca_bacuerdo_alternative_readmore') == null) :
						echo JText::sprintf('COM_BOLETIN_READ_MORE');
				else :
					echo $this->item->tfca_bacuerdo_alternative_readmore;
				endif;?>
			</a>
		</p>
	<?php endif; ?>
